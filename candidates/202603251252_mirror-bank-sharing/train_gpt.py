...
class Hyperparameters:
    ...
    mirror_tied_banks = bool(int(os.environ.get("MIRROR_TIED_BANKS", "1")))
...
def attention_core(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
...
def build_bank_map(num_layers: int, mirror_tied_banks: bool) -> list[int]:
    if not mirror_tied_banks:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.value_residual = value_residual
        self.mirror_tied_banks = mirror_tied_banks
        ...
        # Share only the heavy banked weights across mirror-symmetric layers; keep per-layer
        # norms, scales, residual mixes, and feature toggles untied.
        self.bank_map = build_bank_map(num_layers, mirror_tied_banks)
        self.num_bank_layers = max(self.bank_map) + 1 if self.bank_map else 0
        self.qo_bank = nn.Parameter(torch.empty(2 * self.num_bank_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.num_bank_layers, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(self.num_bank_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(self.num_bank_layers, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.num_bank_layers):
            nn.init.orthogonal_(self.qo_bank.data[i], gain=1.0)        # Q
            nn.init.zeros_(self.qo_bank.data[self.num_bank_layers + i])  # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[self.num_bank_layers + i], gain=1.0)  # V
            ...
            self.qo_bank.data[self.num_bank_layers + i].mul_(proj_scale)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            bank_i = self.bank_map[i]
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[bank_i], self.kv_bank[bank_i], self.kv_bank[self.num_bank_layers + bank_i],
                self.qo_bank[self.num_bank_layers + bank_i], self.mlp_up_bank[bank_i], self.mlp_down_bank[bank_i],
                v_embed=ve, v0=v0)
...
def eval_val_sliding_ttt(
    ...
    # Freeze first N blocks. With mirror-tied banks, any shared heavy-weight slot touched
    # by a frozen block must also stay frozen during TTT, even if an unfrozen mirrored
    # block reuses that bank slot.
    frozen_block_ids = set(range(min(args.ttt_freeze_blocks, len(base_model.blocks))))
    frozen_bank_ids: set[int] = set()
    if frozen_block_ids and getattr(base_model, "mirror_tied_banks", 0):
        bank_map = getattr(base_model, "bank_map", None)
        if bank_map is None:
            raise RuntimeError("mirror-tied TTT freeze requires bank_map on the base model")
        frozen_bank_ids = {bank_map[bi] for bi in frozen_block_ids}
    ttt_params = []
    ...
    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         ...
         f"shared_bank_slots_frozen={len(frozen_bank_ids)}")
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        loss.backward()
                        if frozen_bank_ids:
                            for bank_i in frozen_bank_ids:
                                if base_model.qo_bank.grad is not None:
                                    base_model.qo_bank.grad[bank_i].zero_()
                                    base_model.qo_bank.grad[base_model.num_bank_layers + bank_i].zero_()
                                if base_model.kv_bank.grad is not None:
                                    base_model.kv_bank.grad[bank_i].zero_()
                                    base_model.kv_bank.grad[base_model.num_bank_layers + bank_i].zero_()
                                if base_model.mlp_up_bank.grad is not None:
                                    base_model.mlp_up_bank.grad[bank_i].zero_()
                                if base_model.mlp_down_bank.grad is not None:
                                    base_model.mlp_down_bank.grad[bank_i].zero_()
                        if world_size > 1:
                            for p in ttt_params:
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int, mirror_tied_banks: bool) -> dict[str, Tensor]:
    """Convert 3D bank tensors into unique 2D bank-slot tensors for quantization."""
    out: dict[str, Tensor] = {}
    bank_map = build_bank_map(num_layers, mirror_tied_banks)
    num_bank_layers = max(bank_map) + 1 if bank_map else 0
    for name, tensor in sd.items():
        if name == "qo_bank":
            for bank_i in range(num_bank_layers):
                out[f"bank_layers.{bank_i}.attn.c_q.weight"] = tensor[bank_i]
                out[f"bank_layers.{bank_i}.attn.proj.weight"] = tensor[num_bank_layers + bank_i]
        elif name == "kv_bank":
            for bank_i in range(num_bank_layers):
                out[f"bank_layers.{bank_i}.attn.c_k.weight"] = tensor[bank_i]
                out[f"bank_layers.{bank_i}.attn.c_v.weight"] = tensor[num_bank_layers + bank_i]
        elif name == "mlp_up_bank":
            for bank_i in range(num_bank_layers):
                out[f"bank_layers.{bank_i}.mlp.fc.weight"] = tensor[bank_i]
        elif name == "mlp_down_bank":
            for bank_i in range(num_bank_layers):
                out[f"bank_layers.{bank_i}.mlp.proj.weight"] = tensor[bank_i]
        else:
            out[name] = tensor
    return out

def _rebank_state_dict(
    sd: dict[str, Tensor], num_layers: int, mirror_tied_banks: bool, template_sd: dict[str, Tensor]
) -> dict[str, Tensor]:
    """Convert individual 2D bank-slot tensors back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    bank_map = build_bank_map(num_layers, mirror_tied_banks)
    num_bank_layers = max(bank_map) + 1 if bank_map else 0
    # Reconstruct banks from individual weight keys
    qo_slices = [None] * (2 * num_bank_layers)
    kv_slices = [None] * (2 * num_bank_layers)
    up_slices = [None] * num_bank_layers
    down_slices = [None] * num_bank_layers
    consumed = set()
    for bank_i in range(num_bank_layers):
        qk = f"bank_layers.{bank_i}.attn.c_q.weight"
        if qk in sd:
            if qo_slices[bank_i] is None:
                qo_slices[bank_i] = sd[qk]
            consumed.add(qk)
        ok = f"bank_layers.{bank_i}.attn.proj.weight"
        if ok in sd:
            if qo_slices[num_bank_layers + bank_i] is None:
                qo_slices[num_bank_layers + bank_i] = sd[ok]
            consumed.add(ok)
        kk = f"bank_layers.{bank_i}.attn.c_k.weight"
        if kk in sd:
            if kv_slices[bank_i] is None:
                kv_slices[bank_i] = sd[kk]
            consumed.add(kk)
        vk = f"bank_layers.{bank_i}.attn.c_v.weight"
        if vk in sd:
            if kv_slices[num_bank_layers + bank_i] is None:
                kv_slices[num_bank_layers + bank_i] = sd[vk]
            consumed.add(vk)
        fk = f"bank_layers.{bank_i}.mlp.fc.weight"
        if fk in sd:
            if up_slices[bank_i] is None:
                up_slices[bank_i] = sd[fk]
            consumed.add(fk)
        dk = f"bank_layers.{bank_i}.mlp.proj.weight"
        if dk in sd:
            if down_slices[bank_i] is None:
                down_slices[bank_i] = sd[dk]
            consumed.add(dk)
    if any(t is None for t in qo_slices + kv_slices + up_slices + down_slices):
        raise KeyError("Missing bank slices while reconstructing mirrored parameter banks")
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mirror_tied_banks=args.mirror_tied_banks,
    ...
    log0(
        f"mirror_tied_banks:{args.mirror_tied_banks} unique_bank_layers:{base_model.num_bank_layers} "
        f"bank_map:{base_model.bank_map}"
    )
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers, args.mirror_tied_banks)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, args.mirror_tied_banks, sd_cpu)
    eval_model = GPT(
        ...
        xsa_last_n=args.xsa_last_n,
        rope_dims=args.rope_dims, ln_scale=args.ln_scale, mirror_tied_banks=args.mirror_tied_banks,
        dtg=args.dtg_enabled,
