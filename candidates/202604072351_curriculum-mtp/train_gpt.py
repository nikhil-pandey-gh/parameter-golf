...
class Hyperparameters:
    ...
    mtp_curriculum_start_frac = float(os.environ.get("MTP_CURRICULUM_START_FRAC", 0.15))
    mtp_curriculum_end_frac = float(os.environ.get("MTP_CURRICULUM_END_FRAC", 0.55))
...
def build_mtp_head_weights(args: Hyperparameters, progress: float, device: torch.device) -> Tensor:
    if args.mtp_num_heads <= 0:
        return torch.empty((0,), device=device, dtype=torch.float32)
    if not args.mtp_curriculum_enabled:
        return torch.ones((args.mtp_num_heads,), device=device, dtype=torch.float32)
    start = max(args.mtp_curriculum_start_frac, 0.0)
    end = min(max(args.mtp_curriculum_end_frac, start + 1e-6), 1.0)
    progress = min(max(progress, 0.0), 1.0)
    if progress <= start:
        return torch.zeros((args.mtp_num_heads,), device=device, dtype=torch.float32)
    if progress >= end:
        return torch.ones((args.mtp_num_heads,), device=device, dtype=torch.float32)
    span = (end - start) / max(args.mtp_num_heads, 1)
    phase = (progress - start) / max(span, 1e-6)
    idx = torch.arange(args.mtp_num_heads, device=device, dtype=torch.float32)
    return (phase - idx).clamp(0.0, 1.0)

def count_active_mtp_heads(args: Hyperparameters, progress: float) -> int:
    if args.mtp_num_heads <= 0:
        return 0
    if not args.mtp_curriculum_enabled:
        return args.mtp_num_heads
    start = max(args.mtp_curriculum_start_frac, 0.0)
    end = min(max(args.mtp_curriculum_end_frac, start + 1e-6), 1.0)
    progress = min(max(progress, 0.0), 1.0)
    if progress <= start:
    ...
    if progress >= end:
        return args.mtp_num_heads
    span = (end - start) / max(args.mtp_num_heads, 1)
    return min(args.mtp_num_heads, max(1, math.ceil((progress - start) / max(span, 1e-6))))
...
class GPT(nn.Module):
    def __init__(
        ...
        self._init_weights()
    @torch.no_grad()
    def set_mtp_head_weights(self, weights: Tensor) -> None:
        if weights.shape != self.mtp_head_weights.shape:
            raise ValueError(
                f"Expected mtp_head_weights shape {tuple(self.mtp_head_weights.shape)}, got {tuple(weights.shape)}"
            )
        self.mtp_head_weights.copy_(weights.to(device=self.mtp_head_weights.device, dtype=self.mtp_head_weights.dtype))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_active_heads: int | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            active_mtp_heads = self.mtp_num_heads if mtp_active_heads is None else min(max(mtp_active_heads, 0), self.mtp_num_heads)
            _, seqlen, dim = x.shape
            mtp_loss_sum = x.new_zeros(())
            mtp_weight_sum = x.new_zeros(())
            for k in range(active_mtp_heads):
                mtp_head = self.mtp_heads[k]
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
                ...
                head_weight = self.mtp_head_weights[k].to(dtype=mtp_loss_sum.dtype)
                mtp_loss_sum = mtp_loss_sum + head_weight * F.cross_entropy(mtp_logits.float(), mtp_targets, reduction="mean")
            ...
            ) * (mtp_weight_sum > 0).to(dtype=mtp_loss_sum.dtype)
        return main_loss
...
def smoke_test_main() -> None:
    ...
    batch_size = max(1, 128 // max(seq_len, 1))
    model = GPT(
    ...
    mtp_weights = build_mtp_head_weights(args, 0.0, device)
    model.set_mtp_head_weights(mtp_weights)
    active_mtp_heads = count_active_mtp_heads(args, 0.0)
    model.train()
    input_ids = torch.randint(args.vocab_size, (batch_size, seq_len), device=device)
    target_ids = torch.randint(args.vocab_size, (batch_size, seq_len), device=device)
    loss = model(input_ids, target_ids, active_mtp_heads)
    loss.backward()
    print(
        f"smoke_test_ok loss:{loss.item():.4f} batch:{batch_size} seq_len:{seq_len} "
        f"mtp_head_weights:{model.mtp_head_weights.tolist()}"
    )
...
def main() -> None:
    ...
    mtp_weights = build_mtp_head_weights(args, 0.0, device)
    base_model.set_mtp_head_weights(mtp_weights)
    active_mtp_heads = count_active_mtp_heads(args, 0.0)
    ...
    log0(
        f"mtp_curriculum_enabled:{args.mtp_curriculum_enabled} "
        f"start_frac:{args.mtp_curriculum_start_frac:.3f} end_frac:{args.mtp_curriculum_end_frac:.3f}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, active_mtp_heads)
    ...
    last_logged_active_mtp_heads = active_mtp_heads
    ...
    while True:
        ...
        mtp_progress = (
            elapsed_ms / max_wallclock_ms
            if max_wallclock_ms is not None and max_wallclock_ms > 0
            else step / max(args.iterations, 1)
        )
        mtp_weights = build_mtp_head_weights(args, mtp_progress, device)
        base_model.set_mtp_head_weights(mtp_weights)
        active_mtp_heads = count_active_mtp_heads(args, mtp_progress)
        if active_mtp_heads != last_logged_active_mtp_heads:
            log0(f"mtp_active_heads:{active_mtp_heads} progress:{mtp_progress:.3f}")
            last_logged_active_mtp_heads = active_mtp_heads
