...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_horizon_decay <= 0.0:
            raise ValueError(f"mtp_horizon_decay must be positive, got {mtp_horizon_decay}")
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads and self.mtp_init_from_main_head:
            ref_weight = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
            for head in self.mtp_heads:
                with torch.no_grad():
                    head.weight.copy_(ref_weight)
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_init_from_main_head:{args.mtp_init_from_main_head} mtp_params:{mtp_params}"
    )
    ...
    # Auxiliary MTP heads improve training efficiency but stay out of the exported artifact.
    ...
    eval_model = GPT(
        ...
        mtp_horizon_decay=args.mtp_horizon_decay, mtp_init_from_main_head=args.mtp_init_from_main_head,
