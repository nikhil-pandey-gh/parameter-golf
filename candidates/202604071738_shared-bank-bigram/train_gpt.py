...
class Hyperparameters:
    ...
    bank_layer_map = os.environ.get("BANK_LAYER_MAP", "")
...
def default_bank_layer_map(num_layers: int) -> list[int]:
    if num_layers <= 0:
        raise ValueError(f"num_layers must be positive, got {num_layers}")
    peak = (num_layers - 1) // 2
    forward = list(range(peak + 1))
    back_start = peak if num_layers % 2 == 0 else peak - 1
    backward = list(range(back_start, -1, -1)) if back_start >= 0 else []
    return forward + backward

def parse_bank_layer_map(spec: str, num_layers: int) -> list[int]:
    if not spec.strip():
        return default_bank_layer_map(num_layers)
    mapping = [int(item.strip()) for item in spec.split(",") if item.strip()]
    if len(mapping) != num_layers:
        raise ValueError(
            f"BANK_LAYER_MAP must provide exactly {num_layers} entries, got {len(mapping)}"
        )
    if min(mapping) < 0:
        raise ValueError(f"BANK_LAYER_MAP entries must be non-negative, got {mapping}")
    return mapping

def format_bank_layer_map(mapping: list[int]) -> str:
    return ",".join(str(item) for item in mapping)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bank_layer_map = parse_bank_layer_map(bank_layer_map, num_layers)
        self.num_bank_layers = max(self.bank_layer_map) + 1
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[bank_i], self.kv_bank[bank_i], self.kv_bank[n + bank_i],
                self.qo_bank[n + bank_i], self.mlp_up_bank[bank_i], self.mlp_down_bank[bank_i],
                v_embed=ve, v0=v0)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_layer_map=args.bank_layer_map,
    ...
    log0(
        "shared_banks:"
        f"logical_layers:{args.num_layers} bank_layers:{base_model.num_bank_layers} "
        f"bank_map:{','.join(str(i) for i in base_model.bank_layer_map)}"
    )
    ...
    export_meta = {
        "bank_layer_map": list(base_model.bank_layer_map),
        "num_bank_layers": base_model.num_bank_layers,
    }
    if excluded_mtp > 0:
        log0(f"export_excluding_mtp_params:{excluded_mtp}")
    if master_process:
        torch.save({"state_dict": export_sd, "meta": export_meta}, "final_model.pt")
    ...
    loaded_meta = quant_state.get("meta", {})
    loaded_bank_layer_map = loaded_meta.get("bank_layer_map", list(base_model.bank_layer_map))
    if len(loaded_bank_layer_map) != args.num_layers:
        raise ValueError(
            f"Serialized bank_layer_map length {len(loaded_bank_layer_map)} does not match "
            f"NUM_LAYERS={args.num_layers}"
        )
    loaded_bank_layer_spec = format_bank_layer_map(loaded_bank_layer_map)
    loaded_num_bank_layers = int(loaded_meta.get("num_bank_layers", max(loaded_bank_layer_map) + 1))
    deq_unbanked = dequantize_mixed_int6(quant_state["w"], quant_state["m"], unbanked_sd)
    # Re-bank the dequantized tensors
    deq_state = _rebank_state_dict(deq_unbanked, loaded_num_bank_layers, sd_cpu)
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, bank_layer_map=loaded_bank_layer_spec, model_dim=args.model_dim,
