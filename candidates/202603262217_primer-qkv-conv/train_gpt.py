...
class Hyperparameters:
    ...
    qkv_conv_kernel = int(os.environ.get("QKV_CONV_KERNEL", 3))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.q_conv: nn.Conv1d | None = None
        self.k_conv: nn.Conv1d | None = None
        self.v_conv: nn.Conv1d | None = None
        if qkv_conv_kernel > 1:
            self.q_conv = nn.Conv1d(dim, dim, kernel_size=qkv_conv_kernel, groups=dim, bias=False)
            self.k_conv = nn.Conv1d(kv_dim, kv_dim, kernel_size=qkv_conv_kernel, groups=kv_dim, bias=False)
            self.v_conv = nn.Conv1d(kv_dim, kv_dim, kernel_size=qkv_conv_kernel, groups=kv_dim, bias=False)
            nn.init.zeros_(self.q_conv.weight)
            nn.init.zeros_(self.k_conv.weight)
            nn.init.zeros_(self.v_conv.weight)
    def _apply_causal_dwconv(self, conv: nn.Conv1d, x: Tensor) -> Tensor:
        pad = conv.kernel_size[0] - 1
        x_t = F.pad(x.transpose(1, 2), (pad, 0))
        y = F.conv1d(
            x_t,
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        q = self.c_q(x)
        k = self.c_k(x)
        if self.q_conv is not None and self.k_conv is not None and self.v_conv is not None:
            q = q + self._apply_causal_dwconv(self.q_conv, q)
            k = k + self._apply_causal_dwconv(self.k_conv, k)
            v = v + self._apply_causal_dwconv(self.v_conv, v)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, qkv_conv_kernel)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        qkv_conv_kernel=args.qkv_conv_kernel,
    ).to(device).bfloat16()
    ...
    log0(f"qkv_conv_kernel:{args.qkv_conv_kernel}")
    log0(
    ...
    for m in eval_model.modules():
        if isinstance(m, (CastedLinear, nn.Conv1d)):
            m.float()
