...
class Hyperparameters:
    ...
    qat_project_every = int(os.environ.get("QAT_PROJECT_EVERY", 4))
    qat_project_percentiles = os.environ.get("QAT_PROJECT_PERCENTILES", "0.9990,0.9995,0.9999,0.99999,1.0")
...
def parse_percentiles(spec: str) -> tuple[float, ...]:
    ...
    for value in values:
        if not (0.0 < value <= 1.0):
            raise ValueError(f"QAT_PROJECT_PERCENTILES values must be in (0, 1], got {value}")
    return tuple(values)
def quantize_int6_per_row(
    ...
    t32 = t.float()
    clip_percentiles = tuple(percentiles) if percentiles is not None else (0.9990, 0.9995, 0.9999, 0.99999, 1.0)
    if t32.ndim == 2:
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, percentiles=percentiles)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    return out
def apply_quant_projection_(module: nn.Module, int6_cats: set[str], percentiles: tuple[float, ...]) -> int:
    projected = 0
    with torch.no_grad():
        for name, param in module.named_parameters():
            if param.ndim != 2 or not param.is_floating_point():
                continue
            if param.numel() <= 65536:
                continue
            if any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS):
                continue
            if _classify_param(name) not in int6_cats:
                continue
            q, s = quantize_int6_per_row(param.detach(), percentiles=percentiles)
            param.copy_((q.float() * s.float()[:, None]).to(param.dtype))
            projected += 1
    return projected
def run_smoke_test() -> None:
    torch.manual_seed(0)
    percentiles = parse_percentiles(Hyperparameters.qat_project_percentiles)
    model_kwargs = dict(
        vocab_size=128,
        num_layers=2,
        model_dim=320,
    ...
    input_ids = torch.randint(0, model_kwargs["vocab_size"], (2, 32), dtype=torch.long)
    target_ids = torch.randint(0, model_kwargs["vocab_size"], (2, 32), dtype=torch.long)
    model.train()
    train_loss = model(input_ids, target_ids)
    train_loss.backward()
    projected = apply_quant_projection_(model, {"mlp", "attn"}, percentiles)
    export_sd = {k: v for k, v in model.state_dict().items() if "mtp_heads" not in k}
    export_sd_cpu = {k: v.detach().cpu() for k, v in export_sd.items()}
    quant_result, quant_meta = mixed_quantize_int6(export_sd_cpu, {"mlp", "attn"}, percentiles=percentiles)
    deq_state = dequantize_mixed_int6(quant_result, quant_meta, export_sd_cpu)
    ...
    print(
        f"smoke_test:ok train_loss:{train_loss.item():.4f} "
        f"eval_loss:{eval_loss.item():.4f} projected_tensors:{projected}"
    )
def main() -> None:
    ...
    qat_projection_enabled = args.qat_enabled
    qat_project_percentiles = parse_percentiles(args.qat_project_percentiles)
    base_model = GPT(
    ...
    log0(
        f"qat_projection:enabled={int(qat_projection_enabled)} late_threshold:{args.late_qat_threshold} "
        f"project_every:{args.qat_project_every} percentiles:{qat_project_percentiles}"
    )
    ...
    while True:
        ...
        just_enabled_qat = False
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not qat_projection_enabled:
            qat_projection_enabled = True
            just_enabled_qat = True
        ...
        for opt in optimizers:
            opt.step()
        if qat_projection_enabled and (
            just_enabled_qat or (args.qat_project_every > 0 and (step + 1) % args.qat_project_every == 0)
        ):
            projected_tensors = apply_quant_projection_(base_model, {"mlp", "attn"}, qat_project_percentiles)
            if just_enabled_qat or step < 10 or (
                args.train_log_every > 0 and (step + 1) % args.train_log_every == 0
            ):
                log0(f"late_qat:projected step:{step + 1} tensors:{projected_tensors}")
        zero_grad_all()
        # EMA update
        with torch.no_grad():
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, percentiles=qat_project_percentiles)
