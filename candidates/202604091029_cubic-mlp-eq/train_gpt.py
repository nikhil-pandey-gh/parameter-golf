...
def _int6_proxy_mse(t: Tensor) -> float:
    q, s = quantize_int6_per_row(t)
    recon = _dequantize_qscale(q, s)
    return (t.float() - recon).pow(2).mean().item()

def _choose_mlp_eq_scale(up_w: Tensor, down_w: Tensor, max_scale: float) -> tuple[Tensor, float, float, float]:
    up32 = up_w.float()
    down32 = down_w.float()
    up_stat = up32.abs().amax(dim=1).clamp_min(1e-8)
    down_stat = down32.abs().amax(dim=0).clamp_min(1e-8)
    base = (down_stat / up_stat).pow(1.0 / 3.0)
    base = base / torch.exp(torch.log(base).mean())
    base = base.clamp(1.0 / max_scale, max_scale)
    base_err = _int6_proxy_mse(up32) + _int6_proxy_mse(down32)
    best_scale = torch.ones_like(base)
    best_strength = 0.0
    best_err = base_err
    for strength in (0.25, 0.5, 0.75, 1.0):
        scale = base.pow(strength)
        up_eq = up32 * scale[:, None]
        down_eq = down32 / scale.square()[None, :]
        err = _int6_proxy_mse(up_eq) + _int6_proxy_mse(down_eq)
        if err < best_err:
            best_scale, best_strength, best_err = scale, strength, err
    return best_scale, best_strength, base_err, best_err

def apply_mlp_cubic_equalization(
    state_dict: dict[str, Tensor],
    num_layers: int,
    max_scale: float,
) -> tuple[dict[str, Tensor], dict[int, tuple[float, float, float]]]:
    out = dict(state_dict)
    summary: dict[int, tuple[float, float, float]] = {}
    for layer_idx in range(num_layers):
        ...
        if up_name not in out or down_name not in out:
            continue
        up_w = out[up_name]
        down_w = out[down_name]
        if up_w.ndim != 2 or down_w.ndim != 2 or up_w.shape[0] != down_w.shape[1]:
            continue
        scale, strength, base_err, best_err = _choose_mlp_eq_scale(up_w, down_w, max_scale)
        if strength <= 0.0:
            continue
        scale = scale.to(dtype=up_w.dtype)
        out[up_name] = up_w * scale[:, None]
        out[down_name] = down_w / scale.square()[None, :].to(dtype=down_w.dtype)
        summary[layer_idx] = (strength, base_err, best_err)
    return out, summary
...
def main() -> None:
    ...
    if args.mlp_eq_enabled:
        quant_source_sd, mlp_eq_summary = apply_mlp_cubic_equalization(
            unbanked_sd, args.num_layers, args.mlp_eq_max_scale
        )
        if mlp_eq_summary:
            mean_gain = sum(1.0 - best_err / base_err for _, base_err, best_err in mlp_eq_summary.values()) / len(mlp_eq_summary)
            layer_notes = ", ".join(
                f"{layer_idx}:{strength:.2f}"
                for layer_idx, (strength, _, _) in sorted(mlp_eq_summary.items())
            )
            log0(
                f"mlp_eq:enabled layers:{len(mlp_eq_summary)}/{args.num_layers} "
                f"mean_proxy_gain:{mean_gain:.4f} strengths:{layer_notes}"
            )
        else:
            log0("mlp_eq:enabled layers:0")
