...
class Hyperparameters:
    ...
    awq_batch_seqs = int(os.environ.get("AWQ_BATCH_SEQS", 8))
    awq_clip_candidates = tuple(
        float(v)
        for v in os.environ.get("AWQ_CLIP_CANDIDATES", "0.9990,0.9995,0.9999,0.99999,1.0").split(",")
        if v
    )
...
def collect_activation_sq_means(
    ...
    if calibration_tokens <= 0 or batch_seqs <= 0 or seq_len <= 0:
        return {}
    batch_tokens = batch_seqs * seq_len
    num_steps = max(calibration_tokens // batch_tokens, 1)
    ...
    for module_name, module in model.named_modules():
        ...
        def make_hook(name: str):
            def hook(_module, inputs, _output):
                ...
                if flat.numel() == 0:
                    return
                stats[name]["sum_sq"] += flat.pow(2).sum(dim=0).cpu()
                stats[name]["count"] += float(flat.shape[0])
            return hook
    ...
    try:
        with torch.inference_mode():
            for _ in range(num_steps):
                tokens = stream.take(batch_tokens).to(device=device, dtype=torch.int64)
                input_ids = tokens.reshape(batch_seqs, seq_len)
    ...
    return {
        name: (accum["sum_sq"] / accum["count"]).to(dtype=torch.float32)
        for name, accum in stats.items()
        if float(accum["count"]) > 0.0
    }
def quantize_int6_per_row(
    t: Tensor,
    clip_candidates: tuple[float, ...] = (0.9990, 0.9995, 0.9999, 0.99999, 1.0),
    act_sq: Tensor | None = None,
    clip_range: int = 31,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        candidates = clip_candidates or (1.0,)
        best_q = None
        best_s = None
        best_err = None
        act_weight = None
        if act_sq is not None and act_sq.ndim == 1 and act_sq.numel() == t32.shape[1]:
            act_weight = act_sq.to(dtype=torch.float32).clamp_min(1.0e-8)[None, :]
        for pct in candidates:
            ...
            delta_sq = (t32 - recon).pow(2)
            row_err = (delta_sq * act_weight).mean(dim=1) if act_weight is not None else delta_sq.mean(dim=1)
            if best_err is None:
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            act_sq = activation_sq_means.get(name) if activation_sq_means is not None else None
            q, s = quantize_int6_per_row(t, clip_candidates=clip_candidates, act_sq=act_sq)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "activation_aware": act_sq is not None}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def main() -> None:
    ...
    log0(
        f"awq_enabled:{args.awq_enabled} awq_calibration_tokens:{args.awq_calibration_tokens} "
        f"awq_batch_seqs:{args.awq_batch_seqs}"
    )
    ...
    if master_process:
        activation_sq_means = None
        if args.awq_enabled and args.awq_calibration_tokens > 0:
            torch.cuda.synchronize()
            ...
            torch.cuda.synchronize()
            log0(
                f"awq_calibration modules:{len(activation_sq_means)} "
                f"tokens:{args.awq_calibration_tokens} "
                f"calibration_time:{1000.0 * (time.perf_counter() - t_awq):.0f}ms"
            )
        quant_result, quant_meta = mixed_quantize_int6(
            sd_cpu,
            {"mlp", "attn"},
            activation_sq_means=activation_sq_means,
