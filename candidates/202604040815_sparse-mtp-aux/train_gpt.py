...
class Hyperparameters:
    ...
    mtp_window_tokens = int(os.environ.get("MTP_WINDOW_TOKENS", 256))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_window_tokens = mtp_window_tokens
    ...
    def _mtp_source_start(self, valid_t: int) -> int:
        # Only supervise a tail slice so the auxiliary vocab heads stay cheap enough
        # to fit the repository's fixed 10-minute wallclock regime.
        if self.mtp_window_tokens <= 0 or self.mtp_window_tokens >= valid_t:
            return 0
        return valid_t - self.mtp_window_tokens
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if valid_t <= 0:
                    continue
                source_start = self._mtp_source_start(valid_t)
                mtp_hidden = x[:, source_start:valid_t:self.mtp_stride, :].reshape(-1, dim)
                mtp_targets = target_ids[:, source_start + k + 1 : seqlen:self.mtp_stride].reshape(-1)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_window_tokens=args.mtp_window_tokens,
    ...
    log0(
        ...
        f"mtp_window_tokens:{args.mtp_window_tokens} mtp_stride:{args.mtp_stride} mtp_params:{mtp_params}"
    )
