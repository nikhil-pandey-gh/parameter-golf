...
class Hyperparameters:
    ...
    learned_clip_qat = bool(int(os.environ.get("LEARNED_CLIP_QAT", "1")))
    ...
    learned_clip_threshold = float(
        os.environ.get("LEARNED_CLIP_THRESHOLD", os.environ.get("LATE_QAT_THRESHOLD", "0.15"))
    )
...
def keep_float_tensor(name: str, t: Tensor, passthrough_orig_dtypes: dict[str, str]) -> Tensor:
    ...
    return t
def _rowwise_scale_from_logclip(t32: Tensor, clip_log: Tensor, clip_range: int) -> Tensor:
    if t32.ndim != 2:
        raise ValueError("learned rowwise clipping requires a 2D tensor")
    clip_mult = torch.exp(clip_log.float()).clamp(0.25, 4.0)
    row_clip = (t32.abs().amax(dim=1).detach() * clip_mult).clamp_min(1.0 / clip_range)
    return (row_clip / clip_range).to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()
def quantize_per_row_with_logclip(t: Tensor, clip_log: Tensor, clip_range: int) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim != 2:
        raise ValueError("learned rowwise clipping requires a 2D tensor")
    scale = _rowwise_scale_from_logclip(t32, clip_log, clip_range)
    row_clip = clip_range * scale.float()
    clipped = torch.maximum(torch.minimum(t32, row_clip[:, None]), -row_clip[:, None])
    q = torch.clamp(torch.round(clipped / scale.float()[:, None]), -clip_range, clip_range).to(torch.int8).contiguous()
    return q, scale
def quantize_float_tensor(t: Tensor, clip_log: Tensor | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        if clip_log is not None:
            return quantize_per_row_with_logclip(t, clip_log, 127)
        clip_abs = (
...
def fake_quantize_with_logclip(t: Tensor, clip_log: Tensor, clip_range: int) -> Tensor:
    q, scale = quantize_per_row_with_logclip(t, clip_log, clip_range)
    recon = (q.float() * scale.float()[:, None]).to(dtype=t.dtype)
    return t + (recon - t).detach() + (recon - recon.detach())
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        v_w: Tensor,
        out_w: Tensor,
        q_clip_log: Tensor | None = None,
        k_clip_log: Tensor | None = None,
        v_clip_log: Tensor | None = None,
        out_clip_log: Tensor | None = None,
        bank_qat: bool = False,
        ...
        if bank_qat:
            if q_clip_log is None or k_clip_log is None or v_clip_log is None or out_clip_log is None:
                raise RuntimeError("bank_qat requires learned clip tensors for all banked attention weights")
            q_w = fake_quantize_with_logclip(q_w, q_clip_log, 31)
            k_w = fake_quantize_with_logclip(k_w, k_clip_log, 31)
            v_w = fake_quantize_with_logclip(v_w, v_clip_log, 31)
            out_w = fake_quantize_with_logclip(out_w, out_clip_log, 31)
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        up_w: Tensor,
        down_w: Tensor,
        up_clip_log: Tensor | None = None,
        down_clip_log: Tensor | None = None,
        bank_qat: bool = False,
    ) -> Tensor:
        if bank_qat:
            if up_clip_log is None or down_clip_log is None:
                raise RuntimeError("bank_qat requires learned clip tensors for all banked MLP weights")
            up_w = fake_quantize_with_logclip(up_w, up_clip_log, 31)
            down_w = fake_quantize_with_logclip(down_w, down_clip_log, 31)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        attn_out, raw_v = self.attn(
            ...
            v_w,
            out_w,
            q_clip_log=q_clip_log,
            k_clip_log=k_clip_log,
            v_clip_log=v_clip_log,
            out_clip_log=out_clip_log,
            bank_qat=bank_qat,
            v_embed=v_embed,
            v0=v0,
        )
        ...
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(
            ...
            up_w,
            down_w,
            up_clip_log=up_clip_log,
            down_clip_log=down_clip_log,
            bank_qat=bank_qat,
        )
...
class GPT(nn.Module):
    def __init__(
        ...
        self.learned_clip_qat_active = False
        ...
        self.qo_bank_clip_log = nn.Parameter(torch.zeros(2 * num_layers, model_dim, dtype=torch.float32))
        self.kv_bank_clip_log = nn.Parameter(torch.zeros(2 * num_layers, kv_dim, dtype=torch.float32))
        self.mlp_up_bank_clip_log = nn.Parameter(torch.zeros(num_layers, mlp_dim, dtype=torch.float32))
        self.mlp_down_bank_clip_log = nn.Parameter(torch.zeros(num_layers, model_dim, dtype=torch.float32))
        self.tok_emb_clip_log = nn.Parameter(torch.zeros(vocab_size, dtype=torch.float32))
        ...
        self.lm_head_clip_log = nn.Parameter(torch.zeros(vocab_size, dtype=torch.float32)) if self.lm_head is not None else None
    ...
    def set_learned_clip_qat(self, enabled: bool) -> None:
        self.learned_clip_qat_active = enabled
        CastedLinear._qat_enabled = enabled
    def _tied_embedding_weight(self) -> Tensor:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        tok_weight = self._tied_embedding_weight()
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        bank_qat = self.training and self.learned_clip_qat_active
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                self.qo_bank_clip_log[i], self.kv_bank_clip_log[i], self.kv_bank_clip_log[n + i],
                self.qo_bank_clip_log[n + i], self.mlp_up_bank_clip_log[i], self.mlp_down_bank_clip_log[i],
                bank_qat=bank_qat,
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                self.qo_bank_clip_log[bi], self.kv_bank_clip_log[bi], self.kv_bank_clip_log[n + bi],
                self.qo_bank_clip_log[n + bi], self.mlp_up_bank_clip_log[bi], self.mlp_down_bank_clip_log[bi],
                bank_qat=bank_qat,
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, clip_range: int = 31, clip_log: Tensor | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        if clip_log is not None:
            return quantize_per_row_with_logclip(t, clip_log, clip_range)
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    ...
    for name, tensor in sd.items():
        if name == "qo_bank":
            ...
            out[name] = tensor
    return out
def _unbank_learned_clip_logs(sd: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    n = num_layers
    if "qo_bank_clip_log" in sd:
        tensor = sd["qo_bank_clip_log"]
        for i in range(n):
            out[f"blocks.{i}.attn.c_q.weight"] = tensor[i]
            out[f"blocks.{i}.attn.proj.weight"] = tensor[n + i]
    if "kv_bank_clip_log" in sd:
        tensor = sd["kv_bank_clip_log"]
        for i in range(n):
            out[f"blocks.{i}.attn.c_k.weight"] = tensor[i]
            out[f"blocks.{i}.attn.c_v.weight"] = tensor[n + i]
    if "mlp_up_bank_clip_log" in sd:
        tensor = sd["mlp_up_bank_clip_log"]
        for i in range(n):
            out[f"blocks.{i}.mlp.fc.weight"] = tensor[i]
    if "mlp_down_bank_clip_log" in sd:
        tensor = sd["mlp_down_bank_clip_log"]
        for i in range(n):
            out[f"blocks.{i}.mlp.proj.weight"] = tensor[i]
    if "tok_emb_clip_log" in sd:
        out["tok_emb.weight"] = sd["tok_emb_clip_log"]
    if "lm_head_clip_log" in sd:
        out["lm_head.weight"] = sd["lm_head_clip_log"]
    return out
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    clip_logs: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        clip_log = clip_logs.get(name) if clip_logs is not None else None
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, clip_log=clip_log)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
            q, s = quantize_float_tensor(t, clip_log=clip_log)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    return result, meta
...
def main() -> None:
    ...
    base_model.qo_bank_clip_log.data = base_model.qo_bank_clip_log.data.float()
    base_model.kv_bank_clip_log.data = base_model.kv_bank_clip_log.data.float()
    base_model.mlp_up_bank_clip_log.data = base_model.mlp_up_bank_clip_log.data.float()
    base_model.mlp_down_bank_clip_log.data = base_model.mlp_down_bank_clip_log.data.float()
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
    restore_low_dim_params_to_fp32(base_model)
    base_model.set_learned_clip_qat(args.qat_enabled and args.learned_clip_qat)
    ...
    scalar_params.extend(
        [
            base_model.qo_bank_clip_log,
            base_model.kv_bank_clip_log,
            base_model.mlp_up_bank_clip_log,
            base_model.mlp_down_bank_clip_log,
            base_model.tok_emb_clip_log,
        ]
    )
    ...
    if base_model.lm_head_clip_log is not None:
        scalar_params.append(base_model.lm_head_clip_log)
    ...
    while True:
        ...
        if (
            args.learned_clip_qat
            and args.learned_clip_threshold > 0
            and scale < args.learned_clip_threshold
            and not base_model.learned_clip_qat_active
        ):
            base_model.set_learned_clip_qat(True)
            compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
            model = compiled_model
            log0(f"late_learned_clip_qat:enabled step:{step} scale:{scale:.4f}")
        zero_grad_all()
    ...
    export_sd = {
        ...
        if "mtp_heads" not in k and "clip_log" not in k
    }
    excluded_aux = sum(
        int(t.numel())
        for k, t in full_state_dict.items()
        if "mtp_heads" in k or "clip_log" in k
    )
    if excluded_aux > 0:
        log0(f"export_excluding_aux_params:{excluded_aux}")
    if master_process:
    ...
    clip_logs = (
        _unbank_learned_clip_logs({k: v.detach().cpu() for k, v in full_state_dict.items()}, args.num_layers)
        if args.learned_clip_qat and base_model.learned_clip_qat_active
        else None
    )
    if clip_logs is not None:
        log0("export_quantization:using learned rowwise clipping")
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, clip_logs=clip_logs)
    ...
    for name, tensor in checkpoint_sd.items():
        if "clip_log" in name:
