...
DEFAULT_QAT_CANDIDATE_PCTS = (0.9990, 0.9995, 0.9999, 0.99999, 1.0)

def parse_qat_candidate_pcts(raw: str) -> tuple[float, ...]:
    pcts = tuple(float(x) for x in raw.split(",") if x.strip())
    if not pcts:
        raise ValueError("QAT_CANDIDATE_PCTS must contain at least one percentile")
    return pcts
...
class Hyperparameters:
    ...
    qat_disable_compile = bool(int(os.environ.get("QAT_DISABLE_COMPILE", "1")))
    qat_candidate_pcts = parse_qat_candidate_pcts(
        os.environ.get("QAT_CANDIDATE_PCTS", ",".join(str(x) for x in DEFAULT_QAT_CANDIDATE_PCTS))
    )
...
class CastedLinear(nn.Linear):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.register_buffer("qat_scale", torch.empty(0, dtype=torch.float32), persistent=False)
    @torch.no_grad()
    def refresh_qat_scale(self, candidate_pcts: tuple[float, ...], clip_range: int = 31) -> None:
        if self.weight.ndim != 2:
            self.qat_scale = torch.empty(0, device=self.weight.device, dtype=torch.float32)
            return
...
def _classify_param(name: str) -> str:
    ...
    return "other"
@torch.no_grad()
def refresh_qat_scales(module: nn.Module, candidate_pcts: tuple[float, ...]) -> dict[str, Tensor]:
    cached: dict[str, Tensor] = {}
    for name, submodule in module.named_modules():
        if isinstance(submodule, CastedLinear):
            submodule.refresh_qat_scale(candidate_pcts)
            if submodule.qat_scale.numel() > 0:
                cached[name + ".weight"] = submodule.qat_scale.detach().cpu().clone()
    return cached
def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = 31,
    extra_scale: Tensor | None = None,
    candidate_pcts: tuple[float, ...] = DEFAULT_QAT_CANDIDATE_PCTS,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        if extra_scale is not None and extra_scale.ndim == 1 and extra_scale.numel() == t32.shape[0]:
            s = extra_scale.to(dtype=torch.float32, device=t32.device).clamp_min(1.0 / clip_range)
            q = torch.clamp(torch.round(t32 / s[:, None]), -clip_range, clip_range).to(torch.int8)
            recon = q.float() * s[:, None]
            err = (t32 - recon).pow(2).mean().item()
            best_q, best_s, best_err = q, s.to(torch.float16), err
        for pct in candidate_pcts:
            if pct < 1.0:
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    qat_scales: dict[str, Tensor] | None = None,
    candidate_pcts: tuple[float, ...] = DEFAULT_QAT_CANDIDATE_PCTS,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(
                t,
                extra_scale=None if qat_scales is None else qat_scales.get(name),
...
def main() -> None:
    ...
    qat_scale_cache: dict[str, Tensor] = {}
    using_eager_qat = False
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            qat_scale_cache = refresh_qat_scales(base_model, args.qat_candidate_pcts)
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
            if args.qat_disable_compile:
                model = DDP(base_model, device_ids=[local_rank], broadcast_buffers=False) if distributed else base_model
                using_eager_qat = True
                log0("late_qat:switched_to_eager_model")
        zero_grad_all()
        ...
        if CastedLinear._qat_enabled and args.qat_refresh_every > 0 and step % args.qat_refresh_every == 0:
            qat_scale_cache = refresh_qat_scales(base_model, args.qat_candidate_pcts)
            log0(f"late_qat:refreshed_scales step:{step}", console=False)
    ...
    diag_val_loss, diag_val_bpb = eval_val(
        args, base_model if using_eager_qat else compiled_model, rank, world_size, device, grad_accum_steps,
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu,
        {"mlp", "attn"},
        qat_scales=qat_scale_cache,
        candidate_pcts=args.qat_candidate_pcts,
    )
