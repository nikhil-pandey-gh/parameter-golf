...
class Hyperparameters:
    ...
    kv_head_schedule = os.environ.get("KV_HEAD_SCHEDULE", "")
    ...
    ve_dim = int(os.environ.get("VE_DIM", 192))
...
def parse_kv_head_schedule(
    num_layers: int,
    num_heads: int,
    default_num_kv_heads: int,
    raw_schedule: str,
) -> tuple[int, ...]:
    schedule = [int(piece.strip()) for piece in raw_schedule.split(",") if piece.strip()]
    if not schedule:
        schedule = [default_num_kv_heads]
    if len(schedule) == 1:
        schedule *= num_layers
    if len(schedule) != num_layers:
        raise ValueError(
            f"KV_HEAD_SCHEDULE must provide 1 or NUM_LAYERS={num_layers} entries, got {len(schedule)}"
        )
    for kv_heads in schedule:
        if kv_heads <= 0 or num_heads % kv_heads != 0:
            raise ValueError(
                f"Each KV head count must be positive and divide NUM_HEADS={num_heads}, got {schedule}"
            )
    return tuple(schedule)


def kv_bank_key(num_kv_heads: int) -> str:
    return f"h{num_kv_heads}"


DEFAULT_HYBRID_KV_HEAD_SCHEDULE = "1,1,1,1,1,2,2,4,4,4,4"


def build_kv_group_layout(
    kv_head_schedule: tuple[int, ...],
) -> tuple[dict[int, list[int]], list[tuple[str, int]]]:
    groups: dict[int, list[int]] = {}
    for layer_idx, kv_heads in enumerate(kv_head_schedule):
        groups.setdefault(kv_heads, []).append(layer_idx)
    ordered_groups = dict(sorted(groups.items()))
    layout: list[tuple[str, int]] = [("", 0)] * len(kv_head_schedule)
    for kv_heads, layer_ids in ordered_groups.items():
        bank_key = kv_bank_key(kv_heads)
        for slot, layer_idx in enumerate(layer_ids):
            layout[layer_idx] = (bank_key, slot)
    return ordered_groups, layout
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.value_residual and v0 is not None and v0.shape == v.shape:
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        kv_head_schedule: tuple[int, ...] | None = None,
    ):
        super().__init__()
        ...
        resolved_kv_head_schedule = kv_head_schedule or (num_kv_heads,) * num_layers
        if len(resolved_kv_head_schedule) != num_layers:
            raise ValueError(
                f"kv_head_schedule must match NUM_LAYERS={num_layers}, got {len(resolved_kv_head_schedule)}"
            )
        self.kv_head_schedule = tuple(int(kv_heads) for kv_heads in resolved_kv_head_schedule)
        for kv_heads in self.kv_head_schedule:
            if kv_heads <= 0 or num_heads % kv_heads != 0:
                raise ValueError(
                    f"Each KV head count must be positive and divide NUM_HEADS={num_heads}, got {self.kv_head_schedule}"
                )
        self.max_kv_heads = max(self.kv_head_schedule)
        self._ve_target_dim = self.max_kv_heads * head_dim
        self.kv_bank_groups, self.kv_bank_layout = build_kv_group_layout(self.kv_head_schedule)
        ...
        self.k_banks = nn.ParameterDict()
        self.v_banks = nn.ParameterDict()
        for kv_heads, layer_ids in self.kv_bank_groups.items():
            bank_key = kv_bank_key(kv_heads)
            kv_dim = kv_heads * head_dim
            self.k_banks[bank_key] = nn.Parameter(torch.empty(len(layer_ids), kv_dim, model_dim))
            self.v_banks[bank_key] = nn.Parameter(torch.empty(len(layer_ids), kv_dim, model_dim))
    ...
    def _init_weights(self) -> None:
        ...
        for bank in self.k_banks.values():
            for i in range(bank.size(0)):
                nn.init.orthogonal_(bank.data[i], gain=1.0)
        for bank in self.v_banks.values():
            for i in range(bank.size(0)):
                nn.init.orthogonal_(bank.data[i], gain=1.0)
    ...
    def _get_kv_weights(self, layer_idx: int) -> tuple[Tensor, Tensor]:
        bank_key, slot = self.kv_bank_layout[layer_idx]
        return self.k_banks[bank_key][slot], self.v_banks[bank_key][slot]

    def _layer_kv_dim(self, layer_idx: int) -> int:
        return self.kv_head_schedule[layer_idx] * self.head_dim
    ...
    def _get_ve(self, layer_idx: int, input_ids: Tensor, ve_cache: dict | None = None) -> Tensor | None:
        ...
        ve = ve_base * self.ve_layer_scales[ve_idx].to(dtype=ve_base.dtype)
        return ve[..., : self._layer_kv_dim(layer_idx)]
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            k_w, v_w = self._get_kv_weights(i)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], k_w, v_w,
        ...
        for i in range(self.num_decoder_layers):
            ...
            k_w, v_w = self._get_kv_weights(bi)
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], k_w, v_w,
...
def _unbank_state_dict(sd: dict[str, Tensor], kv_head_schedule: tuple[int, ...]) -> dict[str, Tensor]:
    """Convert 3D bank tensors into individual 2D tensors with standard names."""
    out: dict[str, Tensor] = {}
    n = len(kv_head_schedule)
    _, kv_layout = build_kv_group_layout(kv_head_schedule)
    for name, tensor in sd.items():
        if name == "qo_bank":
            for i in range(n):
                out[f"blocks.{i}.attn.c_q.weight"] = tensor[i]
                out[f"blocks.{i}.attn.proj.weight"] = tensor[n + i]
        elif name.startswith("k_banks.") or name.startswith("v_banks."):
            continue
            ...
            out[name] = tensor
    for i, (bank_key, slot) in enumerate(kv_layout):
        out[f"blocks.{i}.attn.c_k.weight"] = sd[f"k_banks.{bank_key}"][slot]
        out[f"blocks.{i}.attn.c_v.weight"] = sd[f"v_banks.{bank_key}"][slot]
    return out
...
def _rebank_state_dict(
    ...
    out: dict[str, Tensor] = {}
    n = len(kv_head_schedule)
    kv_groups, kv_layout = build_kv_group_layout(kv_head_schedule)
    # Reconstruct banks from individual weight keys
    qo_slices = [None] * (2 * n)
    k_slices = {kv_bank_key(kv_heads): [None] * len(layer_ids) for kv_heads, layer_ids in kv_groups.items()}
    v_slices = {kv_bank_key(kv_heads): [None] * len(layer_ids) for kv_heads, layer_ids in kv_groups.items()}
    up_slices = [None] * n
    down_slices = [None] * n
    consumed = set()
    for i in range(n):
        ...
        if ok in sd:
            qo_slices[n + i] = sd[ok]
            consumed.add(ok)
        bank_key, slot = kv_layout[i]
        kk = f"blocks.{i}.attn.c_k.weight"
        if kk in sd:
            k_slices[bank_key][slot] = sd[kk]
            consumed.add(kk)
        vk = f"blocks.{i}.attn.c_v.weight"
        if vk in sd:
            v_slices[bank_key][slot] = sd[vk]
            consumed.add(vk)
    ...
    for kv_heads, _layer_ids in kv_groups.items():
        bank_key = kv_bank_key(kv_heads)
        out[f"k_banks.{bank_key}"] = torch.stack(k_slices[bank_key]).to(dtype=template_sd[f"k_banks.{bank_key}"].dtype)
        out[f"v_banks.{bank_key}"] = torch.stack(v_slices[bank_key]).to(dtype=template_sd[f"v_banks.{bank_key}"].dtype)
    for name, tensor in sd.items():
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        force_quantize_small_attn = name.endswith(".attn.c_k.weight") or name.endswith(".attn.c_v.weight")
        if not t.is_floating_point() or (t.numel() <= 65536 and not force_quantize_small_attn):
...
def main() -> None:
    ...
    args = Hyperparameters()
    resolved_schedule = args.kv_head_schedule
    if not resolved_schedule and args.num_layers == 11 and args.num_heads == 8 and args.num_kv_heads == 4:
        resolved_schedule = DEFAULT_HYBRID_KV_HEAD_SCHEDULE
    kv_head_schedule = parse_kv_head_schedule(
        args.num_layers,
        args.num_heads,
        args.num_kv_heads,
        resolved_schedule,
    )
    kv_schedule_str = ",".join(str(kv_heads) for kv_heads in kv_head_schedule)
    ...
    base_model = GPT(
        ...
        kv_head_schedule=kv_head_schedule,
    ).to(device).bfloat16()
    # Banks stay FP32 (like CastedLinear weights), cast to BF16 in forward
    base_model.qo_bank.data = base_model.qo_bank.data.float()
    for bank in base_model.k_banks.values():
        bank.data = bank.data.float()
    for bank in base_model.v_banks.values():
        bank.data = bank.data.float()
    ...
    kv_param_count = sum(int(bank.numel()) for bank in base_model.k_banks.values()) + sum(
        int(bank.numel()) for bank in base_model.v_banks.values()
    )
    uniform_kv_param_count = (
        2
        * args.num_layers
        * args.num_kv_heads
        * (args.model_dim // args.num_heads)
        * args.model_dim
    )
    ...
    log0(
        f"attention_mode:hybrid_gqa num_heads:{args.num_heads} max_num_kv_heads:{args.num_kv_heads} "
        f"kv_schedule:{kv_schedule_str} kv_params:{kv_param_count} uniform_kv_params:{uniform_kv_param_count}"
    )
    log0(
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, kv_head_schedule)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, kv_head_schedule, sd_cpu)
    eval_model = GPT(
    ...
    for bank in eval_model.k_banks.values():
        bank.data = bank.data.float()
    for bank in eval_model.v_banks.values():
        bank.data = bank.data.float()
