...
class Hyperparameters:
    ...
    ttt_param_mode = os.environ.get("TTT_PARAM_MODE", "scalar")
...
def _ttt_should_adapt_param(name: str, p: Tensor, mode: str) -> bool:
    if mode == "all":
        return True
    if mode == "scalar":
        return p.ndim < 2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
    raise ValueError(f"Unsupported TTT_PARAM_MODE={mode!r}; expected 'all' or 'scalar'")
...
def eval_val_sliding_ttt(
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"freeze_blocks={args.ttt_freeze_blocks} param_mode={args.ttt_param_mode}")
    ...
    for name, p in base_model.named_parameters():
        ...
        for bi in frozen_block_ids:
            if f"blocks.{bi}." in name:
                freeze = True
                break
        if not freeze and not _ttt_should_adapt_param(name, p, args.ttt_param_mode):
            freeze = True
        if freeze:
    ...
    if not ttt_params:
        raise ValueError("TTT parameter selection produced an empty parameter set")

    log0(f"ttt_sliding:params mode={args.ttt_param_mode} unfrozen={sum(p.numel() for p in ttt_params)} "
