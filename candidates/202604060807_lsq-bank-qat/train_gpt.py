...
class Hyperparameters:
    ...
    bank_qat_threshold = float(os.environ.get("BANK_QAT_THRESHOLD", os.environ.get("LATE_QAT_THRESHOLD", "0.20")))
    bank_qat_scale_lr = float(os.environ.get("BANK_QAT_SCALE_LR", 0.002))
...
def choose_row_scales_from_percentiles(
    t: Tensor,
    clip_range: int = 31,
    ...
    for pct in percentiles:
        ...
        q = torch.clamp(torch.round(t32 / scale.unsqueeze(-1)), -clip_range, clip_range)
    ...
    if best_scale is None:
        raise RuntimeError("Failed to initialize bank QAT scales")
    return best_scale.to(dtype=torch.float32)


def lsq_fake_quantize_int6(t: Tensor, scale_param: Tensor, mix: Tensor, clip_range: int = 31) -> Tensor:
    scale = scale_param.clamp_min(1.0 / clip_range)
    grad_factor = 1.0 / math.sqrt(max(t.size(-1) * clip_range, 1))
    scale = grad_scale(scale, grad_factor).to(dtype=t.dtype)
    scale_view = scale.unsqueeze(-1)
    q = torch.clamp(round_ste(t / scale_view), -clip_range, clip_range)
    q_t = q * scale_view
    return torch.lerp(t, q_t, mix.to(dtype=t.dtype))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.qo_bank_scale = nn.Parameter(torch.full((2 * num_layers, model_dim), 1.0 / 31.0, dtype=torch.float32))
        self.kv_bank_scale = nn.Parameter(torch.full((2 * num_layers, kv_dim), 1.0 / 31.0, dtype=torch.float32))
        self.mlp_up_bank_scale = nn.Parameter(torch.full((num_layers, mlp_dim), 1.0 / 31.0, dtype=torch.float32))
        self.mlp_down_bank_scale = nn.Parameter(torch.full((num_layers, model_dim), 1.0 / 31.0, dtype=torch.float32))
        self.register_buffer("bank_qat_mix", torch.zeros((), dtype=torch.float32), persistent=False)
        self.register_buffer("bank_qat_active", torch.zeros((), dtype=torch.bool), persistent=False)
    ...
    def enable_bank_qat(self) -> None:
        if bool(self.bank_qat_active.item()):
            return
        with torch.no_grad():
            self.qo_bank_scale.copy_(choose_row_scales_from_percentiles(self.qo_bank.data))
            self.kv_bank_scale.copy_(choose_row_scales_from_percentiles(self.kv_bank.data))
            self.mlp_up_bank_scale.copy_(choose_row_scales_from_percentiles(self.mlp_up_bank.data))
            self.mlp_down_bank_scale.copy_(choose_row_scales_from_percentiles(self.mlp_down_bank.data))
            self.bank_qat_mix.fill_(1.0)
            self.bank_qat_active.fill_(True)

    def _bank_weight(self, bank: Tensor, scale_bank: Tensor, index: int) -> Tensor:
        mix = self.bank_qat_mix if self.training else self.bank_qat_mix.new_zeros(())
        return lsq_fake_quantize_int6(bank[index], scale_bank[index], mix)

    def export_bank_qat_scales(self) -> dict[str, Tensor]:
        if not bool(self.bank_qat_active.item()):
            return {}
        n = self.num_layers
        return {
            **{f"blocks.{i}.attn.c_q.weight": self.qo_bank_scale[i].detach().cpu() for i in range(n)},
            **{f"blocks.{i}.attn.proj.weight": self.qo_bank_scale[n + i].detach().cpu() for i in range(n)},
            **{f"blocks.{i}.attn.c_k.weight": self.kv_bank_scale[i].detach().cpu() for i in range(n)},
            **{f"blocks.{i}.attn.c_v.weight": self.kv_bank_scale[n + i].detach().cpu() for i in range(n)},
            **{f"blocks.{i}.mlp.fc.weight": self.mlp_up_bank_scale[i].detach().cpu() for i in range(n)},
            **{f"blocks.{i}.mlp.proj.weight": self.mlp_down_bank_scale[i].detach().cpu() for i in range(n)},
        }
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self._bank_weight(self.qo_bank, self.qo_bank_scale, i),
                self._bank_weight(self.kv_bank, self.kv_bank_scale, i),
                self._bank_weight(self.kv_bank, self.kv_bank_scale, n + i),
                self._bank_weight(self.qo_bank, self.qo_bank_scale, n + i),
                self._bank_weight(self.mlp_up_bank, self.mlp_up_bank_scale, i),
                self._bank_weight(self.mlp_down_bank, self.mlp_down_bank_scale, i),
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                self._bank_weight(self.qo_bank, self.qo_bank_scale, bi),
                self._bank_weight(self.kv_bank, self.kv_bank_scale, bi),
                self._bank_weight(self.kv_bank, self.kv_bank_scale, n + bi),
                self._bank_weight(self.qo_bank, self.qo_bank_scale, n + bi),
                self._bank_weight(self.mlp_up_bank, self.mlp_up_bank_scale, bi),
                self._bank_weight(self.mlp_down_bank, self.mlp_down_bank_scale, bi),
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def quantize_int6_with_scale(t: Tensor, scale: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    s = scale.float().clamp_min(1.0 / clip_range)
    if s.ndim == 0:
        q = torch.clamp(torch.round(t32 / s), -clip_range, clip_range).to(torch.int8)
        return q, s.to(dtype=torch.float16)
    view = s.view(s.shape[0], *([1] * (t32.ndim - 1)))
    q = torch.clamp(torch.round(t32 / view), -clip_range, clip_range).to(torch.int8)
    return q, s.to(dtype=torch.float16)
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            override = scale_overrides.get(name)
            if override is not None:
                q, s = quantize_int6_with_scale(t, override)
            else:
...
def main() -> None:
    ...
    qat_params = [
        base_model.qo_bank_scale,
        base_model.kv_bank_scale,
        base_model.mlp_up_bank_scale,
        base_model.mlp_down_bank_scale,
    ]
    if base_model.bigram is not None:
    ...
    if args.bank_qat_enabled:
        optimizer_qat = torch.optim.AdamW(
            [{"params": qat_params, "lr": args.bank_qat_scale_lr, "base_lr": args.bank_qat_scale_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=0.0,
            fused=True,
    ...
    if optimizer_qat is not None:
        optimizers.append(optimizer_qat)
    ...
    log0(
        f"bank_qat:enabled={args.bank_qat_enabled} threshold:{args.bank_qat_threshold:.3f} "
        f"scale_lr:{args.bank_qat_scale_lr:.4f}"
    )
    ...
    while True:
        ...
        if args.bank_qat_enabled and args.bank_qat_threshold > 0 and scale < args.bank_qat_threshold and not bool(base_model.bank_qat_active.item()):
            base_model.enable_bank_qat()
    ...
    export_sd = {k: v for k, v in full_state_dict.items() if "mtp_heads" not in k and not k.endswith("_bank_scale")}
    ...
    bank_qat_scale_overrides = base_model.export_bank_qat_scales()
    if bank_qat_scale_overrides:
        log0(f"bank_qat:export_overrides {len(bank_qat_scale_overrides)}")
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, scale_overrides=bank_qat_scale_overrides)
    ...
    load_info = eval_model.load_state_dict(deq_state, strict=False)
    expected_missing = {"qo_bank_scale", "kv_bank_scale", "mlp_up_bank_scale", "mlp_down_bank_scale"}
    if set(load_info.missing_keys) != expected_missing or load_info.unexpected_keys:
        raise RuntimeError(
            f"Unexpected eval_model load mismatch missing={load_info.missing_keys} "
