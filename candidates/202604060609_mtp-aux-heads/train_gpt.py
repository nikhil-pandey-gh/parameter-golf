...
def find_repo_root(script_path: Path) -> Path:
    for parent in script_path.parents:
        if (parent / "README.md").is_file() and (parent / "data").is_dir():
            return parent
    raise FileNotFoundError(f"Could not locate repository root from {script_path}")
...
try:
    ...
    _ATTN_BACKEND = "torch_sdpa"

    def flash_attn_3_func(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
        if not causal:
            raise ValueError("Fallback attention only supports causal mode")
        q_t = q.transpose(1, 2)
        k_t = k.transpose(1, 2)
        v_t = v.transpose(1, 2)
...
def main() -> None:
    ...
    if _ATTN_BACKEND == "flash_attn_3":
        sdp_flash, sdp_mem_efficient, sdp_math = True, False, False
    else:
        sdp_flash, sdp_mem_efficient, sdp_math = True, True, True
    ...
    log0(
        f"sdp_backends:cudnn=False flash={sdp_flash} "
        f"mem_efficient={sdp_mem_efficient} math={sdp_math}"
    )
