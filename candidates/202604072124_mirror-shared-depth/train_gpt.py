...
def logical_to_core_indices(num_layers: int, share_mirror: bool) -> list[int]:
    if not share_mirror:
        return list(range(num_layers))
    return [i if i <= (num_layers - 1) // 2 else num_layers - 1 - i for i in range(num_layers)]


def attention_forward(q: Tensor, k: Tensor, v: Tensor, *, causal: bool, enable_gqa: bool) -> Tensor:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None, *, use_xsa: bool = False) -> Tensor:
        ...
        y = attention_forward(q, k, v, causal=True, enable_gqa=(self.num_kv_heads != self.num_heads))
        if use_xsa:
...
class LogicalBlockAdapter(nn.Module):
    ...
    def forward(self, core: SharedBlockCore, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        attn_out = core.attn(attn_in, v_embed=v_embed, use_xsa=self.use_xsa)
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        mlp_in = core.mlp_norm(x_out) * self.ln_scale_factor
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * core.mlp(mlp_in)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.layer_to_core = logical_to_core_indices(num_layers, share_mirror)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.layer_adapters[i](self.shared_cores[self.layer_to_core[i]], x, x0, v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.layer_adapters[bi](self.shared_cores[self.layer_to_core[bi]], x, x0, v_embed=ve)
        x = self.final_norm(x)
...
def smoke_test(args: Hyperparameters) -> None:
    ...
    batch = 2
    seq = min(args.train_seq_len, 64)
    x = torch.randint(0, args.vocab_size, (batch, seq), device=device)
    y = torch.randint(0, args.vocab_size, (batch, seq), device=device)
    loss = model(x, y)
    loss.backward()
    n_params = sum(p.numel() for p in model.parameters())
    print(
        "smoke_test:ok "
        f"device={device.type} loss={loss.item():.4f} params={n_params} "
        f"shared_cores={len(model.shared_cores)} layer_map={model.layer_to_core}"
    )
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for training; use SMOKE_TEST=1 for a CPU-safe startup check")
    ...
    block_named_params = list(base_model.shared_cores.named_parameters()) + list(base_model.layer_adapters.named_parameters())
    matrix_params = [
        p
    ...
    log0(f"share_mirror:{args.share_mirror} shared_cores:{len(base_model.shared_cores)} layer_to_core:{base_model.layer_to_core}")
