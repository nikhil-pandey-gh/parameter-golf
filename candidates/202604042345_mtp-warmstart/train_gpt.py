...
class Hyperparameters:
    ...
    mtp_init_mode = os.environ.get("MTP_INIT_MODE", "embed")
...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_head_decay <= 0.0 or mtp_head_decay > 1.0:
            raise ValueError(f"mtp_head_decay must be in (0, 1], got {mtp_head_decay}")
        self.mtp_head_decay = mtp_head_decay
        if mtp_init_mode not in {"zero", "embed"}:
            raise ValueError(f"mtp_init_mode must be 'zero' or 'embed', got {mtp_init_mode!r}")
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads and self.mtp_init_mode == "embed":
            base_head = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
            if base_head is None:
                raise RuntimeError("lm_head is required when tie_embeddings=False")
            for mtp_head in self.mtp_heads:
                mtp_head.weight.data.copy_(base_head.data)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_weight_sum += head_weight
            ...
            if mtp_loss_weight_sum > 0.0:
                main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / mtp_loss_weight_sum)
        return main_loss
...
def main() -> None:
    ...
    # Train-only MTP heads stay on AdamW and are excluded from export later.
    ...
    log0(
        ...
        f"mtp_head_decay:{args.mtp_head_decay} mtp_init_mode:{args.mtp_init_mode} mtp_params:{mtp_params}"
    )
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_head_decay=args.mtp_head_decay, mtp_init_mode=args.mtp_init_mode,
