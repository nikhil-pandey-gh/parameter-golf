...
class Hyperparameters:
    ...
    mtp_curriculum_end = float(os.environ.get("MTP_CURRICULUM_END", 0.70))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_active_heads = mtp_num_heads
    ...
    def set_mtp_head_weights(
        ...
        if self.mtp_num_heads <= 0:
            return
        if not enabled:
            new_weights = torch.ones_like(self.mtp_head_weights)
        elif progress <= start_frac:
            new_weights = torch.zeros_like(self.mtp_head_weights)
        elif progress >= end_frac:
            new_weights = torch.ones_like(self.mtp_head_weights)
        else:
            alpha = (progress - start_frac) / max(end_frac - start_frac, 1e-6)
            slot = alpha * self.mtp_num_heads
            offsets = torch.arange(
                self.mtp_num_heads,
                device=self.mtp_head_weights.device,
                dtype=torch.float32,
            )
            new_weights = torch.clamp(slot - offsets, min=0.0, max=1.0)
        with torch.no_grad():
            self.mtp_head_weights.copy_(new_weights)
        self.mtp_active_heads = int((new_weights > 0).sum().item())
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            mtp_weights = self.mtp_head_weights
            mtp_weight_sum = mtp_weights.sum().clamp_min(1e-6)
            ...
            main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / mtp_weight_sum.to(dtype=mtp_loss_sum.dtype))
        return main_loss
...
def run_smoke_test() -> None:
    ...
    model.float()
    model.train()
    model.set_mtp_head_weights(0.5, 0.10, 0.70, True)
    x = torch.randint(0, 128, (2, 64), dtype=torch.int64)
    y = torch.randint(0, 128, (2, 64), dtype=torch.int64)
    with torch.no_grad():
        loss = model(x, y)
        logits = model.forward_logits(x[:, :16])
    weights = ",".join(f"{w:.2f}" for w in model.mtp_head_weights.tolist())
    print(
        f"smoke_test_ok loss:{loss.item():.4f} "
        f"logits_shape:{tuple(logits.shape)} mtp_weights:[{weights}] "
...
def main() -> None:
    ...
    base_model.set_mtp_head_weights(
        progress=0.0,
        start_frac=args.mtp_curriculum_start,
        end_frac=args.mtp_curriculum_end,
    ...
    # - bigram proj, VE proj -> AdamW scalar bucket
    # - mtp heads -> Adam (training-only auxiliary heads)
    matrix_params = [
    ...
    init_mtp_weights = ",".join(f"{w:.2f}" for w in base_model.mtp_head_weights.cpu().tolist())
    log0(
        f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} "
        f"mtp_params:{mtp_params} mtp_curriculum:{int(args.mtp_curriculum)} "
        f"mtp_start:{args.mtp_curriculum_start:.2f} mtp_end:{args.mtp_curriculum_end:.2f} "
        f"mtp_weights:[{init_mtp_weights}]"
    )
    ...
    while True:
        ...
        if max_wallclock_ms is not None and max_wallclock_ms > 0:
            step_progress = min(elapsed_ms / max_wallclock_ms, 1.0)
        else:
            step_progress = min(step / max(args.iterations - 1, 1), 1.0)
        base_model.set_mtp_head_weights(
        ...
        if should_log_train:
            mtp_weights = ",".join(f"{w:.2f}" for w in base_model.mtp_head_weights.cpu().tolist())
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms "
                f"mtp_active_heads:{base_model.mtp_active_heads} mtp_weights:[{mtp_weights}]"
            )
