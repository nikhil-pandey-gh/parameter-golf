...
class Hyperparameters:
    ...
    mtp_disable_frac = float(os.environ.get("MTP_DISABLE_FRAC", 0.60))
...
class GPT(nn.Module):
    ...
    def _mtp_loss(self, x: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for k, mtp_head in enumerate(self.mtp_heads):
            ...
            if valid_t <= 0:
                continue
            head_weight = self.mtp_head_decay ** k
            if head_weight <= 0.0:
                continue
        ...
        return mtp_loss_sum / mtp_weight_sum

    def forward_main_loss(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            main_loss = main_loss + self.mtp_loss_weight * self._mtp_loss(x, target_ids)
        return main_loss
...
def main() -> None:
    ...
    if args.mtp_head_decay < 0.0:
        raise ValueError(f"MTP_HEAD_DECAY must be non-negative, got {args.mtp_head_decay}")
    if not 0.0 <= args.mtp_disable_frac <= 1.0:
        raise ValueError(f"MTP_DISABLE_FRAC must be in [0, 1], got {args.mtp_disable_frac}")
    ...
    # Keep a pure-NTP compiled path so reverse-curriculum MTP can switch
    # objectives without relying on mutable flags inside one compiled graph.
    compiled_main_loss = torch.compile(base_model.forward_main_loss, dynamic=False, fullgraph=True)
    ...
    log0(
        ...
        f"mtp_head_decay:{args.mtp_head_decay} mtp_disable_frac:{args.mtp_disable_frac} "
    ...
    mtp_curriculum_enabled = args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0 and args.mtp_disable_frac > 0.0
    if mtp_curriculum_enabled:
        log0(
            f"mtp_curriculum:reverse active_until_frac:{args.mtp_disable_frac:.2f} "
            f"head_decay:{args.mtp_head_decay:.3f}"
        )
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            warmup_use_mtp = mtp_curriculum_enabled and (args.mtp_disable_frac >= 1.0 or warmup_step == 0)
            warmup_loss_fn = compiled_model if warmup_use_mtp else compiled_main_loss
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = warmup_loss_fn(x, y)
    ...
    mtp_active_prev = mtp_curriculum_enabled
    ...
    while True:
        ...
        progress = progress_frac(step, elapsed_ms)
        mtp_active = mtp_curriculum_enabled and progress < args.mtp_disable_frac
        if mtp_active != mtp_active_prev:
            log0(
                f"mtp_curriculum:{'enabled' if mtp_active else 'disabled'} "
                f"step:{step} progress:{progress:.3f} scale:{scale:.4f}"
            )
            mtp_active_prev = mtp_active
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        train_loss_fn = compiled_model if mtp_active else compiled_main_loss
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = train_loss_fn(x, y)
