...
class Hyperparameters:
    ...
    export_rotate_enabled = bool(int(os.environ.get("EXPORT_ROTATE_ENABLED", "1")))
    export_rotate_block_size = int(os.environ.get("EXPORT_ROTATE_BLOCK_SIZE", 512))
    export_rotate_categories = os.environ.get("EXPORT_ROTATE_CATEGORIES", "attn,mlp")
...
def _parse_export_rotate_categories(value: str) -> set[str]:
...
def _normalized_hadamard(block_size: int) -> Tensor:
    ...
    return h / math.sqrt(block_size)


@functools.lru_cache(maxsize=None)
def _signed_hadamard_signs(seed: int, block_size: int, block_index: int) -> Tensor:
    block_seed = zlib.crc32(f"{seed}:{block_size}:{block_index}".encode("utf-8"))
...
def _export_rotate_seed(name: str) -> int:
    return zlib.crc32(name.encode("utf-8"))


def _apply_signed_block_hadamard(t: Tensor, block_size: int, seed: int, inverse: bool = False) -> Tensor | None:
    if t.ndim != 2 or block_size <= 0 or not _is_power_of_two(block_size):
        return None
    cols = t.shape[-1]
    if cols < block_size or cols % block_size != 0:
        return None
    h = _normalized_hadamard(block_size).to(device=t.device)
    chunks = []
    for block_index, chunk in enumerate(t.float().split(block_size, dim=-1)):
        signs = _signed_hadamard_signs(seed, block_size, block_index).to(device=t.device)
        if inverse:
            rotated = torch.matmul(chunk, h)
            rotated = rotated * signs
        else:
            rotated = torch.matmul(chunk * signs, h)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    rotate_cats = _parse_export_rotate_categories(Hyperparameters.export_rotate_categories)
    rotate_block = Hyperparameters.export_rotate_block_size
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        rotate_meta = None
        if Hyperparameters.export_rotate_enabled and cat in rotate_cats:
            rotate_seed = _export_rotate_seed(name)
            rotated = _apply_signed_block_hadamard(t, rotate_block, rotate_seed)
            if rotated is not None:
                t = rotated
                rotate_meta = {
                    "kind": "signed_block_hadamard",
                    "block_size": rotate_block,
                    "seed": rotate_seed,
                }
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict):
            rotate_info = info.get("rotate")
            if isinstance(rotate_info, dict) and rotate_info.get("kind") == "signed_block_hadamard":
                rotated = _apply_signed_block_hadamard(
                    t,
                    int(rotate_info["block_size"]),
                    int(rotate_info["seed"]),
                    inverse=True,
                )
                if rotated is not None:
                    t = rotated
...
def main() -> None:
    ...
    rotated_tensor_count = sum(
        1
        for info in quant_meta.values()
        if isinstance(info, dict) and "rotate" in info
    )
    if master_process and rotated_tensor_count:
        log0(
            f"export_rotate:tensors={rotated_tensor_count} "
            f"block={args.export_rotate_block_size} "
            f"cats={args.export_rotate_categories}"
        )
