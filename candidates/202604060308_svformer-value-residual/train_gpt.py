...
class Hyperparameters:
    ...
    run_id = os.environ.get("RUN_ID", f"svformer_value_residual_{uuid.uuid4().hex[:8]}")
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.value_residual and v0 is not None:
            # SVFormer-style reuse of the first block's value stream adds a cheap
            # token-identity path without increasing the large parameter banks.
