...
class Hyperparameters:
    ...
    ternary_mlp_enabled = bool(int(os.environ.get("TERNARY_MLP_ENABLED", "1")))
    ternary_mlp_layers = os.environ.get("TERNARY_MLP_LAYERS", "3,4,5,6,7")
    ternary_qat_threshold = float(os.environ.get("TERNARY_QAT_THRESHOLD", 0.20))
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and w.ndim == 2:
            ...
            w = w + self.qat_strength.to(dtype=x.dtype) * (w_q - w).detach()
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, w, bias)


def set_castedlinear_qat_strength(module: nn.Module, value: float) -> None:
    with torch.no_grad():
        for submodule in module.modules():
            if isinstance(submodule, CastedLinear):
                submodule.qat_strength.fill_(value)


def fake_quantize_ternary_per_row(t: Tensor, strength: Tensor) -> Tensor:
    if t.ndim != 2:
        return t
    t32 = t.float()
    row_scale = t32.detach().abs().mean(dim=1, keepdim=True).clamp_min(1e-4)
    deq = torch.clamp(torch.round(t32 / row_scale), -1, 1) * row_scale
    out = t32 + strength.to(dtype=t32.dtype) * (deq - t32).detach()
    return out.to(dtype=t.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        ln_scale: bool = False,
        dtg: bool = False,
        ternary_mlp_enabled: bool = False,
        ternary_mlp_layers: str = "",
        ...
        self.ternary_mlp_layers = parse_int_set(ternary_mlp_layers) if ternary_mlp_enabled else set()
        self.register_buffer("ternary_qat_strength", torch.tensor(0.0, dtype=torch.float32), persistent=False)
    ...
    def _maybe_quantize_mlp_bank(self, layer_idx: int, weight: Tensor) -> Tensor:
        if not self.training or layer_idx not in self.ternary_mlp_layers:
            return weight
        return fake_quantize_ternary_per_row(weight, self.ternary_qat_strength)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            up_w = self._maybe_quantize_mlp_bank(i, self.mlp_up_bank[i])
            down_w = self._maybe_quantize_mlp_bank(i, self.mlp_down_bank[i])
        ...
        for i in range(self.num_decoder_layers):
            ...
            up_w = self._maybe_quantize_mlp_bank(bi, self.mlp_up_bank[bi])
            down_w = self._maybe_quantize_mlp_bank(bi, self.mlp_down_bank[bi])
...
TERNARY_SCALE_MULTIPLIERS = (0.75, 1.0, 1.25, 1.5, 2.0)


def quantize_ternary_per_row(t: Tensor) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim != 2:
        scale = torch.tensor(max(t32.abs().mean().item(), 1e-4), dtype=torch.float16)
        q = torch.clamp(torch.round(t32 / scale.float()), -1, 1).to(torch.int8)
        return q, scale
    base_scale = t32.abs().mean(dim=1).clamp_min(1e-4)
    best_q = None
    best_s = None
    best_err = None
    for mult in TERNARY_SCALE_MULTIPLIERS:
        scale = (base_scale * mult).to(torch.float32)
        q = torch.clamp(torch.round(t32 / scale[:, None]), -1, 1).to(torch.int8)
        row_err = (t32 - q.float() * scale[:, None]).pow(2).mean(dim=1)
        if best_err is None:
            best_q = q
            best_s = scale.to(torch.float16)
            best_err = row_err
            continue
        better = row_err < best_err
        best_err = torch.where(better, row_err, best_err)
        best_s = torch.where(better, scale.to(torch.float16), best_s)
        best_q = torch.where(better[:, None], q, best_q)
    if best_q is None or best_s is None:
        raise RuntimeError("Failed to quantize ternary tensor")
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], ternary_mlp_layers: set[int]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat == "mlp" and layer_idx is not None and layer_idx in ternary_mlp_layers and t.ndim >= 1:
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    smoke_layers = min(args.num_layers, 3)
    smoke_heads = min(args.num_heads, 4)
    smoke_dim = max(smoke_heads * 16, min(args.model_dim, 128))
    smoke_dim = (smoke_dim // smoke_heads) * smoke_heads
    smoke_kv_heads = min(args.num_kv_heads, smoke_heads)
    if smoke_heads % smoke_kv_heads != 0:
        smoke_kv_heads = 1
    head_dim = smoke_dim // smoke_heads
    smoke_rope_dims = min(args.rope_dims, head_dim)
    if smoke_rope_dims % 2 != 0:
        smoke_rope_dims -= 1
    smoke_ternary_layers = "1" if args.ternary_mlp_enabled and smoke_layers > 2 else ""
    model_kwargs = dict(
        vocab_size=args.vocab_size,
        num_layers=smoke_layers,
        model_dim=smoke_dim,
        num_heads=smoke_heads,
        num_kv_heads=smoke_kv_heads,
        ...
        xsa_last_n=min(args.xsa_last_n, smoke_layers),
        rope_dims=smoke_rope_dims,
        ln_scale=args.ln_scale,
        dtg=args.dtg_enabled,
        ternary_mlp_enabled=bool(smoke_ternary_layers),
        ternary_mlp_layers=smoke_ternary_layers,
        ve_enabled=False,
    ...
    smoke_model = GPT(**model_kwargs).float()
    restore_low_dim_params_to_fp32(smoke_model)
    set_castedlinear_qat_strength(smoke_model, 1.0)
    if smoke_model.ternary_mlp_layers:
        smoke_model.ternary_qat_strength.fill_(1.0)
    x = torch.randint(args.vocab_size, (2, 32), dtype=torch.int64)
    y = torch.randint(args.vocab_size, (2, 32), dtype=torch.int64)
    smoke_model.train()
    loss = smoke_model(x, y)
    loss.backward()
    state_dict = {k: v.detach().cpu() for k, v in smoke_model.state_dict().items()}
    unbanked = _unbank_state_dict(state_dict, smoke_layers)
    quant_result, quant_meta = mixed_quantize_int6(unbanked, {"mlp", "attn"}, smoke_model.ternary_mlp_layers)
    deq_unbanked = dequantize_mixed_int6(quant_result, quant_meta, unbanked)
    deq_state = _rebank_state_dict(deq_unbanked, smoke_layers, state_dict)
    roundtrip_model = GPT(**model_kwargs).float()
    roundtrip_model.load_state_dict(deq_state, strict=True)
    roundtrip_model.eval()
    with torch.no_grad():
        roundtrip_loss = roundtrip_model(x, y)
    print(
        f"smoke_test:ok layers:{smoke_layers} dim:{smoke_dim} "
        f"loss:{loss.item():.4f} roundtrip_loss:{roundtrip_loss.item():.4f}"
    )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        ln_scale=args.ln_scale,
        dtg=args.dtg_enabled,
        ternary_mlp_enabled=args.ternary_mlp_enabled,
        ternary_mlp_layers=args.ternary_mlp_layers,
    ...
    set_castedlinear_qat_strength(base_model, 1.0 if args.qat_enabled else 0.0)
    if args.ternary_mlp_enabled and args.qat_enabled:
        base_model.ternary_qat_strength.fill_(1.0)
    ...
    ema_decay = 0.997
    casted_qat_active = bool(args.qat_enabled)
    ternary_qat_active = bool(args.ternary_mlp_enabled and args.qat_enabled)
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not casted_qat_active:
            set_castedlinear_qat_strength(base_model, 1.0)
            casted_qat_active = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        if args.ternary_mlp_enabled and args.ternary_qat_threshold > 0 and scale < args.ternary_qat_threshold and not ternary_qat_active:
            base_model.ternary_qat_strength.fill_(1.0)
            ternary_qat_active = True
            log0(f"ternary_mlp_qat:enabled step:{step} scale:{scale:.4f} layers:{sorted(base_model.ternary_mlp_layers)}")
        zero_grad_all()
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, base_model.ternary_mlp_layers)
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
    quant_raw = quant_buf.getvalue()
    quant_blob = lzma.compress(quant_raw, preset=6)
    if master_process:
        with open("final_model.mixq.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model ternary-int6-int8+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size ternary-int6-int8+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    with open("final_model.mixq.ptz", "rb") as f:
