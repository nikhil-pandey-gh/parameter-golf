...
class Hyperparameters:
    ...
    mtp_warmup_frac = float(os.environ.get("MTP_WARMUP_FRAC", 0.08))
    mtp_decay_start_frac = float(os.environ.get("MTP_DECAY_START_FRAC", 0.65))
...
def mtp_loss_weight_for_progress(
    progress: float,
    peak_weight: float,
    warmup_frac: float,
    ...
    if warmup_frac > 0.0 and progress < warmup_frac:
        return peak_weight * (progress / warmup_frac)
    if progress <= decay_start_frac:
        return peak_weight
    if decay_start_frac >= 1.0:
        return final_weight
    tail = (progress - decay_start_frac) / max(1.0 - decay_start_frac, 1e-9)
    return peak_weight + (final_weight - peak_weight) * min(max(tail, 0.0), 1.0)
...
class GPT(nn.Module):
    ...
    def set_mtp_loss_weight(self, weight: float) -> None:
        self.mtp_loss_weight_buffer.fill_(float(weight))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = self.mtp_loss_weight_buffer.to(device=main_loss.device, dtype=main_loss.dtype)
...
def main() -> None:
    ...
    if args.mtp_final_loss_weight < 0.0:
        raise ValueError(
            f"MTP_FINAL_LOSS_WEIGHT must be non-negative, got {args.mtp_final_loss_weight}"
        )
    if not 0.0 <= args.mtp_warmup_frac <= 1.0:
        raise ValueError(f"MTP_WARMUP_FRAC must be in [0, 1], got {args.mtp_warmup_frac}")
    if not 0.0 <= args.mtp_decay_start_frac <= 1.0:
        raise ValueError(
            f"MTP_DECAY_START_FRAC must be in [0, 1], got {args.mtp_decay_start_frac}"
        )
    if args.mtp_warmup_frac > args.mtp_decay_start_frac:
        raise ValueError(
            "MTP_WARMUP_FRAC must be <= MTP_DECAY_START_FRAC, got "
            f"{args.mtp_warmup_frac} > {args.mtp_decay_start_frac}"
        )
    if max(args.mtp_loss_weight, args.mtp_final_loss_weight) <= 0.0:
    ...
    base_model.set_mtp_loss_weight(args.mtp_loss_weight)
    ...
    # - bigram proj, mtp heads, VE proj -> AdamW alongside the scalar group
    matrix_params = [
    ...
    log0(
        f"mtp_num_heads:{args.mtp_num_heads} mtp_peak_loss_weight:{args.mtp_loss_weight} "
        f"mtp_warmup_frac:{args.mtp_warmup_frac} mtp_decay_start_frac:{args.mtp_decay_start_frac} "
        f"mtp_final_loss_weight:{args.mtp_final_loss_weight} mtp_params:{mtp_params}"
    )
    ...
    def training_progress(step: int, elapsed_ms: float) -> float:
        step_progress = min(max(step / max(args.iterations, 1), 0.0), 1.0)
        if max_wallclock_ms is None:
            return step_progress
        wallclock_progress = min(max(elapsed_ms / max_wallclock_ms, 0.0), 1.0)
    ...
    current_mtp_weight = args.mtp_loss_weight
    ...
    while True:
        ...
        current_mtp_weight = mtp_loss_weight_for_progress(
            progress,
            args.mtp_loss_weight,
            args.mtp_warmup_frac,
            args.mtp_decay_start_frac,
            args.mtp_final_loss_weight,
        )
        base_model.set_mtp_loss_weight(current_mtp_weight)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_loss_weight:{current_mtp_weight:.4f}"
            )
