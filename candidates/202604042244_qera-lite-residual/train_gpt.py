...
class Hyperparameters:
    ...
    qera_rank = int(os.environ.get("QERA_RANK", 2))
    qera_top_layers = int(os.environ.get("QERA_TOP_LAYERS", 3))
...
def _compute_qera_factors(t: Tensor, q: Tensor, s: Tensor, rank: int) -> tuple[Tensor, Tensor] | None:
    if rank <= 0 or t.ndim != 2:
        return None
    residual = t.float() - _dequantize_row_scaled(q, s)
    max_rank = min(rank, residual.shape[0], residual.shape[1])
    if max_rank <= 0:
        return None
    u, sigma, vh = torch.linalg.svd(residual, full_matrices=False)
    left = (u[:, :max_rank] * sigma[:max_rank]).to(torch.float16).contiguous()
    right = vh[:max_rank, :].to(torch.float16).contiguous()
    return left, right
def mixed_quantize_int6(
    state_dict: dict[str, Tensor],
    int6_cats: set[str],
    qera_rank: int = 0,
    qera_top_layers: int = 0,
):
    num_layers_total = max(
        (int(k.split(".")[1]) for k in state_dict if k.startswith("blocks.")),
        default=0,
    ) + 1
    qera_layers = set(range(max(0, num_layers_total - qera_top_layers), num_layers_total)) if qera_rank > 0 else set()
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            if layer_idx in qera_layers:
                qera_factors = _compute_qera_factors(t, q, s, qera_rank)
                if qera_factors is not None:
                    left, right = qera_factors
                    result[name + ".qera_left"] = left
                    result[name + ".qera_right"] = right
                    meta[name] = {"type": "int6_qera", "rank": left.shape[1]}
                    continue
            meta[name] = {"type": "int6"}
        else:
            q, s = quantize_float_tensor(t)
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("type") == "int6_qera":
            t = t + result[name + ".qera_left"].float() @ result[name + ".qera_right"].float()
...
def main() -> None:
    ...
    log0(f"qera_rank:{args.qera_rank} qera_top_layers:{args.qera_top_layers}")
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu,
        {"mlp", "attn"},
        qera_rank=args.qera_rank,
        qera_top_layers=args.qera_top_layers,
    )
    qera_mats = sum(
        1 for info in quant_meta.values()
        if isinstance(info, dict) and info.get("type") == "int6_qera"
    )
    log0(f"qera_residual_mats:{qera_mats}")
