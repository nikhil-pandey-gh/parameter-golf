...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_loss_weight: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and mtp_loss_weight is not None:
            mtp_loss_weight = mtp_loss_weight.to(device=x.device, dtype=torch.float32)
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + mtp_loss_weight * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_schedule:lr_scale_to_zero mtp_params:{mtp_params}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        model.train()
        warmup_mtp_loss_weight = torch.tensor(args.mtp_loss_weight, device=device, dtype=torch.float32)
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, warmup_mtp_loss_weight)
    ...
    while True:
        ...
        # Fade the auxiliary MTP objective out alongside LR warmdown to reduce train/eval mismatch.
        mtp_loss_weight = None
        if args.mtp_num_heads > 0:
            scheduled_mtp_loss_weight = args.mtp_loss_weight * scale
            if scheduled_mtp_loss_weight > 0.0:
                mtp_loss_weight = torch.tensor(scheduled_mtp_loss_weight, device=device, dtype=torch.float32)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_loss_weight)
