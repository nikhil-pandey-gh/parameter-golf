...
class Hyperparameters:
    ...
    mtp_distill_weight = float(os.environ.get("MTP_DISTILL_WEIGHT", 0.25))
    ...
    cpu_smoke_test_seq_len = int(os.environ.get("CPU_SMOKE_TEST_SEQ_LEN", 64))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if valid_t <= 0:
                    continue
                mtp_hidden = x[:, :valid_t, :]
                mtp_targets = target_ids[:, k + 1 :]
                mtp_logits_proj = mtp_head(mtp_hidden.reshape(-1, dim)).reshape(bsz, valid_t, -1)
                ...
                if self.mtp_distill_weight > 0.0:
                    # Cheap shifted logit-matching keeps the auxiliary head aligned with
                    # the future main-head distribution without adding export-time params.
                    mtp_loss = mtp_loss + self.mtp_distill_weight * F.mse_loss(
                        mtp_logits.float(),
                        teacher_logits[:, k + 1 :, :].float(),
                    )
...
def run_cpu_smoke(args: Hyperparameters) -> None:
    ...
    inputs = torch.randint(args.vocab_size, (1, smoke_seq_len), dtype=torch.int64)
    targets = torch.randint(args.vocab_size, (1, smoke_seq_len), dtype=torch.int64)
    loss = model(inputs, targets)
    loss.backward()
    print(
        f"cpu_smoke_ok seq_len:{smoke_seq_len} loss:{loss.item():.4f} "
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        if bool(int(os.environ.get("CPU_SMOKE_TEST", "0"))):
            ...
            return
        raise RuntimeError(
            "CUDA is required for training/eval; set CPU_SMOKE_TEST=1 for a no-data CPU sanity check"
        )
    ...
    log0(
        ...
        f"mtp_distill_weight:{args.mtp_distill_weight} mtp_lr:{args.mtp_lr} mtp_params:{mtp_params}"
    )
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_distill_weight=0.0,
