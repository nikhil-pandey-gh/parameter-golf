...
class Hyperparameters:
    ...
    value_fp16_ve = bool(int(os.environ.get("VALUE_FP16_VE", "1")))
    value_fp16_last_kv_layers = int(os.environ.get("VALUE_FP16_LAST_KV_LAYERS", 1))
    extra_fp16_export_patterns = tuple(
        pattern.strip()
        for pattern in os.environ.get("EXTRA_FP16_EXPORT_PATTERNS", "").split(",")
        if pattern.strip()
    )
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def build_fp16_export_patterns(
    ...
    if keep_ve:
        patterns.append("ve_shared.embed.weight")
    for layer_idx in range(max(0, num_layers_total - max(last_kv_layers, 0)), num_layers_total):
        patterns.extend((f"blocks.{layer_idx}.attn.c_k.weight", f"blocks.{layer_idx}.attn.c_v.weight"))
...
def main() -> None:
    ...
    fp16_export_patterns = build_fp16_export_patterns(
        num_layers_total=args.num_layers,
        keep_ve=args.value_fp16_ve,
        last_kv_layers=args.value_fp16_last_kv_layers,
        extra_patterns=args.extra_fp16_export_patterns,
    )
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"export_fp16_patterns:{','.join(fp16_export_patterns) if fp16_export_patterns else 'none'}")
    log0(f"flash_attn_3_available:{flash_attn_3_func is not None}")
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, fp16_export_patterns)
