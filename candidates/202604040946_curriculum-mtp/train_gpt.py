...
class Hyperparameters:
    ...
    mtp_head_gap_frac = float(os.environ.get("MTP_HEAD_GAP_FRAC", 0.20))
    mtp_ramp_frac = float(os.environ.get("MTP_RAMP_FRAC", 0.18))
    mtp_horizon_decay = float(os.environ.get("MTP_HORIZON_DECAY", 0.5))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if flash_attn_3_func is not None and q.is_cuda:
            ...
            att = torch.matmul(q_sdpa.float(), k_sdpa.float().transpose(-2, -1)) * (1.0 / math.sqrt(self.head_dim))
            causal_mask = torch.ones((seqlen, seqlen), device=x.device, dtype=torch.bool).triu(1)
            att = att.masked_fill(causal_mask, float("-inf"))
            att = torch.softmax(att, dim=-1).to(dtype=q_sdpa.dtype)
            y = torch.matmul(att, v_sdpa).transpose(1, 2)
        if self.use_xsa:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_horizon_decay = mtp_horizon_decay
        self.mtp_weight_norm = sum(self.mtp_horizon_decay ** k for k in range(mtp_num_heads)) if mtp_num_heads > 0 else 1.0
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_weights: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0 and mtp_weights is not None:
            if mtp_weights.numel() != self.mtp_num_heads:
                raise ValueError(
                    f"mtp_weights must have {self.mtp_num_heads} entries, got {mtp_weights.numel()}"
                )
            mtp_weights = mtp_weights.to(device=x.device, dtype=torch.float32)
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                head_loss = F.cross_entropy(mtp_logits.float(), mtp_targets, reduction="mean")
                mtp_loss_sum = mtp_loss_sum + mtp_weights[k].to(dtype=head_loss.dtype) * head_loss
            main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / self.mtp_weight_norm)
        return main_loss
...
def training_progress(step: int, elapsed_ms: float, iterations: int, max_wallclock_ms: float | None) -> float:
...
def build_mtp_weights(args: Hyperparameters, progress: float, lr_scale: float, device: torch.device) -> Tensor | None:
    if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
        return None
    weights = torch.zeros((args.mtp_num_heads,), device=device, dtype=torch.float32)
    ramp = max(args.mtp_ramp_frac, 1e-6)
    any_active = False
    for head_idx in range(args.mtp_num_heads):
        head_start = args.mtp_start_frac + head_idx * args.mtp_head_gap_frac
        head_progress = min(max((progress - head_start) / ramp, 0.0), 1.0)
        any_active = any_active or head_progress > 0.0
        weights[head_idx] = (args.mtp_horizon_decay ** head_idx) * head_progress * lr_scale
    return weights if any_active and lr_scale > 0.0 else None
...
def main() -> None:
    ...
    scalar_params.extend(base_model.mtp_heads.parameters())
    ...
    log0(
        " ".join(
            (
                f"mtp_num_heads:{args.mtp_num_heads}",
                f"mtp_loss_weight:{args.mtp_loss_weight}",
                f"mtp_start_frac:{args.mtp_start_frac}",
                f"mtp_head_gap_frac:{args.mtp_head_gap_frac}",
                f"mtp_ramp_frac:{args.mtp_ramp_frac}",
                f"mtp_horizon_decay:{args.mtp_horizon_decay}",
    ...
    log0(
        f"sdp_backends:cudnn=False flash=True mem_efficient=False math=False flash_attn3:{flash_attn_3_func is not None}"
    )
    ...
    while True:
        ...
        progress = training_progress(step, elapsed_ms, args.iterations, max_wallclock_ms)
        mtp_weights = build_mtp_weights(args, progress, scale, device)
        ...
        if should_log_train:
            mtp_status = (
                " mtp_weights:" + "/".join(f"{w:.3f}" for w in mtp_weights.detach().cpu().tolist())
