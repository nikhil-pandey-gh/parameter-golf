...
class Hyperparameters:
    ...
    mtp_distill_weight = float(os.environ.get("MTP_DISTILL_WEIGHT", 0.10))
...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_distill_weight > 0.0 and mtp_distill_temperature <= 0.0:
            raise ValueError(
                f"mtp_distill_temperature must be positive when distillation is enabled, got {mtp_distill_temperature}"
            )
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_init_from_main and self.tie_embeddings:
            with torch.no_grad():
                for head in self.mtp_heads:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and (self.mtp_loss_weight > 0.0 or self.mtp_distill_weight > 0.0):
            batch_size, seqlen, dim = x.shape
            vocab_size = logits.size(-1)
            teacher_logits = logits.detach().reshape(batch_size, seqlen, vocab_size) if self.mtp_distill_weight > 0.0 else None
            mtp_ce_sum = x.new_zeros(())
            mtp_ce_count = 0
            mtp_kl_sum = x.new_zeros(())
            mtp_kl_count = 0
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if self.mtp_loss_weight > 0.0:
                    mtp_ce_sum = mtp_ce_sum + F.cross_entropy(mtp_logits.float(), mtp_targets, reduction="mean")
                    mtp_ce_count += 1
                if teacher_logits is not None:
                    temp = self.mtp_distill_temperature
                    teacher = teacher_logits[:, k + 1 :, :].reshape(-1, vocab_size)
                    student_log_probs = F.log_softmax((mtp_logits / temp).float(), dim=-1)
                    teacher_probs = F.softmax((teacher / temp).float(), dim=-1)
                    mtp_kl_sum = mtp_kl_sum + F.kl_div(student_log_probs, teacher_probs, reduction="batchmean") * (temp * temp)
                    mtp_kl_count += 1
            if mtp_ce_count > 0:
                main_loss = main_loss + self.mtp_loss_weight * (mtp_ce_sum / mtp_ce_count)
            if mtp_kl_count > 0:
                main_loss = main_loss + self.mtp_distill_weight * (mtp_kl_sum / mtp_kl_count)
        return main_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_distill_weight:{args.mtp_distill_weight} "
        f"mtp_distill_temperature:{args.mtp_distill_temperature} "
        f"mtp_init_from_main:{args.mtp_init_from_main} mtp_params:{mtp_params}"
    )
