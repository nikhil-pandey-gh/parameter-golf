...
class Hyperparameters:
    ...
    mtp_warmup_fraction = float(os.environ.get("MTP_WARMUP_FRACTION", 0.20))
    mtp_full_fraction = float(os.environ.get("MTP_FULL_FRACTION", 0.55))
...
class GPT(nn.Module):
    def __init__(
        ...
        # Buffer-based schedules survive torch.compile; Python flags are prone to constant folding.
        self.register_buffer("mtp_aux_weight", torch.tensor(float(mtp_loss_weight), dtype=torch.float32), persistent=False)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            mtp_weight = self.mtp_aux_weight.to(dtype=main_loss.dtype)
...
def main() -> None:
    ...
    base_model.mtp_aux_weight.zero_()
    ...
    log0(
        f"mtp_curriculum:warmup_frac:{args.mtp_warmup_fraction:.2f} "
        f"full_frac:{args.mtp_full_fraction:.2f}"
    )
    ...
    def mtp_curriculum_weight(step: int) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        progress = step / max(args.iterations - 1, 1)
        warmup = min(max(args.mtp_warmup_fraction, 0.0), 1.0)
        full = min(max(args.mtp_full_fraction, warmup), 1.0)
        if progress <= warmup:
            return 0.0
        if progress >= full:
            return args.mtp_loss_weight
        alpha = (progress - warmup) / max(full - warmup, 1e-9)
        alpha = 0.5 - 0.5 * math.cos(math.pi * alpha)
        return args.mtp_loss_weight * alpha
    ...
    while True:
        ...
        current_mtp_weight = mtp_curriculum_weight(step)
        base_model.mtp_aux_weight.fill_(current_mtp_weight)
        zero_grad_all()
