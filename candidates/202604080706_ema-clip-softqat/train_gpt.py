...
class Hyperparameters:
    ...
    qreg_enabled = bool(int(os.environ.get("QREG_ENABLED", "1")))
    qreg_start_scale = float(os.environ.get("QREG_START_SCALE", 0.30))
    qreg_weight = float(os.environ.get("QREG_WEIGHT", 0.02))
    qreg_clip_decay = float(os.environ.get("QREG_CLIP_DECAY", 0.95))
...
def qreg_target_module(name: str) -> bool:
    return name.endswith("attn.proj") or name.endswith("mlp.proj") or name in {"bigram.proj", "ve_shared.proj"}


def qreg_strength(lr_scale: float, start_scale: float) -> float:
    if start_scale <= 0.0 or lr_scale >= start_scale:
        return 0.0
    return min((start_scale - lr_scale) / max(start_scale, 1e-6), 1.0)


def quantization_consistency_loss(
    modules: list[tuple[str, CastedLinear]],
    scale_emas: dict[str, Tensor],
    ...
    for name, module in modules:
        ...
        row_absmax = weight.detach().abs().amax(dim=1)
        scale_ema = scale_emas[name]
        if not initialized[name]:
            scale_ema.copy_(row_absmax)
            initialized[name] = True
        else:
            scale_ema.mul_(clip_decay).add_(row_absmax, alpha=1.0 - clip_decay)
        scale = (scale_ema / 31.0).clamp_min(1.0 / 31.0)
        with torch.no_grad():
            q = torch.clamp(torch.round(weight.detach() / scale[:, None]), -31, 31)
            dequant = q * scale[:, None]
        losses.append(((weight - dequant) / scale[:, None]).square().mean())
...
def main() -> None:
    ...
    flash_sdp_enabled = flash_attn_3_func is not None
    mem_efficient_sdp_enabled = not flash_sdp_enabled
    math_sdp_enabled = not flash_sdp_enabled
    ...
    qreg_modules = [
        ...
        if isinstance(module, CastedLinear) and qreg_target_module(name)
    ]
    qreg_scale_emas = {
        name: torch.zeros(module.weight.shape[0], device=device, dtype=torch.float32)
        for name, module in qreg_modules
    }
    qreg_initialized = {name: False for name, _ in qreg_modules}
    ...
    log0(
        f"sdp_backends:cudnn=False flash={flash_sdp_enabled} "
    ...
    log0(
        f"qreg:enabled={args.qreg_enabled} start_scale:{args.qreg_start_scale:.3f} "
        f"weight:{args.qreg_weight:.4f} clip_decay:{args.qreg_clip_decay:.3f} "
        f"targets:{','.join(name for name, _ in qreg_modules)}"
    )
    log0(
    ...
    while True:
        ...
        qreg_mul = qreg_strength(scale, args.qreg_start_scale) if args.qreg_enabled else 0.0
        if qreg_mul > 0.0 and not qreg_announced:
            log0(
                f"qreg:active step:{step} scale:{scale:.4f} strength:{qreg_mul:.4f} "
                f"targets:{len(qreg_modules)}"
            )
            qreg_announced = True
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        train_qreg = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
            if qreg_mul > 0.0:
                qreg_loss = quantization_consistency_loss(
                    qreg_modules,
                    qreg_scale_emas,
                    qreg_initialized,
                    args.qreg_clip_decay,
                ).to(device=device)
                train_qreg += qreg_loss.detach()
                loss = loss + args.qreg_weight * qreg_mul * qreg_loss
            train_loss += loss.detach()
            (loss * grad_scale).backward()
        train_loss /= grad_accum_steps
        train_qreg /= grad_accum_steps
        ...
        if should_log_train:
            qreg_suffix = f" qreg:{train_qreg.item():.4f} qreg_mul:{qreg_mul:.3f}" if qreg_mul > 0.0 else ""
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f}{qreg_suffix} "
