...
def causal_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if q_t.size(1) != k_t.size(1):
        if q_t.size(1) % k_t.size(1) != 0:
            raise ValueError("num_heads must be divisible by num_kv_heads for grouped attention fallback")
...
def collect_optimizer_param_groups(base_model: GPT, args: Hyperparameters):
    matrix_params = [
        base_model.qo_bank,
        base_model.kv_bank,
    ...
    if base_model.mtp_num_heads > 0:
        # These heads are stripped from the exported artifact, but still need to learn during training.
        matrix_params.extend(p for p in base_model.mtp_heads.parameters() if p.ndim == 2)
    return token_lr, tok_params, matrix_params, scalar_params
...
def main() -> None:
    ...
    # Optimizer split:
    # - 4 parameter banks + training-only MTP heads -> Muon (matrix params)
    # - token / VE / bigram embeddings -> AdamW
    # - scalars, controls, and tiny projection weights -> AdamW
    token_lr, tok_params, matrix_params, scalar_params = collect_optimizer_param_groups(base_model, args)
