...
class Hyperparameters:
    ...
    int6_hadamard_rot = bool(int(os.environ.get("INT6_HADAMARD_ROT", "1")))
    int6_hadamard_block_size = int(os.environ.get("INT6_HADAMARD_BLOCK_SIZE", 512))
...
class CausalSelfAttention(nn.Module):
    ...
    def _attn_fallback(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        q_t = q.transpose(1, 2)
        k_t = k.transpose(1, 2)
        v_t = v.transpose(1, 2)
...
def _block_hadamard_last_dim(x: Tensor, block_size: int) -> Tensor:
    if x.ndim != 2:
        raise ValueError(f"Hadamard rotation expects a 2D tensor, got {tuple(x.shape)}")
    if block_size <= 1 or (block_size & (block_size - 1)) != 0:
        raise ValueError(f"Hadamard block size must be a power of two > 1, got {block_size}")
    if x.shape[-1] % block_size != 0:
        raise ValueError(f"Last dim {x.shape[-1]} must be divisible by Hadamard block size {block_size}")
    blocks = x.shape[-1] // block_size
    y = x.reshape(x.shape[0], blocks, block_size)
    y = _fwht_last_dim(y) / math.sqrt(block_size)
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, dict[str, object] | None]:
    t32 = t.float()
    if t32.ndim == 2:
        block_size = Hyperparameters.int6_hadamard_block_size
        candidates: list[tuple[Tensor, dict[str, object] | None]] = [(t32, None)]
        if (
            Hyperparameters.int6_hadamard_rot
            and t32.shape[1] >= block_size
            and t32.shape[1] % block_size == 0
        ):
            rotated = _block_hadamard_last_dim(t32, block_size)
            candidates.append((rotated, {"rotation": "hadamard", "block_size": block_size}))
        best_q, best_s, best_err, best_meta = None, None, float("inf"), None
        for candidate, candidate_meta in candidates:
            for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
                if pct < 1.0:
                    row_clip = torch.quantile(candidate.abs(), pct, dim=1)
                else:
                    row_clip = candidate.abs().amax(dim=1)
                s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
                q = torch.clamp(torch.round(candidate / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
                recon = q.float() * s.float()[:, None]
                if candidate_meta is not None:
                    recon = _block_hadamard_last_dim(recon, int(candidate_meta["block_size"]))
                err = (t32 - recon).pow(2).mean().item()
                if err < best_err:
                    best_q, best_s, best_err, best_meta = q, s, err, candidate_meta
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, extra_meta = quantize_int6_per_row(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            info = {"type": "int6"}
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("rotation") == "hadamard":
            t = _block_hadamard_last_dim(t.float(), int(info["block_size"]))
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    model.eval()
    seq_len = max(8, min(args.smoke_seq_len, args.train_seq_len))
    x = torch.randint(args.vocab_size, (1, seq_len), dtype=torch.int64)
    y = torch.randint(args.vocab_size, (1, seq_len), dtype=torch.int64)
    with torch.no_grad():
        loss = model(x, y)
    ...
    sample_sd = {name: unbanked[name] for name in sample_keys}
    quant_state, quant_meta = mixed_quantize_int6(sample_sd, {"mlp", "attn"})
    dequantized = dequantize_mixed_int6(quant_state, quant_meta, sample_sd)
    max_abs_err = max(
        (sample_sd[name].float() - dequantized[name].float()).abs().max().item()
        for name in sample_sd
    )
    rotation_summary = {
        name: meta.get("rotation", "none")
        for name, meta in quant_meta.items()
        if isinstance(meta, dict) and meta.get("type") == "int6"
    }
    print(
        f"smoke_test_ok loss:{loss.item():.4f} max_abs_err:{max_abs_err:.6f} "
        f"rotations:{rotation_summary}"
    )
