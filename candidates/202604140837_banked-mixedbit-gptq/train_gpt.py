...
QUANT_SEARCH_PERCENTILES = tuple(
    float(x)
    for x in os.environ.get("QUANT_SEARCH_PERCENTILES", "0.9990,0.9995,0.9999,0.99999,1.0").split(",")
    if x
)
ATTN_INT_CLIP = int(os.environ.get("ATTN_INT_CLIP", 31))
MLP_INT_CLIP = int(os.environ.get("MLP_INT_CLIP", 15))
...
# --- Bank-aware GPTQ-lite mixed-bit quantization ---
...
def quantize_intN_per_row(t: Tensor, clip_range: int, percentiles: tuple[float, ...] = QUANT_SEARCH_PERCENTILES) -> tuple[Tensor, Tensor]:
    if clip_range <= 0:
    ...
    if t32.ndim >= 2:
        ...
        for pct in percentiles:
            if pct < 1.0:
                row_clip = torch.quantile(abs_rows, pct, dim=1)
            else:
                row_clip = abs_rows.amax(dim=1)
            s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
            s = s.clamp_min(torch.finfo(torch.float16).tiny)
            q = torch.clamp(torch.round(rows / s.float().unsqueeze(1)), -(clip_range + 1), clip_range).to(torch.int8)
            recon = q.float() * s.float().unsqueeze(1)
            err = (rows - recon).pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_err = q, s, err
        if best_q is None or best_s is None:
            raise RuntimeError("failed to choose a quantization scale")
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], passthrough_max_numel: int = 65536):
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    for name, tensor in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        cat = _classify_param(name)
        if not t.is_floating_point() or t.numel() <= passthrough_max_numel:
        ...
        if cat in int6_cats and t.ndim >= 1:
            clip = MLP_INT_CLIP if cat == "mlp" else ATTN_INT_CLIP
            q, s = quantize_intN_per_row(t, clip_range=clip)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            bits = int(round(math.log2(clip + 1))) + 1
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            scale = s.float().view(*s.shape, *([1] * (q.ndim - s.ndim)))
...
def run_smoke_test() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(state_dict, {"mlp", "attn"}, passthrough_max_numel=1)
    deq_state = dequantize_mixed_int6(quant_result, quant_meta, state_dict)
    for name in ("qo_bank", "kv_bank", "mlp_up_bank", "mlp_down_bank", "tok_emb.weight"):
        if name not in deq_state:
            raise RuntimeError(f"missing {name} after dequantization")
        if deq_state[name].shape != state_dict[name].shape:
            raise RuntimeError(f"shape mismatch for {name}: {deq_state[name].shape} != {state_dict[name].shape}")
        if not isinstance(quant_meta[name], dict):
            raise RuntimeError(f"smoke test did not quantize {name}")
    quantized = sum(isinstance(info, dict) for info in quant_meta.values())
    print(f"smoke_test:ok loss:{loss.item():.4f} quantized_tensors:{quantized}")
