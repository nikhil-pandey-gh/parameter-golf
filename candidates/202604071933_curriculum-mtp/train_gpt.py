...
class Hyperparameters:
    ...
    mtp_curriculum_frac = float(os.environ.get("MTP_CURRICULUM_FRAC", 0.4))
    mtp_head_weight_decay = float(os.environ.get("MTP_HEAD_WEIGHT_DECAY", 0.5))
...
class GPT(nn.Module):
    def __init__(
        ...
        mtp_head_weight_decay: float = 0.5,
        ...
        mtp_head_weights = [mtp_head_weight_decay ** k for k in range(mtp_num_heads)]
        self.register_buffer(
            "mtp_head_weights",
            torch.tensor(mtp_head_weights, dtype=torch.float32),
            persistent=False,
        )
        # Keep curriculum state in a tensor buffer so torch.compile sees per-step updates.
        self.register_buffer("mtp_curriculum_scale", torch.zeros((), dtype=torch.float32), persistent=False)
        ...
        self._init_weights()
    def set_mtp_curriculum_scale(self, scale: float) -> None:
        self.mtp_curriculum_scale.fill_(max(0.0, min(float(scale), 1.0)))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            curriculum_scale = self.mtp_curriculum_scale.to(dtype=x.dtype).clamp(0.0, 1.0)
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                head_weight = self.mtp_head_weights[k].to(dtype=x.dtype) * curriculum_scale.pow(k + 1)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_head_weight_decay=args.mtp_head_weight_decay,
    ...
    if base_model.mtp_num_heads > 0:
        scalar_params.extend(list(base_model.mtp_heads.parameters()))
    ...
    log0(
        ...
        f"mtp_curriculum_frac:{args.mtp_curriculum_frac} "
        f"mtp_head_weight_decay:{args.mtp_head_weight_decay} mtp_params:{mtp_params}"
    )
    ...
    def mtp_curriculum_scale(step: int, elapsed_ms: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_curriculum_frac <= 0.0:
            return 1.0
        if max_wallclock_ms is not None:
            ramp_ms = max_wallclock_ms * args.mtp_curriculum_frac
            return min(elapsed_ms / max(ramp_ms, 1e-9), 1.0)
        ramp_steps = max(int(args.iterations * args.mtp_curriculum_frac), 1)
        return min(step / ramp_steps, 1.0)
    base_model.set_mtp_curriculum_scale(mtp_curriculum_scale(0, 0.0))
    ...
    while True:
        ...
        base_model.set_mtp_curriculum_scale(mtp_curriculum_scale(step, elapsed_ms))
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_head_weight_decay=args.mtp_head_weight_decay,
