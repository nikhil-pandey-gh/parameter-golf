...
class GPT(nn.Module):
    def __init__(
        ...
        # Keep the auxiliary MTP weight out of the exported state_dict while still
        # letting the training loop schedule it without hitting torch.compile constant folding.
        self.register_buffer("mtp_loss_weight", torch.tensor(float(mtp_loss_weight), dtype=torch.float32), persistent=False)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if valid_t <= 0:
                    continue
                head_weight = self.mtp_horizon_decay ** k
                if head_weight <= 0.0:
                    continue
            ...
            if mtp_weight_sum > 0.0:
                main_loss = main_loss + self.mtp_loss_weight.to(dtype=main_loss.dtype) * (mtp_loss_sum / mtp_weight_sum)
        return main_loss
...
def main() -> None:
    ...
    args = Hyperparameters()
    if args.mtp_loss_weight <= 0.0 or args.mtp_num_heads <= 0:
    ...
    log0(
        ...
        f"mtp_horizon_decay:{args.mtp_horizon_decay} mtp_schedule:0.1+0.9*lr_scale mtp_params:{mtp_params}"
    )
    ...
    while True:
        ...
        if base_model.mtp_num_heads > 0:
            # Keep a small nonzero floor so the auxiliary heads never burn compute
            # on a zero-weight objective during the scheduled warmdown.
            scheduled_mtp_weight = args.mtp_loss_weight * (0.1 + 0.9 * scale)
            base_model.mtp_loss_weight.fill_(scheduled_mtp_weight)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_w:{float(base_model.mtp_loss_weight):.4f} "
    ...
    if master_process:
        ...
        log0(f"Serialized training checkpoint: {model_bytes} bytes")
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_horizon_decay=args.mtp_horizon_decay,
