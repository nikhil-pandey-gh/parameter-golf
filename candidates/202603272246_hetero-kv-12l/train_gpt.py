...
def ensure_training_imports() -> None:
    ...
    if np is None:
        ...
        np = numpy_mod
    if spm is None:
        import sentencepiece as sentencepiece_mod
        spm = sentencepiece_mod
...
class Hyperparameters:
    ...
    kv_heads_schedule = os.environ.get("KV_HEADS_SCHEDULE", "4,4,4,2,2,2,2,1,1,1,1,1")
...
def parse_kv_heads_schedule(schedule: str, num_layers: int, default_num_kv_heads: int) -> list[int]:
    cleaned = schedule.strip()
    if not cleaned:
        return [default_num_kv_heads] * num_layers
    vals = [int(part.strip()) for part in cleaned.split(",") if part.strip()]
    if len(vals) == 1:
        vals *= num_layers
    if len(vals) != num_layers:
        raise ValueError(
            f"KV_HEADS_SCHEDULE must provide exactly NUM_LAYERS={num_layers} entries or one shared value; got {vals}"
        )
    if any(v <= 0 for v in vals):
        raise ValueError(f"All KV_HEADS_SCHEDULE entries must be positive, got {vals}")
    return vals
...
class ValueEmbedding(nn.Module):
    ...
    Each table maps vocab tokens to a low-dim embedding, projected to the largest KV width."""
...
class GPT(nn.Module):
    def __init__(
        ...
        self.kv_heads_schedule = parse_kv_heads_schedule(kv_heads_schedule, num_layers, num_kv_heads)
        self._ve_target_dim = max(self.kv_heads_schedule) * (model_dim // num_heads)
    ...
    def _get_ve(self, layer_idx: int, input_ids: Tensor, target_dim: int, ve_cache: dict | None = None) -> Tensor | None:
        ...
        return ve_base[..., :target_dim] * self.ve_layer_scales[ve_idx].to(dtype=ve_base.dtype)
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ve = self._get_ve(i, input_ids, self.blocks[i].attn.kv_dim, ve_cache)
        ...
        for i in range(self.num_decoder_layers):
            ...
            ve = self._get_ve(bi, input_ids, self.blocks[bi].attn.kv_dim, ve_cache)
...
def run_cpu_smoke(args: Hyperparameters) -> None:
    model = GPT(
        ...
        kv_heads_schedule=args.kv_heads_schedule,
        ...
        mtp_num_heads=0,
        mtp_loss_weight=0.0,
        bigram_vocab_size=min(args.bigram_vocab_size, 512),
    ...
    loss = model(x, y)
    loss.backward()
    print(f"cpu_smoke_ok loss:{loss.item():.4f}")
...
def main() -> None:
    ...
    log0(f"kv_heads_schedule:{base_model.kv_heads_schedule}")
    ...
    eval_model = GPT(
        ...
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, kv_heads_schedule=args.kv_heads_schedule,
        mlp_mult=args.mlp_mult, leaky_relu_slope=args.leaky_relu_slope,
