...
def ste_fake_quantize_int6(t: Tensor, clip_range: int = 31) -> Tensor:
    if t.ndim < 2:
        return t
    t32 = t.float()
    ...
    q = torch.clamp(torch.round(rows / scale), -clip_range, clip_range)
    dequant = (q * scale).reshape_as(t32).to(dtype=t.dtype)
    return t + (dequant - t).detach()
...
class GPT(nn.Module):
    ...
    def _get_active_banks(self) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        if self.training and self.bank_qat_enabled:
            return (
                ste_fake_quantize_int6(self.qo_bank),
                ste_fake_quantize_int6(self.kv_bank),
                ste_fake_quantize_int6(self.mlp_up_bank),
                ste_fake_quantize_int6(self.mlp_down_bank),
            )
        return self.qo_bank, self.kv_bank, self.mlp_up_bank, self.mlp_down_bank
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        qo_bank, kv_bank, mlp_up_bank, mlp_down_bank = self._get_active_banks()
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_qat_enabled=args.qat_enabled,
    ).to(device).bfloat16()
    ...
    log0(
        ...
        f"bank_qat_enabled:{base_model.bank_qat_enabled}"
    )
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not base_model.bank_qat_enabled:
            # Recompile after flipping the Python flags so torch.compile traces the QAT path.
            base_model.bank_qat_enabled = True
            CastedLinear._qat_enabled = True
            model = torch.compile(base_model, dynamic=False, fullgraph=True)
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} bank_qat:1 recompiled:1")
        zero_grad_all()
