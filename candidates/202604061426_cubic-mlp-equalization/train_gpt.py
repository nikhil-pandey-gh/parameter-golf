...
class Hyperparameters:
    ...
    mlp_eq_enabled = bool(int(os.environ.get("MLP_EQ_ENABLED", "1")))
    mlp_eq_max_scale = float(os.environ.get("MLP_EQ_MAX_SCALE", 4.0))
    mlp_eq_passes = int(os.environ.get("MLP_EQ_PASSES", 1))
    smoke_test_only = bool(int(os.environ.get("SMOKE_TEST_ONLY", "0")))
    smoke_test_batch_size = int(os.environ.get("SMOKE_TEST_BATCH_SIZE", 2))
    smoke_test_seq_len = int(os.environ.get("SMOKE_TEST_SEQ_LEN", 32))
...
def flash_or_sdpa(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    y = F.scaled_dot_product_attention(qh, kh, vh, is_causal=causal)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = flash_or_sdpa(q, k, v, causal=True)
        if self.use_xsa:
...
def _stabilize_equalization_scale(scale: Tensor, max_scale: float) -> Tensor:
    scale = scale.clamp_min(1e-8)
    if max_scale > 1.0:
        limit = math.log(max_scale)
        scale = scale.log().clamp(-limit, limit).exp()
...
def _cubic_equalize_mlp_pair(up: Tensor, down: Tensor, max_scale: float) -> tuple[Tensor, Tensor, dict[str, float]]:
    up32 = up.float()
    down32 = down.float()
    eps = 1e-8
    up_stat = up32.abs().amax(dim=1).clamp_min(eps)
    down_stat = down32.abs().amax(dim=0).clamp_min(eps)
    scale = _stabilize_equalization_scale((down_stat / up_stat).pow(1.0 / 3.0), max_scale)
    scale_sq = scale.square()
    equalized_up = (up32 * scale[:, None]).to(dtype=up.dtype).contiguous()
    equalized_down = (down32 / scale_sq[None, :]).to(dtype=down.dtype).contiguous()
    return equalized_up, equalized_down, {
...
def apply_mlp_cubic_equalization(
    state_dict: dict[str, Tensor],
    max_scale: float,
    passes: int,
) -> tuple[dict[str, Tensor], list[dict[str, float]]]:
    if passes <= 0 or max_scale <= 1.0:
        return state_dict, []
    out = {name: tensor.detach().cpu().contiguous() for name, tensor in state_dict.items()}
    num_layers_total = max(
        (int(k.split(".")[1]) for k in out if k.startswith("blocks.")),
        default=0,
    ) + 1
    summaries: list[dict[str, float]] = []
    for _ in range(passes):
        scale_mins: list[float] = []
        scale_maxs: list[float] = []
        ...
        for layer_idx in range(num_layers_total):
            ...
            if up_key not in out or down_key not in out:
                continue
            equalized_up, equalized_down, stats = _cubic_equalize_mlp_pair(out[up_key], out[down_key], max_scale)
            out[up_key] = equalized_up
            out[down_key] = equalized_down
            scale_mins.append(stats["scale_min"])
            scale_maxs.append(stats["scale_max"])
            scale_means.append(stats["scale_mean"])
        if scale_mins:
            summaries.append(
                {
                    "layers": float(len(scale_mins)),
                    "scale_min": min(scale_mins),
                    "scale_max": max(scale_maxs),
                    "scale_mean": sum(scale_means) / len(scale_means),
                }
            )
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    model.eval()
    seq_len = max(2, min(args.smoke_test_seq_len, args.train_seq_len))
    batch_size = max(1, args.smoke_test_batch_size)
    input_ids = torch.randint(args.vocab_size, (batch_size, seq_len), dtype=torch.int64, device=device)
    ...
    if args.mlp_eq_enabled:
        unbanked_sd, _ = apply_mlp_cubic_equalization(unbanked_sd, args.mlp_eq_max_scale, args.mlp_eq_passes)
    ...
    print(
        f"smoke_test_ok loss:{loss.item():.6f} roundtrip_loss:{roundtrip_loss.item():.6f} "
...
def main() -> None:
    ...
    enable_mem_efficient_sdp(not use_flash_sdp)
    ...
    log0(
        "sdp_backends:"
        f"cudnn=False flash={use_flash_sdp} mem_efficient={not use_flash_sdp} math={not use_flash_sdp}"
    )
    ...
    if args.mlp_eq_enabled:
        unbanked_sd, eq_summaries = apply_mlp_cubic_equalization(
            unbanked_sd,
            args.mlp_eq_max_scale,
            args.mlp_eq_passes,
        )
        for pass_idx, summary in enumerate(eq_summaries, start=1):
            log0(
                f"mlp_eq:pass={pass_idx} layers={int(summary['layers'])} "
                f"scale_min:{summary['scale_min']:.4f} scale_max:{summary['scale_max']:.4f} "
                f"scale_mean:{summary['scale_mean']:.4f}"
            )
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"})
