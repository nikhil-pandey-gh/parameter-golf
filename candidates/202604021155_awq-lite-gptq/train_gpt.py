...
class Hyperparameters:
    ...
    awq_buffer_batches = int(os.environ.get("AWQ_BUFFER_BATCHES", 8))
    awq_buffer_every = int(os.environ.get("AWQ_BUFFER_EVERY", 64))
    awq_collect_threshold = float(os.environ.get("AWQ_COLLECT_THRESHOLD", 0.20))
    awq_max_batch_seqs = int(os.environ.get("AWQ_MAX_BATCH_SEQS", 16))
    awq_activation_power = float(os.environ.get("AWQ_ACTIVATION_POWER", 1.0))
...
class ActivationAwareQuantStats:
    def __init__(self, power: float = 1.0, eps: float = 1e-6):
        self.power = power
        self.eps = eps
        self.sum_sq: dict[str, Tensor] = {}
        self.counts: dict[str, int] = {}

    def capture(self, name: str, x: Tensor) -> None:
        dims = tuple(range(x.ndim - 1))
        second_moment = x.float().square().mean(dim=dims).to(device="cpu", dtype=torch.float32)
        if name in self.sum_sq:
            self.sum_sq[name] += second_moment
            self.counts[name] += 1
        else:
            self.sum_sq[name] = second_moment
            self.counts[name] = 1

    def weight(self, name: str, width: int) -> Tensor | None:
        avg = self.sum_sq.get(name)
        if avg is None or avg.numel() != width:
            return None
        avg = (avg / max(self.counts.get(name, 1), 1)).clamp_min(self.eps)
        avg = avg / avg.mean().clamp_min(self.eps)
        if self.power != 1.0:
            avg = avg.pow(self.power)
        return avg

    def all_reduce_(self, device: torch.device) -> None:
        if not (dist.is_available() and dist.is_initialized()):
            return
        for name in sorted(self.sum_sq):
            sum_sq = self.sum_sq[name].to(device=device, dtype=torch.float32)
            count = torch.tensor(self.counts.get(name, 0), device=device, dtype=torch.int64)
            dist.all_reduce(sum_sq, op=dist.ReduceOp.SUM)
            dist.all_reduce(count, op=dist.ReduceOp.SUM)
            self.sum_sq[name] = sum_sq.to(device="cpu")
            self.counts[name] = int(count.item())
...
class CausalSelfAttention(nn.Module):
    ...
    def forward_with_stats(
        ...
        v_w: Tensor,
        out_w: Tensor,
        stats: ActivationAwareQuantStats,
        v_embed: Tensor | None = None,
        v0: Tensor | None = None,
    ) -> tuple[Tensor, Tensor | None]:
        stats.capture(f"{prefix}.c_q.weight", x)
        stats.capture(f"{prefix}.c_k.weight", x)
        stats.capture(f"{prefix}.c_v.weight", x)
        bsz, seqlen, dim = x.shape
        q = F.linear(x, q_w.to(x.dtype)).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k = F.linear(x, k_w.to(x.dtype)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        v = F.linear(x, v_w.to(x.dtype))
        ...
        if self.gated_attention:
            ...
            y = y * gate
        y = y.reshape(bsz, seqlen, dim)
        stats.capture(f"{prefix}.proj.weight", y)
        return F.linear(y, out_w.to(x.dtype)), raw_v
...
class BigramHashEmbedding(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        return self.forward_with_stats(token_ids)

    def forward_with_stats(self, token_ids: Tensor, stats: ActivationAwareQuantStats | None = None) -> Tensor:
        h = self.embed(self.bigram_hash(token_ids))
        if self.proj is not None:
            if stats is not None:
                stats.capture("bigram.proj.weight", h)
            h = self.proj(h)
...
class ValueEmbedding(nn.Module):
    ...
    def forward_with_stats(self, token_ids: Tensor, stats: ActivationAwareQuantStats | None = None) -> Tensor:
        ...
        if self.proj is not None:
            if stats is not None:
                stats.capture("ve_shared.proj.weight", h)
            h = self.proj(h)
        return h * self.scale.to(dtype=h.dtype)
...
class MLP(nn.Module):
    ...
    def forward_with_stats(
        ...
        stats.capture(f"{prefix}.fc.weight", x)
        hidden = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=0.5).square()
        stats.capture(f"{prefix}.proj.weight", hidden)
...
class Block(nn.Module):
    ...
    def forward_with_stats(
        ...
        attn_out, raw_v = self.attn.forward_with_stats(
        ...
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp.forward_with_stats(
            f"blocks.{layer_idx}.mlp",
            mlp_in,
            up_w,
            down_w,
            stats,
...
class GPT(nn.Module):
    ...
    def _get_ve_with_stats(
        ...
        if self.ve_shared is None or layer_idx not in self.ve_layer_indices:
            return None
        if ve_cache is not None and "ve" not in ve_cache:
            ve_cache["ve"] = self.ve_shared.forward_with_stats(input_ids, stats)
        ve_base = ve_cache["ve"] if ve_cache is not None else self.ve_shared.forward_with_stats(input_ids, stats)
        ve_idx = self.ve_layer_indices.index(layer_idx)
        return ve_base * self.ve_layer_scales[ve_idx].to(dtype=ve_base.dtype)

    def collect_quant_stats(self, input_ids: Tensor, stats: ActivationAwareQuantStats) -> None:
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram.forward_with_stats(input_ids, stats)
        ...
        for i in range(self.num_encoder_layers):
            ve = self._get_ve_with_stats(i, input_ids, stats, ve_cache)
            x, raw_v = self.blocks[i].forward_with_stats(
                i,
                x,
                x0,
                self.qo_bank[i],
        ...
        for i in range(self.num_decoder_layers):
            ...
            ve = self._get_ve_with_stats(bi, input_ids, stats, ve_cache)
            x, _ = self.blocks[bi].forward_with_stats(
                bi,
                x,
                x0,
                self.qo_bank[bi],
...
def quantize_int6_per_row(
    ...
    if t32.ndim == 2:
        col_weight = None
        if input_weight is not None and input_weight.numel() == t32.shape[1]:
            col_weight = input_weight.to(dtype=t32.dtype, device=t32.device)
            col_weight = col_weight / col_weight.mean().clamp_min(1e-6)
        best_q = torch.empty_like(t32, dtype=torch.int8)
        best_s = torch.empty((t32.shape[0],), dtype=torch.float16)
        best_err = torch.full((t32.shape[0],), float("inf"), dtype=t32.dtype)
        for pct in INT6_CLIP_CANDIDATES:
            ...
            row_err = diff_sq.mean(dim=1) if col_weight is None else (diff_sq * col_weight[None, :]).mean(dim=1)
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    awq_stats: ActivationAwareQuantStats | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            input_weight = awq_stats.weight(name, t.shape[1]) if awq_stats is not None and t.ndim == 2 else None
            q, s = quantize_int6_per_row(t, input_weight=input_weight)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "awq": input_weight is not None}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def main() -> None:
    ...
    log0(
        f"awq_enabled:{args.awq_enabled} awq_collect_threshold:{args.awq_collect_threshold:.3f} "
        f"awq_buffer_every:{args.awq_buffer_every} awq_buffer_batches:{args.awq_buffer_batches} "
        f"awq_max_batch_seqs:{args.awq_max_batch_seqs} awq_activation_power:{args.awq_activation_power:.2f}"
    )
    log0(
    ...
    awq_input_buffer: deque[Tensor] | None = (
        deque(maxlen=args.awq_buffer_batches)
        if args.awq_enabled and args.awq_buffer_batches > 0 and args.awq_max_batch_seqs > 0
        else None
    )
    ...
    awq_stats = None
    if awq_input_buffer is not None and len(awq_input_buffer) > 0:
        awq_stats = ActivationAwareQuantStats(power=args.awq_activation_power)
        log0(
            f"awq:replaying buffered late-train batches batches:{len(awq_input_buffer)} "
            f"max_batch_seqs:{args.awq_max_batch_seqs}"
        )
        base_model.eval()
        with torch.inference_mode():
            for token_batch in awq_input_buffer:
                base_model.collect_quant_stats(token_batch.to(device=device, dtype=torch.int64, non_blocking=True), awq_stats)
        awq_stats.all_reduce_(device)
        log0(f"awq:activation_stats tensors:{len(awq_stats.sum_sq)}")
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, awq_stats=awq_stats)
    awq_backed_tensors = sum(
        1
        for info in quant_meta.values()
        if isinstance(info, dict) and info.get("type") == "int6" and info.get("awq")
    )
    if awq_backed_tensors > 0:
        log0(f"awq:int6_tensors_with_stats {awq_backed_tensors}")
