...
class Hyperparameters:
    ...
    mlp_share_start = int(os.environ.get("MLP_SHARE_START", 3))
    mlp_share_end = int(os.environ.get("MLP_SHARE_END", 8))
    mlp_shared_banks = int(os.environ.get("MLP_SHARED_BANKS", 3))
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_dim: int = 128,
        ve_layers: str = "9,10",
        mlp_share_start: int = -1,
        mlp_share_end: int = -1,
        mlp_shared_banks: int = 0,
    ):
        super().__init__()
        ...
        self._share_middle_mlps(mlp_share_start, mlp_share_end, mlp_shared_banks)
        if rope_dims > 0:
        ...
        self._init_weights()
    def _share_middle_mlps(self, start: int, end: int, num_banks: int) -> None:
        if num_banks <= 0 or not self.blocks:
            return
        first = max(start, 0)
        last = min(end, len(self.blocks) - 1)
        if first >= last:
            return
        span = last - first + 1
        num_banks = min(num_banks, span)
        for layer_idx in range(first, last + 1):
            source_idx = first + ((layer_idx - first) % num_banks)
            self.mlp_share_map[layer_idx] = source_idx
            if layer_idx != source_idx:
                self.blocks[layer_idx].mlp = self.blocks[source_idx].mlp
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    return q, scale
def tensor_alias_key(t: Tensor) -> tuple[int, int, tuple[int, ...], tuple[int, ...]]:
    return (
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], fp16_names: set[str] | None = None):
    ...
    seen_aliases: dict[tuple[int, int, tuple[int, ...], tuple[int, ...]], str] = {}
    for name, tensor in state_dict.items():
        alias_target = seen_aliases.get(tensor_alias_key(tensor))
        if alias_target is not None:
            meta[name] = {"type": "alias", "target": alias_target}
            continue
        seen_aliases[tensor_alias_key(tensor)] = name
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mlp_negative_slope=args.leaky_relu_slope,
        ...
        mlp_share_start=args.mlp_share_start,
        mlp_share_end=args.mlp_share_end,
        mlp_shared_banks=args.mlp_shared_banks,
    ).to(device).bfloat16()
    ...
    shared_mlp_groups: dict[int, list[int]] = {}
    for layer_idx, source_idx in enumerate(base_model.mlp_share_map):
        shared_mlp_groups.setdefault(source_idx, []).append(layer_idx)
    log0(f"mlp_share_groups:{sorted(shared_mlp_groups.values())}")
    ...
    fp16_export_names = {"tok_emb.weight"} if args.fp16_embed_export else set()
    if not args.tie_embeddings and args.fp16_embed_export:
        fp16_export_names.add("lm_head.weight")
    quant_result, quant_meta = mixed_quantize_int6(export_sd, {"mlp", "attn"}, fp16_names=fp16_export_names)
