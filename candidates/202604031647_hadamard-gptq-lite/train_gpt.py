...
def _dequantize_qs(q: Tensor, s: Tensor) -> Tensor:
    if s.ndim > 0:
...
def _hadamard_last_dim(x: Tensor) -> Tensor:
    ...
    if not _is_power_of_two(n):
        raise ValueError(f"Hadamard transform requires a power-of-two width, got {n}")
    y = x.float().contiguous().reshape(-1, n)
    h = 1
    while h < n:
...
def _apply_hadamard_rotation(t: Tensor, rot: str) -> Tensor:
    if rot == "hadamard_col":
        return _hadamard_last_dim(t)
    if rot == "hadamard_row":
        return _hadamard_last_dim(t.mT).mT
    return t


def quantize_int6_hadamard_aware(
    t: Tensor,
    clip_range: int = 31,
    ...
    t32 = t.float()
    if t32.ndim != 2 or not enable_hadamard:
        q, s = quantize_int6_per_row(t32, clip_range=clip_range)
        return q, s, "none"
    candidates: list[str] = ["none"]
    if t32.shape[1] >= min_dim and _is_power_of_two(t32.shape[1]):
        candidates.append("hadamard_col")
    if t32.shape[0] >= min_dim and _is_power_of_two(t32.shape[0]):
        candidates.append("hadamard_row")
    best_q, best_s, best_rot, best_err = None, None, "none", float("inf")
    # Try no rotation plus fixed Hadamard row/column preprocessing and keep the lowest-MSE roundtrip.
    for rot in candidates:
        rotated = _apply_hadamard_rotation(t32, rot)
        q, s = quantize_int6_per_row(rotated, clip_range=clip_range)
        recon = _apply_hadamard_rotation(_dequantize_qs(q, s), rot)
        err = (t32 - recon).pow(2).mean().item()
        if err < best_err:
            best_q, best_s, best_rot, best_err = q, s, rot, err
    return best_q, best_s, best_rot


def mixed_quantize_int6(
    state_dict: dict[str, Tensor],
    int6_cats: set[str],
    hadamard_gptq: bool = True,
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, rot = quantize_int6_hadamard_aware(
                t,
                enable_hadamard=hadamard_gptq,
                min_dim=hadamard_min_dim,
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "rot": rot}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        deq = _dequantize_qs(q, s)
        if isinstance(info, dict):
            deq = _apply_hadamard_rotation(deq, str(info.get("rot", "none")))
...
def main() -> None:
    ...
    rot_counts = {"none": 0, "hadamard_col": 0, "hadamard_row": 0}
    for info in quant_meta.values():
        if isinstance(info, dict) and info.get("type") == "int6":
            rot = str(info.get("rot", "none"))
            rot_counts[rot] = rot_counts.get(rot, 0) + 1
    log0(
        "gptq_rotations:"
        + ",".join(f"{name}={rot_counts.get(name, 0)}" for name in ("none", "hadamard_col", "hadamard_row"))
    )
