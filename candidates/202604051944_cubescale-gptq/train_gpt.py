...
class Hyperparameters:
    ...
    export_mlp_equalize = bool(int(os.environ.get("EXPORT_MLP_EQUALIZE", "1")))
    export_mlp_equalize_max_scale = float(os.environ.get("EXPORT_MLP_EQUALIZE_MAX_SCALE", 2.5))
...
def flash_or_sdp_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if q_sdp.size(1) != k_sdp.size(1):
        repeat = q_sdp.size(1) // k_sdp.size(1)
        k_sdp = k_sdp.repeat_interleave(repeat, dim=1)
        v_sdp = v_sdp.repeat_interleave(repeat, dim=1)
    y = F.scaled_dot_product_attention(q_sdp, k_sdp, v_sdp, is_causal=True)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = flash_or_sdp_attention(q, k, v)
        if self.use_xsa:
...
def _channel_absmax(t: Tensor, dim: int) -> Tensor:
    return t.float().abs().amax(dim=dim).clamp_min(1e-8)


def cubescale_leakyrelu2_mlp(
    state_dict: dict[str, Tensor],
    num_layers: int,
    max_scale: float,
) -> tuple[dict[str, Tensor], dict[int, dict[str, float]]]:
    """Exact MLP equalization for leaky_relu(x)^2 via cubic hidden-channel scaling.

    For hidden-channel scale s > 0:
      up'(x) = diag(s) up(x)
      down'   = down * diag(s^-2)
    because leaky_relu is positively homogeneous and squaring makes the MLP path
    ...
    for layer_idx in range(num_layers):
        ...
        if up_key not in out or down_key not in out:
            continue
        up = out[up_key].float()
        down = out[down_key].float()
        up_stat = _channel_absmax(up, dim=1)
        down_stat = _channel_absmax(down, dim=0)
        scale = torch.pow(down_stat / up_stat, 1.0 / 3.0)
        scale = scale / torch.exp(torch.mean(torch.log(scale)))
        scale = scale.clamp(1.0 / max_scale, max_scale)
        scale_sq = scale.square()
        out[up_key] = (up * scale[:, None]).to(dtype=state_dict[up_key].dtype)
        out[down_key] = (down / scale_sq[None, :]).to(dtype=state_dict[down_key].dtype)
        stats[layer_idx] = {
...
def run_smoke_test() -> None:
    ...
    model = GPT(**model_kwargs).eval()
    with torch.no_grad():
        nn.init.normal_(model.mlp_down_bank, std=0.02)
    with torch.no_grad():
        base_logits = model.forward_logits(input_ids)
        base_loss = float(model(input_ids, target_ids).item())

    sd_cpu = {k: v.detach().cpu() for k, v in model.state_dict().items()}
    unbanked = _unbank_state_dict(sd_cpu, model_kwargs["num_layers"])
    equalized, eq_stats = cubescale_leakyrelu2_mlp(unbanked, model_kwargs["num_layers"], max_scale=2.5)

    eq_model = GPT(**model_kwargs).eval()
    eq_model.load_state_dict(_rebank_state_dict(equalized, model_kwargs["num_layers"], sd_cpu), strict=True)
    with torch.no_grad():
        equalized_logits = eq_model.forward_logits(input_ids)
        equalized_loss = float(eq_model(input_ids, target_ids).item())

    quant_result, quant_meta = mixed_quantize_int6(equalized, {"mlp", "attn"})
    tmp_path = Path("/tmp/gh-aw/agent") / f"cubescale_smoke_{os.getpid()}.int6.ptz"
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
    quant_blob = lzma.compress(quant_buf.getvalue(), preset=6)
    try:
        tmp_path.write_bytes(quant_blob)
        quant_state = torch.load(io.BytesIO(lzma.decompress(tmp_path.read_bytes())), map_location="cpu")
    finally:
        tmp_path.unlink(missing_ok=True)
    deq_unbanked = dequantize_mixed_int6(quant_state["w"], quant_state["m"], equalized)
    q_model = GPT(**model_kwargs).eval()
    q_model.load_state_dict(_rebank_state_dict(deq_unbanked, model_kwargs["num_layers"], sd_cpu), strict=True)
    with torch.no_grad():
        quant_loss = float(q_model(input_ids, target_ids).item())

    exact_logit_delta = float((base_logits - equalized_logits).abs().max().item())
    exact_loss_delta = abs(base_loss - equalized_loss)
    if not math.isfinite(base_loss) or not math.isfinite(equalized_loss) or not math.isfinite(quant_loss):
        raise RuntimeError("smoke_test produced a non-finite loss")
    if exact_logit_delta > 1e-5 or exact_loss_delta > 1e-6:
        raise RuntimeError(
            "smoke_test exactness check failed "
            f"(logit_delta={exact_logit_delta:.3e}, loss_delta={exact_loss_delta:.3e})"
        )
    print(
        "smoke_test:ok "
        f"base_loss:{base_loss:.6f} "
        f"equalized_loss:{equalized_loss:.6f} "
        f"quant_loss:{quant_loss:.6f} "
        f"logit_delta:{exact_logit_delta:.3e} "
        f"loss_delta:{exact_loss_delta:.3e} "
        f"equalized_layers:{len(eq_stats)}"
    )
...
def main() -> None:
    ...
    if args.export_mlp_equalize:
        unbanked_sd, eq_stats = cubescale_leakyrelu2_mlp(
            unbanked_sd,
            args.num_layers,
            args.export_mlp_equalize_max_scale,
        )
        if eq_stats:
            scale_min = min(stat["min"] for stat in eq_stats.values())
            scale_mean = sum(stat["mean"] for stat in eq_stats.values()) / len(eq_stats)
            scale_max = max(stat["max"] for stat in eq_stats.values())
            log0(
                "export_mlp_equalize:enabled "
                f"layers:{len(eq_stats)} "
                f"scale_min:{scale_min:.3f} "
                f"scale_mean:{scale_mean:.3f} "
                f"scale_max:{scale_max:.3f}"
            )
