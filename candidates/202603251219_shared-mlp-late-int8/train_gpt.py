...
class Hyperparameters:
    ...
    shared_mlp_group_size = int(os.environ.get("SHARED_MLP_GROUP_SIZE", 2))
    shared_mlp_unshared_late = int(os.environ.get("SHARED_MLP_UNSHARED_LATE", 3))
    late_mlp_int8_layers = int(os.environ.get("LATE_MLP_INT8_LAYERS", 2))
...
def causal_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    return out.to(dtype=q.dtype).transpose(1, 2).contiguous()
...
def build_mlp_bank_map(num_layers: int, group_size: int, unshared_late: int) -> list[int]:
    if num_layers <= 0:
        return []
    group_size = max(1, group_size)
    unshared_late = max(0, min(unshared_late, num_layers))
    shared_prefix = num_layers - unshared_late
    bank_map = [0] * num_layers
    bank_id = 0
    layer = 0
    while layer < shared_prefix:
        group_end = min(layer + group_size, shared_prefix)
        for idx in range(layer, group_end):
            bank_map[idx] = bank_id
        bank_id += 1
        layer = group_end
    for idx in range(shared_prefix, num_layers):
        bank_map[idx] = bank_id
        bank_id += 1
    return bank_map
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        shared_mlp_group_size: int = 1,
        shared_mlp_unshared_late: int = 0,
    ):
        super().__init__()
        ...
        self.shared_mlp_group_size = shared_mlp_group_size
        self.shared_mlp_unshared_late = shared_mlp_unshared_late
        self.mlp_bank_map = build_mlp_bank_map(num_layers, shared_mlp_group_size, shared_mlp_unshared_late)
        self.num_mlp_banks = max(self.mlp_bank_map) + 1 if self.mlp_bank_map else 0
        self.qo_bank = nn.Parameter(torch.empty(2 * num_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * num_layers, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(self.num_mlp_banks, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(self.num_mlp_banks, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.num_mlp_banks):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            mlp_bank = self.mlp_bank_map[i]
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[mlp_bank], self.mlp_down_bank[mlp_bank],
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            mlp_bank = self.mlp_bank_map[bi]
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[mlp_bank], self.mlp_down_bank[mlp_bank],
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _classify_param(name: str) -> str:
    if "tok_emb" in name or "lm_head" in name:
        return "embed"
    if ".mlp." in name or name.startswith("mlp_banks."):
        return "mlp"
...
def _unbank_state_dict(
    sd: dict[str, Tensor],
    num_layers: int,
    mlp_bank_map: list[int] | None = None,
) -> dict[str, Tensor]:
    """Convert 3D bank tensors into quantizable 2D tensors without duplicating shared MLP banks."""
    out: dict[str, Tensor] = {}
    n = num_layers
    if mlp_bank_map is None:
        mlp_bank_map = list(range(n))
    num_mlp_banks = max(mlp_bank_map) + 1 if mlp_bank_map else 0
    for name, tensor in sd.items():
        if name == "qo_bank":
            ...
            for bank_id in range(num_mlp_banks):
                out[f"mlp_banks.{bank_id}.fc.weight"] = tensor[bank_id]
        elif name == "mlp_down_bank":
            for bank_id in range(num_mlp_banks):
                out[f"mlp_banks.{bank_id}.proj.weight"] = tensor[bank_id]
        else:
            out[name] = tensor
    return out
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    qo_slices = [None] * (2 * n)
    kv_slices = [None] * (2 * n)
    num_mlp_banks = int(template_sd["mlp_up_bank"].shape[0])
    up_slices = [template_sd["mlp_up_bank"][i] for i in range(num_mlp_banks)]
    down_slices = [template_sd["mlp_down_bank"][i] for i in range(num_mlp_banks)]
    consumed = set()
    for i in range(n):
        ...
        if vk in sd:
            ...
            consumed.add(vk)
    for bank_id in range(num_mlp_banks):
        fk = f"mlp_banks.{bank_id}.fc.weight"
        if fk in sd:
            up_slices[bank_id] = sd[fk]
            consumed.add(fk)
        dk = f"mlp_banks.{bank_id}.proj.weight"
        if dk in sd:
            down_slices[bank_id] = sd[dk]
            consumed.add(dk)
...
def late_protected_mlp_bank_ids(mlp_bank_map: list[int], late_layers: int) -> set[int]:
    if late_layers <= 0 or not mlp_bank_map:
        return set()
    start = max(len(mlp_bank_map) - late_layers, 0)
    return {mlp_bank_map[idx] for idx in range(start, len(mlp_bank_map))}


def mixed_quantize_int6(
    state_dict: dict[str, Tensor],
    int6_cats: set[str],
    late_mlp_bank_ids: set[int] | None = None,
):
    late_mlp_bank_ids = set() if late_mlp_bank_ids is None else late_mlp_bank_ids
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        protect_late_mlp = False
        if name.startswith("mlp_banks.") and name.endswith(".proj.weight"):
            protect_late_mlp = int(name.split(".")[1]) in late_mlp_bank_ids
        if cat in int6_cats and t.ndim >= 1 and not protect_late_mlp:
...
def run_smoke_test(args: Hyperparameters) -> None:
    shared_mlp_unshared_late = max(args.shared_mlp_unshared_late, args.late_mlp_int8_layers)
    model = GPT(
        ...
        shared_mlp_unshared_late=shared_mlp_unshared_late,
    ).float()
    batch_size = int(os.environ.get("SMOKE_BATCH_SIZE", 2))
    seq_len = int(os.environ.get("SMOKE_SEQ_LEN", 32))
    input_ids = torch.randint(args.vocab_size, (batch_size, seq_len), dtype=torch.int64)
    target_ids = torch.randint(args.vocab_size, (batch_size, seq_len), dtype=torch.int64)
    loss = model(input_ids, target_ids)
    loss.backward()
    sd_cpu = {k: v.detach().cpu() for k, v in model.state_dict().items() if "mtp_heads" not in k}
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers, model.mlp_bank_map)
    protected_banks = late_protected_mlp_bank_ids(model.mlp_bank_map, args.late_mlp_int8_layers)
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, protected_banks)
    ...
    reload_model.load_state_dict(deq_state, strict=True)
    with torch.no_grad():
        logits = reload_model.forward_logits(input_ids)
    print(
        "smoke_test:ok "
        f"loss:{loss.item():.4f} logits_shape:{tuple(logits.shape)} "
        f"shared_mlp_banks:{reload_model.num_mlp_banks} protected_banks:{sorted(protected_banks)}"
    )
...
def main() -> None:
    ...
    if np is None:
        missing_runtime_deps.append("numpy")
    if spm is None:
        missing_runtime_deps.append("sentencepiece")
    ...
    protected_mlp_bank_ids = late_protected_mlp_bank_ids(base_model.mlp_bank_map, args.late_mlp_int8_layers)
    ...
    log0(
        "shared_mlp:"
        f"group_size:{args.shared_mlp_group_size} "
        f"unshared_late:{shared_mlp_unshared_late} "
        f"unique_banks:{base_model.num_mlp_banks} map:{base_model.mlp_bank_map}"
    )
    log0(
        f"late_mlp_int8_layers:{args.late_mlp_int8_layers} "
        f"protected_banks:{sorted(protected_mlp_bank_ids)}"
    )
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers, base_model.mlp_bank_map)
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        late_mlp_bank_ids=protected_mlp_bank_ids,
    )
