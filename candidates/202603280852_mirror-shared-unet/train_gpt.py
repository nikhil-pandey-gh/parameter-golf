"""
Mirror-shared U-Net candidate: strong 11-layer stack with mirrored block sharing,
private per-layer control tensors, and LeakyReLU^2 MLPs.
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_gain: Tensor, use_xsa: bool = False) -> Tensor:
...
class SharedBlockCore(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, rope_dims=rope_dims)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_unique_blocks = (num_layers + 1) // 2 if mirror_share else num_layers
        self.layer_to_block = tuple(
            i if i < self.num_unique_blocks else num_layers - 1 - i for i in range(num_layers)
        )
        ...
        self.ln_scale_factors = tuple(1.0 / math.sqrt(i + 1) if ln_scale else 1.0 for i in range(num_layers))
        self.layer_uses_xsa = tuple(i >= max(0, num_layers - xsa_last_n) for i in range(num_layers))
    ...
    def _run_layer(self, x: Tensor, x0: Tensor, layer_idx: int) -> Tensor:
        core = self.shared_blocks[self.layer_to_block[layer_idx]]
        mix = self.resid_mixes[layer_idx].to(dtype=x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        s = self.ln_scale_factors[layer_idx]
        attn_out = core.attn(
            core.attn_norm(x) * s,
            self.q_gains[layer_idx],
            use_xsa=self.layer_uses_xsa[layer_idx],
        )
        x = x + self.attn_scales[layer_idx].to(dtype=x.dtype)[None, None, :] * attn_out
        x = x + self.mlp_scales[layer_idx].to(dtype=x.dtype)[None, None, :] * core.mlp(core.mlp_norm(x) * s)
        return x

    def _encode_decode(self, x: Tensor, x0: Tensor) -> Tensor:
        skips: list[Tensor] = []
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self._run_layer(x, x0, self.num_encoder_layers + i)
        return x
...
def main() -> None:
    ...
    scalar_params.extend([base_model.attn_scales, base_model.mlp_scales, base_model.resid_mixes, base_model.q_gains])
    ...
    log0(
        f"mirror_share:{args.mirror_share} unique_blocks:{base_model.num_unique_blocks} "
        f"layer_to_block:{','.join(str(i) for i in base_model.layer_to_block)}"
    )
