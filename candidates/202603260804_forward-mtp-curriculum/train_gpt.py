...
class Hyperparameters:
    ...
    mtp_curriculum_enabled = bool(int(os.environ.get("MTP_CURRICULUM_ENABLED", "1")))
    mtp_curriculum_start_frac = float(os.environ.get("MTP_CURRICULUM_START_FRAC", 0.10))
    mtp_curriculum_end_frac = float(os.environ.get("MTP_CURRICULUM_END_FRAC", 0.45))
...
class GPT(nn.Module):
    def __init__(
        ...
        mtp_curriculum_enabled: bool = False,
        mtp_curriculum_start_frac: float = 0.0,
        mtp_curriculum_end_frac: float = 1.0,
        ...
        self.mtp_curriculum_enabled = mtp_curriculum_enabled
        self.mtp_curriculum_start_frac = mtp_curriculum_start_frac
        self.mtp_curriculum_end_frac = mtp_curriculum_end_frac
        ...
        self.register_buffer("mtp_head_weights", torch.ones(mtp_num_heads, dtype=torch.float32), persistent=False)
        self.register_buffer("mtp_loss_scale", torch.tensor(1.0, dtype=torch.float32), persistent=False)
        if xsa_last_n > 0:
            for i in range(max(0, num_layers - xsa_last_n), num_layers):
                self.blocks[i].attn.use_xsa = True
        self._init_weights()
        self.set_mtp_progress(0.0 if self.mtp_curriculum_enabled else 1.0)
    ...
    @torch.no_grad()
    def set_mtp_progress(self, progress: float) -> tuple[int, float]:
        if self.mtp_num_heads <= 0:
            self.mtp_loss_scale.zero_()
            return 0, 0.0
        clamped_progress = float(min(max(progress, 0.0), 1.0))
        if not self.mtp_curriculum_enabled:
            head_weights = torch.ones_like(self.mtp_head_weights)
            loss_scale = 1.0
        else:
            ramp_span = max(self.mtp_curriculum_end_frac - self.mtp_curriculum_start_frac, 1e-6)
            ramp = min(max((clamped_progress - self.mtp_curriculum_start_frac) / ramp_span, 0.0), 1.0)
            phase = ramp * self.mtp_num_heads
            head_weights = torch.clamp(
                phase - torch.arange(
                    self.mtp_num_heads,
                    device=self.mtp_head_weights.device,
                    dtype=self.mtp_head_weights.dtype,
                ),
                min=0.0,
                max=1.0,
            )
        ...
        self.mtp_loss_scale.fill_(loss_scale)
        active_heads = int((head_weights > 0).sum().item())
        return active_heads, float(loss_scale)
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_active_heads: int | None = None) -> Tensor:
        n = self.num_layers
        ...
        active_heads = self.mtp_num_heads if mtp_active_heads is None else min(mtp_active_heads, self.mtp_num_heads)
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0 and active_heads > 0:
            _, seqlen, dim = x.shape
            mtp_loss_sum = x.new_zeros(())
            mtp_weight_sum = x.new_zeros(())
            head_weights = self.mtp_head_weights.to(dtype=main_loss.dtype)
            for k, mtp_head in enumerate(self.mtp_heads[:active_heads]):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
            ...
            main_loss = main_loss + (
                ...
                * self.mtp_loss_scale.to(dtype=main_loss.dtype)
                * (mtp_loss_sum / mtp_weight_sum.clamp_min(1e-6))
            )
        return main_loss
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_curriculum_enabled=args.mtp_curriculum_enabled,
        mtp_curriculum_start_frac=args.mtp_curriculum_start_frac,
        mtp_curriculum_end_frac=args.mtp_curriculum_end_frac,
    ...
    base_model.set_mtp_progress(0.0 if args.mtp_curriculum_enabled else 1.0)
    ...
    # Optimizer split:
    # - 4 parameter banks + MTP heads -> Muon (batched Newton-Schulz)
    ...
    matrix_params.extend([head.weight for head in base_model.mtp_heads])
    ...
    log0(
        ...
        f"mtp_params:{mtp_params} mtp_curriculum:{int(args.mtp_curriculum_enabled)} "
        f"mtp_curriculum_start:{args.mtp_curriculum_start_frac:.2f} "
        f"mtp_curriculum_end:{args.mtp_curriculum_end_frac:.2f}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            warmup_active_mtp_heads, _ = base_model.set_mtp_progress(0.0 if args.mtp_curriculum_enabled else 1.0)
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, warmup_active_mtp_heads)
    ...
    while True:
        ...
        mtp_progress = min(step / max(args.iterations - 1, 1), 1.0) if args.iterations > 1 else 1.0
        active_mtp_heads, mtp_scale = base_model.set_mtp_progress(mtp_progress)
        if active_mtp_heads != prev_mtp_active_heads or int(mtp_scale == 1.0) != int(prev_mtp_scale == 1.0):
            head_weights = ",".join(f"{w:.2f}" for w in base_model.mtp_head_weights.detach().cpu().tolist())
            log0(
                f"mtp_curriculum:step:{step} progress:{mtp_progress:.3f} "
                f"active_heads:{active_mtp_heads} loss_scale:{mtp_scale:.3f} "
                f"head_weights:[{head_weights}]"
            )
            prev_mtp_active_heads = active_mtp_heads
            prev_mtp_scale = mtp_scale
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, active_mtp_heads)
