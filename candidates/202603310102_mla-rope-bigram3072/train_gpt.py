...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if mla_latent_dim < 0:
            raise ValueError(f"mla_latent_dim must be non-negative, got {mla_latent_dim}")
        ...
        if mla_latent_dim > 0:
            self.c_k = None
            self.c_v = None
            self.c_kv_latent = CastedLinear(dim, mla_latent_dim, bias=False)
            self.k_from_latent = CastedLinear(mla_latent_dim, kv_dim, bias=False)
            self.v_from_latent = CastedLinear(mla_latent_dim, kv_dim, bias=False)
        else:
            self.c_k = CastedLinear(dim, kv_dim, bias=False)
            self.c_v = CastedLinear(dim, kv_dim, bias=False)
            self.c_kv_latent = None
            self.k_from_latent = None
            self.v_from_latent = None
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if self.c_kv_latent is not None and self.k_from_latent is not None and self.v_from_latent is not None:
            kv_latent = self.c_kv_latent(x)
            k = self.k_from_latent(kv_latent).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
            v = self.v_from_latent(kv_latent)
        else:
            if self.c_k is None or self.c_v is None:
                raise RuntimeError("Expected either latent KV projections or direct KV projections")
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, mla_latent_dim=mla_latent_dim)
...
def main() -> None:
    ...
    attention_mode = "mla_gqa" if args.mla_latent_dim > 0 else "gqa"
    log0(
        f"attention_mode:{attention_mode} num_heads:{args.num_heads} "
        f"num_kv_heads:{args.num_kv_heads} mla_latent_dim:{args.mla_latent_dim}"
    )
    log0(
