...
class Hyperparameters:
    ...
    qkv_conv_enabled = bool(int(os.environ.get("QKV_CONV_ENABLED", "1")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,q_conv,k_conv,v_conv",
    ).split(",")
    if pattern
)
...
class CausalDepthwiseConv1d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        yt = F.conv1d(xt, self.weight.to(dtype=xt.dtype), bias=None, groups=self.channels)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        qkv_conv_enabled: bool = False,
        qkv_conv_kernel: int = 3,
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.qkv_conv_enabled = qkv_conv_enabled
        if qkv_conv_enabled:
            kv_dim = num_kv_heads * self.head_dim
            self.q_conv = CausalDepthwiseConv1d(dim, qkv_conv_kernel)
            self.k_conv = CausalDepthwiseConv1d(kv_dim, qkv_conv_kernel)
            self.v_conv = CausalDepthwiseConv1d(kv_dim, qkv_conv_kernel)
        else:
            self.q_conv = None
            self.k_conv = None
            self.v_conv = None
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            ...
            qkv_conv_enabled=qkv_conv_enabled,
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        qkv_conv_enabled=args.qkv_conv_enabled,
    ...
    log0(f"qkv_conv:enabled={args.qkv_conv_enabled} kernel={args.qkv_conv_kernel}")
    ...
    eval_model = GPT(
        ...
        qkv_conv_enabled=args.qkv_conv_enabled, qkv_conv_kernel=args.qkv_conv_kernel,
    ).to(device).bfloat16()
