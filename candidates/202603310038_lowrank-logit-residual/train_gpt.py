...
class Hyperparameters:
    ...
    logit_resid_rank = int(os.environ.get("LOGIT_RESID_RANK", 16))
...
class LowRankLogitResidual(nn.Module):
    """Small output-only correction head that relaxes full weight tying at low byte cost."""
...
class GPT(nn.Module):
    def __init__(
        ...
        self.logit_residual = (
            LowRankLogitResidual(model_dim, vocab_size, logit_resid_rank)
            if logit_resid_rank > 0
            else None
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.logit_residual is not None:
            logits_proj = logits_proj + self.logit_residual(x_flat)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        if self.logit_residual is not None:
            logits_proj = logits_proj + self.logit_residual(x)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if name.startswith("logit_residual.") and t.is_floating_point():
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        logit_resid_rank=args.logit_resid_rank,
    ).to(device).bfloat16()
    ...
    if base_model.logit_residual is not None:
        scalar_params.append(base_model.logit_residual.down.weight)
        scalar_params.append(base_model.logit_residual.up.weight)
    ...
    logit_resid_params = (
        sum(p.numel() for p in base_model.logit_residual.parameters())
        if base_model.logit_residual is not None
        else 0
    )
    log0(f"model_params:{n_params}")
    log0(f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} mtp_params:{mtp_params}")
    log0(f"logit_resid_rank:{args.logit_resid_rank} logit_resid_params:{logit_resid_params}")
