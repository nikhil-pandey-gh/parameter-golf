...
class Hyperparameters:
    ...
    awq_batch_tokens = int(os.environ.get("AWQ_BATCH_TOKENS", 131_072))
    awq_calibration_batches = int(os.environ.get("AWQ_CALIBRATION_BATCHES", 8))
    awq_group_size = int(os.environ.get("AWQ_GROUP_SIZE", 64))
    awq_alpha_candidates = os.environ.get("AWQ_ALPHA_CANDIDATES", "0.0,0.25,0.5,0.75,1.0")
...
class AWQCalibrationStats:
    def __init__(self):
        self.sum_squares: dict[str, Tensor] = {}
    ...
    def add(self, name: str, x: Tensor) -> None:
        ...
        if x_flat.numel() == 0:
            return
        sum_sq = x_flat.square().sum(dim=0).cpu()
        if name in self.sum_squares:
            self.sum_squares[name] += sum_sq
            self.counts[name] += x_flat.shape[0]
        else:
            self.sum_squares[name] = sum_sq
            self.counts[name] = x_flat.shape[0]

    def rms(self, name: str) -> Tensor | None:
        sum_sq = self.sum_squares.get(name)
        count = self.counts.get(name, 0)
        if sum_sq is None or count <= 0:
            return None
        return (sum_sq / count).sqrt()

def _group_tensor_means(x: Tensor, group_size: int) -> Tensor:
    if group_size <= 0:
        raise ValueError(f"group_size must be positive, got {group_size}")
    return torch.stack([x[i : i + group_size].mean() for i in range(0, x.numel(), group_size)])

def _expand_group_scales(scales: Tensor, in_dim: int, group_size: int, device: torch.device) -> Tensor:
    return scales.to(device=device, dtype=torch.float32).repeat_interleave(group_size)[:in_dim]
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        awq_stats: AWQCalibrationStats | None = None,
    ) -> tuple[Tensor, Tensor | None]:
        bsz, seqlen, dim = x.shape
        if awq_stats is not None and layer_idx is not None:
            awq_stats.add(f"blocks.{layer_idx}.attn.c_q.weight", x)
            awq_stats.add(f"blocks.{layer_idx}.attn.c_k.weight", x)
            awq_stats.add(f"blocks.{layer_idx}.attn.c_v.weight", x)
        ...
        if awq_stats is not None and layer_idx is not None:
            awq_stats.add(f"blocks.{layer_idx}.attn.proj.weight", y)
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        if awq_stats is not None and layer_idx is not None:
            awq_stats.add(f"blocks.{layer_idx}.mlp.fc.weight", x)
        x = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=0.5)
        x = x.square()
        if awq_stats is not None and layer_idx is not None:
            awq_stats.add(f"blocks.{layer_idx}.mlp.proj.weight", x)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        attn_out, raw_v = self.attn(
            attn_in, q_w, k_w, v_w, out_w,
            v_embed=v_embed, v0=v0, layer_idx=self.layer_idx, awq_stats=awq_stats,
        )
        ...
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(
            mlp_in, up_w, down_w, layer_idx=self.layer_idx, awq_stats=awq_stats,
        )
...
class GPT(nn.Module):
    def __init__(
        ...
        self._awq_stats: AWQCalibrationStats | None = None
        self.value_residual = value_residual
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                v_embed=ve, v0=v0, awq_stats=awq_stats)
...
def collect_awq_calibration_stats(
    model: GPT,
    args: Hyperparameters,
    device: torch.device,
    log0,
) -> AWQCalibrationStats | None:
    if not args.awq_enabled or args.awq_calibration_batches <= 0:
        return None
    batch_tokens = max(args.train_seq_len, (args.awq_batch_tokens // args.train_seq_len) * args.train_seq_len)
    stats = AWQCalibrationStats()
    stream = TokenStream(args.train_files)
    prev_stats = model._awq_stats
    was_training = model.training
    model._awq_stats = stats
    model.eval()
    torch.cuda.synchronize()
    t0 = time.perf_counter()
    with torch.inference_mode():
        for _ in range(args.awq_calibration_batches):
            local = stream.take(batch_tokens + 1).to(device=device, dtype=torch.int64, non_blocking=True)
    ...
    model._awq_stats = prev_stats
    model.train(was_training)
    log0(
        f"awq:collected tensors:{len(stats.sum_squares)} "
        f"batches:{args.awq_calibration_batches} calibration_tokens:{batch_tokens * args.awq_calibration_batches} "
...
def quantize_int6_awq_per_row(
    t: Tensor,
    activation_rms: Tensor | None,
    group_size: int,
    alpha_candidates: tuple[float, ...],
    clip_range: int = 31,
) -> tuple[Tensor, Tensor, Tensor | None, float, float]:
    t32 = t.float()
    best_q, best_s = quantize_int6_per_row(t32, clip_range=clip_range)
    best_recon = best_q.float() * best_s.float()[:, None] if t32.ndim == 2 else best_q.float() * float(best_s.item())
    best_err = (t32 - best_recon).pow(2).mean().item()
    baseline_err = best_err
    best_group_scales: Tensor | None = None
    best_alpha = 0.0
    if t32.ndim != 2 or activation_rms is None or activation_rms.numel() != t32.shape[1]:
        return best_q, best_s, None, best_alpha, 0.0
    group_rms = _group_tensor_means(activation_rms.float().clamp_min(1e-6), group_size)
    for alpha in alpha_candidates:
        if alpha <= 0.0:
            continue
        group_scales = group_rms.pow(alpha)
        group_scales = group_scales / group_scales.log().mean().exp()
        group_scales = group_scales.clamp_(0.5, 2.0).to(torch.float16)
        full_scales = _expand_group_scales(group_scales, t32.shape[1], group_size, t32.device)
        q, s = quantize_int6_per_row(t32 * full_scales[None, :], clip_range=clip_range)
        recon = (q.float() * s.float()[:, None]) / full_scales[None, :]
        err = (t32 - recon).pow(2).mean().item()
        if err < best_err:
            best_q, best_s = q, s
            best_err = err
            best_group_scales = group_scales.cpu()
            best_alpha = alpha
    return best_q, best_s, best_group_scales, best_alpha, baseline_err - best_err
...
def mixed_quantize_int6(
    ...
    awq_alpha_candidates: tuple[float, ...] = (0.0, 0.25, 0.5, 0.75, 1.0),
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            act_rms = awq_stats.rms(name) if awq_stats is not None else None
            q, s, awq_scale, awq_alpha, awq_gain = quantize_int6_awq_per_row(
                t,
                act_rms,
            ...
            if awq_scale is not None:
                ...
                info["awq_group_size"] = awq_group_size
                info["awq_alpha"] = awq_alpha
                awq_applied += 1
                awq_gain_sum += awq_gain
                awq_alpha_hist[awq_alpha] = awq_alpha_hist.get(awq_alpha, 0) + 1
            meta[name] = info
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    meta["__awq_summary__"] = {
        "applied_tensors": awq_applied,
        "mean_mse_gain": awq_gain_sum / max(awq_applied, 1),
        "alpha_hist": awq_alpha_hist,
    }
    return result, meta
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and "awq_group_size" in info and name + ".awq_scale" in result and q.ndim == 2:
            awq_scale = result[name + ".awq_scale"]
            full_scale = _expand_group_scales(awq_scale, q.shape[1], int(info["awq_group_size"]), q.device)
            recon = recon / full_scale[None, :]
        if s.ndim > 0:
...
def main() -> None:
    ...
    if master_process:
        awq_stats = collect_awq_calibration_stats(base_model, args, device, log0)
        quant_result, quant_meta = mixed_quantize_int6(
            unbanked_sd,
            {"mlp", "attn"},
            awq_stats=awq_stats,
            awq_group_size=args.awq_group_size,
            awq_alpha_candidates=_parse_float_list(args.awq_alpha_candidates),
        )
        awq_summary = quant_meta.get("__awq_summary__", {})
        log0(
            f"awq:applied_tensors:{awq_summary.get('applied_tensors', 0)} "
            f"mean_mse_gain:{awq_summary.get('mean_mse_gain', 0.0):.6e} "
            f"group_size:{args.awq_group_size} alpha_hist:{awq_summary.get('alpha_hist', {})}"
        )
