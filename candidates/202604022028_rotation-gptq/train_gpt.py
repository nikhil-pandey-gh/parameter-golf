...
class Hyperparameters:
    ...
    rotation_int6 = bool(int(os.environ.get("ROTATION_INT6", "1")))
    rotation_block_size = int(os.environ.get("ROTATION_BLOCK_SIZE", 512))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if flash_attn_3_func is None:
            raise RuntimeError("flash_attn_interface is required to run this candidate")
...
def _get_hadamard(block_size: int) -> Tensor:
    if block_size <= 0 or block_size & (block_size - 1):
        raise ValueError(f"Hadamard block size must be a positive power of two, got {block_size}")
    ...
    _HADAMARD_CACHE[block_size] = h
    return h

def _can_rotate_columns(t: Tensor, block_size: int) -> bool:
    return t.ndim == 2 and block_size > 0 and t.shape[1] >= block_size and t.shape[1] % block_size == 0

def _apply_block_hadamard_columns(t: Tensor, block_size: int) -> Tensor:
    if not _can_rotate_columns(t, block_size):
        return t
    h = _get_hadamard(block_size).to(device=t.device, dtype=torch.float32)
    blocks = t.float().reshape(t.shape[0], -1, block_size)
    return torch.matmul(blocks, h).reshape_as(t)
...
def _quantize_int6_per_row_with_err(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, float]:
    t32 = t.float()
    if t32.ndim == 2:
    ...
    err = (t32 - _dequantize_rowwise(q, scale)).pow(2).mean().item()
    return q, scale, err

def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    q, s, _ = _quantize_int6_per_row_with_err(t, clip_range)
    return q, s

def quantize_int6_with_optional_rotation(
    t: Tensor,
    block_size: int,
    clip_range: int = 31,
) -> tuple[Tensor, Tensor, dict[str, object]]:
    q_plain, s_plain, plain_err = _quantize_int6_per_row_with_err(t, clip_range)
    if not _can_rotate_columns(t, block_size):
        return q_plain, s_plain, {"type": "int6"}
    rotated = _apply_block_hadamard_columns(t, block_size)
    q_rot, s_rot, _ = _quantize_int6_per_row_with_err(rotated, clip_range)
    recon_rot = _apply_block_hadamard_columns(_dequantize_rowwise(q_rot, s_rot), block_size)
    rot_err = (t.float() - recon_rot).pow(2).mean().item()
    if rot_err < plain_err:
        return q_rot, s_rot, {"type": "int6", "rotation": "hadamard", "block_size": block_size}
    return q_plain, s_plain, {"type": "int6"}
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], rotation_int6: bool, rotation_block_size: int):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            if rotation_int6:
                q, s, info = quantize_int6_with_optional_rotation(t, rotation_block_size)
            else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("rotation") == "hadamard":
            deq = _apply_block_hadamard_columns(deq, int(info["block_size"]))
...
def main() -> None:
    ...
    log0(f"rotation_int6:{args.rotation_int6} rotation_block_size:{args.rotation_block_size}")
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        rotation_int6=args.rotation_int6,
    ...
    if rotated_tensors:
        log0(
            f"rotation_int6:selected {len(rotated_tensors)} tensors "
            f"sample:{rotated_tensors[:6]}"
        )
    else:
        log0("rotation_int6:selected 0 tensors")
