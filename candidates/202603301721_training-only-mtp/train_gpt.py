...
class Hyperparameters:
    ...
    mtp_init_from_embed = bool(int(os.environ.get("MTP_INIT_FROM_EMBED", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_init_from_embed = mtp_init_from_embed and tie_embeddings
    ...
    def _init_weights(self) -> None:
        ...
        # Seeding auxiliary heads from the tied embedding makes the extra future-token
        # objective informative from the first update instead of waiting for zero-init heads
        # to learn a usable lexical projection.
        if self.mtp_init_from_embed:
            for head in self.mtp_heads:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_init_from_embed=args.mtp_init_from_embed,
    ...
    # Optimizer split:
    # - 4 parameter banks + training-only MTP heads -> Muon (batched Newton-Schulz)
    ...
    log0(
        ...
        f"mtp_init_from_embed:{args.mtp_init_from_embed} mtp_params:{mtp_params}"
    )
