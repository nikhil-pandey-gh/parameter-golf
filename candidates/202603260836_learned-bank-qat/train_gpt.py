...
class Hyperparameters:
    ...
    bank_qat_min_mult = float(os.environ.get("BANK_QAT_MIN_MULT", 0.5))
    bank_qat_max_mult = float(os.environ.get("BANK_QAT_MAX_MULT", 1.5))
...
def _ste_fake_quantize_int6_per_row(
    w: Tensor,
    clip_mult: Tensor | float = 1.0,
    mix: Tensor | float = 1.0,
    clip_range: int = 31,
) -> Tensor:
    if w.ndim != 2:
        return w
    w32 = w.float()
    clip_mult_t = torch.as_tensor(clip_mult, device=w.device, dtype=w32.dtype)
    mix_t = torch.as_tensor(mix, device=w.device, dtype=w32.dtype)
    row_clip = (w32.abs().amax(dim=1, keepdim=True) * clip_mult_t).clamp_min(1.0 / clip_range)
    scale = (row_clip / clip_range).clamp_min(1.0 / clip_range)
    w_q = torch.clamp(torch.round(w32 / scale), -clip_range, clip_range) * scale
    return w + mix_t * (w_q.to(dtype=w.dtype) - w).detach()
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        bank_qat_min_mult: float = 0.5,
        bank_qat_max_mult: float = 1.5,
    ):
        super().__init__()
        ...
        self.bank_qat_min_mult = bank_qat_min_mult
        self.bank_qat_max_mult = bank_qat_max_mult
    ...
    def _bank_qat_clip_multiplier(self, log_scale: Tensor) -> Tensor:
        return torch.clamp(log_scale.exp(), min=self.bank_qat_min_mult, max=self.bank_qat_max_mult)
    def _bank_slice(self, bank: Tensor, idx: int, log_scales: Tensor) -> Tensor:
        w = bank[idx]
        if not self.training or not self.bank_qat_active:
            return w
        clip_mult = self._bank_qat_clip_multiplier(log_scales[idx])
        return _ste_fake_quantize_int6_per_row(w, clip_mult=clip_mult, mix=self.bank_qat_mix)
    def export_qat_clip_multipliers(self) -> dict[str, float]:
        n = self.num_layers
        qo = self._bank_qat_clip_multiplier(self.qo_qat_log_scale.detach().float()).cpu()
        kv = self._bank_qat_clip_multiplier(self.kv_qat_log_scale.detach().float()).cpu()
        up = self._bank_qat_clip_multiplier(self.mlp_up_qat_log_scale.detach().float()).cpu()
        down = self._bank_qat_clip_multiplier(self.mlp_down_qat_log_scale.detach().float()).cpu()
        clip_multipliers: dict[str, float] = {}
        for i in range(n):
            clip_multipliers[f"blocks.{i}.attn.c_q.weight"] = float(qo[i].item())
            clip_multipliers[f"blocks.{i}.attn.proj.weight"] = float(qo[n + i].item())
            clip_multipliers[f"blocks.{i}.attn.c_k.weight"] = float(kv[i].item())
            clip_multipliers[f"blocks.{i}.attn.c_v.weight"] = float(kv[n + i].item())
            clip_multipliers[f"blocks.{i}.mlp.fc.weight"] = float(up[i].item())
            clip_multipliers[f"blocks.{i}.mlp.proj.weight"] = float(down[i].item())
        return clip_multipliers
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            q_w = self._bank_slice(self.qo_bank, i, self.qo_qat_log_scale)
            k_w = self._bank_slice(self.kv_bank, i, self.kv_qat_log_scale)
            v_w = self._bank_slice(self.kv_bank, n + i, self.kv_qat_log_scale)
            out_w = self._bank_slice(self.qo_bank, n + i, self.qo_qat_log_scale)
            up_w = self._bank_slice(self.mlp_up_bank, i, self.mlp_up_qat_log_scale)
            down_w = self._bank_slice(self.mlp_down_bank, i, self.mlp_down_qat_log_scale)
        ...
        for i in range(self.num_decoder_layers):
            ...
            q_w = self._bank_slice(self.qo_bank, bi, self.qo_qat_log_scale)
            k_w = self._bank_slice(self.kv_bank, bi, self.kv_qat_log_scale)
            v_w = self._bank_slice(self.kv_bank, n + bi, self.kv_qat_log_scale)
            out_w = self._bank_slice(self.qo_bank, n + bi, self.qo_qat_log_scale)
            up_w = self._bank_slice(self.mlp_up_bank, bi, self.mlp_up_qat_log_scale)
            down_w = self._bank_slice(self.mlp_down_bank, bi, self.mlp_down_qat_log_scale)
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, clip_range: int = 31, clip_mult: float = 1.0) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            row_clip = (row_clip * clip_mult).clamp_min(1.0 / clip_range)
    ...
    amax = t32.abs().max().item() * clip_mult
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            clip_mult = clip_multipliers.get(name, 1.0) if clip_multipliers is not None else 1.0
            q, s = quantize_int6_per_row(t, clip_mult=clip_mult)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_qat_min_mult=args.bank_qat_min_mult,
        bank_qat_max_mult=args.bank_qat_max_mult,
    ).to(device).bfloat16()
    ...
    log0(
        f"bank_qat:threshold={args.late_qat_threshold:.3f} "
        f"clip_range=[{args.bank_qat_min_mult:.2f},{args.bank_qat_max_mult:.2f}]"
    )
    log0(
    ...
    while True:
        ...
        if args.late_qat_threshold > 0:
            bank_qat_mix = max(0.0, min(1.0, 1.0 - (scale / args.late_qat_threshold)))
        if bank_qat_mix > 0.0:
            if not base_model.bank_qat_active:
                base_model.bank_qat_active = True
                CastedLinear._qat_enabled = True
                compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
                model = compiled_model
            base_model.bank_qat_mix.fill_(bank_qat_mix)
            if not bank_qat_logged:
                bank_qat_logged = True
                log0(f"late_qat:enabled step:{step} scale:{scale:.4f} bank_qat_mix:{bank_qat_mix:.3f}")
        zero_grad_all()
    ...
    clip_multipliers = base_model.export_qat_clip_multipliers()
    if master_process and clip_multipliers:
        clip_vals = list(clip_multipliers.values())
        log0(
            f"bank_qat_clip_multipliers min:{min(clip_vals):.3f} "
            f"mean:{sum(clip_vals) / len(clip_vals):.3f} max:{max(clip_vals):.3f}"
        )
