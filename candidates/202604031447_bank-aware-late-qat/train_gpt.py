...
class Hyperparameters:
    ...
    bank_qat_enabled = bool(int(os.environ.get("BANK_QAT_ENABLED", "1")))
    bank_qat_recompile = bool(int(os.environ.get("BANK_QAT_RECOMPILE", "1")))
    bank_qat_mult_min = float(os.environ.get("BANK_QAT_MULT_MIN", 0.50))
    bank_qat_mult_max = float(os.environ.get("BANK_QAT_MULT_MAX", 1.50))
    bank_qat_grad_scale = float(os.environ.get("BANK_QAT_GRAD_SCALE", 1.0))
...
def fake_quantize_int6_per_row_lsq(
    w: Tensor,
    log_mult: Tensor | None = None,
    *,
    clip_range: int = 31,
    mult_min: float = 0.50,
    mult_max: float = 1.50,
    ...
    if w.ndim != 2:
        return w
    w32 = w.float()
    base_scale = (w32.detach().abs().amax(dim=1) / clip_range).clamp_min(1.0 / clip_range)
    if log_mult is None:
        scale = base_scale
    else:
        mult = log_mult.float().exp().clamp(min=mult_min, max=mult_max)
        mult = grad_scale(mult, grad_scale_factor)
        scale = (base_scale * mult).clamp_min(1.0 / clip_range)
    q = torch.clamp(round_ste(w32 / scale[:, None]), -clip_range, clip_range)
    return (q * scale[:, None]).to(dtype=w.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_dims: int = 0,
        ln_scale: bool = False,
        bank_qat_enabled: bool = False,
        bank_qat_mult_min: float = 0.50,
        bank_qat_mult_max: float = 1.50,
        bank_qat_grad_scale: float = 1.0,
        bank_qat_trainable: bool = True,
        dtg: bool = False,
        ...
        self.bank_qat_enabled = bank_qat_enabled
        self.bank_qat_mult_min = bank_qat_mult_min
        self.bank_qat_mult_max = bank_qat_mult_max
        self.bank_qat_grad_scale = bank_qat_grad_scale
        if bank_qat_trainable:
            self.qo_bank_qat_log_scale = nn.Parameter(torch.zeros(2 * num_layers, model_dim, dtype=torch.float32))
            self.kv_bank_qat_log_scale = nn.Parameter(torch.zeros(2 * num_layers, kv_dim, dtype=torch.float32))
            self.mlp_up_bank_qat_log_scale = nn.Parameter(torch.zeros(num_layers, mlp_dim, dtype=torch.float32))
            self.mlp_down_bank_qat_log_scale = nn.Parameter(torch.zeros(num_layers, model_dim, dtype=torch.float32))
        else:
            self.qo_bank_qat_log_scale = None
            self.kv_bank_qat_log_scale = None
            self.mlp_up_bank_qat_log_scale = None
            self.mlp_down_bank_qat_log_scale = None
    ...
    def set_bank_qat_enabled(self, enabled: bool) -> None:
        self.bank_qat_enabled = enabled
    def _maybe_qat_bank_weight(self, w: Tensor, log_scale: Tensor | None) -> Tensor:
        if not (self.training and self.bank_qat_enabled):
            return w
        grad_scale_factor = self.bank_qat_grad_scale / math.sqrt(max(w.shape[1] * 31, 1))
        return fake_quantize_int6_per_row_lsq(
            w,
            log_scale,
            clip_range=31,
            mult_min=self.bank_qat_mult_min,
            mult_max=self.bank_qat_mult_max,
            grad_scale_factor=grad_scale_factor,
        )
    def _bank_weight(self, bank: Tensor, idx: int, log_scale_bank: Tensor | None) -> Tensor:
        log_scale = None if log_scale_bank is None else log_scale_bank[idx]
        return self._maybe_qat_bank_weight(bank[idx], log_scale)
    def export_bank_qat_row_mults(self) -> dict[str, Tensor]:
        if self.qo_bank_qat_log_scale is None:
            return {}
        def clamped_mult(log_scale_bank: Tensor) -> Tensor:
            return log_scale_bank.detach().float().exp().clamp(
                min=self.bank_qat_mult_min,
                max=self.bank_qat_mult_max,
            ).cpu()
        qo = clamped_mult(self.qo_bank_qat_log_scale)
        kv = clamped_mult(self.kv_bank_qat_log_scale)
        up = clamped_mult(self.mlp_up_bank_qat_log_scale)
        down = clamped_mult(self.mlp_down_bank_qat_log_scale)
        n = self.num_layers
        out: dict[str, Tensor] = {}
        for i in range(n):
            out[f"blocks.{i}.attn.c_q.weight"] = qo[i]
            out[f"blocks.{i}.attn.proj.weight"] = qo[n + i]
            out[f"blocks.{i}.attn.c_k.weight"] = kv[i]
            out[f"blocks.{i}.attn.c_v.weight"] = kv[n + i]
            out[f"blocks.{i}.mlp.fc.weight"] = up[i]
            out[f"blocks.{i}.mlp.proj.weight"] = down[i]
        return out
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            q_w = self._bank_weight(self.qo_bank, i, self.qo_bank_qat_log_scale)
            k_w = self._bank_weight(self.kv_bank, i, self.kv_bank_qat_log_scale)
            v_w = self._bank_weight(self.kv_bank, n + i, self.kv_bank_qat_log_scale)
            out_w = self._bank_weight(self.qo_bank, n + i, self.qo_bank_qat_log_scale)
            up_w = self._bank_weight(self.mlp_up_bank, i, self.mlp_up_bank_qat_log_scale)
            down_w = self._bank_weight(self.mlp_down_bank, i, self.mlp_down_bank_qat_log_scale)
        ...
        for i in range(self.num_decoder_layers):
            ...
            q_w = self._bank_weight(self.qo_bank, bi, self.qo_bank_qat_log_scale)
            k_w = self._bank_weight(self.kv_bank, bi, self.kv_bank_qat_log_scale)
            v_w = self._bank_weight(self.kv_bank, n + bi, self.kv_bank_qat_log_scale)
            out_w = self._bank_weight(self.qo_bank, n + bi, self.qo_bank_qat_log_scale)
            up_w = self._bank_weight(self.mlp_up_bank, bi, self.mlp_up_bank_qat_log_scale)
            down_w = self._bank_weight(self.mlp_down_bank, bi, self.mlp_down_bank_qat_log_scale)
...
def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = 31,
    row_mult: Tensor | None = None,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        row_mult_f = row_mult.float() if row_mult is not None else None
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            if pct < 1.0:
                row_clip = torch.quantile(t32.abs(), pct, dim=1)
            else:
                row_clip = t32.abs().amax(dim=1)
            if row_mult_f is not None:
                row_clip = row_clip * row_mult_f
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    row_mult_overrides: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            row_mult = None if row_mult_overrides is None else row_mult_overrides.get(name)
            q, s = quantize_int6_per_row(t, row_mult=row_mult)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_qat_enabled=False,
        bank_qat_mult_min=args.bank_qat_mult_min,
        bank_qat_mult_max=args.bank_qat_mult_max,
        bank_qat_grad_scale=args.bank_qat_grad_scale,
        bank_qat_trainable=args.bank_qat_enabled,
    ...
    for qat_scale in (
        base_model.qo_bank_qat_log_scale,
        base_model.kv_bank_qat_log_scale,
        base_model.mlp_up_bank_qat_log_scale,
        base_model.mlp_down_bank_qat_log_scale,
    ):
        if qat_scale is not None:
            scalar_params.append(qat_scale)
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            if args.bank_qat_enabled:
                base_model.set_bank_qat_enabled(True)
            if args.bank_qat_recompile:
                compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
                model = compiled_model
                log0(f"late_qat:recompiled step:{step} scale:{scale:.4f}")
            log0(
                f"late_qat:enabled step:{step} scale:{scale:.4f} "
                f"bank_qat:{int(args.bank_qat_enabled)}"
            )
        zero_grad_all()
    ...
    bank_qat_row_mults = base_model.export_bank_qat_row_mults() if args.bank_qat_enabled else {}
    export_sd = {
        k: v
        for k, v in full_state_dict.items()
        if "mtp_heads" not in k and "qat_log_scale" not in k
    }
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_heads" in k)
    excluded_qat = sum(int(t.numel()) for k, t in full_state_dict.items() if "qat_log_scale" in k)
    if excluded_mtp > 0:
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        row_mult_overrides=bank_qat_row_mults,
    )
    ...
    eval_model = GPT(
        ...
        bank_qat_enabled=False, bank_qat_trainable=False,
        dtg=args.dtg_enabled,
