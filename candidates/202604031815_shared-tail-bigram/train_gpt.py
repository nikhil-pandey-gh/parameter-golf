...
def find_repo_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if (candidate / "data").is_dir() and (candidate / "README.md").is_file():
            return candidate
...
class Hyperparameters:
    ...
    shared_tail_groups = int(os.environ.get("SHARED_TAIL_GROUPS", 2))
...
def build_layer_bank_map(
    num_layers: int,
    shared_tail_layers: int,
    shared_tail_groups: int,
) -> tuple[list[int], int]:
    if shared_tail_layers <= 1 or shared_tail_groups <= 0:
        return list(range(num_layers)), num_layers
    shared_tail_layers = min(shared_tail_layers, num_layers)
    shared_tail_groups = min(shared_tail_groups, shared_tail_layers)
    tail_start = num_layers - shared_tail_layers
    layer_bank_map = list(range(num_layers))
    for layer_idx in range(tail_start, num_layers):
        layer_bank_map[layer_idx] = tail_start + ((layer_idx - tail_start) % shared_tail_groups)
    return layer_bank_map, tail_start + shared_tail_groups
...
class GPT(nn.Module):
    def __init__(
        ...
        layer_bank_map, num_bank_layers = build_layer_bank_map(num_layers, shared_tail_layers, shared_tail_groups)
        self.layer_bank_map = tuple(layer_bank_map)
        self.num_bank_layers = num_bank_layers
        self.shared_tail_layers = shared_tail_layers
        self.shared_tail_groups = shared_tail_groups
        self.qo_bank = nn.Parameter(torch.empty(2 * num_bank_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * num_bank_layers, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(num_bank_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(num_bank_layers, model_dim, mlp_dim))
    ...
    def _bank_idx(self, layer_idx: int) -> int:
        return self.layer_bank_map[layer_idx]
...
def _rebank_state_dict(sd: dict[str, Tensor], template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    out: dict[str, Tensor] = {}
    n = template_sd["mlp_up_bank"].shape[0]
...
def build_export_config(args: Hyperparameters, model: GPT) -> dict[str, object]:
    return {
        ...
        "shared_tail_groups": args.shared_tail_groups,
        "layer_bank_map": list(model.layer_bank_map),
        "num_bank_layers": model.num_bank_layers,
...
def main() -> None:
    ...
    log0(
        f"shared_tail_layers:{args.shared_tail_layers} shared_tail_groups:{args.shared_tail_groups} "
        f"num_bank_layers:{base_model.num_bank_layers} layer_bank_map:{list(base_model.layer_bank_map)}"
    )
    ...
    export_config = build_export_config(args, base_model)
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_heads" in k)
    if excluded_mtp > 0:
        log0(f"export_excluding_mtp_params:{excluded_mtp}")
    if master_process:
        torch.save({"state_dict": export_sd, "config": export_config}, "final_model.pt")
    ...
    torch.save({"w": quant_result, "m": quant_meta, "config": export_config}, quant_buf)
    ...
    quant_config = quant_state.get("config")
    if quant_config is not None and list(quant_config.get("layer_bank_map", [])) != list(base_model.layer_bank_map):
        raise ValueError(
            "Quantized artifact layer_bank_map does not match the current model config: "
            f"saved={quant_config.get('layer_bank_map')} current={list(base_model.layer_bank_map)}"
        )
