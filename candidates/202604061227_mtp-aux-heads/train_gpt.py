...
def main() -> None:
    ...
    mtp_matrix_params = [head.weight for head in base_model.mtp_heads]
    optimizer_mtp = None
    if mtp_matrix_params:
        optimizer_mtp = torch.optim.AdamW(
            [{"params": mtp_matrix_params, "lr": args.mtp_head_lr, "base_lr": args.mtp_head_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=args.adam_wd,
            fused=True,
    ...
    replicated_params.extend(mtp_matrix_params)
    ...
    while True:
        ...
        # Phase 2: All-reduce non-bank grads, clip the synced replicated grads, then step Adam.
        if distributed:
            for p in replicated_params:
                if p.grad is not None:
                    dist.all_reduce(p.grad, op=dist.ReduceOp.AVG)
        if args.grad_clip_norm > 0:
            clip_params = replicated_params if distributed else list(base_model.parameters())
