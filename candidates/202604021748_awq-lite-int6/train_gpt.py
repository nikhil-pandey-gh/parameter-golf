...
def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = 31,
    input_second_moment: Tensor | None = None,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        act_weight = None
        if input_second_moment is not None and input_second_moment.numel() == t32.shape[1]:
            act_weight = input_second_moment.to(dtype=torch.float32, device=t32.device).clamp_min(1e-8)
            act_weight = act_weight / act_weight.mean().clamp_min(1e-8)
        ...
        for pct in INT6_CLIP_CANDIDATES:
            ...
            err = (
                (err_mat * act_weight[None, :]).mean().item()
                if act_weight is not None
                else err_mat.mean().item()
            )
            if err < best_err:
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    activation_second_moment: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(
                t,
                input_second_moment=None if activation_second_moment is None else activation_second_moment.get(name),
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
...
def collect_activation_second_moments(
    ...
    def make_hook(weight_name: str):
        def hook(_module: nn.Module, inputs: tuple[Tensor, ...]) -> None:
            ...
            x2 = x.square().sum(dim=dims)
            count = float(x.numel() // x.shape[-1])
            if weight_name in sums:
                sums[weight_name] = sums[weight_name] + x2
                counts[weight_name] = counts[weight_name] + count
            else:
                sums[weight_name] = x2
                counts[weight_name] = torch.tensor(count, device=x.device, dtype=torch.float32)
        return hook
    for name, module in model.named_modules():
        if isinstance(module, CastedLinear) and "mtp_heads" not in name:
            hooks.append(module.register_forward_pre_hook(make_hook(f"{name}.weight")))
    was_training = model.training
    model.eval()
    global_tokens_per_step = max(args.awq_batch_seqs, 1) * args.train_seq_len * max(world_size, 1)
    steps = max(1, math.ceil(args.awq_calibration_tokens / global_tokens_per_step))
    tokens_seen = 0
    with torch.inference_mode():
        for _ in range(steps):
            x, _ = loader.next_batch(global_tokens_per_step, args.train_seq_len, 1)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
...
def main() -> None:
    ...
    awq_stats, awq_tokens = collect_activation_second_moments(base_model, args, rank, world_size, device)
    torch.cuda.synchronize()
    log0(
        f"awq_calibration enabled:{args.awq_enabled} layers:{len(awq_stats)} "
        f"tokens:{awq_tokens} eval_time:{1000.0 * (time.perf_counter() - t_awq):.0f}ms"
    )
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu,
        {"mlp", "attn"},
        activation_second_moment=awq_stats,
    )
