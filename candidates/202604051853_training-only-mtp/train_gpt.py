...
def find_repo_root() -> Path:
    ...
    for candidate in script_path.parents:
        if (candidate / "data" / "tokenizers").exists():
            return candidate
    raise FileNotFoundError("Could not locate repository root with data/tokenizers/")
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.tie_embeddings and self.mtp_num_heads > 0:
            # Start training-only auxiliary heads from the same tied vocab geometry.
            for head in self.mtp_heads:
