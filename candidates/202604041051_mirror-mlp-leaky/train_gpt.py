...
class GPT(nn.Module):
    def __init__(
        ...
        self.mirror_shared_mlp_pairs: tuple[tuple[int, int], ...] = ()
        ...
        if self.mirror_share_mlp:
            ...
            for i in range(self.num_encoder_layers):
                ...
                mirror_pairs.append((i, mirror_idx))
            self.mirror_shared_mlp_pairs = tuple(mirror_pairs)
        if rope_dims > 0:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    return out
def dedupe_shared_state_dict(state_dict: dict[str, Tensor]) -> tuple[dict[str, Tensor], dict[str, str]]:
...
def restore_shared_state_dict_aliases(state_dict: dict[str, Tensor], aliases: dict[str, str]) -> dict[str, Tensor]:
    if not aliases:
        return state_dict
...
def main() -> None:
    ...
    if base_model.mirror_shared_mlp_pairs:
        log0(f"mirror_share_mlp:pairs:{','.join(f'{a}<->{b}' for a, b in base_model.mirror_shared_mlp_pairs)}")
    else:
        log0("mirror_share_mlp:disabled")
    ...
    export_sd, export_aliases = dedupe_shared_state_dict(export_sd)
    if export_aliases:
        log0(f"export_shared_aliases:{len(export_aliases)}")
    if master_process:
        torch.save(restore_shared_state_dict_aliases(export_sd, export_aliases), "final_model.pt")
    ...
    deq_state = restore_shared_state_dict_aliases(deq_state, quant_state.get("aliases", {}))
    eval_model = GPT(
        ...
        leaky_relu_slope=args.leaky_relu_slope, mirror_share_mlp=args.mirror_share_mlp,
    ).to(device).bfloat16()
