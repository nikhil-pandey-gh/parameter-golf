...
class Hyperparameters:
    ...
    layerdrop_start = float(os.environ.get("LAYERDROP_START", 0.0))
    layerdrop_end = float(os.environ.get("LAYERDROP_END", 0.18))
    layerdrop_warmup_steps = int(os.environ.get("LAYERDROP_WARMUP_STEPS", 1000))
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_dim: int = 128,
        ve_layers: str = "9,10",
        layerdrop_start: float = 0.0,
        layerdrop_end: float = 0.0,
    ):
        super().__init__()
        ...
        self.layerdrop_probs = tuple(float(x) for x in torch.linspace(layerdrop_start, layerdrop_end, steps=num_layers).tolist())
        self.layerdrop_scale = 0.0
        if rope_dims > 0:
    ...
    def set_layerdrop_scale(self, value: float) -> None:
        self.layerdrop_scale = float(value)

    @torch.compiler.disable
    def _should_drop_layer(self, layer_idx: int) -> bool:
        if not self.training:
            return False
        drop_prob = self.layerdrop_probs[layer_idx] * self.layerdrop_scale
        return drop_prob > 0.0 and random.random() < drop_prob
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            if not self._should_drop_layer(i):
...
def main() -> None:
    ...
    use_external_fa3 = flash_attn_3_func is not None
    enable_flash_sdp(not use_external_fa3)
    enable_mem_efficient_sdp(not use_external_fa3)
    enable_math_sdp(not use_external_fa3)
    logfile = None
    if master_process:
    ...
    base_model = GPT(
        ...
        layerdrop_start=args.layerdrop_start,
        layerdrop_end=args.layerdrop_end,
    ).to(device).bfloat16()
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
    restore_low_dim_params_to_fp32(base_model)
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=args.layerdrop_end <= 0.0)
    ...
    log0(
        f"layerdrop:start:{args.layerdrop_start:.3f} end:{args.layerdrop_end:.3f} "
        f"warmup_steps:{args.layerdrop_warmup_steps}"
    )
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        f"sdp_backends:cudnn=False flash={int(not use_external_fa3)} "
        f"mem_efficient={int(not use_external_fa3)} math={int(not use_external_fa3)}"
    )
    log0(f"attention_impl:{'flash_attn_3' if use_external_fa3 else 'torch_sdpa_fallback'}")
    ...
    if args.warmup_steps > 0:
        base_model.set_layerdrop_scale(0.0)
    ...
    while True:
        ...
        layerdrop_scale = 1.0 if args.layerdrop_warmup_steps <= 0 else min(step / args.layerdrop_warmup_steps, 1.0)
        base_model.set_layerdrop_scale(layerdrop_scale)
        zero_grad_all()
    ...
    eval_model = GPT(
        ...
        mlp_leak=args.mlp_leak,
        layerdrop_start=0.0, layerdrop_end=0.0,
    ).to(device).bfloat16()
