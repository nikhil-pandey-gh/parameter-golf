...
class Hyperparameters:
    ...
    mtp_anneal_scale = float(os.environ.get("MTP_ANNEAL_SCALE", 0.25))
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and w.ndim == 2:
            ...
            w = w + (w_q - w).detach() * self.qat_enabled.to(dtype=x.dtype)
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, w, bias)
def set_casted_linear_qat(module: nn.Module, enabled: bool) -> None:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + self.mtp_loss_weight_buffer.to(dtype=main_loss.dtype) * (
...
def main() -> None:
    ...
    set_casted_linear_qat(base_model, qat_enabled)
    ...
    log0(
        ...
        f"mtp_anneal_scale:{args.mtp_anneal_scale} mtp_params:{mtp_params}"
    )
    ...
    while True:
        ...
        if args.mtp_anneal_scale > 0:
            mtp_weight *= min(scale / args.mtp_anneal_scale, 1.0)
        base_model.mtp_loss_weight_buffer.fill_(mtp_weight)
