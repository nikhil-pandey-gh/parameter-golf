...
class Hyperparameters:
    ...
    awq_calibration_batch_seqs = int(os.environ.get("AWQ_CALIBRATION_BATCH_SEQS", 2))
...
INT6_CLIP_CANDIDATES = tuple(
    float(x)
    for x in os.environ.get("INT6_CLIP_CANDIDATES", "0.9990,0.9995,0.9999,0.99999,1.0").split(",")
    if x.strip()
)
if not INT6_CLIP_CANDIDATES:
    raise ValueError("INT6_CLIP_CANDIDATES must contain at least one value")
...
def quantize_int6_per_row(
    t: Tensor, act_rms: Tensor | None = None, clip_range: int = 31
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        act_weight = None
        if act_rms is not None and act_rms.ndim == 1 and act_rms.numel() == t32.shape[1]:
            act_weight = act_rms.float().pow(2).clamp_min(1e-6)
        ...
        for pct in INT6_CLIP_CANDIDATES:
            if pct < 1.0:
            ...
            sq_err = (t32 - recon).pow(2)
            if act_weight is not None:
                row_err = (sq_err * act_weight[None, :]).mean(dim=1)
            else:
                row_err = sq_err.mean(dim=1)
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    act_rms_map: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, act_rms=act_rms_map.get(name) if act_rms_map is not None else None)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "act_aware": act_rms_map is not None and name in act_rms_map}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
class ActivationStatsCollector:
    def __init__(self, device: torch.device):
        ...
        self.counts: dict[str, Tensor] = {}
        self.total_tokens = 0

    @torch.no_grad()
    def update(self, name: str, x: Tensor) -> None:
        flat = x.detach().reshape(-1, x.shape[-1]).float()
        if flat.numel() == 0:
            return
        sum_sq = flat.square().sum(dim=0)
        ...
        if name not in self.sum_sq:
            ...
            self.sum_sq[name].add_(sum_sq)
            self.counts[name].add_(count)

    def add_tokens(self, n_tokens: int) -> None:
        self.total_tokens += int(n_tokens)

    @torch.no_grad()
    def finalize(self) -> tuple[dict[str, Tensor], int]:
        total_tokens = torch.tensor(float(self.total_tokens), device=self.device, dtype=torch.float32)
        if dist.is_available() and dist.is_initialized():
            dist.all_reduce(total_tokens, op=dist.ReduceOp.SUM)
            for name in sorted(self.sum_sq):
                dist.all_reduce(self.sum_sq[name], op=dist.ReduceOp.SUM)
                dist.all_reduce(self.counts[name], op=dist.ReduceOp.SUM)
        rms: dict[str, Tensor] = {}
        for name in self.sum_sq:
            denom = self.counts[name].clamp_min(1.0)
            rms[name] = torch.sqrt(self.sum_sq[name] / denom).to(dtype=torch.float32).detach().cpu().contiguous()
        return rms, int(total_tokens.item())


def _collect_attention_projection_stats(
    layer_idx: int,
    ...
    collector.update(prefix + "c_q.weight", x)
    collector.update(prefix + "c_k.weight", x)
    collector.update(prefix + "c_v.weight", x)
    bsz, seqlen, dim = x.shape
    q = F.linear(x, q_w.to(x.dtype)).reshape(bsz, seqlen, attn.num_heads, attn.head_dim)
    k = F.linear(x, k_w.to(x.dtype)).reshape(bsz, seqlen, attn.num_kv_heads, attn.head_dim)
    v = F.linear(x, v_w.to(x.dtype))
    if v_embed is not None:
        v = v + v_embed
    ...
    if attn.gated_attention:
        gate = torch.sigmoid(attn.attn_gate(x)).unsqueeze(-1)
        y = y * gate
    y = y.reshape(bsz, seqlen, dim)
    collector.update(prefix + "proj.weight", y)
    return F.linear(y, out_w.to(x.dtype)), raw_v


def _collect_block_activation_stats(
    layer_idx: int,
    block: Block,
    x: Tensor,
    x0: Tensor,
    ...
    attn_out, raw_v = _collect_attention_projection_stats(
        layer_idx,
        block.attn,
        attn_in,
        q_w,
    ...
    collector.update(f"blocks.{layer_idx}.mlp.fc.weight", mlp_in)
    hidden = F.leaky_relu(F.linear(mlp_in, up_w.to(mlp_in.dtype)), negative_slope=0.5).square()
    collector.update(f"blocks.{layer_idx}.mlp.proj.weight", hidden)
    x_out = x_out + block.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * F.linear(hidden, down_w.to(hidden.dtype))
...
def collect_awq_activation_stats(
    ...
    if not args.awq_enabled or args.awq_calibration_batches <= 0 or args.awq_calibration_batch_seqs <= 0:
        return {}
    collector = ActivationStatsCollector(device)
    calib_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
    global_tokens = args.awq_calibration_batch_seqs * args.train_seq_len * world_size
    was_training = base_model.training
    base_model.eval()
    with torch.inference_mode():
        for batch_idx in range(args.awq_calibration_batches):
            input_ids, _ = calib_loader.next_batch(global_tokens, args.train_seq_len, grad_accum_steps=1)
            collector.add_tokens(input_ids.numel())
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                n = base_model.num_layers
                x = base_model.tok_emb(input_ids)
                if base_model.bigram is not None:
                    x = x + base_model.bigram(input_ids)
                x = F.rms_norm(x, (x.size(-1),))
                ...
                for i in range(base_model.num_encoder_layers):
                    ve = base_model._get_ve(i, input_ids, ve_cache)
                    x, raw_v = _collect_block_activation_stats(
                        i,
                        base_model.blocks[i],
                        x,
                        x0,
                        base_model.qo_bank[i],
                        base_model.kv_bank[i],
                        base_model.kv_bank[n + i],
                        base_model.qo_bank[n + i],
                        base_model.mlp_up_bank[i],
                        base_model.mlp_down_bank[i],
                        collector,
                        v_embed=ve,
                        v0=v0,
                    )
                ...
                for i in range(base_model.num_decoder_layers):
                    bi = base_model.num_encoder_layers + i
                    if skips:
                        x = x + base_model.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
                    ve = base_model._get_ve(bi, input_ids, ve_cache)
                    x, _ = _collect_block_activation_stats(
                        bi,
                        base_model.blocks[bi],
                        x,
                        x0,
                        base_model.qo_bank[bi],
                        base_model.kv_bank[bi],
                        base_model.kv_bank[n + bi],
                        base_model.qo_bank[n + bi],
                        base_model.mlp_up_bank[bi],
                        base_model.mlp_down_bank[bi],
                        collector,
                        v_embed=ve,
                        v0=v0,
                    )
            if rank == 0 and (batch_idx == 0 or batch_idx + 1 == args.awq_calibration_batches):
                log0(
                    f"awq_calibration:batch {batch_idx + 1}/{args.awq_calibration_batches} "
                    f"local_tokens:{input_ids.numel()}"
                )
    if was_training:
        base_model.train()
    stats, total_tokens = collector.finalize()
    if rank == 0:
        log0(
            f"awq_calibration:done tensors:{len(stats)} "
            f"batches:{args.awq_calibration_batches} "
            f"local_batch_seqs:{args.awq_calibration_batch_seqs} "
            f"global_tokens:{total_tokens}"
        )
    return stats
...
def main() -> None:
    ...
    log0(
        f"awq_enabled:{args.awq_enabled} awq_calibration_batches:{args.awq_calibration_batches} "
        f"awq_calibration_batch_seqs:{args.awq_calibration_batch_seqs}"
    )
    ...
    act_rms_map = collect_awq_activation_stats(args, base_model, rank, world_size, device, log0)
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, act_rms_map=act_rms_map)
    awq_tensors = sum(
        1
        for info in quant_meta.values()
        if isinstance(info, dict) and info.get("type") == "int6" and info.get("act_aware")
    )
    if rank == 0:
        log0(f"awq_quantization:act_aware_tensors:{awq_tensors}")
