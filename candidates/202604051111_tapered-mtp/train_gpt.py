...
class Hyperparameters:
    ...
    mtp_taper_scale = float(os.environ.get("MTP_TAPER_SCALE", 0.30))
    mtp_init_mode = os.environ.get("MTP_INIT_MODE", "main")
...
def eval_val(
    ...
    model.eval()
    mtp_lambda = torch.zeros((), device=device)
...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_init_mode not in {"main", "zero"}:
            raise ValueError(f"mtp_init_mode must be 'main' or 'zero', got {mtp_init_mode}")
        self.tie_embeddings = tie_embeddings
        self.tied_embed_init_std = tied_embed_init_std
        self.logit_softcap = logit_softcap
        self.mtp_num_heads = mtp_num_heads
        self.mtp_loss_weight = mtp_loss_weight
        self.mtp_init_mode = mtp_init_mode
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_init_mode == "main" and self.mtp_heads:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_lambda: Tensor | None = None) -> Tensor:
        ...
        mtp_weight = x.new_tensor(self.mtp_loss_weight) if mtp_lambda is None else mtp_lambda.to(device=x.device, dtype=x.dtype)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_init_mode=args.mtp_init_mode,
    ...
    log0(
        ...
        f"mtp_warmup_steps:{args.mtp_warmup_steps} mtp_taper_scale:{args.mtp_taper_scale} "
        f"mtp_init_mode:{args.mtp_init_mode} mtp_params:{mtp_params}"
    )
    ...
    def mtp_mul(step: int, scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        warmup_mul = 1.0 if args.mtp_warmup_steps <= 0 else min((step + 1) / args.mtp_warmup_steps, 1.0)
        taper_mul = 1.0 if args.mtp_taper_scale <= 0.0 else min(scale / max(args.mtp_taper_scale, 1e-9), 1.0)
        return args.mtp_loss_weight * warmup_mul * taper_mul
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            warmup_mtp = torch.full((), mtp_mul(warmup_step, 1.0), device=device)
            for micro_step in range(grad_accum_steps):
                if distributed:
                    model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, warmup_mtp)
    ...
    while True:
        ...
        mtp_scale = mtp_mul(step, scale)
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        mtp_lambda = torch.full((), mtp_scale, device=device)
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_lambda:{mtp_scale:.4f} train_time:{approx_training_time_ms:.0f}ms "
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_init_mode=args.mtp_init_mode,
