...
class Hyperparameters:
    ...
    cla_share_kv = bool(int(os.environ.get("CLA_SHARE_KV", "1")))
    cla_group_size = int(os.environ.get("CLA_GROUP_SIZE", 2))
...
def _num_cla_slots(num_layers: int, cla_group_size: int) -> int:
    group = max(1, cla_group_size)
    return (num_layers + group - 1) // group


def _cla_slot_index(layer_idx: int, cla_group_size: int, num_slots: int) -> int:
    return min(layer_idx // max(1, cla_group_size), num_slots - 1)
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        cla_share_kv: bool = False,
        cla_group_size: int = 1,
    ):
        super().__init__()
        ...
        self.cla_share_kv = cla_share_kv and cla_group_size > 1
        self.cla_group_size = max(1, cla_group_size)
        self.num_kv_slots = _num_cla_slots(num_layers, self.cla_group_size) if self.cla_share_kv else num_layers
        self.qo_bank = nn.Parameter(torch.empty(2 * num_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.num_kv_slots, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(num_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(num_layers, model_dim, mlp_dim))
        self.kv_slot_use_counts = tuple(
            sum(1 for layer_idx in range(num_layers) if self._kv_slot(layer_idx) == slot)
            for slot in range(self.num_kv_slots)
        )
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(kv_slots):
            ...
            nn.init.orthogonal_(self.kv_bank.data[kv_slots + i], gain=1.0)  # V
    ...
    def _kv_slot(self, layer_idx: int) -> int:
        if not self.cla_share_kv:
            return layer_idx
        return _cla_slot_index(layer_idx, self.cla_group_size, self.num_kv_slots)
    def normalize_shared_kv_grad(self) -> None:
        if not self.cla_share_kv or self.kv_bank.grad is None:
            return
        with torch.no_grad():
            counts = self.kv_bank.grad.new_tensor(self.kv_slot_use_counts).view(self.num_kv_slots, 1, 1)
            self.kv_bank.grad[:self.num_kv_slots].div_(counts)
            self.kv_bank.grad[self.num_kv_slots:].div_(counts)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            kv_i = self._kv_slot(i)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[kv_i], self.kv_bank[self.num_kv_slots + kv_i],
        ...
        for i in range(self.num_decoder_layers):
            ...
            kv_i = self._kv_slot(bi)
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[kv_i], self.kv_bank[self.num_kv_slots + kv_i],
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        base_model.normalize_shared_kv_grad()
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    ...
    for name, tensor in sd.items():
        if name == "qo_bank":
            ...
            kv_slots = tensor.shape[0] // 2
            if kv_slots == n:
                for i in range(n):
                    out[f"blocks.{i}.attn.c_k.weight"] = tensor[i]
                    out[f"blocks.{i}.attn.c_v.weight"] = tensor[kv_slots + i]
            else:
                for slot in range(kv_slots):
                    out[f"cla_kv.{slot}.attn.c_k.weight"] = tensor[slot]
                    out[f"cla_kv.{slot}.attn.c_v.weight"] = tensor[kv_slots + slot]
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    out: dict[str, Tensor] = {}
    n = num_layers
    kv_slots = template_sd["kv_bank"].shape[0] // 2
    # Reconstruct banks from individual weight keys
    qo_slices = [None] * (2 * n)
    kv_slices = [None] * (2 * kv_slots)
    up_slices = [None] * n
    down_slices = [None] * n
    consumed = set()
    for i in range(n):
    ...
    if kv_slots == n:
        for i in range(n):
            ...
            if vk in sd:
                kv_slices[kv_slots + i] = sd[vk]
                consumed.add(vk)
    else:
        for slot in range(kv_slots):
            kk = f"cla_kv.{slot}.attn.c_k.weight"
            if kk in sd:
                kv_slices[slot] = sd[kk]
                consumed.add(kk)
            vk = f"cla_kv.{slot}.attn.c_v.weight"
            if vk in sd:
                kv_slices[kv_slots + slot] = sd[vk]
                consumed.add(vk)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        cla_share_kv=args.cla_share_kv,
        cla_group_size=args.cla_group_size,
    ).to(device).bfloat16()
    ...
    log0(
        f"cla_share_kv:{args.cla_share_kv} cla_group_size:{args.cla_group_size} "
        f"kv_slots:{base_model.num_kv_slots}"
    )
    log0(
    ...
    eval_model = GPT(
        ...
        cla_share_kv=args.cla_share_kv, cla_group_size=args.cla_group_size,
    ).to(device).bfloat16()
