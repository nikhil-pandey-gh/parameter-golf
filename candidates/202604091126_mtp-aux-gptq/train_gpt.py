...
DEFAULT_DATA_PATH = str((REPO_ROOT / "data/datasets/fineweb10B_sp1024").resolve())
DEFAULT_TOKENIZER_PATH = str((REPO_ROOT / "data/tokenizers/fineweb_1024_bpe.model").resolve())
class Hyperparameters:
    ...
    # Training-only multi-token prediction heads improve sample efficiency and
    # are excluded from export, so they can spend train-time compute without
    # inflating the final artifact.
    ...
    # Keep late-QAT disabled by default in this candidate. The copied base used
    # a compile-time class flag for late activation, which is easy to make
    # inert under torch.compile; users can still opt into always-on QAT.
