...
class Hyperparameters:
    data_path = os.environ.get("DATA_PATH", _repo_path("data", "datasets", "fineweb10B_sp1024"))
    train_files = os.path.join(data_path, "fineweb_train_*.bin")
    val_files = os.path.join(data_path, "fineweb_val_*.bin")
    tokenizer_path = os.environ.get("TOKENIZER_PATH", _repo_path("data", "tokenizers", "fineweb_1024_bpe.model"))
    ...
    mlp_equalize_strength = float(os.environ.get("MLP_EQUALIZE_STRENGTH", 1.0))
    mlp_equalize_max_scale = float(os.environ.get("MLP_EQUALIZE_MAX_SCALE", 2.5))
...
def causal_attention(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = causal_attention(q, k, v, causal=True)
        if self.use_xsa:
...
def equalize_degree2_mlp_pair(
    ...
    up32 = up_w.float()
    down32 = down_w.float()
    up_stat = up32.abs().amax(dim=1).clamp_min(1e-6)
    down_stat = down32.abs().amax(dim=0).clamp_min(1e-6)
    scale = (down_stat / up_stat).pow(strength / 3.0)
    if max_scale > 0:
        scale = scale.clamp(1.0 / max_scale, max_scale)
    up_eq = (up32 * scale[:, None]).to(dtype=up_w.dtype).contiguous()
    down_eq = (down32 / scale.square()[None, :]).to(dtype=down_w.dtype).contiguous()
    stats = {
        "scale_min": float(scale.min().item()),
        "scale_max": float(scale.max().item()),
        "scale_mean": float(scale.mean().item()),
    }
    return up_eq, down_eq, stats


def apply_degree2_mlp_equalization(
    ...
    for layer_idx in range(num_layers):
        ...
        if up_key not in out or down_key not in out:
            continue
        out[up_key], out[down_key], layer_stats = equalize_degree2_mlp_pair(
            out[up_key],
            out[down_key],
            strength=strength,
            max_scale=max_scale,
...
def _prepare_model_for_eval(model: GPT) -> GPT:
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    def build_model() -> GPT:
        ...
        return _prepare_model_for_eval(model)

    model = build_model()
    x = torch.randint(0, vocab_size, (2, seq_len), device=device)
    y = torch.randint(0, vocab_size, (2, seq_len), device=device)
    loss = model(x, y)
    loss.backward()

    export_sd = {k: v.detach().cpu() for k, v in model.state_dict().items() if "mtp_heads" not in k}
    unbanked_sd = _unbank_state_dict(export_sd, num_layers)
    eq_stats: list[dict[str, float]] = []
    if args.mlp_equalize:
        unbanked_sd, eq_stats = apply_degree2_mlp_equalization(
            unbanked_sd,
            num_layers,
            strength=args.mlp_equalize_strength,
            max_scale=args.mlp_equalize_max_scale,
        )
    ...
    deq_state = _rebank_state_dict(deq_unbanked, num_layers, export_sd)
    ...
    print(
        ...
        f"quant_bytes:{len(quant_blob)} "
        f"eq_layers:{len(eq_stats)}"
    )
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for full training; use SMOKE_TEST=1 for a CPU export smoke test.")
    ...
    _prepare_model_for_eval(base_model)
    ...
    if args.mlp_equalize:
        ...
        if master_process and eq_stats:
            mean_scale = sum(item["scale_mean"] for item in eq_stats) / len(eq_stats)
            min_scale = min(item["scale_min"] for item in eq_stats)
            max_scale = max(item["scale_max"] for item in eq_stats)
            log0(
                f"mlp_equalize:enabled layers:{len(eq_stats)} strength:{args.mlp_equalize_strength:.2f} "
                f"clamp:{args.mlp_equalize_max_scale:.2f} scale_range:[{min_scale:.3f},{max_scale:.3f}] "
                f"mean_scale:{mean_scale:.3f}"
            )
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"})
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
    ...
    _prepare_model_for_eval(eval_model)
