...
def build_mirrored_core_schedule(num_layers: int, num_shared_cores: int) -> list[int]:
    min_cores = math.ceil(num_layers / 2)
    if num_shared_cores < min_cores:
        raise ValueError(
            f"NUM_SHARED_CORES must be at least ceil(NUM_LAYERS/2)={min_cores} for mirror sharing; "
            f"got {num_shared_cores}"
        )
    forward = list(range(num_shared_cores))
    backward = list(range(num_shared_cores - 2, -1, -1))
    schedule = (forward + backward)[:num_layers]
    if len(schedule) != num_layers:
        raise ValueError(f"Failed to build mirrored schedule for num_layers={num_layers}, num_shared_cores={num_shared_cores}")
    return schedule
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_shared_cores = num_shared_cores
        ...
        self.shared_cores = nn.ModuleList(
            [
                ...
                for _ in range(self.num_shared_cores)
            ]
        )
        self.layer_to_core = build_mirrored_core_schedule(num_layers, self.num_shared_cores)
        ...
        self.xsa_start = max(0, num_layers - xsa_last_n)
        self._init_weights()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](
                x,
                x0,
                self.shared_cores[self.layer_to_core[i]],
                v_embed=ve,
        ...
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](
                x,
                x0,
                self.shared_cores[self.layer_to_core[bi]],
                v_embed=ve,
                use_xsa=bi >= self.xsa_start,
            )
        x = self.final_norm(x)
...
def main() -> None:
    ...
    block_named_params = list(base_model.blocks.named_parameters()) + [
        (f"shared_cores.{name}", p) for name, p in base_model.shared_cores.named_parameters()
    ]
    matrix_params = [
        p
    ...
    log0(f"shared_cores:{args.num_shared_cores} mirrored_schedule:{base_model.layer_to_core}")
