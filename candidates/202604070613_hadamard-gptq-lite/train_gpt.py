...
class Hyperparameters:
    ...
    hadamard_min_dim = int(os.environ.get("HADAMARD_MIN_DIM", 128))
...
def fast_hadamard_transform(x: Tensor) -> Tensor:
    n = x.size(-1)
    ...
    while h < n:
        ...
        h *= 2
    scale = x.new_tensor(1.0 / math.sqrt(n))
    return y.reshape(*prefix, n) * scale


class CastedLinear(nn.Linear):
    _qat_enabled: bool = False
    _hadamard_enabled: bool = True
    _hadamard_min_dim: int = 128

    def __init__(self, in_features: int, out_features: int, bias: bool = True, hadamard_input: bool = False):
        super().__init__(in_features, out_features, bias=bias)
        if hadamard_input and not is_power_of_two(in_features):
            raise ValueError(f"Hadamard input mixing requires power-of-two in_features, got {in_features}")
        self.hadamard_input = hadamard_input

    def forward(self, x: Tensor) -> Tensor:
        if self.hadamard_input and CastedLinear._hadamard_enabled and self.in_features >= CastedLinear._hadamard_min_dim:
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.c_q = CastedLinear(dim, dim, bias=False, hadamard_input=use_hadamard)
        self.c_k = CastedLinear(dim, kv_dim, bias=False, hadamard_input=use_hadamard)
        self.c_v = CastedLinear(dim, kv_dim, bias=False, hadamard_input=use_hadamard)
        self.proj = CastedLinear(dim, dim, bias=False, hadamard_input=use_hadamard)
    ...
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int):
        ...
        self.proj = (
            CastedLinear(
                ...
                model_dim,
                bias=False,
                hadamard_input=is_power_of_two(bigram_dim),
            )
...
class ValueEmbedding(nn.Module):
    ...
    def __init__(self, vocab_size: int, ve_dim: int, model_dim: int):
        ...
        self.proj = (
            CastedLinear(ve_dim, model_dim, bias=False, hadamard_input=is_power_of_two(ve_dim))
            if ve_dim != model_dim
            else None
        )
        if self.proj is not None:
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.fc = CastedLinear(dim, hidden, bias=False, hadamard_input=is_power_of_two(dim))
...
def main() -> None:
    ...
    CastedLinear._hadamard_enabled = args.hadamard_enabled
    CastedLinear._hadamard_min_dim = args.hadamard_min_dim
    base_model = GPT(
    ...
    def build_model_wrapper() -> nn.Module:
    ...
    model: nn.Module = build_model_wrapper()
    ...
    log0(
        f"hadamard_enabled:{args.hadamard_enabled} hadamard_min_dim:{args.hadamard_min_dim} "
        "hadamard_layers:attn_qkv_proj,mlp_fc,bigram_proj,ve_proj"
    )
    log0(
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            model.train()
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} recompiling_model:1")
        zero_grad_all()
