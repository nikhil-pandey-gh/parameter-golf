...
class Hyperparameters:
    ...
    bank_share_mode = os.environ.get("BANK_SHARE_MODE", "mirror").strip().lower()
...
def build_layer_to_bank(num_layers: int, bank_share_mode: str) -> list[int]:
    if bank_share_mode == "none":
        return list(range(num_layers))
    if bank_share_mode == "mirror":
        return [min(i, num_layers - 1 - i) for i in range(num_layers)]
    raise ValueError(f"Unsupported BANK_SHARE_MODE={bank_share_mode!r}; expected 'mirror' or 'none'")
...
class GPT(nn.Module):
    def __init__(
        ...
        self.layer_to_bank = build_layer_to_bank(num_layers, bank_share_mode)
        self.num_shared_banks = max(self.layer_to_bank) + 1
        self.qo_bank = nn.Parameter(torch.empty(2 * self.num_shared_banks, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.num_shared_banks, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(self.num_shared_banks, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(self.num_shared_banks, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        bank_count = self.num_shared_banks
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        bank_offset = self.num_shared_banks
        ...
        for i in range(self.num_encoder_layers):
            bank_i = self.layer_to_bank[i]
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[bank_i], self.kv_bank[bank_i], self.kv_bank[bank_offset + bank_i],
                self.qo_bank[bank_offset + bank_i], self.mlp_up_bank[bank_i], self.mlp_down_bank[bank_i],
                v_embed=ve, v0=v0)
...
def _unbank_state_dict(sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert 3D bank tensors into individual 2D tensors keyed by shared-bank index."""
    out: dict[str, Tensor] = {}
    for name, tensor in sd.items():
        if name == "qo_bank":
            bank_count = tensor.shape[0] // 2
            for i in range(bank_count):
                out[f"shared_blocks.{i}.attn.c_q.weight"] = tensor[i]
                out[f"shared_blocks.{i}.attn.proj.weight"] = tensor[bank_count + i]
        elif name == "kv_bank":
            bank_count = tensor.shape[0] // 2
            for i in range(bank_count):
                out[f"shared_blocks.{i}.attn.c_k.weight"] = tensor[i]
                out[f"shared_blocks.{i}.attn.c_v.weight"] = tensor[bank_count + i]
...
def _rebank_state_dict(sd: dict[str, Tensor], template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert individual 2D tensors keyed by shared-bank index back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    bank_count = template_sd["mlp_up_bank"].shape[0]
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    input_ids = torch.randint(args.vocab_size, (2, 32), dtype=torch.int64)
    target_ids = torch.randint(args.vocab_size, (2, 32), dtype=torch.int64)
    model.eval()
    with torch.no_grad():
...
def main() -> None:
    ...
    log0(
        f"bank_share_mode:{base_model.bank_share_mode} shared_banks:{base_model.num_shared_banks} "
        f"layer_to_bank:{base_model.layer_to_bank}"
    )
