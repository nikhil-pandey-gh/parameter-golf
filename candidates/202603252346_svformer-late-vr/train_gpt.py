...
class Hyperparameters:
    ...
    value_residual_source_layer = int(os.environ.get("VALUE_RESIDUAL_SOURCE_LAYER", "0"))
    value_residual_layers = os.environ.get("VALUE_RESIDUAL_LAYERS", "")
...
def parse_layer_list(spec: str, num_layers: int) -> list[int]:
    if not spec.strip():
        return []
    layers: set[int] = set()
    for raw_part in spec.split(","):
        ...
        if not 0 <= idx < num_layers:
            raise ValueError(f"Layer index {raw_part!r} is out of range for {num_layers} layers")
        layers.add(idx)
    return sorted(layers)

def parse_layer_index(idx: int, num_layers: int, name: str) -> int:
    if idx < 0:
        idx += num_layers
    if not 0 <= idx < num_layers:
        raise ValueError(f"{name}={idx} is out of range for {num_layers} layers")
    return idx
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.capture_v = capture_v or value_residual
        if value_residual:
            # Keep the residual path bounded with a convex combination and start close to the local value path.
            self.vr_lambda = nn.Parameter(torch.tensor([-1.25, 1.25], dtype=torch.float32))
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        raw_v = v if self.capture_v else None
        if self.value_residual and v0 is not None:
            lam = torch.softmax(self.vr_lambda, dim=0).to(dtype=v.dtype)
            v = lam[0] * v0 + lam[1] * v
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        value_residual_source_layer: int = 0,
        value_residual_layers: str = "",
    ):
        super().__init__()
        ...
        self.value_residual = value_residual
        self.value_residual_source_layer = parse_layer_index(
            value_residual_source_layer, num_layers, "VALUE_RESIDUAL_SOURCE_LAYER"
        )
        if value_residual:
            parsed_vr_layers = parse_layer_list(value_residual_layers, num_layers)
            if parsed_vr_layers:
                self.value_residual_layers = parsed_vr_layers
            elif xsa_last_n > 0:
                self.value_residual_layers = list(range(max(0, num_layers - xsa_last_n), num_layers))
            else:
                raise ValueError(
                    "VALUE_RESIDUAL_LAYERS must be set explicitly when VALUE_RESIDUAL=1 and XSA_LAST_N=0"
                )
        else:
            self.value_residual_layers = []
        if value_residual and any(layer <= self.value_residual_source_layer for layer in self.value_residual_layers):
            raise ValueError(
                "VALUE_RESIDUAL_SOURCE_LAYER must be earlier than every VALUE_RESIDUAL_LAYERS target"
            )
        self.value_residual_layer_set = set(self.value_residual_layers)
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    value_residual=i in self.value_residual_layer_set,
                    capture_v=value_residual and i == self.value_residual_source_layer,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        value_residual_source_layer=args.value_residual_source_layer,
        value_residual_layers=args.value_residual_layers,
    ).to(device).bfloat16()
