...
class Hyperparameters:
    ...
    bank_qat_start = float(os.environ.get("BANK_QAT_START", 0.20))
    bank_qat_full = float(os.environ.get("BANK_QAT_FULL", 0.05))
...
def fake_quantize_int6_rows(weight: Tensor, clip_range: int = 31) -> Tensor:
    with torch.no_grad():
        w32 = weight.float()
...
def qdrop_int6_rows(weight: Tensor, prob: float) -> Tensor:
    if prob <= 0.0 or weight.ndim != 2:
        return weight
    w_q = fake_quantize_int6_rows(weight)
    if prob >= 1.0:
        return weight + (w_q - weight).detach()
    mask = (torch.rand((weight.size(0), 1), device=weight.device) < prob).to(dtype=weight.dtype)
    mixed = mask * w_q + (1.0 - mask) * weight
    return weight + (mixed - weight).detach()
...
class GPT(nn.Module):
    ...
    def set_bank_qat_prob(self, prob: float) -> None:
        self.bank_qat_prob = max(0.0, min(float(prob), 1.0))
    def _maybe_qat_bank(self, weight: Tensor) -> Tensor:
        if not self.training or self.bank_qat_prob <= 0.0:
            return weight
        return qdrop_int6_rows(weight, self.bank_qat_prob)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self._maybe_qat_bank(self.qo_bank[i]), self._maybe_qat_bank(self.kv_bank[i]), self._maybe_qat_bank(self.kv_bank[n + i]),
                self._maybe_qat_bank(self.qo_bank[n + i]), self._maybe_qat_bank(self.mlp_up_bank[i]), self._maybe_qat_bank(self.mlp_down_bank[i]),
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                self._maybe_qat_bank(self.qo_bank[bi]), self._maybe_qat_bank(self.kv_bank[bi]), self._maybe_qat_bank(self.kv_bank[n + bi]),
                self._maybe_qat_bank(self.qo_bank[n + bi]), self._maybe_qat_bank(self.mlp_up_bank[bi]), self._maybe_qat_bank(self.mlp_down_bank[bi]),
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    if args.bank_qat_enabled and not (0.0 <= args.bank_qat_full < args.bank_qat_start <= 1.0):
        raise ValueError(
            f"BANK_QAT_FULL ({args.bank_qat_full}) must be in [0, BANK_QAT_START) and "
            f"BANK_QAT_START ({args.bank_qat_start}) must be <= 1.0"
        )
    ...
    def bank_qat_prob(scale: float) -> float:
        if not args.bank_qat_enabled or scale >= args.bank_qat_start:
            return 0.0
        if scale <= args.bank_qat_full:
            return 1.0
        return (args.bank_qat_start - scale) / max(args.bank_qat_start - args.bank_qat_full, 1e-9)
    ...
    while True:
        ...
        current_bank_qat_prob = bank_qat_prob(scale)
        base_model.set_bank_qat_prob(current_bank_qat_prob)
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        if current_bank_qat_prob > 0.0:
            if model is compiled_model:
                model = base_model
                log0(
                    f"bank_qat:eager_switch step:{step} scale:{scale:.4f} "
                    f"bank_prob:{current_bank_qat_prob:.2f}"
                )
            if current_bank_qat_prob >= 1.0 and not bank_qat_full_logged:
                bank_qat_full_logged = True
                log0(f"bank_qat:full step:{step} scale:{scale:.4f}")
        zero_grad_all()
    ...
    base_model.set_bank_qat_prob(0.0)
