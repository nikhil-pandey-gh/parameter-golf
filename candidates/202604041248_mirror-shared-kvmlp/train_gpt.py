...
class Hyperparameters:
    ...
    ve_dim = int(os.environ.get("VE_DIM", 160))
    ve_layers = os.environ.get("VE_LAYERS", "7,8,9,10")
    mirror_share_qo = bool(int(os.environ.get("MIRROR_SHARE_QO", "0")))
    mirror_share_kv = bool(int(os.environ.get("MIRROR_SHARE_KV", "1")))
    mirror_share_mlp = bool(int(os.environ.get("MIRROR_SHARE_MLP", "1")))
...
def build_mirror_layer_map(num_layers: int, enabled: bool) -> list[int]:
    if not enabled:
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_dim: int = 128,
        ve_layers: str = "9,10",
        mirror_share_qo: bool = False,
        mirror_share_kv: bool = False,
        mirror_share_mlp: bool = False,
        gated_attention: bool = False,
        value_residual: bool = False,
    ):
        super().__init__()
        ...
        self.qo_layer_map = build_mirror_layer_map(num_layers, mirror_share_qo)
        self.kv_layer_map = build_mirror_layer_map(num_layers, mirror_share_kv)
        self.mlp_layer_map = build_mirror_layer_map(num_layers, mirror_share_mlp)
        self.qo_bank_layers = max(self.qo_layer_map) + 1
        self.kv_bank_layers = max(self.kv_layer_map) + 1
        self.mlp_bank_layers = max(self.mlp_layer_map) + 1
        self.qo_bank = nn.Parameter(torch.empty(2 * self.qo_bank_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.kv_bank_layers, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(self.mlp_bank_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(self.mlp_bank_layers, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        # Init only the unique bank slices; per-layer control tensors stay untied.
        for i in range(self.qo_bank_layers):
            nn.init.orthogonal_(self.qo_bank.data[i], gain=1.0)
            nn.init.zeros_(self.qo_bank.data[self.qo_bank_layers + i])
            self.qo_bank.data[self.qo_bank_layers + i].mul_(proj_scale)
        for i in range(self.kv_bank_layers):
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)
            nn.init.orthogonal_(self.kv_bank.data[self.kv_bank_layers + i], gain=1.0)
        for i in range(self.mlp_bank_layers):
    ...
    def _layer_bank_indices(self, layer_idx: int) -> tuple[int, int, int]:
        return (
            self.qo_layer_map[layer_idx],
            self.kv_layer_map[layer_idx],
            self.mlp_layer_map[layer_idx],
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            qo_idx, kv_idx, mlp_idx = self._layer_bank_indices(i)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[qo_idx], self.kv_bank[kv_idx], self.kv_bank[self.kv_bank_layers + kv_idx],
                self.qo_bank[self.qo_bank_layers + qo_idx], self.mlp_up_bank[mlp_idx], self.mlp_down_bank[mlp_idx],
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            qo_idx, kv_idx, mlp_idx = self._layer_bank_indices(bi)
...
def eval_val_sliding_ttt(
    ...
    frozen_qo_bank_ids = {base_model.qo_layer_map[i] for i in frozen_block_ids}
    frozen_kv_bank_ids = {base_model.kv_layer_map[i] for i in frozen_block_ids}
    frozen_mlp_bank_ids = {base_model.mlp_layer_map[i] for i in frozen_block_ids}
    ttt_params = []
    ...
    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         ...
         f"frozen_banks=qo:{sorted(frozen_qo_bank_ids)} kv:{sorted(frozen_kv_bank_ids)} "
         f"mlp:{sorted(frozen_mlp_bank_ids)}")

    optimizer = torch.optim.SGD(ttt_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
    t0 = time.perf_counter()

    def zero_frozen_bank_grads() -> None:
        if base_model.qo_bank.grad is not None:
            for bank_idx in frozen_qo_bank_ids:
                base_model.qo_bank.grad[bank_idx].zero_()
                base_model.qo_bank.grad[base_model.qo_bank_layers + bank_idx].zero_()
        if base_model.kv_bank.grad is not None:
            for bank_idx in frozen_kv_bank_ids:
                base_model.kv_bank.grad[bank_idx].zero_()
                base_model.kv_bank.grad[base_model.kv_bank_layers + bank_idx].zero_()
        if base_model.mlp_up_bank.grad is not None:
            for bank_idx in frozen_mlp_bank_ids:
                base_model.mlp_up_bank.grad[bank_idx].zero_()
        if base_model.mlp_down_bank.grad is not None:
            for bank_idx in frozen_mlp_bank_ids:
                base_model.mlp_down_bank.grad[bank_idx].zero_()
...
def _unbank_state_dict(
    ...
    """Convert unique bank slices into individually quantizable 2D tensors."""
    out: dict[str, Tensor] = {}
    for name, tensor in sd.items():
        if name == "qo_bank":
            for i in range(qo_bank_layers):
                out[f"qo_blocks.{i}.attn.c_q.weight"] = tensor[i]
                out[f"qo_blocks.{i}.attn.proj.weight"] = tensor[qo_bank_layers + i]
        elif name == "kv_bank":
            for i in range(kv_bank_layers):
                out[f"kv_blocks.{i}.attn.c_k.weight"] = tensor[i]
                out[f"kv_blocks.{i}.attn.c_v.weight"] = tensor[kv_bank_layers + i]
        elif name == "mlp_up_bank":
            for i in range(mlp_bank_layers):
                out[f"mlp_blocks.{i}.mlp.fc.weight"] = tensor[i]
        elif name == "mlp_down_bank":
            for i in range(mlp_bank_layers):
                out[f"mlp_blocks.{i}.mlp.proj.weight"] = tensor[i]
        else:
            out[name] = tensor
    return out
...
def _rebank_state_dict(
    ...
    """Convert quantized 2D tensors back into compact bank tensors."""
    out: dict[str, Tensor] = {}
    qo_slices = [None] * (2 * qo_bank_layers)
    kv_slices = [None] * (2 * kv_bank_layers)
    up_slices = [None] * mlp_bank_layers
    down_slices = [None] * mlp_bank_layers
    bank_weight_names: set[str] = set()
    for i in range(qo_bank_layers):
        qk = f"qo_blocks.{i}.attn.c_q.weight"
        ok = f"qo_blocks.{i}.attn.proj.weight"
        qo_slices[i] = sd[qk]
        qo_slices[qo_bank_layers + i] = sd[ok]
        bank_weight_names.update((qk, ok))
    for i in range(kv_bank_layers):
        kk = f"kv_blocks.{i}.attn.c_k.weight"
        vk = f"kv_blocks.{i}.attn.c_v.weight"
        kv_slices[i] = sd[kk]
        kv_slices[kv_bank_layers + i] = sd[vk]
        bank_weight_names.update((kk, vk))
    for i in range(mlp_bank_layers):
        fk = f"mlp_blocks.{i}.mlp.fc.weight"
        dk = f"mlp_blocks.{i}.mlp.proj.weight"
        up_slices[i] = sd[fk]
        down_slices[i] = sd[dk]
        bank_weight_names.update((fk, dk))
    ...
    for name, tensor in sd.items():
        if name not in bank_weight_names:
            out[name] = tensor
    return out
...
def main() -> None:
    ...
    log0("=" * 100, console=False)
    if flash_attn_3_func is None:
        log0("attention_backend:flash_attn_interface_missing using_torch_sdpa_fallback")
    else:
        log0("attention_backend:flash_attn_interface")
    ...
    base_model = GPT(
        ...
        mirror_share_qo=args.mirror_share_qo,
        mirror_share_kv=args.mirror_share_kv,
        mirror_share_mlp=args.mirror_share_mlp,
    ...
    log0(
        "mirror_sharing:"
        f" qo={base_model.qo_layer_map}({base_model.qo_bank_layers})"
        f" kv={base_model.kv_layer_map}({base_model.kv_bank_layers})"
        f" mlp={base_model.mlp_layer_map}({base_model.mlp_bank_layers})"
    )
    ...
    arch_meta = {
        ...
        "mirror_share_kv": args.mirror_share_kv,
        "mirror_share_mlp": args.mirror_share_mlp,
        "qo_layer_map": list(base_model.qo_layer_map),
        "kv_layer_map": list(base_model.kv_layer_map),
        "mlp_layer_map": list(base_model.mlp_layer_map),
        "qo_bank_layers": base_model.qo_bank_layers,
        "kv_bank_layers": base_model.kv_bank_layers,
        "mlp_bank_layers": base_model.mlp_bank_layers,
    }
    if master_process:
        torch.save({"state_dict": export_sd, "arch": arch_meta}, "final_model.pt")
    ...
    unbanked_sd = _unbank_state_dict(
        sd_cpu,
        base_model.qo_bank_layers,
        base_model.kv_bank_layers,
        base_model.mlp_bank_layers,
    )
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"})
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta, "arch": arch_meta}, quant_buf)
    ...
    reload_arch = quant_state.get("arch", {})
    # Re-bank the dequantized tensors
    deq_state = _rebank_state_dict(
        deq_unbanked,
        sd_cpu,
        int(reload_arch.get("qo_bank_layers", base_model.qo_bank_layers)),
        int(reload_arch.get("kv_bank_layers", base_model.kv_bank_layers)),
        int(reload_arch.get("mlp_bank_layers", base_model.mlp_bank_layers)),
    )
    eval_model = GPT(
        ...
        mirror_share_qo=bool(reload_arch.get("mirror_share_qo", args.mirror_share_qo)),
        mirror_share_kv=bool(reload_arch.get("mirror_share_kv", args.mirror_share_kv)),
        mirror_share_mlp=bool(reload_arch.get("mirror_share_mlp", args.mirror_share_mlp)),
