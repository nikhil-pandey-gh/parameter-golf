...
class Hyperparameters:
    ...
    mtp_start_frac = float(os.environ.get("MTP_START_FRAC", 0.25))
    mtp_end_frac = float(os.environ.get("MTP_END_FRAC", 0.7))
...
class CausalSelfAttention(nn.Module):
    ...
    def _fallback_attention(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        q_t = q.transpose(1, 2)
        k_t = k.transpose(1, 2)
        v_t = v.transpose(1, 2)
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if flash_attn_3_func is not None and q.is_cuda:
            ...
            y = self._fallback_attention(q, k, v)
        if self.use_xsa:
...
class GPT(nn.Module):
        ...
        if self.mtp_num_heads > 0:
            if self.tie_embeddings:
                for head in self.mtp_heads:
                    head.weight.data.copy_(self.tok_emb.weight.data)
            else:
                for head in self.mtp_heads:
                    nn.init.zeros_(head.weight)
    @torch.no_grad()
    def set_mtp_progress(self, progress: float, start_frac: float, end_frac: float, enabled: bool) -> None:
        if self.mtp_num_heads <= 0:
            return
        if not enabled:
        ...
        if end_frac <= start_frac:
            raise ValueError(f"MTP_END_FRAC must be > MTP_START_FRAC, got {start_frac} and {end_frac}")
        ramp = max(0.0, min((progress - start_frac) / (end_frac - start_frac), 1.0))
        head_positions = ramp * self.mtp_num_heads - torch.arange(
            self.mtp_num_heads, device=self.mtp_head_weights.device, dtype=self.mtp_head_weights.dtype
        )
        self.mtp_head_weights.copy_(head_positions.clamp_(0.0, 1.0))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            mtp_weights = self.mtp_head_weights.to(device=x.device, dtype=x.dtype)
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + mtp_weights[k] * F.cross_entropy(
...
def main() -> None:
    ...
    base_model.set_mtp_progress(0.0, args.mtp_start_frac, args.mtp_end_frac, args.mtp_curriculum)
    ...
    log0(
        ...
        f"mtp_curriculum:{args.mtp_curriculum} mtp_window:[{args.mtp_start_frac:.2f},{args.mtp_end_frac:.2f}] "
    ...
    while True:
        ...
        progress = elapsed_ms / max_wallclock_ms if max_wallclock_ms is not None else step / max(args.iterations, 1)
        base_model.set_mtp_progress(progress, args.mtp_start_frac, args.mtp_end_frac, args.mtp_curriculum)
        zero_grad_all()
        ...
        if should_log_train:
            mtp_weights_str = ",".join(f"{float(w):.2f}" for w in base_model.mtp_head_weights.detach().cpu().tolist())
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms "
                f"mtp_weights:[{mtp_weights_str}]"
            )
