...
class Hyperparameters:
    ...
    mixbits_bits = tuple(int(x) for x in os.environ.get("MIXBITS_BITS", "5,6,7").split(",") if x.strip())
    mixbits_base_bits = int(os.environ.get("MIXBITS_BASE_BITS", 6))
    mixbits_extra_bytes = int(os.environ.get("MIXBITS_EXTRA_BYTES", 0))
    mixbits_log_topk = int(os.environ.get("MIXBITS_LOG_TOPK", 12))
...
# --- Budgeted mixed-bit GPTQ-lite quantization ---
...
LOWBIT_CLIP_PERCENTILES = (0.9990, 0.9995, 0.9999, 0.99999, 1.0)


@dataclass
class LowBitCandidate:
    bits: int
    packed: Tensor
    scale: Tensor
    mse: float
...
def _signed_clip_range(bits: int) -> int:
    if bits < 2 or bits > 7:
        raise ValueError(f"Supported low-bit range is 2..7, got {bits}")
    return (1 << (bits - 1)) - 1


def pack_signed_lowbit(q: Tensor, bits: int, clip_range: int) -> Tensor:
    flat = q.detach().cpu().to(torch.int16).reshape(-1).numpy().astype(np.int32, copy=False)
    if flat.size == 0:
        return torch.empty((0,), dtype=torch.uint8)
    unsigned = flat + clip_range
    if unsigned.min() < 0 or unsigned.max() >= (1 << bits):
        raise ValueError(f"Values out of range for {bits}-bit packing")
    shifts = np.arange(bits - 1, -1, -1, dtype=np.int32)
    bitplanes = ((unsigned[:, None] >> shifts[None, :]) & 1).astype(np.uint8, copy=False).reshape(-1)
    packed = np.packbits(bitplanes, bitorder="big")
    return torch.from_numpy(packed.copy())


def unpack_signed_lowbit(blob: Tensor, shape: tuple[int, ...], bits: int, clip_range: int) -> Tensor:
    count = math.prod(shape)
    if count == 0:
        return torch.empty(shape, dtype=torch.int16)
    packed_np = blob.detach().cpu().to(torch.uint8).numpy()
    bitplanes = np.unpackbits(packed_np, bitorder="big")[: count * bits].reshape(count, bits)
    shifts = (1 << np.arange(bits - 1, -1, -1, dtype=np.int32))[None, :]
    unsigned = (bitplanes.astype(np.int32, copy=False) * shifts).sum(axis=1, dtype=np.int32)
    signed = unsigned.astype(np.int16, copy=False) - clip_range
    return torch.from_numpy(signed.reshape(shape).copy())


def quantize_lowbit_per_row(t: Tensor, bits: int) -> tuple[Tensor, Tensor, float]:
    if t.ndim != 2:
        raise ValueError(f"Low-bit GPTQ-lite expects 2D weights, got shape={tuple(t.shape)}")
    clip_range = _signed_clip_range(bits)
    t32 = t.float()
    best_q: Tensor | None = None
    best_s: Tensor | None = None
    best_err = float("inf")
    for pct in LOWBIT_CLIP_PERCENTILES:
        if pct < 1.0:
            row_clip = torch.quantile(t32.abs(), pct, dim=1)
        else:
            row_clip = t32.abs().amax(dim=1)
        s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16).contiguous()
        q = torch.clamp(torch.round(t32 / s.float()[:, None]), -clip_range, clip_range).to(torch.int8).contiguous()
        recon = q.float() * s.float()[:, None]
        err = float((t32 - recon).pow(2).mean().item())
        if err < best_err:
            best_q, best_s, best_err = q, s, err
    if best_q is None or best_s is None:
        raise RuntimeError("Failed to quantize tensor")
    return best_q, best_s, best_err


def build_lowbit_candidates(t: Tensor, bits_options: tuple[int, ...]) -> dict[int, LowBitCandidate]:
    candidates: dict[int, LowBitCandidate] = {}
    for bits in bits_options:
        q, s, mse = quantize_lowbit_per_row(t, bits)
        packed = pack_signed_lowbit(q, bits, _signed_clip_range(bits))
        candidates[bits] = LowBitCandidate(
            bits=bits,
            packed=packed.contiguous(),
            scale=s.contiguous(),
            mse=mse,
            payload_bytes=tensor_nbytes(packed) + tensor_nbytes(s),
        )
    return candidates


def choose_mixed_bitwidths(
    candidate_bank: dict[str, dict[int, LowBitCandidate]],
    base_bits: int,
    extra_bytes: int,
) -> tuple[dict[str, int], dict[str, object]]:
    if extra_bytes < 0:
        raise ValueError(f"MIXBITS_EXTRA_BYTES must be non-negative, got {extra_bytes}")
    chosen_bits = {name: base_bits for name in candidate_bank}
    if not candidate_bank:
        return chosen_bits, {
            "baseline_payload_bytes": 0,
            "chosen_payload_bytes": 0,
            "upgrades": [],
            "downgrades": [],
            "bank_left": extra_bytes,
        }
    if any(base_bits not in candidates for candidates in candidate_bank.values()):
        raise ValueError(f"Base bit-width {base_bits} must be present in every candidate set")

    upgrades: list[BitTradeoff] = []
    downgrades: list[BitTradeoff] = []
    baseline_payload_bytes = 0

    for name, candidates in candidate_bank.items():
        baseline = candidates[base_bits]
        baseline_payload_bytes += baseline.payload_bytes

        lower = [
            BitTradeoff(
                name=name,
                bits=bits,
                bytes_delta=baseline.payload_bytes - candidate.payload_bytes,
                mse_delta=max(candidate.mse - baseline.mse, 0.0),
                ratio=max(candidate.mse - baseline.mse, 0.0) / max(baseline.payload_bytes - candidate.payload_bytes, 1),
            )
            for bits, candidate in candidates.items()
            if bits < base_bits and baseline.payload_bytes > candidate.payload_bytes
        ]
        if lower:
            downgrades.append(min(lower, key=lambda item: (item.ratio, item.mse_delta, item.bits)))

        upper = [
            BitTradeoff(
                name=name,
                bits=bits,
                bytes_delta=candidate.payload_bytes - baseline.payload_bytes,
                mse_delta=max(baseline.mse - candidate.mse, 0.0),
                ratio=max(baseline.mse - candidate.mse, 0.0) / max(candidate.payload_bytes - baseline.payload_bytes, 1),
            )
            for bits, candidate in candidates.items()
            if bits > base_bits and candidate.payload_bytes > baseline.payload_bytes
        ]
        if upper:
            upgrades.append(max(upper, key=lambda item: (item.ratio, item.mse_delta, item.bits)))

    downgrades.sort(key=lambda item: (item.ratio, item.mse_delta, item.name))
    upgrades.sort(key=lambda item: (-item.ratio, -item.mse_delta, item.name))

    selected_upgrades: list[BitTradeoff] = []
    selected_downgrades: list[BitTradeoff] = []
    byte_bank = extra_bytes
    down_idx = 0

    for upgrade in upgrades:
        if chosen_bits[upgrade.name] != base_bits:
            continue
        if upgrade.ratio <= 0.0:
            continue
        trial_idx = down_idx
        trial_bank = byte_bank
        staged: list[BitTradeoff] = []
        staged_penalty = 0.0
        while trial_bank < upgrade.bytes_delta and trial_idx < len(downgrades):
            while (
                trial_idx < len(downgrades)
                and (chosen_bits[downgrades[trial_idx].name] != base_bits or downgrades[trial_idx].name == upgrade.name)
            ):
                trial_idx += 1
            if trial_idx >= len(downgrades) or downgrades[trial_idx].ratio >= upgrade.ratio:
                break
            staged.append(downgrades[trial_idx])
            trial_bank += downgrades[trial_idx].bytes_delta
            staged_penalty += downgrades[trial_idx].mse_delta
            trial_idx += 1
        if trial_bank < upgrade.bytes_delta:
            continue
        if staged_penalty >= upgrade.mse_delta:
            continue
        for downgrade in staged:
            if chosen_bits[downgrade.name] == base_bits:
                chosen_bits[downgrade.name] = downgrade.bits
                selected_downgrades.append(downgrade)
        down_idx = trial_idx
        byte_bank = trial_bank - upgrade.bytes_delta
        chosen_bits[upgrade.name] = upgrade.bits
        selected_upgrades.append(upgrade)

    chosen_payload_bytes = sum(
        candidate_bank[name][bits].payload_bytes for name, bits in chosen_bits.items()
    )
    return chosen_bits, {
        "baseline_payload_bytes": baseline_payload_bytes,
        "chosen_payload_bytes": chosen_payload_bytes,
...
def mixed_quantize_budgeted(
    ...
    bits_options = tuple(sorted(set(bits_options)))
    if base_bits not in bits_options:
        raise ValueError(f"MIXBITS_BASE_BITS={base_bits} must be present in MIXBITS_BITS={bits_options}")
    ...
    for name, tensor in state_dict.items():
        ...
        if not t.is_floating_point() or t.numel() <= 65536:
            ...
            continue
        if cat in lowbit_cats and t.ndim == 2:
            candidate_bank[name] = build_lowbit_candidates(t, bits_options)
            tensor_shapes[name] = tuple(t.shape)
            continue
        q, s = quantize_float_tensor(t)
        result[name + ".q"] = q
        result[name + ".scale"] = s
    ...
    chosen_bits, summary = choose_mixed_bitwidths(candidate_bank, base_bits=base_bits, extra_bytes=extra_bytes)
    for name, candidates in candidate_bank.items():
        chosen = candidates[chosen_bits[name]]
        result[name + ".packed"] = chosen.packed
        result[name + ".scale"] = chosen.scale
        meta[name] = {"type": "lowbit", "bits": chosen.bits, "shape": list(tensor_shapes[name])}
    return result, meta, summary


def dequantize_mixed_budgeted(
    result: dict[str, Tensor],
    meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            out[name] = t
            continue
        if isinstance(info, dict) and info.get("type") == "lowbit":
            bits = int(info["bits"])
            shape = tuple(int(v) for v in info["shape"])
            q = unpack_signed_lowbit(result[name + ".packed"], shape, bits, _signed_clip_range(bits)).float()
            s = result[name + ".scale"]
            out[name] = (q * s.float().view(shape[0], *([1] * (len(shape) - 1)))).to(orig_dtype)
            continue
...
def run_quantization_smoke_test() -> None:
    torch.manual_seed(0)
    for bits in (5, 6, 7):
        clip = _signed_clip_range(bits)
        probe = torch.tensor([-clip, -1, 0, 1, clip], dtype=torch.int8)
        packed = pack_signed_lowbit(probe, bits, clip)
        unpacked = unpack_signed_lowbit(packed, (probe.numel(),), bits, clip).to(torch.int8)
        if not torch.equal(probe, unpacked):
            raise AssertionError(f"Pack/unpack mismatch for {bits}-bit probe")
    sample = {
        "blocks.0.attn.c_q.weight": torch.randn(320, 256),
        "blocks.0.attn.proj.weight": torch.randn(256, 320),
        "blocks.0.mlp.fc.weight": torch.randn(512, 256),
        "blocks.0.mlp.proj.weight": torch.randn(256, 512),
        "tok_emb.weight": torch.randn(1024, 256),
        "blocks.0.attn_scale": torch.ones(256),
    }
    result, meta, summary = mixed_quantize_budgeted(
        sample,
        {"mlp", "attn"},
        bits_options=(5, 6, 7),
        base_bits=6,
    ...
    deq = dequantize_mixed_budgeted(result, meta, sample)
    lowbit_assignments = {
        name: int(info["bits"])
    ...
    worst_mse = 0.0
    for name, orig in sample.items():
        recon = deq[name]
        if recon.shape != orig.shape:
            raise AssertionError(f"Shape mismatch for {name}: {recon.shape} vs {orig.shape}")
        if orig.is_floating_point():
            worst_mse = max(worst_mse, float((orig - recon.to(orig.dtype)).float().pow(2).mean().item()))
        elif not torch.equal(orig, recon):
            raise AssertionError(f"Exact passthrough mismatch for {name}")
    model = GPT(
        vocab_size=1024,
        num_layers=4,
        model_dim=64,
    ...
    x = torch.randint(0, 1024, (2, 32), dtype=torch.int64)
    y = torch.randint(0, 1024, (2, 32), dtype=torch.int64)
    loss = model(x, y)
    loss.backward()
    print(
        "smoke_ok",
        f"lowbit={len(lowbit_assignments)}",
        f"payload_delta={summary['chosen_payload_bytes'] - summary['baseline_payload_bytes']}",
        f"worst_mse={worst_mse:.6e}",
        f"model_loss={loss.item():.6f}",
    )
    for name, bits in sorted(lowbit_assignments.items()):
        print(f"  {name}: {bits}b")
...
def main() -> None:
    ...
    quant_result, quant_meta, quant_summary = mixed_quantize_budgeted(
        unbanked_sd,
        {"mlp", "attn"},
        bits_options=args.mixbits_bits,
        base_bits=args.mixbits_base_bits,
        extra_bytes=args.mixbits_extra_bytes,
    )
    ...
    if master_process:
        with open("final_model.mixbits.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(
            f"mixbits:payload baseline{args.mixbits_base_bits}:{quant_summary['baseline_payload_bytes']} "
            f"chosen:{quant_summary['chosen_payload_bytes']} bank_left:{quant_summary['bank_left']}"
        )
        for upgrade in quant_summary["upgrades"][: args.mixbits_log_topk]:
            log0(
                f"mixbits:upgrade name:{upgrade.name} bits:{upgrade.bits} "
                f"benefit_per_byte:{upgrade.ratio:.6e}"
            )
        for downgrade in quant_summary["downgrades"][: args.mixbits_log_topk]:
            log0(
                f"mixbits:downgrade name:{downgrade.name} bits:{downgrade.bits} "
                f"cost_per_byte:{downgrade.ratio:.6e}"
            )
        log0(f"Serialized model mixbits+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size mixbits+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    with open("final_model.mixbits.ptz", "rb") as f:
        quant_blob_disk = f.read()
    quant_state = torch.load(
        io.BytesIO(lzma.decompress(quant_blob_disk)),
        map_location="cpu",
    )
    deq_unbanked = dequantize_mixed_budgeted(quant_state["w"], quant_state["m"], unbanked_sd)
...
if __name__ == "__main__":
    if os.environ.get("SMOKE_TEST_MODE", "0") == "1":
        run_quantization_smoke_test()
    else:
        main()
