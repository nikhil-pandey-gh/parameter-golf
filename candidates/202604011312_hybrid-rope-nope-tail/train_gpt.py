...
class Hyperparameters:
    ...
    nope_last_n = int(os.environ.get("NOPE_LAST_N", 4))
...
def build_sentencepiece_luts(
    ...
    if np is None:
        raise ImportError("numpy is required to build SentencePiece byte lookup tables")
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    negative_slope=mlp_negative_slope,
                    layer_idx=i,
                    ln_scale=ln_scale,
                    dtg=dtg,
                )
        ...
        nope_start = max(0, num_layers - nope_last_n) if nope_last_n > 0 else num_layers
        if rope_dims > 0:
...
def run_cpu_smoke_test(args: Hyperparameters) -> None:
    ...
    with torch.no_grad():
        ...
        logits = base_model.forward_logits(input_ids[:, : min(seq_len, 16)])
...
def main() -> None:
    ...
    if spm is None:
        raise ImportError("sentencepiece is required for real training/evaluation runs")
    ...
    nope_layers = [i for i, b in enumerate(base_model.blocks) if not b.attn.use_rope]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"hybrid_rope_nope:nope_last_{args.nope_last_n} active_layers:{nope_layers} rope_dims:{args.rope_dims}")
    ...
    eval_model = GPT(
        ...
        rope_dims=args.rope_dims, nope_last_n=args.nope_last_n, ln_scale=args.ln_scale, dtg=args.dtg_enabled,
