...
class Hyperparameters:
    ...
    shared_mlp_tail = int(os.environ.get("SHARED_MLP_TAIL", 3))
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult) if include_mlp else None
    ...
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        if self.mlp is None:
            raise RuntimeError("Block MLP is shared externally; call through GPT.forward.")
...
class GPT(nn.Module):
    def __init__(
        ...
        self.shared_mlp_tail = max(0, min(shared_mlp_tail, num_layers))
        self.shared_mlp_start = num_layers - self.shared_mlp_tail if self.shared_mlp_tail > 1 else num_layers
        self.shared_tail_layers = tuple(range(self.shared_mlp_start, num_layers)) if self.shared_mlp_tail > 1 else tuple()
        # Share only the deepest MLPs; attention stays unique per layer.
        self.shared_tail_mlp = MLP(model_dim, mlp_mult) if self.shared_tail_layers else None
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    include_mlp=i < self.shared_mlp_start,
                )
    ...
    def _forward_block(self, layer_idx: int, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        block = self.blocks[layer_idx]
        mix = block.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = block.attn(block.attn_norm(x_in) * block.ln_scale_factor, v_embed=v_embed)
        x_out = x_in + block.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        mlp = self.shared_tail_mlp if layer_idx >= self.shared_mlp_start and self.shared_tail_mlp is not None else block.mlp
        if mlp is None:
            raise RuntimeError(f"No MLP available for layer {layer_idx}")
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self._forward_block(i, x, x0, v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self._forward_block(bi, x, x0, v_embed=ve)
        x = self.final_norm(x)
...
def _classify_param(name: str) -> str:
    if "tok_emb" in name or "lm_head" in name:
        return "embed"
    if ".mlp." in name or "shared_tail_mlp" in name:
        return "mlp"
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_mlp_tail=args.shared_mlp_tail,
    ...
    shared_tail_named_params = list(base_model.shared_tail_mlp.named_parameters()) if base_model.shared_tail_mlp is not None else []
    matrix_params = [
        p
    ...
    matrix_params.extend(
        p
        for name, p in shared_tail_named_params
        if p.ndim == 2 and not any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)
    )
    ...
    log0(f"shared_tail_mlp:layers:{list(base_model.shared_tail_layers)}")
