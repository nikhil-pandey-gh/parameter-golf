...
class Hyperparameters:
    ...
    int6_sidecar_budget_bytes = int(os.environ.get("INT6_SIDECAR_BUDGET_BYTES", 65_536))
...
# --- GPTQ-lite int6 quantization + SpQR-lite row sidecar ---
...
def _row_sidecar_bytes(num_cols: int) -> int:
    return num_cols * torch.tensor([], dtype=torch.float16).element_size() + torch.tensor([], dtype=torch.int16).element_size()


def _attach_int6_row_sidecar(
    result: dict[str, Tensor],
    meta: dict[str, object],
    residual_cache: dict[str, Tensor],
    row_scores: dict[str, Tensor],
    budget_bytes: int,
) -> dict[str, int]:
    stats = {"rows": 0, "bytes": 0, "budget_bytes": max(0, budget_bytes)}
    if budget_bytes <= 0:
        return stats

    candidates: list[tuple[float, str, int, int]] = []
    for name, scores in row_scores.items():
        residual = residual_cache[name]
        row_bytes = _row_sidecar_bytes(residual.shape[1])
        for row_idx, score in enumerate(scores.tolist()):
            if score <= 0.0:
                continue
            candidates.append((float(score), name, row_idx, row_bytes))
    candidates.sort(key=lambda item: item[0], reverse=True)

    selected_rows: dict[str, list[int]] = {}
    used_bytes = 0
    for _, name, row_idx, row_bytes in candidates:
        if used_bytes + row_bytes > budget_bytes:
            continue
        selected_rows.setdefault(name, []).append(row_idx)
        used_bytes += row_bytes

    for name, rows in selected_rows.items():
        rows = sorted(rows)
        row_idx = torch.tensor(rows, dtype=torch.int16)
        result[name + ".sidecar_idx"] = row_idx
        result[name + ".sidecar_resid"] = residual_cache[name][rows].contiguous()
        info = meta.get(name)
        if isinstance(info, dict):
            info["sidecar"] = {"kind": "fp16_row_residual", "rows": len(rows)}

    stats["rows"] = sum(len(rows) for rows in selected_rows.values())
    stats["bytes"] = used_bytes
    return stats
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], sidecar_budget_bytes: int = 0):
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    residual_cache: dict[str, Tensor] = {}
    row_scores: dict[str, Tensor] = {}
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
            if sidecar_budget_bytes > 0 and t.ndim == 2:
                residual = (t.float() - dequantize_per_row(q, s)).to(torch.float16).contiguous()
                residual_cache[name] = residual
                row_scores[name] = residual.float().square().mean(dim=1)
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    sidecar_stats = _attach_int6_row_sidecar(result, meta, residual_cache, row_scores, sidecar_budget_bytes)
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        out_t = dequantize_per_row(q, s).to(orig_dtype)
        sidecar_idx_key = name + ".sidecar_idx"
        sidecar_resid_key = name + ".sidecar_resid"
        if sidecar_idx_key in result and sidecar_resid_key in result:
            sidecar_idx = result[sidecar_idx_key].to(dtype=torch.int64)
            sidecar_resid = result[sidecar_resid_key].to(dtype=out_t.dtype)
            out_t[sidecar_idx] = out_t[sidecar_idx] + sidecar_resid
        out[name] = out_t
    return out
...
def main() -> None:
    ...
    quant_result, quant_meta, sidecar_stats = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        sidecar_budget_bytes=args.int6_sidecar_budget_bytes,
    )
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta, "sidecar": sidecar_stats}, quant_buf)
    ...
    if master_process:
        ...
        log0(
            "int6_sidecar "
            f"rows:{sidecar_stats['rows']} raw_bytes:{sidecar_stats['bytes']} "
            f"budget_bytes:{sidecar_stats['budget_bytes']}"
        )
