...
try:
    ...
    _ATTN_BACKEND = "scaled_dot_product_attention"
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads:
            ref_head = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
            with torch.no_grad():
                for head in self.mtp_heads:
                    head.weight.copy_(ref_head)
...
def main() -> None:
    ...
    use_native_sdpa_fallback = _ATTN_BACKEND != "flash_attn_3"
    flash_sdp_enabled = not use_native_sdpa_fallback
    mem_efficient_sdp_enabled = use_native_sdpa_fallback
    math_sdp_enabled = use_native_sdpa_fallback
    ...
    if base_model.mtp_heads:
        mtp_head_lr = args.head_lr if args.head_lr > 0 else args.scalar_lr
        mtp_head_params = list(base_model.mtp_heads.parameters())
        optimizer_mtp = torch.optim.AdamW(
            [{"params": mtp_head_params, "lr": mtp_head_lr, "base_lr": mtp_head_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=args.adam_wd,
            fused=True,
    ...
    log0(
        ...
        f"mtp_head_lr:{(args.head_lr if args.head_lr > 0 else args.scalar_lr) if optimizer_mtp is not None else 0.0} "
