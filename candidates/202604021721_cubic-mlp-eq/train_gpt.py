...
class Hyperparameters:
    ...
    mlp_eq_alpha = float(os.environ.get("MLP_EQ_ALPHA", 0.75))
    mlp_eq_clip = float(os.environ.get("MLP_EQ_CLIP", 4.0))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if flash_attn_3_func is not None and x.is_cuda:
            ...
            y = F.scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, dropout_p=0.0, is_causal=True).transpose(1, 2)
        if self.use_xsa:
...
def apply_mlp_cubic_equalization(
    ...
    """Rebalance MLP hidden channels before export.

    For y = W_down * g(W_up x) with g(z) = leaky_relu(z)^2, any positive diagonal
    scale S can be migrated exactly between the two matrices by using
    W'_up = S W_up and W'_down = W_down S^{-2}. We use that freedom to reduce the
    mismatch between up-projection row ranges and down-projection column ranges
    before int6 GPTQ-lite quantization.
    """
    if alpha < 0.0:
        raise ValueError(f"MLP_EQ_ALPHA must be non-negative, got {alpha}")
    if clip < 1.0:
        raise ValueError(f"MLP_EQ_CLIP must be >= 1.0, got {clip}")
    if alpha == 0.0:
        return state_dict, {}
    out = {name: tensor.clone().contiguous() for name, tensor in state_dict.items()}
    ...
    for layer_idx in range(num_layers):
        ...
        up_row_abs = up.abs().amax(dim=1).clamp_min(eps)
        down_col_abs = down.abs().amax(dim=0).clamp_min(eps)
        scale = (down_col_abs / up_row_abs).pow(alpha / 3.0).clamp(1.0 / clip, clip)
        out[up_key] = (up * scale[:, None]).to(dtype=out[up_key].dtype).contiguous()
        out[down_key] = (down / scale.square()[None, :]).to(dtype=out[down_key].dtype).contiguous()
        stats[layer_idx] = {
...
def main() -> None:
    ...
    if args.mlp_eq_enabled:
        unbanked_sd, mlp_eq_stats = apply_mlp_cubic_equalization(
            unbanked_sd,
            args.num_layers,
            alpha=args.mlp_eq_alpha,
            clip=args.mlp_eq_clip,
        )
        if master_process and mlp_eq_stats:
            min_scale = min(stat["min_scale"] for stat in mlp_eq_stats.values())
            max_scale = max(stat["max_scale"] for stat in mlp_eq_stats.values())
            mean_scale = sum(stat["mean_scale"] for stat in mlp_eq_stats.values()) / len(mlp_eq_stats)
            log0(
                "mlp_cubic_eq:"
                f"layers:{len(mlp_eq_stats)} alpha:{args.mlp_eq_alpha:.2f} clip:{args.mlp_eq_clip:.2f} "
                f"scale_min:{min_scale:.3f} scale_mean:{mean_scale:.3f} scale_max:{max_scale:.3f}"
            )
