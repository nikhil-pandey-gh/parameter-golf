...
class Hyperparameters:
    ...
    mixq_reinvest_frac = float(os.environ.get("MIXQ_REINVEST_FRAC", 0.35))
...
# --- Adaptive mixed low-bit quantization ---
...
def _candidate_proxy_bytes(q: Tensor, scale: Tensor) -> int:
    payload = q.contiguous().numpy().tobytes() + scale.contiguous().numpy().tobytes()
    return len(zlib.compress(payload, level=9))


def _candidate_mse(t32: Tensor, q: Tensor, scale: Tensor) -> float:
    if scale.ndim > 0:
        recon = q.float() * scale.float().view(q.shape[0], *([1] * (q.ndim - 1)))
    else:
        recon = q.float() * float(scale.item())
    return float((t32 - recon).pow(2).mean().item())
...
def build_quant_candidate(t: Tensor, bits: int) -> QuantCandidate:
    if bits == 8:
        q, scale = quantize_float_tensor(t)
    else:
        q, scale = quantize_lowbit_per_row(t, bits=bits)
    t32 = t.float()
    return QuantCandidate(
        q=q,
        scale=scale,
        bits=bits,
        mse=_candidate_mse(t32, q, scale),
        proxy_bytes=_candidate_proxy_bytes(q, scale),
    )


def _candidate_bits_for_param(name: str, cat: str) -> tuple[int, ...]:
    if cat == "mlp":
        return (5, 6)
    if cat == "attn":
        return (6, 8)
    if name.startswith("bigram.") or name.startswith("ve_shared."):
        return (6, 8)
    return (8,)
...
def adaptive_mixed_quantize(state_dict: dict[str, Tensor], reinvest_frac: float):
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    selected: dict[str, QuantCandidate] = {}
    opportunities: list[tuple[float, str, QuantCandidate]] = []
    baseline_proxy_total = 0
    best_proxy_total = 0

    for name, tensor in state_dict.items():
        t = tensor.detach().cpu().contiguous()
        cat = _classify_param(name)
        if not t.is_floating_point() or t.numel() <= MIXQ_MIN_TENSOR_NUMEL:
        ...
        candidates = [build_quant_candidate(t, bits) for bits in _candidate_bits_for_param(name, cat)]
        baseline = min(candidates, key=lambda cand: (cand.proxy_bytes, cand.mse, cand.bits))
        best = min(candidates, key=lambda cand: (cand.mse, cand.proxy_bytes, cand.bits))
        selected[name] = baseline
        baseline_proxy_total += baseline.proxy_bytes
        best_proxy_total += best.proxy_bytes
        if best.bits != baseline.bits:
            extra_bytes = best.proxy_bytes - baseline.proxy_bytes
            mse_gain = baseline.mse - best.mse
            if extra_bytes > 0 and mse_gain > 0:
                score = mse_gain / extra_bytes
                opportunities.append((score, name, best))

    budget = int(max(0.0, (best_proxy_total - baseline_proxy_total) * reinvest_frac))
    spent = 0
    for _score, name, best in sorted(opportunities, reverse=True):
        current = selected[name]
        extra_bytes = best.proxy_bytes - current.proxy_bytes
        if extra_bytes <= 0 or spent + extra_bytes > budget:
            continue
        if best.mse >= current.mse:
            continue
        selected[name] = best
        spent += extra_bytes

    bit_counts = {5: 0, 6: 0, 8: 0}
    selected_proxy_total = 0
    for name, cand in selected.items():
        result[name + ".q"] = cand.q
        result[name + ".scale"] = cand.scale
        meta[name] = {"type": f"int{cand.bits}"}
        bit_counts[cand.bits] = bit_counts.get(cand.bits, 0) + 1
        selected_proxy_total += cand.proxy_bytes

    stats = {
        "proxy_baseline_bytes": baseline_proxy_total,
        "proxy_best_bytes": best_proxy_total,
        "proxy_selected_bytes": selected_proxy_total,
        "upgrade_budget_bytes": budget,
        "upgrade_spent_bytes": spent,
        "int5_tensors": bit_counts.get(5, 0),
        "int6_tensors": bit_counts.get(6, 0),
        "int8_tensors": bit_counts.get(8, 0),
    }
    return result, meta, stats


def dequantize_adaptive_mixed(result: dict[str, Tensor], meta: dict[str, object],
...
def main() -> None:
    ...
    quant_result, quant_meta, mixq_stats = adaptive_mixed_quantize(unbanked_sd, args.mixq_reinvest_frac)
    ...
    if master_process:
        ...
        log0(
            "mixq:"
            f" int5_tensors:{mixq_stats['int5_tensors']}"
            f" int6_tensors:{mixq_stats['int6_tensors']}"
            f" int8_tensors:{mixq_stats['int8_tensors']}"
            f" proxy_baseline_bytes:{mixq_stats['proxy_baseline_bytes']}"
            f" proxy_best_bytes:{mixq_stats['proxy_best_bytes']}"
            f" proxy_selected_bytes:{mixq_stats['proxy_selected_bytes']}"
            f" budget_bytes:{mixq_stats['upgrade_budget_bytes']}"
            f" spent_bytes:{mixq_stats['upgrade_spent_bytes']}"
        )
        log0(f"Serialized model mixq+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size mixq+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    ...
    deq_unbanked = dequantize_adaptive_mixed(quant_state["w"], quant_state["m"], unbanked_sd)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_mixq_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_mixq_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixq_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_mixq_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
    ...
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixq_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(f"final_mixq_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
