...
class GPT(nn.Module):
    ...
    def forward(
        ...
        input_ids: Tensor,
        target_ids: Tensor,
        mtp_loss_weight: Tensor | None = None,
    ) -> Tensor:
        n = self.num_layers
        ...
        runtime_mtp_loss_weight = self.default_mtp_loss_weight if mtp_loss_weight is None else mtp_loss_weight
        if self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + runtime_mtp_loss_weight.to(dtype=main_loss.dtype) * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    effective_mtp_num_heads = args.mtp_num_heads if args.mtp_loss_weight > 0.0 else 0
    base_model = GPT(
        ...
        mtp_num_heads=effective_mtp_num_heads,
    ...
    log0(
        f"mtp_num_heads:{base_model.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} "
        f"mtp_warmdown_threshold:{args.mtp_warmdown_threshold} mtp_params:{mtp_params}"
    )
    if args.mtp_num_heads > 0 and base_model.mtp_num_heads == 0:
        log0("mtp:disabled because MTP_LOSS_WEIGHT<=0")
    ...
    def mtp_weight_for_scale(scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_warmdown_threshold <= 0.0 or scale >= args.mtp_warmdown_threshold:
            return args.mtp_loss_weight
        return args.mtp_loss_weight * max(scale, 0.0) / args.mtp_warmdown_threshold
    if args.warmup_steps > 0:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        warmup_mtp_weight = torch.tensor(mtp_weight_for_scale(1.0), device=device, dtype=torch.float32)
    ...
    while True:
        ...
        mtp_schedule_elapsed_ms = elapsed_ms
        if max_wallclock_ms is not None:
            mtp_schedule_elapsed_ms += elapsed_ms / max(step, 1)
        mtp_loss_weight = mtp_weight_for_scale(lr_mul(step + 1, mtp_schedule_elapsed_ms))
        mtp_loss_weight_tensor = torch.tensor(mtp_loss_weight, device=device, dtype=torch.float32)
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_loss_weight_tensor)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_weight:{mtp_loss_weight:.4f}"
            )
