...
class Hyperparameters:
    ...
    dconv_last_n = int(os.environ.get("DCONV_LAST_N", 4))
    dconv_kernel_size = int(os.environ.get("DCONV_KERNEL_SIZE", 3))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,dconv",
    ).split(",")
    if pattern
)
...
class DepthwiseCausalConv1d(nn.Module):
    def __init__(self, channels: int, kernel_size: int):
        super().__init__()
        if kernel_size <= 0:
            raise ValueError(f"DCONV_KERNEL_SIZE must be positive, got {kernel_size}")
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y_t = F.conv1d(x_t, self.weight.to(dtype=x.dtype), groups=self.weight.size(0))
...
def causal_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if k_t.size(1) != q_t.size(1):
        if _SDPA_SUPPORTS_GQA:
            return F.scaled_dot_product_attention(q_t, k_t, v_t, is_causal=True, enable_gqa=True).transpose(1, 2)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.use_dconv = dconv_qkv
        kv_dim = self.num_kv_heads * self.head_dim
        self.q_dconv = DepthwiseCausalConv1d(dim, dconv_kernel_size) if dconv_qkv else None
        self.k_dconv = DepthwiseCausalConv1d(kv_dim, dconv_kernel_size) if dconv_qkv else None
        self.v_dconv = DepthwiseCausalConv1d(kv_dim, dconv_kernel_size) if dconv_qkv else None
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        dconv_qkv=dconv, dconv_kernel_size=dconv_kernel_size,
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    dconv=dconv_last_n > 0 and i >= max(0, num_layers - dconv_last_n),
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        dconv_last_n=args.dconv_last_n,
        dconv_kernel_size=args.dconv_kernel_size,
    ...
    dconv_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_dconv]
    log0(f"DCONV:last_{args.dconv_last_n} kernel:{args.dconv_kernel_size} active_layers:{dconv_layers}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        f"sdp_backends:cudnn=False flash=True "
        f"mem_efficient={use_sdpa_fallback} math={use_sdpa_fallback} "
        f"external_flash={flash_attn_3_func is not None} sdpa_gqa={_SDPA_SUPPORTS_GQA}"
    )
    ...
    eval_model = GPT(
        ...
        xsa_last_n=args.xsa_last_n,
        dconv_last_n=args.dconv_last_n, dconv_kernel_size=args.dconv_kernel_size,
