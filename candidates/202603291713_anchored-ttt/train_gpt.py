...
class Hyperparameters:
    ...
    ttt_anchor_lambda = float(os.environ.get("TTT_ANCHOR_LAMBDA", 0.001))
...
def eval_val_sliding_ttt(
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"anchor_lambda={args.ttt_anchor_lambda}")
    ...
    ttt_anchor_refs: list[Tensor] = []
    if args.ttt_anchor_lambda > 0.0:
        ttt_anchor_refs = [p.detach().clone() for p in ttt_params]
        log0(f"ttt_sliding:anchoring params={sum(p.numel() for p in ttt_params)}")

    optimizer = torch.optim.SGD(ttt_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
    t0 = time.perf_counter()
    last_anchor_penalty = 0.0
    ...
    for ci in range(num_chunks):
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            loss = base_model(x, y)
                        if ttt_anchor_refs:
                            # A small proximal anchor helps sequential TTT keep useful global structure
                            # instead of drifting too far toward the most recent chunk.
                            anchor_penalty = loss.new_zeros((), dtype=torch.float32)
                            for p, ref in zip(ttt_params, ttt_anchor_refs, strict=True):
                                anchor_penalty = anchor_penalty + (p.float() - ref.float()).square().sum()
                            last_anchor_penalty = float(anchor_penalty.detach().item())
                            loss = loss + args.ttt_anchor_lambda * anchor_penalty
                        loss.backward()
                        if world_size > 1:
                            for p in ttt_params:
        ...
        if rank == 0 and (ci % 10 == 0 or ci == num_chunks - 1):
            ...
            anchor_note = f" anchor_penalty={last_anchor_penalty:.3f}" if ttt_anchor_refs else ""
            log0(f"  ttt_chunk [{ci+1}/{num_chunks}] bpb={rbpb:.6f} time={elapsed:.1f}s{anchor_note}")
