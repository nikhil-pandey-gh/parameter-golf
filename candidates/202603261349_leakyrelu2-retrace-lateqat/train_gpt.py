...
def main() -> None:
    ...
    def compile_training_model(force_retrace: bool = False) -> nn.Module:
        if force_retrace and hasattr(torch, "_dynamo") and hasattr(torch._dynamo, "reset"):
            torch._dynamo.reset()
        ...
        wrapped.train()
        return wrapped
    model: nn.Module = compile_training_model()
    ...
    while True:
        ...
        late_qat_trigger = args.late_qat_threshold > 0 and scale < args.late_qat_threshold
        if distributed:
            trigger_tensor = torch.tensor(int(late_qat_trigger), device=device)
            dist.all_reduce(trigger_tensor, op=dist.ReduceOp.MAX)
            late_qat_trigger = bool(trigger_tensor.item())
        if late_qat_trigger and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
            model = compile_training_model(force_retrace=True)
        zero_grad_all()
