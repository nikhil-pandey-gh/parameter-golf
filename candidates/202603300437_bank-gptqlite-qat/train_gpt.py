...
class Hyperparameters:
    ...
    bank_qat_percentiles = os.environ.get("BANK_QAT_PERCENTILES", "0.9990,0.9995,0.9999,0.99999,1.0")
...
def compile_fullgraph(obj, device: torch.device | None = None):
    if not hasattr(torch, "compile"):
        return obj
    if not Hyperparameters.use_compile:
        return obj
...
class GPT(nn.Module):
    def __init__(
        ...
        ln_scale: bool = False,
        dtg: bool = False,
        bank_qat_percentiles: str = "0.9990,0.9995,0.9999,0.99999,1.0",
        ...
        self.bank_qat_percentiles = tuple(float(p) for p in bank_qat_percentiles.split(",") if p.strip())
        self.register_buffer("qo_bank_qscale", torch.ones(2 * num_layers, model_dim, dtype=torch.float32), persistent=False)
        self.register_buffer("kv_bank_qscale", torch.ones(2 * num_layers, kv_dim, dtype=torch.float32), persistent=False)
        self.register_buffer("mlp_up_bank_qscale", torch.ones(num_layers, mlp_dim, dtype=torch.float32), persistent=False)
        self.register_buffer("mlp_down_bank_qscale", torch.ones(num_layers, model_dim, dtype=torch.float32), persistent=False)
    ...
    @torch.no_grad()
    def refresh_bank_qat_scales(self) -> None:
        if not self.bank_qat_percentiles:
            raise ValueError("bank_qat_percentiles must be non-empty")
        for bank, qscale in (
            (self.qo_bank, self.qo_bank_qscale),
            (self.kv_bank, self.kv_bank_qscale),
            (self.mlp_up_bank, self.mlp_up_bank_qscale),
            (self.mlp_down_bank, self.mlp_down_bank_qscale),
        ):
            for idx in range(bank.size(0)):
                qscale[idx].copy_(select_int6_scales_for_matrix(bank[idx], self.bank_qat_percentiles))

    def enable_bank_qat(self) -> None:
        self.refresh_bank_qat_scales()
        self.bank_qat_enabled = True

    def _bank_slice(self, bank: Tensor, qscale: Tensor, idx: int) -> Tensor:
        weight = bank[idx]
        if self.bank_qat_enabled and self.training:
            return fake_quantize_int6_weight(weight, qscale[idx])
        return weight
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self._bank_slice(self.qo_bank, self.qo_bank_qscale, i),
                self._bank_slice(self.kv_bank, self.kv_bank_qscale, i),
                self._bank_slice(self.kv_bank, self.kv_bank_qscale, n + i),
                self._bank_slice(self.qo_bank, self.qo_bank_qscale, n + i),
                self._bank_slice(self.mlp_up_bank, self.mlp_up_bank_qscale, i),
                self._bank_slice(self.mlp_down_bank, self.mlp_down_bank_qscale, i),
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                self._bank_slice(self.qo_bank, self.qo_bank_qscale, bi),
                self._bank_slice(self.kv_bank, self.kv_bank_qscale, bi),
                self._bank_slice(self.kv_bank, self.kv_bank_qscale, n + bi),
                self._bank_slice(self.qo_bank, self.qo_bank_qscale, n + bi),
                self._bank_slice(self.mlp_up_bank, self.mlp_up_bank_qscale, bi),
                self._bank_slice(self.mlp_down_bank, self.mlp_down_bank_qscale, bi),
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = compile_fullgraph(base_model.forward_logits, device)
    with torch.inference_mode():
...
def select_int6_scales_for_matrix(t: Tensor, percentiles: tuple[float, ...], clip_range: int = 31) -> Tensor:
    t32 = t.float()
    best_scale = None
    best_err = float("inf")
    ...
    if best_scale is None:
        raise RuntimeError("Failed to select int6 scales")
    return best_scale.to(dtype=torch.float32)


def fake_quantize_int6_weight(weight: Tensor, scale: Tensor, clip_range: int = 31) -> Tensor:
    with torch.no_grad():
        w32 = weight.float()
        s32 = scale.float()
        q = torch.clamp(torch.round(w32 / s32[:, None]), -clip_range, clip_range)
        q_weight = (q * s32[:, None]).to(dtype=weight.dtype)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_qat_percentiles=args.bank_qat_percentiles,
    ...
    base_model.qo_bank_qscale.data = base_model.qo_bank_qscale.data.float()
    base_model.kv_bank_qscale.data = base_model.kv_bank_qscale.data.float()
    base_model.mlp_up_bank_qscale.data = base_model.mlp_up_bank_qscale.data.float()
    base_model.mlp_down_bank_qscale.data = base_model.mlp_down_bank_qscale.data.float()
    ...
    compiled_model = compile_fullgraph(base_model, device)
    ...
    attn_backend = "flash_attn_3" if flash_attn_3_func is not None and device.type == "cuda" else "torch_sdp"
    ...
    log0(
        f"bank_qat_enabled:{args.bank_qat_enabled} "
        f"bank_qat_percentiles:{args.bank_qat_percentiles}"
    )
    ...
    while True:
        ...
        late_qat_ready = args.late_qat_threshold > 0 and scale < args.late_qat_threshold
        recompile_for_late_qat = False
        if late_qat_ready and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            recompile_for_late_qat = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f}")
        if late_qat_ready and args.bank_qat_enabled and not base_model.bank_qat_enabled:
            base_model.enable_bank_qat()
            recompile_for_late_qat = True
            log0(
                f"late_bank_qat:enabled step:{step} scale:{scale:.4f} "
                f"percentiles:{args.bank_qat_percentiles}"
            )
        if recompile_for_late_qat:
            compiled_model = compile_fullgraph(base_model, device)
            model = compiled_model
        zero_grad_all()
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        log0("ema:disabled using live weights")
    torch.cuda.synchronize()
    ...
    torch.cuda.synchronize()
    log0(
        f"DIAGNOSTIC post_{averaging_mode} val_loss:{diag_val_loss:.4f} val_bpb:{diag_val_bpb:.4f} "
    ...
    compiled_eval = compile_fullgraph(eval_model, device)
    torch.cuda.synchronize()
