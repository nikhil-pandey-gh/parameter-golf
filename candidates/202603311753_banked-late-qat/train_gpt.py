...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.training and CastedLinear._qat_enabled:
            w = ste_fake_quant_int6_per_row(self.weight, pos_clip=31, neg_clip=32).to(x.dtype)
        else:
            w = self.weight.to(x.dtype)
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, w, bias)
def ste_fake_quant_int6_per_row(weight: Tensor, pos_clip: int = 31, neg_clip: int | None = None) -> Tensor:
    ...
    with torch.no_grad():
        ...
        scale = (row_max / float(pos_clip)).clamp_min(1.0 / float(pos_clip))
        w_q = torch.clamp(torch.round(w32 / scale[:, None]), -neg_clip, pos_clip) * scale[:, None]
    return weight + (w_q.to(dtype=weight.dtype) - weight).detach()
def maybe_fake_quantize_int6(weight: Tensor, training: bool) -> Tensor:
    if training and CastedLinear._qat_enabled:
        return ste_fake_quant_int6_per_row(weight)
    return weight
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        q_w = maybe_fake_quantize_int6(q_w, self.training)
        k_w = maybe_fake_quantize_int6(k_w, self.training)
        v_w = maybe_fake_quantize_int6(v_w, self.training)
        out_w = maybe_fake_quantize_int6(out_w, self.training)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        up_w = maybe_fake_quantize_int6(up_w, self.training)
        down_w = maybe_fake_quantize_int6(down_w, self.training)
...
def main() -> None:
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            log0(
                ...
                "scope:banked_and_casted recompiled:1"
            )
        zero_grad_all()
    ...
    if CastedLinear._qat_enabled:
        ...
        compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
        model = compiled_model
        log0("late_qat:disabled_for_eval recompiled:1")
    log0(
