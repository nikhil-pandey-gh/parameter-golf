...
class Hyperparameters:
    ...
    shared_depth = int(os.environ.get("SHARED_DEPTH", (num_layers + 1) // 2))
...
def build_mirror_layer_map(num_layers: int, shared_depth: int) -> list[int]:
    num_encoder_layers = num_layers // 2
    num_decoder_layers = num_layers - num_encoder_layers
    required_shared_depth = num_decoder_layers
    if shared_depth != required_shared_depth:
        raise ValueError(
            "Mirror-shared depth expects the U-Net-aligned shared depth "
            "(equal to the decoder depth for this stack); "
            f"got NUM_LAYERS={num_layers}, SHARED_DEPTH={shared_depth}, "
            f"required={required_shared_depth}"
        )
    layer_map = list(range(num_encoder_layers))
    for decoder_idx in range(num_decoder_layers):
        if decoder_idx < num_encoder_layers:
            layer_map.append(num_encoder_layers - 1 - decoder_idx)
        else:
            layer_map.append(num_encoder_layers + (decoder_idx - num_encoder_layers))
    if len(layer_map) != num_layers:
        raise RuntimeError(
            f"Mirror layer map length mismatch for NUM_LAYERS={num_layers}, "
            f"SHARED_DEPTH={shared_depth}: got {len(layer_map)}"
        )
    return layer_map
...
class GPT(nn.Module):
    def __init__(
        ...
        # Large matrices live in a mirrored bank, while each logical layer keeps
        # its own norms/scales/gates for specialization.
        head_dim = model_dim // num_heads
        kv_dim = num_kv_heads * head_dim
        mlp_dim = int(mlp_mult * model_dim)
        self.num_layers = num_layers
        self.shared_depth = shared_depth
        self.layer_to_bank = build_mirror_layer_map(num_layers, shared_depth)
        self.qo_bank = nn.Parameter(torch.empty(2 * shared_depth, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * shared_depth, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(shared_depth, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(shared_depth, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.shared_depth):
            ...
            nn.init.zeros_(self.qo_bank.data[self.shared_depth + i])    # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[self.shared_depth + i], gain=1.0)    # V
            ...
            self.qo_bank.data[self.shared_depth + i].mul_(proj_scale)
    ...
    def _bank_weights(self, layer_idx: int) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
        shared_idx = self.layer_to_bank[layer_idx]
        return (
            self.qo_bank[shared_idx],
            self.kv_bank[shared_idx],
            self.kv_bank[self.shared_depth + shared_idx],
            self.qo_bank[self.shared_depth + shared_idx],
            self.mlp_up_bank[shared_idx],
            self.mlp_down_bank[shared_idx],
        )
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0, *self._bank_weights(i), v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0, *self._bank_weights(bi), v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layers: int, shared_depth: int) -> dict[str, Tensor]:
    """Convert 3D bank tensors into individual 2D tensors with standard names."""
    out: dict[str, Tensor] = {}
    layer_to_bank = build_mirror_layer_map(num_layers, shared_depth)
    for name, tensor in sd.items():
        if name == "qo_bank":
            for i, shared_idx in enumerate(layer_to_bank):
                out[f"blocks.{i}.attn.c_q.weight"] = tensor[shared_idx]
                out[f"blocks.{i}.attn.proj.weight"] = tensor[shared_depth + shared_idx]
        elif name == "kv_bank":
            for i, shared_idx in enumerate(layer_to_bank):
                out[f"blocks.{i}.attn.c_k.weight"] = tensor[shared_idx]
                out[f"blocks.{i}.attn.c_v.weight"] = tensor[shared_depth + shared_idx]
        elif name == "mlp_up_bank":
            for i, shared_idx in enumerate(layer_to_bank):
                out[f"blocks.{i}.mlp.fc.weight"] = tensor[shared_idx]
        elif name == "mlp_down_bank":
            for i, shared_idx in enumerate(layer_to_bank):
                out[f"blocks.{i}.mlp.proj.weight"] = tensor[shared_idx]
        else:
            out[name] = tensor
    return out

def _rebank_state_dict(
    sd: dict[str, Tensor], num_layers: int, shared_depth: int, template_sd: dict[str, Tensor]
) -> dict[str, Tensor]:
    """Convert individual 2D tensors back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    layer_to_bank = build_mirror_layer_map(num_layers, shared_depth)
    # Reconstruct banks from individual weight keys
    qo_slices = [None] * (2 * shared_depth)
    kv_slices = [None] * (2 * shared_depth)
    up_slices = [None] * shared_depth
    down_slices = [None] * shared_depth
    consumed = set()
    for i, shared_idx in enumerate(layer_to_bank):
        qk = f"blocks.{i}.attn.c_q.weight"
        if qk in sd:
            if qo_slices[shared_idx] is None:
                qo_slices[shared_idx] = sd[qk]
            consumed.add(qk)
        ok = f"blocks.{i}.attn.proj.weight"
        if ok in sd:
            if qo_slices[shared_depth + shared_idx] is None:
                qo_slices[shared_depth + shared_idx] = sd[ok]
            consumed.add(ok)
        kk = f"blocks.{i}.attn.c_k.weight"
        if kk in sd:
            if kv_slices[shared_idx] is None:
                kv_slices[shared_idx] = sd[kk]
            consumed.add(kk)
        vk = f"blocks.{i}.attn.c_v.weight"
        if vk in sd:
            if kv_slices[shared_depth + shared_idx] is None:
                kv_slices[shared_depth + shared_idx] = sd[vk]
            consumed.add(vk)
        fk = f"blocks.{i}.mlp.fc.weight"
        if fk in sd:
            if up_slices[shared_idx] is None:
                up_slices[shared_idx] = sd[fk]
            consumed.add(fk)
        dk = f"blocks.{i}.mlp.proj.weight"
        if dk in sd:
            if down_slices[shared_idx] is None:
                down_slices[shared_idx] = sd[dk]
            consumed.add(dk)
...
def main() -> None:
    ...
    log0(f"shared_depth:{args.shared_depth} layer_to_bank:{base_model.layer_to_bank}")
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers, args.shared_depth)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, args.shared_depth, sd_cpu)
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, shared_depth=args.shared_depth, model_dim=args.model_dim,
