...
def flash_attention(q: Tensor, k: Tensor, v: Tensor, *, causal: bool, enable_gqa: bool) -> Tensor:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        y = flash_attention(q, k, v, causal=True, enable_gqa=(self.num_kv_heads != self.num_heads))
        if self.use_xsa:
...
def run_smoke_test() -> None:
    ...
    model = GPT(**smoke_kwargs)
    input_ids = torch.randint(0, smoke_kwargs["vocab_size"], (2, 24))
    target_ids = torch.randint(0, smoke_kwargs["vocab_size"], (2, 24))
    loss = model(input_ids, target_ids)
    if not torch.isfinite(loss):
        raise RuntimeError(f"smoke loss is not finite: {loss.item()}")
    loss.backward()

    export_sd = {k: v.detach().cpu() for k, v in model.state_dict().items() if "mtp_heads" not in k}
    if any("mtp_heads" in name for name in export_sd):
        raise RuntimeError("MTP heads should be excluded from export")
    ...
    eval_model = GPT(**(smoke_kwargs | {"mtp_num_heads": 0, "mtp_loss_weight": 0.0}))
    eval_model.load_state_dict(deq_state, strict=True)
    with torch.no_grad():
        logits = eval_model.forward_logits(input_ids)
    if logits.shape != (2, 24, smoke_kwargs["vocab_size"]):
        raise RuntimeError(f"unexpected smoke logits shape: {tuple(logits.shape)}")
    print(
        "smoke_test:ok "
        f"loss:{loss.item():.4f} logits_shape:{tuple(logits.shape)} "
        f"quant_tensors:{sum(name.endswith('.q') for name in quant_result)} "
        f"flash_attn:{flash_attn_3_func is not None}"
    )
