...
def parse_shared_mlp_groups(spec: str, num_layers: int) -> list[tuple[int, ...]]:
    ...
    for raw_group in spec.split(","):
        ...
        if not group_text:
            continue
        if "-" in group_text:
            lo_text, hi_text = group_text.split("-", 1)
            lo = int(lo_text)
            hi = int(hi_text)
            if hi < lo:
                raise ValueError(f"Invalid shared MLP group {group_text!r}: hi < lo")
            layers = tuple(range(lo, hi + 1))
        else:
            layers = (int(group_text),)
        if len(layers) < 2:
            raise ValueError(
                f"Shared MLP group {group_text!r} must contain at least two layers"
            )
        for layer_idx in layers:
            if not 0 <= layer_idx < num_layers:
                raise ValueError(
                    f"Shared MLP layer index {layer_idx} is out of range for NUM_LAYERS={num_layers}"
                )
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult) if create_mlp else None
    ...
    def forward(
        ...
        mlp_override: MLP | None = None,
    ) -> Tensor:
        mlp = mlp_override if mlp_override is not None else self.mlp
        if mlp is None:
            raise RuntimeError("Block MLP is not configured")
...
class GPT(nn.Module):
    def __init__(
        ...
        parsed_shared_groups = parse_shared_mlp_groups(shared_mlp_groups, num_layers)
        self.shared_mlp_groups = tuple(parsed_shared_groups)
        self.shared_mlp_assignment = [-1] * num_layers
        self.shared_mlps = nn.ModuleList()
        for group in parsed_shared_groups:
            shared_idx = len(self.shared_mlps)
            self.shared_mlps.append(MLP(model_dim, mlp_mult))
            for layer_idx in group:
                self.shared_mlp_assignment[layer_idx] = shared_idx
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    create_mlp=self.shared_mlp_assignment[i] < 0,
                )
    ...
    def _get_shared_mlp(self, layer_idx: int) -> MLP | None:
        shared_idx = self.shared_mlp_assignment[layer_idx]
        if shared_idx < 0:
            return None
        return self.shared_mlps[shared_idx]
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, v_embed=ve, mlp_override=self._get_shared_mlp(i))
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, v_embed=ve, mlp_override=self._get_shared_mlp(bi))
        x = self.final_norm(x)
...
def main() -> None:
    ...
    shared_group_desc = ",".join(
        f"{group[0]}-{group[-1]}" for group in base_model.shared_mlp_groups
    ) if base_model.shared_mlp_groups else "none"
    log0(f"shared_mlp_groups:{shared_group_desc} shared_mlp_export:int8")
