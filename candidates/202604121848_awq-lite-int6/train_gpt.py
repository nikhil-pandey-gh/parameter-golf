...
class Hyperparameters:
    ...
    awq_importance_power = float(os.environ.get("AWQ_IMPORTANCE_POWER", 0.5))
    awq_importance_clip = float(os.environ.get("AWQ_IMPORTANCE_CLIP", 4.0))
...
def _accum_channel_sq(stats: dict[str, list[Tensor]], name: str, x: Tensor) -> None:
    sq_sum = x.detach().float().pow(2).sum(dim=(0, 1)).to(dtype=torch.float64)
    count = torch.tensor(float(x.shape[0] * x.shape[1]), device=x.device, dtype=torch.float64)
    if name in stats:
        stats[name][0] += sq_sum
        stats[name][1] += count
    else:
        stats[name] = [sq_sum, count]
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.gated_attention:
            ...
            y = y * gate
        proj_input = y.reshape(bsz, seqlen, dim)
        out = F.linear(proj_input, out_w.to(x.dtype))
        if return_proj_input:
            return out, raw_v, proj_input
        return out, raw_v
...
class GPT(nn.Module):
    ...
    @torch.no_grad()
    def collect_quant_input_stats(self, input_ids: Tensor, stats: dict[str, list[Tensor]]) -> None:
        n = self.num_layers
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        ...
        def run_block(layer_idx: int, hidden: Tensor) -> tuple[Tensor, Tensor | None]:
            block = self.blocks[layer_idx]
            ve = self._get_ve(layer_idx, input_ids, ve_cache)
            mix = block.resid_mix.to(dtype=hidden.dtype)
            x_in = mix[0][None, None, :] * hidden + mix[1][None, None, :] * x0
            attn_in = block.attn_norm(x_in) * block.ln_scale_factor
            for suffix in ("c_q", "c_k", "c_v"):
                _accum_channel_sq(stats, f"blocks.{layer_idx}.attn.{suffix}.weight", attn_in)
            attn_out, raw_v_local, attn_proj_in = block.attn(
                attn_in,
                ...
                v_embed=ve,
                v0=v0,
                return_proj_input=True,
            )
            _accum_channel_sq(stats, f"blocks.{layer_idx}.attn.proj.weight", attn_proj_in)
            x_out = x_in + block.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
            mlp_in = block.mlp_norm(x_out) * block.ln_scale_factor
            _accum_channel_sq(stats, f"blocks.{layer_idx}.mlp.fc.weight", mlp_in)
            mlp_out, mlp_hidden = block.mlp(
                mlp_in,
                self.mlp_up_bank[layer_idx],
                self.mlp_down_bank[layer_idx],
                return_hidden=True,
            )
            _accum_channel_sq(stats, f"blocks.{layer_idx}.mlp.proj.weight", mlp_hidden)
            ...
            if block.dtg_gate is not None:
                ...
                x_out = x_in + gate * (x_out - x_in)
            return x_out, raw_v_local

        for i in range(self.num_encoder_layers):
            x, raw_v = run_block(i, x)
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        for i in range(self.num_decoder_layers):
...
def collect_awq_lite_stats(
    ...
    if not args.awq_lite_enabled or args.awq_calibration_steps <= 0:
        return {}
    batch_multiple = args.train_seq_len * max(world_size, 1)
    global_batch_tokens = max(args.awq_calibration_batch_tokens, batch_multiple)
    global_batch_tokens = max((global_batch_tokens // batch_multiple) * batch_multiple, batch_multiple)
    ...
    for _ in range(args.awq_calibration_steps):
        x, _ = calib_loader.next_batch(global_batch_tokens, args.train_seq_len, 1)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
            base_model.collect_quant_input_stats(x, stats)
    if dist.is_available() and dist.is_initialized():
        for sum_sq, count in stats.values():
            dist.all_reduce(sum_sq, op=dist.ReduceOp.SUM)
            dist.all_reduce(count, op=dist.ReduceOp.SUM)
    finalized: dict[str, Tensor] = {}
    clip = max(args.awq_importance_clip, 1.0)
    for name, (sum_sq, count) in stats.items():
        importance = (sum_sq / count.clamp_min(1.0)).to(dtype=torch.float32)
        importance = importance.clamp_min(1e-8).pow(args.awq_importance_power)
        importance = importance / importance.mean().clamp_min(1e-8)
        if clip > 1.0:
            importance = importance.clamp(min=1.0 / clip, max=clip)
            importance = importance / importance.mean().clamp_min(1e-8)
        finalized[name] = importance.cpu().contiguous()
    if rank == 0:
        log0(
            f"awq_lite:enabled steps:{args.awq_calibration_steps} "
            f"batch_tokens:{global_batch_tokens} tracked_tensors:{len(finalized)} "
            f"power:{args.awq_importance_power:.3f} clip:{clip:.2f}"
        )
...
def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = 31,
    input_m2: Tensor | None = None,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        importance = None
        if input_m2 is not None and input_m2.ndim == 1 and input_m2.numel() == t32.shape[1]:
            importance = input_m2.float()
            importance = importance / importance.mean().clamp_min(1e-8)
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            err = (sq_err * importance[None, :]).mean().item() if importance is not None else sq_err.mean().item()
            if err < best_err:
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            input_m2 = awq_stats.get(name) if awq_stats is not None else None
            q, s = quantize_int6_per_row(t, input_m2=input_m2)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    awq_stats = collect_awq_lite_stats(args, base_model, rank, world_size, device, log0)
