...
ROTATION_GPTQ_ENABLED = bool(int(os.environ.get("ROTATION_GPTQ", "1")))
ROTATION_GPTQ_MIN_DIM = int(os.environ.get("ROTATION_GPTQ_MIN_DIM", 256))
GPTQ_INT6_PERCENTILES = (0.9990, 0.9995, 0.9999, 0.99999, 1.0)
GPTQ_INT8_PERCENTILES = (INT8_CLIP_Q, 1.0)
...
def _signed_hadamard_signs(name: str, size: int, axis: str) -> Tensor:
    seed = zlib.crc32(f"{name}:{axis}:{size}".encode("utf-8")) & 0xFFFFFFFF
    rng = np.random.default_rng(seed)
    signs = rng.integers(0, 2, size=size, dtype=np.int8)
    signs = 1 - 2 * signs
    return torch.from_numpy(signs.astype(np.float32, copy=False))


def _hadamard_last_dim(x: Tensor) -> Tensor:
    n = x.shape[-1]
    if not _is_power_of_two(n):
        raise ValueError(f"Hadamard width must be a power of two, got {n}")
    y = x.reshape(-1, n).float().contiguous().clone()
...
def _apply_hadamard_right(t: Tensor, signs: Tensor) -> Tensor:
    return _hadamard_last_dim(t * signs.view(1, -1))


def _invert_hadamard_right(t: Tensor, signs: Tensor) -> Tensor:
    return _hadamard_last_dim(t) * signs.view(1, -1)


def _apply_hadamard_left(t: Tensor, signs: Tensor) -> Tensor:
    tt = t.transpose(0, 1).contiguous()
    return _hadamard_last_dim(tt * signs.view(1, -1)).transpose(0, 1).contiguous()


def _invert_hadamard_left(t: Tensor, signs: Tensor) -> Tensor:
    tt = t.transpose(0, 1).contiguous()
    return (_hadamard_last_dim(tt) * signs.view(1, -1)).transpose(0, 1).contiguous()


def _rotation_candidates(name: str, t32: Tensor) -> list[tuple[str, Tensor, object]]:
    candidates: list[tuple[str, Tensor, object]] = [("none", t32, lambda x: x)]
    if not ROTATION_GPTQ_ENABLED or t32.ndim != 2:
        return candidates

    right_signs: Tensor | None = None
    left_signs: Tensor | None = None

    if t32.shape[1] >= ROTATION_GPTQ_MIN_DIM and _is_power_of_two(t32.shape[1]):
        right_signs = _signed_hadamard_signs(name, t32.shape[1], "right").to(dtype=t32.dtype)
        candidates.append(
            (
                "hadamard_right",
                _apply_hadamard_right(t32, right_signs),
                lambda x, signs=right_signs: _invert_hadamard_right(x, signs),
            )
        )
    if t32.shape[0] >= ROTATION_GPTQ_MIN_DIM and _is_power_of_two(t32.shape[0]):
        left_signs = _signed_hadamard_signs(name, t32.shape[0], "left").to(dtype=t32.dtype)
        candidates.append(
            (
                "hadamard_left",
                _apply_hadamard_left(t32, left_signs),
                lambda x, signs=left_signs: _invert_hadamard_left(x, signs),
            )
        )
    if right_signs is not None and left_signs is not None:
        candidates.append(
            (
                "hadamard_both",
                _apply_hadamard_left(_apply_hadamard_right(t32, right_signs), left_signs),
                lambda x, lsigns=left_signs, rsigns=right_signs: _invert_hadamard_right(
                    _invert_hadamard_left(x, lsigns), rsigns
                ),
            )
        )
    return candidates
...
def _invert_rotation(name: str, t: Tensor, rotation: str) -> Tensor:
    if rotation == "none":
        return t
    if rotation == "hadamard_right":
        signs = _signed_hadamard_signs(name, t.shape[1], "right").to(dtype=t.dtype)
        return _invert_hadamard_right(t, signs)
    if rotation == "hadamard_left":
        signs = _signed_hadamard_signs(name, t.shape[0], "left").to(dtype=t.dtype)
        return _invert_hadamard_left(t, signs)
    if rotation == "hadamard_both":
        left_signs = _signed_hadamard_signs(name, t.shape[0], "left").to(dtype=t.dtype)
        right_signs = _signed_hadamard_signs(name, t.shape[1], "right").to(dtype=t.dtype)
        return _invert_hadamard_right(_invert_hadamard_left(t, left_signs), right_signs)
    raise ValueError(f"Unknown rotation mode: {rotation}")


def _quantize_matrix_search(
    name: str,
    t: Tensor,
    clip_range: int,
    percentiles: tuple[float, ...],
    per_tensor_scale_dtype: torch.dtype,
) -> tuple[Tensor, Tensor, str]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err, best_rotation = None, None, float("inf"), "none"
        for rotation, rotated, inverse in _rotation_candidates(name, t32):
            rotated_abs = rotated.abs()
            for pct in percentiles:
                if pct < 1.0:
                    row_clip = torch.quantile(rotated_abs, pct, dim=1)
                else:
                    row_clip = rotated_abs.amax(dim=1)
                s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
                q = torch.clamp(torch.round(rotated / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
                recon = inverse(q.float() * s.float()[:, None])
                err = (t32 - recon).pow(2).mean().item()
                if err < best_err:
                    best_q, best_s, best_err, best_rotation = q, s, err, rotation
        if best_q is None or best_s is None:
            raise RuntimeError(f"Failed to quantize matrix {name}")
        return best_q, best_s, best_rotation

    pct = percentiles[0]
    clip_abs = float(torch.quantile(t32.abs().flatten(), pct).item()) if pct < 1.0 and t32.numel() else 0.0
    if pct >= 1.0 and t32.numel():
        clip_abs = float(t32.abs().max().item())
    scale = torch.tensor(clip_abs / clip_range if clip_abs > 0 else 1.0, dtype=per_tensor_scale_dtype)
    q = torch.clamp(torch.round(t32 / scale.float()), -clip_range, clip_range).to(torch.int8)
    return q, scale, "none"


def quantize_int6_per_row(name: str, t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, str]:
    return _quantize_matrix_search(name, t, clip_range, GPTQ_INT6_PERCENTILES, torch.float16)


def quantize_int8_per_row(name: str, t: Tensor, clip_range: int = 127) -> tuple[Tensor, Tensor, str]:
    return _quantize_matrix_search(name, t, clip_range, GPTQ_INT8_PERCENTILES, torch.float32)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, rotation = quantize_int6_per_row(name, t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "rotation": rotation}
        else:
            q, s, rotation = quantize_int8_per_row(name, t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8", "rotation": rotation}
    return result, meta
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict):
            deq = _invert_rotation(name, deq, str(info.get("rotation", "none")))
