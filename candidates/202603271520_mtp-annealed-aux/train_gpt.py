...
class Hyperparameters:
    ...
    mtp_anneal_threshold = float(os.environ.get("MTP_ANNEAL_THRESHOLD", 0.20))
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2 and not getattr(self, "_skip_qat", False):
            with torch.no_grad():
...
def mtp_aux_loss_scale(lr_scale: float, anneal_threshold: float) -> float:
    if anneal_threshold <= 0.0:
        return 1.0
    return min(max(lr_scale / anneal_threshold, 0.0), 1.0)
def export_model_kwargs(args: Hyperparameters) -> dict[str, object]:
    return {
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                aux_weight = self.mtp_loss_scale.to(dtype=main_loss.dtype) * self.mtp_loss_weight
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_anneal_threshold:{args.mtp_anneal_threshold} mtp_params:{mtp_params}"
    )
    ...
    while True:
        ...
        if base_model.mtp_num_heads > 0:
            current_mtp_scale = mtp_aux_loss_scale(scale, args.mtp_anneal_threshold)
            base_model.mtp_loss_scale.fill_(current_mtp_scale)
        ...
        if should_log_train:
            mtp_log = ""
            if base_model.mtp_num_heads > 0:
                mtp_log = f" mtp_scale:{float(base_model.mtp_loss_scale.item()):.4f}"
            log0(
    ...
    export_kwargs = export_model_kwargs(args)
    if excluded_mtp > 0:
        log0(f"export_excluding_mtp_params:{excluded_mtp}")
    if master_process:
        torch.save({"state_dict": export_sd, "model_kwargs": export_kwargs}, "final_model.pt")
    ...
    torch.save({"w": quant_result, "m": quant_meta, "model_kwargs": export_kwargs}, quant_buf)
    ...
    quant_model_kwargs = quant_state.get("model_kwargs", export_kwargs)
    deq_state = dequantize_mixed_int6(quant_state["w"], quant_state["m"], sd_cpu)
    eval_model = GPT(**quant_model_kwargs).to(device).bfloat16()
