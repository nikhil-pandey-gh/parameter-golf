...
class Hyperparameters:
    ...
    mtp_init_from_lm_head = bool(int(os.environ.get("MTP_INIT_FROM_LM_HEAD", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        mtp_init_from_lm_head: bool = True,
        ...
        self.mtp_init_from_lm_head = mtp_init_from_lm_head
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads and self.mtp_init_from_lm_head:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_init_from_lm_head=args.mtp_init_from_lm_head,
    ...
    log0(
        ...
        f"mtp_params:{mtp_params} mtp_init_from_lm_head:{args.mtp_init_from_lm_head}"
    )
    ...
    if master_process:
        final_model_path = SCRIPT_DIR / "final_model.pt"
        torch.save({"model_kwargs": export_model_kwargs, "state_dict": export_sd}, final_model_path)
    ...
    torch.save({"model_kwargs": export_model_kwargs, "w": quant_result, "m": quant_meta}, quant_buf)
    quant_raw = quant_buf.getvalue()
    quant_blob = zstandard.ZstdCompressor(level=22).compress(quant_raw) if _COMPRESSOR == "zstd" else zlib.compress(quant_raw, 9)
    if master_process:
        quant_path = SCRIPT_DIR / "final_model.int6.ptz"
    ...
    if distributed:
        dist.barrier()
    with open(SCRIPT_DIR / "final_model.int6.ptz", "rb") as f:
    ...
    eval_model = GPT(**quant_state["model_kwargs"]).to(device).bfloat16()
