    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if flash_attn_3_func is not None and q.is_cuda:
            ...
            if self.num_kv_heads != self.num_heads:
                ...
                k_attn = k.repeat_interleave(group_size, dim=2)
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    if any("mtp_heads" in name for name in export_sd):
        raise RuntimeError("smoke_test export unexpectedly kept MTP heads")
    ...
    if logits.shape != (2, 32, vocab_size):
        raise RuntimeError(f"smoke_test logits shape mismatch: {tuple(logits.shape)}")
    print(
        f"smoke_test:ok loss:{loss.item():.4f} "
        f"logits_shape:{tuple(logits.shape)} mtp_heads:{mtp_num_heads}"
    )
def main() -> None:
