...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,depth_bias",
    ).split(",")
    if pattern
)
...
class SharedBlockCore(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, layer: LayerAdapter, v_embed: Tensor | None = None) -> Tensor:
        mix = layer.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        x_in = x_in + layer.depth_bias.to(dtype=x.dtype)[None, None, :]
        attn_out = self.attn(
        ...
        x_out = x_in + layer.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        x_out = x_out + layer.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(
...
class GPT(nn.Module):
    def __init__(
        ...
        if xsa_last_n > 0:
            for i in range(max(0, num_layers - xsa_last_n), num_layers):
                self.layer_adapters[i].use_xsa = True
        self._init_weights()
    ...
    def _run_backbone(self, input_ids: Tensor) -> Tensor:
        ...
        skips: list[Tensor] = []
        ve_cache: dict = {}
        for layer_idx, layer in enumerate(self.layer_adapters):
            if layer_idx < self.num_encoder_layers:
                ve = self._get_ve(layer_idx, input_ids, ve_cache)
                core = self.shared_blocks[layer_idx % self.num_shared_blocks]
                x = core(x, x0, layer, v_embed=ve)
                skips.append(x)
                continue
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    return out
def build_export_config(args: Hyperparameters) -> dict[str, object]:
    return {
...
def main() -> None:
    ...
    block_named_params = [
        ...
        if name.startswith("shared_blocks.") or name.startswith("layer_adapters.")
    ]
    matrix_params = [
        p
    ...
    log0(f"shared_blocks:{args.num_shared_blocks} virtual_layers:{args.num_layers} mlp_leaky_slope:{args.mlp_leaky_slope}")
    ...
    if master_process:
        torch.save({"config": export_config, "state_dict": export_sd}, "final_model.pt")
    ...
    torch.save({"config": export_config, "w": quant_result, "m": quant_meta}, quant_buf)
    ...
    eval_model = GPT(**quant_state.get("config", export_config)).to(device).bfloat16()
