...
class Hyperparameters:
    ...
    quant_boundary_layers = int(os.environ.get("QUANT_BOUNDARY_LAYERS", 1))
...
def quantize_gptq_per_row(t: Tensor, clip_range: int) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    return quantize_gptq_per_row(t, clip_range)
def _use_boundary_int8(name: str, num_layers_total: int, boundary_layers: int) -> bool:
    if boundary_layers <= 0 or _classify_param(name) not in {"mlp", "attn"}:
        return False
    block_idx = _block_index(name)
    if block_idx is None:
        return False
    return block_idx < boundary_layers or block_idx >= num_layers_total - boundary_layers
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], boundary_layers: int):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        if _use_boundary_int8(name, num_layers_total, boundary_layers):
            q, s = quantize_gptq_per_row(t, clip_range=127)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8_boundary"}
...
def run_smoke_test(args: Hyperparameters) -> None:
    if args.quant_boundary_layers < 0:
        raise ValueError(f"QUANT_BOUNDARY_LAYERS must be non-negative, got {args.quant_boundary_layers}")
    ...
    quant_result, quant_meta = mixed_quantize_int6(state_dict, {"mlp", "attn"}, args.quant_boundary_layers)
    deq_state = dequantize_mixed_int6(quant_result, quant_meta, state_dict)
    missing = sorted(set(state_dict) - set(deq_state))
    if missing:
        raise RuntimeError(f"Smoke dequantization missed tensors: {missing[:5]}")
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, args.quant_boundary_layers)
    ...
    if master_process:
        ...
        boundary_promoted = sum(
            1
            for info in quant_meta.values()
            if isinstance(info, dict) and info.get("type") == "int8_boundary"
        )
        log0(
            f"quant_boundary_layers:{args.quant_boundary_layers} "
            f"boundary_promoted_tensors:{boundary_promoted}"
        )
    if distributed:
        dist.barrier()
