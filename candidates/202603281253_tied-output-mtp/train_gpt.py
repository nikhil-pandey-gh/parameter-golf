...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            mtp_loss_count = 0
            for k, mtp_block in enumerate(self.mtp_blocks):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
                mtp_hidden = F.rms_norm(
                    x[:, :valid_t, :] + mtp_block(x[:, :valid_t, :]),
                    (dim,),
                ).reshape(-1, dim)
                mtp_targets = target_ids[:, k + 1 :].reshape(-1)
                mtp_logits = self._project_logits(mtp_hidden)
...
def main() -> None:
    ...
    if base_model.mtp_num_heads > 0:
        matrix_params.extend([p for p in base_model.mtp_blocks.parameters() if p.ndim == 2])
    scalar_params = [
        p
        for name, p in block_named_params
    ...
    mtp_params = sum(p.numel() for p in base_model.mtp_blocks.parameters())
    log0(f"model_params:{n_params}")
    log0(
        f"mtp_mode:tied_residual mtp_num_heads:{args.mtp_num_heads} "
    ...
    export_sd = {k: v for k, v in full_state_dict.items() if "mtp_blocks" not in k}
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_blocks" in k)
    if excluded_mtp > 0:
