...
class Hyperparameters:
    ...
    softmax1_enabled = bool(int(os.environ.get("SOFTMAX1_ENABLED", "1")))
...
def scaled_dot_product_attention_fallback(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    scores = torch.matmul(qh, kh.transpose(-2, -1)) * (qh.size(-1) ** -0.5)
    q_idx = torch.arange(qh.size(-2), device=scores.device)[:, None]
    k_idx = torch.arange(kh.size(-2), device=scores.device)[None, :]
    causal_mask = k_idx <= q_idx
    scores = scores.masked_fill(~causal_mask, float("-inf"))
    probs = torch.softmax(scores.float(), dim=-1).to(dtype=qh.dtype)
    return torch.matmul(probs, vh).permute(0, 2, 1, 3).contiguous()
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.use_softmax1 = softmax1_enabled
    ...
    def _add_softmax1_null_slot(self, q: Tensor, k: Tensor, v: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        # Appending a leading zero Q/K/V token makes standard causal softmax exactly match
        # softmax-1 on the original tokens, while preserving FlashAttention compatibility.
        q0 = q.new_zeros((q.size(0), 1, q.size(2), q.size(3)))
        k0 = k.new_zeros((k.size(0), 1, k.size(2), k.size(3)))
        v0 = v.new_zeros((v.size(0), 1, v.size(2), v.size(3)))
        return torch.cat((q0, q), dim=1), torch.cat((k0, k), dim=1), torch.cat((v0, v), dim=1)

    def _attend(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        if q.is_cuda:
            if flash_attn_3_func is None:
                raise RuntimeError("flash_attn_interface is required for CUDA runs; the dense fallback is CPU-only")
            return flash_attn_3_func(q, k, v, causal=True)
        return scaled_dot_product_attention_fallback(q, k, v)
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if self.use_softmax1:
            q, k, v = self._add_softmax1_null_slot(q, k, v)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            ...
            rope_base,
            qk_gain_init,
            softmax1_enabled=softmax1_enabled,
        )
...
def main() -> None:
    ...
    if flash_attn_3_func is None:
        raise RuntimeError("flash_attn_interface is required for CUDA training/eval; fallback is only for CPU smoke use")
    ...
    base_model = GPT(
        ...
        softmax1_enabled=args.softmax1_enabled,
    ...
    log0(f"softmax1_enabled:{args.softmax1_enabled} mlp_negative_slope:{args.mlp_negative_slope}")
    ...
    if master_process:
        ...
        log0(f"Serialized dense model: {model_bytes} bytes")
    ...
    eval_model = GPT(
        ...
        softmax1_enabled=args.softmax1_enabled, mlp_negative_slope=args.mlp_negative_slope,
    ).to(device).bfloat16()
