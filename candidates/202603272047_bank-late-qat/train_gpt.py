...
def fake_quantize_int6_ste(weight: Tensor, clip_range: int = 31) -> Tensor:
    w32 = weight.float()
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # No CastedLinear -- weights come from banks
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor, qat_active: bool = False) -> Tensor:
        if qat_active:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, qat_active: bool = False) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                v_embed=ve, v0=v0, qat_active=qat_active)
...
def main() -> None:
    ...
    log0(
        f"bank_qat_enabled:{args.bank_qat_enabled} late_qat_threshold:{args.late_qat_threshold:.4f}"
    )
    ...
    while True:
        ...
        bank_qat_active = args.bank_qat_enabled and args.late_qat_threshold > 0 and scale < args.late_qat_threshold
        if bank_qat_active and not bank_qat_logged:
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, bank_qat_active)
