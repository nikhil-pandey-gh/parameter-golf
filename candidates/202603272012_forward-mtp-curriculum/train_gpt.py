...
class Hyperparameters:
    ...
    mtp_ramp_frac = float(os.environ.get("MTP_RAMP_FRAC", 0.50))
    ...
    def set_mtp_head_weights(self, head_weights: Tensor, active_heads: int) -> None:
        if head_weights.shape != self.mtp_head_weights.shape:
            raise ValueError(
                ...
                f"got {tuple(head_weights.shape)}"
            )
        with torch.no_grad():
...
def build_mtp_head_weights(
    ...
    start_step = max(warmup_steps, int(start_frac * total_steps))
...
def main() -> None:
    ...
    # - scalars/control tensors + small auxiliary matrices -> Adam
    # - bigram proj, MTP heads, VE proj stay out of the banks to keep Muon batched
    matrix_params = [
    ...
    log0(
        ...
        f"mtp_start_frac:{args.mtp_start_frac:.2f} mtp_ramp_frac:{args.mtp_ramp_frac:.2f}"
    )
    log0(f"mtp_active_heads:{base_model.active_mtp_heads}")
    ...
    while True:
        ...
        if should_log_train:
            mtp_status = ""
            if base_model.mtp_head_weights.numel() > 0:
                mtp_weights = ",".join(f"{float(w):.2f}" for w in base_model.mtp_head_weights.tolist())
                mtp_status = f" mtp_active_heads:{base_model.active_mtp_heads} mtp_weights:[{mtp_weights}]"
            log0(
