...
class Hyperparameters:
    ...
    int6_outlier_frac = float(os.environ.get("INT6_OUTLIER_FRAC", 5e-4))
    int6_outlier_max_k = int(os.environ.get("INT6_OUTLIER_MAX_K", 512))
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def _pick_sparse_outliers(residual: Tensor, fraction: float, max_k: int) -> tuple[Tensor | None, Tensor | None]:
    if not 0.0 <= fraction <= 1.0:
        raise ValueError(f"INT6_OUTLIER_FRAC must be in [0, 1], got {fraction}")
    if max_k < 0:
        raise ValueError(f"INT6_OUTLIER_MAX_K must be non-negative, got {max_k}")
    if fraction <= 0.0 or max_k == 0:
        return None, None
    flat = residual.reshape(-1)
    if flat.numel() == 0:
        return None, None
    k = int(math.ceil(flat.numel() * fraction))
    k = min(k, max_k, flat.numel())
    if k <= 0:
        return None, None
    topk = torch.topk(flat.abs(), k, sorted=False)
    if topk.indices.numel() == 0:
        return None, None
    keep = topk.values > 0
    if not torch.any(keep):
        return None, None
    idx = topk.indices[keep]
    return idx.to(torch.int32).contiguous(), flat[idx.long()].to(torch.float16).contiguous()
def quantize_int6_per_row(
    ...
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_idx, best_vals, best_err = None, None, None, None, float('inf')
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            residual = t32 - recon
            idx, vals = _pick_sparse_outliers(residual, outlier_frac, outlier_max_k)
            if idx is not None:
                flat = residual.reshape(-1)
                err = ((flat.square().sum() - flat[idx.long()].square().sum()) / flat.numel()).item()
            else:
                err = residual.pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_idx, best_vals, best_err = q, s, idx, vals, err
        return best_q, best_s, best_idx, best_vals
    amax = t32.abs().max().item()
    scale = torch.tensor(amax / clip_range if amax > 0 else 1.0, dtype=torch.float16)
    q = torch.clamp(torch.round(t32 / scale.float()), -clip_range, clip_range).to(torch.int8)
    residual = t32 - q.float() * float(scale.item())
    idx, vals = _pick_sparse_outliers(residual, outlier_frac, outlier_max_k)
    return q, scale, idx, vals
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, out_idx, out_vals = quantize_int6_per_row(
                t,
                outlier_frac=outlier_frac,
                outlier_max_k=outlier_max_k,
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
            info: dict[str, object] = {"type": "int6"}
            if out_idx is not None and out_vals is not None:
                result[name + ".outlier_idx"] = out_idx
                result[name + ".outlier_val"] = out_vals
                info = {"type": "int6_sparse", "outliers": int(out_idx.numel())}
            meta[name] = info
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("type") == "int6_sparse":
            t.reshape(-1).index_add_(0, result[name + ".outlier_idx"].long(), result[name + ".outlier_val"].float())
...
def main() -> None:
    ...
    log0(f"int6_sparse_outliers:frac={args.int6_outlier_frac:g} max_k={args.int6_outlier_max_k}")
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, args.int6_outlier_frac, args.int6_outlier_max_k)
