...
class Hyperparameters:
    ...
    mtp_head_weights = tuple(
        float(x)
        for x in os.environ.get("MTP_HEAD_WEIGHTS", "1.0,0.5").split(",")
        if x.strip()
    )
...
def causal_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    return F.scaled_dot_product_attention(qh, kh, vh, attn_mask=None, is_causal=True).transpose(1, 2)
...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_num_heads > 0:
            if mtp_head_weights:
                padded_weights = list(mtp_head_weights)
                if len(padded_weights) < mtp_num_heads:
                    padded_weights.extend([padded_weights[-1]] * (mtp_num_heads - len(padded_weights)))
                self.mtp_head_weights = tuple(float(max(w, 0.0)) for w in padded_weights[:mtp_num_heads])
            else:
                self.mtp_head_weights = tuple(0.5 ** k for k in range(mtp_num_heads))
        else:
            self.mtp_head_weights = ()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                head_weight = self.mtp_head_weights[k] if k < len(self.mtp_head_weights) else 1.0
                if head_weight <= 0.0:
                    continue
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
...
def run_cpu_smoke_test() -> None:
    ...
    target_ids = torch.randint(0, 128, (2, 16), dtype=torch.long)
    model.train()
    loss = model(input_ids, target_ids)
    if not torch.isfinite(loss):
        raise RuntimeError("cpu smoke test produced a non-finite loss")
    loss.backward()
    ...
    if logits.shape != (2, 16, 128):
        raise RuntimeError(f"unexpected logits shape from cpu smoke test: {tuple(logits.shape)}")
    print("cpu_smoke_test:ok")
...
def main() -> None:
    ...
    args = Hyperparameters()
    if os.environ.get("CPU_SMOKE_TEST") == "1":
        run_cpu_smoke_test()
        return
    ...
    base_model = GPT(
        ...
        mtp_head_weights=args.mtp_head_weights,
    ...
    log0(
        ...
        f"mtp_head_weights:{args.mtp_head_weights} mtp_params:{mtp_params}"
    )
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_head_weights=(),
