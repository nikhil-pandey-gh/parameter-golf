...
def attention_with_fallback(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    if flash_attn_3_func is not None and q.is_cuda and k.is_cuda and v.is_cuda:
...
class Hyperparameters:
    ...
    qat_clip_gain_range = float(os.environ.get("QAT_CLIP_GAIN_RANGE", 0.5))
...
def clip_multiplier_from_log_scale(log_scale: Tensor, gain_range: float) -> Tensor:
    return 1.0 + gain_range * torch.tanh(log_scale.float())

def _ste_round(x: Tensor) -> Tensor:
    return x + (torch.round(x) - x).detach()

def fake_quantize_int6_per_row_ste(t: Tensor, clip_multiplier: Tensor | float | None, clip_range: int = 31) -> Tensor:
    if t.ndim != 2:
        raise ValueError(f"Expected a 2D weight matrix for STE fake quantization, got shape {tuple(t.shape)}")
    t32 = t.float()
    row_clip = t32.abs().amax(dim=1).clamp_min(1.0 / clip_range)
    if clip_multiplier is not None:
        row_clip = row_clip * torch.as_tensor(clip_multiplier, dtype=torch.float32, device=t32.device)
    scale = (row_clip / clip_range).clamp_min(1.0 / clip_range)
    scaled = t32 / scale[:, None]
    clipped = scaled.clamp(-clip_range, clip_range)
    rounded = _ste_round(clipped)
    return (rounded * scale[:, None]).to(dtype=t.dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = attention_with_fallback(q, k, v, causal=True)
        if self.use_xsa:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.qat_clip_gain_range = qat_clip_gain_range
        ...
        self.qo_qat_log_scale = nn.Parameter(torch.zeros(2 * num_layers, dtype=torch.float32))
        self.kv_qat_log_scale = nn.Parameter(torch.zeros(2 * num_layers, dtype=torch.float32))
        self.mlp_up_qat_log_scale = nn.Parameter(torch.zeros(num_layers, dtype=torch.float32))
        self.mlp_down_qat_log_scale = nn.Parameter(torch.zeros(num_layers, dtype=torch.float32))
        ...
        self._init_weights()
    def _bank_clip_multiplier(self, log_scale: Tensor) -> Tensor:
        return clip_multiplier_from_log_scale(log_scale, self.qat_clip_gain_range)

    def _maybe_qat_bank_weight(self, weight: Tensor, log_scale: Tensor, qat_strength: Tensor | None) -> Tensor:
        if qat_strength is None or not self.training or weight.ndim != 2:
            return weight
        clip_multiplier = self._bank_clip_multiplier(log_scale).to(device=weight.device)
        qat_strength = qat_strength.to(device=weight.device, dtype=weight.dtype)
        fake = fake_quantize_int6_per_row_ste(weight, clip_multiplier)
        return weight + (fake - weight) * qat_strength
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, qat_strength: Tensor | None = None) -> Tensor:
        n = self.num_layers
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self._maybe_qat_bank_weight(self.qo_bank[i], self.qo_qat_log_scale[i], qat_strength),
                self._maybe_qat_bank_weight(self.kv_bank[i], self.kv_qat_log_scale[i], qat_strength),
                self._maybe_qat_bank_weight(self.kv_bank[n + i], self.kv_qat_log_scale[n + i], qat_strength),
                self._maybe_qat_bank_weight(self.qo_bank[n + i], self.qo_qat_log_scale[n + i], qat_strength),
                self._maybe_qat_bank_weight(self.mlp_up_bank[i], self.mlp_up_qat_log_scale[i], qat_strength),
                self._maybe_qat_bank_weight(self.mlp_down_bank[i], self.mlp_down_qat_log_scale[i], qat_strength),
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                self._maybe_qat_bank_weight(self.qo_bank[bi], self.qo_qat_log_scale[bi], qat_strength),
                self._maybe_qat_bank_weight(self.kv_bank[bi], self.kv_qat_log_scale[bi], qat_strength),
                self._maybe_qat_bank_weight(self.kv_bank[n + bi], self.kv_qat_log_scale[n + bi], qat_strength),
                self._maybe_qat_bank_weight(self.qo_bank[n + bi], self.qo_qat_log_scale[n + bi], qat_strength),
                self._maybe_qat_bank_weight(self.mlp_up_bank[bi], self.mlp_up_qat_log_scale[bi], qat_strength),
                self._maybe_qat_bank_weight(self.mlp_down_bank[bi], self.mlp_down_qat_log_scale[bi], qat_strength),
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, clip_range: int = 31, clip_multiplier: Tensor | float | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    clip_multiplier_tensor = None
    if clip_multiplier is not None:
        clip_multiplier_tensor = torch.as_tensor(clip_multiplier, dtype=torch.float32)
    if t32.ndim == 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            if clip_multiplier_tensor is not None:
                row_clip = row_clip * clip_multiplier_tensor
    ...
    if clip_multiplier_tensor is not None:
        amax *= float(clip_multiplier_tensor.item())
...
def _unbank_qat_clip_multipliers(sd: dict[str, Tensor], num_layers: int, gain_range: float) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    n = num_layers
    if "qo_qat_log_scale" in sd:
        for i in range(n):
            out[f"blocks.{i}.attn.c_q.weight"] = clip_multiplier_from_log_scale(sd["qo_qat_log_scale"][i], gain_range).cpu()
            out[f"blocks.{i}.attn.proj.weight"] = clip_multiplier_from_log_scale(sd["qo_qat_log_scale"][n + i], gain_range).cpu()
    if "kv_qat_log_scale" in sd:
        for i in range(n):
            out[f"blocks.{i}.attn.c_k.weight"] = clip_multiplier_from_log_scale(sd["kv_qat_log_scale"][i], gain_range).cpu()
            out[f"blocks.{i}.attn.c_v.weight"] = clip_multiplier_from_log_scale(sd["kv_qat_log_scale"][n + i], gain_range).cpu()
    if "mlp_up_qat_log_scale" in sd:
        for i in range(n):
            out[f"blocks.{i}.mlp.fc.weight"] = clip_multiplier_from_log_scale(sd["mlp_up_qat_log_scale"][i], gain_range).cpu()
    if "mlp_down_qat_log_scale" in sd:
        for i in range(n):
            out[f"blocks.{i}.mlp.proj.weight"] = clip_multiplier_from_log_scale(sd["mlp_down_qat_log_scale"][i], gain_range).cpu()
    return out
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    clip_multipliers: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, clip_multiplier=None if clip_multipliers is None else clip_multipliers.get(name))
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def run_smoke_test() -> None:
    ...
    model.train()
    x = torch.randint(0, 64, (2, 32), dtype=torch.int64)
    y = torch.randint(0, 64, (2, 32), dtype=torch.int64)
    qat_strength = torch.tensor(0.5, dtype=torch.float32)
    loss = model(x, y, qat_strength)
    loss.backward()
    print(f"smoke_test:ok loss:{loss.item():.4f}")
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        qat_clip_gain_range=args.qat_clip_gain_range,
    ).to(device).bfloat16()
    ...
    log0(f"late_qat_threshold:{args.late_qat_threshold} qat_clip_gain_range:{args.qat_clip_gain_range}")
    ...
    while True:
        ...
        qat_strength = None
        if args.qat_enabled:
            qat_strength = torch.tensor(1.0, device=device, dtype=torch.float32)
        elif args.late_qat_threshold > 0 and scale < args.late_qat_threshold:
            qat_strength = torch.tensor(
                min(max(1.0 - (scale / max(args.late_qat_threshold, 1e-9)), 0.0), 1.0),
                device=device,
                dtype=torch.float32,
            )
            if not late_qat_logged:
                log0(f"late_qat:ramp_start step:{step} scale:{scale:.4f}")
                late_qat_logged = True
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, qat_strength) if qat_strength is not None else model(x, y)
    ...
    clip_multipliers = _unbank_qat_clip_multipliers(sd_cpu, args.num_layers, args.qat_clip_gain_range)
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, clip_multipliers=clip_multipliers)
