...
def run_smoke_test() -> None:
    ...
    smoke_model = GPT(mtp_num_heads=2, mtp_loss_weight=0.15, **model_kwargs).float()
    restore_low_dim_params_to_fp32(smoke_model)
    smoke_model.train()
    tokens = torch.arange(34, dtype=torch.int64).reshape(2, 17) % smoke_vocab_size
    input_ids = tokens[:, :-1]
    target_ids = tokens[:, 1:]
    loss = smoke_model(input_ids, target_ids)
    loss.backward()
    expected_extra_targets = [tokens[:, 2 + k :] for k in range(smoke_model.mtp_num_heads)]
    actual_extra_targets = [target_ids[:, 1 + k :] for k in range(smoke_model.mtp_num_heads)]
    for expected, actual in zip(expected_extra_targets, actual_extra_targets, strict=True):
        if not torch.equal(expected, actual):
            raise RuntimeError("Smoke test target alignment mismatch for MTP heads")
    mtp_grad_sums = [float(head.weight.grad.abs().sum().item()) for head in smoke_model.mtp_heads]
    if not all(grad_sum > 0.0 for grad_sum in mtp_grad_sums):
        raise RuntimeError(f"Smoke test expected non-zero MTP gradients, got {mtp_grad_sums}")

    full_state_dict = {k: v.detach().cpu().contiguous() for k, v in smoke_model.state_dict().items()}
    ...
    eval_model = GPT(mtp_num_heads=0, mtp_loss_weight=0.0, **model_kwargs).float()
    ...
    print(
        ...
        f"excluded_mtp_params:{excluded_mtp} "
        f"mtp_grad_sums:{[round(v, 4) for v in mtp_grad_sums]}"
    )
...
def main() -> None:
    ...
    if flash_attn_3_func is None:
        if _flash_attn_import_error is None:
            log0("attention_backend:sdpa flash_attn_status:missing", console=False)
        else:
            log0(
                "attention_backend:sdpa "
                f"flash_attn_status:{type(_flash_attn_import_error).__name__}:{_flash_attn_import_error}",
                console=False,
            )
    else:
        log0("attention_backend:flash_attn_3", console=False)
