...
class Hyperparameters:
    ...
    mtp_warmdown_threshold = float(os.environ.get("MTP_WARMDOWN_THRESHOLD", 0.2))
    mtp_disable_fraction = float(os.environ.get("MTP_DISABLE_FRACTION", 0.05))
...
def causal_attention_fallback(q: Tensor, k: Tensor, v: Tensor, num_heads: int, num_kv_heads: int) -> Tensor:
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
...
def can_use_flash_attn_3(x: Tensor) -> bool:
    if flash_attn_3_func is None or not x.is_cuda:
        return False
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if can_use_flash_attn_3(q):
            ...
            y = causal_attention_fallback(q, k, v, self.num_heads, self.num_kv_heads)
        if self.use_xsa:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer("mtp_loss_weight_buf", torch.tensor(float(mtp_loss_weight), dtype=torch.float32), persistent=False)
    ...
    def set_mtp_loss_weight(self, weight: float) -> None:
        self.mtp_loss_weight_buf.fill_(max(weight, 0.0))

    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_enabled: bool = True) -> Tensor:
        ...
        if mtp_enabled and self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = self.mtp_loss_weight_buf.to(dtype=main_loss.dtype)
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_warmdown_threshold:{args.mtp_warmdown_threshold} "
        f"mtp_disable_fraction:{args.mtp_disable_fraction} mtp_params:{mtp_params}"
    )
    ...
    def mtp_weight_for_scale(scale: float) -> float:
        ...
        return args.mtp_loss_weight * min(max(scale, 0.0) / args.mtp_warmdown_threshold, 1.0)
    ...
    while True:
        ...
        current_mtp_weight = mtp_weight_for_scale(scale)
        if distributed:
            mtp_weight_tensor = torch.tensor(current_mtp_weight, device=device, dtype=torch.float32)
            dist.all_reduce(mtp_weight_tensor, op=dist.ReduceOp.MAX)
            current_mtp_weight = float(mtp_weight_tensor.item())
        mtp_disable_weight = max(args.mtp_loss_weight * args.mtp_disable_fraction, 1e-6)
        mtp_enabled = current_mtp_weight > mtp_disable_weight
        base_model.set_mtp_loss_weight(current_mtp_weight if mtp_enabled else 0.0)
        zero_grad_all()
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_weight:{current_mtp_weight:.4f}"
            )
