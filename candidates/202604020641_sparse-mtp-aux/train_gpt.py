...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if flash_attn_3_func is not None and x.is_cuda:
            ...
            y = F.scaled_dot_product_attention(q_s, k_s, v_s, is_causal=True).transpose(1, 2)
        if self.use_xsa:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if valid_t <= 0:
                    continue
                mtp_hidden = x[:, :valid_t : self.mtp_stride, :].reshape(-1, dim)
                mtp_targets = target_ids[:, k + 1 :: self.mtp_stride].reshape(-1)
...
def main() -> None:
    ...
    if args.smoke_test:
        ...
        print(
            ...
            f"mtp_num_heads:{args.mtp_num_heads} mtp_stride:{args.mtp_stride}"
        )
        return
    ...
    random.seed(args.seed)
    if np is None:
        raise RuntimeError("numpy is required for full training/evaluation; install repo dependencies or use SMOKE_TEST=1")
    if spm is None:
        raise RuntimeError("sentencepiece is required for full training/evaluation; install repo dependencies or use SMOKE_TEST=1")
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    ...
    log0(
        ...
        f"mtp_stride:{args.mtp_stride} mtp_params:{mtp_params}"
    )
