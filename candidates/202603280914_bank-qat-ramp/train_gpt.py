...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer("bank_qat_alpha", torch.tensor(0.0, dtype=torch.float32), persistent=False)
        self.register_buffer(
            "qo_bank_qat_scales",
            torch.ones(2 * num_layers, model_dim, dtype=torch.float16),
            persistent=False,
        )
        self.register_buffer(
            "kv_bank_qat_scales",
            torch.ones(2 * num_layers, kv_dim, dtype=torch.float16),
            persistent=False,
        )
        self.register_buffer(
            "mlp_up_bank_qat_scales",
            torch.ones(num_layers, mlp_dim, dtype=torch.float16),
            persistent=False,
        )
        self.register_buffer(
            "mlp_down_bank_qat_scales",
            torch.ones(num_layers, model_dim, dtype=torch.float16),
            persistent=False,
        )
    ...
    def refresh_bank_qat_scales(self) -> None:
        for idx in range(self.qo_bank.size(0)):
            _, scale = quantize_int6_per_row(self.qo_bank[idx].detach())
            self.qo_bank_qat_scales[idx].copy_(scale)
        for idx in range(self.kv_bank.size(0)):
            _, scale = quantize_int6_per_row(self.kv_bank[idx].detach())
            self.kv_bank_qat_scales[idx].copy_(scale)
        for idx in range(self.mlp_up_bank.size(0)):
            _, scale = quantize_int6_per_row(self.mlp_up_bank[idx].detach())
            self.mlp_up_bank_qat_scales[idx].copy_(scale)
        for idx in range(self.mlp_down_bank.size(0)):
            _, scale = quantize_int6_per_row(self.mlp_down_bank[idx].detach())
            self.mlp_down_bank_qat_scales[idx].copy_(scale)
    @torch.no_grad()
    def set_bank_qat_alpha(self, alpha: float) -> None:
        self.bank_qat_alpha.fill_(min(max(alpha, 0.0), 1.0))
    def _bank_weight(self, bank: Tensor, qat_scales: Tensor, idx: int, dtype: torch.dtype) -> Tensor:
        w = bank[idx]
        w_cast = w.to(dtype)
        if not (self.training and self.bank_qat_active):
            return w_cast
        scale = qat_scales[idx].to(device=w.device, dtype=torch.float32)
        q = torch.clamp(torch.round(w.float() / scale[:, None]), -31, 31)
        w_q = (q * scale[:, None]).to(dtype)
        alpha = self.bank_qat_alpha.to(device=w.device, dtype=dtype)
        return w_cast + alpha * (w_q - w_cast).detach()
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            q_w = self._bank_weight(self.qo_bank, self.qo_bank_qat_scales, i, x.dtype)
            k_w = self._bank_weight(self.kv_bank, self.kv_bank_qat_scales, i, x.dtype)
            v_w = self._bank_weight(self.kv_bank, self.kv_bank_qat_scales, n + i, x.dtype)
            out_w = self._bank_weight(self.qo_bank, self.qo_bank_qat_scales, n + i, x.dtype)
            up_w = self._bank_weight(self.mlp_up_bank, self.mlp_up_bank_qat_scales, i, x.dtype)
            down_w = self._bank_weight(self.mlp_down_bank, self.mlp_down_bank_qat_scales, i, x.dtype)
        ...
        for i in range(self.num_decoder_layers):
            ...
            q_w = self._bank_weight(self.qo_bank, self.qo_bank_qat_scales, bi, x.dtype)
            k_w = self._bank_weight(self.kv_bank, self.kv_bank_qat_scales, bi, x.dtype)
            v_w = self._bank_weight(self.kv_bank, self.kv_bank_qat_scales, n + bi, x.dtype)
            out_w = self._bank_weight(self.qo_bank, self.qo_bank_qat_scales, n + bi, x.dtype)
            up_w = self._bank_weight(self.mlp_up_bank, self.mlp_up_bank_qat_scales, bi, x.dtype)
            down_w = self._bank_weight(self.mlp_down_bank, self.mlp_down_bank_qat_scales, bi, x.dtype)
...
def main() -> None:
    ...
    log0(
        f"qat_enabled:{args.qat_enabled} bank_qat:{args.bank_qat} "
    ...
    while True:
        ...
        if args.bank_qat and base_model.bank_qat_active:
            if master_process:
                base_model.set_bank_qat_alpha(
                    (args.late_qat_threshold - scale) / max(args.late_qat_threshold, 1e-9)
                )
            if distributed:
                dist.broadcast(base_model.bank_qat_alpha, src=0)
        should_enable_late_qat = args.late_qat_threshold > 0 and scale < args.late_qat_threshold
        if distributed:
            late_qat_flag = torch.tensor(int(should_enable_late_qat), device=device)
            dist.broadcast(late_qat_flag, src=0)
            should_enable_late_qat = bool(late_qat_flag.item())
        late_qat_recompiled = False
        if should_enable_late_qat and args.bank_qat and not base_model.bank_qat_active:
            base_model.bank_qat_active = True
            if master_process:
                log0(f"bank_qat:refresh_scales step:{step} scale:{scale:.4f}")
                base_model.refresh_bank_qat_scales()
                base_model.set_bank_qat_alpha(
                    (args.late_qat_threshold - scale) / max(args.late_qat_threshold, 1e-9)
                )
            if distributed:
                dist.broadcast(base_model.bank_qat_alpha, src=0)
                dist.broadcast(base_model.qo_bank_qat_scales, src=0)
                dist.broadcast(base_model.kv_bank_qat_scales, src=0)
                dist.broadcast(base_model.mlp_up_bank_qat_scales, src=0)
                dist.broadcast(base_model.mlp_down_bank_qat_scales, src=0)
            late_qat_recompiled = True
        if should_enable_late_qat and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            late_qat_recompiled = True
        if late_qat_recompiled:
            compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
            model = compiled_model
            log0(
                f"late_qat:enabled step:{step} scale:{scale:.4f} "
                f"bank_qat:{int(base_model.bank_qat_active)} "
                f"bank_qat_alpha:{float(base_model.bank_qat_alpha.item()):.4f}"
            )
        zero_grad_all()
