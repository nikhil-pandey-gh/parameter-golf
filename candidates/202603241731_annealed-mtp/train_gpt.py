...
class Hyperparameters:
    ...
    mtp_final_frac = float(os.environ.get("MTP_FINAL_FRAC", 0.0))
    mtp_disable_below = float(os.environ.get("MTP_DISABLE_BELOW", 0.02))
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        input_ids: Tensor,
        target_ids: Tensor,
        mtp_scale: Tensor | float | None = None,
        ...
        if mtp_scale is None:
            mtp_scale_tensor = main_loss.new_tensor(float(self.mtp_loss_weight))
        elif isinstance(mtp_scale, Tensor):
            mtp_scale_tensor = mtp_scale.to(device=main_loss.device, dtype=main_loss.dtype)
        else:
            mtp_scale_tensor = main_loss.new_tensor(float(mtp_scale))
        if mtp_active is None:
            mtp_active = self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0
        if self.training and mtp_active:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + mtp_scale_tensor * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_final_frac:{args.mtp_final_frac} mtp_disable_below:{args.mtp_disable_below} "
    ...
    if args.warmup_steps > 0:
        ...
        model.train()
        warmup_mtp_scale = torch.tensor(args.mtp_loss_weight, device=device)
    ...
    while True:
        ...
        mtp_scale_value = args.mtp_loss_weight * (args.mtp_final_frac + (1.0 - args.mtp_final_frac) * scale)
        mtp_active = args.mtp_num_heads > 0 and mtp_scale_value > args.mtp_disable_below
        mtp_scale = torch.tensor(mtp_scale_value if mtp_active else 0.0, device=device)
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_scale, mtp_active)
        ...
        if should_log_train:
            mtp_log = (
                f" mtp_scale:{float(mtp_scale.item()):.4f}"
