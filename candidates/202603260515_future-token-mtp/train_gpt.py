...
class Hyperparameters:
    ...
    mtp_head_lr = float(os.environ.get("MTP_HEAD_LR", os.environ.get("HEAD_LR", "0.008")))
...
def maybe_compile(obj, device: torch.device):
    if device.type != "cuda" or os.environ.get("COMPILE", "1") == "0":
        return obj
...
def use_flash_attention(device: torch.device) -> bool:
    return flash_attn_3_func is not None and device.type == "cuda"
    ...
    def _build(self):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                tail = p.shape[1:]
                dev = p.device
                bank_dtype = torch.bfloat16 if dev.type == "cuda" else torch.float32
                self._bank_meta.append({
                    'p': p,
                    'B': B,
                    'padded_grad': torch.zeros(padded_B, *tail, device=dev, dtype=bank_dtype),
                    'shard': torch.zeros(shard_B, *tail, device=dev, dtype=bank_dtype),
                    'shard_mom': torch.zeros(shard_B, *tail, device=dev, dtype=bank_dtype),
                    'full_update': torch.zeros(padded_B, *tail, device=dev, dtype=bank_dtype),
    ...
    def launch_reduce_scatters(self):
        ...
        for m in self._bank_meta:
            ...
            if p.grad is None:
                ...
                continue
            pg = m['padded_grad']
            pg[:m['B']].copy_(p.grad.to(dtype=pg.dtype))
            if pg.shape[0] > m['B']:
                pg[m['B']:].zero_()
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                if sharded and self._rs_futures[i] is not None:
                    ...
                    bank_dtype = torch.bfloat16 if p.device.type == "cuda" else torch.float32
                    g = p.grad.to(dtype=bank_dtype)
                    state = self.state[p]
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if use_flash_attention(x.device):
            ...
            y = F.scaled_dot_product_attention(q_sdp, k_sdp, v_sdp, is_causal=True).transpose(1, 2)
        if self.use_xsa:
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = maybe_compile(base_model.forward_logits, device)
    with torch.inference_mode():
...
def main() -> None:
    ...
    if requested_device not in {"cuda", "cpu"}:
        raise ValueError(f"DEVICE must be 'cuda' or 'cpu', got {requested_device!r}")
    if requested_device == "cuda" and torch.cuda.is_available():
        device = torch.device("cuda", local_rank)
    else:
        if requested_device == "cpu" and not args.allow_cpu:
            raise RuntimeError("DEVICE=cpu requires ALLOW_CPU=1 so slow smoke runs stay opt-in")
        if requested_device == "cuda" and not torch.cuda.is_available() and not args.allow_cpu:
            raise RuntimeError("CUDA is required unless ALLOW_CPU=1 is set for a CPU smoke test")
        if distributed:
            raise RuntimeError("CPU mode only supports single-process execution")
        device = torch.device("cpu")
    use_cuda = device.type == "cuda"
    grad_accum_base = 8 if use_cuda else 1
    if grad_accum_base % world_size != 0:
        raise ValueError(
            f"WORLD_SIZE={world_size} must divide {grad_accum_base} so grad_accum_steps stays integral"
        )
    grad_accum_steps = grad_accum_base // world_size
    ...
    if use_cuda:
        ...
        log0("CPU smoke mode active; flash attention, fused optimizers, and torch.compile are disabled.", console=False)
    log0("=" * 100, console=False)
    ...
    log0(f"train_loader:dataset:{dataset_dir.name} pattern:{args.train_files} train_shards:{actual_train_files}")
    ...
    compiled_model = maybe_compile(base_model, device)
    ...
    fused_optimizers = use_cuda
    optimizer_tok = torch.optim.AdamW(
        tok_params,
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        weight_decay=args.adam_wd,
        fused=fused_optimizers,
    )
    optimizer_muon = Muon(
        matrix_params,
        lr=args.matrix_lr,
    ...
    if mtp_head_params:
        head_param_groups.append(
            {"params": mtp_head_params, "lr": args.mtp_head_lr, "base_lr": args.mtp_head_lr}
        )
        replicated_params.extend(mtp_head_params)
    if head_param_groups:
        # MTP heads are training-only, but still need an optimizer so the auxiliary loss is live.
    ...
    mtp_params = sum(p.numel() for p in mtp_head_params)
    log0(f"model_params:{n_params}")
    log0(
        f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} "
        f"mtp_head_lr:{args.mtp_head_lr} mtp_params:{mtp_params}"
    )
    ...
    if use_cuda:
        ...
        log0(f"sdp_backends:cpu_fallback flash={int(use_flash_attention(device))}")
    ...
    if use_cuda:
        ...
        log0("peak memory allocated: cpu-mode (CUDA memory stats unavailable)")
    ...
    compiled_eval = maybe_compile(eval_model, device)
    device_synchronize(device)
