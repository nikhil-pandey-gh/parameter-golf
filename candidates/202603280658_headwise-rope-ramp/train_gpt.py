...
class Hyperparameters:
    ...
    rope_heads_min = int(os.environ.get("ROPE_HEADS_MIN", 2))
    rope_heads_max = int(os.environ.get("ROPE_HEADS_MAX", 8))
...
def build_rope_head_mask(num_heads: int, active_heads: int) -> Tensor:
    if active_heads < 0 or active_heads > num_heads:
        raise ValueError(f"active_heads must be in [0, {num_heads}], got {active_heads}")
...
def layerwise_rope_heads(layer_idx: int, num_layers: int, start_heads: int, end_heads: int) -> int:
    if num_layers <= 1:
        return end_heads
    frac = layer_idx / (num_layers - 1)
    return int(round(start_heads + (end_heads - start_heads) * frac))
...
def apply_rotary_emb(
    ...
    if rope_dims > 0 and rope_dims < x.size(-1):
        ...
        x = torch.cat((x, x_pass), dim=-1)
    else:
        half = x.size(-1) // 2
        x1, x2 = x[..., :half], x[..., half:]
        x = torch.cat((x1 * cos + x2 * sin, x1 * (-sin) + x2 * cos), dim=-1)
    if head_mask is not None:
        x = torch.where(head_mask[None, None, :, None], x, x_orig)
    return x
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.register_buffer("rope_query_head_mask", torch.ones(num_heads, dtype=torch.bool), persistent=False)
        self.register_buffer("rope_key_head_mask", torch.ones(num_kv_heads, dtype=torch.bool), persistent=False)
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        q = apply_rotary_emb(q, cos, sin, self.rope_dims, self.rope_query_head_mask)
        k = apply_rotary_emb(k, cos, sin, self.rope_dims, self.rope_key_head_mask)
...
class GPT(nn.Module):
    def __init__(
        ...
        xsa_last_n: int = 0,
        rope_dims: int = 0,
        rope_heads_min: int = 0,
        rope_heads_max: int = 0,
        ln_scale: bool = False,
        dtg: bool = False,
        ...
        if rope_dims > 0:
            head_dim = model_dim // num_heads
            rope_heads_min = max(0, min(num_heads, rope_heads_min))
            rope_heads_max = max(0, min(num_heads, rope_heads_max))
            if rope_heads_min > rope_heads_max:
                raise ValueError(
                    f"ROPE_HEADS_MIN={rope_heads_min} must be <= ROPE_HEADS_MAX={rope_heads_max}"
                )
            q_heads_per_kv = num_heads // num_kv_heads
            rope_kv_heads_min = math.ceil(rope_heads_min / q_heads_per_kv)
            rope_kv_heads_max = math.ceil(rope_heads_max / q_heads_per_kv)
            self.rope_heads_schedule = []
            self.rope_kv_heads_schedule = []
            for layer_idx, block in enumerate(self.blocks):
                active_kv_heads = layerwise_rope_heads(
                    layer_idx, num_layers, rope_kv_heads_min, rope_kv_heads_max
                )
                active_heads = active_kv_heads * q_heads_per_kv
                self.rope_heads_schedule.append(active_heads)
                self.rope_kv_heads_schedule.append(active_kv_heads)
                block.attn.rope_dims = rope_dims
                block.attn.rotary = Rotary(head_dim, base=rope_base, train_seq_len=1024, rope_dims=rope_dims)
                block.attn.rope_query_head_mask = build_rope_head_mask(num_heads, active_heads)
                block.attn.rope_key_head_mask = build_rope_head_mask(num_kv_heads, active_kv_heads)
        else:
            self.rope_heads_schedule = [0 for _ in range(num_layers)]
            self.rope_kv_heads_schedule = [0 for _ in range(num_layers)]
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        rope_heads_min=args.rope_heads_min,
        rope_heads_max=args.rope_heads_max,
    ...
    log0(
        f"rope:dims:{args.rope_dims} q_head_schedule:{base_model.rope_heads_schedule} "
        f"kv_head_schedule:{base_model.rope_kv_heads_schedule} "
        f"min_heads:{args.rope_heads_min} max_heads:{args.rope_heads_max}"
    )
