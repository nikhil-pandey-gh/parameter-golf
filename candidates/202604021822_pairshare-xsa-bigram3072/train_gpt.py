...
class Hyperparameters:
    ...
    shared_core_group_size = int(os.environ.get("SHARED_CORE_GROUP_SIZE", 2))
...
class GPT(nn.Module):
    def __init__(
        ...
        shared_core_group_size = max(1, shared_core_group_size)
        shared_cores = [
            SharedBlockCore(
                model_dim,
                num_heads,
            ...
            for _ in range((num_layers + shared_core_group_size - 1) // shared_core_group_size)
        ]
        self.blocks = nn.ModuleList(
            [
                Block(
                    model_dim,
                    shared_core=shared_cores[i // shared_core_group_size],
                    layer_idx=i,
                    ln_scale=ln_scale,
                    dtg=dtg,
...
def _tensor_alias_signature(t: Tensor) -> tuple[int, int, tuple[int, ...], tuple[int, ...], str]:
    return (
...
def dedupe_state_dict_aliases(state_dict: dict[str, Tensor]) -> tuple[dict[str, Tensor], dict[str, str]]:
    unique: dict[str, Tensor] = {}
    aliases: dict[str, str] = {}
    seen: dict[tuple[int, int, tuple[int, ...], tuple[int, ...], str], str] = {}
    for name, tensor in state_dict.items():
        sig = _tensor_alias_signature(tensor)
        target = seen.get(sig)
        if target is not None:
            aliases[name] = target
            continue
        seen[sig] = name
        unique[name] = tensor.detach().cpu()
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_core_group_size=args.shared_core_group_size,
    ).to(device).bfloat16()
    ...
    log0(f"shared_core_group_size:{args.shared_core_group_size}")
    ...
    sd_cpu, alias_map = dedupe_state_dict_aliases(export_sd)
    full_sd_cpu = {k: v.detach().cpu() for k, v in export_sd.items()}
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"})
    for alias_name, target_name in alias_map.items():
        quant_meta[alias_name] = {"type": "alias", "target": target_name}
    if master_process and alias_map:
        log0(f"shared_export_aliases:{len(alias_map)}")
    ...
    deq_state = dequantize_mixed_int6(quant_state["w"], quant_state["m"], full_sd_cpu)
    eval_model = GPT(
