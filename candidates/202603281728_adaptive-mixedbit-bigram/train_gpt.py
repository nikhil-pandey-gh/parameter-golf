...
class Hyperparameters:
    ...
    mlp_int6_promote_frac = float(os.environ.get("MLP_INT6_PROMOTE_FRAC", 0.0625))
...
def quantize_intN_gptqlite_per_row(
    t: Tensor,
    clip_range: int,
    *,
    return_row_error: bool = False,
    ...
    if return_row_error:
        ...
        err = torch.tensor(float((t32 - recon).pow(2).mean().item()), dtype=torch.float32)
        return q, scale, err
    return q, scale


def quantize_mlp_rowmix(
    t: Tensor,
    promote_frac: float,
) -> tuple[Tensor, Tensor, int]:
    q5, s5, err5 = quantize_intN_gptqlite_per_row(t, clip_range=15, return_row_error=True)
    if t.ndim != 2 or promote_frac <= 0.0 or t.shape[0] == 0:
        return q5, s5, 0
    q6, s6, err6 = quantize_intN_gptqlite_per_row(t, clip_range=31, return_row_error=True)
    promote_rows = min(max(int(math.ceil(t.shape[0] * promote_frac)), 0), t.shape[0])
    if promote_rows == 0:
        return q5, s5, 0
    gain = (err5 - err6).clamp_min(0.0)
    if not torch.any(gain > 0):
        return q5, s5, 0
    promote_rows = min(promote_rows, int((gain > 0).sum().item()))
    if promote_rows == 0:
        return q5, s5, 0
    topk = torch.topk(gain, k=promote_rows).indices
    q = q5.clone()
    s = s5.clone()
    q[topk] = q6[topk]
    s[topk] = s6[topk]
    return q, s, int(topk.numel())
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    *,
    mlp_int6_promote_frac: float = 0.0,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat == "mlp" and t.ndim == 2:
            q, s, promoted = quantize_mlp_rowmix(t, promote_frac=mlp_int6_promote_frac)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            rows = int(t.shape[0])
            mlp_promoted_rows += promoted
            mlp_total_rows += rows
            meta[name] = {"type": "int5_rowmix", "promoted_rows": promoted, "rows": rows}
        elif cat in int6_cats and t.ndim >= 1:
            q, s = quantize_intN_gptqlite_per_row(t, clip_range=31)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
            ...
            meta[name] = {"type": "int8"}
    meta["__summary__"] = {
        "mlp_promoted_rows": mlp_promoted_rows,
        "mlp_total_rows": mlp_total_rows,
    }
    return result, meta
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn", "bigram"},
        mlp_int6_promote_frac=args.mlp_int6_promote_frac,
    )
    quant_summary = quant_meta.get("__summary__", {})
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        promoted_rows = int(quant_summary.get("mlp_promoted_rows", 0))
        total_rows = int(quant_summary.get("mlp_total_rows", 0))
        promoted_frac = promoted_rows / max(total_rows, 1)
        log0(
            f"mixedbit_export mlp_promoted_rows:{promoted_rows}/{total_rows} "
            f"promoted_frac:{promoted_frac:.4f} target_frac:{args.mlp_int6_promote_frac:.4f}"
        )
        log0(f"Serialized model mixedbits+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size mixedbits+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    ...
    torch.cuda.synchronize()
    log0(
        f"final_mixedbits_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_mixedbits_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixedbits_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_mixedbits_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixedbits_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(f"final_mixedbits_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
