...
class Hyperparameters:
    ...
    resq_enabled = bool(int(os.environ.get("RESQ_ENABLED", "1")))
    resq_rank = int(os.environ.get("RESQ_RANK", 1))
    resq_byte_budget = int(os.environ.get("RESQ_BYTE_BUDGET", os.environ.get("RESQ_PARAM_BUDGET", 24576)))
    resq_target_patterns = tuple(
        pattern
        for pattern in os.environ.get(
            "RESQ_TARGET_PATTERNS",
            "mlp.proj.weight,attn.proj.weight",
        ).split(",")
        if pattern
    )
...
# --- GPTQ-lite int6 quantization + ResQ-lite residual rescue ---
...
def _compute_resq_factors(residual: Tensor, rank: int) -> tuple[Tensor, Tensor, float] | None:
    if residual.ndim != 2 or rank <= 0:
        return None
    ...
    if s.numel() == 0 or float(s[0].item()) <= 0.0:
        return None
    sqrt_s = s.sqrt().to(torch.float16)
    u_fac = (u[:, :rank] * sqrt_s[None, :]).to(torch.float16).contiguous()
    v_fac = (sqrt_s[:, None] * vh[:rank, :]).to(torch.float16).contiguous()
    gain = float((s.square()).sum().item())
    return u_fac, v_fac, gain
...
def mixed_quantize_int6(
    ...
    *,
    resq_enabled: bool,
    resq_rank: int,
    resq_byte_budget: int,
    resq_target_patterns: tuple[str, ...],
):
    result: dict[str, Tensor] = {}
    meta: dict[str, object] = {}
    resq_candidates: list[dict[str, object]] = []
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            if (
                resq_enabled
                and resq_byte_budget > 0
                and t.ndim == 2
                and _matches_any_pattern(name, resq_target_patterns)
            ):
                recon = q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))
                factors = _compute_resq_factors(t.float() - recon, resq_rank)
                if factors is not None:
                    u_fac, v_fac, gain = factors
                    resq_candidates.append(
                        {
                            "name": name,
                            "u": u_fac,
                            "v": v_fac,
                            "gain": gain,
                            "bytes": tensor_nbytes(u_fac) + tensor_nbytes(v_fac),
                        }
                    )
        else:
            q, s = quantize_float_tensor(t)
            ...
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    resq_candidates.sort(key=lambda item: float(item["gain"]) / max(int(item["bytes"]), 1), reverse=True)
    used_bytes = 0
    selected: list[dict[str, object]] = []
    for item in resq_candidates:
        byte_cost = int(item["bytes"])
        if used_bytes + byte_cost > resq_byte_budget:
            continue
        name = str(item["name"])
        u_fac = item["u"]
        v_fac = item["v"]
        result[name + ".resq_u"] = u_fac
        result[name + ".resq_v"] = v_fac
        info = meta.get(name)
        if isinstance(info, dict):
            info["resq_rank"] = int(u_fac.shape[1])
        selected.append(
            {
                "name": name,
                "rank": int(u_fac.shape[1]),
                "bytes": byte_cost,
    ...
    stats = {
        "budget_bytes": resq_byte_budget,
        "candidate_count": len(resq_candidates),
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if name + ".resq_u" in result:
            resq_u = result[name + ".resq_u"].float()
            resq_v = result[name + ".resq_v"].float()
            deq = deq + (resq_u @ resq_v)
...
def main() -> None:
    ...
    quant_result, quant_meta, resq_stats = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        resq_enabled=args.resq_enabled,
        resq_rank=args.resq_rank,
        resq_byte_budget=args.resq_byte_budget,
        resq_target_patterns=args.resq_target_patterns,
    )
    if master_process:
        selected = resq_stats["selected"]
        if selected:
            summary = ", ".join(
                f"{item['name']}[r{item['rank']},b={item['bytes']}]"
                for item in selected
            )
            log0(
                f"resq_lite:selected used_bytes:{resq_stats['used_bytes']}/{resq_stats['budget_bytes']} "
                f"candidates:{resq_stats['candidate_count']} {summary}"
            )
        else:
            log0(
                f"resq_lite:selected none budget_bytes:{resq_stats['budget_bytes']} "
                f"candidates:{resq_stats['candidate_count']}"
            )
