...
class Hyperparameters:
    ...
    awq_calibration_steps = int(os.environ.get("AWQ_CALIBRATION_STEPS", 4))
...
class ActivationStatCollector:
    ...
    def add(self, name: str, x: Tensor) -> None:
        ...
        if name in self.sumsq:
            self.sumsq[name] += sumsq
    ...
    def finalize(self) -> dict[str, Tensor]:
        out: dict[str, Tensor] = {}
        for name, sumsq in self.sumsq.items():
            mean_sq = sumsq / max(self.counts[name], 1)
            out[name] = (mean_sq / mean_sq.mean().clamp_min(1e-8)).float()
        return out
...
def collect_activation_aware_stats(
    model: nn.Module,
    ...
    collector = ActivationStatCollector()
    handles = []
        ...
        if isinstance(module, CastedLinear):
            ...
            handles.append(
                module.register_forward_pre_hook(
                    lambda _module, inputs, weight_name=weight_name: collector.add(weight_name, inputs[0])
                )
            )
    ...
    if dist.is_available() and dist.is_initialized() and stats:
        for name in sorted(stats):
            t = stats[name].to(device)
            dist.all_reduce(t, op=dist.ReduceOp.SUM)
            stats[name] = (t / dist.get_world_size()).cpu()
    return stats
...
def quantize_int6_per_row(
    ...
    t32 = t.float()
    importance_vec = None
    if t32.ndim == 2 and importance is not None and importance.ndim == 1 and importance.numel() == t32.shape[1]:
        importance_vec = importance.float().reshape(1, -1)
        importance_vec = importance_vec / importance_vec.mean().clamp_min(1e-8)
    if t32.ndim == 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            err = (sq_err * importance_vec).mean().item() if importance_vec is not None else sq_err.mean().item()
            if err < best_err:
...
def mixed_quantize_int6(
...
) -> tuple[dict[str, Tensor], dict[str, object], int]:
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            importance = activation_stats.get(name) if activation_stats is not None else None
            if importance is not None and t.ndim == 2 and importance.numel() == t.shape[1]:
                activation_aware_tensors += 1
            else:
                importance = None
            q, s = quantize_int6_per_row(t, importance=importance)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "activation_aware": importance is not None}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    return result, meta, activation_aware_tensors
...
def main() -> None:
    ...
    if args.awq_enabled and args.awq_calibration_steps > 0:
        ...
        awq_stats = collect_activation_aware_stats(base_model, calib_loader, args, device, grad_accum_steps)
        torch.cuda.synchronize()
        log0(
            f"awq_calibration:tensors:{len(awq_stats)} steps:{args.awq_calibration_steps} "
            f"calib_time:{1000.0 * (time.perf_counter() - t_awq):.0f}ms"
        )
    else:
        log0("awq_calibration:disabled")
    ...
    quant_result, quant_meta, activation_aware_tensors = mixed_quantize_int6(
        sd_cpu,
        {"mlp", "attn"},
        activation_stats=awq_stats if args.awq_enabled else None,
    )
    log0(f"awq_quantization:activation_aware_tensors:{activation_aware_tensors}")
