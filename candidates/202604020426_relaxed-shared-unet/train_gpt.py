...
class Hyperparameters:
    ...
    shared_block_scheme = os.environ.get("SHARED_BLOCK_SCHEME", "mirror")
    depth_lora_rank = int(os.environ.get("DEPTH_LORA_RANK", 8))
    depth_lora_lr = float(os.environ.get("DEPTH_LORA_LR", 0.01))
...
class Block(nn.Module):
    ...
    def forward(
        ...
        return x_out
def build_shared_pattern(num_layers: int, num_shared_blocks: int, scheme: str) -> tuple[int, ...]:
    if num_layers <= 0:
        raise ValueError(f"num_layers must be positive, got {num_layers}")
    if num_shared_blocks <= 0 or num_shared_blocks > num_layers:
        raise ValueError(
            f"NUM_SHARED_BLOCKS must be in [1, {num_layers}], got {num_shared_blocks}"
        )
    if scheme == "cycle":
        return tuple(i % num_shared_blocks for i in range(num_layers))
    if scheme == "mirror":
        if num_shared_blocks == 1:
            return (0,) * num_layers
        pattern: list[int] = []
        idx = 0
        direction = 1
        for _ in range(num_layers):
            ...
            if next_idx >= num_shared_blocks:
                direction = -1
                next_idx = num_shared_blocks - 2
            elif next_idx < 0:
                direction = 1
                next_idx = 1
            idx = next_idx
        return tuple(pattern)
    raise ValueError(f"Unsupported SHARED_BLOCK_SCHEME={scheme!r}; expected 'mirror' or 'cycle'")
class GPT(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        ...
        shared_block_scheme: str = "mirror",
        ...
        self.num_shared_blocks = num_shared_blocks if num_shared_blocks > 0 else num_layers
        self.shared_block_scheme = shared_block_scheme
        self.depth_lora_rank = depth_lora_rank
        self.ln_scale_enabled = ln_scale
        ...
        self.block_pattern = build_shared_pattern(num_layers, self.num_shared_blocks, shared_block_scheme)
        ...
        self.xsa_flags = tuple(i >= max(0, num_layers - xsa_last_n) for i in range(num_layers))
        kv_dim = num_kv_heads * (model_dim // num_heads)
        if depth_lora_rank > 0 and self.num_shared_blocks < num_layers:
            self.depth_loras = nn.ModuleList(
                [AttentionLoRA(model_dim, kv_dim, depth_lora_rank) for _ in range(num_layers)]
            )
        else:
            self.depth_loras = None
    ...
    def _run_logical_block(
        ...
        shared_idx = self.block_pattern[logical_idx]
        block = self.shared_blocks[shared_idx]
        lora = self.depth_loras[logical_idx] if self.depth_loras is not None else None
        ln_scale_factor = 1.0 / math.sqrt(logical_idx + 1) if self.ln_scale_enabled else 1.0
        ...
        return block(
            ...
            v_embed=ve,
            lora=lora,
            ln_scale_factor=ln_scale_factor,
            use_xsa=self.xsa_flags[logical_idx],
        )
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            x = self._run_logical_block(i, x, x0, input_ids, ve_cache)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            bi = self.num_encoder_layers + i
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            x = self._run_logical_block(bi, x, x0, input_ids, ve_cache)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_block_scheme=args.shared_block_scheme,
    ...
    shared_block_named_params = list(base_model.shared_blocks.named_parameters())
    matrix_params = [
        p
        for name, p in shared_block_named_params
    ...
    if base_model.depth_loras is not None:
        optimizer_lora = torch.optim.AdamW(
            [{"params": list(base_model.depth_loras.parameters()), "lr": args.depth_lora_lr, "base_lr": args.depth_lora_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=args.adam_wd,
            fused=True,
    ...
    depth_lora_params = sum(p.numel() for p in base_model.depth_loras.parameters()) if base_model.depth_loras is not None else 0
    log0(
        f"shared_blocks:{base_model.num_shared_blocks} shared_scheme:{base_model.shared_block_scheme} "
        f"block_pattern:{','.join(str(i) for i in base_model.block_pattern)}"
    )
    log0(
        f"depth_lora_rank:{args.depth_lora_rank} depth_lora_lr:{args.depth_lora_lr} "
        f"depth_lora_params:{depth_lora_params}"
    )
    log0(
        f"XSA:last_{args.xsa_last_n} active_layers:"
        f"{[i for i, active in enumerate(base_model.xsa_flags) if active]}"
    )
    ...
    for m in eval_model.modules():
        if isinstance(m, (CastedLinear, AttentionLoRA)):
            m.float()
