...
class Hyperparameters:
    ...
    mlp_balance_enabled = bool(int(os.environ.get("MLP_BALANCE_ENABLED", "1")))
    mlp_balance_max_scale = float(os.environ.get("MLP_BALANCE_MAX_SCALE", 4.0))
    mlp_balance_exponents = os.environ.get(
        "MLP_BALANCE_EXPONENTS",
        "0.0,0.1666667,0.25,0.3333333,0.5",
    )
...
def _parse_float_csv(spec: str) -> tuple[float, ...]:
    values: list[float] = []
...
def _normalize_positive_scale(scale: Tensor, max_scale: float) -> Tensor:
    ...
    if max_scale > 0:
        ...
        scale = (scale / scale.log().mean().exp()).clamp(min=lo, max=hi)
    return scale

def _apply_mlp_hidden_balance(up_w: Tensor, down_w: Tensor, scale: Tensor) -> tuple[Tensor, Tensor]:
    scale = scale.to(device=up_w.device, dtype=up_w.dtype)
    inv_sq = scale.reciprocal().square()
    up_balanced = up_w * scale[:, None]
    down_balanced = down_w * inv_sq[None, :]
    return up_balanced.contiguous(), down_balanced.contiguous()

def _score_mlp_balance_candidate(up_w: Tensor, down_w: Tensor, exponent: float, max_scale: float) -> tuple[float, Tensor]:
    up_rms = up_w.float().square().mean(dim=1).sqrt()
    down_rms = down_w.float().square().mean(dim=0).sqrt()
    raw_scale = ((down_rms + 1e-8) / (up_rms + 1e-8)).pow(exponent)
    scale = _normalize_positive_scale(raw_scale, max_scale)
    up_balanced, down_balanced = _apply_mlp_hidden_balance(up_w.float(), down_w.float(), scale)
    up_q, up_s = quantize_int6_per_row(up_balanced)
    down_q, down_s = quantize_int6_per_row(down_balanced)
    up_rec = up_q.float() * up_s.float()[:, None]
    down_rec = down_q.float() * down_s.float()[:, None]
    up_orig = up_rec / scale[:, None]
    down_orig = down_rec * scale.square()[None, :]
    err = (up_orig - up_w.float()).square().mean().item()
    err += (down_orig - down_w.float()).square().mean().item()
    return err, scale

def apply_mlp_balance_to_state_dict(
    state_dict: dict[str, Tensor],
    max_scale: float,
    exponents: tuple[float, ...],
) -> tuple[dict[str, Tensor], dict[int, dict[str, float]]]:
    if not exponents:
        return state_dict, {}
    out = dict(state_dict)
    summary: dict[int, dict[str, float]] = {}
    block_ids = sorted({int(name.split(".")[1]) for name in state_dict if name.startswith("blocks.")})
    for layer_idx in block_ids:
        up_name = f"blocks.{layer_idx}.mlp.fc.weight"
        down_name = f"blocks.{layer_idx}.mlp.proj.weight"
        if up_name not in state_dict or down_name not in state_dict:
            continue
        up_w = state_dict[up_name].detach().cpu().float()
        down_w = state_dict[down_name].detach().cpu().float()
        best_err = None
        best_scale = None
        best_exponent = 0.0
        for exponent in exponents:
            err, scale = _score_mlp_balance_candidate(up_w, down_w, exponent, max_scale)
            if best_err is None or err < best_err:
                best_err = err
                best_scale = scale
                best_exponent = exponent
        if best_scale is None or best_err is None:
            continue
        out[up_name], out[down_name] = _apply_mlp_hidden_balance(
            state_dict[up_name],
            state_dict[down_name],
            best_scale,
        )
        summary[layer_idx] = {
            "exponent": float(best_exponent),
            "scale_min": float(best_scale.min().item()),
            "scale_max": float(best_scale.max().item()),
            "scale_mean": float(best_scale.mean().item()),
            "proxy_mse": float(best_err),
        }
    return out, summary
...
def run_smoke_test() -> None:
    ...
    input_ids = torch.randint(0, model_kwargs["vocab_size"], (2, 32), dtype=torch.int64)
    target_ids = torch.randint(0, model_kwargs["vocab_size"], (2, 32), dtype=torch.int64)
    model = GPT(**model_kwargs).float()
    model.eval()
    with torch.no_grad():
        ref_logits = model.forward_logits(input_ids)
        ref_loss = model(input_ids, target_ids)
    state_dict = {k: v.detach().cpu().float().contiguous() for k, v in model.state_dict().items()}
    unbanked_sd = _unbank_state_dict(state_dict, model_kwargs["num_layers"])
    balanced_sd, summary = apply_mlp_balance_to_state_dict(
        unbanked_sd,
        max_scale=4.0,
        exponents=_parse_float_csv("0.0,0.1666667,0.25,0.3333333,0.5"),
    )
    balanced_state = _rebank_state_dict(balanced_sd, model_kwargs["num_layers"], state_dict)
    balanced_model = GPT(**model_kwargs).float()
    balanced_model.load_state_dict(balanced_state, strict=True)
    balanced_model.eval()
    with torch.no_grad():
        balanced_logits = balanced_model.forward_logits(input_ids)
    eq_diff = float((ref_logits - balanced_logits).abs().max().item())
    if eq_diff > 1e-4:
        raise RuntimeError(f"MLP balance transform changed float logits: max_diff={eq_diff:.2e}")
    ref_bf16_model = GPT(**model_kwargs).bfloat16()
    ref_bf16_model.load_state_dict(state_dict, strict=True)
    ref_bf16_model.eval()
    balanced_bf16_model = GPT(**model_kwargs).bfloat16()
    balanced_bf16_model.load_state_dict(balanced_state, strict=True)
    balanced_bf16_model.eval()
    with torch.no_grad():
        ref_bf16_logits = ref_bf16_model.forward_logits(input_ids)
        balanced_bf16_logits = balanced_bf16_model.forward_logits(input_ids)
    bf16_diff = float((ref_bf16_logits.float() - balanced_bf16_logits.float()).abs().max().item())
    if bf16_diff > 5e-2:
        raise RuntimeError(f"MLP balance drifted too far in bf16 path: max_diff={bf16_diff:.2e}")
    quant_result, quant_meta = mixed_quantize_int6(balanced_sd, {"mlp", "attn"})
    int6_mlp_keys = [
        name for name, info in quant_meta.items()
        if isinstance(info, dict) and info.get("type") == "int6" and ".mlp." in name
    ]
    if not int6_mlp_keys:
        raise RuntimeError("Smoke test did not exercise int6 MLP quantization")
    deq_unbanked = dequantize_mixed_int6(quant_result, quant_meta, balanced_sd)
    deq_state = _rebank_state_dict(deq_unbanked, model_kwargs["num_layers"], state_dict)
    quant_model = GPT(**model_kwargs).bfloat16()
    quant_model.load_state_dict(deq_state, strict=True)
    quant_model.eval()
    with torch.no_grad():
        quant_logits = quant_model.forward_logits(input_ids)
        quant_loss = quant_model(input_ids, target_ids)
    if not torch.isfinite(quant_loss):
        raise RuntimeError("Quantized smoke test produced a non-finite loss")
    print(
        "smoke_test: ok "
        f"float_loss={ref_loss.item():.4f} "
        f"quant_loss={quant_loss.item():.4f} "
        f"eq_diff={eq_diff:.2e} "
        f"bf16_diff={bf16_diff:.2e} "
        f"quant_logits_std={quant_logits.float().std().item():.4f} "
        f"balanced_layers={len(summary)} "
        f"int6_mlp_tensors={len(int6_mlp_keys)}"
    )
...
def main() -> None:
    ...
    args = Hyperparameters()
    if np is None:
        raise ImportError("numpy is required for full training/evaluation runs")
    if spm is None:
        raise ImportError("sentencepiece is required for full training/evaluation runs")
    ...
    quant_source_sd = unbanked_sd
    if args.mlp_balance_enabled:
        balance_exponents = _parse_float_csv(args.mlp_balance_exponents)
        quant_source_sd, balance_summary = apply_mlp_balance_to_state_dict(
            unbanked_sd,
            max_scale=args.mlp_balance_max_scale,
            exponents=balance_exponents,
        )
        if balance_summary:
            exp_hist: dict[float, int] = {}
            for info in balance_summary.values():
                exp_hist[info["exponent"]] = exp_hist.get(info["exponent"], 0) + 1
            hist = ",".join(f"{exponent:g}:{count}" for exponent, count in sorted(exp_hist.items()))
            scale_lo = min(info["scale_min"] for info in balance_summary.values())
            scale_hi = max(info["scale_max"] for info in balance_summary.values())
            proxy_mse = sum(info["proxy_mse"] for info in balance_summary.values()) / len(balance_summary)
            log0(
                f"mlp_balance:enabled layers={len(balance_summary)} exp_hist={hist} "
                f"scale_range=[{scale_lo:.3f},{scale_hi:.3f}] mean_proxy_mse={proxy_mse:.6e}"
            )
    quant_result, quant_meta = mixed_quantize_int6(quant_source_sd, {"mlp", "attn"})
    ...
    deq_unbanked = dequantize_mixed_int6(quant_state["w"], quant_state["m"], quant_source_sd)
