...
class Hyperparameters:
    ...
    share_mlp_mirror = bool(int(os.environ.get("SHARE_MLP_MIRROR", "1")))
...
def flash_attention_or_sdpa(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if q_t.size(1) != k_t.size(1):
        ...
        if repeat * k_t.size(1) != q_t.size(1):
            raise ValueError(
                f"num_heads must be divisible by num_kv_heads, got {q_t.size(1)} and {k_t.size(1)}"
            )
...
class GPT(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            share_idx = self.mlp_share_pattern[i]
            if share_idx not in shared_mlps:
                shared_mlps[share_idx] = MLP(model_dim, mlp_mult, negative_slope=mlp_negative_slope)
            blocks.append(
                Block(
                    model_dim,
                    num_heads,
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    mlp=shared_mlps[share_idx],
                    mlp_negative_slope=mlp_negative_slope,
                )
            )
        self.blocks = nn.ModuleList(blocks)
        self.unique_mlp_count = len(shared_mlps)
        if rope_dims > 0:
...
def _tensor_alias_key(t: Tensor) -> tuple[int, int, tuple[int, ...], tuple[int, ...], str]:
...
def split_state_dict_aliases(state_dict: dict[str, Tensor]) -> tuple[dict[str, Tensor], dict[str, str]]:
    unique_tensors: dict[str, Tensor] = {}
    alias_map: dict[str, str] = {}
    seen: dict[tuple[int, int, tuple[int, ...], tuple[int, ...], str], str] = {}
    for name, tensor in state_dict.items():
        alias_key = _tensor_alias_key(tensor.detach())
        if alias_key in seen:
            alias_map[name] = seen[alias_key]
            continue
        seen[alias_key] = name
        unique_tensors[name] = tensor
    return unique_tensors, alias_map
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], alias_map: dict[str, str] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            meta[name] = {"type": "int8"}
    if alias_map:
        meta["__aliases__"] = dict(alias_map)
    return result, meta
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
                          template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    alias_map = meta.get("__aliases__", {})
    for name, orig in template_sd.items():
        ...
        if source_name in out:
            ...
            continue
        info = meta.get(source_name)
        if info is None:
            continue
        orig_dtype = orig.dtype
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            t = result[source_name]
            if t.dtype == torch.float16 and orig_dtype in (torch.float32, torch.bfloat16):
                t = t.to(orig_dtype)
            out[source_name] = t
            out[name] = t
            continue
        q, s = result[source_name + ".q"], result[source_name + ".scale"]
        if s.ndim > 0:
            dequant = (q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))).to(orig_dtype)
        else:
            dequant = (q.float() * float(s.item())).to(orig_dtype)
        out[source_name] = dequant
        out[name] = dequant
    return out
def main() -> None:
    ...
    base_model = GPT(
        ...
        share_mlp_mirror=args.share_mlp_mirror,
    ...
    block_named_params = list(base_model.blocks.named_parameters(remove_duplicate=True))
    matrix_params = [
        p
    ...
    log0(
        f"share_mlp_mirror:{base_model.share_mlp_mirror} unique_mlps:{base_model.unique_mlp_count} "
        f"share_pattern:{base_model.mlp_share_pattern}"
    )
    ...
    unique_export_sd, export_alias_map = split_state_dict_aliases(export_sd)
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_heads" in k)
    if excluded_mtp > 0:
        log0(f"export_excluding_mtp_params:{excluded_mtp}")
    if export_alias_map:
        log0(
            f"export_aliases:{len(export_alias_map)} "
            f"unique_export_tensors:{len(unique_export_sd)} total_export_tensors:{len(export_sd)}"
        )
    if master_process:
    ...
    sd_cpu_unique = {k: sd_cpu[k] for k in unique_export_sd}
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu_unique, {"mlp", "attn"}, alias_map=export_alias_map)
    ...
    eval_model = GPT(
        ...
        share_mlp_mirror=args.share_mlp_mirror, mlp_negative_slope=args.mlp_negative_slope,
    ).to(device).bfloat16()
