...
class Hyperparameters:
    ...
    mtp_curriculum = os.environ.get("MTP_CURRICULUM", "forward").lower()
...
class GPT(nn.Module):
    def __init__(
        ...
        self._init_weights()
    @torch.no_grad()
    def set_mtp_active_heads(self, active_heads: int) -> None:
        if self.mtp_head_weights.numel() == 0:
            return
        active_heads = max(0, min(active_heads, self.mtp_head_weights.numel()))
        self.mtp_head_weights.zero_()
        if active_heads > 0:
            self.mtp_head_weights[:active_heads].fill_(1.0)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            head_weights = self.mtp_head_weights.to(dtype=x.dtype)
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + head_weights[k] * head_loss
                mtp_weight_sum = mtp_weight_sum + head_weights[k]
...
def main() -> None:
    ...
    args = Hyperparameters()
    if args.mtp_curriculum not in {"none", "forward"}:
        raise ValueError(f"Unsupported MTP_CURRICULUM={args.mtp_curriculum!r}; expected 'none' or 'forward'")
    ...
    base_model.set_mtp_active_heads(args.mtp_num_heads if args.mtp_curriculum == "none" else 0)
    ...
    log0(
        ...
        f"mtp_curriculum:{args.mtp_curriculum} mtp_params:{mtp_params}"
    )
    ...
    def mtp_active_heads(step: int, elapsed_ms: float) -> int:
        if args.mtp_num_heads <= 0:
            return 0
        if args.mtp_curriculum == "none":
            return args.mtp_num_heads
        total_heads = args.mtp_num_heads + 1
        progress = training_progress(step, elapsed_ms)
        active_total_heads = min(total_heads, math.floor(progress * total_heads + 1e-9) + 1)
        return active_total_heads - 1
    ...
    current_mtp_active_heads = args.mtp_num_heads if args.mtp_curriculum == "none" else 0
    ...
    while True:
        ...
        next_mtp_active_heads = mtp_active_heads(step, elapsed_ms)
        if next_mtp_active_heads != current_mtp_active_heads:
            base_model.set_mtp_active_heads(next_mtp_active_heads)
            current_mtp_active_heads = next_mtp_active_heads
            log0(
                f"mtp_curriculum:update step:{step} progress:{training_progress(step, elapsed_ms):.4f} "
                f"active_extra_heads:{current_mtp_active_heads}/{args.mtp_num_heads}"
            )
