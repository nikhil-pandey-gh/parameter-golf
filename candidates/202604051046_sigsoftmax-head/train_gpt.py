...
class Hyperparameters:
    ...
    output_head = os.environ.get("OUTPUT_HEAD", "sigsoftmax").strip().lower()
...
class GPT(nn.Module):
    def __init__(
        ...
        if output_head not in {"softmax", "sigsoftmax"}:
            raise ValueError(f"OUTPUT_HEAD must be 'softmax' or 'sigsoftmax', got {output_head}")
    ...
    def _apply_output_head(self, logits_proj: Tensor, *, loss_path: bool = False) -> Tensor:
        logits = self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)
        if self.output_head == "sigsoftmax":
            sig_logits = logits.float()
            sig_logits = sig_logits + F.logsigmoid(sig_logits)
            return sig_logits if loss_path else sig_logits.to(dtype=logits.dtype)
        return logits.float() if loss_path else logits
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        logits = self._apply_output_head(logits_proj, loss_path=True)
        main_loss = F.cross_entropy(logits, targets, reduction="mean")
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_logits = self._apply_output_head(mtp_logits_proj, loss_path=True)
                mtp_loss_sum = mtp_loss_sum + F.cross_entropy(mtp_logits, mtp_targets, reduction="mean")
                mtp_loss_count += 1
            if mtp_loss_count > 0:
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        return self._apply_output_head(logits_proj)
...
def main() -> None:
    ...
    log0(f"output_head:{args.output_head} logit_softcap:{args.logit_softcap}")
    log0(
    ...
    eval_model = GPT(
        ...
        logit_softcap=args.logit_softcap, output_head=args.output_head,
