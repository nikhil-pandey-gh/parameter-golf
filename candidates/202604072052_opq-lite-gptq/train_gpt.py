...
class Hyperparameters:
    ...
    int6_outlier_topk = int(os.environ.get("INT6_OUTLIER_TOPK", 96))
...
# --- GPTQ-lite int6 quantization with sparse fp16 outlier preservation ---
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def extract_sparse_outliers(t32: Tensor, topk: int) -> tuple[Tensor, dict[str, Tensor] | None]:
    if topk <= 0 or t32.ndim != 2:
        return t32, None
    k = min(topk, t32.numel())
    if k <= 0:
        return t32, None
    flat_abs = t32.abs().flatten()
    top_vals, flat_idx = torch.topk(flat_abs, k, sorted=False)
    nonzero = top_vals > 0
    if not torch.any(nonzero):
        return t32, None
    flat_idx = flat_idx[nonzero]
    ncols = t32.shape[1]
    rows = (flat_idx // ncols).to(torch.int16).contiguous()
    cols = (flat_idx % ncols).to(torch.int16).contiguous()
    vals = t32.flatten()[flat_idx].to(torch.float16).contiguous()
    residual = t32.clone()
    residual[rows.long(), cols.long()] = 0.0
    return residual, {"rows": rows, "cols": cols, "vals": vals}


def quantize_int6_per_row(
    t: Tensor, clip_range: int = 31, outlier_topk: int = 0
) -> tuple[Tensor, Tensor, dict[str, Tensor] | None]:
    t32 = t.float()
    if t32.ndim == 2:
        work, sparse_outliers = extract_sparse_outliers(t32, outlier_topk)
        ...
        return best_q, best_s, sparse_outliers
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], outlier_topk: int = 0):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, sparse_outliers = quantize_int6_per_row(t, outlier_topk=outlier_topk)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            info: dict[str, object] = {"type": "int6"}
            if sparse_outliers is not None:
                result[name + ".outlier_rows"] = sparse_outliers["rows"]
                result[name + ".outlier_cols"] = sparse_outliers["cols"]
                result[name + ".outlier_vals"] = sparse_outliers["vals"]
                info["outliers"] = int(sparse_outliers["vals"].numel())
            meta[name] = info
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and int(info.get("outliers", 0)) > 0:
            rows = result[name + ".outlier_rows"].long()
            cols = result[name + ".outlier_cols"].long()
            recon[rows, cols] += result[name + ".outlier_vals"].float()
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd, {"mlp", "attn"}, outlier_topk=args.int6_outlier_topk
    )
    sparse_outlier_count = sum(
        int(info.get("outliers", 0))
        for info in quant_meta.values()
        if isinstance(info, dict)
    )
    log0(
        f"int6_quantization:outlier_topk={args.int6_outlier_topk} "
        f"preserved_values={sparse_outlier_count}"
    )
