...
def main() -> None:
    ...
    while True:
        ...
        mtp_weight_tensor: Tensor | None = None
        if args.mtp_num_heads > 0:
            mtp_frac = min(step / args.mtp_warmup_steps, 1.0) if args.mtp_warmup_steps > 0 else 1.0
            current_mtp_weight = args.mtp_loss_weight * mtp_frac
            if current_mtp_weight > 0.0:
                mtp_weight_tensor = torch.tensor(current_mtp_weight, device=device, dtype=torch.float32)
            if args.mtp_warmup_steps > 0 and step == args.mtp_warmup_steps:
                log0(f"mtp:curriculum_complete step:{step} weight:{current_mtp_weight:.4f}")
        zero_grad_all()
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_weight_tensor)
