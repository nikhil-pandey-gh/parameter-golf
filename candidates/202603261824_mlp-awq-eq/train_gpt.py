...
class Hyperparameters:
    ...
    ptq_eq_enabled = bool(int(os.environ.get("PTQ_EQ_ENABLED", "1")))
    ptq_eq_alpha_candidates = os.environ.get("PTQ_EQ_ALPHA_CANDIDATES", "0.0,0.25,0.5,0.75,1.0")
    ptq_eq_max_scale = float(os.environ.get("PTQ_EQ_MAX_SCALE", 4.0))
    ptq_eq_calib_tokens = int(os.environ.get("PTQ_EQ_CALIB_TOKENS", 262_144))
    ptq_eq_batch_seqs = int(os.environ.get("PTQ_EQ_BATCH_SEQS", 8))
    smoke_test = bool(int(os.environ.get("SMOKE_TEST", "0")))
    smoke_batch_size = int(os.environ.get("SMOKE_BATCH_SIZE", 2))
    smoke_seq_len = int(os.environ.get("SMOKE_SEQ_LEN", 64))


def parse_float_csv(raw: str) -> tuple[float, ...]:
    values = tuple(float(part.strip()) for part in raw.split(",") if part.strip())
    if not values:
        raise ValueError("Expected at least one float value in CSV string")
    return values
...
def build_sentencepiece_luts(
    ...
    if np is None:
        raise ModuleNotFoundError("numpy is required for tokenizer byte accounting")
...
def load_calibration_tokens(pattern: str, seq_len: int, num_tokens: int) -> Tensor:
    if num_tokens < seq_len:
        raise ValueError(f"PTQ_EQ_CALIB_TOKENS must be at least one sequence, got {num_tokens}")
    usable = max((num_tokens // seq_len) * seq_len, seq_len)
    stream = TokenStream(pattern)
    return stream.take(usable + 1).contiguous()
...
def load_data_shard(file: Path) -> Tensor:
    if np is None:
        raise ModuleNotFoundError("numpy is required for loading dataset shards")
...
def _dequantize_per_row(q: Tensor, s: Tensor) -> Tensor:
    if s.ndim > 0:
        return q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))
    return q.float() * float(s.item())


def _eq_channel_scale(stats: Tensor, alpha: float, max_scale: float) -> Tensor:
    clamped = stats.float().clamp_min(1e-8)
    center = torch.exp(clamped.log().mean())
    scale = (clamped / center).pow(alpha)
    return scale.clamp(1.0 / max_scale, max_scale)


def _weighted_col_mse(ref: Tensor, recon: Tensor, weights: Tensor | None = None) -> float:
    err = (ref.float() - recon.float()).pow(2)
    if weights is not None and err.ndim == 2:
...
def choose_equalized_mlp_weights(
    up_w: Tensor,
    down_w: Tensor,
    act_stats: Tensor,
    ...
    importance = act_stats.float().clamp_min(1e-8)
    for alpha in alpha_candidates:
        scale = _eq_channel_scale(importance, alpha, max_scale)
        inv_sq = scale.reciprocal().square()
        cand_up = (up_w.float() * scale[:, None]).contiguous()
        cand_down = (down_w.float() * inv_sq[None, :]).contiguous()
        q_up, s_up = quantize_int6_per_row(cand_up)
        q_down, s_down = quantize_int6_per_row(cand_down)
        dq_up = _dequantize_per_row(q_up, s_up)
        dq_down = _dequantize_per_row(q_down, s_down)
        score_up = _weighted_col_mse(cand_up, dq_up)
        score_down = _weighted_col_mse(cand_down, dq_down, importance)
        score = score_up + score_down
        if score < best_score:
            best_score = score
            best_up = cand_up.to(dtype=up_w.dtype)
            best_down = cand_down.to(dtype=down_w.dtype)
            best_meta = {
                "alpha": float(alpha),
                "score": float(score),
    ...
    return best_up, best_down, best_meta


def _forward_block_with_hidden(
    model: "GPT",
    layer_idx: int,
    x: Tensor,
    x0: Tensor,
    ...
    attn_out, raw_v = block.attn(
        ...
        model.kv_bank[model.num_layers + layer_idx],
        model.qo_bank[model.num_layers + layer_idx],
        v_embed=ve,
        v0=v0,
    )
    x_out = x_in + block.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
    mlp_in = block.mlp_norm(x_out) * block.ln_scale_factor
    hidden = F.leaky_relu(F.linear(mlp_in, model.mlp_up_bank[layer_idx].to(mlp_in.dtype)), negative_slope=0.5).square()
    mlp_out = F.linear(hidden, model.mlp_down_bank[layer_idx].to(hidden.dtype))
    x_next = x_out + block.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * mlp_out
    if block.dtg_gate is not None:
        gate = torch.sigmoid(block.dtg_gate(x_in.detach()))
        x_next = x_in + gate * (x_next - x_in)
    return x_next, raw_v, hidden


def collect_mlp_activation_stats(model: "GPT", calib_tokens: Tensor, seq_len: int, batch_seqs: int) -> list[Tensor]:
    device = next(model.parameters()).device
    total_seqs = (calib_tokens.numel() - 1) // seq_len
    if total_seqs <= 0:
        raise ValueError("Calibration token buffer is too short")
    stats = [
        torch.zeros(model.mlp_up_bank.shape[1], dtype=torch.float64, device=device)
    ...
    with torch.inference_mode():
        for seq_start in range(0, total_seqs, batch_seqs):
            ...
            local = calib_tokens[raw_start:raw_end].to(device=device, dtype=torch.int64)
            input_ids = local[:-1].reshape(-1, seq_len)
            ...
            for i in range(model.num_encoder_layers):
                x, raw_v, hidden = _forward_block_with_hidden(model, i, x, x0, v0, input_ids, ve_cache)
                stats[i] += hidden.float().mean(dim=(0, 1)).to(torch.float64)
                if v0 is None and raw_v is not None:
                    v0 = raw_v
                skips.append(x)
            for i in range(model.num_decoder_layers):
                ...
                x, _, hidden = _forward_block_with_hidden(model, bi, x, x0, v0, input_ids, ve_cache)
                stats[bi] += hidden.float().mean(dim=(0, 1)).to(torch.float64)
            batches += 1
    if was_training:
        model.train()
    return [(s / max(batches, 1)).cpu() for s in stats]


def apply_activation_aware_mlp_equalization(
    state_dict: dict[str, Tensor],
    activation_stats: list[Tensor],
    alpha_candidates: tuple[float, ...],
    max_scale: float,
    log0,
) -> dict[str, Tensor]:
    out = {name: tensor.clone() for name, tensor in state_dict.items()}
    for layer_idx, act_stats in enumerate(activation_stats):
        fc_key = f"blocks.{layer_idx}.mlp.fc.weight"
        proj_key = f"blocks.{layer_idx}.mlp.proj.weight"
        if fc_key not in out or proj_key not in out:
            continue
        eq_up, eq_down, meta = choose_equalized_mlp_weights(
            out[fc_key],
            out[proj_key],
            act_stats,
            alpha_candidates,
        ...
        out[fc_key] = eq_up
        out[proj_key] = eq_down
        log0(
            "ptq_eq:"
            f"layer:{layer_idx} alpha:{meta['alpha']:.2f} score:{meta['score']:.6e} "
            f"up:{meta['score_up']:.6e} down:{meta['score_down']:.6e} "
            f"scale_min:{meta['scale_min']:.3f} scale_max:{meta['scale_max']:.3f}"
        )
    return out


def run_smoke_test(args: Hyperparameters) -> None:
    device = torch.device("cpu")
    seq_len = min(args.smoke_seq_len, args.train_seq_len)
    def build_model() -> GPT:
        return GPT(
    ...
    model = build_model()
    x = torch.randint(0, args.vocab_size, (args.smoke_batch_size, seq_len), device=device)
    y = torch.randint(0, args.vocab_size, (args.smoke_batch_size, seq_len), device=device)
    calib_tokens = torch.randint(
        0,
        args.vocab_size,
        (seq_len * max(args.smoke_batch_size, 2) + 1,),
        device=device,
    )
    with torch.inference_mode():
        loss = model(x, y)
        logits = model.forward_logits(x)
    export_sd = {k: v.detach().cpu() for k, v in model.state_dict().items() if "mtp_heads" not in k}
    activation_stats = collect_mlp_activation_stats(model, calib_tokens.cpu(), seq_len, batch_seqs=1)
    unbanked_sd = _unbank_state_dict(export_sd, args.num_layers)
    equalized_sd = apply_activation_aware_mlp_equalization(
        unbanked_sd,
        activation_stats,
        parse_float_csv(args.ptq_eq_alpha_candidates),
        args.ptq_eq_max_scale,
        lambda _msg: None,
    )
    quant_result, quant_meta = mixed_quantize_int6(equalized_sd, {"mlp", "attn"})
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
    quant_state = torch.load(io.BytesIO(lzma.decompress(lzma.compress(quant_buf.getvalue(), preset=6))), map_location="cpu")
    deq_unbanked = dequantize_mixed_int6(quant_state["w"], quant_state["m"], equalized_sd)
    deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, export_sd)
    roundtrip_model = build_model()
    roundtrip_model.load_state_dict(deq_state, strict=True)
    with torch.inference_mode():
...
def main() -> None:
    ...
    if spm is None:
        raise ModuleNotFoundError("sentencepiece is required for training and evaluation")
    ...
    log0(
        f"ptq_eq:{args.ptq_eq_enabled} calib_tokens:{args.ptq_eq_calib_tokens} "
        f"batch_seqs:{args.ptq_eq_batch_seqs} alphas:{args.ptq_eq_alpha_candidates} "
        f"max_scale:{args.ptq_eq_max_scale}"
    )
    ...
    if master_process and args.ptq_eq_enabled:
        calib_tokens = load_calibration_tokens(args.train_files, args.train_seq_len, args.ptq_eq_calib_tokens)
        log0(
            f"ptq_eq:collecting_stats tokens:{calib_tokens.numel() - 1} "
            f"seq_len:{args.train_seq_len} batch_seqs:{args.ptq_eq_batch_seqs}"
        )
        activation_stats = collect_mlp_activation_stats(
            base_model,
            calib_tokens,
            args.train_seq_len,
            args.ptq_eq_batch_seqs,
        )
        unbanked_sd = apply_activation_aware_mlp_equalization(
            unbanked_sd,
            activation_stats,
            parse_float_csv(args.ptq_eq_alpha_candidates),
            args.ptq_eq_max_scale,
