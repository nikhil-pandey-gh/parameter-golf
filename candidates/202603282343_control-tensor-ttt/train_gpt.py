...
class Hyperparameters:
    ...
    ttt_param_mode = os.environ.get("TTT_PARAM_MODE", "control").strip().lower()
...
TTT_PARAM_MODE_CHOICES = ("all", "control")
TTT_CONTROL_NAME_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get(
        "TTT_CONTROL_NAME_PATTERNS",
        ",".join((*CONTROL_TENSOR_NAME_PATTERNS, "bigram.scale")),
    ).split(",")
    if pattern
)
...
def is_ttt_control_param(name: str, p: Tensor) -> bool:
    return p.ndim < 2 or any(pattern in name for pattern in TTT_CONTROL_NAME_PATTERNS)


def ttt_param_selected(name: str, p: Tensor, mode: str) -> bool:
    if mode == "all":
        return True
    if mode == "control":
        return is_ttt_control_param(name, p)
    raise ValueError(f"Unsupported TTT_PARAM_MODE={mode!r}; expected one of {TTT_PARAM_MODE_CHOICES}")
...
def eval_val_sliding_ttt(
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"freeze_blocks={args.ttt_freeze_blocks} mode={args.ttt_param_mode}")
    ...
    for name, p in base_model.named_parameters():
        ...
        for bi in frozen_block_ids:
            if f"blocks.{bi}." in name:
                freeze = True
                break
        if freeze or not ttt_param_selected(name, p, args.ttt_param_mode):
    ...
    if not ttt_params:
        raise ValueError(f"TTT selected zero parameters for mode={args.ttt_param_mode}")

    log0(f"ttt_sliding:params mode={args.ttt_param_mode} tensors={len(ttt_params)} "
         f"unfrozen={sum(p.numel() for p in ttt_params)} "
