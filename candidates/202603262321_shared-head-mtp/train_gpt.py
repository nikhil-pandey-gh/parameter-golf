"""
Shared-head multi-token prediction candidate built on the current 11L/XSA/EMA/TTT stack.
"""
...
class GPT(nn.Module):
    def __init__(
        ...
        if self.mtp_num_heads < 0:
            raise ValueError(f"mtp_num_heads must be non-negative, got {self.mtp_num_heads}")
        if self.mtp_horizon_decay <= 0.0:
            raise ValueError(f"mtp_horizon_decay must be positive, got {self.mtp_horizon_decay}")
        ...
        if mtp_num_heads > 0 and not mtp_shared_head:
            self.mtp_heads = nn.ModuleList(
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k in range(self.mtp_num_heads):
                ...
                horizon_weight = float(self.mtp_horizon_decay**k)
            ...
            if mtp_weight_sum > 0.0:
                # Shared-head MTP keeps the auxiliary task aligned with the main decoder geometry.
...
def run_smoke_test() -> None:
    ...
    print(f"smoke_test_ok loss:{loss.item():.4f} logits_shape:{tuple(logits.shape)}")
...
def main() -> None:
    ...
    if np is None or spm is None:
        raise ModuleNotFoundError(
            "numpy and sentencepiece are required for training/evaluation; "
            "run with SMOKE_TEST=1 for the lightweight CPU smoke path"
        )
    ...
    log0(
        ...
        f"mtp_shared_head:{int(args.mtp_shared_head)} mtp_horizon_decay:{args.mtp_horizon_decay} "
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_shared_head=False, mtp_horizon_decay=args.mtp_horizon_decay,
