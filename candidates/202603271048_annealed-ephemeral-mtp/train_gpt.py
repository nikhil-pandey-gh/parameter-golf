...
class Hyperparameters:
    ...
    mtp_loss_decay = float(os.environ.get("MTP_LOSS_DECAY", 0.5))
    mtp_anneal = bool(int(os.environ.get("MTP_ANNEAL", "1")))
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if HAS_FLASH_ATTN and q.is_cuda:
            ...
            if self.num_kv_heads != self.num_heads:
                ...
                k_sdp = k_sdp.repeat_interleave(group, dim=1)
                v_sdp = v_sdp.repeat_interleave(group, dim=1)
...
class GPT(nn.Module):
    def __init__(
        ...
        mtp_loss_decay: float = 0.5,
        mtp_anneal: bool = True,
        ...
        if mtp_loss_decay <= 0.0:
            raise ValueError(f"mtp_loss_decay must be positive, got {mtp_loss_decay}")
        self.mtp_loss_decay = mtp_loss_decay
        self.mtp_anneal = mtp_anneal
        self.register_buffer("mtp_loss_scale", torch.ones((), dtype=torch.float32), persistent=False)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        effective_mtp_weight = self.mtp_loss_weight * (
            self.mtp_loss_scale.to(dtype=main_loss.dtype) if self.mtp_anneal else main_loss.new_tensor(1.0)
        )
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_weight_sum += horizon_weight
            if mtp_weight_sum > 0.0:
                main_loss = main_loss + effective_mtp_weight * (mtp_loss_sum / mtp_weight_sum)
        return main_loss
...
def non_mtp_state_dict(state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
...
def main() -> None:
    ...
    fallback_sdp = not HAS_FLASH_ATTN
    enable_cudnn_sdp(False)
    enable_flash_sdp(True)
    enable_mem_efficient_sdp(fallback_sdp)
    enable_math_sdp(fallback_sdp)
    logfile = None
    if master_process:
    ...
    log0(
        ...
        f"mtp_loss_decay:{args.mtp_loss_decay} mtp_anneal:{args.mtp_anneal} mtp_params:{mtp_params}"
    )
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        f"attention_backends:external_flash_attn={HAS_FLASH_ATTN} "
        f"sdp_flash=True mem_efficient={fallback_sdp} math={fallback_sdp}"
    )
    ...
    ema_state = {name: t.detach().float().clone() for name, t in non_mtp_state_dict(base_model.state_dict()).items()}
    ema_decay = 0.997
    ...
    while True:
        ...
        base_model.mtp_loss_scale.fill_(scale if args.mtp_anneal else 1.0)
        ...
        # EMA update
        with torch.no_grad():
            for name, t in non_mtp_state_dict(base_model.state_dict()).items():
                ema_state[name].mul_(ema_decay).add_(t.detach().float(), alpha=1.0 - ema_decay)
        step += 1
        approx_training_time_ms = training_time_ms + 1000.0 * (time.perf_counter() - t0)
        if args.swa_enabled and scale < 0.2 and step % args.swa_every == 0:
            if swa_state is None:
                swa_state = {name: t.detach().cpu().clone() for name, t in non_mtp_state_dict(base_model.state_dict()).items()}
                swa_count = 1
        ...
        if args.lawa_enabled and step % args.lawa_freq == 0:
            lawa_queue.append({name: t.detach().cpu().clone() for name, t in non_mtp_state_dict(base_model.state_dict()).items()})
        should_log_train = (
        ...
        if should_log_train:
            effective_mtp_weight = (
                args.mtp_loss_weight * float(base_model.mtp_loss_scale.item())
                if args.mtp_anneal
                else args.mtp_loss_weight
            )
            log0(
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        avg_state = {name: torch.zeros(t.shape, dtype=torch.float32, device='cpu') for name, t in non_mtp_state_dict(current_state).items()}
        for snap in lawa_queue:
            for name in avg_state:
                avg_state[name] += snap[name].float()
        for name in avg_state:
            avg_state[name] /= len(lawa_queue)
            current_state[name] = avg_state[name].to(dtype=current_state[name].dtype)
    ...
    export_sd = non_mtp_state_dict(full_state_dict)
