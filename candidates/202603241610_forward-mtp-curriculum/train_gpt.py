...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_lambda: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and mtp_lambda is not None:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = mtp_lambda.to(device=main_loss.device, dtype=main_loss.dtype)
                main_loss = main_loss + mtp_weight * (mtp_loss_sum / mtp_loss_count)
        elif self.training and self.mtp_num_heads > 0:
            # Keep the dormant MTP heads in the autograd graph during the NTP-only phase
            # so default DDP does not flag them as unused before the curriculum turns on.
            for mtp_head in self.mtp_heads:
                main_loss = main_loss + mtp_head.weight.reshape(-1)[0].float() * 0.0
        return main_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_start_frac:{args.mtp_start_frac} mtp_full_frac:{args.mtp_full_frac} mtp_params:{mtp_params}"
    )
    ...
    if not args.mtp_start_frac <= args.mtp_full_frac <= 1.0:
        raise ValueError(
            f"MTP_FULL_FRAC must be in [MTP_START_FRAC, 1], got start={args.mtp_start_frac}, full={args.mtp_full_frac}"
        )
    ...
    def mtp_curriculum_weight(step: int, elapsed_ms: float) -> float:
        ...
        if progress <= args.mtp_start_frac:
            return 0.0
        if args.mtp_full_frac <= args.mtp_start_frac:
            return args.mtp_loss_weight
        ramp = (progress - args.mtp_start_frac) / max(args.mtp_full_frac - args.mtp_start_frac, 1e-9)
        return args.mtp_loss_weight * min(max(ramp, 0.0), 1.0)
    ...
    while True:
        ...
        mtp_weight_value = mtp_curriculum_weight(step, elapsed_ms)
        mtp_lambda = None if mtp_weight_value <= 0.0 else torch.tensor(mtp_weight_value, device=device, dtype=torch.float32)
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y) if mtp_lambda is None else model(x, y, mtp_lambda)
