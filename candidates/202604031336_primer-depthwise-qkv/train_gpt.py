...
class Hyperparameters:
    ...
    primer_enabled = bool(int(os.environ.get("PRIMER_ENABLED", "1")))
    primer_last_n = int(os.environ.get("PRIMER_LAST_N", 4))
    primer_kernel_size = int(os.environ.get("PRIMER_KERNEL_SIZE", 3))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,primer_",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.primer_enabled = use_primer and primer_kernel_size > 1
        self.primer_kernel_size = primer_kernel_size if self.primer_enabled else 0
        kv_dim = self.num_kv_heads * self.head_dim
        if self.primer_enabled:
            self.primer_q = nn.Parameter(torch.zeros(dim, self.primer_kernel_size, dtype=torch.float32))
            self.primer_k = nn.Parameter(torch.zeros(kv_dim, self.primer_kernel_size, dtype=torch.float32))
            self.primer_v = nn.Parameter(torch.zeros(kv_dim, self.primer_kernel_size, dtype=torch.float32))
            self.primer_q.data[:, -1] = 1.0
            self.primer_k.data[:, -1] = 1.0
            self.primer_v.data[:, -1] = 1.0
        else:
            self.register_parameter("primer_q", None)
            self.register_parameter("primer_k", None)
            self.register_parameter("primer_v", None)
    def _causal_depthwise_mix(self, x: Tensor, weight: Tensor | None) -> Tensor:
        if weight is None or weight.size(-1) <= 1:
            return x
        x_t = x.transpose(1, 2)
        mixed = F.conv1d(
            F.pad(x_t, (weight.size(-1) - 1, 0)),
            weight.to(dtype=x.dtype)[:, None, :],
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.primer_enabled:
            q = self._causal_depthwise_mix(q, self.primer_q)
            k = self._causal_depthwise_mix(k, self.primer_k)
            v = self._causal_depthwise_mix(v, self.primer_v)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        ...
                                        use_primer=use_primer, primer_kernel_size=primer_kernel_size)
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    use_primer=primer_enabled and primer_kernel_size > 1 and i >= max(0, num_layers - primer_last_n),
                    primer_kernel_size=primer_kernel_size,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        primer_enabled=args.primer_enabled,
        primer_last_n=args.primer_last_n,
        primer_kernel_size=args.primer_kernel_size,
    ).to(device).bfloat16()
    ...
    primer_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.primer_enabled]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(
        f"primer:enabled:{args.primer_enabled} kernel:{args.primer_kernel_size} "
        f"last_n:{args.primer_last_n} active_layers:{primer_layers}"
    )
    ...
    eval_model = GPT(
        ...
        primer_enabled=args.primer_enabled, primer_last_n=args.primer_last_n,
        primer_kernel_size=args.primer_kernel_size,
    ).to(device).bfloat16()
