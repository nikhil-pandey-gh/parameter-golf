...
def _sdpa_flash_attn_fallback(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    q_t = q.transpose(1, 2)
    k_t = k.transpose(1, 2)
    v_t = v.transpose(1, 2)
...
def flash_attn_3_func(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    if _flash_attn_3_func is None or q.device.type != "cuda":
        return _sdpa_flash_attn_fallback(q, k, v, causal=causal)
...
class Hyperparameters:
    ...
    mtp_loss_decay = float(os.environ.get("MTP_LOSS_DECAY", 0.7))
    mtp_head_lr = float(os.environ.get("MTP_HEAD_LR", 0.01))
    mtp_init = os.environ.get("MTP_INIT", "tie")
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads:
            if self.mtp_init == "tie":
                source = self.tok_emb.weight if self.tie_embeddings or self.lm_head is None else self.lm_head.weight
                for k, head in enumerate(self.mtp_heads):
                    head.weight.data.copy_(source.data)
                    head.weight.data.mul_(1.0 / math.sqrt(k + 1))
            elif self.mtp_init not in {"orthogonal", "zero"}:
                raise ValueError(f"Unsupported MTP_INIT={self.mtp_init!r}")
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            mtp_has_valid_target = False
            for k, mtp_head in enumerate(self.mtp_heads):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
                mtp_has_valid_target = True
                mtp_hidden = x[:, :valid_t, :].reshape(-1, dim)
                mtp_targets = target_ids[:, k + 1 :].reshape(-1)
                mtp_logits_proj = mtp_head(mtp_hidden)
                mtp_logits = self.logit_softcap * torch.tanh(mtp_logits_proj / self.logit_softcap)
                horizon_weight = self.mtp_loss_decay ** k
            ...
            if mtp_has_valid_target:
                main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / mtp_weight_sum)
        return main_loss
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    model = GPT(
        ...
        num_layers=min(args.num_layers, 4),
        ...
        mtp_loss_decay=args.mtp_loss_decay,
        mtp_init=args.mtp_init,
        bigram_vocab_size=args.bigram_vocab_size,
        bigram_dim=args.bigram_dim,
        xsa_last_n=min(args.xsa_last_n, 1),
        rope_dims=min(args.rope_dims, args.model_dim // args.num_heads),
    ...
    model.train()
    x = torch.randint(args.vocab_size, (batch_size, seq_len), device=device)
    y = torch.randint(args.vocab_size, (batch_size, seq_len), device=device)
    mtp_before = None
    optimizer_mtp = None
    if model.mtp_heads:
        mtp_before = model.mtp_heads[0].weight.detach().clone()
        optimizer_mtp = torch.optim.AdamW(
            [{"params": [head.weight for head in model.mtp_heads], "lr": args.mtp_head_lr}],
    ...
    loss.backward()
    mtp_grad = sum(
        float(head.weight.grad.abs().sum().item())
        for head in model.mtp_heads
        if head.weight.grad is not None
    )
    mtp_delta = 0.0
    if optimizer_mtp is not None and mtp_before is not None:
        optimizer_mtp.step()
        mtp_delta = float((model.mtp_heads[0].weight.detach() - mtp_before).abs().sum().item())
    print(
        f"smoke_test:ok loss:{loss.item():.4f} "
        f"mtp_heads:{len(model.mtp_heads)} mtp_grad_sum:{mtp_grad:.4f} mtp_delta:{mtp_delta:.4f}"
    )
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_loss_decay:{args.mtp_loss_decay} mtp_head_lr:{args.mtp_head_lr} "
        f"mtp_init:{args.mtp_init} mtp_params:{mtp_params}"
    )
