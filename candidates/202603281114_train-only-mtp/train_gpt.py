...
def causal_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if q_sdpa.size(1) != k_sdpa.size(1):
        groups = q_sdpa.size(1) // k_sdpa.size(1)
        if groups * k_sdpa.size(1) != q_sdpa.size(1):
            raise ValueError("num_heads must be divisible by num_kv_heads for GQA fallback")
        k_sdpa = k_sdpa.repeat_interleave(groups, dim=1)
        v_sdpa = v_sdpa.repeat_interleave(groups, dim=1)
...
def main() -> None:
    ...
    # - non-banked small matrix params -> Adam-side optimizers
    matrix_params = [
