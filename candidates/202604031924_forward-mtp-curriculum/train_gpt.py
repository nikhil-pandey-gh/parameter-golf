...
class Hyperparameters:
    ...
    mtp_start_frac = float(os.environ.get("MTP_START_FRAC", 0.15))
    mtp_full_frac = float(os.environ.get("MTP_FULL_FRAC", 0.45))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_active = mtp_num_heads > 0 and mtp_loss_weight > 0.0
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_active:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + self.mtp_loss_weight_buf.to(dtype=main_loss.dtype) * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    base_model.set_mtp_loss_weight(0.0 if args.mtp_num_heads > 0 else args.mtp_loss_weight)
    ...
    # - bigram proj, VE proj -> Adam
    # - MTP heads -> replicated AdamW (standalone 2D matrices do not fit Parallel Muon sharding)
    matrix_params = [
    ...
    if base_model.mtp_num_heads > 0:
        optimizer_mtp = torch.optim.AdamW(
            [{"params": list(base_model.mtp_heads.parameters()), "lr": args.scalar_lr, "base_lr": args.scalar_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=args.adam_wd,
            fused=True,
    ...
    log0(
        f"mtp_num_heads:{args.mtp_num_heads} mtp_max_loss_weight:{args.mtp_loss_weight} "
    ...
    def mtp_loss_weight_for_progress(progress: float) -> float:
        ...
        return args.mtp_loss_weight * ((progress - start) / max(full - start, 1e-9))
    ...
    while True:
        ...
        mtp_loss_weight = mtp_loss_weight_for_progress(training_progress(step, elapsed_ms))
        base_model.set_mtp_loss_weight(mtp_loss_weight)
