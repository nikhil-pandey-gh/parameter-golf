...
class Hyperparameters:
    ...
    shared_mtp_steps = int(os.environ.get("SHARED_MTP_STEPS", 1))
    shared_mtp_loss_weight = float(os.environ.get("SHARED_MTP_LOSS_WEIGHT", 0.12))
    shared_mtp_warmup_steps = int(os.environ.get("SHARED_MTP_WARMUP_STEPS", 1500))
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, mtp_lambda).detach()
...
class GPT(nn.Module):
    def __init__(
        ...
        if shared_mtp_steps < 0:
            raise ValueError(f"shared_mtp_steps must be non-negative, got {shared_mtp_steps}")
        if shared_mtp_steps > 1:
            raise ValueError("shared-head MTP supports at most one auxiliary horizon; use 1 or add horizon conditioning")
        self.tie_embeddings = tie_embeddings
        self.tied_embed_init_std = tied_embed_init_std
        self.logit_softcap = logit_softcap
        self.value_residual = value_residual
        self.shared_mtp_steps = shared_mtp_steps
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_lambda: Tensor | None = None) -> Tensor:
        n = self.num_layers
        ...
        if self.training and self.shared_mtp_steps > 0 and mtp_lambda is not None:
            _, seqlen, dim = x.shape
            aux_scale = mtp_lambda.to(device=x.device, dtype=x.dtype)
            mtp_loss_sum = x.new_zeros(())
            mtp_loss_count = 0
            for k in range(self.shared_mtp_steps):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + aux_scale * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def eval_val_sliding_ttt(
    ...
    stride: int, batch_seqs: int = 32, log0=print, mtp_lambda: Tensor | None = None,
) -> tuple[float, float]:
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            loss = base_model(x, y, mtp_lambda)
                        loss.backward()
                        if world_size > 1:
                            for p in ttt_params:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_mtp_steps=args.shared_mtp_steps,
    ...
    zero_mtp_lambda = torch.zeros((), device=device, dtype=torch.float32)
    ...
    log0(
        "shared_mtp:"
        f"steps:{args.shared_mtp_steps} "
        f"target_weight:{args.shared_mtp_loss_weight} "
        f"warmup_steps:{args.shared_mtp_warmup_steps}"
    )
    ...
    def shared_mtp_weight(step: int) -> float:
        if args.shared_mtp_steps <= 0 or args.shared_mtp_loss_weight <= 0.0:
            return 0.0
        if args.shared_mtp_warmup_steps <= 0:
            return args.shared_mtp_loss_weight
        return args.shared_mtp_loss_weight * min(step / args.shared_mtp_warmup_steps, 1.0)
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            mtp_lambda = torch.tensor(shared_mtp_weight(warmup_step), device=device, dtype=torch.float32)
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, mtp_lambda)
    ...
    while True:
        ...
        zero_grad_all()
        mtp_lambda = torch.tensor(shared_mtp_weight(step), device=device, dtype=torch.float32)
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_lambda)
    ...
    if args.ttt_enabled:
        ...
        ttt_loss, ttt_bpb = eval_val_sliding_ttt(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, log0=log0, mtp_lambda=zero_mtp_lambda,
        )
        torch.cuda.synchronize()
