...
class Hyperparameters:
    ...
    aq_clip_enabled = bool(int(os.environ.get("AQ_CLIP_ENABLED", "1")))
    aq_calibration_steps = int(os.environ.get("AQ_CALIBRATION_STEPS", 8))
    aq_calibration_threshold = float(os.environ.get("AQ_CALIBRATION_THRESHOLD", 0.2))

AQ_CLIP_PERCENTILES = tuple(
    float(x)
    for x in os.environ.get("AQ_PERCENTILES", "0.9990,0.9995,0.9999,0.99999,1.0").split(",")
    if x
)
...
class ActivationCalibrator:
    ...
    def watch(self, name: str, module: nn.Module) -> None:
        ...
        def hook(_module: nn.Module, inputs: tuple[Tensor, ...]) -> None:
            ...
            if flat.numel() == 0:
                return
            mean_sq = flat.square().mean(dim=0)
            if weight_name not in self._sums:
                self._sums[weight_name] = torch.zeros_like(mean_sq)
                self._counts[weight_name] = torch.zeros((), device=mean_sq.device, dtype=torch.float32)
            self._sums[weight_name].add_(mean_sq)
            self._counts[weight_name].add_(1.0)
    ...
    def finalize(self) -> dict[str, Tensor]:
        ...
        for name, total in self._sums.items():
            ...
            stats[name] = (total / denom).sqrt().cpu()
        return stats
...
def collect_activation_rms(
    ...
    if not args.aq_clip_enabled or args.aq_calibration_steps <= 0:
        log0("aq_clip:disabled")
        return {}
    calibrator = ActivationCalibrator()
    watched = 0
    for name, module in model.named_modules():
        if isinstance(module, CastedLinear):
            calibrator.watch(name, module)
            watched += 1
    ...
    if was_training:
        model.train()
    log0(
        f"aq_clip:enabled calibration_steps:{args.aq_calibration_steps} "
        f"watched_modules:{watched} collected_stats:{len(stats)}"
    )
    return stats
...
def _normalize_act_rms(act_rms: Tensor | None, in_features: int) -> Tensor | None:
    if act_rms is None or act_rms.ndim != 1 or act_rms.numel() != in_features:
        return None
    weights = act_rms.float()
    mean = weights.mean().clamp_min(1e-8)
    return (weights / mean).clamp_min(1e-4)
...
def quantize_int6_per_row(
    ...
    t32 = t.float()
    if t32.ndim == 2:
        act_weights = _normalize_act_rms(act_rms, t32.shape[1])
        best_q, best_s, best_err = None, None, float('inf')
        for pct in AQ_CLIP_PERCENTILES or (1.0,):
            if pct < 1.0:
            ...
            if act_weights is not None:
                diff = diff * act_weights[None, :]
            err = diff.pow(2).mean().item()
            if err < best_err:
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, act_rms=activation_rms.get(name) if activation_rms else None)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    while True:
        ...
        if (
            args.aq_clip_enabled
            and not activation_rms
            and args.aq_calibration_steps > 0
            and scale < args.aq_calibration_threshold
        ):
    ...
    if args.aq_clip_enabled and not activation_rms:
        log0("aq_clip:fallback weight_only_quantization no_training_stats_collected")
