...
class Hyperparameters:
    ...
    qat_scale_lr = float(os.environ.get("QAT_SCALE_LR", 0.01))
    qat_log_scale_min = float(os.environ.get("QAT_LOG_SCALE_MIN", math.log(0.5)))
    qat_log_scale_max = float(os.environ.get("QAT_LOG_SCALE_MAX", math.log(2.0)))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,qat_log_scale",
    ).split(",")
    if pattern
)
...
def qat_row_scales_from_log_scale(weight: Tensor, log_scale: Tensor, clip_range: int = 31) -> Tensor:
    row_max = weight.float().abs().amax(dim=-1).clamp_min(1.0 / clip_range)
    row_scale_mul = log_scale.float().clamp(Hyperparameters.qat_log_scale_min, Hyperparameters.qat_log_scale_max).exp()
    return (row_max / clip_range) * row_scale_mul
def fake_quantize_int6_weight(weight: Tensor, log_scale: Tensor, strength: Tensor, clip_range: int = 31) -> Tensor:
    if weight.ndim != 2:
        return weight
    scale = qat_row_scales_from_log_scale(weight, log_scale, clip_range=clip_range).clamp_min(1.0 / clip_range)
    w32 = weight.float()
    w_q = torch.clamp(torch.round(w32 / scale[:, None]), -clip_range, clip_range) * scale[:, None]
    w_q = w_q.to(dtype=weight.dtype)
    w_ste = w_q + (weight - weight.detach())
    return weight + (w_ste - weight) * strength.to(dtype=weight.dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        k_log_scale: Tensor,
        v_log_scale: Tensor,
        out_log_scale: Tensor,
        qat_strength: Tensor,
        v_embed: Tensor | None = None,
        v0: Tensor | None = None,
    ) -> tuple[Tensor, Tensor | None]:
        bsz, seqlen, dim = x.shape
        qat_strength = qat_strength if self.training else torch.zeros_like(qat_strength)
        q_w = fake_quantize_int6_weight(q_w, q_log_scale, qat_strength)
        k_w = fake_quantize_int6_weight(k_w, k_log_scale, qat_strength)
        v_w = fake_quantize_int6_weight(v_w, v_log_scale, qat_strength)
        out_w = fake_quantize_int6_weight(out_w, out_log_scale, qat_strength)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # No CastedLinear -- weights come from banks
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor, up_log_scale: Tensor, down_log_scale: Tensor, qat_strength: Tensor) -> Tensor:
        qat_strength = qat_strength if self.training else torch.zeros_like(qat_strength)
        up_w = fake_quantize_int6_weight(up_w, up_log_scale, qat_strength)
        down_w = fake_quantize_int6_weight(down_w, down_log_scale, qat_strength)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        up_w: Tensor,
        down_w: Tensor,
        attn_qat: tuple[Tensor, Tensor, Tensor, Tensor],
        mlp_qat: tuple[Tensor, Tensor],
        qat_strength: Tensor,
        v_embed: Tensor | None = None,
        v0: Tensor | None = None,
    ) -> tuple[Tensor, Tensor | None]:
        q_log_scale, k_log_scale, v_log_scale, out_log_scale = attn_qat
        up_log_scale, down_log_scale = mlp_qat
...
class GPT(nn.Module):
    def __init__(
        ...
        self.qo_qat_log_scale = nn.Parameter(torch.zeros(2 * num_layers, model_dim, dtype=torch.float32))
        self.kv_qat_log_scale = nn.Parameter(torch.zeros(2 * num_layers, kv_dim, dtype=torch.float32))
        self.mlp_up_qat_log_scale = nn.Parameter(torch.zeros(num_layers, mlp_dim, dtype=torch.float32))
        self.mlp_down_qat_log_scale = nn.Parameter(torch.zeros(num_layers, model_dim, dtype=torch.float32))
        ...
        self._init_weights()
    def set_qat_strength(self, strength: float) -> None:
        self.qat_strength.fill_(max(0.0, min(1.0, strength)))
    def export_qat_scale_overrides(self) -> dict[str, Tensor]:
        n = self.num_layers
        qo_scale = qat_row_scales_from_log_scale(self.qo_bank.detach(), self.qo_qat_log_scale.detach()).cpu()
        kv_scale = qat_row_scales_from_log_scale(self.kv_bank.detach(), self.kv_qat_log_scale.detach()).cpu()
        up_scale = qat_row_scales_from_log_scale(self.mlp_up_bank.detach(), self.mlp_up_qat_log_scale.detach()).cpu()
        down_scale = qat_row_scales_from_log_scale(self.mlp_down_bank.detach(), self.mlp_down_qat_log_scale.detach()).cpu()
        overrides: dict[str, Tensor] = {}
        for i in range(n):
            overrides[f"blocks.{i}.attn.c_q.weight"] = qo_scale[i]
            overrides[f"blocks.{i}.attn.proj.weight"] = qo_scale[n + i]
            overrides[f"blocks.{i}.attn.c_k.weight"] = kv_scale[i]
            overrides[f"blocks.{i}.attn.c_v.weight"] = kv_scale[n + i]
            overrides[f"blocks.{i}.mlp.fc.weight"] = up_scale[i]
            overrides[f"blocks.{i}.mlp.proj.weight"] = down_scale[i]
        return overrides
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                attn_qat=(self.qo_qat_log_scale[i], self.kv_qat_log_scale[i], self.kv_qat_log_scale[n + i], self.qo_qat_log_scale[n + i]),
                mlp_qat=(self.mlp_up_qat_log_scale[i], self.mlp_down_qat_log_scale[i]),
                qat_strength=self.qat_strength,
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                attn_qat=(self.qo_qat_log_scale[bi], self.kv_qat_log_scale[bi], self.kv_qat_log_scale[n + bi], self.qo_qat_log_scale[n + bi]),
                mlp_qat=(self.mlp_up_qat_log_scale[bi], self.mlp_down_qat_log_scale[bi]),
                qat_strength=self.qat_strength,
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, learned_scale: Tensor | None = None, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        scale_candidates: list[Tensor] = []
        if learned_scale is not None and learned_scale.ndim == 1 and learned_scale.numel() == t32.size(0):
            scale_candidates.append(learned_scale.float().clamp_min(1.0 / clip_range))
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            if pct < 1.0:
                row_clip = torch.quantile(t32.abs(), pct, dim=1)
            else:
                row_clip = t32.abs().amax(dim=1)
            scale_candidates.append((row_clip / clip_range).clamp_min(1.0 / clip_range))
        q_candidates: list[Tensor] = []
        row_errors: list[Tensor] = []
        for s in scale_candidates:
            ...
            row_errors.append((t32 - recon).pow(2).mean(dim=1))
        best_choice = torch.stack(row_errors, dim=0).argmin(dim=0)
        best_q = torch.empty_like(q_candidates[0])
        best_s = torch.empty(t32.size(0), dtype=torch.float16)
        for idx, (q, s) in enumerate(zip(q_candidates, scale_candidates, strict=True)):
            mask = best_choice == idx
            if mask.any():
                best_q[mask] = q[mask]
                best_s[mask] = s[mask].to(torch.float16)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], learned_scales: dict[str, Tensor] | None = None):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            learned_scale = learned_scales.get(name) if learned_scales is not None else None
            q, s = quantize_int6_per_row(t, learned_scale=learned_scale)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    base_model.qo_qat_log_scale.data = base_model.qo_qat_log_scale.data.float()
    base_model.kv_qat_log_scale.data = base_model.kv_qat_log_scale.data.float()
    base_model.mlp_up_qat_log_scale.data = base_model.mlp_up_qat_log_scale.data.float()
    base_model.mlp_down_qat_log_scale.data = base_model.mlp_down_qat_log_scale.data.float()
    ...
    qat_params = [
        base_model.qo_qat_log_scale,
        base_model.kv_qat_log_scale,
        base_model.mlp_up_qat_log_scale,
        base_model.mlp_down_qat_log_scale,
    ]
    ...
    optimizer_scalar = torch.optim.AdamW(
        [
            ...
            {"params": qat_params, "lr": args.qat_scale_lr, "base_lr": args.qat_scale_lr, "weight_decay": 0.0},
        ],
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        weight_decay=args.adam_wd,
    ...
    replicated_params.extend(qat_params)
    ...
    log0(f"qat_enabled:{args.qat_enabled} late_qat_threshold:{args.late_qat_threshold} qat_scale_lr:{args.qat_scale_lr}")
    ...
    while True:
        ...
        if args.qat_enabled:
            if args.late_qat_threshold <= 0:
                qat_strength = 1.0
            elif scale < args.late_qat_threshold:
                qat_strength = 1.0 - (scale / max(args.late_qat_threshold, 1e-6))
        base_model.set_qat_strength(qat_strength)
        if qat_strength > 0.0 and not qat_started:
            qat_started = True
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} strength:{qat_strength:.4f}")
        zero_grad_all()
    ...
    checkpoint_sd = {k: v for k, v in full_state_dict.items() if "mtp_heads" not in k}
    export_sd = {
        k: v
        for k, v in checkpoint_sd.items()
        if "_qat_log_scale" not in k
    }
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_heads" in k)
    excluded_qat = sum(int(t.numel()) for k, t in full_state_dict.items() if "_qat_log_scale" in k)
    if excluded_mtp > 0:
        log0(f"export_excluding_mtp_params:{excluded_mtp}")
    if excluded_qat > 0:
        log0(f"export_excluding_qat_params:{excluded_qat}")
    if master_process:
        torch.save(checkpoint_sd, "final_model.pt")
    ...
    qat_scale_overrides = base_model.export_qat_scale_overrides() if args.qat_enabled else None
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, learned_scales=qat_scale_overrides)
    ...
    for name, tensor in eval_model.state_dict().items():
        if name.endswith("_qat_log_scale"):
            deq_state[name] = tensor
