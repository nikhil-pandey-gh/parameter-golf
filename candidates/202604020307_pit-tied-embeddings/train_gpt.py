...
class Hyperparameters:
    ...
    pit_ortho_reg = float(os.environ.get("PIT_ORTHO_REG", 5e-5))
    pit_scale_init = float(os.environ.get("PIT_SCALE_INIT", 1.0))
...
class GPT(nn.Module):
    def __init__(
        ...
        if pit_scale_init <= 0.0:
            raise ValueError(f"pit_scale_init must be positive, got {pit_scale_init}")
        ...
        if self.pit_enabled:
            self.pit_log_scale = nn.Parameter(
                torch.full((model_dim,), math.log(pit_scale_init), dtype=torch.float32)
            )
        else:
            self.register_parameter("pit_log_scale", None)
    ...
    def _pit_scale(self, dtype: torch.dtype) -> Tensor | None:
        if not self.pit_enabled or self.pit_log_scale is None:
            return None
        return self.pit_log_scale.exp().to(dtype=dtype)
    def _embed_tokens(self, input_ids: Tensor) -> Tensor:
        x = self.tok_emb(input_ids)
        scale = self._pit_scale(x.dtype)
        if scale is None:
            return x
        return x / scale[None, None, :]
    def _unembed(self, hidden: Tensor) -> Tensor:
        if self.tie_embeddings:
            scale = self._pit_scale(hidden.dtype)
    ...
    def _pit_ortho_penalty(self) -> Tensor | None:
        if not self.pit_enabled or self.pit_ortho_reg <= 0.0:
            return None
        w = F.normalize(self.tok_emb.weight.float(), dim=0)
        gram = w.T @ w
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        logits_proj = self._unembed(x_flat)
        logits = self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)
        main_loss = F.cross_entropy(logits.float(), targets, reduction="mean")
        pit_penalty = self._pit_ortho_penalty() if self.training else None
        if pit_penalty is not None:
            main_loss = main_loss + self.pit_ortho_reg * pit_penalty.to(dtype=main_loss.dtype)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        pit_scale_init=args.pit_scale_init,
    ...
    log0(
        f"pit:enabled={base_model.pit_enabled} pit_ortho_reg:{args.pit_ortho_reg} "
        f"pit_scale_init:{args.pit_scale_init}"
    )
    log0(
    ...
    eval_model = GPT(
        ...
        tie_embeddings=args.tie_embeddings, pit_enabled=args.pit_enabled,
        pit_ortho_reg=args.pit_ortho_reg, pit_scale_init=args.pit_scale_init,
        tied_embed_init_std=args.tied_embed_init_std,
