...
class Hyperparameters:
    ...
    shared_mlp_until = int(os.environ.get("SHARED_MLP_UNTIL", 6))
...
class GPT(nn.Module):
    def __init__(
        ...
        shared_mlp_until: int = 0,
    ):
        super().__init__()
        if shared_mlp_group_size <= 0:
            raise ValueError(f"shared_mlp_group_size must be positive, got {shared_mlp_group_size}")
        shared_mlp_until = max(0, min(shared_mlp_until, num_layers))
        ...
        blocks: list[Block] = []
        shared_mlps: list[MLP] = []
        for i in range(num_layers):
            shared_mlp = None
            if i < shared_mlp_until:
                group_idx = i // shared_mlp_group_size
                while len(shared_mlps) <= group_idx:
                    shared_mlps.append(MLP(model_dim, mlp_mult, negative_slope=mlp_negative_slope))
                shared_mlp = shared_mlps[group_idx]
            blocks.append(
                Block(
                    model_dim,
                    num_heads,
        ...
        self.shared_mlp_groups = tuple(
            tuple(range(group_idx * shared_mlp_group_size, min((group_idx + 1) * shared_mlp_group_size, shared_mlp_until)))
            for group_idx in range(len(shared_mlps))
        )
        if rope_dims > 0:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    alias_targets: dict[tuple[int, int, tuple[int, ...], tuple[int, ...], str], str] = {}
    for name, tensor in state_dict.items():
        alias_key = _tensor_alias_key(tensor.detach())
        if alias_key in alias_targets:
            meta[name] = {"type": "alias", "target": alias_targets[alias_key]}
            continue
        alias_targets[alias_key] = name
    ...
    while aliases:
        ...
        for name, target in list(aliases.items()):
            if target in out:
                ...
                progressed = True
        if not progressed:
            unresolved = ", ".join(f"{name}->{target}" for name, target in aliases.items())
            raise KeyError(f"Could not resolve aliased tensors during dequantization: {unresolved}")
    return out
def build_gpt_kwargs(
    args: Hyperparameters,
    *,
...
def validate_arch_meta(saved_arch: object, expected_arch: dict[str, object]) -> None:
    if not isinstance(saved_arch, dict):
        raise ValueError("Quantized artifact is missing architecture metadata required to validate shared MLP topology")
    mismatches = [
        f"{key}: artifact={saved_arch.get(key)!r} runtime={value!r}"
        for key, value in expected_arch.items()
        if saved_arch.get(key) != value
    ]
    if mismatches:
...
def main() -> None:
    ...
    base_model = GPT(**build_gpt_kwargs(args)).to(device).bfloat16()
    ...
    log0(
        f"paired_mlp:groups:{list(base_model.shared_mlp_groups)} "
        f"negative_slope:{args.mlp_negative_slope:.3f} flash_attn_3:{flash_attn_3_func is not None}"
    )
    ...
    alias_count = sum(1 for info in quant_meta.values() if isinstance(info, dict) and info.get("type") == "alias")
    if alias_count:
        log0(f"shared_export_aliases:{alias_count}")
