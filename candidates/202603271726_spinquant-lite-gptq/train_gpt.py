...
class Hyperparameters:
    ...
    rotate_int6 = bool(int(os.environ.get("ROTATE_INT6", "1")))
    rotate_group_size = int(os.environ.get("ROTATE_GROUP_SIZE", 128))
...
def _normalized_hadamard_lastdim(x: Tensor) -> Tensor:
    n = x.shape[-1]
...
def apply_group_hadamard_lastdim(x: Tensor, group_size: int) -> Tensor:
    if group_size <= 0 or x.ndim == 0 or x.shape[-1] % group_size != 0 or not _is_power_of_two(group_size):
        return x
    reshaped = x.reshape(*x.shape[:-1], x.shape[-1] // group_size, group_size)
    return _normalized_hadamard_lastdim(reshaped).reshape_as(x)
...
def quantize_int6_per_row(
    ...
    t32 = t.float()
    best_q, best_s, best_err = _quantize_int6_candidate(t32, clip_range=clip_range)
    best_meta: dict[str, object] = {"type": "int6"}
    if rotate and t32.ndim == 2 and group_size > 0 and t32.shape[-1] % group_size == 0 and _is_power_of_two(group_size):
        rotated = apply_group_hadamard_lastdim(t32, group_size)
        rot_q, rot_s, rot_err = _quantize_int6_candidate(rotated, clip_range=clip_range)
        if rot_err < best_err:
            best_q, best_s = rot_q, rot_s
            best_meta = {"type": "int6", "rotation": "hadamard", "group_size": group_size}
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, qmeta = quantize_int6_per_row(
                t,
                rotate=rotate_int6,
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("rotation") == "hadamard":
            deq = apply_group_hadamard_lastdim(deq, int(info["group_size"]))
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    weight = torch.randn(256, max(args.rotate_group_size * 2, 256), dtype=torch.float32)
    q, s, info = quantize_int6_per_row(
        weight,
        rotate=args.rotate_int6,
        group_size=args.rotate_group_size,
    )
    if s.ndim > 0:
        recon = q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))
    else:
        recon = q.float() * float(s.item())
    if info.get("rotation") == "hadamard":
        recon = apply_group_hadamard_lastdim(recon, int(info["group_size"]))
    quant_mse = (weight - recon).pow(2).mean().item()
    model = GPT(
        vocab_size=64,
        num_layers=2,
        model_dim=128,
    ...
    tokens = torch.randint(0, 64, (2, 16), dtype=torch.int64)
    with torch.no_grad():
        loss = model(tokens, tokens)
    print(
        "smoke_test:ok "
        f"rotation={info.get('rotation', 'identity')} "
        f"group_size={info.get('group_size', 0)} "
        f"quant_mse={quant_mse:.6f} "
...
def main() -> None:
    ...
    if args.smoke_test:
        run_smoke_test(args)
        return
    missing_deps = [name for name, module in (("numpy", np), ("sentencepiece", spm)) if module is None]
    if missing_deps:
        raise ImportError(
            "Full training/eval requires missing dependencies: "
            + ", ".join(missing_deps)
            + ". Use requirements.txt or run SMOKE_TEST=1 for a dependency-light validation path."
        )
    ...
    use_external_flash_attn = flash_attn_3_func is not None
    enable_cudnn_sdp(False)
    enable_flash_sdp(use_external_flash_attn)
    enable_mem_efficient_sdp(not use_external_flash_attn)
    enable_math_sdp(not use_external_flash_attn)
    logfile = None
    if master_process:
    ...
    log0(
        "sdp_backends:"
        f"cudnn=False flash={use_external_flash_attn} "
        f"mem_efficient={not use_external_flash_attn} math={not use_external_flash_attn}"
    )
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        rotate_int6=args.rotate_int6,
        rotate_group_size=args.rotate_group_size,
    )
