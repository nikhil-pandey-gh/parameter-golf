...
class Hyperparameters:
    ...
    # Training-only auxiliary heads; excluded from export later in the script.
...
def main() -> None:
    ...
    aux_head_params = [head.weight for head in base_model.mtp_heads]
    if base_model.lm_head is not None:
        aux_head_params.append(base_model.lm_head.weight)
    optimizer_head = None
    if aux_head_params:
        optimizer_head = torch.optim.Adam(
            [{"params": aux_head_params, "lr": args.head_lr, "base_lr": args.head_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            fused=True,
        )
