...
INT6_CLIP_RANGE = 31
QAT_CLIP_MULT_MIN = 0.5
QAT_CLIP_MULT_MAX = 1.25

def clip_multiplier_from_log(log_scale: Tensor) -> Tensor:
    return log_scale.float().exp().clamp(QAT_CLIP_MULT_MIN, QAT_CLIP_MULT_MAX)

def fake_quantize_int6_per_row(
    t: Tensor,
    clip_multiplier: Tensor | float | None = None,
    ...
    if t32.ndim < 2:
        ...
        if clip_multiplier is not None:
            clip = clip * float(clip_multiplier)
        scale = (clip / clip_range).clamp_min(1.0 / clip_range)
        x = (t32 / scale).clamp(-clip_range, clip_range)
        x = x + (torch.round(x) - x).detach()
        return (x * scale).to(dtype=t.dtype)
    row_clip = t32.abs().amax(dim=-1, keepdim=True)
    if clip_multiplier is not None:
        mult = clip_multiplier if isinstance(clip_multiplier, Tensor) else torch.tensor(clip_multiplier)
        mult = mult.to(dtype=row_clip.dtype, device=row_clip.device)
        while mult.ndim < row_clip.ndim:
            mult = mult.unsqueeze(-1)
        row_clip = row_clip * mult
...
class GPT(nn.Module):
    ...
    def _effective_banks(self) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        if not self.bank_qat_enabled or not self.training:
            return self.qo_bank, self.kv_bank, self.mlp_up_bank, self.mlp_down_bank
        return (
            fake_quantize_int6_per_row(self.qo_bank, clip_multiplier=clip_multiplier_from_log(self.qo_qat_log_scale)),
            fake_quantize_int6_per_row(self.kv_bank, clip_multiplier=clip_multiplier_from_log(self.kv_qat_log_scale)),
            fake_quantize_int6_per_row(self.mlp_up_bank, clip_multiplier=clip_multiplier_from_log(self.mlp_up_qat_log_scale)),
            fake_quantize_int6_per_row(self.mlp_down_bank, clip_multiplier=clip_multiplier_from_log(self.mlp_down_qat_log_scale)),
        )
    def export_qat_clip_multipliers(self) -> dict[str, float]:
        out: dict[str, float] = {}
        qo_mult = clip_multiplier_from_log(self.qo_qat_log_scale.detach().cpu())
        kv_mult = clip_multiplier_from_log(self.kv_qat_log_scale.detach().cpu())
        up_mult = clip_multiplier_from_log(self.mlp_up_qat_log_scale.detach().cpu())
        down_mult = clip_multiplier_from_log(self.mlp_down_qat_log_scale.detach().cpu())
        n = self.num_layers
        for i in range(n):
            out[f"blocks.{i}.attn.c_q.weight"] = float(qo_mult[i].item())
            out[f"blocks.{i}.attn.proj.weight"] = float(qo_mult[n + i].item())
            out[f"blocks.{i}.attn.c_k.weight"] = float(kv_mult[i].item())
            out[f"blocks.{i}.attn.c_v.weight"] = float(kv_mult[n + i].item())
            out[f"blocks.{i}.mlp.fc.weight"] = float(up_mult[i].item())
            out[f"blocks.{i}.mlp.proj.weight"] = float(down_mult[i].item())
        return out
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        qo_bank, kv_bank, mlp_up_bank, mlp_down_bank = self._effective_banks()
...
def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = INT6_CLIP_RANGE,
    learned_clip_multiplier: float | None = None,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        ...
        if learned_clip_multiplier is not None:
            mult = max(QAT_CLIP_MULT_MIN, min(QAT_CLIP_MULT_MAX, float(learned_clip_multiplier)))
            row_clip = row_amax * mult
            s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
            q = torch.clamp(torch.round(t32 / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
            recon = q.float() * s.float()[:, None]
            err = (t32 - recon).pow(2).mean().item()
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    learned_clip_multipliers: dict[str, float] | None = None,
):
    learned_clip_multipliers = learned_clip_multipliers or {}
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, learned_clip_multiplier=learned_clip_multipliers.get(name))
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    optimizer_scalar = torch.optim.AdamW(
        [
            ...
            {"params": qat_params, "lr": args.scalar_lr, "base_lr": args.scalar_lr, "weight_decay": 0.0},
        ],
        betas=(args.beta1, args.beta2),
        eps=args.adam_eps,
        weight_decay=args.adam_wd,
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not base_model.bank_qat_enabled:
            ...
            compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
            model = compiled_model
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} bank_qat:1")
        zero_grad_all()
    ...
    learned_clip_multipliers = base_model.export_qat_clip_multipliers()
    clip_values = list(learned_clip_multipliers.values())
    if clip_values:
        log0(
            f"bank_qat_clip_multipliers mean:{sum(clip_values) / len(clip_values):.4f} "
            f"min:{min(clip_values):.4f} max:{max(clip_values):.4f}"
        )
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        learned_clip_multipliers=learned_clip_multipliers,
    )
