...
class Hyperparameters:
    ...
    mtp_num_heads = int(os.environ.get("MTP_NUM_HEADS", 4))
    mtp_loss_weight = float(os.environ.get("MTP_LOSS_WEIGHT", 0.2))
    mtp_loss_decay = float(os.environ.get("MTP_LOSS_DECAY", 0.5))
    mtp_ramp_steps = int(os.environ.get("MTP_RAMP_STEPS", 750))
...
def eval_val(
    ...
    mtp_scale = torch.zeros((), device=device, dtype=torch.float32)
    model.eval()
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, mtp_scale).detach()
...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_num_heads > 0 and mtp_loss_decay <= 0.0:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_scale: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            mtp_scale_tensor = torch.as_tensor(
                1.0 if mtp_scale is None else mtp_scale,
                device=x.device,
            ...
            if seqlen > 1:
                ...
                main_loss = main_loss + (self.mtp_loss_weight * mtp_scale_tensor) * (mtp_loss_sum / mtp_weight_sum)
        return main_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_loss_decay:{args.mtp_loss_decay} mtp_ramp_steps:{args.mtp_ramp_steps} mtp_params:{mtp_params}"
    )
    ...
    def mtp_scale_for_step(step: int) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_ramp_steps <= 0:
            return 1.0
        return min((step + 1) / args.mtp_ramp_steps, 1.0)
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                mtp_scale = torch.tensor(mtp_scale_for_step(warmup_step), device=device, dtype=torch.float32)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, mtp_scale)
    ...
    while True:
        ...
        mtp_scale = torch.tensor(mtp_scale_for_step(step), device=device, dtype=torch.float32)
        zero_grad_all()
