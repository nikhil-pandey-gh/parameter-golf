...
class Hyperparameters:
    ...
    mtp_ramp_steps = int(os.environ.get("MTP_RAMP_STEPS", 1600))
    mtp_head_stagger_steps = int(os.environ.get("MTP_HEAD_STAGGER_STEPS", 800))
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads[: self.active_mtp_heads]):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
...
def main() -> None:
    ...
    def compute_mtp_head_weights(step: int) -> Tensor:
        if args.mtp_num_heads <= 0:
            return torch.zeros((0,), dtype=torch.float32)
        ramp_steps = max(args.mtp_ramp_steps, 1)
        weights = []
        for head_idx in range(args.mtp_num_heads):
            start_step = args.mtp_warmup_steps + head_idx * args.mtp_head_stagger_steps
            weight = min(max((step - start_step) / ramp_steps, 0.0), 1.0)
    ...
    current_mtp_active_mask = [False] * args.mtp_num_heads

    def update_mtp_schedule(step: int) -> None:
        nonlocal current_mtp_active_mask, model
        if base_model.mtp_head_weights.numel() == 0:
            return
        head_weights = compute_mtp_head_weights(step)
        active_mask = [weight > 0.0 for weight in head_weights.tolist()]
        active_head_count = sum(active_mask)
        with torch.no_grad():
            base_model.mtp_head_weights.copy_(head_weights.to(base_model.mtp_head_weights.device))
        if active_head_count != base_model.active_mtp_heads:
            base_model.active_mtp_heads = active_head_count
            model = torch.compile(base_model, dynamic=False, fullgraph=True)
            model.train()
            log0(f"mtp_curriculum:active_heads:{active_head_count} step:{step}")
        current_mtp_active_mask = active_mask
    ...
    mtp_optimizers = [
        torch.optim.AdamW(
            [{"params": [head.weight], "lr": args.scalar_lr, "base_lr": args.scalar_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=args.adam_wd,
            fused=True,
    ...
    log0(
        f"mtp_curriculum warmup_steps:{args.mtp_warmup_steps} "
        f"ramp_steps:{args.mtp_ramp_steps} head_stagger_steps:{args.mtp_head_stagger_steps}"
    )
    ...
    update_mtp_schedule(0)
    if args.warmup_steps > 0:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        initial_mtp_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in mtp_optimizers]
        model.train()
        for warmup_step in range(args.warmup_steps):
            ...
            for opt in optimizers:
                opt.step()
            for is_active, opt in zip(current_mtp_active_mask, mtp_optimizers, strict=True):
                if is_active:
                    opt.step()
            zero_grad_all()
        ...
        for opt, state in zip(mtp_optimizers, initial_mtp_optimizer_states, strict=True):
            opt.load_state_dict(state)
        zero_grad_all()
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
    ...
    while True:
        ...
        if distributed:
            for p in replicated_params:
                if p.grad is not None:
                    dist.all_reduce(p.grad, op=dist.ReduceOp.AVG)
            for is_active, head in zip(current_mtp_active_mask, base_model.mtp_heads, strict=True):
                if is_active and head.weight.grad is not None:
                    dist.all_reduce(head.weight.grad, op=dist.ReduceOp.AVG)
        optimizer_tok.step()
        ...
        if should_log_train:
            mtp_weights = ",".join(f"{w:.2f}" for w in base_model.mtp_head_weights.detach().cpu().tolist())
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms "
                f"mtp_head_weights:[{mtp_weights}]"
            )
