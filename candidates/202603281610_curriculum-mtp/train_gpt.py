...
def build_mtp_head_weights(args: Hyperparameters, step: int, device: torch.device) -> Tensor | None:
    ...
    steps_per_head = max(args.mtp_curriculum_steps / max(args.mtp_num_heads, 1), 1.0)
    step_tensor = torch.tensor(float(step + 1), device=device, dtype=torch.float32)
    head_offsets = torch.arange(args.mtp_num_heads, device=device, dtype=torch.float32) * steps_per_head
    return ((step_tensor - head_offsets) / steps_per_head).clamp(0.0, 1.0)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_head_weights: Tensor | None = None) -> Tensor:
        n = self.num_layers
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            if mtp_head_weights is None:
                mtp_head_weights = torch.ones(self.mtp_num_heads, device=x.device, dtype=torch.float32)
            mtp_head_weights = mtp_head_weights.to(device=x.device, dtype=x.dtype)
