...
def infer_repo_root(script_dir: Path) -> Path:
    for parent in (script_dir, *script_dir.parents):
        if (parent / "data").is_dir() and (parent / "README.md").is_file():
            return parent
    raise FileNotFoundError(
        "Could not infer repository root containing data/ and README.md. "
        "Set DATA_PATH and TOKENIZER_PATH explicitly."
    )

SCRIPT_DIR = Path(__file__).resolve().parent
REPO_ROOT = infer_repo_root(SCRIPT_DIR)
class Hyperparameters:
    ...
    mtp_lr = float(os.environ.get("MTP_LR", 0.025))
    mtp_curriculum_start = float(os.environ.get("MTP_CURRICULUM_START", 0.10))
    mtp_curriculum_end = float(os.environ.get("MTP_CURRICULUM_END", 0.60))
...
class GPT(nn.Module):
    ...
    def set_mtp_head_weights(self, weights: Tensor) -> None:
        ...
        self.mtp_head_weights.copy_(
            weights.to(device=self.mtp_head_weights.device, dtype=self.mtp_head_weights.dtype)
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            head_weights = self.mtp_head_weights.to(device=x.device, dtype=torch.float32)
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + head_weights[k] * F.cross_entropy(
                    mtp_logits.float(),
                    mtp_targets,
                    reduction="mean",
                )
            main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / max(self.mtp_num_heads, 1))
        return main_loss
...
def mtp_progress(args: Hyperparameters, step: int, elapsed_ms: float) -> float:
    if args.max_wallclock_seconds > 0:
        return min(max(elapsed_ms / (1000.0 * args.max_wallclock_seconds), 0.0), 1.0)
    if args.iterations <= 0:
        return 1.0
    return min(max(step / args.iterations, 0.0), 1.0)

def build_mtp_head_weights(args: Hyperparameters, progress: float) -> Tensor:
    if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
        return torch.zeros(args.mtp_num_heads, dtype=torch.float32)
    if not 0.0 <= args.mtp_curriculum_start < args.mtp_curriculum_end <= 1.0:
        raise ValueError(
            "MTP curriculum expects 0 <= MTP_CURRICULUM_START < MTP_CURRICULUM_END <= 1"
        )
    if progress <= args.mtp_curriculum_start:
        return torch.zeros(args.mtp_num_heads, dtype=torch.float32)
    if progress >= args.mtp_curriculum_end:
        return torch.ones(args.mtp_num_heads, dtype=torch.float32)
    active_phase = (progress - args.mtp_curriculum_start) / (
        args.mtp_curriculum_end - args.mtp_curriculum_start
    )
    stage = active_phase * args.mtp_num_heads
    weights = torch.zeros(args.mtp_num_heads, dtype=torch.float32)
    for idx in range(args.mtp_num_heads):
        weights[idx] = min(max(stage - idx, 0.0), 1.0)
    return weights
...
def main() -> None:
    ...
    base_model.set_mtp_head_weights(build_mtp_head_weights(args, 0.0))
    ...
    if base_model.mtp_num_heads > 0:
        optimizer_mtp = torch.optim.AdamW(
            [{"params": list(base_model.mtp_heads.parameters()), "lr": args.mtp_lr, "base_lr": args.mtp_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            weight_decay=args.adam_wd,
            fused=True,
    ...
    if optimizer_mtp is not None:
        for pg in optimizer_mtp.param_groups:
            replicated_params.extend(pg["params"])
    ...
    log0(
        ...
        f"mtp_lr:{args.mtp_lr} mtp_params:{mtp_params} "
    ...
    while True:
        ...
        base_model.set_mtp_head_weights(build_mtp_head_weights(args, mtp_progress(args, step, elapsed_ms)))
        zero_grad_all()
        ...
        if should_log_train:
            mtp_w = ",".join(f"{w:.2f}" for w in base_model.mtp_head_weights.detach().cpu().tolist()) or "-"
            log0(
