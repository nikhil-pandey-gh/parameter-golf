...
class Hyperparameters:
    ...
    quant_rotate_enabled = bool(int(os.environ.get("QUANT_ROTATE_ENABLED", "1")))
    quant_rotate_min_dim = int(os.environ.get("QUANT_ROTATE_MIN_DIM", 128))
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def _quantize_int6_per_row_mse(t32: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, float]:
    t32 = t32.float()
    if t32.ndim == 2:
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    q, s, _ = _quantize_int6_per_row_mse(t.float(), clip_range)
    return q, s


def _signed_seed_from_name(name: str) -> int:
    return int.from_bytes(hashlib.sha256(name.encode("utf-8")).digest()[:8], "little") & ((1 << 63) - 1)


def _largest_hadamard_block(width: int, min_dim: int) -> int:
    block = width & -width
    return block if block >= min_dim and (block & (block - 1)) == 0 else 0


def _signed_hadamard_signs(name: str, width: int) -> Tensor:
    g = torch.Generator()
    g.manual_seed(_signed_seed_from_name(name))
    bits = torch.randint(0, 2, (width,), generator=g)
    return bits.to(dtype=torch.float32).mul_(2.0).sub_(1.0)


def _hadamard_last_dim(x: Tensor) -> Tensor:
    n = x.shape[-1]
    if n <= 0 or (n & (n - 1)) != 0:
        raise ValueError(f"Hadamard width must be a positive power of two, got {n}")
    y = x.reshape(-1, n)
    h = 1
    while h < n:
        ...
        h *= 2
    y = y.reshape(-1, n) / math.sqrt(float(n))
    return y.reshape_as(x)


def _block_hadamard_last_dim(x: Tensor, block_size: int) -> Tensor:
    if block_size <= 0:
        raise ValueError(f"block_size must be positive, got {block_size}")
    if x.shape[-1] % block_size != 0:
        raise ValueError(f"Width {x.shape[-1]} must be divisible by block_size {block_size}")
    if x.shape[-1] == block_size:
        return _hadamard_last_dim(x)
    reshaped = x.reshape(-1, x.shape[-1] // block_size, block_size)
    transformed = _hadamard_last_dim(reshaped)
    return transformed.reshape_as(x)


def _dequantize_rowwise(q: Tensor, s: Tensor) -> Tensor:
    if s.ndim > 0:
        return q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))
    return q.float() * float(s.item())


def _apply_signed_hadamard_rotation(x: Tensor, name: str, block_size: int) -> Tensor:
    signs = _signed_hadamard_signs(name, x.shape[-1]).to(dtype=x.dtype)
    return _block_hadamard_last_dim(x * signs, block_size)


def _invert_signed_hadamard_rotation(x: Tensor, name: str, block_size: int) -> Tensor:
    signs = _signed_hadamard_signs(name, x.shape[-1]).to(dtype=x.dtype)
    return _block_hadamard_last_dim(x, block_size) * signs


def quantize_int6_per_row_spinquant_lite(
    name: str,
    t: Tensor,
    clip_range: int = 31,
    min_dim: int = 128,
    ...
    t32 = t.float()
    best_q, best_s, best_err = _quantize_int6_per_row_mse(t32, clip_range)
    if t32.ndim != 2:
        return best_q, best_s, None
    block_size = _largest_hadamard_block(t32.shape[-1], min_dim)
    if block_size == 0:
        return best_q, best_s, None
    rotated = _apply_signed_hadamard_rotation(t32, name, block_size)
    rq, rs, _ = _quantize_int6_per_row_mse(rotated, clip_range)
    rotated_recon = _dequantize_rowwise(rq, rs)
    recon = _invert_signed_hadamard_rotation(rotated_recon, name, block_size)
    rot_err = (t32 - recon).pow(2).mean().item()
    if rot_err + 1e-12 < best_err:
        return rq, rs, {"kind": "signed_hadamard", "block_size": block_size}
    return best_q, best_s, None
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            if Hyperparameters.quant_rotate_enabled:
                q, s, rotation = quantize_int6_per_row_spinquant_lite(
                    name,
                    t,
                    min_dim=Hyperparameters.quant_rotate_min_dim,
                )
            else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        restored = _dequantize_rowwise(q, s)
        rotation = info.get("rotation") if isinstance(info, dict) else None
        if rotation is not None:
            if rotation.get("kind") != "signed_hadamard":
                raise ValueError(f"Unsupported rotation kind for {name}: {rotation}")
            restored = _invert_signed_hadamard_rotation(restored, name, int(rotation["block_size"]))
...
def main() -> None:
    ...
    rotated_tensors = sum(
        ...
        if isinstance(info, dict) and info.get("type") == "int6" and info.get("rotation") is not None
    )
    int6_tensors = sum(
        1
        for info in quant_meta.values()
    ...
    if master_process:
        log0(
            f"spinquant_lite rotated_tensors:{rotated_tensors}/{int6_tensors} "
            f"enabled:{int(args.quant_rotate_enabled)} min_dim:{args.quant_rotate_min_dim}"
        )
