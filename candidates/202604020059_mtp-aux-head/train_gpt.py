...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_num_heads > 0:
            warm_start = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
            with torch.no_grad():
                for head in self.mtp_heads:
                    head.weight.copy_(warm_start)
...
def main() -> None:
    ...
    # - output / auxiliary token heads -> Adam
    matrix_params = [
    ...
    mtp_head_params = [p for head in base_model.mtp_heads for p in head.parameters()]
    if mtp_head_params:
    ...
    if head_param_groups:
        ...
        for group in head_param_groups:
            replicated_params.extend(group["params"])
    ...
    log0(
        ...
        f"mtp_params:{mtp_params} mtp_optimizer:{'on' if mtp_head_params else 'off'}"
    )
