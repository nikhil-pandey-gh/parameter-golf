...
class Hyperparameters:
    ...
    mtp_start_frac = float(os.environ.get("MTP_START_FRAC", 0.10))
    mtp_full_frac = float(os.environ.get("MTP_FULL_FRAC", 0.35))
    mtp_end_frac = float(os.environ.get("MTP_END_FRAC", 0.75))
    mtp_weight_buckets = int(os.environ.get("MTP_WEIGHT_BUCKETS", 8))
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if self.training and self.mtp_num_heads > 0 and mtp_active_heads > 0 and mtp_loss_weight > 0.0:
            _, seqlen, dim = x.shape
            mtp_loss_sum = x.new_zeros(())
            active_heads = min(max(mtp_active_heads, 0), self.mtp_num_heads)
            ...
            main_loss = main_loss + mtp_loss_weight * (mtp_loss_sum / max(active_heads, 1))
        return main_loss
...
def training_progress(step: int, elapsed_ms: float, max_wallclock_ms: float | None, iterations: int) -> float:
...
def build_mtp_curriculum(args: Hyperparameters, progress: float) -> tuple[int, float]:
    if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
        return 0, 0.0

    start = min(max(args.mtp_start_frac, 0.0), 1.0)
    full = min(max(args.mtp_full_frac, start), 1.0)
    end = min(max(args.mtp_end_frac, full), 1.0)

    if progress < start:
        frac = 0.0
    elif progress < full:
        frac = (progress - start) / max(full - start, 1e-8)
    elif progress < end:
        frac = 1.0
    else:
        frac = max(1.0 - (progress - end) / max(1.0 - end, 1e-8), 0.0)

    active_heads = 0 if frac <= 0.0 else max(1, math.ceil(frac * args.mtp_num_heads))
    weight = args.mtp_loss_weight * frac
    if args.mtp_weight_buckets > 0 and args.mtp_loss_weight > 0.0:
        bucket = args.mtp_loss_weight / args.mtp_weight_buckets
        if bucket > 0.0:
            weight = round(weight / bucket) * bucket
    weight = min(max(weight, 0.0), args.mtp_loss_weight)
    if weight <= 0.0:
        active_heads = 0
    return active_heads, weight
...
def run_smoke_test(
    ...
    optimizer = torch.optim.AdamW(base_model.parameters(), lr=1e-3, fused=use_cuda)

    phases: list[tuple[int, float]] = [(0, 0.0)]
    if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0:
        active_heads, active_weight = build_mtp_curriculum(args, 0.5)
        phases.append((active_heads, active_weight))
    phases.extend([phases[-1]] * max(args.smoke_steps - 1, 0))

    compiled_model.train()
    for step, (mtp_active_heads, mtp_weight) in enumerate(phases, start=1):
        x = torch.randint(0, args.vocab_size, (args.smoke_batch_seqs, args.smoke_seq_len), device=device)
        y = torch.randint(0, args.vocab_size, (args.smoke_batch_seqs, args.smoke_seq_len), device=device)
        optimizer.zero_grad(set_to_none=True)
        loss = compiled_model(x, y, mtp_active_heads, mtp_weight)
        loss.backward()
        optimizer.step()
        log0(
            f"smoke_step:{step}/{len(phases)} active_heads:{mtp_active_heads} "
            f"weight:{mtp_weight:.4f} loss:{loss.item():.4f}"
        )
    ...
    eval_model.eval()
    with torch.no_grad():
        x = torch.randint(0, args.vocab_size, (1, args.smoke_seq_len), device=device)
        logits = eval_model.forward_logits(x)
    log0(f"smoke_test:ok logits_shape:{tuple(logits.shape)} export_reload:ok")
...
def main() -> None:
    ...
    if distributed and not use_cuda:
        raise RuntimeError("Distributed execution requires CUDA in this candidate")
    ...
    if args.smoke_test:
        run_smoke_test(args, device, use_cuda, log0)
        if distributed:
    ...
    log0(
        f"mtp_curriculum:start_frac:{args.mtp_start_frac:.2f} "
        f"full_frac:{args.mtp_full_frac:.2f} end_frac:{args.mtp_end_frac:.2f}"
    )
    ...
    log0(
        ...
        f"flash_attn3:{flash_attn_3_func is not None}"
    )
    ...
    warmup_mtp_active_heads = 0
    warmup_mtp_weight = 0.0
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, warmup_mtp_active_heads, warmup_mtp_weight)
    ...
    t0 = time.perf_counter()
    step = 0
    last_mtp_state: tuple[int, float] | None = None
    while True:
        ...
        mtp_progress = training_progress(step, elapsed_ms, max_wallclock_ms, args.iterations)
        mtp_active_heads, mtp_weight_value = build_mtp_curriculum(args, mtp_progress)
        mtp_state = (mtp_active_heads, round(mtp_weight_value, 4))
        if mtp_state != last_mtp_state:
            log0(
                f"mtp_curriculum:progress:{mtp_progress:.3f} "
                f"active_heads:{mtp_state[0]}/{args.mtp_num_heads} weight:{mtp_weight_value:.4f}"
            )
            last_mtp_state = mtp_state
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_active_heads, mtp_weight_value)
