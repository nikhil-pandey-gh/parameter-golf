...
def fused_attention(q: Tensor, k: Tensor, v: Tensor, *, causal: bool = True) -> Tensor:
...
class Hyperparameters:
    ...
    attn_gate_last_n = int(os.environ.get("ATTN_GATE_LAST_N", 4))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        capture_value_anchor: bool = False,
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.capture_value_anchor = capture_value_anchor
        if value_residual:
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        raw_v = v if self.capture_value_anchor else None
        ...
        y = fused_attention(q, k, v, causal=True)
        if self.use_xsa:
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        ...
                                        capture_value_anchor=capture_value_anchor)
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    gated_attention=gated_attention and (gated_attention_last_n <= 0 or i >= num_layers - gated_attention_last_n),
                    value_residual=value_residual and (value_residual_last_n <= 0 or i >= num_layers - value_residual_last_n),
                    capture_value_anchor=value_residual and i == 0,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        gated_attention_last_n=args.attn_gate_last_n,
    ...
    value_residual_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_value_residual]
    value_anchor_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.capture_value_anchor]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"ATTN_GATE:last_{args.attn_gate_last_n} active_layers:{gated_layers}")
    log0(
        f"VALUE_RESIDUAL:last_{args.value_residual_last_n} "
        f"active_layers:{value_residual_layers} anchor_layers:{value_anchor_layers}"
    )
    ...
    eval_model = GPT(
        ...
        gated_attention=args.gated_attention, gated_attention_last_n=args.attn_gate_last_n,
        value_residual=args.value_residual, value_residual_last_n=args.value_residual_last_n,
    ).to(device).bfloat16()
