...
class Hyperparameters:
    ...
    mtp_tail_off_scale = float(os.environ.get("MTP_TAIL_OFF_SCALE", 0.25))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer("mtp_loss_scale", torch.tensor(mtp_loss_weight, dtype=torch.float32), persistent=False)
    ...
    def set_mtp_loss_scale(self, value: float) -> None:
        self.mtp_loss_scale.fill_(float(value))
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + self.mtp_loss_scale.to(dtype=main_loss.dtype) * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_tail_off_scale:{args.mtp_tail_off_scale} mtp_params:{mtp_params}"
    )
    ...
    ema_decay = 0.997
    base_model.set_mtp_loss_scale(args.mtp_loss_weight)
    ...
    while True:
        ...
        if args.mtp_tail_off_scale > 0.0:
            mtp_scale = args.mtp_loss_weight * min(scale / args.mtp_tail_off_scale, 1.0)
        else:
            mtp_scale = args.mtp_loss_weight
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_scale:{float(base_model.mtp_loss_scale.item()):.4f}"
            )
