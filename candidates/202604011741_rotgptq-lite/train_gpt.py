...
class Hyperparameters:
    ...
    rotptq_enabled = bool(int(os.environ.get("ROTPTQ_ENABLED", "1")))
    rotptq_candidates = int(os.environ.get("ROTPTQ_CANDIDATES", 4))
    rotptq_block_size = int(os.environ.get("ROTPTQ_BLOCK_SIZE", 512))
...
def _rotptq_block_size(num_cols: int, max_block_size: int) -> int:
    limit = min(max_block_size, num_cols)
    if limit < 64:
        return 0
    block_size = 1 << (limit.bit_length() - 1)
    while block_size >= 64:
        if block_size & (block_size - 1) == 0 and num_cols % block_size == 0:
            return block_size
        block_size //= 2
    return 0


def _rotptq_signs(block_size: int, seed: int) -> Tensor:
    gen = torch.Generator()
    gen.manual_seed(seed)
    signs = torch.randint(0, 2, (block_size,), generator=gen, dtype=torch.int64)
...
def _apply_rotptq_transform(t: Tensor, block_size: int, seed: int) -> Tensor:
    if t.ndim != 2 or block_size <= 0 or t.size(1) % block_size != 0:
        return t.float()
    x = t.float().reshape(t.size(0), t.size(1) // block_size, block_size)
    signs = _rotptq_signs(block_size, seed).to(dtype=x.dtype, device=x.device)
    x = _fwht_last_dim(x * signs.view(1, 1, block_size))
    return x.reshape_as(t)


def _invert_rotptq_transform(t: Tensor, block_size: int, seed: int) -> Tensor:
    if t.ndim != 2 or block_size <= 0 or t.size(1) % block_size != 0:
        return t.float()
    x = t.float().reshape(t.size(0), t.size(1) // block_size, block_size)
    signs = _rotptq_signs(block_size, seed).to(dtype=x.dtype, device=x.device)
    x = _fwht_last_dim(x)
    x = x * signs.view(1, 1, block_size)
    return x.reshape_as(t)
...
def quantize_int6_per_row(
    ...
    clip_range: int = 31,
    *,
    rotptq_enabled: bool = False,
    rotptq_candidates: int = 1,
    rotptq_block_size: int = 512,
) -> tuple[Tensor, Tensor, int, int]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float("inf")
        best_rot_seed, best_rot_block = -1, 0
        rot_block = _rotptq_block_size(t32.size(1), rotptq_block_size) if rotptq_enabled else 0
        rot_seeds = [-1]
        if rot_block > 0:
            rot_seeds.extend(range(max(rotptq_candidates - 1, 0)))
        for rot_seed in rot_seeds:
            work = t32 if rot_seed < 0 or rot_block <= 0 else _apply_rotptq_transform(t32, rot_block, rot_seed)
            for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
                ...
                if rot_seed >= 0 and rot_block > 0:
                    recon = _invert_rotptq_transform(recon, rot_block, rot_seed)
                err = (t32 - recon).pow(2).mean().item()
                if err < best_err:
                    best_q, best_s, best_err = q, s, err
                    best_rot_seed = rot_seed
                    best_rot_block = rot_block if rot_seed >= 0 else 0
        return best_q, best_s, best_rot_seed, best_rot_block
    ...
    *,
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, rot_seed, rot_block = quantize_int6_per_row(
                t,
                rotptq_enabled=rotptq_enabled,
                rotptq_candidates=rotptq_candidates,
                rotptq_block_size=rotptq_block_size,
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", "rot_seed": rot_seed, "rot_block": rot_block}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("type") == "int6":
            rot_seed = int(info.get("rot_seed", -1))
            rot_block = int(info.get("rot_block", 0))
            if rot_seed >= 0 and rot_block > 0:
                deq = _invert_rotptq_transform(deq, rot_block, rot_seed)
...
def main() -> None:
    ...
    log0(
        f"rotptq_enabled:{args.rotptq_enabled} rotptq_candidates:{args.rotptq_candidates} "
        f"rotptq_block_size:{args.rotptq_block_size}"
    )
    log0(
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        rotptq_enabled=args.rotptq_enabled,
        rotptq_candidates=args.rotptq_candidates,
        rotptq_block_size=args.rotptq_block_size,
    )
