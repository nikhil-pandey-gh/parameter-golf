...
try:
    ...
    _ATTN_BACKEND_LABEL = "flash_attn_3"
except ImportError:
    flash_attn_3_func = None
    _ATTN_BACKEND_LABEL = "scaled_dot_product_attention"
class Hyperparameters:
    ...
    # Training-only aux heads: they improve sample efficiency but are excluded from export.
...
def main() -> None:
    ...
    if flash_attn_3_func is not None:
        ...
        sdp_backends_label = "cudnn=False flash=True mem_efficient=False math=False"
    else:
        enable_cudnn_sdp(True)
        enable_flash_sdp(True)
        enable_mem_efficient_sdp(True)
        enable_math_sdp(True)
        sdp_backends_label = "cudnn=True flash=True mem_efficient=True math=True"
    logfile = None
    if master_process:
    ...
    log0(f"attention_backend:{_ATTN_BACKEND_LABEL}")
    log0(f"sdp_backends:{sdp_backends_label}")
