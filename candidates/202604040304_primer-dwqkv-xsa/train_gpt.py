...
class Hyperparameters:
    ...
    primer_dwconv = bool(int(os.environ.get("PRIMER_DWCONV", "1")))
    primer_dwconv_kernel = int(os.environ.get("PRIMER_DWCONV_KERNEL", 3))
    primer_dwconv_last_n = int(os.environ.get("PRIMER_DWCONV_LAST_N", 4))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,dwconv",
    ).split(",")
    if pattern
)
...
class CausalDepthwiseConv1d(nn.Module):
    def __init__(self, channels: int, kernel_size: int):
        ...
        if kernel_size < 2 or kernel_size % 2 == 0:
            raise ValueError(f"PRIMER_DWCONV_KERNEL must be an odd integer >= 3, got {kernel_size}")
        self.channels = channels
        self.kernel_size = kernel_size
        self.weight = nn.Parameter(torch.zeros(channels, 1, kernel_size, dtype=torch.float32))
        with torch.no_grad():
            self.weight[:, 0, -1] = 1.0
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = F.conv1d(y, self.weight.to(dtype=x.dtype), groups=self.channels)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        primer_dwconv: bool = False,
        primer_dwconv_kernel: int = 3,
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.q_dwconv = CausalDepthwiseConv1d(dim, primer_dwconv_kernel) if primer_dwconv else None
        self.k_dwconv = CausalDepthwiseConv1d(kv_dim, primer_dwconv_kernel) if primer_dwconv else None
        self.v_dwconv = CausalDepthwiseConv1d(kv_dim, primer_dwconv_kernel) if primer_dwconv else None
    ...
    def _apply_primer_dwconv(self, x: Tensor, dwconv: CausalDepthwiseConv1d | None) -> Tensor:
        if dwconv is None:
            return x
        bsz, seqlen, heads, head_dim = x.shape
        mixed = dwconv(x.reshape(bsz, seqlen, heads * head_dim))
        return mixed.reshape(bsz, seqlen, heads, head_dim)
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        q = self._apply_primer_dwconv(q, self.q_dwconv)
        k = self._apply_primer_dwconv(k, self.k_dwconv)
        v = self._apply_primer_dwconv(v, self.v_dwconv)
        if v_embed is not None:
            v = v + v_embed.reshape(bsz, seqlen, self.num_kv_heads, self.head_dim).to(dtype=v.dtype)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        ...
                                        primer_dwconv=primer_dwconv, primer_dwconv_kernel=primer_dwconv_kernel)
...
class GPT(nn.Module):
    def __init__(
        ...
        primer_start = max(0, num_layers - primer_dwconv_last_n) if primer_dwconv else num_layers
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    primer_dwconv=primer_dwconv and i >= primer_start,
                    primer_dwconv_kernel=primer_dwconv_kernel,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        primer_dwconv=args.primer_dwconv,
        primer_dwconv_kernel=args.primer_dwconv_kernel,
        primer_dwconv_last_n=args.primer_dwconv_last_n,
    ).to(device).bfloat16()
