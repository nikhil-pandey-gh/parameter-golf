...
class CompiledTrainSwitcher(nn.Module):
    """Keep no-QAT and QAT-specialized compiled graphs available under one module."""

    def __init__(self, base_model: GPT, compiled_default: nn.Module, compiled_late_qat: nn.Module | None):
        super().__init__()
        self.base_model = base_model
        self.use_late_qat = False
        self.__dict__["_compiled_default"] = compiled_default
        self.__dict__["_compiled_late_qat"] = compiled_late_qat

    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        if self.use_late_qat and self._compiled_late_qat is not None:
            return self._compiled_late_qat(input_ids, target_ids)
        return self._compiled_default(input_ids, target_ids)

    def forward_logits(self, input_ids: Tensor) -> Tensor:
        return self.base_model.forward_logits(input_ids)
...
def main() -> None:
    ...
    compiled_default = torch.compile(base_model, dynamic=False, fullgraph=True)
    compiled_late_qat = None
    if not args.qat_enabled and args.late_qat_threshold > 0:
        CastedLinear._qat_enabled = True
        compiled_late_qat = torch.compile(base_model, dynamic=False, fullgraph=True)
        warmup_batch_seqs = max(1, args.train_batch_tokens // (world_size * grad_accum_steps * args.train_seq_len))
        dummy_x = torch.zeros(warmup_batch_seqs, args.train_seq_len, device=device, dtype=torch.int64)
        dummy_y = torch.zeros_like(dummy_x)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
            warmup_loss = compiled_late_qat(dummy_x, dummy_y)
    ...
    compiled_model = CompiledTrainSwitcher(base_model, compiled_default, compiled_late_qat)
    model: nn.Module = DDP(compiled_model, device_ids=[local_rank], broadcast_buffers=False) if distributed else compiled_model
    late_qat_active = args.qat_enabled
    ...
    while True:
        ...
        if distributed:
            elapsed_ms_tensor = torch.tensor(elapsed_ms, device=device)
            dist.all_reduce(elapsed_ms_tensor, op=dist.ReduceOp.MAX)
            elapsed_ms = float(elapsed_ms_tensor.item())
        scale = lr_mul(step, elapsed_ms)
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not late_qat_active:
            CastedLinear._qat_enabled = True
            target_model = model.module if isinstance(model, DDP) else model
            if isinstance(target_model, CompiledTrainSwitcher):
                target_model.use_late_qat = True
            late_qat_active = True
