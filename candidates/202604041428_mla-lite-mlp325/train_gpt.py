...
def find_repo_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if (candidate / "train_gpt.py").is_file() and (candidate / "data").is_dir():
            return candidate
    raise FileNotFoundError(f"Could not locate repository root from {start}")


REPO_ROOT = Path(os.environ.get("REPO_ROOT", str(find_repo_root(Path(__file__).resolve().parent))))
...
class Hyperparameters:
    ...
    kv_latent_dim = int(os.environ.get("KV_LATENT_DIM", 96))
...
def main() -> None:
    ...
    log0(f"mla_lite:enabled kv_latent_dim:{args.kv_latent_dim}")
    log0(f"rope_anchor_seq_len:{args.train_seq_len}")
