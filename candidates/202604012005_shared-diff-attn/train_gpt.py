...
class Hyperparameters:
    ...
    diff_rank = int(os.environ.get("DIFF_RANK", 16))
    diff_lambda_init = float(os.environ.get("DIFF_LAMBDA_INIT", 0.0))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,diff_lambda,diff_q_down,diff_q_up,diff_k_down,diff_k_up",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.use_diff_attn = diff_rank > 0
        ...
        if self.use_diff_attn:
            ...
            self.diff_q_down = nn.Linear(dim, diff_rank, bias=False)
            self.diff_q_up = nn.Linear(diff_rank, dim, bias=False)
            self.diff_k_down = nn.Linear(dim, diff_rank, bias=False)
            self.diff_k_up = nn.Linear(diff_rank, kv_dim, bias=False)
            self.diff_lambda = nn.Parameter(torch.full((num_heads,), diff_lambda_init, dtype=torch.float32))
            nn.init.normal_(self.diff_q_down.weight, std=1.0 / math.sqrt(dim))
            nn.init.zeros_(self.diff_q_up.weight)
            nn.init.normal_(self.diff_k_down.weight, std=1.0 / math.sqrt(dim))
            nn.init.zeros_(self.diff_k_up.weight)
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.use_diff_attn:
            q2 = q + self.diff_q_up(self.diff_q_down(x)).reshape(bsz, seqlen, self.num_heads, self.head_dim)
            k2 = k + self.diff_k_up(self.diff_k_down(x)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        ...
        if self.use_diff_attn:
            q2 = q2 * self.q_gain.to(dtype=q2.dtype)[None, None, :, None]
        y = flash_attn_3_func(q, k, v, causal=True)
        if self.use_diff_attn:
            y = y - self.diff_lambda.to(dtype=y.dtype)[None, None, :, None] * flash_attn_3_func(q2, k2, v, causal=True)
        if self.use_xsa:
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        ...
                                        diff_rank=diff_rank, diff_lambda_init=diff_lambda_init)
...
class GPT(nn.Module):
    def __init__(
        ...
        diff_attn_start = max(0, num_layers - diff_attn_last_n) if diff_attn_last_n > 0 else num_layers
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    diff_rank=diff_rank if i >= diff_attn_start else 0,
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        diff_rank=args.diff_rank,
        diff_lambda_init=args.diff_lambda_init,
    ).to(device).bfloat16()
    ...
    diff_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_diff_attn]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(
        f"DIFF_ATTN:last_{args.diff_attn_last_n} diff_rank:{args.diff_rank} "
        f"lambda_init:{args.diff_lambda_init:.4f} active_layers:{diff_layers}"
    )
    ...
    eval_model = GPT(
        ...
        diff_attn_last_n=args.diff_attn_last_n, diff_rank=args.diff_rank,
        diff_lambda_init=args.diff_lambda_init,
    ).to(device).bfloat16()
