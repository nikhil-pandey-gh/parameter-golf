...
class Hyperparameters:
    ...
    mtp_start_frac = float(os.environ.get("MTP_START_FRAC", 0.35))
    mtp_full_frac = float(os.environ.get("MTP_FULL_FRAC", 0.75))
    mtp_mid_loss_scale = float(os.environ.get("MTP_MID_LOSS_SCALE", 0.5))
...
class GPT(nn.Module):
    _mtp_active_heads: int = 0
    _mtp_loss_scale: float = 0.0
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        active_heads = min(self.mtp_num_heads, GPT._mtp_active_heads)
        loss_scale = GPT._mtp_loss_scale
        if self.training and active_heads > 0 and self.mtp_loss_weight > 0.0 and loss_scale > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads[:active_heads]):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + (self.mtp_loss_weight * loss_scale) * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    if not 0.0 <= args.mtp_start_frac <= args.mtp_full_frac <= 1.0:
        raise ValueError(
            "MTP curriculum fractions must satisfy 0 <= MTP_START_FRAC <= MTP_FULL_FRAC <= 1; "
            f"got {args.mtp_start_frac} and {args.mtp_full_frac}"
        )
    if not 0.0 <= args.mtp_mid_loss_scale <= 1.0:
        raise ValueError(f"MTP_MID_LOSS_SCALE must be in [0, 1], got {args.mtp_mid_loss_scale}")
    ...
    if args.mtp_num_heads > 0:
        if args.mtp_curriculum_enabled:
            GPT._mtp_active_heads = 0
            GPT._mtp_loss_scale = 0.0
        else:
            GPT._mtp_active_heads = args.mtp_num_heads
            GPT._mtp_loss_scale = 1.0
    else:
        GPT._mtp_active_heads = 0
        GPT._mtp_loss_scale = 0.0
    ...
    log0(
        ...
        f"mtp_start_frac:{args.mtp_start_frac:.2f} mtp_full_frac:{args.mtp_full_frac:.2f} "
        f"mtp_mid_loss_scale:{args.mtp_mid_loss_scale:.2f}"
    )
    ...
    def mtp_stage(progress: float) -> tuple[int, float, str]:
        if args.mtp_num_heads <= 0:
            return 0, 0.0, "disabled"
        if not args.mtp_curriculum_enabled:
            return args.mtp_num_heads, 1.0, "full"
        if progress < args.mtp_start_frac:
            return 0, 0.0, "ntp_only"
        if progress < args.mtp_full_frac:
            mid_heads = max(1, min(args.mtp_num_heads, math.ceil(args.mtp_num_heads / 2)))
            return mid_heads, args.mtp_mid_loss_scale, "mtp_mid"
        return args.mtp_num_heads, 1.0, "mtp_full"
    ...
    while True:
        ...
        progress = min(elapsed_ms / max_wallclock_ms, 1.0) if max_wallclock_ms is not None else min(step / max(args.iterations, 1), 1.0)
        mtp_active_heads, mtp_loss_scale, mtp_label = mtp_stage(progress)
        if (mtp_active_heads, mtp_loss_scale) != (GPT._mtp_active_heads, GPT._mtp_loss_scale):
            GPT._mtp_active_heads = mtp_active_heads
            GPT._mtp_loss_scale = mtp_loss_scale
            log0(
                f"mtp_curriculum:stage={mtp_label} progress:{progress:.3f} "
                f"active_heads:{mtp_active_heads} loss_scale:{mtp_loss_scale:.2f}"
            )
