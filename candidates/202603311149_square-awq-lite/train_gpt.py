...
class Hyperparameters:
    ...
    awq_lite_alpha = float(os.environ.get("AWQ_LITE_ALPHA", 0.5))
    awq_lite_max_scale = float(os.environ.get("AWQ_LITE_MAX_SCALE", 2.0))
    awq_lite_calibration_batches = int(os.environ.get("AWQ_LITE_CALIBRATION_BATCHES", 8))
    awq_lite_calibration_tokens = int(os.environ.get("AWQ_LITE_CALIBRATION_TOKENS", 131_072))
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self._collect_hidden_stats = False
        self.awq_lite_ema = float(os.environ.get("AWQ_LITE_EMA", 0.98))
        self.register_buffer("hidden_rms_ema", torch.ones(hidden, dtype=torch.float32), persistent=False)
        self.register_buffer("hidden_rms_updates", torch.zeros((), dtype=torch.int64), persistent=False)

    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        hidden = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=0.5).square()
        if self._collect_hidden_stats:
            batch_rms = hidden.float().pow(2).mean(dim=(0, 1)).sqrt()
            if int(self.hidden_rms_updates.item()) == 0:
                self.hidden_rms_ema.copy_(batch_rms)
            else:
                self.hidden_rms_ema.lerp_(batch_rms, 1.0 - self.awq_lite_ema)
            self.hidden_rms_updates.add_(1)
...
def collect_square_awq_stats(
    ...
    if not args.awq_lite_enabled or args.awq_lite_calibration_batches <= 0:
        return {}
    min_tokens = world_size * args.train_seq_len
    if args.awq_lite_calibration_tokens < min_tokens or args.awq_lite_calibration_tokens % min_tokens != 0:
        raise ValueError(
            "AWQ_LITE_CALIBRATION_TOKENS must be divisible by WORLD_SIZE * TRAIN_SEQ_LEN; "
            f"got {args.awq_lite_calibration_tokens} vs {min_tokens}"
        )
    calib_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
    was_training = model.training
    for block in model.blocks:
        block.mlp.hidden_rms_ema.fill_(1.0)
        block.mlp.hidden_rms_updates.zero_()
        block.mlp._collect_hidden_stats = True
    model.eval()
    with torch.inference_mode():
        for _ in range(args.awq_lite_calibration_batches):
            x, _ = calib_loader.next_batch(args.awq_lite_calibration_tokens, args.train_seq_len, 1)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                _ = model.forward_logits(x)
    if dist.is_available() and dist.is_initialized():
        for block in model.blocks:
            if int(block.mlp.hidden_rms_updates.item()) > 0:
                dist.all_reduce(block.mlp.hidden_rms_ema, op=dist.ReduceOp.SUM)
                block.mlp.hidden_rms_ema /= world_size
    stats: dict[int, Tensor] = {}
    for layer_idx, block in enumerate(model.blocks):
        block.mlp._collect_hidden_stats = False
        if int(block.mlp.hidden_rms_updates.item()) > 0:
            stats[layer_idx] = block.mlp.hidden_rms_ema.detach().float().cpu().clone()
    if was_training:
        model.train()
    return stats


def _compute_square_awq_scales(hidden_rms: Tensor, alpha: float, max_scale: float) -> Tensor:
    if max_scale < 1.0:
        raise ValueError(f"AWQ_LITE_MAX_SCALE must be >= 1.0, got {max_scale}")
    if alpha < 0.0:
        raise ValueError(f"AWQ_LITE_ALPHA must be >= 0.0, got {alpha}")
    if alpha == 0.0:
        return torch.ones_like(hidden_rms, dtype=torch.float32)
    stats = hidden_rms.float().clamp_min(1e-6)
    geom = torch.exp(stats.log().mean())
    scales = torch.pow(stats / geom, alpha).clamp(1.0 / max_scale, max_scale)
    scales = scales / torch.exp(scales.log().mean())
    return scales.clamp(1.0 / max_scale, max_scale)


def apply_square_awq_lite(
    unbanked_sd: dict[str, Tensor],
    hidden_stats: dict[int, Tensor],
    alpha: float,
    max_scale: float,
    log0=None,
) -> int:
    applied = 0
    for layer_idx, hidden_rms in sorted(hidden_stats.items()):
        up_name = f"blocks.{layer_idx}.mlp.fc.weight"
        down_name = f"blocks.{layer_idx}.mlp.proj.weight"
        up_orig = unbanked_sd.get(up_name)
        down_orig = unbanked_sd.get(down_name)
        if up_orig is None or down_orig is None:
            continue
        if up_orig.ndim != 2 or down_orig.ndim != 2 or up_orig.shape[0] != down_orig.shape[1]:
            raise ValueError(
                f"AWQ-lite expected matching 2D MLP weights for layer {layer_idx}, "
                f"got {tuple(up_orig.shape)} and {tuple(down_orig.shape)}"
            )
        if hidden_rms.numel() != up_orig.shape[0]:
            raise ValueError(
                f"AWQ-lite hidden stats mismatch for layer {layer_idx}: "
                f"{hidden_rms.numel()} vs hidden {up_orig.shape[0]}"
            )
        scales = _compute_square_awq_scales(hidden_rms, alpha, max_scale)
        sqrt_scales = scales.sqrt()
        # LeakyReLU(x)^2 is positively homogeneous of degree 2, so
        # up/sqrt(s) and down*s preserves the fp model while redistributing
        # quantization range toward the salient down-proj channels.
        unbanked_sd[up_name] = (up_orig.float() / sqrt_scales[:, None]).to(dtype=up_orig.dtype).contiguous()
        unbanked_sd[down_name] = (down_orig.float() * scales[None, :]).to(dtype=down_orig.dtype).contiguous()
        applied += 1
        if log0 is not None:
            log0(
                f"awq_lite:layer:{layer_idx} scale_min:{float(scales.min().item()):.4f} "
                f"scale_max:{float(scales.max().item()):.4f} "
                f"hidden_rms_mean:{float(hidden_rms.mean().item()):.6f}"
            )
    if log0 is not None:
        log0(f"awq_lite:applied_layers:{applied}")
    return applied
...
def main() -> None:
    ...
    log0(
        f"awq_lite:enabled:{int(args.awq_lite_enabled)} alpha:{args.awq_lite_alpha:.3f} "
        f"max_scale:{args.awq_lite_max_scale:.3f} "
        f"calib_batches:{args.awq_lite_calibration_batches} "
        f"calib_tokens:{args.awq_lite_calibration_tokens}"
    )
    ...
    awq_hidden_stats: dict[int, Tensor] = {}
    if args.awq_lite_enabled:
        torch.cuda.synchronize()
        t_awq = time.perf_counter()
        awq_hidden_stats = collect_square_awq_stats(base_model, args, rank, world_size, device)
        torch.cuda.synchronize()
        if awq_hidden_stats:
            log0(
                "awq_lite:collected_layers:"
                + ",".join(str(layer_idx) for layer_idx in sorted(awq_hidden_stats))
                + f" collect_time:{1000.0 * (time.perf_counter() - t_awq):.0f}ms"
            )
        else:
            log0("awq_lite:enabled but no calibration stats collected")
    ...
    if awq_hidden_stats:
        apply_square_awq_lite(
            unbanked_sd,
            awq_hidden_stats,
            args.awq_lite_alpha,
            args.awq_lite_max_scale,
            log0=log0,
        )
