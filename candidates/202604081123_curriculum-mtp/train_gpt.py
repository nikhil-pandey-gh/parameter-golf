...
def fa3_supported(device: torch.device) -> bool:
    if flash_attn_3_func is None or device.type != "cuda":
        return False
...
def eval_val(
    ...
    mtp_eval_scales = torch.zeros(args.mtp_num_heads, device=device, dtype=torch.float32)
    model.eval()
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, mtp_eval_scales).detach()
...
def training_progress(step: int, elapsed_ms: float, iterations: int, max_wallclock_ms: float | None) -> float:
    # Match the trainer's existing MAX_WALLCLOCK_SECONDS accounting, which tracks
    # measured training time rather than end-to-end script runtime.
...
def mtp_head_scales_for_progress(
    num_heads: int,
    progress: float,
    start_frac: float,
    end_frac: float,
    ...
    scales = torch.zeros(num_heads, device=device, dtype=torch.float32)
    if num_heads <= 0:
        return scales
    start = min(max(start_frac, 0.0), 1.0)
    end = min(max(end_frac, 0.0), 1.0)
    if end <= start:
        if progress < start:
            return scales
        return torch.ones(num_heads, device=device, dtype=torch.float32)
    ramp = min(max((progress - start) / (end - start), 0.0), 1.0)
    staged_heads = ramp * num_heads
    for head_idx in range(num_heads):
        scales[head_idx] = min(max(staged_heads - head_idx, 0.0), 1.0)
    return scales
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        input_ids: Tensor,
        target_ids: Tensor,
        mtp_head_scales: Tensor | None = None,
    ) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            if mtp_head_scales is None:
                raise RuntimeError("mtp_head_scales are required during training when MTP is enabled")
            if mtp_head_scales.numel() != self.mtp_num_heads:
                raise ValueError(
                    f"Expected {self.mtp_num_heads} MTP head scales, got {mtp_head_scales.numel()}"
                )
            _, seqlen, dim = x.shape
            scales = mtp_head_scales.to(device=x.device, dtype=x.dtype)
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + scales[k] * F.cross_entropy(
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    return out
def main() -> None:
    global USE_FA3, zeropower_via_newtonschulz5
    ...
    enable_flash_sdp(USE_FA3)
    enable_mem_efficient_sdp(not USE_FA3)
    enable_math_sdp(not USE_FA3)
    logfile = None
    if master_process:
    ...
    log0(
        ...
        f"end_frac:{args.mtp_end_frac:.2f} late_qat_threshold:{args.late_qat_threshold:.2f}"
    )
    ...
    log0(
        "sdp_backends:"
        f"cudnn=False flash={USE_FA3} mem_efficient={not USE_FA3} math={not USE_FA3}"
    )
    ...
    zero_mtp_head_scales = torch.zeros(args.mtp_num_heads, device=device, dtype=torch.float32)
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, zero_mtp_head_scales)
    ...
    while True:
        ...
        mtp_progress = training_progress(step, elapsed_ms, args.iterations, max_wallclock_ms)
        mtp_head_scales = mtp_head_scales_for_progress(
            args.mtp_num_heads,
            mtp_progress,
            args.mtp_start_frac,
            args.mtp_end_frac,
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_head_scales)
