...
class Hyperparameters:
    ...
    mtp_decay_end_scale = float(os.environ.get("MTP_DECAY_END_SCALE", 0.15))
...
def mtp_loss_weight_for_scale(args: Hyperparameters, scale: float) -> float:
    if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
        return 0.0
    start = args.mtp_decay_start_scale
    end = args.mtp_decay_end_scale
    if start <= end:
        return args.mtp_loss_weight if scale > end else 0.0
    if scale >= start:
        return args.mtp_loss_weight
    if scale <= end:
        return 0.0
    return args.mtp_loss_weight * ((scale - end) / (start - end))
...
def eval_val(
    ...
    eval_mtp_weight = torch.zeros((), device=device, dtype=torch.float32)
    model.eval()
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, eval_mtp_weight, False).detach()
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        input_ids: Tensor,
        target_ids: Tensor,
        mtp_loss_weight: Tensor | float | None = None,
        mtp_active: bool = True,
    ) -> Tensor:
        n = self.num_layers
        ...
        if mtp_loss_weight is None:
            mtp_weight = x.new_tensor(self.mtp_loss_weight, dtype=torch.float32)
        elif isinstance(mtp_loss_weight, Tensor):
            mtp_weight = mtp_loss_weight.to(device=x.device, dtype=torch.float32)
        else:
            mtp_weight = x.new_tensor(float(mtp_loss_weight), dtype=torch.float32)
        if self.training and self.mtp_num_heads > 0 and mtp_active:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + mtp_weight.to(dtype=main_loss.dtype) * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    model = GPT(
        ...
        mtp_num_heads=max(args.mtp_num_heads, 1),
        mtp_loss_weight=max(args.mtp_loss_weight, 0.15),
    ...
    target_ids = torch.randint(0, vocab_size, (batch_size, seq_len), dtype=torch.int64)
    model.train()
    mtp_weight = torch.tensor(max(args.mtp_loss_weight, 0.15), dtype=torch.float32)
    loss = model(input_ids, target_ids, mtp_weight, True)
    loss.backward()
    model.eval()
    with torch.inference_mode():
...
def main() -> None:
    ...
    head_params.extend(list(base_model.mtp_heads.parameters()))
    if head_params:
    ...
    log0(
        f"mtp_schedule:decay_start_scale:{args.mtp_decay_start_scale:.3f} "
        f"decay_end_scale:{args.mtp_decay_end_scale:.3f}"
    )
    ...
    log0(
        ...
        f"mtp_head_lr:{args.head_lr if mtp_params > 0 else 0.0} "
    ...
    warmup_mtp_weight = torch.tensor(args.mtp_loss_weight, device=device, dtype=torch.float32)
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(
                        ...
                        y,
                        warmup_mtp_weight,
                        args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0,
                    )
    ...
    while True:
        ...
        effective_mtp_weight = mtp_loss_weight_for_scale(args, scale)
        if args.mtp_num_heads > 0 and not mtp_decay_logged and effective_mtp_weight < args.mtp_loss_weight:
            log0(
                f"mtp_schedule:decay_started step:{step} scale:{scale:.4f} "
                f"mtp_weight:{effective_mtp_weight:.4f}"
            )
            mtp_decay_logged = True
        if args.mtp_num_heads > 0 and not mtp_disabled_logged and effective_mtp_weight <= 0.0:
            log0(f"mtp_schedule:disabled step:{step} scale:{scale:.4f}")
            mtp_disabled_logged = True
        mtp_weight = torch.tensor(effective_mtp_weight, device=device, dtype=torch.float32)
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_weight, effective_mtp_weight > 0.0)
