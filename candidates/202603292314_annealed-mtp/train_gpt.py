...
class Hyperparameters:
    ...
    mtp_warmdown_cutoff = float(os.environ.get("MTP_WARMDOWN_CUTOFF", 0.2))
    mtp_final_zero_ms = float(os.environ.get("MTP_FINAL_ZERO_MS", 5000.0))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer("mtp_loss_mult", torch.tensor(1.0, dtype=torch.float32), persistent=False)
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads:
            init_weight = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
            for head in self.mtp_heads:
                head.weight.data.copy_(init_weight.data)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                effective_mtp_weight = self.mtp_loss_mult.to(dtype=main_loss.dtype) * self.mtp_loss_weight
                main_loss = main_loss + effective_mtp_weight * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    # - bigram proj, mtp heads, VE proj -> AdamW (small matrix params not worth banking)
    matrix_params = [
    ...
    log0(
        ...
        f"mtp_warmdown_cutoff:{args.mtp_warmdown_cutoff} mtp_final_zero_ms:{args.mtp_final_zero_ms} "
    ...
    def mtp_mul(step: int, elapsed_ms: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_warmdown_cutoff <= 0.0 or args.warmdown_iters <= 0:
            return 1.0
        if step + 1 >= args.iterations:
            return 0.0
        remaining_after_update = max(args.iterations - (step + 1), 0)
        if remaining_after_update > args.warmdown_iters:
            scale_after_update = 1.0
        else:
            scale_after_update = remaining_after_update / max(args.warmdown_iters, 1)
        if max_wallclock_ms is not None:
            remaining_ms = max(max_wallclock_ms - elapsed_ms, 0.0)
            if remaining_ms <= args.mtp_final_zero_ms:
                return 0.0
            step_ms = elapsed_ms / max(step, 1)
            warmdown_ms = args.warmdown_iters * step_ms
            remaining_after_update_ms = max(remaining_ms - step_ms, 0.0)
            if remaining_after_update_ms > warmdown_ms:
                wallclock_scale_after_update = 1.0
            else:
                wallclock_scale_after_update = remaining_after_update_ms / max(warmdown_ms, 1e-9)
            scale_after_update = min(scale_after_update, wallclock_scale_after_update)
        return min(max(scale_after_update / args.mtp_warmdown_cutoff, 0.0), 1.0)
    ...
    while True:
        ...
        base_model.mtp_loss_mult.fill_(mtp_mul(step, elapsed_ms))
