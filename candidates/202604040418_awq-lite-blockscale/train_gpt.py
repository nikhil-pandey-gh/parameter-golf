...
class Hyperparameters:
    ...
    awq_lite_calib_steps = int(os.environ.get("AWQ_LITE_CALIB_STEPS", 8))
    awq_lite_calib_tokens = int(os.environ.get("AWQ_LITE_CALIB_TOKENS", 262_144))
    awq_lite_collect_threshold = float(os.environ.get("AWQ_LITE_COLLECT_THRESHOLD", 0.25))
    awq_lite_max_scale = float(os.environ.get("AWQ_LITE_MAX_SCALE", 4.0))
...
class Block(nn.Module):
    def __init__(
        ...
        self.register_buffer("block_input_scale", torch.ones(dim, dtype=torch.float16), persistent=True)
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        block_scale = self.block_input_scale.to(dtype=x_in.dtype)[None, None, :]
        attn_in = (self.attn_norm(x_in) * self.ln_scale_factor) / block_scale
        attn_out, raw_v = self.attn(attn_in, q_w, k_w, v_w, out_w, v_embed=v_embed, v0=v0)
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        mlp_in = (self.mlp_norm(x_out) * self.ln_scale_factor) / self.block_input_scale.to(dtype=x_out.dtype)[None, None, :]
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(mlp_in, up_w, down_w)
...
def update_awq_lite_block_stats(
    base_model: nn.Module,
    batch_tokens: Tensor,
    block_stats: dict[str, list[Tensor]],
) -> None:
    handles = []
    for i, block in enumerate(base_model.blocks):
        def attn_hook(_module, inputs, idx=i):
            x = inputs[0]
            block_stats["attn"][idx].copy_(torch.maximum(block_stats["attn"][idx], x.detach().abs().amax(dim=(0, 1)).float()))
        def mlp_hook(_module, inputs, idx=i):
            x = inputs[0]
            block_stats["mlp"][idx].copy_(torch.maximum(block_stats["mlp"][idx], x.detach().abs().amax(dim=(0, 1)).float()))
        handles.append(block.attn.register_forward_pre_hook(attn_hook))
        handles.append(block.mlp.register_forward_pre_hook(mlp_hook))
    was_training = base_model.training
    base_model.eval()
    with torch.inference_mode():
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            _ = base_model.forward_logits(batch_tokens)
...
def finalize_awq_lite_block_stats(
    block_stats: dict[str, list[Tensor]] | None,
    token_count: int,
    device: torch.device,
) -> tuple[dict[str, list[Tensor]] | None, int]:
    if block_stats is None or token_count <= 0:
        return None, 0
    total_tokens = torch.tensor(token_count, device=device, dtype=torch.int64)
    if dist.is_available() and dist.is_initialized():
        dist.all_reduce(total_tokens, op=dist.ReduceOp.SUM)
        for bucket in block_stats["attn"] + block_stats["mlp"]:
            dist.all_reduce(bucket, op=dist.ReduceOp.MAX)
    return {
        "attn": [bucket.detach().cpu() for bucket in block_stats["attn"]],
        "mlp": [bucket.detach().cpu() for bucket in block_stats["mlp"]],
    }, int(total_tokens.item())


def apply_awq_lite_block_scales(
    args: Hyperparameters,
    unbanked_sd: dict[str, Tensor],
    block_stats: dict[str, list[Tensor]] | None,
) -> list[tuple[float, float, float]] | None:
    if not args.awq_lite_enabled or block_stats is None:
        return None
    eps = 1e-6
    alpha = min(max(args.awq_lite_alpha, 0.0), 1.0)
    scale_summaries: list[tuple[float, float, float]] = []
    for i in range(args.num_layers):
        act_stat = torch.maximum(block_stats["attn"][i], block_stats["mlp"][i]).float().clamp_min(eps)
        weight_names = [
        ...
        weight_stat = torch.stack(
            [unbanked_sd[name].float().abs().amax(dim=0) for name in weight_names],
            dim=0,
        ).amax(dim=0).clamp_min(eps)
        scale = act_stat.pow(alpha) / weight_stat.pow(1.0 - alpha)
        scale = scale / torch.exp(torch.log(scale).mean())
        scale = scale.clamp(1.0 / args.awq_lite_max_scale, args.awq_lite_max_scale).to(torch.float16)
        unbanked_sd[f"blocks.{i}.block_input_scale"] = scale
        scale_f = scale.float()[None, :]
        for name in weight_names:
            orig = unbanked_sd[name]
            unbanked_sd[name] = (orig.float() * scale_f).to(dtype=orig.dtype)
        scale_summaries.append((float(scale.min()), float(scale.max()), float(scale.mean())))
...
def main() -> None:
    ...
    awq_lite_local_tokens = max(args.train_seq_len, args.awq_lite_calib_tokens // max(world_size, 1))
    awq_lite_local_tokens = max(args.train_seq_len, (awq_lite_local_tokens // args.train_seq_len) * args.train_seq_len)
    awq_lite_local_tokens = min(
        awq_lite_local_tokens,
    ...
    awq_lite_stats = (
        {
            "attn": [torch.zeros(args.model_dim, dtype=torch.float32, device=device) for _ in base_model.blocks],
            "mlp": [torch.zeros(args.model_dim, dtype=torch.float32, device=device) for _ in base_model.blocks],
        }
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            if (
                args.awq_lite_enabled
                and micro_step == 0
                and scale < args.awq_lite_collect_threshold
                and awq_lite_captures < args.awq_lite_calib_steps
                and awq_lite_stats is not None
            ):
                awq_tokens = x.reshape(-1)[:awq_lite_local_tokens].reshape(-1, args.train_seq_len).contiguous()
                update_awq_lite_block_stats(base_model, awq_tokens, awq_lite_stats)
                awq_lite_captures += 1
                awq_lite_tokens += int(awq_tokens.numel())
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
    ...
    awq_lite_stats, awq_lite_tokens = finalize_awq_lite_block_stats(
        awq_lite_stats, awq_lite_tokens, device,
    )
    # Unbank 3D tensors into individual 2D tensors for quantization
    sd_cpu = {k: v.detach().cpu() for k, v in export_sd.items()}
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers)
    awq_lite_summaries = apply_awq_lite_block_scales(args, unbanked_sd, awq_lite_stats)
    if awq_lite_summaries is not None and master_process:
        scale_min = min(item[0] for item in awq_lite_summaries)
        scale_max = max(item[1] for item in awq_lite_summaries)
        scale_mean = sum(item[2] for item in awq_lite_summaries) / len(awq_lite_summaries)
        log0(
            f"awq_lite:blockscale calib_tokens:{awq_lite_tokens} alpha:{args.awq_lite_alpha:.2f} "
            f"scale_range:[{scale_min:.3f},{scale_max:.3f}] scale_mean:{scale_mean:.3f}"
        )
    scaled_export_sd = _rebank_state_dict(unbanked_sd, args.num_layers, sd_cpu)
    if master_process:
        torch.save(scaled_export_sd, "final_model.pt")
