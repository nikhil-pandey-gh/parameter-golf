...
try:
    ...
    raise RuntimeError(
        "This candidate requires the FlashAttention interface module "
        "`flash_attn_interface`, matching the recent 11-layer record scripts."
    ) from exc


def _find_repo_root() -> Path:
    for parent in Path(__file__).resolve().parents:
        if (parent / "requirements.txt").exists() and (parent / "data").is_dir():
            return parent
    raise FileNotFoundError("Could not locate repository root containing requirements.txt and data/")
...
class Hyperparameters:
    ...
    act_aware_ptq = bool(int(os.environ.get("ACT_AWARE_PTQ", "1")))
    ptq_calibration_tokens = int(os.environ.get("PTQ_CALIBRATION_TOKENS", 262_144))
    ptq_calibration_batch_tokens = int(os.environ.get("PTQ_CALIBRATION_BATCH_TOKENS", 65_536))
    ptq_scale_limit = float(os.environ.get("PTQ_SCALE_LIMIT", 2.0))
    ptq_mlp_enabled = bool(int(os.environ.get("PTQ_MLP_ENABLED", "1")))
    ptq_mlp_alpha = float(os.environ.get("PTQ_MLP_ALPHA", 0.5))
    ptq_vo_enabled = bool(int(os.environ.get("PTQ_VO_ENABLED", "1")))
    ptq_vo_alpha = float(os.environ.get("PTQ_VO_ALPHA", 0.5))
...
def _parse_layer_csv(layer_spec: str) -> set[int]:
    return {int(x) for x in layer_spec.split(",") if x.strip()}
...
def _iter_calibration_batches(
    ...
    if usable_total <= 0:
        return
    usable_batch = max(seq_len, (batch_tokens // seq_len) * seq_len)
    stream = TokenStream(pattern)
    consumed = 0
    while consumed < usable_total:
        local_tokens = min(usable_batch, usable_total - consumed)
        local = stream.take(local_tokens + 1).to(dtype=torch.int64)
        consumed += local_tokens
        yield local[:-1].reshape(-1, seq_len).to(device, non_blocking=True)


def _activation_to_scale(abs_sum: Tensor, count: int, alpha: float, limit: float) -> Tensor:
    stats = (abs_sum / max(count, 1)).clamp_min(1e-8)
    scale = stats.pow(alpha)
    scale = scale / torch.exp(scale.log().mean())
    if limit > 1.0:
        scale = scale.clamp(1.0 / limit, limit)
        scale = scale / torch.exp(scale.log().mean())
    return scale.to(dtype=torch.float32)


def collect_activation_aware_ptq_scales(
    args: Hyperparameters,
    model: nn.Module,
    device: torch.device,
    log0,
    ...
    mlp_abs_sums = [torch.zeros(mlp_dim, dtype=torch.float64) if args.ptq_mlp_enabled else None for _ in range(num_layers)]
    mlp_counts = [0 for _ in range(num_layers)]
    vo_abs_sums = [torch.zeros(kv_dim, dtype=torch.float64) if args.ptq_vo_enabled else None for _ in range(num_layers)]
    vo_counts = [0 for _ in range(num_layers)]
    ve_layers = _parse_layer_csv(args.ve_layers) if args.ve_enabled else set()
    xsa_start = max(args.num_layers - args.xsa_last_n, 0)
    xsa_layers = set(range(xsa_start, args.num_layers))
    vo_eligible = {
        layer_idx
        for layer_idx in range(num_layers)
        if not args.value_residual and layer_idx not in ve_layers and layer_idx not in xsa_layers
    }
    handles = []
    if args.ptq_mlp_enabled:
        for layer_idx, block in enumerate(model.blocks):
            def _mlp_pre_hook(_module, inputs, idx=layer_idx):
                x, up_w, _down_w = inputs[:3]
                hidden = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=0.5)
                mlp_abs_sums[idx].add_(hidden.detach().abs().sum(dim=(0, 1)).to(dtype=torch.float64, device="cpu"))
                mlp_counts[idx] += hidden.shape[0] * hidden.shape[1]
            handles.append(block.mlp.register_forward_pre_hook(_mlp_pre_hook))
    if args.ptq_vo_enabled:
        for layer_idx, block in enumerate(model.blocks):
            if layer_idx not in vo_eligible:
                continue
            def _attn_pre_hook(module, inputs, idx=layer_idx):
                x, _q_w, _k_w, v_w, _out_w, *rest = inputs
                v_embed = rest[0] if len(rest) > 0 else None
                v = F.linear(x, v_w.to(x.dtype))
                if v_embed is not None:
                    v = v + v_embed
                v = v.reshape(x.shape[0], x.shape[1], module.num_kv_heads, module.head_dim)
                v_flat = v.reshape(x.shape[0], x.shape[1], -1)
                vo_abs_sums[idx].add_(v_flat.detach().abs().sum(dim=(0, 1)).to(dtype=torch.float64, device="cpu"))
                vo_counts[idx] += v_flat.shape[0] * v_flat.shape[1]
            handles.append(block.attn.register_forward_pre_hook(_attn_pre_hook))
    model.eval()
    calib_batches = 0
    total_tokens = 0
    with torch.inference_mode():
        for x in _iter_calibration_batches(
            args.train_files,
            args.train_seq_len,
            args.ptq_calibration_tokens,
            args.ptq_calibration_batch_tokens,
            device,
        ):
    ...
    if calib_batches == 0:
        raise ValueError(
            f"PTQ calibration requested but PTQ_CALIBRATION_TOKENS={args.ptq_calibration_tokens} "
            f"is smaller than TRAIN_SEQ_LEN={args.train_seq_len}"
        )
    mlp_scales = [None] * num_layers
    for layer_idx in range(num_layers):
        if mlp_abs_sums[layer_idx] is not None and mlp_counts[layer_idx] > 0 and args.ptq_mlp_alpha > 0:
            mlp_scales[layer_idx] = _activation_to_scale(
                mlp_abs_sums[layer_idx],
                mlp_counts[layer_idx],
                args.ptq_mlp_alpha,
                args.ptq_scale_limit,
            )
    vo_scales = [None] * num_layers
    for layer_idx in sorted(vo_eligible):
        if vo_abs_sums[layer_idx] is not None and vo_counts[layer_idx] > 0 and args.ptq_vo_alpha > 0:
            vo_scales[layer_idx] = _activation_to_scale(
                vo_abs_sums[layer_idx],
                vo_counts[layer_idx],
                args.ptq_vo_alpha,
                args.ptq_scale_limit,
            )
    log0(
        "act_aware_ptq:calibrated "
        f"tokens:{total_tokens} batches:{calib_batches} "
        f"mlp_layers:{sum(scale is not None for scale in mlp_scales)} "
        f"vo_layers:{sum(scale is not None for scale in vo_scales)}"
    )
    return mlp_scales, vo_scales


def apply_activation_aware_ptq(
    state_dict: dict[str, Tensor],
    args: Hyperparameters,
    mlp_scales: list[Tensor | None],
    vo_scales: list[Tensor | None],
) -> dict[str, Tensor]:
    ...
    for layer_idx, scale in enumerate(mlp_scales):
        if scale is None:
            continue
        fc_key = f"blocks.{layer_idx}.mlp.fc.weight"
        proj_key = f"blocks.{layer_idx}.mlp.proj.weight"
        out[fc_key] = out[fc_key] * scale[:, None]
        out[proj_key] = out[proj_key] / scale.square()[None, :]
    for layer_idx, scale in enumerate(vo_scales):
        if scale is None:
            continue
        v_key = f"blocks.{layer_idx}.attn.c_v.weight"
        out_key = f"blocks.{layer_idx}.attn.proj.weight"
        expanded = (
            scale.view(args.num_kv_heads, head_dim)[:, None, :]
            .expand(args.num_kv_heads, q_per_kv, head_dim)
            .reshape(-1)
        )
        out[v_key] = out[v_key] * scale[:, None]
        out[out_key] = out[out_key] / expanded[None, :]
    return out
...
def main() -> None:
    ...
    if master_process:
        ...
        if args.act_aware_ptq:
            t_ptq = time.perf_counter()
            mlp_scales, vo_scales = collect_activation_aware_ptq_scales(args, base_model, device, log0)
            quant_source_sd = apply_activation_aware_ptq(unbanked_sd, args, mlp_scales, vo_scales)
            log0(
                "act_aware_ptq:enabled "
                f"mlp_alpha:{args.ptq_mlp_alpha:.2f} vo_alpha:{args.ptq_vo_alpha:.2f} "
                f"scale_limit:{args.ptq_scale_limit:.2f} "
                f"elapsed:{time.perf_counter() - t_ptq:.1f}s"
            )
