...
class Hyperparameters:
    ...
    mtp_enable_frac = float(os.environ.get("MTP_ENABLE_FRAC", 0.2))
...
class GPT(nn.Module):
    ...
    @torch.no_grad()
    def bootstrap_mtp_heads_from_main(self) -> None:
        if self.mtp_num_heads <= 0:
            return
        main_weight = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
        if main_weight is None:
            raise RuntimeError("bootstrap requires either tied embeddings or lm_head")
        for head in self.mtp_heads:
            head.weight.copy_(main_weight.detach())
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_enabled and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
...
def main() -> None:
    ...
    base_model.mtp_enabled = args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0 and args.mtp_enable_frac <= 0.0
    ...
    log0(
        ...
        f"mtp_enable_frac:{args.mtp_enable_frac} mtp_enabled:{int(base_model.mtp_enabled)} mtp_params:{mtp_params}"
    )
    ...
    while True:
        ...
        if (
            not base_model.mtp_enabled
            and args.mtp_num_heads > 0
            and args.mtp_loss_weight > 0.0
            and progress >= args.mtp_enable_frac
        ):
            # torch.compile specializes the boolean branch, so flip once and recompile.
            base_model.bootstrap_mtp_heads_from_main()
            base_model.mtp_enabled = True
            compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
            model = compiled_model
            log0(f"mtp:enabled step:{step} progress:{progress:.4f} bootstrap:main_head")
        zero_grad_all()
