...
class CastedLinear(nn.Linear):
    def forward(self, x: Tensor, qat_active: bool = False) -> Tensor:
        ...
        # Expose late QAT as a compiled forward input so torch.compile retraces
        # once when QAT activates instead of constant-folding a global flag.
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, qat_active: bool = False) -> Tensor:
        x = F.leaky_relu(self.fc(x, qat_active=qat_active), negative_slope=self.negative_slope)
        return self.proj(x.square(), qat_active=qat_active)
class Block(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, v_embed: Tensor | None = None, qat_active: bool = False) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x_in) * self.ln_scale_factor, v_embed=v_embed, qat_active=qat_active)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, qat_active: bool = False) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, v_embed=ve, qat_active=qat_active)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, v_embed=ve, qat_active=qat_active)
        x = self.final_norm(x)
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, qat_active=qat_active)
