...
class Hyperparameters:
    ...
    awq_batch_seqs = int(os.environ.get("AWQ_BATCH_SEQS", 16))
...
class ActivationStatsCollector:
    def __init__(self) -> None:
        self._sum_sq: dict[str, Tensor] = {}
        self._counts: dict[str, int] = {}

    def record(self, name: str, x: Tensor) -> None:
        self.record_many((name,), x)

    def record_many(self, names: tuple[str, ...] | list[str], x: Tensor) -> None:
        if x.numel() == 0:
            return
        flat = x.detach().float().reshape(-1, x.shape[-1])
        sum_sq = flat.square().sum(dim=0).to(dtype=torch.float64).cpu()
        count = int(flat.shape[0])
        for name in names:
            prev = self._sum_sq.get(name)
            self._sum_sq[name] = sum_sq.clone() if prev is None else prev + sum_sq
            self._counts[name] = self._counts.get(name, 0) + count

    def finalize(self) -> dict[str, Tensor]:
        out: dict[str, Tensor] = {}
        for name, sum_sq in self._sum_sq.items():
            count = max(self._counts.get(name, 0), 1)
            out[name] = (sum_sq / count).sqrt().to(dtype=torch.float32)
        return out

def set_awq_collector(module: nn.Module, collector: ActivationStatsCollector | None) -> None:
    for submodule in module.modules():
        setattr(submodule, "_awq_collector", collector)
...
def collect_awq_activation_stats(
    ...
    if not args.awq_enabled or args.awq_calibration_tokens <= 0:
        return {}
    seq_len = args.train_seq_len
    total_seqs = max(args.awq_calibration_tokens // seq_len, 1)
    batch_seqs = max(args.awq_batch_seqs, 1)
    collector = ActivationStatsCollector()
    stream = TokenStream(args.train_files)
    was_training = model.training
    model.eval()
    set_awq_collector(model, collector)
    try:
        with torch.inference_mode():
            for seq_start in range(0, total_seqs, batch_seqs):
                bsz = min(batch_seqs, total_seqs - seq_start)
                toks = stream.take(bsz * seq_len).to(dtype=torch.int64).reshape(bsz, seq_len)
                _ = model.forward_logits(toks.to(device, non_blocking=True))
    finally:
        set_awq_collector(model, None)
        model.train(was_training)
    stats = collector.finalize()
    log0(
        f"awq_calibration:tokens:{total_seqs * seq_len} batch_seqs:{batch_seqs} "
        f"recorded_tensors:{len(stats)}"
    )
    return stats
...
INT8_CLIP_CANDIDATES = (0.9995, INT8_CLIP_Q, 0.9999, 0.99999, 1.0)
...
def keep_float_tensor(name: str, t: Tensor, passthrough_orig_dtypes: dict[str, str]) -> Tensor:
    ...
    return t
def activation_weighted_error(t32: Tensor, recon: Tensor, input_rms: Tensor | None) -> float:
    if (
        input_rms is None
        or t32.ndim != 2
        or input_rms.ndim != 1
        or input_rms.numel() != t32.shape[1]
    ):
        return (t32 - recon).pow(2).mean().item()
    weights = input_rms.to(device=t32.device, dtype=torch.float32)
    weights = (weights / weights.mean().clamp_min(1e-6)).square()
    return ((t32 - recon).pow(2) * weights[None, :]).mean().item()
def quantize_float_tensor(t: Tensor, input_rms: Tensor | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float("inf")
        for pct in INT8_CLIP_CANDIDATES:
            clip_abs = (
            ...
            err = activation_weighted_error(t32, recon, input_rms)
            if err < best_err:
                best_q = q.contiguous()
                best_s = scale.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()
                best_err = err
        if best_q is None or best_s is None:
            raise RuntimeError("int8 quantization failed to select a clip percentile")
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        collector = getattr(self, "_awq_collector", None)
        layer_idx = getattr(self, "layer_idx", -1)
        if collector is not None and layer_idx >= 0:
            collector.record_many(
                (
                    f"blocks.{layer_idx}.attn.c_q.weight",
                    f"blocks.{layer_idx}.attn.c_k.weight",
                    f"blocks.{layer_idx}.attn.c_v.weight",
                ),
                x,
            )
        ...
        if collector is not None and layer_idx >= 0:
            collector.record(f"blocks.{layer_idx}.attn.proj.weight", y)
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        if self.proj is not None:
            ...
            if collector is not None:
                collector.record("bigram.proj.weight", h)
            h = self.proj(h)
...
class ValueEmbedding(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        if self.proj is not None:
            collector = getattr(self, "_awq_collector", None)
            if collector is not None:
                collector.record("ve_shared.proj.weight", h)
            h = self.proj(h)
        return h * self.scale.to(dtype=h.dtype)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        ...
        layer_idx = getattr(self, "layer_idx", -1)
        if collector is not None and layer_idx >= 0:
            collector.record(f"blocks.{layer_idx}.mlp.fc.weight", x)
        x = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=0.5)
        x_sq = x.square()
        if collector is not None and layer_idx >= 0:
            collector.record(f"blocks.{layer_idx}.mlp.proj.weight", x_sq)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        collector = getattr(self, "_awq_collector", None)
        if collector is not None:
            collector.record("tok_emb.weight" if self.tie_embeddings else "lm_head.weight", x)
...
def quantize_int6_per_row(
    ...
    if t32.ndim == 2:
        ...
        if best_q is None or best_s is None:
            raise RuntimeError("int6 quantization failed to select a clip percentile")
        return best_q, best_s
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, input_rms=input_rms)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
            q, s = quantize_float_tensor(t, input_rms=input_rms)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    return result, meta
...
def main() -> None:
    ...
    if master_process and args.awq_enabled:
        torch.cuda.synchronize()
        t_awq = time.perf_counter()
        awq_stats = collect_awq_activation_stats(args, base_model, device, log0)
        torch.cuda.synchronize()
        log0(f"awq_calibration_time:{1000.0 * (time.perf_counter() - t_awq):.0f}ms")
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        activation_rms=awq_stats if master_process else None,
    )
