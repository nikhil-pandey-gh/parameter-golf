...
def attention_forward(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    if flash_attn_3_func is not None and q.is_cuda and k.is_cuda and v.is_cuda:
        return flash_attn_3_func(q, k, v, causal=causal)
    if not ATTN_FALLBACK_ENABLED:
        raise RuntimeError("flash_attn_interface is required for full runs; use SMOKE_TEST=1 for CPU fallback")
    ...
    def __init__(
        ...
        if rope_dims > 0:
            ...
            if rope_dims > head_dim:
                raise ValueError(f"ROPE_DIMS={rope_dims} must not exceed head_dim={head_dim}")
            if rope_dims % 2 != 0:
                raise ValueError(f"ROPE_DIMS={rope_dims} must be even")
...
def eval_val_sliding_ttt(
    ...
    stride: int, batch_seqs: int = 32, log0=print, eval_seq_len: int | None = None,
) -> tuple[float, float]:
...
def build_optimizer_groups(base_model, args: Hyperparameters):
    matrix_params = [
        base_model.qo_bank,
        base_model.kv_bank,
    ...
    return matrix_params, scalar_params, tok_params, token_lr


def run_smoke_test(args: Hyperparameters) -> None:
    global ATTN_FALLBACK_ENABLED
    ATTN_FALLBACK_ENABLED = True
    ...
    matrix_params, scalar_params, tok_params, _ = build_optimizer_groups(model, args)
    optimizer_param_ids = {id(p) for p in matrix_params + scalar_params}
    optimizer_param_ids.update(id(p) for group in tok_params for p in group["params"])
    missing_mtp = [name for name, p in model.mtp_heads.named_parameters() if id(p) not in optimizer_param_ids]
    if missing_mtp:
        raise RuntimeError(f"smoke_test found MTP heads missing from optimizer groups: {missing_mtp}")
    batch_size = max(2, min(args.train_batch_tokens // max(args.train_seq_len, 1), 4))
    input_ids = torch.randint(args.vocab_size, (batch_size, args.train_seq_len), device=device)
    target_ids = torch.randint(args.vocab_size, (batch_size, args.train_seq_len), device=device)
    ...
    excluded_mtp = sum(int(t.numel()) for k, t in model.state_dict().items() if "mtp_heads" in k)
    if args.mtp_num_heads > 0 and excluded_mtp == 0:
        raise RuntimeError("smoke_test expected MTP parameters to be present and excluded from export")
    eval_model = GPT(
        vocab_size=args.vocab_size,
        num_layers=args.num_layers,
        model_dim=args.model_dim,
    ...
    reloaded_logits = eval_model.forward_logits(input_ids[:1])
    torch.testing.assert_close(logits, reloaded_logits)
    print(
        "smoke_test:ok "
        f"loss:{loss.item():.4f} logits_shape:{tuple(logits.shape)} "
        f"reload_logits_shape:{tuple(reloaded_logits.shape)} "
        f"excluded_mtp_params:{excluded_mtp} exported_tensors:{len(export_sd)}"
    )
...
def main() -> None:
    ...
    if flash_attn_3_func is None:
        raise RuntimeError("flash_attn_interface is required for full candidate runs; use SMOKE_TEST=1 for CPU validation")
    ...
    matrix_params, scalar_params, tok_params, token_lr = build_optimizer_groups(base_model, args)
    ...
    optimizer_param_ids = {id(p) for opt in optimizers for group in opt.param_groups for p in group["params"]}
    if base_model.mtp_num_heads > 0:
        missing_mtp = [name for name, p in base_model.mtp_heads.named_parameters() if id(p) not in optimizer_param_ids]
        if missing_mtp:
            raise RuntimeError(f"MTP heads missing from optimizer groups: {missing_mtp}")
    ...
    if args.ttt_enabled:
        ...
        ttt_loss, ttt_bpb = eval_val_sliding_ttt(
            args, eval_model, rank, world_size, device,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
            stride=args.eval_stride, log0=log0, eval_seq_len=sw_seq_len,
        )
        torch.cuda.synchronize()
