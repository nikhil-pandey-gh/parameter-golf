...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "mtp_aux_scale",
            torch.tensor(1.0 if mtp_num_heads > 0 and mtp_loss_weight > 0.0 else 0.0, dtype=torch.float32),
            persistent=False,
        )
    ...
    def set_mtp_aux_scale(self, scale: float) -> None:
        self.mtp_aux_scale.fill_(float(scale))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                aux_scale = self.mtp_aux_scale.to(dtype=main_loss.dtype)
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_warmdown_scale:{args.mtp_warmdown_scale} mtp_params:{mtp_params}"
    )
    ...
    while True:
        ...
        if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0:
            mtp_aux_scale = 1.0
            if args.mtp_warmdown_scale > 0.0:
                mtp_aux_scale = min(scale / args.mtp_warmdown_scale, 1.0)
        base_model.set_mtp_aux_scale(mtp_aux_scale)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_aux:{base_model.mtp_aux_scale.item():.3f}"
            )
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mlp_negative_slope=args.mlp_negative_slope,
