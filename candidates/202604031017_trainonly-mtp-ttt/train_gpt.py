...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_head_weights = tuple(mtp_loss_decay ** k for k in range(mtp_num_heads))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, use_aux_loss: bool = True) -> Tensor:
        n = self.num_layers
        ...
        if self.training and use_aux_loss and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_weight = self.mtp_head_weights[k]
                mtp_loss_sum = mtp_loss_sum + mtp_weight * F.cross_entropy(
                    mtp_logits.float(),
                    mtp_targets,
                    reduction="mean",
                )
                mtp_weight_sum += mtp_weight
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            loss = base_model(x, y, use_aux_loss=False)
                        loss.backward()
                        if world_size > 1:
                            for p in ttt_params:
...
def main() -> None:
    ...
    # - scalars/control tensors + small non-banked matrices -> Adam
    matrix_params = [
