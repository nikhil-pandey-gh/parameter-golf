...
ALLOW_SDPA_FALLBACK = bool(int(os.environ.get("ALLOW_SDPA_FALLBACK", "0")))
class Hyperparameters:
    ...
    bank_qat_start_scale = float(os.environ.get("BANK_QAT_START_SCALE", 0.25))
    bank_qat_max_mix = float(os.environ.get("BANK_QAT_MAX_MIX", 1.0))
...
def attention_with_fallback(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if q.is_cuda and flash_attn_3_func is None and not ALLOW_SDPA_FALLBACK:
        raise RuntimeError(
            "flash_attn_interface is required on CUDA for this candidate. "
            "Set ALLOW_SDPA_FALLBACK=1 only if you intentionally want the slower SDPA fallback."
        )
    q_t = q.transpose(1, 2)
    k_t = k.transpose(1, 2)
    v_t = v.transpose(1, 2)
...
def project_tensor_per_row_(tensor: Tensor, clip_range: int, mix: float) -> None:
    if mix <= 0.0 or not tensor.is_floating_point():
        return
    t32 = tensor.detach().float()
    if t32.ndim >= 2:
        clip = t32.abs().amax(dim=-1, keepdim=True)
        scale = (clip / clip_range).clamp_min(1.0 / clip_range)
        q = torch.clamp(torch.round(t32 / scale), -clip_range, clip_range)
        recon = (q * scale).to(dtype=tensor.dtype)
    else:
        clip = max(float(t32.abs().max().item()), 1.0 / clip_range)
        scale = clip / clip_range
        recon = (torch.clamp(torch.round(t32 / scale), -clip_range, clip_range) * scale).to(dtype=tensor.dtype)
    tensor.data.lerp_(recon, mix)


def apply_bank_late_qat_(base_model: GPT, mix: float) -> None:
    if mix <= 0.0:
        return
    for bank in (base_model.qo_bank, base_model.kv_bank, base_model.mlp_up_bank, base_model.mlp_down_bank):
        project_tensor_per_row_(bank, clip_range=31, mix=mix)
...
def main() -> None:
    ...
    log0(
        f"bank_qat_enabled:{int(args.bank_qat_enabled)} "
        f"bank_qat_start_scale:{args.bank_qat_start_scale:.3f} "
        f"bank_qat_max_mix:{args.bank_qat_max_mix:.3f}"
    )
    ...
    log0(f"allow_sdpa_fallback:{int(ALLOW_SDPA_FALLBACK)}")
    ...
    while True:
        ...
        bank_qat_mix = 0.0
        if args.bank_qat_enabled and args.bank_qat_start_scale > 0.0 and scale < args.bank_qat_start_scale:
            bank_qat_mix = max(0.0, min(args.bank_qat_max_mix, 1.0 - (scale / args.bank_qat_start_scale)))
        ...
        optimizer_muon.step()
        if bank_qat_mix > 0.0:
            apply_bank_late_qat_(base_model, bank_qat_mix)
            if not bank_qat_started or (args.train_log_every > 0 and step % args.train_log_every == 0):
                log0(f"bank_qat:enabled step:{step} scale:{scale:.4f} mix:{bank_qat_mix:.4f}")
