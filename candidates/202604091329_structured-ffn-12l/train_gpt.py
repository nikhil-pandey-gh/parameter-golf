...
class Hyperparameters:
    ...
    ffn_rank = int(os.environ.get("FFN_RANK", 256))
...
class FactorizedLinear(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, rank: int):
        super().__init__()
        if rank <= 0:
            raise ValueError(f"FFN_RANK must be positive, got {rank}")
        if rank >= min(in_dim, out_dim):
            raise ValueError(
                f"FFN_RANK={rank} must be smaller than min(in_dim={in_dim}, out_dim={out_dim}) for a factorized FFN"
            )
        self.in_proj = CastedLinear(in_dim, rank, bias=False)
        self.out_proj = CastedLinear(rank, out_dim, bias=False)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: float, ffn_rank: int):
        super().__init__()
        hidden = int(mlp_mult * dim)
        self.fc = FactorizedLinear(dim, hidden, rank=ffn_rank)
        self.proj_in = CastedLinear(hidden, ffn_rank, bias=False)
        self.proj = CastedLinear(ffn_rank, dim, bias=False)
        self.proj._zero_init = True
    def forward(self, x: Tensor) -> Tensor:
        x = torch.relu(self.fc(x))
        return self.proj(self.proj_in(x.square()))
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult, ffn_rank)
...
def main() -> None:
    ...
    log0(f"structured_ffn:rank_{args.ffn_rank}")
    ...
    eval_model = GPT(
        ...
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult, ffn_rank=args.ffn_rank,
