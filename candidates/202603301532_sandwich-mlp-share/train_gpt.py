...
def build_mlp_share_indices(num_layers: int, enabled: bool) -> list[int]:
    if num_layers <= 0:
        return []
    if not enabled:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.shared_mlp_indices = build_mlp_share_indices(num_layers, share_mlp)
        self.shared_mlps = nn.ModuleList(
            [MLP(model_dim, mlp_mult, negative_slope=mlp_negative_slope) for _ in range(max(self.shared_mlp_indices, default=-1) + 1)]
        )
        ...
        self.num_unique_mlps = len(self.shared_mlps)
        if rope_dims > 0:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, self.shared_mlps[self.shared_mlp_indices[i]], v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, self.shared_mlps[self.shared_mlp_indices[bi]], v_embed=ve)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    log0(f"mlp_share:{args.share_mlp} unique_mlps:{base_model.num_unique_mlps} mlp_map:{base_model.shared_mlp_indices}")
    ...
    eval_model = GPT(
        ...
        share_mlp=args.share_mlp, mlp_negative_slope=args.mlp_negative_slope,
    ).to(device).bfloat16()
