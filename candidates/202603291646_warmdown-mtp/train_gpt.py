...
class Hyperparameters:
    ...
    # Candidate change: enable a single training-only MTP head by default.
...
def maybe_compile(obj, *, enable: bool):
    ...
    def _build(self):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                tail = p.shape[1:]
                dev = p.device
                work_dtype = torch.bfloat16 if dev.type == "cuda" else torch.float32
                self._bank_meta.append({
                    'p': p,
                    'B': B,
                    'padded_grad': torch.zeros(padded_B, *tail, device=dev, dtype=work_dtype),
                    'shard': torch.zeros(shard_B, *tail, device=dev, dtype=work_dtype),
                    'shard_mom': torch.zeros(shard_B, *tail, device=dev, dtype=work_dtype),
                    'full_update': torch.zeros(padded_B, *tail, device=dev, dtype=work_dtype),
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                if sharded and self._rs_futures[i] is not None:
                    ...
                    g = p.grad.to(dtype=torch.bfloat16 if p.is_cuda else torch.float32)
                    state = self.state[p]
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = maybe_compile(base_model.forward_logits, enable=args.enable_compile and device.type == "cuda")
    with torch.inference_mode():
...
def main() -> None:
    ...
    if device.type == "cuda":
        ...
        log0("Running without CUDA; using SDPA fallback and unfused optimizers.", console=False)
    log0("=" * 100, console=False)
    ...
    compiled_model = maybe_compile(base_model, enable=args.enable_compile and device.type == "cuda")
    model = compiled_model
    use_fused_optim = device.type == "cuda"
    ...
    log0(
        ...
        else "attention_backend:scaled_dot_product_attention"
    )
    ...
    if device.type == "cuda":
        ...
        log0("peak memory allocated: CPU run (CUDA memory stats unavailable)")
    ...
    compiled_eval = maybe_compile(eval_model, enable=args.enable_compile and device.type == "cuda")
    device_synchronize(device)
