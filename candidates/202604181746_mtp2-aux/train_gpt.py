...
class Hyperparameters:
    ...
    cpu_smoke = bool(int(os.environ.get("CPU_SMOKE", "0")))
...
def run_cpu_smoke(args: Hyperparameters) -> None:
    ...
    input_ids = torch.randint(args.vocab_size, (smoke_batch_size, smoke_seq_len), device=device)
    target_ids = torch.randint(args.vocab_size, (smoke_batch_size, smoke_seq_len), device=device)
    model.train()
    loss = model(input_ids, target_ids)
    loss.backward()
    full_state_dict = model.state_dict()
    export_sd = {k: v for k, v in full_state_dict.items() if not is_training_only_param(k)}
    ...
    _rebank_state_dict(deq_unbanked, args.num_layers, sd_cpu)
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if is_training_only_param(k))
    print(
        "cpu_smoke:ok "
        f"loss={loss.item():.4f} "
        f"seq_len={smoke_seq_len} "
...
def main() -> None:
    ...
    use_flash_attn_kernel = flash_attn_3_func is not None
    enable_cudnn_sdp(False)
    enable_flash_sdp(use_flash_attn_kernel)
    enable_mem_efficient_sdp(False)
    enable_math_sdp(not use_flash_attn_kernel)
    logfile = None
    if master_process:
    ...
    log0(
        f"sdp_backends:cudnn=False flash={use_flash_attn_kernel} "
        f"mem_efficient=False math={not use_flash_attn_kernel}"
    )
