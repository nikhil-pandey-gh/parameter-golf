...
class Hyperparameters:
    ...
    mtp_disable_scale = float(os.environ.get("MTP_DISABLE_SCALE", 0.2))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + self.mtp_runtime_weight.to(dtype=main_loss.dtype) * (
...
def eval_val_sliding_ttt(
    ...
    saved_mtp_num_heads = getattr(base_model, "mtp_num_heads", 0)
    saved_mtp_weight = None
    if hasattr(base_model, "mtp_runtime_weight"):
        saved_mtp_weight = base_model.mtp_runtime_weight.detach().clone()
        base_model.mtp_runtime_weight.zero_()
    ...
    for p in base_model.parameters():
        p.requires_grad_(True)
    base_model.mtp_num_heads = saved_mtp_num_heads
    if saved_mtp_weight is not None:
        base_model.mtp_runtime_weight.copy_(saved_mtp_weight)
    base_model.eval()
...
def main() -> None:
    ...
    head_params = list(base_model.mtp_heads.parameters())
    ...
    log0(
        ...
        f"mtp_disable_scale:{args.mtp_disable_scale} mtp_params:{mtp_params}"
    )
    ...
    while True:
        ...
        if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0:
            mtp_weight = args.mtp_loss_weight if scale > args.mtp_disable_scale else 0.0
            base_model.mtp_runtime_weight.fill_(mtp_weight)
            if mtp_weight == 0.0 and not mtp_disabled_logged:
                base_model.mtp_num_heads = 0
                compiled_model_mtp_off = torch.compile(base_model, dynamic=False, fullgraph=True)
                model = compiled_model_mtp_off
                log0(f"mtp:disabled_for_warmdown step:{step} scale:{scale:.4f} compiled_mtp_off:1")
