...
class Hyperparameters:
    ...
    mtp_decay_scale = float(os.environ.get("MTP_DECAY_SCALE", 0.15))
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, torch.zeros((), device=device, dtype=torch.float32)).detach()
...
def mtp_loss_scale_for_step(args: Hyperparameters, lr_scale: float) -> float:
    if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
        return 0.0
    if args.mtp_decay_scale <= 0.0:
        return 1.0
    return min(max(lr_scale / args.mtp_decay_scale, 0.0), 1.0)
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads:
            # Warm-start auxiliary heads from the main token projection so they
            # provide non-trivial trunk gradients immediately.
            if self.tie_embeddings:
                source_weight = self.tok_emb.weight.data
                for head in self.mtp_heads:
                    head.weight.data.copy_(source_weight)
            else:
                for head in self.mtp_heads:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_loss_scale: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            mtp_loss_count = 0
            mtp_weight = x.new_tensor(self.mtp_loss_weight, dtype=x.dtype)
            if mtp_loss_scale is not None:
                mtp_weight = mtp_weight * mtp_loss_scale.to(device=x.device, dtype=x.dtype)
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_decay_scale:{args.mtp_decay_scale} mtp_params:{mtp_params}"
    )
    ...
    log0(
        ...
        f"mem_efficient=False math={flash_attn_3_func is None}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, torch.zeros((), device=device, dtype=torch.float32))
    ...
    while True:
        ...
        mtp_scale = torch.tensor(mtp_loss_scale_for_step(args, scale), device=device, dtype=torch.float32)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
