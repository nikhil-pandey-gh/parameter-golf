...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        w = ste_fake_quantize_int6(self.weight, CastedLinear._qat_enabled and self.training).to(x.dtype)
        bias = self.bias.to(x.dtype) if self.bias is not None else None
        return F.linear(x, w, bias)
def ste_fake_quantize_int6(weight: Tensor, enabled: bool) -> Tensor:
    if not enabled or weight.ndim != 2:
        return weight
    with torch.no_grad():
        q, s = quantize_int6_per_row(weight)
        if s.ndim > 0:
            shape = (q.shape[0],) + (1,) * (q.ndim - 1)
            q_weight = (q.float() * s.float().view(*shape)).to(dtype=weight.dtype)
        else:
            q_weight = (q.float() * float(s.item())).to(dtype=weight.dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        qat_enabled = CastedLinear._qat_enabled and self.training
        q_w = ste_fake_quantize_int6(q_w, qat_enabled).to(x.dtype)
        k_w = ste_fake_quantize_int6(k_w, qat_enabled).to(x.dtype)
        v_w = ste_fake_quantize_int6(v_w, qat_enabled).to(x.dtype)
        out_w = ste_fake_quantize_int6(out_w, qat_enabled).to(x.dtype)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        qat_enabled = CastedLinear._qat_enabled and self.training
        up_w = ste_fake_quantize_int6(up_w, qat_enabled).to(x.dtype)
        down_w = ste_fake_quantize_int6(down_w, qat_enabled).to(x.dtype)
...
def main() -> None:
    ...
    # - small non-banked matrices (bigram/VE/MTP projections) -> AdamW
    matrix_params = [
        base_model.qo_bank, base_model.kv_bank,
        base_model.mlp_up_bank, base_model.mlp_down_bank,
    ]
    block_named_params = list(base_model.blocks.named_parameters())
    small_matrix_params: list[Tensor] = []
    scalar_params = [
        p
    ...
    if base_model.bigram is not None:
        ...
        if base_model.bigram.proj is not None:
            small_matrix_params.append(base_model.bigram.proj.weight)
    if base_model.ve_shared is not None:
        tok_params.append({"params": [base_model.ve_shared.embed.weight], "lr": token_lr, "base_lr": token_lr})
        if base_model.ve_shared.proj is not None:
            small_matrix_params.append(base_model.ve_shared.proj.weight)
        scalar_params.append(base_model.ve_shared.scale)
        for s in base_model.ve_layer_scales:
            scalar_params.append(s)
    small_matrix_params.extend([p for p in base_model.mtp_heads.parameters() if p.ndim == 2])
    ...
    optimizer_scalar = torch.optim.AdamW(
        [
            ...
            *(
                [{"params": small_matrix_params, "lr": args.scalar_lr, "base_lr": args.scalar_lr}]
    ...
    replicated_params.extend(small_matrix_params)
    ...
    log0(f"late_qat_threshold:{args.late_qat_threshold:.3f} bank_qat:attn+mlp late_qat_mode:eager_on_activation")
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            model = base_model
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} bank_weights:1 mode:eager")
        zero_grad_all()
