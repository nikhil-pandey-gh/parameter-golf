...
class Hyperparameters:
    ...
    rope_tail_dims = int(os.environ.get("ROPE_TAIL_DIMS", max(8, rope_dims // 2)))
    rope_tail_layers = int(os.environ.get("ROPE_TAIL_LAYERS", xsa_last_n))
...
def build_rope_dims_by_layer(
    num_layers: int,
    head_dim: int,
    base_rope_dims: int,
    tail_rope_dims: int,
    tail_rope_layers: int,
) -> list[int]:
    def normalize(name: str, value: int) -> int:
        if value < 0:
            raise ValueError(f"{name} must be non-negative, got {value}")
        effective = head_dim if value == 0 else value
        if effective > head_dim:
            raise ValueError(f"{name}={value} exceeds head_dim={head_dim}")
        if effective % 2 != 0:
            raise ValueError(f"{name} must be even, got {value}")
        return effective

    base_dims = normalize("ROPE_DIMS", base_rope_dims)
    dims = [base_dims] * num_layers
    if tail_rope_layers <= 0:
        return dims
    tail_dims = normalize("ROPE_TAIL_DIMS", tail_rope_dims)
    tail_start = max(0, num_layers - tail_rope_layers)
    for i in range(tail_start, num_layers):
        dims[i] = tail_dims
    return dims
...
class GPT(nn.Module):
    def __init__(
        ...
        xsa_last_n: int = 0,
        rope_dims: int = 0,
        rope_tail_dims: int = 0,
        rope_tail_layers: int = 0,
        ln_scale: bool = False,
        dtg: bool = False,
        ...
        self.rope_dims_by_layer = build_rope_dims_by_layer(
            num_layers,
            head_dim,
            rope_dims,
            rope_tail_dims,
        ...
        for block, layer_rope_dims in zip(self.blocks, self.rope_dims_by_layer):
            block.attn.rope_dims = layer_rope_dims
            block.attn.rotary = Rotary(head_dim, base=rope_base, train_seq_len=1024, rope_dims=layer_rope_dims)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        rope_tail_dims=args.rope_tail_dims,
        rope_tail_layers=args.rope_tail_layers,
    ...
    log0(f"RoPE:dims_by_layer {[b.attn.rope_dims for b in base_model.blocks]}")
    ...
    eval_model = GPT(
        ...
        rope_dims=args.rope_dims, rope_tail_dims=args.rope_tail_dims,
        rope_tail_layers=args.rope_tail_layers, ln_scale=args.ln_scale, dtg=args.dtg_enabled,
