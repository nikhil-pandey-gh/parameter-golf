...
def build_training_wrapper(base_model: nn.Module, distributed: bool, local_rank: int) -> tuple[nn.Module, nn.Module]:
    # Rebuild the compiled wrapper when late QAT turns on so torch.compile sees the new branch.
...
def main() -> None:
    ...
    compiled_model, model = build_training_wrapper(base_model, distributed, local_rank)
    ...
    log0(f"flash_attn3_available:{flash_attn_3_func is not None}")
    ...
    while True:
        ...
        enable_late_qat = args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled
        if distributed:
        ...
        if enable_late_qat and not CastedLinear._qat_enabled:
