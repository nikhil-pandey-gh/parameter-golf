...
class Hyperparameters:
    ...
    shared_tail_layers = int(os.environ.get("SHARED_TAIL_LAYERS", 4))
    shared_tail_cores = int(os.environ.get("SHARED_TAIL_CORES", 2))
...
def build_layer_to_core(
    num_layers: int,
    shared_tail_layers: int,
    shared_tail_cores: int,
) -> tuple[list[int], int]:
    if shared_tail_layers < 0 or shared_tail_cores < 0:
        raise ValueError(
            f"SHARED_TAIL_LAYERS and SHARED_TAIL_CORES must be non-negative, "
            f"got {shared_tail_layers} and {shared_tail_cores}"
        )
    if shared_tail_layers == 0:
        if shared_tail_cores != 0:
            raise ValueError("SHARED_TAIL_CORES must be 0 when SHARED_TAIL_LAYERS is 0")
        return list(range(num_layers)), num_layers
    if shared_tail_layers > num_layers:
        raise ValueError(
            f"SHARED_TAIL_LAYERS={shared_tail_layers} exceeds NUM_LAYERS={num_layers}"
        )
    if shared_tail_cores == 0 or shared_tail_cores > shared_tail_layers:
        raise ValueError(
            f"SHARED_TAIL_CORES must be in [1, SHARED_TAIL_LAYERS], "
            f"got {shared_tail_cores} for SHARED_TAIL_LAYERS={shared_tail_layers}"
        )
    unique_prefix = num_layers - shared_tail_layers
    shared_core_start = unique_prefix
    layer_to_core = list(range(unique_prefix))
    for layer_offset in range(shared_tail_layers):
        layer_to_core.append(shared_core_start + (layer_offset % shared_tail_cores))
    return layer_to_core, unique_prefix + shared_tail_cores


def validate_shared_core_xsa_alignment(
    layer_to_core: list[int],
    num_layers: int,
    xsa_last_n: int,
) -> None:
    xsa_start = max(0, num_layers - xsa_last_n)
    core_to_xsa: dict[int, bool] = {}
    for layer_idx, core_idx in enumerate(layer_to_core):
        uses_xsa = layer_idx >= xsa_start
        if core_idx in core_to_xsa and core_to_xsa[core_idx] != uses_xsa:
            raise ValueError(
                f"Shared core {core_idx} mixes XSA and non-XSA layers; "
                f"layer_to_core={layer_to_core}, XSA_LAST_N={xsa_last_n}"
            )
        core_to_xsa[core_idx] = uses_xsa
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        shared_tail_layers: int = 0,
        shared_tail_cores: int = 0,
    ):
        super().__init__()
        ...
        self.layer_to_core, self.num_layer_cores = build_layer_to_core(
            num_layers,
            shared_tail_layers,
            shared_tail_cores,
        )
        validate_shared_core_xsa_alignment(self.layer_to_core, num_layers, xsa_last_n)
        self.shared_tail_layers = shared_tail_layers
        self.shared_tail_cores = shared_tail_cores
        self.qo_bank = nn.Parameter(torch.empty(2 * self.num_layer_cores, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.num_layer_cores, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(self.num_layer_cores, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(self.num_layer_cores, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        n = self.num_layers
        num_layer_cores = self.num_layer_cores
        proj_scale = 1.0 / math.sqrt(2 * n)
        # Init banks: orthogonal, with proj layers scaled down and out/down zero-init
        for i in range(num_layer_cores):
            nn.init.orthogonal_(self.qo_bank.data[i], gain=1.0)        # Q
            nn.init.zeros_(self.qo_bank.data[num_layer_cores + i])      # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[num_layer_cores + i], gain=1.0)  # V
            ...
            self.qo_bank.data[num_layer_cores + i].mul_(proj_scale)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            core_idx = self.layer_to_core[i]
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[core_idx], self.kv_bank[core_idx], self.kv_bank[num_layer_cores + core_idx],
                self.qo_bank[num_layer_cores + core_idx], self.mlp_up_bank[core_idx], self.mlp_down_bank[core_idx],
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            core_idx = self.layer_to_core[bi]
            if skips:
...
def _unbank_state_dict(sd: dict[str, Tensor], num_layer_cores: int) -> dict[str, Tensor]:
    """Convert 3D bank tensors into individual 2D tensors keyed by shared core index."""
    out: dict[str, Tensor] = {}
    n = num_layer_cores
    for name, tensor in sd.items():
        if name == "qo_bank":
            for i in range(n):
                out[f"cores.{i}.attn.c_q.weight"] = tensor[i]
                out[f"cores.{i}.attn.proj.weight"] = tensor[n + i]
        elif name == "kv_bank":
            for i in range(n):
                out[f"cores.{i}.attn.c_k.weight"] = tensor[i]
                out[f"cores.{i}.attn.c_v.weight"] = tensor[n + i]
        elif name == "mlp_up_bank":
            for i in range(n):
                out[f"cores.{i}.mlp.fc.weight"] = tensor[i]
        elif name == "mlp_down_bank":
            for i in range(n):
                out[f"cores.{i}.mlp.proj.weight"] = tensor[i]
        else:
            out[name] = tensor
    return out
...
def _rebank_state_dict(
    ...
    """Convert individual 2D tensors keyed by shared core index back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    n = num_layer_cores
    qo_slices = [None] * (2 * n)
    kv_slices = [None] * (2 * n)
    ...
    consumed = set()
    for i in range(n):
        qk = f"cores.{i}.attn.c_q.weight"
        if qk in sd:
            qo_slices[i] = sd[qk]
            consumed.add(qk)
        ok = f"cores.{i}.attn.proj.weight"
        if ok in sd:
            qo_slices[n + i] = sd[ok]
            consumed.add(ok)
        kk = f"cores.{i}.attn.c_k.weight"
        if kk in sd:
            kv_slices[i] = sd[kk]
            consumed.add(kk)
        vk = f"cores.{i}.attn.c_v.weight"
        if vk in sd:
            kv_slices[n + i] = sd[vk]
            consumed.add(vk)
        fk = f"cores.{i}.mlp.fc.weight"
        if fk in sd:
            up_slices[i] = sd[fk]
            consumed.add(fk)
        dk = f"cores.{i}.mlp.proj.weight"
        if dk in sd:
            down_slices[i] = sd[dk]
            consumed.add(dk)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_tail_layers=args.shared_tail_layers,
        shared_tail_cores=args.shared_tail_cores,
    ).to(device).bfloat16()
    ...
    log0(
        f"shared_tail:layers={args.shared_tail_layers} cores={args.shared_tail_cores} "
        f"unique_layer_cores:{base_model.num_layer_cores} layer_to_core:{base_model.layer_to_core}"
    )
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, base_model.num_layer_cores)
    ...
    quant_arch = {
        ...
        "shared_tail_layers": args.shared_tail_layers,
        "shared_tail_cores": args.shared_tail_cores,
        "layer_to_core": list(base_model.layer_to_core),
    }
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta, "arch": quant_arch}, quant_buf)
    ...
    quant_arch = quant_state.get("arch", {})
    artifact_num_layers = int(quant_arch.get("num_layers", args.num_layers))
    if artifact_num_layers != args.num_layers:
        raise ValueError(
            f"Quantized artifact expects NUM_LAYERS={artifact_num_layers}, got {args.num_layers}"
        )
    artifact_xsa_last_n = int(quant_arch.get("xsa_last_n", args.xsa_last_n))
    artifact_shared_tail_layers = int(quant_arch.get("shared_tail_layers", args.shared_tail_layers))
    artifact_shared_tail_cores = int(quant_arch.get("shared_tail_cores", args.shared_tail_cores))
    artifact_layer_to_core, artifact_num_layer_cores = build_layer_to_core(
        artifact_num_layers,
        artifact_shared_tail_layers,
        artifact_shared_tail_cores,
    )
    saved_layer_to_core = quant_arch.get("layer_to_core")
    if saved_layer_to_core is not None and list(saved_layer_to_core) != artifact_layer_to_core:
        raise ValueError(
            f"Quantized artifact layer_to_core mismatch: saved={saved_layer_to_core} "
            f"rebuilt={artifact_layer_to_core}"
        )
    eval_model = GPT(
        ...
        xsa_last_n=artifact_xsa_last_n,
        rope_dims=args.rope_dims, ln_scale=args.ln_scale, dtg=args.dtg_enabled,
        ve_enabled=args.ve_enabled, ve_dim=args.ve_dim, ve_layers=args.ve_layers,
        gated_attention=args.gated_attention, value_residual=args.value_residual,
        shared_tail_layers=artifact_shared_tail_layers,
        shared_tail_cores=artifact_shared_tail_cores,
    ).to(device).bfloat16()
    ...
    eval_sd_template = {k: v.detach().cpu() for k, v in eval_model.state_dict().items() if "mtp_heads" not in k}
    eval_unbanked_template = _unbank_state_dict(eval_sd_template, artifact_num_layer_cores)
    deq_unbanked = dequantize_mixed_int6(quant_state["w"], quant_state["m"], eval_unbanked_template)
    # Re-bank the dequantized tensors using the architecture carried in the artifact.
    deq_state = _rebank_state_dict(deq_unbanked, artifact_num_layer_cores, eval_sd_template)
