...
class Hyperparameters:
    ...
    embed_residual_rank = int(os.environ.get("EMBED_RESIDUAL_RANK", 32))
...
def compute_low_rank_residual(t: Tensor, recon: Tensor, rank: int) -> tuple[Tensor, Tensor] | None:
    if t.ndim != 2 or rank <= 0:
        return None
    max_rank = min(rank, t.shape[0], t.shape[1])
    if max_rank <= 0:
        return None
    error = t.float() - recon.float()
    if error.abs().max().item() == 0.0:
        return None
    u, s, vh = torch.linalg.svd(error, full_matrices=False)
    left = (u[:, :max_rank] * s[:max_rank]).to(torch.float16).contiguous()
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        if name == "tok_emb.weight" and t.ndim == 2 and embed_residual_rank > 0:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            recon = dequantize_quantized_tensor(q, s)
            residual = compute_low_rank_residual(t, recon, embed_residual_rank)
            if residual is None:
                meta[name] = {"type": "int8"}
            else:
                left, right = residual
                result[name + ".resid_l"] = left
                result[name + ".resid_r"] = right
                meta[name] = {"type": "int8_residual", "rank": left.shape[1]}
            continue
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        t = dequantize_quantized_tensor(q, s)
        if isinstance(info, dict) and info.get("type") == "int8_residual":
            t = t + result[name + ".resid_l"].float() @ result[name + ".resid_r"].float()
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu,
        {"mlp", "attn"},
        embed_residual_rank=args.embed_residual_rank,
    )
