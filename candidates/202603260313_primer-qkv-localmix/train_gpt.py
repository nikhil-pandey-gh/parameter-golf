...
class CausalDepthwiseConv1d(nn.Module):
    def __init__(self, channels: int, kernel_size: int):
        super().__init__()
        if kernel_size < 2:
            raise ValueError(f"QKV_CONV_KERNEL must be >= 2 when enabled, got {kernel_size}")
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.q_local = CausalDepthwiseConv1d(dim, qkv_conv_kernel) if qkv_conv_enabled else None
        self.k_local = CausalDepthwiseConv1d(kv_dim, qkv_conv_kernel) if qkv_conv_enabled else None
        self.v_local = CausalDepthwiseConv1d(kv_dim, qkv_conv_kernel) if qkv_conv_enabled else None
...
def main() -> None:
    ...
    for module in base_model.modules():
        if isinstance(module, (CastedLinear, CausalDepthwiseConv1d)):
            module.float()
    ...
    scalar_params.extend(extra_params)
    ...
    log0(f"qkv_conv:enabled:{int(args.qkv_conv_enabled)} kernel:{args.qkv_conv_kernel}")
    ...
    for m in eval_model.modules():
        if isinstance(m, (CastedLinear, CausalDepthwiseConv1d)):
            m.float()
