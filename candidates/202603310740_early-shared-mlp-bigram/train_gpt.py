...
def build_layer_share_map(num_layers: int, spec: str) -> tuple[list[int], tuple[tuple[int, ...], ...]]:
    layer_to_group: dict[int, int] = {}
    groups: list[tuple[int, ...]] = []
    seen: set[int] = set()
    if spec.strip():
        for raw_group in spec.split(";"):
            members = tuple(int(part.strip()) for part in raw_group.split(",") if part.strip())
            if not members:
                continue
            if len(set(members)) != len(members):
                raise ValueError(f"Duplicate layer in SHARED_MLP_GROUPS group: {raw_group!r}")
            for layer_idx in members:
                if not 0 <= layer_idx < num_layers:
                    raise ValueError(
                        f"Layer index {layer_idx} is out of range for NUM_LAYERS={num_layers} in SHARED_MLP_GROUPS"
                    )
    ...
    for layer_idx in range(num_layers):
        if layer_idx not in layer_to_group:
            group_id = len(groups)
            groups.append((layer_idx,))
            layer_to_group[layer_idx] = group_id

    return [layer_to_group[layer_idx] for layer_idx in range(num_layers)], tuple(groups)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, negative_slope: float):
        ...
        if negative_slope < 0.0:
            raise ValueError(f"MLP_NEGATIVE_SLOPE must be non-negative, got {negative_slope}")
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mlp_share_map, self.mlp_share_groups = build_layer_share_map(num_layers, shared_mlp_groups)
        self.mlps = nn.ModuleList(
            [MLP(model_dim, mlp_mult, mlp_negative_slope) for _ in self.mlp_share_groups]
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, self.mlps[self.mlp_share_map[i]], v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, self.mlps[self.mlp_share_map[bi]], v_embed=ve)
        x = self.final_norm(x)
...
def _classify_param(name: str) -> str:
    if "tok_emb" in name or "lm_head" in name:
        return "embed"
    if ".mlp." in name or name.startswith("mlps.") or ".mlps." in name:
        return "mlp"
...
def main() -> None:
    ...
    shared_groups = [group for group in base_model.mlp_share_groups if len(group) > 1]
    log0(f"mlp_negative_slope:{args.mlp_negative_slope}")
    log0(f"shared_mlp_groups:{shared_groups if shared_groups else 'none'} unique_mlps:{len(base_model.mlps)}")
