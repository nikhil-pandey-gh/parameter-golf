"""
Candidate: annealed bank-QAT on top of the LeakyReLU² + legal TTT + Parallel Muon stack.

Hypothesis: the current best stack leaves a meaningful int6 quantization gap on the
banked core weights. An annealed fake-quant blend on those bank tensors should make
the trunk more export-friendly without paying any artifact bytes at evaluation time.
"""
...
class Hyperparameters:
    ...
    bank_qat_start_scale = float(os.environ.get("BANK_QAT_START_SCALE", 0.20))
...
def fake_quantize_weight_ste(t: Tensor, bits: int, blend: Tensor) -> Tensor:
    if bits < 2:
        raise ValueError(f"bank_qat_bits must be >= 2, got {bits}")
    if t.ndim < 2:
        return t
    ...
    scale = (row_max / qmax).clamp_min(1.0 / qmax)
    q_rows = torch.clamp(torch.round(rows / scale), qmin, qmax) * scale
    q = q_rows.reshape_as(t32)
    ste = t32 + (q - t32).detach()
    return torch.lerp(t32, ste, blend.to(device=t.device, dtype=t32.dtype)).to(dtype=t.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bank_qat_bits = bank_qat_bits
        self.register_buffer("bank_qat_blend", torch.zeros((), dtype=torch.float32), persistent=False)
    ...
    def set_bank_qat_blend(self, blend: float) -> None:
        with torch.no_grad():
            self.bank_qat_blend.fill_(float(min(max(blend, 0.0), 1.0)))
    def _maybe_qat_banks(self) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        if not self.training or not self.bank_qat_enabled:
            return self.qo_bank, self.kv_bank, self.mlp_up_bank, self.mlp_down_bank
        blend = self.bank_qat_blend
        return (
            fake_quantize_weight_ste(self.qo_bank, self.bank_qat_bits, blend),
            fake_quantize_weight_ste(self.kv_bank, self.bank_qat_bits, blend),
            fake_quantize_weight_ste(self.mlp_up_bank, self.bank_qat_bits, blend),
            fake_quantize_weight_ste(self.mlp_down_bank, self.bank_qat_bits, blend),
        )
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        qo_bank, kv_bank, mlp_up_bank, mlp_down_bank = self._maybe_qat_banks()
...
def main() -> None:
    ...
    log0(
        f"bank_qat_enabled:{args.bank_qat_enabled} bank_qat_start_scale:{args.bank_qat_start_scale:.3f} "
        f"bank_qat_bits:{args.bank_qat_bits}"
    )
    ...
    while True:
        ...
        bank_qat_blend = 0.0
        if args.bank_qat_enabled and args.bank_qat_start_scale > 0:
            bank_qat_blend = max(min((args.bank_qat_start_scale - scale) / args.bank_qat_start_scale, 1.0), 0.0)
        base_model.set_bank_qat_blend(bank_qat_blend)
        if bank_qat_blend > 0.0 and not bank_qat_started:
            bank_qat_started = True
            log0(f"bank_qat:enabled step:{step} scale:{scale:.4f} blend:{bank_qat_blend:.3f}")
        ...
        if should_log_train:
            log0(
                ...
                f"bank_qat_blend:{float(base_model.bank_qat_blend.item()):.3f}"
            )
    ...
    base_model.set_bank_qat_blend(0.0)
