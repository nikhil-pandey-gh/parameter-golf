...
class Hyperparameters:
    ...
    quarot_enabled = bool(int(os.environ.get("QUAROT_ENABLED", "1")))
    quarot_chunk = int(os.environ.get("QUAROT_CHUNK", 512))
...
def fwht_lastdim(x: Tensor) -> Tensor:
    n = x.size(-1)
    if n <= 0 or n & (n - 1):
        raise ValueError(f"FWHT requires a power-of-two last dim, got {n}")
    ...
    while h < n:
        ...
        h *= 2
    return (y.reshape_as(x) * (n ** -0.5)).contiguous()
def hadamard_rotate_chunks(x: Tensor, chunk: int) -> Tensor:
    if chunk <= 0:
        raise ValueError(f"chunk must be positive, got {chunk}")
    if x.size(-1) % chunk != 0:
        raise ValueError(f"Last dim {x.size(-1)} is not divisible by chunk {chunk}")
    if x.size(-1) == chunk:
        return fwht_lastdim(x)
    orig_shape = x.shape
    y = x.reshape(*orig_shape[:-1], orig_shape[-1] // chunk, chunk)
    y = fwht_lastdim(y)
    return y.reshape(orig_shape).contiguous()
QUAROT_TARGET_SUFFIXES = (
    "attn.c_q.weight",
    "attn.c_k.weight",
    "attn.c_v.weight",
    "attn.proj.weight",
...
def should_quarot_weight(name: str, tensor: Tensor, chunk: int) -> bool:
    return (
        tensor.ndim == 2
        and any(name.endswith(suffix) for suffix in QUAROT_TARGET_SUFFIXES)
        and tensor.shape[-1] >= chunk
        and tensor.shape[-1] % chunk == 0
    )
def apply_quarot_to_state_dict(state_dict: dict[str, Tensor], chunk: int) -> tuple[dict[str, Tensor], list[str]]:
    rotated: dict[str, Tensor] = {}
    rotated_names: list[str] = []
    for name, tensor in state_dict.items():
        if should_quarot_weight(name, tensor, chunk):
            rotated[name] = hadamard_rotate_chunks(tensor.float(), chunk).to(dtype=tensor.dtype)
            rotated_names.append(name)
        else:
            rotated[name] = tensor
    return rotated, rotated_names
def set_quarot_state_dict_metadata(state_dict: dict[str, Tensor], enabled: bool, chunk: int) -> dict[str, Tensor]:
    updated = dict(state_dict)
    for name, tensor in state_dict.items():
        if name.endswith("quarot_active"):
            updated[name] = torch.tensor(int(enabled), dtype=tensor.dtype)
        elif name.endswith("quarot_chunk_buffer"):
            updated[name] = torch.tensor(chunk, dtype=tensor.dtype)
    return updated
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.register_buffer("quarot_active", torch.tensor(int(quarot_enabled), dtype=torch.uint8))
        self.register_buffer("quarot_chunk_buffer", torch.tensor(quarot_chunk, dtype=torch.int32))
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        quarot_enabled = bool(self.quarot_active.item())
        quarot_chunk = int(self.quarot_chunk_buffer.item())
        x_proj = hadamard_rotate_chunks(x, quarot_chunk) if quarot_enabled and x.size(-1) % quarot_chunk == 0 else x
        q = F.linear(x_proj, q_w.to(x.dtype)).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k = F.linear(x_proj, k_w.to(x.dtype)).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        v = F.linear(x_proj, v_w.to(x.dtype))
        ...
        if quarot_enabled and y.size(-1) % quarot_chunk == 0:
            y = hadamard_rotate_chunks(y, quarot_chunk)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # No CastedLinear -- weights come from banks
        self.register_buffer("quarot_active", torch.tensor(0, dtype=torch.uint8))
        self.register_buffer("quarot_chunk_buffer", torch.tensor(dim, dtype=torch.int32))
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        quarot_enabled = bool(self.quarot_active.item())
        quarot_chunk = int(self.quarot_chunk_buffer.item())
        x_up = hadamard_rotate_chunks(x, quarot_chunk) if quarot_enabled and x.size(-1) % quarot_chunk == 0 else x
        x_act = F.leaky_relu(F.linear(x_up, up_w.to(x.dtype)), negative_slope=0.5)
        x_sq = x_act.square()
        if quarot_enabled and x_sq.size(-1) % quarot_chunk == 0:
            x_sq = hadamard_rotate_chunks(x_sq, quarot_chunk)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        quarot_enabled=quarot_enabled, quarot_chunk=quarot_chunk,
                                        gated_attention=gated_attention, value_residual=value_residual)
        self.mlp = MLP(dim, mlp_mult)
        self.mlp.quarot_active.fill_(int(quarot_enabled))
        self.mlp.quarot_chunk_buffer.fill_(int(quarot_chunk))
...
def eval_val_sliding(
    ...
    compiled_logits = (
        ...
        if bool(getattr(base_model, "quarot_active").item())
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        quarot_chunk=args.quarot_chunk,
    ...
    log0(f"quarot_enabled:{args.quarot_enabled} quarot_chunk:{args.quarot_chunk}")
    ...
    sd_cpu = set_quarot_state_dict_metadata({k: v.detach().cpu() for k, v in export_sd.items()}, False, args.quarot_chunk)
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_layers)
    if args.quarot_enabled:
        unbanked_sd, quarot_names = apply_quarot_to_state_dict(unbanked_sd, args.quarot_chunk)
        unbanked_sd = set_quarot_state_dict_metadata(unbanked_sd, True, args.quarot_chunk)
        log0(f"quarot_rotated_tensors:{len(quarot_names)}")
        raw_export_sd = _rebank_state_dict(unbanked_sd, args.num_layers, sd_cpu)
    else:
        raw_export_sd = sd_cpu
    if master_process:
    ...
    eval_model = GPT(
        ...
        quarot_enabled=args.quarot_enabled,
        quarot_chunk=args.quarot_chunk,
    ...
    compiled_eval = eval_model if bool(eval_model.quarot_active.item()) else torch.compile(eval_model, dynamic=False, fullgraph=True)
    torch.cuda.synchronize()
