...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_init_from_main_head and self.mtp_heads:
            init_src = self.tok_emb.weight if self.tie_embeddings else None
            if init_src is None and self.lm_head is not None:
                init_src = self.lm_head.weight
            if init_src is not None:
                with torch.no_grad():
                    for head in self.mtp_heads:
                        head.weight.copy_(init_src)
...
def main() -> None:
    ...
    # - bigram / VE projections -> AdamW with scalars
    # - MTP heads -> dedicated Adam output-head optimizer
    matrix_params = [
    ...
    log0(
        ...
        f"mtp_head_lr:{args.mtp_head_lr} mtp_init_from_main_head:{int(args.mtp_init_from_main_head)} "
