...
class Hyperparameters:
    ...
    mtp_offramp_scale = float(os.environ.get("MTP_OFFRAMP_SCALE", 0.2))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = self.mtp_loss_scale.to(dtype=main_loss.dtype) * self.mtp_loss_weight
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_offramp_scale:{args.mtp_offramp_scale} mtp_params:{mtp_params}"
    )
    ...
    t0 = time.perf_counter()
    step = 0
    mtp_aux_scale = 1.0 if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0 else 0.0
    while True:
        ...
        if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0:
            if args.mtp_offramp_scale <= 0.0:
                mtp_aux_scale = 1.0
            else:
                mtp_aux_scale = min(scale / args.mtp_offramp_scale, 1.0)
            base_model.mtp_loss_scale.fill_(mtp_aux_scale)
