...
class Hyperparameters:
    ...
    mtp_fade_end_scale = float(os.environ.get("MTP_FADE_END_SCALE", 0.05))
...
def eval_val(
    ...
    zero_mtp_weight = torch.zeros((), device=device, dtype=torch.float32)
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, zero_mtp_weight).detach()
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_weight: Tensor) -> Tensor:
        n = self.num_layers
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            loss = base_model(x, y, zero_mtp_weight)
                        loss.backward()
                        if world_size > 1:
                            for p in ttt_params:
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_warmup_steps:{args.mtp_warmup_steps} "
        f"mtp_fade_start_scale:{args.mtp_fade_start_scale} mtp_fade_end_scale:{args.mtp_fade_end_scale} "
    ...
    def mtp_weight_for_step(step: int, lr_scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        warmup = 1.0 if args.mtp_warmup_steps <= 0 else min((step + 1) / args.mtp_warmup_steps, 1.0)
        if args.mtp_fade_start_scale <= args.mtp_fade_end_scale:
            fade = 1.0 if lr_scale >= args.mtp_fade_start_scale else 0.0
        elif lr_scale >= args.mtp_fade_start_scale:
            fade = 1.0
        elif lr_scale <= args.mtp_fade_end_scale:
            fade = 0.0
        else:
            fade = (lr_scale - args.mtp_fade_end_scale) / (args.mtp_fade_start_scale - args.mtp_fade_end_scale)
        return args.mtp_loss_weight * warmup * fade
    if args.warmup_steps > 0:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        warmup_mtp_weight = torch.tensor(
            args.mtp_loss_weight if args.mtp_num_heads > 0 else 0.0,
            device=device,
            dtype=torch.float32,
        )
    ...
    while True:
        ...
        mtp_weight_value = mtp_weight_for_step(step, scale)
        mtp_weight = torch.tensor(mtp_weight_value, device=device, dtype=torch.float32)
