...
class Hyperparameters:
    ...
    rotate_block_size = int(os.environ.get("ROTATE_BLOCK_SIZE", 128))
    rotate_search_seeds = tuple(
        int(x) for x in os.environ.get("ROTATE_SEARCH_SEEDS", "0,1,2,3").split(",") if x.strip()
    )
...
_ROTATION_SIGN_CACHE: dict[tuple[int, int, int], Tensor] = {}


def _get_hadamard(block_size: int) -> Tensor:
    cached = _HADAMARD_CACHE.get(block_size)
    if cached is not None:
        return cached
    if block_size <= 0 or block_size & (block_size - 1):
        raise ValueError(f"Hadamard block size must be a positive power of 2, got {block_size}")
    h = torch.ones(1, 1, dtype=torch.float32)
    while h.size(0) < block_size:
        h = torch.cat(
            (
                torch.cat((h, h), dim=1),
                torch.cat((h, -h), dim=1),
    ...
    h /= math.sqrt(block_size)
    _HADAMARD_CACHE[block_size] = h.contiguous()
    return _HADAMARD_CACHE[block_size]


def _get_rotation_signs(width: int, block_size: int, seed: int) -> Tensor:
    key = (width, block_size, seed)
    cached = _ROTATION_SIGN_CACHE.get(key)
    if cached is not None:
        return cached
    if width % block_size != 0:
        raise ValueError(f"Width {width} must be divisible by block size {block_size}")
    gen = torch.Generator(device="cpu")
    gen.manual_seed(seed)
    signs = torch.randint(0, 2, (width,), generator=gen, dtype=torch.int8)
    signs = signs.to(dtype=torch.float32).mul_(2.0).sub_(1.0)
    _ROTATION_SIGN_CACHE[key] = signs.contiguous()
    return _ROTATION_SIGN_CACHE[key]


def _rotate_int6_columns(t: Tensor, block_size: int, seed: int) -> Tensor:
    if t.ndim != 2 or block_size <= 0 or t.size(1) % block_size != 0:
        return t
    hadamard = _get_hadamard(block_size)
    signs = _get_rotation_signs(t.size(1), block_size, seed).view(1, -1, block_size)
    chunks = t.reshape(t.size(0), -1, block_size)
    chunks = chunks * signs
    return torch.matmul(chunks, hadamard).reshape_as(t)


def _unrotate_int6_columns(t: Tensor, block_size: int, seed: int) -> Tensor:
    if t.ndim != 2 or block_size <= 0 or t.size(1) % block_size != 0:
        return t
    hadamard = _get_hadamard(block_size)
    signs = _get_rotation_signs(t.size(1), block_size, seed).view(1, -1, block_size)
    chunks = t.reshape(t.size(0), -1, block_size)
    chunks = torch.matmul(chunks, hadamard)
    chunks = chunks * signs
    return chunks.reshape_as(t)


def _quantize_int6_candidate(t32: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, Tensor, float]:
    t32 = t32.float()
    if t32.ndim == 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            if err < best_err:
                best_q, best_s, best_recon, best_err = q, s, recon, err
        if best_recon is None:
            raise RuntimeError("Failed to build an int6 quantization candidate")
    ...
    err = (t32 - recon).pow(2).mean().item()
    return q, scale, recon, err


def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = 31,
    rotate_block_size: int = 0,
    rotate_search_seeds: tuple[int, ...] = (),
) -> tuple[Tensor, Tensor, dict[str, int] | None]:
    t32 = t.float()
    best_q, best_s, _, best_err = _quantize_int6_candidate(t32, clip_range)
    best_rotation: dict[str, int] | None = None
    if t32.ndim != 2 or rotate_block_size <= 0 or t32.size(1) % rotate_block_size != 0:
        return best_q, best_s, best_rotation
    for seed in rotate_search_seeds:
        rotated = _rotate_int6_columns(t32, rotate_block_size, seed)
        q, s, recon_rot, _ = _quantize_int6_candidate(rotated, clip_range)
        recon = _unrotate_int6_columns(recon_rot, rotate_block_size, seed)
        err = (t32 - recon).pow(2).mean().item()
        if err < best_err:
            best_q, best_s, best_err = q, s, err
            best_rotation = {"kind": "hadamard", "block_size": rotate_block_size, "seed": seed}
    return best_q, best_s, best_rotation
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, rotation = quantize_int6_per_row(
                t,
                rotate_block_size=rotate_block_size,
                rotate_search_seeds=rotate_search_seeds,
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        rotation = info.get("rotation") if isinstance(info, dict) else None
        if isinstance(rotation, dict) and rotation.get("kind") == "hadamard":
            t = _unrotate_int6_columns(t, int(rotation["block_size"]), int(rotation["seed"]))
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        rotate_block_size=args.rotate_block_size,
        rotate_search_seeds=args.rotate_search_seeds,
    )
    rotated_tensors = [
        name
        for name, info in quant_meta.items()
        if isinstance(info, dict) and isinstance(info.get("rotation"), dict)
    ]
    log0(
        f"int6_rotation:block_size:{args.rotate_block_size} "
        f"seeds:{','.join(str(seed) for seed in args.rotate_search_seeds) or 'none'} "
        f"rotated_tensors:{len(rotated_tensors)}"
    )
