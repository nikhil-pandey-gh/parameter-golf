...
class Hyperparameters:
    ...
    run_id = os.environ.get("RUN_ID", "mtp_value_residual")
...
def main() -> None:
    ...
    use_flash_backend = flash_attn_3_func is not None and torch.cuda.get_device_capability(device)[0] >= 9
    enable_cudnn_sdp(False)
    enable_flash_sdp(use_flash_backend)
    enable_mem_efficient_sdp(not use_flash_backend)
    enable_math_sdp(not use_flash_backend)
    logfile = None
    if master_process:
    ...
    log0(
        f"sdp_backends:cudnn=False flash={use_flash_backend} "
        f"mem_efficient={not use_flash_backend} math={not use_flash_backend}"
    )
