...
class Hyperparameters:
    ...
    mtp_init_from_tied = bool(int(os.environ.get("MTP_INIT_FROM_TIED", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_init_from_tied = mtp_init_from_tied
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_num_heads > 0 and self.mtp_init_from_tied and self.tie_embeddings:
            with torch.no_grad():
                for head in self.mtp_heads:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_init_from_tied=args.mtp_init_from_tied,
    ...
    # - bigram proj, VE proj, untied output heads, and training-only MTP heads -> Adam
    matrix_params = [
    ...
    head_like_params: list[Tensor] = [head.weight for head in base_model.mtp_heads]
    if base_model.lm_head is not None:
        head_like_params.append(base_model.lm_head.weight)
    optimizer_head = None
    if head_like_params:
        optimizer_head = torch.optim.Adam(
            [{"params": head_like_params, "lr": args.head_lr, "base_lr": args.head_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            fused=True,
        )
        replicated_params.extend(head_like_params)
    ...
    log0(
        ...
        f"mtp_init_from_tied:{args.mtp_init_from_tied} mtp_params:{mtp_params}"
    )
    ...
    log0(
        ...
        f"head_lr:{args.head_lr if head_like_params else 0.0} "
