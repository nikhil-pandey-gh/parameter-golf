...
def is_train_only_tensor_name(name: str) -> bool:
    return name.startswith("mtp_heads.")


def filter_export_state_dict(state_dict: dict[str, Tensor]) -> dict[str, Tensor]:
    return {name: tensor for name, tensor in state_dict.items() if not is_train_only_tensor_name(name)}
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    print(
        ...
        f"logits_shape:{tuple(logits.shape)} mtp_num_heads:{args.mtp_num_heads}"
    )
...
def main() -> None:
    ...
    log0(
        ...
        f"flash={bool(flash_attn_3_func is not None)} mem_efficient=False math=False"
    )
    ...
    if args.warmup_steps > 0:
        ...
        train_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
    export_names = set(filter_export_state_dict(base_model.state_dict()).keys())
    train_only_params = sum(int(p.numel()) for p in base_model.mtp_heads.parameters())
    if train_only_params > 0:
        log0(f"train_only_mtp_params:{train_only_params} ema_and_export_excluded:True")
    ...
    while True:
        ...
        if args.swa_enabled and scale < 0.2 and step % args.swa_every == 0:
            ...
            if swa_state is None:
                swa_state = {name: t.detach().cpu().clone() for name, t in export_state.items()}
                swa_count = 1
                log0(f"swa:start step:{step}")
            else:
                for name, t in export_state.items():
    ...
    avg_state.update({name: t.to(dtype=current_state[name].dtype) for name, t in ema_state.items()})
    ...
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if is_train_only_tensor_name(k))
    if excluded_mtp > 0:
