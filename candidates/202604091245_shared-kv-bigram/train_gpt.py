...
class Hyperparameters:
    ...
    shared_kv_layers = os.environ.get("SHARED_KV_LAYERS", "7,8,9,10")
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,kv_scales,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.kv_scales = nn.Parameter(torch.ones(2, kv_dim, dtype=torch.float32))
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        kv_scales = self.kv_scales.to(dtype=x.dtype)
        k = (self.c_k(x) * kv_scales[0][None, None, :]).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
        v = self.c_v(x) * kv_scales[1][None, None, :]
...
class GPT(nn.Module):
    def __init__(
        ...
        self.shared_kv_layer_indices = sorted({int(x) for x in shared_kv_layers.split(",") if x.strip()})
        for layer_idx in self.shared_kv_layer_indices:
            if layer_idx < 0 or layer_idx >= num_layers:
                raise ValueError(f"shared_kv_layers contains out-of-range layer {layer_idx} for NUM_LAYERS={num_layers}")
        if len(self.shared_kv_layer_indices) > 1:
            shared_attn = self.blocks[self.shared_kv_layer_indices[0]].attn
            for layer_idx in self.shared_kv_layer_indices[1:]:
                self.blocks[layer_idx].attn.c_k = shared_attn.c_k
                self.blocks[layer_idx].attn.c_v = shared_attn.c_v
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    seen_aliases: dict[tuple[object, ...], str] = {}
    for name, tensor in state_dict.items():
        detached = tensor.detach()
        alias_key = (
            int(detached.data_ptr()),
        ...
        if alias_key in seen_aliases:
            meta[name] = {"type": "alias", "target": seen_aliases[alias_key]}
            continue
        seen_aliases[alias_key] = name
        t = detached.cpu().contiguous()
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_kv_layers=args.shared_kv_layers,
    ).to(device).bfloat16()
    ...
    log0(f"shared_kv_layers:{base_model.shared_kv_layer_indices}")
