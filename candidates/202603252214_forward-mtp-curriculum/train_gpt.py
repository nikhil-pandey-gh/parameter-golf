...
class Hyperparameters:
    ...
    mtp_ramp_frac = float(os.environ.get("MTP_RAMP_FRAC", 0.35))
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_scale: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and mtp_scale is not None:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = mtp_scale.to(device=main_loss.device, dtype=main_loss.dtype) * self.mtp_loss_weight
...
def main() -> None:
    ...
    log0(
        f"mtp_curriculum:enabled={args.mtp_curriculum} start_frac:{args.mtp_start_frac:.2f} "
        f"ramp_frac:{args.mtp_ramp_frac:.2f}"
    )
    ...
    def progress_frac(step: int, elapsed_ms: float) -> float:
        ...
        if max_wallclock_ms is None or max_wallclock_ms <= 0.0:
            return min(max(iter_frac, 0.0), 1.0)
        time_frac = elapsed_ms / max_wallclock_ms
        return min(max(iter_frac, time_frac, 0.0), 1.0)
    def mtp_scale_for_progress(progress: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if not args.mtp_curriculum:
            return 1.0
        start = min(max(args.mtp_start_frac, 0.0), 1.0)
        ramp = max(args.mtp_ramp_frac, 0.0)
        if progress <= start:
            return 0.0
        if ramp == 0.0:
            return 1.0
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            warmup_mtp_scale = None
            if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0 and warmup_step + 1 == args.warmup_steps:
                # Compile the auxiliary-head path outside the timed training window.
                warmup_mtp_scale = torch.tensor(1.0, device=device, dtype=torch.float32)
    ...
    mtp_active_logged = args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0
    mtp_full_logged = mtp_active_logged or not args.mtp_curriculum
    ...
    while True:
        ...
        mtp_scale = mtp_scale_for_progress(progress)
        mtp_scale_tensor = torch.tensor(mtp_scale, device=device, dtype=torch.float32) if mtp_scale > 0.0 else None
        if not mtp_active_logged and mtp_scale > 0.0:
            log0(f"mtp_curriculum:active step:{step} progress:{progress:.3f} scale:{mtp_scale:.3f}")
            mtp_active_logged = True
        if not mtp_full_logged and mtp_scale >= 1.0:
            log0(f"mtp_curriculum:full step:{step} progress:{progress:.3f}")
            mtp_full_logged = True
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_scale_tensor) if mtp_scale_tensor is not None else model(x, y)
