...
class Hyperparameters:
    ...
    mirror_tie = bool(int(os.environ.get("MIRROR_TIE", "1")))
    shared_layers = int(os.environ.get("SHARED_LAYERS", "0"))
...
def resolve_num_shared_layers(num_layers: int, mirror_tie: bool, shared_layers: int) -> int:
    expected = (num_layers + 1) // 2 if mirror_tie else num_layers
    if shared_layers not in (0, expected):
        raise ValueError(
            f"SHARED_LAYERS must be 0 or {expected} for NUM_LAYERS={num_layers}, MIRROR_TIE={int(mirror_tie)}; "
            f"got {shared_layers}"
        )
    return expected if shared_layers == 0 else shared_layers
...
class GPT(nn.Module):
    def __init__(
        ...
        # Mirror-tying reuses the heavy projection banks across encoder/decoder counterparts
        # while keeping each logical block's norms, scales, and skip behavior independent.
        self.layer_core_indices = tuple(
            min(i, num_layers - 1 - i) if mirror_tie else i
            for i in range(num_layers)
        )
        self.num_shared_layers = resolve_num_shared_layers(num_layers, mirror_tie, shared_layers)
        ...
        if ve_enabled:
            if ve_layers.strip():
                ...
                self.ve_layer_indices = list(range(max(0, num_layers - 2), num_layers))
        else:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ci = self.layer_core_indices[i]
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[ci], self.kv_bank[ci], self.kv_bank[n + ci],
                self.qo_bank[n + ci], self.mlp_up_bank[ci], self.mlp_down_bank[ci],
                v_embed=ve, v0=v0)
...
def eval_val_sliding_ttt(
    ...
    if args.ttt_freeze_blocks > 0:
        raise ValueError(
            "TTT_FREEZE_BLOCKS>0 is not supported in this banked model because the heavy attention/MLP "
            "weights live in shared bank tensors rather than block-local parameters; use TTT_FREEZE_BLOCKS=0"
        )
...
def _unbank_state_dict(sd: dict[str, Tensor], num_shared_layers: int) -> dict[str, Tensor]:
    """Convert shared 3D bank tensors into individual 2D tensors."""
    out: dict[str, Tensor] = {}
    n = num_shared_layers
...
def _rebank_state_dict(sd: dict[str, Tensor], num_shared_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert individual shared-core tensors back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    n = num_shared_layers
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    num_layers_total = max(
        (int(k.split(".")[1]) for k in state_dict if k.startswith(("blocks.", "shared_blocks."))),
        default=0,
    ) + 1
...
def main() -> None:
    ...
    log0(
        f"mirror_tie:{args.mirror_tie} shared_layers:{base_model.num_shared_layers} "
        f"layer_core_indices:{list(base_model.layer_core_indices)}"
    )
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, base_model.num_shared_layers)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, base_model.num_shared_layers, sd_cpu)
    eval_model = GPT(
        ...
        mirror_tie=args.mirror_tie, shared_layers=args.shared_layers,
    ).to(device).bfloat16()
