...
class Hyperparameters:
    ...
    mtp_copy_lm_init = bool(int(os.environ.get("MTP_COPY_LM_INIT", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_copy_lm_init = mtp_copy_lm_init
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_copy_lm_init and len(self.mtp_heads) > 0:
            lexical_head = self.tok_emb.weight
            with torch.no_grad():
                for head in self.mtp_heads:
                    head.weight.copy_(lexical_head)
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    return out
def gpt_kwargs_from_args(args: Hyperparameters, export_artifact: bool = False) -> dict[str, object]:
    kwargs: dict[str, object] = dict(
        ...
        mtp_copy_lm_init=args.mtp_copy_lm_init,
...
def main() -> None:
    ...
    train_model_kwargs = gpt_kwargs_from_args(args)
    export_model_kwargs = gpt_kwargs_from_args(args, export_artifact=True)
    base_model = GPT(**train_model_kwargs).to(device).bfloat16()
    ...
    log0(
        ...
        f"mtp_loss_decay:{args.mtp_loss_decay} mtp_copy_lm_init:{args.mtp_copy_lm_init} "
    ...
    loaded_export_model_kwargs = quant_state.get("model_kwargs", export_model_kwargs)
    eval_model = GPT(**loaded_export_model_kwargs).to(device).bfloat16()
