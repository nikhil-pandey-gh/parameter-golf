...
class Hyperparameters:
    ...
    mtp_target_steps = tuple(
        int(x) for x in os.environ.get("MTP_TARGET_STEPS", "2,4").split(",") if x.strip()
    )
    mtp_adapter_rank = int(os.environ.get("MTP_ADAPTER_RANK", 16))
    mtp_tail_tokens = int(os.environ.get("MTP_TAIL_TOKENS", 256))
...
class CausalSelfAttention(nn.Module):
    ...
    def _attend(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        ...
        if self.num_heads != self.num_kv_heads:
            ...
            k_s = k_s.repeat_interleave(group, dim=1)
            v_s = v_s.repeat_interleave(group, dim=1)
        ...
        return y.to(dtype=orig_dtype).transpose(1, 2)
...
class GPT(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        mtp_target_steps: tuple[int, ...] = (),
        mtp_adapter_rank: int = 16,
        mtp_tail_tokens: int = 256,
        ...
        self.mtp_target_steps = tuple(sorted(set(mtp_target_steps)))
        if any(step < 2 for step in self.mtp_target_steps):
            raise ValueError(f"MTP_TARGET_STEPS must be >= 2, got {self.mtp_target_steps}")
        if self.mtp_target_steps and mtp_adapter_rank <= 0:
            raise ValueError(f"MTP_ADAPTER_RANK must be positive when MTP is enabled, got {mtp_adapter_rank}")
        self.mtp_tail_tokens = mtp_tail_tokens
        ...
        self.mtp_heads = nn.ModuleList(
            [MTPAdapter(model_dim, mtp_adapter_rank) for _ in self.mtp_target_steps]
        )
        if xsa_last_n > 0:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and len(self.mtp_heads) > 0 and self.mtp_loss_weight > 0.0:
            _, seqlen, dim = x.shape
            mtp_loss_sum = x.new_zeros(())
            mtp_loss_count = 0
            for step_ahead, mtp_head in zip(self.mtp_target_steps, self.mtp_heads, strict=True):
                shift = step_ahead - 1
                valid_t = seqlen - shift
                if valid_t <= 0:
                    continue
                start_t = max(valid_t - self.mtp_tail_tokens, 0) if self.mtp_tail_tokens > 0 else 0
                if start_t >= valid_t:
                    continue
                mtp_hidden = x[:, start_t:valid_t, :].reshape(-1, dim)
                mtp_targets = target_ids[:, start_t + shift : shift + valid_t].reshape(-1)
                mtp_hidden = mtp_head(mtp_hidden)
                if self.tie_embeddings:
                    mtp_logits_proj = F.linear(mtp_hidden, self.tok_emb.weight)
                else:
                    if self.lm_head is None:
                        raise RuntimeError("lm_head is required when tie_embeddings=False")
                    mtp_logits_proj = self.lm_head(mtp_hidden)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_target_steps=args.mtp_target_steps,
        mtp_adapter_rank=args.mtp_adapter_rank,
        mtp_tail_tokens=args.mtp_tail_tokens,
    ...
    log0(
        f"mtp_target_steps:{list(args.mtp_target_steps)} mtp_adapter_rank:{args.mtp_adapter_rank} "
        f"mtp_tail_tokens:{args.mtp_tail_tokens} mtp_loss_weight:{args.mtp_loss_weight} mtp_params:{mtp_params}"
    )
    ...
    log0(
        ...
        f"flash:{use_flash_sdp} mem_efficient=False math:{not use_flash_sdp}"
    )
    ...
    eval_model = GPT(
        ...
        mtp_target_steps=(), mtp_adapter_rank=0, mtp_tail_tokens=0, mtp_loss_weight=0.0,
