...
class Hyperparameters:
    ...
    mtp_start_step = int(os.environ.get("MTP_START_STEP", 0))
    mtp_ramp_steps = int(os.environ.get("MTP_RAMP_STEPS", 1500))
...
def attention_kernel(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    if qh.size(1) != kh.size(1):
        group_size = qh.size(1) // kh.size(1)
    ...
    return yh.transpose(1, 2).contiguous()
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_active: bool = True) -> Tensor:
        n = self.num_layers
        ...
        if self.training and mtp_active and self.mtp_num_heads > 0:
...
def mtp_curriculum_weight(args: Hyperparameters, step: int) -> float:
    ...
    ramp_progress = min(max(step - args.mtp_start_step, 0) / max(args.mtp_ramp_steps, 1), 1.0)
    return args.mtp_loss_weight * ramp_progress
...
def main() -> None:
    ...
    base_model.mtp_loss_weight_buf.fill_(mtp_curriculum_weight(args, 0))
    ...
    log0(
        f"mtp_schedule:start_step:{args.mtp_start_step} ramp_steps:{args.mtp_ramp_steps} "
        f"backend:{_ATTN_BACKEND}"
    )
    ...
    while True:
        ...
        current_mtp_weight = mtp_curriculum_weight(args, step)
        base_model.mtp_loss_weight_buf.fill_(current_mtp_weight)
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, current_mtp_weight > 0.0)
