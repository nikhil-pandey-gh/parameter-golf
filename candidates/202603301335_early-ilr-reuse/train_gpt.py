...
class Hyperparameters:
    ...
    recurrent_layers = os.environ.get("RECURRENT_LAYERS", "0,1")
    recurrent_steps = int(os.environ.get("RECURRENT_STEPS", 1))
    recurrent_gate_init = float(os.environ.get("RECURRENT_GATE_INIT", -2.0))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,recurrent_attn_scale,recurrent_mlp_scale,recurrent_input_scale,recurrent_skip_scale,recurrent_gate",
    ).split(",")
    if pattern
)
...
class Block(nn.Module):
    def __init__(
        ...
        self.recurrent_input_scale = nn.ParameterList(
            [nn.Parameter(torch.ones(dim, dtype=torch.float32)) for _ in range(recurrent_steps)]
        )
        self.recurrent_skip_scale = nn.ParameterList(
            [nn.Parameter(torch.zeros(dim, dtype=torch.float32)) for _ in range(recurrent_steps)]
        )
        self.recurrent_attn_scale = nn.ParameterList(
            [nn.Parameter(torch.ones(dim, dtype=torch.float32)) for _ in range(recurrent_steps)]
        )
        self.recurrent_mlp_scale = nn.ParameterList(
            [nn.Parameter(torch.ones(dim, dtype=torch.float32)) for _ in range(recurrent_steps)]
        )
        self.recurrent_gate = nn.ParameterList(
            [nn.Parameter(torch.full((dim,), recurrent_gate_init, dtype=torch.float32)) for _ in range(recurrent_steps)]
        )
        if dtg:
    ...
    def _forward_pass(
        ...
        x_in = input_scale.to(dtype=x.dtype)[None, None, :] * x + skip_scale.to(dtype=x.dtype)[None, None, :] * x0
        ...
        x_out = x_out + mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(
            self.mlp_norm(x_out) * self.ln_scale_factor,
            up_w,
            down_w,
        )
        if gate is not None:
            static_gate = torch.sigmoid(gate.to(dtype=x_out.dtype))[None, None, :]
            x_out = x_in + static_gate * (x_out - x_in)
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        x_out, raw_v = self._forward_pass(
            x,
            x0,
            q_w,
            k_w,
        ...
        for input_scale, skip_scale, attn_scale, mlp_scale, gate in zip(
            self.recurrent_input_scale,
            self.recurrent_skip_scale,
            self.recurrent_attn_scale,
            self.recurrent_mlp_scale,
            self.recurrent_gate,
        ):
            x_out, raw_v = self._forward_pass(
                x_out,
...
class GPT(nn.Module):
    def __init__(
        ...
        self.recurrent_layer_indices = sorted(
            {
                int(layer_idx)
                for layer_idx in recurrent_layers.split(",")
                if layer_idx.strip()
            }
        )
        invalid_recurrent_layers = [layer_idx for layer_idx in self.recurrent_layer_indices if layer_idx < 0 or layer_idx >= num_layers]
        if invalid_recurrent_layers:
            raise ValueError(
                f"RECURRENT_LAYERS entries must be in [0, {num_layers - 1}], got {invalid_recurrent_layers}"
            )
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    recurrent_steps=recurrent_steps if i in self.recurrent_layer_indices else 0,
...
def main() -> None:
    ...
    log0(f"recurrent_layers:{base_model.recurrent_layer_indices} recurrent_steps:{args.recurrent_steps}")
    ...
    eval_model = GPT(
        ...
        recurrent_layers=args.recurrent_layers, recurrent_steps=args.recurrent_steps,
        recurrent_gate_init=args.recurrent_gate_init,
    ).to(device).bfloat16()
