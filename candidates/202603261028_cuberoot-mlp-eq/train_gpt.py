...
class Hyperparameters:
    ...
    mlp_equalize_enabled = bool(int(os.environ.get("MLP_EQUALIZE_ENABLED", "1")))
    mlp_equalize_quantile = float(os.environ.get("MLP_EQUALIZE_QUANTILE", 0.999))
    mlp_equalize_max_scale = float(os.environ.get("MLP_EQUALIZE_MAX_SCALE", 4.0))
...
def _channel_abs_stat(t: Tensor, dim: int, quantile: float) -> Tensor:
    t_abs = t.float().abs()
    if quantile >= 1.0:
        return t_abs.amax(dim=dim)
    return torch.quantile(t_abs, quantile, dim=dim)


def apply_square_mlp_channel_equalization(
    state_dict: dict[str, Tensor],
    num_layers: int,
    quantile: float,
    max_scale: float,
) -> tuple[dict[str, Tensor], dict[str, float]]:
    """Rebalance MLP hidden channels before int6 export.

    For phi(x) = leaky_relu(x)^2, phi(s * x) = s^2 * phi(x) for positive s, so
    W_down @ phi(W_up x) == (W_down / s^2) @ phi((s * W_up) x). We use this exact
    symmetry to balance MLP rows/columns into a more quantization-friendly range.
    """
    equalized = dict(state_dict)
    layer_count = 0
    mean_abs_log_scale = 0.0
    max_observed_scale = 1.0
    eps = 1e-6
    ...
    for layer_idx in range(num_layers):
        ...
        if up_name not in equalized or down_name not in equalized:
            continue

        up = equalized[up_name].float().contiguous()
        down = equalized[down_name].float().contiguous()
        if up.ndim != 2 or down.ndim != 2:
            continue

        up_stat = _channel_abs_stat(up, dim=1, quantile=quantile).clamp_min(eps)
        down_stat = _channel_abs_stat(down, dim=0, quantile=quantile).clamp_min(eps)
        scale = torch.pow(down_stat / up_stat, 1.0 / 3.0)
        scale = scale / torch.exp(torch.mean(torch.log(scale)))
        scale = scale.clamp(inv_max_scale, max(max_scale, 1.0))
        scale_sq = scale.square()

        equalized[up_name] = (up * scale[:, None]).to(dtype=state_dict[up_name].dtype)
        equalized[down_name] = (down / scale_sq[None, :]).to(dtype=state_dict[down_name].dtype)

        layer_count += 1
        mean_abs_log_scale += float(scale.log().abs().mean().item())
        max_observed_scale = max(
            max_observed_scale,
            float(scale.max().item()),
            float(scale.reciprocal().max().item()),
        )

    if layer_count:
        mean_abs_log_scale /= layer_count
    return equalized, {
        "layers": float(layer_count),
        "mean_abs_log_scale": mean_abs_log_scale,
        "max_scale": max_observed_scale,
    }
...
def main() -> None:
    ...
    if args.mlp_equalize_enabled:
        quant_source_sd, eq_stats = apply_square_mlp_channel_equalization(
            unbanked_sd,
            num_layers=args.num_layers,
            quantile=args.mlp_equalize_quantile,
            max_scale=args.mlp_equalize_max_scale,
        )
        log0(
            "mlp_equalize "
            f"layers:{int(eq_stats['layers'])} "
            f"mean_abs_log_scale:{eq_stats['mean_abs_log_scale']:.4f} "
            f"max_scale:{eq_stats['max_scale']:.3f} "
            f"quantile:{args.mlp_equalize_quantile:.4f}"
        )
