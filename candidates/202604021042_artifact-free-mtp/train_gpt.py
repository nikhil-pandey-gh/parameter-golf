...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_num_heads > 0 and mtp_horizon_decay <= 0.0:
            raise ValueError(
                f"mtp_horizon_decay must be positive when MTP is enabled, got {mtp_horizon_decay}"
            )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_total_weight += horizon_weight
            if mtp_loss_total_weight > 0.0:
                main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / mtp_loss_total_weight)
        return main_loss
