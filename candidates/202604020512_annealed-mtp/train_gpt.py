...
class Hyperparameters:
    ...
    mtp_decay_threshold = float(os.environ.get("MTP_DECAY_THRESHOLD", 0.25))
...
class GPT(nn.Module):
    ...
    def set_mtp_loss_scale(self, value: float) -> None:
        self.mtp_loss_scale.fill_(max(value, 0.0))
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + self.mtp_loss_scale.to(dtype=main_loss.dtype) * self.mtp_loss_weight * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_decay_threshold:{args.mtp_decay_threshold} mtp_params:{mtp_params}"
    )
    ...
    def mtp_mul(lr_scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_decay_threshold <= 0.0:
            return 1.0
        return min(max(lr_scale / args.mtp_decay_threshold, 0.0), 1.0)
    ...
    base_model.set_mtp_loss_scale(mtp_mul(1.0))
    ...
    while True:
        ...
        base_model.set_mtp_loss_scale(mtp_mul(scale))
        zero_grad_all()
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_scale:{float(base_model.mtp_loss_scale.item()):.3f}"
            )
