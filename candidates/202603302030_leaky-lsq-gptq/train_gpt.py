...
class Hyperparameters:
    ...
    qat_clip_min = float(os.environ.get("QAT_CLIP_MIN", 0.50))
    qat_clip_max = float(os.environ.get("QAT_CLIP_MAX", 1.00))
    qat_mse_init = bool(int(os.environ.get("QAT_MSE_INIT", "1")))
...
class CastedLinear(nn.Linear):
    ...
    _qat_clip_min: float = 0.50
    _qat_clip_max: float = 1.00
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            ...
            row_max = w32.detach().abs().amax(dim=1).clamp_min(1.0 / 31.0)
            clip_mul = torch.exp(self.qat_log_scale).clamp(
                min=CastedLinear._qat_clip_min,
                max=CastedLinear._qat_clip_max,
            )
            scale = (row_max * clip_mul / 31.0).clamp_min(1.0 / 31.0)
            w_q = torch.clamp(round_ste(w32 / scale[:, None]), -31, 31) * scale[:, None]
            w = w_q.to(x.dtype)
...
def quantize_int6_per_row(
    ...
    if t32.ndim == 2:
        ...
        if learned_log_scale is not None:
            row_clip = t32.abs().amax(dim=1) * torch.exp(learned_log_scale.float()).clamp(min=clip_min, max=clip_max)
            s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
            q = torch.clamp(torch.round(t32 / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
            recon = q.float() * s.float()[:, None]
            err = (t32 - recon).pow(2).mean().item()
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            if name.endswith(".weight"):
                learned_name = name.removesuffix(".weight") + ".qat_log_scale"
                learned_log_scale = state_dict.get(learned_name)
...
def init_qat_log_scales_from_mse(
    module: nn.Module,
    clip_range: int = 31,
    clip_candidates: tuple[float, ...] = (0.5, 0.75, 0.875, 1.0),
    clip_min: float = 0.50,
    clip_max: float = 1.00,
) -> None:
    with torch.no_grad():
        for submodule in module.modules():
            if not isinstance(submodule, CastedLinear) or submodule.weight.ndim != 2:
                continue
            w32 = submodule.weight.float()
            row_max = w32.abs().amax(dim=1).clamp_min(1.0 / clip_range)
            best_mul = torch.ones_like(row_max)
            best_err = torch.full_like(row_max, float("inf"))
            for mul in clip_candidates:
                mul = min(max(mul, clip_min), clip_max)
                scale = (row_max * mul / clip_range).clamp_min(1.0 / clip_range)
                q = torch.clamp(torch.round(w32 / scale[:, None]), -clip_range, clip_range)
                recon = q * scale[:, None]
                err = (w32 - recon).pow(2).mean(dim=1)
                better = err < best_err
                best_err = torch.where(better, err, best_err)
                best_mul = torch.where(better, torch.full_like(best_mul, mul), best_mul)
            submodule.qat_log_scale.copy_(best_mul.log())
...
def main() -> None:
    ...
    CastedLinear._qat_clip_min = args.qat_clip_min
    CastedLinear._qat_clip_max = args.qat_clip_max
    base_model = GPT(
    ...
    if args.qat_enabled and args.qat_mse_init:
        init_qat_log_scales_from_mse(
            base_model,
            clip_min=args.qat_clip_min,
            clip_max=args.qat_clip_max,
        )

    def compile_train_model() -> tuple[nn.Module, nn.Module]:
    ...
    compiled_model, model = compile_train_model()
