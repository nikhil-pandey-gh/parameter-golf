...
class Hyperparameters:
    ...
    mtp_start_frac = float(os.environ.get("MTP_START_FRAC", 0.2))
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, False).detach()
...
def causal_self_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if q_t.size(1) != k_t.size(1):
        group_size = q_t.size(1) // k_t.size(1)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_head_decay = mtp_head_decay
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_enabled: bool = False) -> Tensor:
        n = self.num_layers
        ...
        if self.training and mtp_enabled and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + weight * F.cross_entropy(
                    mtp_logits.float(),
                    mtp_targets,
                    reduction="mean",
                )
                mtp_weight_sum += weight
            if mtp_weight_sum > 0.0:
...
def run_cpu_smoke_test(args: Hyperparameters) -> None:
    ...
    model = GPT(
        ...
        mtp_num_heads=max(1, min(args.mtp_num_heads, 2)),
        mtp_loss_weight=max(args.mtp_loss_weight, 0.1),
        mtp_head_decay=args.mtp_head_decay,
    ...
    print(
        f"cpu_smoke_ok loss:{loss.item():.4f} logits_shape:{tuple(logits.shape)} "
        f"flash_fallback:{flash_attn_3_func is None}"
    )
...
def main() -> None:
    ...
    args = Hyperparameters()
    if not 0.0 <= args.mtp_start_frac <= 1.0:
        raise ValueError(f"MTP_START_FRAC must be in [0, 1], got {args.mtp_start_frac}")
    ...
    log0(
        ...
        f"mtp_head_decay:{args.mtp_head_decay} mtp_start_frac:{args.mtp_start_frac} mtp_params:{mtp_params}"
    )
    ...
    log0(
        "sdp_backends:"
        f"cudnn=False flash={flash_attn_3_func is not None} mem_efficient=False math={flash_attn_3_func is None}"
    )
    ...
    mtp_start_step = max(int(args.iterations * args.mtp_start_frac), 0)
    ...
    if args.mtp_num_heads > 0:
        model.train()
        dummy_tokens = args.train_batch_tokens // (world_size * grad_accum_steps)
        dummy_bsz = max(dummy_tokens // args.train_seq_len, 1)
        dummy_len = args.train_seq_len
        dummy_x = torch.zeros((dummy_bsz, dummy_len), dtype=torch.int64, device=device)
        dummy_y = torch.zeros((dummy_bsz, dummy_len), dtype=torch.int64, device=device)
        for mtp_phase in (False, True):
            zero_grad_all()
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                dry_loss = model(dummy_x, dummy_y, mtp_phase)
            dry_loss.backward()
        zero_grad_all()
        torch.cuda.synchronize()
        log0("mtp_curriculum:precompiled ntp_and_mtp graphs")
    ...
    while True:
        ...
        zero_grad_all()
        mtp_enabled = args.mtp_num_heads > 0 and step >= mtp_start_step
        if args.mtp_num_heads > 0 and step == mtp_start_step:
            log0(f"mtp_curriculum:enabled step:{step} frac:{step / max(args.iterations, 1):.3f}")
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_enabled)
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_head_decay=args.mtp_head_decay,
