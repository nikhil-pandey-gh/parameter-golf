...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_num_heads > 0:
            self.mtp_scales = nn.Parameter(torch.ones(mtp_num_heads, model_dim, dtype=torch.float32))
            self.mtp_biases = nn.Parameter(torch.zeros(mtp_num_heads, model_dim, dtype=torch.float32))
        else:
            self.mtp_scales = None
            self.mtp_biases = None
        if xsa_last_n > 0:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_scales is not None and self.mtp_loss_weight > 0.0:
            ...
            for k in range(self.mtp_num_heads):
                ...
                scale = self.mtp_scales[k].to(dtype=mtp_hidden.dtype)[None, None, :]
                bias = self.mtp_biases[k].to(dtype=mtp_hidden.dtype)[None, None, :]
                mtp_hidden = mtp_hidden * scale + bias
                mtp_targets = target_ids[:, k + 1 :].reshape(-1)
                mtp_logits = self._project_logits(mtp_hidden.reshape(-1, dim))
...
def main() -> None:
    ...
    if base_model.mtp_scales is not None:
        scalar_params.append(base_model.mtp_scales)
        scalar_params.append(base_model.mtp_biases)
    ...
    mtp_params = 0 if base_model.mtp_scales is None else base_model.mtp_scales.numel() + base_model.mtp_biases.numel()
