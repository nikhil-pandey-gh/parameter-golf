...
class Hyperparameters:
    ...
    run_id = os.environ.get("RUN_ID", f"{SCRIPT_DIR.name}_{uuid.uuid4().hex[:8]}")
    ...
    mtp_fade_with_warmdown = bool(int(os.environ.get("MTP_FADE_WITH_WARMDOWN", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        self._init_weights()
    @torch.no_grad()
    def set_mtp_loss_scale(self, scale: float) -> None:
        self.mtp_loss_scale.fill_(max(float(scale), 0.0))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_scale = self.mtp_loss_scale.to(dtype=main_loss.dtype)
                main_loss = main_loss + (self.mtp_loss_weight * mtp_scale) * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def main() -> None:
    ...
    if base_model.mtp_num_heads > 0:
        scalar_params.extend([p for p in base_model.mtp_heads.parameters()])
    ...
    log0(
        ...
        f"mtp_fade_with_warmdown:{args.mtp_fade_with_warmdown} mtp_params:{mtp_params}"
    )
    ...
    while True:
        ...
        mtp_scale = scale if args.mtp_fade_with_warmdown else 1.0
        base_model.set_mtp_loss_scale(mtp_scale)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_weight:{args.mtp_loss_weight * mtp_scale:.4f}"
            )
