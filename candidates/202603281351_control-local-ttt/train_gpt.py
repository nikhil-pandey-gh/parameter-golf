...
class Hyperparameters:
    ...
    ttt_mode = os.environ.get("TTT_MODE", "control_local").strip().lower()
    ttt_trainable_patterns = tuple(
        pattern
        for pattern in os.environ.get(
            "TTT_TRAINABLE_PATTERNS",
            "q_gain,attn_scale,mlp_scale,resid_mix,skip_weight,smear.gate,bigram.scale,"
            "ve_layer_scales,ve_shared.scale,bigram.embed.weight,bigram.proj.weight,"
            "ve_shared.embed.weight,ve_shared.proj.weight",
        ).split(",")
        if pattern
    )
    ttt_exclude_patterns = tuple(
        pattern for pattern in os.environ.get("TTT_EXCLUDE_PATTERNS", "").split(",") if pattern
    )
    ttt_log_params = int(os.environ.get("TTT_LOG_PARAMS", 12))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,bigram.scale,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda",
    ).split(",")
    if pattern
)
TTT_LOCAL_MODULE_NAME_PATTERNS = tuple(
    pattern
    for pattern in os.environ.get(
        "TTT_LOCAL_MODULE_NAME_PATTERNS",
        "bigram.embed.weight,bigram.proj.weight,ve_shared.embed.weight,ve_shared.proj.weight",
    ).split(",")
    if pattern
)
...
def _select_ttt_named_params(base_model: nn.Module, args: Hyperparameters) -> list[tuple[str, nn.Parameter]]:
    """BitFit/TENT-style sparse TTT: adapt only control tensors and local modules by default."""
    if args.ttt_mode != "all" and args.ttt_freeze_blocks > 0:
        raise ValueError(
            "TTT_FREEZE_BLOCKS > 0 is only supported with TTT_MODE=all; "
            "sparse TTT modes update top-level control/local modules that affect every block"
        )
    frozen_block_ids = set(range(min(args.ttt_freeze_blocks, len(base_model.blocks))))
    ttt_named_params: list[tuple[str, nn.Parameter]] = []
    for name, p in base_model.named_parameters():
        ...
        if freeze:
            selected = False
        elif args.ttt_mode == "all":
            selected = True
        elif args.ttt_mode == "control":
            selected = _matches_any_pattern(name, CONTROL_TENSOR_NAME_PATTERNS)
        elif args.ttt_mode == "control_local":
            selected = (
                _matches_any_pattern(name, CONTROL_TENSOR_NAME_PATTERNS)
                or _matches_any_pattern(name, TTT_LOCAL_MODULE_NAME_PATTERNS)
            )
        elif args.ttt_mode == "custom":
            selected = _matches_any_pattern(name, args.ttt_trainable_patterns)
        else:
            raise ValueError(
                f"Unsupported TTT_MODE={args.ttt_mode!r}; expected all, control, control_local, or custom"
            )
        if selected and args.ttt_exclude_patterns:
            selected = not _matches_any_pattern(name, args.ttt_exclude_patterns)
        p.requires_grad_(selected)
        if selected:
            ttt_named_params.append((name, p))
    if not ttt_named_params:
        raise ValueError(f"TTT_MODE={args.ttt_mode!r} selected no trainable parameters")
    return ttt_named_params
...
def eval_val_sliding_ttt(
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"freeze_blocks={args.ttt_freeze_blocks} ttt_mode={args.ttt_mode}")

    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)

    ttt_named_params = _select_ttt_named_params(base_model, args)
    ttt_params = [p for _, p in ttt_named_params]
    sample_names = ", ".join(name for name, _ in ttt_named_params[: args.ttt_log_params])
    if len(ttt_named_params) > args.ttt_log_params:
        sample_names += ", ..."
    log0(
        f"ttt_sliding:params selected_tensors={len(ttt_named_params)} "
        f"selected_numel={sum(p.numel() for p in ttt_params)} "
        f"frozen_numel={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)} "
