...
class Hyperparameters:
    ...
    # Keep MTP lightweight enough for the 600s training budget; exported artifacts drop these heads.
...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        if local_tokens < seq_len:
            raise ValueError(
                f"TRAIN_BATCH_TOKENS per rank/micro-step must be at least one sequence: "
                f"local_tokens={local_tokens}, TRAIN_SEQ_LEN={seq_len}"
            )
        if local_tokens % seq_len != 0:
            raise ValueError(
                f"TRAIN_BATCH_TOKENS per rank/micro-step must divide evenly into TRAIN_SEQ_LEN: "
                f"local_tokens={local_tokens}, TRAIN_SEQ_LEN={seq_len}"
            )
...
def attention_func(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    if not hasattr(F, "scaled_dot_product_attention"):
        raise RuntimeError("flash_attn_interface is unavailable and torch SDPA fallback is missing")
    ...
    if q_sdpa.size(1) != k_sdpa.size(1):
        if q_sdpa.size(1) % k_sdpa.size(1) != 0:
            raise ValueError(
                f"num_heads={q_sdpa.size(1)} must be divisible by num_kv_heads={k_sdpa.size(1)}"
            )
    ...
    y = F.scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, dropout_p=0.0, is_causal=causal)
...
def main() -> None:
    ...
    # MTP heads are training-only, but they still need optimizer coverage before export strips them.
