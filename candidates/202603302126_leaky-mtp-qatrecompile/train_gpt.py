...
class Hyperparameters:
    ...
    mtp_anneal_frac = float(os.environ.get("MTP_ANNEAL_FRAC", 0.15))
...
def attend(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    return F.scaled_dot_product_attention(
        ...
        attn_mask=None,
        is_causal=True,
        enable_gqa=(q_sdp.size(1) != k_sdp.size(1)),
    ).transpose(1, 2)
...
def compile_training_model(base_model: GPT) -> nn.Module:
    compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
    return compiled_model


def mtp_aux_weight(base_weight: float, anneal_frac: float, scale: float) -> float:
    if base_weight <= 0.0:
        return 0.0
    if anneal_frac <= 0.0:
        return base_weight
    return base_weight * min(scale / anneal_frac, 1.0)


def set_mtp_aux_weight(model: GPT, weight: float) -> None:
    model.mtp_loss_weight_buf.fill_(float(max(weight, 0.0)))
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    input_ids = torch.randint(0, 128, (2, 32), dtype=torch.int64)
    target_ids = torch.randint(0, 128, (2, 32), dtype=torch.int64)
    loss = smoke_model(input_ids, target_ids)
    if not torch.isfinite(loss):
        raise RuntimeError(f"smoke_test loss is not finite: {loss.item()}")
    loss.backward()
    state_dict = {k: v.detach().cpu() for k, v in smoke_model.state_dict().items() if "mtp_heads" not in k}
    ...
    reload_loss = reload_model(input_ids, target_ids)
    if not torch.isfinite(reload_loss):
        raise RuntimeError(f"smoke_test reload_loss is not finite: {reload_loss.item()}")
    print(f"smoke_test:ok loss:{loss.item():.4f} reload_loss:{reload_loss.item():.4f}")
...
def main() -> None:
    ...
    if flash_attn_3_func is None:
        log0("flash_attn:fallback=scaled_dot_product_attention")
    ...
    compiled_model = compile_training_model(base_model)
    ...
    while True:
        ...
        set_mtp_aux_weight(base_model, mtp_aux_weight(args.mtp_loss_weight, args.mtp_anneal_frac, scale))
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            if args.late_qat_recompile:
                if distributed:
                    dist.barrier()
                compiled_model = compile_training_model(base_model)
                if distributed:
            ...
            log0(
                ...
                f"recompiled:{int(args.late_qat_recompile)}"
            )
        zero_grad_all()
