...
class Hyperparameters:
    ...
    recursive_attn_tail_core = int(os.environ.get("RECURSIVE_ATTN_TAIL_CORE", 2))
    recursive_attn_tail_repeat = int(os.environ.get("RECURSIVE_ATTN_TAIL_REPEAT", 2))
...
def build_recursive_tail_slot_map(num_layers: int, core_layers: int, repeats: int) -> list[int]:
    if core_layers <= 0 or repeats <= 1:
        return list(range(num_layers))
    tail_layers = core_layers * repeats
    if tail_layers > num_layers:
        raise ValueError(
            f"recursive attention tail exceeds model depth: "
            f"core_layers={core_layers} repeats={repeats} num_layers={num_layers}"
        )
    prefix_unique = num_layers - tail_layers
    core_slots = list(range(prefix_unique, prefix_unique + core_layers))
    slot_map = list(range(prefix_unique))
    for _ in range(repeats):
        slot_map.extend(core_slots)
    return slot_map
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        recursive_attn_tail_core: int = 0,
        recursive_attn_tail_repeat: int = 2,
    ):
        super().__init__()
        ...
        self.attn_slot_map = build_recursive_tail_slot_map(
            num_layers, recursive_attn_tail_core, recursive_attn_tail_repeat
        )
        self.num_attn_slots = max(self.attn_slot_map, default=-1) + 1
        self.qo_bank = nn.Parameter(torch.empty(2 * self.num_attn_slots, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.num_attn_slots, kv_dim, model_dim))
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.num_attn_slots):
            nn.init.orthogonal_(self.qo_bank.data[i], gain=1.0)        # Q
            nn.init.zeros_(self.qo_bank.data[self.num_attn_slots + i])  # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[self.num_attn_slots + i], gain=1.0)    # V
        for i in range(n):
        ...
        for i in range(self.num_attn_slots):
            self.qo_bank.data[self.num_attn_slots + i].mul_(proj_scale)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            attn_slot = self.attn_slot_map[i]
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[attn_slot], self.kv_bank[attn_slot], self.kv_bank[self.num_attn_slots + attn_slot],
                self.qo_bank[self.num_attn_slots + attn_slot], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            attn_slot = self.attn_slot_map[bi]
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[attn_slot], self.kv_bank[attn_slot], self.kv_bank[self.num_attn_slots + attn_slot],
                self.qo_bank[self.num_attn_slots + attn_slot], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _unbank_state_dict(sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert bank tensors into individual 2D tensors keyed by unique slot."""
    out: dict[str, Tensor] = {}
    for name, tensor in sd.items():
        if name == "qo_bank":
            num_attn_slots = tensor.shape[0] // 2
            for i in range(num_attn_slots):
                out[f"attn_slots.{i}.attn.c_q.weight"] = tensor[i]
                out[f"attn_slots.{i}.attn.proj.weight"] = tensor[num_attn_slots + i]
        elif name == "kv_bank":
            num_attn_slots = tensor.shape[0] // 2
            for i in range(num_attn_slots):
                out[f"attn_slots.{i}.attn.c_k.weight"] = tensor[i]
                out[f"attn_slots.{i}.attn.c_v.weight"] = tensor[num_attn_slots + i]
        elif name == "mlp_up_bank":
            for i in range(tensor.shape[0]):
                out[f"mlp_slots.{i}.mlp.fc.weight"] = tensor[i]
        elif name == "mlp_down_bank":
            for i in range(tensor.shape[0]):
                out[f"mlp_slots.{i}.mlp.proj.weight"] = tensor[i]
        else:
            out[name] = tensor
    return out

def _rebank_state_dict(sd: dict[str, Tensor], template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert quantized unique-slot tensors back into bank tensors."""
    out: dict[str, Tensor] = {}
    num_attn_slots = template_sd["qo_bank"].shape[0] // 2
    num_mlp_slots = template_sd["mlp_up_bank"].shape[0]
    # Reconstruct banks from individual weight keys
    qo_slices = [None] * (2 * num_attn_slots)
    kv_slices = [None] * (2 * num_attn_slots)
    up_slices = [None] * num_mlp_slots
    down_slices = [None] * num_mlp_slots
    consumed = set()
    for i in range(num_attn_slots):
        qk = f"attn_slots.{i}.attn.c_q.weight"
        if qk in sd:
            qo_slices[i] = sd[qk]
            consumed.add(qk)
        ok = f"attn_slots.{i}.attn.proj.weight"
        if ok in sd:
            qo_slices[num_attn_slots + i] = sd[ok]
            consumed.add(ok)
        kk = f"attn_slots.{i}.attn.c_k.weight"
        if kk in sd:
            kv_slices[i] = sd[kk]
            consumed.add(kk)
        vk = f"attn_slots.{i}.attn.c_v.weight"
        if vk in sd:
            kv_slices[num_attn_slots + i] = sd[vk]
            consumed.add(vk)
    for i in range(num_mlp_slots):
        fk = f"mlp_slots.{i}.mlp.fc.weight"
        if fk in sd:
            up_slices[i] = sd[fk]
            consumed.add(fk)
        dk = f"mlp_slots.{i}.mlp.proj.weight"
        if dk in sd:
            down_slices[i] = sd[dk]
            consumed.add(dk)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        recursive_attn_tail_core=args.recursive_attn_tail_core,
        recursive_attn_tail_repeat=args.recursive_attn_tail_repeat,
    ).to(device).bfloat16()
    ...
    log0(
        f"recursive_attn_tail:core={args.recursive_attn_tail_core} "
        f"repeat={args.recursive_attn_tail_repeat} "
        f"unique_slots={base_model.num_attn_slots}/{args.num_layers} "
        f"slot_map={base_model.attn_slot_map}"
    )
    ...
    export_meta = {
        ...
        "attn_slot_map": list(base_model.attn_slot_map),
    }
    ...
    torch.save({"w": quant_result, "m": quant_meta, "meta": export_meta}, quant_buf)
    ...
    artifact_meta = quant_state.get("meta")
    if not isinstance(artifact_meta, dict):
        raise RuntimeError("Quantized artifact is missing recursive attention metadata")
    artifact_model_config = artifact_meta.get("model_config")
    if not isinstance(artifact_model_config, dict):
        raise RuntimeError("Quantized artifact is missing model_config metadata")
    artifact_num_layers = int(artifact_model_config.get("num_layers", -1))
    artifact_core = int(artifact_model_config.get("recursive_attn_tail_core", -1))
    artifact_repeat = int(artifact_model_config.get("recursive_attn_tail_repeat", -1))
    artifact_slot_map = artifact_meta.get("attn_slot_map")
    if not isinstance(artifact_slot_map, list):
        raise RuntimeError("Quantized artifact is missing attn_slot_map metadata")
    expected_slot_map = build_recursive_tail_slot_map(artifact_num_layers, artifact_core, artifact_repeat)
    if [int(x) for x in artifact_slot_map] != expected_slot_map:
        raise RuntimeError(
            "Quantized artifact recursive attention metadata is inconsistent: "
            f"artifact_slot_map={artifact_slot_map} expected_slot_map={expected_slot_map}"
        )
    if int(artifact_model_config.get("vocab_size", -1)) != args.vocab_size:
        raise RuntimeError(
            "Artifact vocab_size does not match runtime tokenizer configuration: "
            f"artifact_vocab_size={artifact_model_config.get('vocab_size')} args_vocab_size={args.vocab_size}"
        )
    ...
    if eval_model.attn_slot_map != expected_slot_map:
        raise RuntimeError(
            "Eval model recursive attention map does not match serialized artifact metadata: "
            f"model_slot_map={eval_model.attn_slot_map} artifact_slot_map={expected_slot_map}"
        )
