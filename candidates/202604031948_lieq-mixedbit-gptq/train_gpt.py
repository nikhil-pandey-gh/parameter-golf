...
class Hyperparameters:
    ...
    mpq_int8_mlp_proj_last_n = int(os.environ.get("MPQ_INT8_MLP_PROJ_LAST_N", 2))
    mpq_int8_attn_out_last_n = int(os.environ.get("MPQ_INT8_ATTN_OUT_LAST_N", 2))
    mpq_int5_attn_qkv_first_n = int(os.environ.get("MPQ_INT5_ATTN_QKV_FIRST_N", 5))
    mpq_int5_mlp_up_first_n = int(os.environ.get("MPQ_INT5_MLP_UP_FIRST_N", 3))
...
# --- LieQ-inspired mixed-bit GPTQ-lite quantization ---
...
def _mixed_bitwidth(name: str, num_layers_total: int, args: Hyperparameters) -> int:
    layer_idx = _block_layer_idx(name)
    if layer_idx is None:
        return 8
    if (
        args.mpq_int8_mlp_proj_last_n > 0
        and name.endswith(".mlp.proj.weight")
        and layer_idx >= max(0, num_layers_total - args.mpq_int8_mlp_proj_last_n)
    ):
        return 8
    if (
        args.mpq_int8_attn_out_last_n > 0
        and name.endswith(".attn.proj.weight")
        and layer_idx >= max(0, num_layers_total - args.mpq_int8_attn_out_last_n)
    ):
        return 8
    if (
        args.mpq_int5_attn_qkv_first_n > 0
        and layer_idx < args.mpq_int5_attn_qkv_first_n
        and (
            name.endswith(".attn.c_q.weight")
            or name.endswith(".attn.c_k.weight")
            or name.endswith(".attn.c_v.weight")
        )
    ):
        return 5
    if (
        args.mpq_int5_mlp_up_first_n > 0
        and layer_idx < args.mpq_int5_mlp_up_first_n
        and name.endswith(".mlp.fc.weight")
    ):
        return 5
    return 6
...
def mixed_quantize_mpq(state_dict: dict[str, Tensor], int6_cats: set[str], args: Hyperparameters):
    ...
    bit_tensor_counts = {5: 0, 6: 0, 8: 0}
    bit_param_counts = {5: 0, 6: 0, 8: 0}
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            bitwidth = _mixed_bitwidth(name, num_layers_total, args)
            q, s = quantize_int_per_row(t, bitwidth)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": f"int{bitwidth}", "bits": bitwidth}
            bit_tensor_counts[bitwidth] += 1
            bit_param_counts[bitwidth] += int(t.numel())
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8", "bits": 8}
            bit_tensor_counts[8] += 1
            bit_param_counts[8] += int(t.numel())
    meta["__mpq_summary__"] = {
        "tensor_counts": bit_tensor_counts,
        "param_counts": bit_param_counts,
    }
    return result, meta
def dequantize_mixed_mpq(result: dict[str, Tensor], meta: dict[str, object],
...
def main() -> None:
    ...
    log0(
        "mpq_settings "
        f"int8_mlp_proj_last_n:{args.mpq_int8_mlp_proj_last_n} "
        f"int8_attn_out_last_n:{args.mpq_int8_attn_out_last_n} "
        f"int5_attn_qkv_first_n:{args.mpq_int5_attn_qkv_first_n} "
        f"int5_mlp_up_first_n:{args.mpq_int5_mlp_up_first_n}"
    )
    log0(
    ...
    quant_result, quant_meta = mixed_quantize_mpq(unbanked_sd, {"mlp", "attn"}, args)
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
    quant_raw = quant_buf.getvalue()
    quant_blob = lzma.compress(quant_raw, preset=6)
    if master_process:
        mpq_summary = quant_meta.get("__mpq_summary__", {})
        tensor_counts = mpq_summary.get("tensor_counts", {})
        param_counts = mpq_summary.get("param_counts", {})
        log0(
            "mpq_plan "
            f"int5_tensors:{tensor_counts.get(5, 0)} int5_params:{param_counts.get(5, 0)} "
            f"int6_tensors:{tensor_counts.get(6, 0)} int6_params:{param_counts.get(6, 0)} "
            f"int8_tensors:{tensor_counts.get(8, 0)} int8_params:{param_counts.get(8, 0)}"
        )
        with open("final_model.intmix.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model mixedbit+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size mixedbit+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    ...
    deq_unbanked = dequantize_mixed_mpq(quant_state["w"], quant_state["m"], unbanked_sd)
    ...
    # Compatibility alias for existing repo tooling that keys off this legacy line name.
