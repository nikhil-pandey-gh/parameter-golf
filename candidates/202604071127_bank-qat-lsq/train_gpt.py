...
class Hyperparameters:
    ...
    bank_qat_bits = int(os.environ.get("BANK_QAT_BITS", 6))
    bank_qat_clip_min = float(os.environ.get("BANK_QAT_CLIP_MIN", 0.75))
    bank_qat_clip_max = float(os.environ.get("BANK_QAT_CLIP_MAX", 1.25))
    bank_qat_ramp_power = float(os.environ.get("BANK_QAT_RAMP_POWER", 1.0))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,bank_qat_logclip",
    ).split(",")
    if pattern
)
...
def bank_qat_clip_multiplier(logits: Tensor, clip_min: float, clip_max: float) -> Tensor:
    return clip_min + (clip_max - clip_min) * torch.sigmoid(logits.float())


def fake_quantize_bank_int6(
    bank: Tensor,
    clip_logits: Tensor,
    mix: Tensor,
    clip_range: int,
    ...
    if bank.ndim != 3:
        raise ValueError(f"Expected a 3D bank tensor, got shape {tuple(bank.shape)}")
    # Quantize a detached view so the bank weights keep STE-style identity gradients,
    # while the zero-forward term below still lets clip_logits learn.
    w32 = bank.detach().float()
    row_clip = w32.abs().amax(dim=2)
    row_clip = row_clip * bank_qat_clip_multiplier(clip_logits, clip_min, clip_max)[:, None]
    scale = (row_clip / clip_range).clamp_min(1.0 / clip_range)
    q = torch.clamp(torch.round(w32 / scale[..., None]), -clip_range, clip_range)
    w_q = (q * scale[..., None]).to(dtype=bank.dtype)
    mix_t = mix.to(dtype=bank.dtype)
    return bank + mix_t * ((w_q - bank).detach() + (w_q - w_q.detach()))
...
class GPT(nn.Module):
    def __init__(
        ...
        bank_qat_bits: int = 6,
        bank_qat_clip_min: float = 0.75,
        bank_qat_clip_max: float = 1.25,
        ...
        if bank_qat_bits < 2:
            raise ValueError(f"bank_qat_bits must be >= 2, got {bank_qat_bits}")
        self.bank_qat_clip_range = (1 << (bank_qat_bits - 1)) - 1
        self.bank_qat_clip_min = bank_qat_clip_min
        self.bank_qat_clip_max = bank_qat_clip_max
        self.bank_qat_active = False
        self.register_buffer("bank_qat_mix", torch.tensor(0.0, dtype=torch.float32), persistent=False)
        self.qo_bank_qat_logclip = nn.Parameter(torch.zeros(2 * num_layers, dtype=torch.float32))
        self.kv_bank_qat_logclip = nn.Parameter(torch.zeros(2 * num_layers, dtype=torch.float32))
        self.mlp_up_bank_qat_logclip = nn.Parameter(torch.zeros(num_layers, dtype=torch.float32))
        self.mlp_down_bank_qat_logclip = nn.Parameter(torch.zeros(num_layers, dtype=torch.float32))
        ...
        self._init_weights()
    def set_bank_qat_active(self, enabled: bool) -> None:
        self.bank_qat_active = bool(enabled)
    def set_bank_qat_mix(self, mix: float) -> None:
        mix = min(max(float(mix), 0.0), 1.0)
        self.bank_qat_mix.fill_(mix)
    def _materialize_banks(self) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        if not (self.training and self.bank_qat_enabled and self.bank_qat_active):
            return self.qo_bank, self.kv_bank, self.mlp_up_bank, self.mlp_down_bank
        return (
            fake_quantize_bank_int6(
                self.qo_bank,
                self.qo_bank_qat_logclip,
                self.bank_qat_mix,
                self.bank_qat_clip_range,
                self.bank_qat_clip_min,
                self.bank_qat_clip_max,
            ),
            fake_quantize_bank_int6(
                self.kv_bank,
                self.kv_bank_qat_logclip,
                self.bank_qat_mix,
                self.bank_qat_clip_range,
                self.bank_qat_clip_min,
                self.bank_qat_clip_max,
            ...
            fake_quantize_bank_int6(
                self.mlp_down_bank,
                self.mlp_down_bank_qat_logclip,
                self.bank_qat_mix,
                self.bank_qat_clip_range,
                self.bank_qat_clip_min,
                self.bank_qat_clip_max,
    ...
    def export_bank_qat_clip_map(self) -> dict[str, float]:
        clip_map: dict[str, float] = {}
        qo = bank_qat_clip_multiplier(
            self.qo_bank_qat_logclip.detach().cpu(),
            self.bank_qat_clip_min,
            self.bank_qat_clip_max,
        )
        kv = bank_qat_clip_multiplier(
            self.kv_bank_qat_logclip.detach().cpu(),
            self.bank_qat_clip_min,
            self.bank_qat_clip_max,
        )
        up = bank_qat_clip_multiplier(
            self.mlp_up_bank_qat_logclip.detach().cpu(),
            self.bank_qat_clip_min,
            self.bank_qat_clip_max,
        )
        down = bank_qat_clip_multiplier(
            self.mlp_down_bank_qat_logclip.detach().cpu(),
            self.bank_qat_clip_min,
            self.bank_qat_clip_max,
        )
        n = self.num_layers
        for i in range(n):
            clip_map[f"blocks.{i}.attn.c_q.weight"] = float(qo[i].item())
            clip_map[f"blocks.{i}.attn.proj.weight"] = float(qo[n + i].item())
            clip_map[f"blocks.{i}.attn.c_k.weight"] = float(kv[i].item())
            clip_map[f"blocks.{i}.attn.c_v.weight"] = float(kv[n + i].item())
            clip_map[f"blocks.{i}.mlp.fc.weight"] = float(up[i].item())
            clip_map[f"blocks.{i}.mlp.proj.weight"] = float(down[i].item())
        return clip_map
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        qo_bank, kv_bank, mlp_up_bank, mlp_down_bank = self._materialize_banks()
...
def quantize_int6_per_row(
    ...
    if t32.ndim == 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            row_clip = row_clip * clip_multiplier
    ...
    scale = torch.tensor((amax * clip_multiplier) / clip_range if amax > 0 else 1.0, dtype=torch.float16)
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    clip_multipliers: dict[str, float] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, clip_multiplier=(clip_multipliers or {}).get(name, 1.0))
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_qat_enabled=args.bank_qat_enabled,
        bank_qat_bits=args.bank_qat_bits,
        bank_qat_clip_min=args.bank_qat_clip_min,
        bank_qat_clip_max=args.bank_qat_clip_max,
    ...
    base_model.set_bank_qat_active(False)
    base_model.set_bank_qat_mix(0.0)
    ...
    def refresh_compiled_model(reason: str) -> None:
        nonlocal compiled_model, model
        torch.cuda.synchronize()
        compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
        model = compiled_model
        log0(f"compile:refresh reason={reason}")
    ...
    scalar_params.extend(
        [
            base_model.qo_bank_qat_logclip,
            base_model.kv_bank_qat_logclip,
            base_model.mlp_up_bank_qat_logclip,
            base_model.mlp_down_bank_qat_logclip,
        ]
    )
    ...
    log0(
        f"bank_qat:enabled={args.bank_qat_enabled} bits={args.bank_qat_bits} "
        f"clip=({args.bank_qat_clip_min:.2f},{args.bank_qat_clip_max:.2f}) "
        f"threshold={args.late_qat_threshold:.3f} ramp_power={args.bank_qat_ramp_power:.2f}"
    )
    ...
    while True:
        ...
        late_qat_active = args.late_qat_threshold > 0 and scale < args.late_qat_threshold
        bank_qat_mix = 0.0
        if late_qat_active:
            frac_qat = 1.0 - (scale / max(args.late_qat_threshold, 1e-9))
            bank_qat_mix = frac_qat ** args.bank_qat_ramp_power
            if args.bank_qat_enabled and not base_model.bank_qat_active:
                base_model.set_bank_qat_active(True)
                base_model.set_bank_qat_mix(bank_qat_mix)
                CastedLinear._qat_enabled = args.qat_enabled
                refresh_compiled_model("late_qat_start")
                log0(f"late_qat:enabled step:{step} scale:{scale:.4f} bank_qat_mix:{bank_qat_mix:.4f}")
            elif CastedLinear._qat_enabled is False and args.qat_enabled:
                CastedLinear._qat_enabled = True
                refresh_compiled_model("late_qat_start_castedlinear")
        if base_model.bank_qat_active:
            base_model.set_bank_qat_mix(bank_qat_mix)
        zero_grad_all()
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        clip_multipliers=base_model.export_bank_qat_clip_map(),
    )
