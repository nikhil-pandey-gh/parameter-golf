...
try:
    ...
    _ATTN_BACKEND = "flash_attn_interface"
except ImportError:
...
class Hyperparameters:
    ...
    mtp_stride = max(1, int(os.environ.get("MTP_STRIDE", 2)))
    mtp_fade_from_scale = float(os.environ.get("MTP_FADE_FROM_SCALE", 0.35))
    mtp_fade_to_scale = float(os.environ.get("MTP_FADE_TO_SCALE", 0.15))
...
def eval_val(
    ...
    model.eval()
    zero_mtp_scale = torch.zeros((), device=device)
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, zero_mtp_scale, False).detach()
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_stride = max(mtp_stride, 1)
    ...
    def forward(
        ...
        input_ids: Tensor,
        target_ids: Tensor,
        mtp_loss_scale: Tensor | None = None,
        ...
        if self.training and mtp_enabled and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            aux_scale = (
                ...
                else mtp_loss_scale.to(device=x.device, dtype=main_loss.dtype)
            )
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if valid_t <= 0:
                    continue
                mtp_hidden = x[:, :valid_t:stride, :]
                mtp_targets = target_ids[:, k + 1 : k + 1 + valid_t : stride]
                if mtp_hidden.numel() == 0 or mtp_targets.numel() == 0:
                    continue
                mtp_hidden = mtp_hidden.reshape(-1, dim)
                mtp_targets = mtp_targets.reshape(-1)
                mtp_logits_proj = mtp_head(mtp_hidden)
                mtp_logits = self.logit_softcap * torch.tanh(mtp_logits_proj / self.logit_softcap)
                mtp_loss_sum = mtp_loss_sum + F.cross_entropy(mtp_logits.float(), mtp_targets, reduction="mean")
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + (aux_scale * self.mtp_loss_weight) * (mtp_loss_sum / mtp_loss_count)
        return main_loss
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, args.ttt_batch_seqs):
                        ...
                        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                            loss = base_model(x, y, zero_mtp_scale, False)
                        loss.backward()
                        if world_size > 1:
                            for p in ttt_params:
...
def main() -> None:
    ...
    if _ATTN_BACKEND != "flash_attn_interface":
        raise RuntimeError(
            "flash_attn_interface is required for main(); the SDPA fallback is only for import-time smoke tests"
        )
    ...
    # - lm_head / mtp heads -> Adam
    # - bigram proj, VE proj -> AdamW scalar bucket (small matrix params not worth banking)
    matrix_params = [
    ...
    optimizer_head = None
    optimizer_mtp = None
    mtp_head_weights = [head.weight for head in base_model.mtp_heads]
    ...
    if mtp_head_weights:
        optimizer_mtp = torch.optim.Adam(
            [{"params": mtp_head_weights, "lr": args.head_lr, "base_lr": args.head_lr}],
            betas=(args.beta1, args.beta2),
            eps=args.adam_eps,
            fused=True,
        )
        replicated_params.extend(mtp_head_weights)
    ...
    log0(
        ...
        f"mtp_stride:{args.mtp_stride} mtp_fade_from_scale:{args.mtp_fade_from_scale} "
        f"mtp_fade_to_scale:{args.mtp_fade_to_scale} mtp_params:{mtp_params}"
    )
    ...
    aux_head_lr = args.head_lr if (base_model.lm_head is not None or mtp_head_weights) else 0.0
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} "
        f"head_lr:{aux_head_lr} "
    ...
    def mtp_scale_for_lr(scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_fade_from_scale <= args.mtp_fade_to_scale:
            return 1.0 if scale > args.mtp_fade_to_scale else 0.0
        if scale >= args.mtp_fade_from_scale:
            return 1.0
        if scale <= args.mtp_fade_to_scale:
            return 0.0
        return (scale - args.mtp_fade_to_scale) / (args.mtp_fade_from_scale - args.mtp_fade_to_scale)
    if args.warmup_steps > 0:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        warmup_mtp_scale = torch.tensor(mtp_scale_for_lr(1.0), device=device)
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            for micro_step in range(grad_accum_steps):
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, warmup_mtp_scale, True)
    ...
    while True:
        ...
        mtp_scale_value = mtp_scale_for_lr(scale)
        mtp_scale = torch.tensor(mtp_scale_value, device=device)
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_scale, mtp_scale_value > 0.0)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_scale:{mtp_scale_value:.3f}"
            )
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_stride=args.mtp_stride,
