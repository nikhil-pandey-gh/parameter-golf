...
class GPT(nn.Module):
    def __init__(
        ...
        self.active_mtp_heads = mtp_num_heads
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        active_mtp_heads = min(self.active_mtp_heads, self.mtp_num_heads)
        if self.training and active_mtp_heads > 0 and self.mtp_loss_weight > 0.0:
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_params:{mtp_params} mtp_curriculum:{args.mtp_curriculum} "
    ...
    log0(f"attention_backend:{'flash-attn3' if flash_attn_3_func is not None else 'torch-sdpa-fallback'}")
    ...
    def mtp_active_heads(step: int, elapsed_ms: float) -> int:
        ...
        if args.mtp_num_heads == 1:
            return 1
        phase = (progress - start) / max(full - start, 1e-9)
        return min(args.mtp_num_heads - 1, 1 + int(phase * (args.mtp_num_heads - 1)))
    base_model.active_mtp_heads = mtp_active_heads(0, 0.0)
    last_logged_mtp_heads = base_model.active_mtp_heads
    ...
    while True:
        ...
        current_mtp_heads = mtp_active_heads(step, elapsed_ms)
        if current_mtp_heads != base_model.active_mtp_heads:
            base_model.active_mtp_heads = current_mtp_heads
        if current_mtp_heads != last_logged_mtp_heads:
            log0(
                f"mtp_curriculum:active_heads:{current_mtp_heads}/{args.mtp_num_heads} "
                f"progress:{training_progress(step, elapsed_ms):.3f}"
            )
            last_logged_mtp_heads = current_mtp_heads
