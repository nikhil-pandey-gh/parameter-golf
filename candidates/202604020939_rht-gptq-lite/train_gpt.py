...
INT6_ROTATION_BLOCKS = tuple(
    sorted(
        {
            int(value)
            for value in os.environ.get("INT6_ROTATION_BLOCKS", "256,512").split(",")
...
def _make_rht_signs(name: str, num_blocks: int, block_size: int, device: torch.device, dtype: torch.dtype) -> Tensor:
    seed = zlib.crc32(f"{name}:{block_size}".encode("utf-8")) & 0xFFFFFFFF
    gen = torch.Generator(device="cpu")
    gen.manual_seed(seed)
    signs = torch.randint(0, 2, (num_blocks, block_size), generator=gen, dtype=torch.int8)
...
def hadamard_transform_last_dim(x: Tensor) -> Tensor:
    ...
    while h < n:
        ...
        h *= 2
    return y.reshape_as(x) * (1.0 / math.sqrt(n))
def apply_block_rht(x: Tensor, name: str, block_size: int) -> Tensor:
    if x.ndim != 2:
        raise ValueError(f"RHT expects a 2D tensor, got shape {tuple(x.shape)}")
    if not _is_power_of_two(block_size):
        raise ValueError(f"RHT block size must be a power of two, got {block_size}")
    if x.size(-1) % block_size != 0:
        raise ValueError(f"RHT block size {block_size} must divide trailing dim {x.size(-1)}")
    num_blocks = x.size(-1) // block_size
    blocks = x.reshape(x.size(0), num_blocks, block_size)
    signs = _make_rht_signs(name, num_blocks, block_size, x.device, x.dtype)
    blocks = blocks * signs.unsqueeze(0)
    blocks = hadamard_transform_last_dim(blocks)
    blocks = blocks * signs.unsqueeze(0)
    return blocks.reshape_as(x)
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def _quantize_int6_per_row_core(t32: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, float]:
    if t32.ndim == 2:
        ...
        if best_q is None or best_s is None:
            raise RuntimeError("Failed to quantize int6 matrix")
    ...
    err = (t32 - recon).pow(2).mean().item()
    return q, scale, err
def quantize_int6_per_row(name: str, t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, dict[str, object] | None]:
    t32 = t.float()
    best_q, best_s, best_err = _quantize_int6_per_row_core(t32, clip_range)
    best_meta: dict[str, object] | None = None
    if t32.ndim == 2:
        for block_size in INT6_ROTATION_BLOCKS:
            if t32.size(-1) < block_size or t32.size(-1) % block_size != 0:
                continue
            rotated = apply_block_rht(t32, name, block_size)
            q, s, _ = _quantize_int6_per_row_core(rotated, clip_range)
            recon = apply_block_rht(q.float() * s.float()[:, None], name, block_size)
            err = (t32 - recon).pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_err = q, s, err
                best_meta = {"rotation": "rht_dhd_v1", "block_size": block_size}
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, rot_meta = quantize_int6_per_row(name, t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6", **(rot_meta or {})}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("rotation") == "rht_dhd_v1":
            restored = apply_block_rht(restored.float(), name, int(info["block_size"]))
...
def main() -> None:
    ...
    log0(
        f"int6_rotation blocks:{','.join(str(b) for b in INT6_ROTATION_BLOCKS) or 'disabled'} "
        f"rotated_tensors:{len(rotated_tensors)}"
    )
    if rotated_tensors:
        log0(f"int6_rotation_tensors:{','.join(rotated_tensors)}", console=False)
    ...
    if master_process:
        ...
        log0(f"Total submission size exported_model: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
