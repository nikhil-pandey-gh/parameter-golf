...
class Hyperparameters:
    ...
    pit_ortho_reg = float(os.environ.get("PIT_ORTHO_REG", 1e-4))
    pit_log_scale_init = float(os.environ.get("PIT_LOG_SCALE_INIT", 0.0))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,pit_log_scale",
    ).split(",")
    if pattern
)
...
class PseudoInverseTiedEmbedding(nn.Module):
    """PIT-lite: shared token memory with a diagonal hidden-space transform."""

    def __init__(self, vocab_size: int, dim: int, log_scale_init: float = 0.0):
        super().__init__()
        self.weight = nn.Parameter(torch.empty(vocab_size, dim))
        self.pit_log_scale = nn.Parameter(torch.full((dim,), log_scale_init, dtype=torch.float32))

    def reset_parameters(self) -> None:
        nn.init.orthogonal_(self.weight)

    def forward(self, input_ids: Tensor) -> Tensor:
        scale_inv = torch.exp(-self.pit_log_scale).to(dtype=self.weight.dtype)
        return F.embedding(input_ids, self.weight) * scale_inv[None, None, :]

    def project(self, hidden: Tensor) -> Tensor:
        scale = torch.exp(self.pit_log_scale).to(dtype=hidden.dtype)
        return F.linear(hidden * scale, self.weight.to(dtype=hidden.dtype))
...
class GPT(nn.Module):
    def __init__(
        ...
        pit_ortho_reg: float = 1e-4,
        pit_log_scale_init: float = 0.0,
    ):
        super().__init__()
        ...
        self.pit_enabled = tie_embeddings and pit_enabled
        self.pit_ortho_reg = pit_ortho_reg
        if self.pit_enabled:
            self.tok_emb = PseudoInverseTiedEmbedding(vocab_size, model_dim, pit_log_scale_init)
        else:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            if self.pit_enabled:
                logits_proj = self.tok_emb.project(x_flat)
            else:
        ...
        if self.training and self.pit_enabled and self.pit_ortho_reg > 0.0:
            main_loss = main_loss + self.pit_ortho_reg * self.tok_emb.orthogonality_loss()
        return main_loss
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        pit_ortho_reg=args.pit_ortho_reg,
        pit_log_scale_init=args.pit_log_scale_init,
    ).to(device).bfloat16()
    ...
    if isinstance(base_model.tok_emb, PseudoInverseTiedEmbedding):
        scalar_params.append(base_model.tok_emb.pit_log_scale)
    ...
    log0(
        f"attention_kernel:{'flash_attn_3' if flash_attn_3_func is not None else 'torch_sdpa'} "
    ...
    if args.tie_embeddings:
        log0(f"pit_enabled:{int(args.pit_enabled)} pit_ortho_reg:{args.pit_ortho_reg:g}")
    log0(
    ...
    eval_model = GPT(
        ...
        pit_enabled=args.pit_enabled, pit_ortho_reg=args.pit_ortho_reg,
        pit_log_scale_init=args.pit_log_scale_init,
    ).to(device).bfloat16()
