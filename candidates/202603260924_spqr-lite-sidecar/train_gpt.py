...
class Hyperparameters:
    ...
    outlier_topk = int(os.environ.get("OUTLIER_TOPK", 1))
    outlier_late_layers = int(os.environ.get("OUTLIER_LATE_LAYERS", 4))
    outlier_mlp_only = bool(int(os.environ.get("OUTLIER_MLP_ONLY", "0")))
...
def quantize_int6_with_outlier_sidecar(
    t: Tensor,
    topk: int,
    clip_range: int = 31,
) -> tuple[Tensor, Tensor, Tensor | None, Tensor | None]:
    q, s = quantize_int6_per_row(t, clip_range=clip_range)
    if topk <= 0 or t.ndim != 2:
        return q, s, None, None
    topk = min(topk, t.shape[1])
    if topk <= 0:
        return q, s, None, None
    t32 = t.float()
    recon = q.float() * s.float()[:, None]
    residual = t32 - recon
    outlier_idx = residual.abs().topk(k=topk, dim=1, largest=True, sorted=False).indices.to(torch.int16)
    outlier_val = residual.gather(1, outlier_idx.long()).to(torch.float16)
    return q, s, outlier_idx, outlier_val
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    outlier_topk: int = 0,
    outlier_late_layers: int = 0,
    outlier_mlp_only: bool = False,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            # Spend the sparse sidecar budget on the deepest int6 layers, which are already the
            # most specialized part of the current record stack (XSA / VE / partial RoPE).
            use_outlier_sidecar = (
                outlier_topk > 0
                and t.ndim == 2
                and layer_idx is not None
                and layer_idx >= max(num_layers_total - outlier_late_layers, 0)
                and (not outlier_mlp_only or cat == "mlp")
            )
            if use_outlier_sidecar:
                q, s, outlier_idx, outlier_val = quantize_int6_with_outlier_sidecar(t, outlier_topk)
            else:
                q, s = quantize_int6_per_row(t)
                outlier_idx, outlier_val = None, None
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
            if outlier_idx is not None and outlier_val is not None:
                result[name + ".oi"] = outlier_idx
                result[name + ".ov"] = outlier_val
                stats["outlier_tensors"] += 1
                stats["outlier_entries"] += int(outlier_idx.numel())
                stats["outlier_payload_bytes"] += tensor_nbytes(outlier_idx) + tensor_nbytes(outlier_val)
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        oi_key = name + ".oi"
        ov_key = name + ".ov"
        if oi_key in result and ov_key in result:
            outlier_idx = result[oi_key].long()
            outlier_val = result[ov_key].float()
            row_idx = torch.arange(restored.shape[0], dtype=torch.long).unsqueeze(1)
            restored[row_idx, outlier_idx] += outlier_val
...
def main() -> None:
    ...
    quant_result, quant_meta, quant_stats = mixed_quantize_int6(
        ...
        outlier_late_layers=args.outlier_late_layers,
        outlier_mlp_only=args.outlier_mlp_only,
    )
    ...
    if master_process:
        ...
        log0(
            "outlier_sidecar:"
            f" topk={args.outlier_topk}"
            f" late_layers={args.outlier_late_layers}"
            f" mlp_only={args.outlier_mlp_only}"
            f" tensors={quant_stats['outlier_tensors']}"
            f" entries={quant_stats['outlier_entries']}"
            f" payload_bytes={quant_stats['outlier_payload_bytes']}"
        )
        log0(f"Serialized model int6+sidecar+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size int6+sidecar+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
