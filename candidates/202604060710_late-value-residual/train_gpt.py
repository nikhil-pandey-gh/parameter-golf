...
class Hyperparameters:
    ...
    value_residual_start_layer = int(os.environ.get("VALUE_RESIDUAL_START_LAYER", "6"))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if value_residual:
            self.vr_lambda = nn.Parameter(torch.tensor([-0.75, 0.75], dtype=torch.float32))
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        raw_v = v if (v0 is None or self.value_residual) else None
        if self.value_residual and v0 is not None:
            lam = torch.softmax(self.vr_lambda.float(), dim=0).to(dtype=v.dtype)
            v = lam[0] * v0 + lam[1] * v
...
class Block(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        value_residual_start_layer: int = 0,
    ):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        use_value_residual = value_residual and layer_idx >= value_residual_start_layer
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init,
                                        gated_attention=gated_attention, value_residual=use_value_residual)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.value_residual = value_residual
        self.value_residual_start_layer = value_residual_start_layer
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    value_residual_start_layer=value_residual_start_layer,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        value_residual_start_layer=args.value_residual_start_layer,
    ).to(device).bfloat16()
