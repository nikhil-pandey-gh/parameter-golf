...
class Hyperparameters:
    ...
    mtp_final_loss_weight = float(os.environ.get("MTP_FINAL_LOSS_WEIGHT", 0.0))
    mtp_head_lr = float(os.environ.get("MTP_HEAD_LR", 0.025))
    mtp_curriculum = os.environ.get("MTP_CURRICULUM", "reverse")
    mtp_warmdown_scale = float(os.environ.get("MTP_WARMDOWN_SCALE", 0.25))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "mtp_loss_weight_buf",
            torch.tensor(float(mtp_loss_weight), dtype=torch.float32),
            persistent=False,
        )
            ...
            with torch.no_grad():
                for head in self.mtp_heads:
                    if self.tie_embeddings:
                        head.weight.copy_(self.tok_emb.weight)
                    else:
                        nn.init.normal_(head.weight, mean=0.0, std=self.tied_embed_init_std)
    def set_mtp_loss_weight(self, weight: float) -> None:
        with torch.no_grad():
            self.mtp_loss_weight_buf.fill_(float(weight))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = self.mtp_loss_weight_buf.to(dtype=main_loss.dtype, device=main_loss.device)
                main_loss = main_loss + mtp_weight * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    model.train()
    x = torch.randint(args.vocab_size, (2, args.train_seq_len), device=device)
    y = torch.randint(args.vocab_size, (2, args.train_seq_len), device=device)
    loss = model(x, y)
    if not torch.isfinite(loss):
        raise RuntimeError("SMOKE_TEST produced a non-finite loss")
    print(f"smoke_test_loss:{loss.item():.6f}")
    print(f"smoke_test_attention:{'flash_attn3' if flash_attn_3_func is not None else 'sdpa_fallback'}")
...
def main() -> None:
    ...
    args = Hyperparameters()
    if args.mtp_curriculum not in {"none", "reverse"}:
        raise ValueError(f"Unsupported MTP_CURRICULUM={args.mtp_curriculum!r}")
    ...
    optimizer_mtp = None
    if args.mtp_num_heads > 0 and len(base_model.mtp_heads) > 0:
    ...
    if optimizer_mtp is not None:
        replicated_params.extend(optimizer_mtp.param_groups[0]["params"])
    ...
    log0(
        ...
        f"mtp_final_loss_weight:{args.mtp_final_loss_weight} mtp_curriculum:{args.mtp_curriculum} "
    ...
    log0(
        f"sdp_backends:cudnn=False flash={flash_attn_3_func is not None} "
    ...
    def mtp_loss_weight_for_scale(scale: float) -> float:
        if args.mtp_num_heads <= 0:
            return 0.0
        if args.mtp_curriculum == "none":
            return args.mtp_loss_weight
        if args.mtp_warmdown_scale <= 0:
            return args.mtp_final_loss_weight
        ramp = min(max(scale / args.mtp_warmdown_scale, 0.0), 1.0)
        return args.mtp_final_loss_weight + (args.mtp_loss_weight - args.mtp_final_loss_weight) * ramp
    ...
    if args.warmup_steps > 0:
        ...
        base_model.set_mtp_loss_weight(mtp_loss_weight_for_scale(1.0))
        model.train()
    ...
    while True:
        ...
        curr_mtp_loss_weight = mtp_loss_weight_for_scale(scale)
        base_model.set_mtp_loss_weight(curr_mtp_loss_weight)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_loss_weight:{curr_mtp_loss_weight:.4f}"
            )
