...
def shared_layers_from_num_layers(num_layers: int) -> int:
    if num_layers <= 0:
        raise ValueError(f"NUM_LAYERS must be positive, got {num_layers}")
    return (num_layers + 1) // 2


def build_mirrored_share_map(num_layers: int) -> list[int]:
    shared_layers = shared_layers_from_num_layers(num_layers)
    return [i if i < shared_layers else num_layers - 1 - i for i in range(num_layers)]
...
class GPT(nn.Module):
    def __init__(
        ...
        self.layer_share_map = build_mirrored_share_map(num_layers)
        self.num_shared_layers = max(self.layer_share_map) + 1
        ...
        self.qo_bank = nn.Parameter(torch.empty(2 * n_shared, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * n_shared, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(n_shared, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(n_shared, model_dim, mlp_dim))
    ...
    def _shared_idx(self, layer_idx: int) -> int:
        return self.layer_share_map[layer_idx]
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            si = self._shared_idx(i)
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[si], self.kv_bank[si], self.kv_bank[n + si],
                self.qo_bank[n + si], self.mlp_up_bank[si], self.mlp_down_bank[si],
                v_embed=ve, v0=v0)
...
def eval_val_sliding_ttt(
    ...
    frozen_shared_ids = {base_model._shared_idx(bi) for bi in frozen_block_ids}
    ttt_params = []
    ...
    def mask_frozen_shared_bank_grads() -> None:
        if not frozen_shared_ids:
            return
        n_shared = base_model.num_shared_layers
        if base_model.qo_bank.grad is not None:
            for si in frozen_shared_ids:
                base_model.qo_bank.grad[si].zero_()
                base_model.qo_bank.grad[n_shared + si].zero_()
        if base_model.kv_bank.grad is not None:
            for si in frozen_shared_ids:
                base_model.kv_bank.grad[si].zero_()
                base_model.kv_bank.grad[n_shared + si].zero_()
        if base_model.mlp_up_bank.grad is not None:
            for si in frozen_shared_ids:
                base_model.mlp_up_bank.grad[si].zero_()
        if base_model.mlp_down_bank.grad is not None:
            for si in frozen_shared_ids:
                base_model.mlp_down_bank.grad[si].zero_()

    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
         f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)} "
         f"frozen_shared={sorted(frozen_shared_ids)}")
...
def _unbank_state_dict(sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert 3D bank tensors into individual 2D tensors with synthetic block names."""
    out: dict[str, Tensor] = {}
    n = int(sd["mlp_up_bank"].shape[0])
...
def _rebank_state_dict(sd: dict[str, Tensor], template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    out: dict[str, Tensor] = {}
    n = int(template_sd["mlp_up_bank"].shape[0])
...
def main() -> None:
    ...
    log0(
        f"matrix_sharing:mirror shared_layers:{base_model.num_shared_layers} "
        f"share_map:{base_model.layer_share_map}"
    )
