...
def fake_quantize_bank_weight_int6(t: Tensor, clip_range: int = 31) -> Tensor:
    if t.ndim != 2:
        raise ValueError(f"bank QAT expects a matrix, got shape {tuple(t.shape)}")
    with torch.no_grad():
        t32 = t.float()
        row_max = t32.abs().amax(dim=1, keepdim=True)
        scale = (row_max / clip_range).clamp_min(1.0 / clip_range)
        q = torch.clamp(torch.round(t32 / scale), -clip_range, clip_range) * scale
    return t + (q.to(dtype=t.dtype) - t).detach()
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if BANK_QAT_ENABLED and self.training:
...
def compile_training_model(base_model: nn.Module) -> nn.Module:
    return torch.compile(base_model, dynamic=False, fullgraph=True)

# --- Training ---

def main() -> None:
    global BANK_QAT_ENABLED
    ...
    BANK_QAT_ENABLED = args.qat_enabled and args.bank_qat_enabled
    base_model = GPT(
    ...
    log0(
        f"qat:linear={int(CastedLinear._qat_enabled)} "
        f"bank={int(BANK_QAT_ENABLED)} late_qat_threshold:{args.late_qat_threshold:.4f}"
    )
    ...
    while True:
        ...
        needs_late_qat = (not CastedLinear._qat_enabled) or (args.bank_qat_enabled and not BANK_QAT_ENABLED)
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and needs_late_qat:
            ...
            log0(
                ...
                f"bank_qat:{int(BANK_QAT_ENABLED)} recompiling:1"
            )
        zero_grad_all()
