...
def clip_multiplier_from_logits(clip_logits: Tensor) -> Tensor:
    return 2.0 * torch.sigmoid(clip_logits.float())


def fake_quantize_int6_tensor(t: Tensor, clip_logits: Tensor | None = None, clip_range: int = 31) -> Tensor:
    t32 = t.float()
    if t32.ndim == 2:
        row_clip = t32.abs().amax(dim=1)
        if clip_logits is not None:
            row_clip = row_clip * clip_multiplier_from_logits(clip_logits).to(dtype=row_clip.dtype)
        ...
        quant = (torch.round(scaled) - scaled).detach() + scaled
        quant = torch.clamp(quant, -clip_range, clip_range)
        return (quant * scale[:, None]).to(dtype=t.dtype)
    clip_abs = t32.abs().max()
    if clip_logits is not None:
        clip_abs = clip_abs * clip_multiplier_from_logits(clip_logits).to(dtype=clip_abs.dtype)
    clip_abs = clip_abs.clamp_min(1.0 / clip_range)
    scale = (clip_abs / clip_range).clamp_min(1.0 / clip_range)
    clipped = torch.clamp(t32, -clip_abs, clip_abs)
    scaled = clipped / scale
    quant = (torch.round(scaled) - scaled).detach() + scaled
    quant = torch.clamp(quant, -clip_range, clip_range)
    return (quant * scale).to(dtype=t.dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        v_w: Tensor,
        out_w: Tensor,
        q_clip: Tensor | None = None,
        k_clip: Tensor | None = None,
        v_clip: Tensor | None = None,
        out_clip: Tensor | None = None,
        v_embed: Tensor | None = None,
        v0: Tensor | None = None,
    ) -> tuple[Tensor, Tensor | None]:
        bsz, seqlen, dim = x.shape
        q_w = fake_quantize_int6_tensor(q_w, q_clip) if q_clip is not None else q_w
        k_w = fake_quantize_int6_tensor(k_w, k_clip) if k_clip is not None else k_w
        v_w = fake_quantize_int6_tensor(v_w, v_clip) if v_clip is not None else v_w
        out_w = fake_quantize_int6_tensor(out_w, out_clip) if out_clip is not None else out_w
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        up_w: Tensor,
        down_w: Tensor,
        up_clip: Tensor | None = None,
        down_clip: Tensor | None = None,
    ) -> Tensor:
        up_w = fake_quantize_int6_tensor(up_w, up_clip) if up_clip is not None else up_w
        down_w = fake_quantize_int6_tensor(down_w, down_clip) if down_clip is not None else down_w
...
class GPT(nn.Module):
    def __init__(
        ...
        # One learned clip scalar per exported bank matrix keeps the QAT path aligned with int6 export.
        self.qo_clip_logits = nn.Parameter(torch.zeros(2 * num_layers, dtype=torch.float32))
        self.kv_clip_logits = nn.Parameter(torch.zeros(2 * num_layers, dtype=torch.float32))
        self.mlp_up_clip_logits = nn.Parameter(torch.zeros(num_layers, dtype=torch.float32))
        self.mlp_down_clip_logits = nn.Parameter(torch.zeros(num_layers, dtype=torch.float32))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        qat_active = self.training and self.bank_qat_enabled
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](
                ...
                q_clip=self.qo_clip_logits[i] if qat_active else None,
                k_clip=self.kv_clip_logits[i] if qat_active else None,
                v_clip=self.kv_clip_logits[n + i] if qat_active else None,
                out_clip=self.qo_clip_logits[n + i] if qat_active else None,
                up_clip=self.mlp_up_clip_logits[i] if qat_active else None,
                down_clip=self.mlp_down_clip_logits[i] if qat_active else None,
                v_embed=ve,
                v0=v0,
            )
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](
                ...
                q_clip=self.qo_clip_logits[bi] if qat_active else None,
                k_clip=self.kv_clip_logits[bi] if qat_active else None,
                v_clip=self.kv_clip_logits[n + bi] if qat_active else None,
                out_clip=self.qo_clip_logits[n + bi] if qat_active else None,
                up_clip=self.mlp_up_clip_logits[bi] if qat_active else None,
                down_clip=self.mlp_down_clip_logits[bi] if qat_active else None,
                v_embed=ve,
                v0=v0,
            )
        x = self.final_norm(x)
...
def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = 31,
    learned_clip_logits: Tensor | None = None,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        candidate_row_clips: list[Tensor] = []
        if learned_clip_logits is not None:
            candidate_row_clips.append(
                (t32.abs().amax(dim=1) * clip_multiplier_from_logits(learned_clip_logits).to(dtype=t32.dtype))
                .clamp_min(1.0 / clip_range)
            )
    ...
    if learned_clip_logits is not None:
        amax *= float(clip_multiplier_from_logits(learned_clip_logits).item())
...
def _unbank_clip_logits(sd: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    out: dict[str, Tensor] = {}
    n = num_layers
    if "qo_clip_logits" in sd:
        logits = sd["qo_clip_logits"]
        for i in range(n):
            out[f"blocks.{i}.attn.c_q.weight"] = logits[i]
            out[f"blocks.{i}.attn.proj.weight"] = logits[n + i]
    if "kv_clip_logits" in sd:
        logits = sd["kv_clip_logits"]
        for i in range(n):
            out[f"blocks.{i}.attn.c_k.weight"] = logits[i]
            out[f"blocks.{i}.attn.c_v.weight"] = logits[n + i]
    if "mlp_up_clip_logits" in sd:
        logits = sd["mlp_up_clip_logits"]
        for i in range(n):
            out[f"blocks.{i}.mlp.fc.weight"] = logits[i]
    if "mlp_down_clip_logits" in sd:
        logits = sd["mlp_down_clip_logits"]
        for i in range(n):
            out[f"blocks.{i}.mlp.proj.weight"] = logits[i]
    return out
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    learned_clip_logits: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(
                t,
                learned_clip_logits=learned_clip_logits.get(name) if learned_clip_logits is not None else None,
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
...
def main() -> None:
    ...
    base_model.bank_qat_enabled = args.qat_enabled and args.bank_qat_enabled
    CastedLinear._qat_enabled = args.qat_enabled
    ...
    if args.bank_qat_enabled:
        scalar_params.extend(
            [
                base_model.qo_clip_logits,
                base_model.kv_clip_logits,
                base_model.mlp_up_clip_logits,
                base_model.mlp_down_clip_logits,
            ]
        )
    ...
    late_qat_activated = bool(args.qat_enabled)
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not late_qat_activated:
            late_qat_activated = True
            CastedLinear._qat_enabled = True
            base_model.bank_qat_enabled = args.bank_qat_enabled
            if model is compiled_model:
                model = base_model
                compiled_model = base_model
                log0("late_qat:switching_to_eager")
            log0(
                f"late_qat:enabled step:{step} scale:{scale:.4f} "
                f"bank_qat:{int(base_model.bank_qat_enabled)}"
            )
        zero_grad_all()
    ...
    clip_logits = _unbank_clip_logits(sd_cpu, args.num_layers)
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, clip_logits)
