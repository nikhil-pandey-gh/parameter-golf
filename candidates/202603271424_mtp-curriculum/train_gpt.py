...
def flash_attention(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    if q_t.size(1) != k_t.size(1):
        if q_t.size(1) % k_t.size(1) != 0:
            raise ValueError("query heads must be divisible by kv heads for GQA fallback")
...
class Hyperparameters:
    ...
    mtp_warmup_frac = float(os.environ.get("MTP_WARMUP_FRAC", 0.10))
    mtp_end_frac = float(os.environ.get("MTP_END_FRAC", 0.75))
    mtp_decay_frac = float(os.environ.get("MTP_DECAY_FRAC", 0.10))
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_loss_scale: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            mtp_scale = (
                mtp_loss_scale.to(device=x.device, dtype=x.dtype)
                if mtp_loss_scale is not None
                else x.new_tensor(self.mtp_loss_weight)
            )
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                main_loss = main_loss + mtp_scale * (mtp_loss_sum / mtp_loss_count)
        return main_loss
...
def training_progress_fraction(step: int, elapsed_ms: float, max_wallclock_ms: float | None, iterations: int) -> float:
    if max_wallclock_ms is not None and max_wallclock_ms > 0:
        return min(max(elapsed_ms, 0.0) / max_wallclock_ms, 1.0)
    return min(step / max(iterations, 1), 1.0)


def scheduled_mtp_loss_scale(
    args: Hyperparameters,
    step: int,
    elapsed_ms: float,
    ...
    if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
        return 0.0
    progress = training_progress_fraction(step, elapsed_ms, max_wallclock_ms, args.iterations)
    warmup = min(max(args.mtp_warmup_frac, 0.0), 1.0)
    end = min(max(args.mtp_end_frac, warmup), 1.0)
    decay = min(max(args.mtp_decay_frac, 0.0), end - warmup)
    peak = args.mtp_loss_weight
    if warmup > 0.0 and progress < warmup:
        return peak * (progress / warmup)
    decay_start = end - decay
    if progress >= end:
        return 0.0
    if decay > 0.0 and progress > decay_start:
        return peak * ((end - progress) / decay)
    return peak
...
def run_smoke_test() -> None:
    ...
    target_ids = torch.randint(0, 64, (2, 16))
    model.train()
    loss = model(input_ids, target_ids, torch.tensor(0.05))
    loss.backward()
    export_sd = {k: v.detach().cpu() for k, v in model.state_dict().items() if "mtp_heads" not in k}
    unbanked = _unbank_state_dict(export_sd, model.num_layers)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, model.num_layers, export_sd)
    eval_model = GPT(
        vocab_size=64,
        num_layers=4,
        model_dim=64,
    ...
    if logits.shape != (2, 16, 64):
        raise RuntimeError(f"unexpected smoke logits shape {tuple(logits.shape)}")
    print(f"smoke_test:ok loss:{float(loss):.4f} logits_shape:{tuple(logits.shape)}")
...
def main() -> None:
    ...
    if args.smoke_test:
        ...
        return
    if np is None:
        raise ImportError("numpy is required for the training/evaluation path")
    if spm is None:
        raise ImportError("sentencepiece is required for the training/evaluation path")
    ...
    log0(f"mtp_num_heads:{args.mtp_num_heads} mtp_peak_loss_weight:{args.mtp_loss_weight} mtp_params:{mtp_params}")
    log0(f"mtp_schedule:warmup_frac={args.mtp_warmup_frac:.2f} end_frac={args.mtp_end_frac:.2f} decay_frac={args.mtp_decay_frac:.2f}")
    ...
    if args.warmup_steps > 0:
        ...
        model.train()
        warmup_mtp_scale = torch.zeros((), device=device)
    ...
    while True:
        ...
        mtp_scale = torch.tensor(
            scheduled_mtp_loss_scale(args, step, elapsed_ms, max_wallclock_ms),
            device=device,
            dtype=torch.float32,
        )
        zero_grad_all()
