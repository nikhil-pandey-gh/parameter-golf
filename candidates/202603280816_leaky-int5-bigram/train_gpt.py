    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                ...
                clip_range = int(getattr(self, "qat_clip_range", 31))
                row_max = w32.abs().amax(dim=1)
                scale = (row_max / clip_range).clamp_min(1.0 / clip_range)
                w_q = (
                    torch.clamp(torch.round(w32 / scale[:, None]), -(clip_range + 1), clip_range) * scale[:, None]
                ).to(x.dtype)
            w = w + (w_q - w).detach()
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        ...
        self.fc.qat_clip_range = 15
        self.proj = CastedLinear(hidden, dim, bias=False)
        self.proj.qat_clip_range = 15
        self.proj._zero_init = True
    def forward(self, x: Tensor) -> Tensor:
...
def main() -> None:
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model mixed_int5_int6+{_COMPRESSOR}: {quant_file_bytes} bytes")
        log0(f"Total submission size mixed_int5_int6+{_COMPRESSOR}: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    ...
    torch.cuda.synchronize()
    log0(
        f"final_mixed_int5_int6_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_mixed_int5_int6_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixed_int5_int6_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_mixed_int5_int6_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixed_int5_int6_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(f"final_mixed_int5_int6_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
    if distributed:
