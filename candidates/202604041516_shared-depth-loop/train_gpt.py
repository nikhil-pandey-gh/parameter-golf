...
class Hyperparameters:
    ...
    num_layers = int(os.environ.get("NUM_LAYERS", str(num_unique_layers * num_loops)))
...
class LayerControl(nn.Module):
    def __init__(
        ...
        if dtg:
            ...
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, core: SharedBlockCore, use_xsa: bool, v_embed: Tensor | None = None) -> Tensor:
        mix = self.resid_mix.to(dtype=x.dtype)
        x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = core.attn(self.attn_norm(x_in) * self.ln_scale_factor, v_embed=v_embed, use_xsa=use_xsa)
...
class GPT(nn.Module):
    def __init__(
        ...
        expected_layers = num_unique_layers * num_loops
        if num_layers != expected_layers:
            raise ValueError(
                f"num_layers must equal num_unique_layers * num_loops; "
                f"got num_layers={num_layers}, num_unique_layers={num_unique_layers}, num_loops={num_loops}"
            )
        ...
        self.xsa_layers = set(range(max(0, num_layers - xsa_last_n), num_layers)) if xsa_last_n > 0 else set()
        self._init_weights()
    ...
    def _forward_hidden(self, input_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            core = self.shared_blocks[i % self.num_unique_layers]
            x = self.layer_controls[i](x, x0, core, use_xsa=i in self.xsa_layers, v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            core = self.shared_blocks[bi % self.num_unique_layers]
            x = self.layer_controls[bi](x, x0, core, use_xsa=bi in self.xsa_layers, v_embed=ve)
...
def _is_special_matrix_param(name: str) -> bool:
    return name in {"tok_emb.weight", "bigram.embed.weight", "ve_shared.embed.weight", "lm_head.weight"}
...
def main() -> None:
    ...
    matrix_params = [
        ...
        if (
            ...
            and not _is_special_matrix_param(name)
        )
    ]
    scalar_params = [
        p
    ...
    log0(f"shared_depth:unique_layers:{base_model.num_unique_layers} loops:{base_model.num_loops}")
    log0(f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} mtp_params:{mtp_params}")
    xsa_layers = sorted(base_model.xsa_layers)
    ...
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, num_unique_layers=args.num_unique_layers,
        num_loops=args.num_loops, model_dim=args.model_dim,
