...
class Hyperparameters:
    ...
    bank_qat_percentile = float(os.environ.get("BANK_QAT_PERCENTILE", 1.0))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        q_w = maybe_fake_quantize_bank_weight(q_w, self.training)
        k_w = maybe_fake_quantize_bank_weight(k_w, self.training)
        v_w = maybe_fake_quantize_bank_weight(v_w, self.training)
        out_w = maybe_fake_quantize_bank_weight(out_w, self.training)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        up_w = maybe_fake_quantize_bank_weight(up_w, self.training)
        down_w = maybe_fake_quantize_bank_weight(down_w, self.training)
...
def fake_quantize_int6_ste(t: Tensor, clip_range: int = 31, percentile: float = 1.0) -> Tensor:
    if t.ndim != 2:
        return t
    t32 = t.float()
    if percentile < 1.0:
        row_clip = torch.quantile(t32.detach().abs(), percentile, dim=1)
    else:
        row_clip = t32.detach().abs().amax(dim=1)
    scale = (row_clip / clip_range).clamp_min(1.0 / clip_range)
    q = torch.clamp(torch.round(t32 / scale[:, None]), -clip_range, clip_range)
    deq = q * scale[:, None]
    return t + (deq.to(dtype=t.dtype) - t).detach()


def maybe_fake_quantize_bank_weight(t: Tensor, training: bool) -> Tensor:
    if not training or not BankQATState.enabled:
        return t
    return fake_quantize_int6_ste(t, percentile=BankQATState.percentile)
...
def main() -> None:
    ...
    if use_cuda and 8 % world_size != 0:
        raise ValueError(f"WORLD_SIZE={world_size} must divide 8 so grad_accum_steps stays integral")
    grad_accum_steps = 8 // world_size if use_cuda else 1
    grad_scale = 1.0 / grad_accum_steps
    if not use_cuda and not args.smoke_test:
        raise RuntimeError("CUDA is required for full training/eval; use SMOKE_TEST=1 DEVICE=cpu for a CPU smoke test")
    ...
    if use_cuda:
        ...
        log0("CUDA disabled; smoke-test mode only", console=False)
    log0("=" * 100, console=False)
    ...
    BankQATState.enabled = False
    BankQATState.percentile = args.bank_qat_percentile
    if args.smoke_test:
        seq_len = max(2, min(args.smoke_seq_len, args.train_seq_len))
        smoke_model = GPT(
        ...
        if use_cuda:
            smoke_model = smoke_model.bfloat16()
        ...
        x = torch.randint(args.vocab_size, (args.smoke_batch_size, seq_len), device=device)
        y = torch.randint(args.vocab_size, (args.smoke_batch_size, seq_len), device=device)
        autocast_factory = (lambda: torch.autocast(device_type="cuda", dtype=torch.bfloat16)) if use_cuda else nullcontext
        smoke_model.train()
        with autocast_factory():
            smoke_loss = smoke_model(x, y)
        smoke_loss.backward()
        BankQATState.enabled = args.bank_qat_enabled
        with autocast_factory():
            bank_qat_loss = smoke_model(x, y)
        smoke_logits = smoke_model.forward_logits(x[:, : min(seq_len, 8)])
        log0(
            f"smoke_ok device={device.type} seq_len={seq_len} loss={smoke_loss.item():.6f} "
            f"bank_qat_loss={bank_qat_loss.item():.6f} logits_shape={tuple(smoke_logits.shape)}"
        )
        if distributed:
    ...
    log0(f"bank_qat_enabled:{args.bank_qat_enabled} bank_qat_percentile:{args.bank_qat_percentile}")
    base_model = GPT(
        vocab_size=args.vocab_size,
        num_layers=args.num_layers,
        model_dim=args.model_dim,
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold:
            ...
            if not CastedLinear._qat_enabled:
                ...
                enable_qat = True
            if args.bank_qat_enabled and not BankQATState.enabled:
                BankQATState.enabled = True
                enable_qat = True
            if enable_qat:
                log0(
                    f"late_qat:enabled step:{step} scale:{scale:.4f} "
                    f"linear_qat:{CastedLinear._qat_enabled} bank_qat:{BankQATState.enabled}"
                )
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        base_model.load_state_dict(avg_state, strict=True)
    if CastedLinear._qat_enabled or BankQATState.enabled:
        CastedLinear._qat_enabled = False
        BankQATState.enabled = False
        log0("late_qat:disabled_for_eval_export")
    torch.cuda.synchronize()
