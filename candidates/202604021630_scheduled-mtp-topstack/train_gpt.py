...
class Hyperparameters:
    ...
    mtp_warmup_steps = int(os.environ.get("MTP_WARMUP_STEPS", 250))
    mtp_decay_start_scale = float(os.environ.get("MTP_DECAY_START_SCALE", 0.25))
...
class GPT(nn.Module):
    def __init__(
        ...
        mtp_init_from_main_head: bool = False,
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_init_from_main_head and len(self.mtp_heads) > 0:
            source_weight = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
            with torch.no_grad():
                for head in self.mtp_heads:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_scale: Tensor | float | None = None) -> Tensor:
        n = self.num_layers
        ...
        if mtp_scale is None:
            mtp_weight = float(self.mtp_loss_weight)
            mtp_enabled = True
        elif isinstance(mtp_scale, Tensor):
            mtp_weight = mtp_scale.to(device=x.device, dtype=x.dtype) * float(self.mtp_loss_weight)
            mtp_enabled = True
        else:
            mtp_weight = float(mtp_scale) * float(self.mtp_loss_weight)
            mtp_enabled = mtp_weight > 0.0
        if self.training and self.mtp_num_heads > 0 and mtp_enabled:
...
def main() -> None:
    ...
    if args.mtp_num_heads > 0:
        log0(
            f"mtp_schedule:warmup_steps:{args.mtp_warmup_steps} "
            f"decay_start_scale:{args.mtp_decay_start_scale:.3f} "
            f"init_from_main_head:{args.mtp_init_from_main_head}"
        )
    ...
    def mtp_scale_mul(step: int, lr_scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        warmup_mul = 1.0
        if args.mtp_warmup_steps > 0:
            warmup_mul = min((step + 1) / args.mtp_warmup_steps, 1.0)
        decay_mul = 1.0
        if args.mtp_decay_start_scale > 0.0 and lr_scale < args.mtp_decay_start_scale:
            decay_mul = max(lr_scale, 0.0) / args.mtp_decay_start_scale
        return warmup_mul * decay_mul
    if args.warmup_steps > 0:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        warmup_mtp_scale = torch.ones((), device=device, dtype=torch.float32)
    ...
    while True:
        ...
        mtp_runtime_scale = mtp_scale_mul(step, scale)
        mtp_scale_arg: Tensor | float = 0.0
        if mtp_runtime_scale > 1e-3:
            mtp_scale_arg = torch.full((), mtp_runtime_scale, device=device, dtype=torch.float32)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_scale:{mtp_runtime_scale:.3f}"
            )
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_init_from_main_head=False,
