...
class Hyperparameters:
    ...
    bank_group_size = int(os.environ.get("BANK_GROUP_SIZE", 2))
...
def num_bank_layers_for(num_layers: int, bank_group_size: int) -> int:
    group_size = max(bank_group_size, 1)
    return (num_layers + group_size - 1) // group_size
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bank_group_size = max(bank_group_size, 1)
        self.num_bank_layers = num_bank_layers_for(num_layers, self.bank_group_size)
        self.layer_bank_map = tuple(
            min(i // self.bank_group_size, self.num_bank_layers - 1) for i in range(num_layers)
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_decoder_layers):
            ...
            bi = self.layer_bank_map[block_idx]
            if skips:
                x = x + self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skips.pop()
            ve = self._get_ve(block_idx, input_ids, ve_cache)
            x, _ = self.blocks[block_idx](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[bi], self.mlp_down_bank[bi],
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def _unbank_state_dict(sd: dict[str, Tensor], num_bank_layers: int) -> dict[str, Tensor]:
    """Convert 3D shared bank tensors into individual 2D tensors with standard names."""
    out: dict[str, Tensor] = {}
    n = num_bank_layers
...
def _rebank_state_dict(sd: dict[str, Tensor], num_bank_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert individual 2D tensors back into 3D shared bank tensors."""
    out: dict[str, Tensor] = {}
    n = num_bank_layers
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    model = GPT(
        ...
        bank_group_size=args.bank_group_size,
    )
    ...
    print(
        ...
        f"bank_group_size:{args.bank_group_size} unique_bank_layers:{model.num_bank_layers}"
    )
...
def main() -> None:
    ...
    if missing:
        raise ImportError(f"Training requires missing dependencies: {', '.join(missing)}")
    ...
    using_sdp_fallback = flash_attn_3_func is None
    enable_cudnn_sdp(False)
    enable_flash_sdp(True)
    enable_mem_efficient_sdp(using_sdp_fallback)
    enable_math_sdp(using_sdp_fallback)
    logfile = None
    if master_process:
    ...
    log0(
        f"bank_sharing:group_size:{args.bank_group_size} "
        f"unique_bank_layers:{base_model.num_bank_layers}"
    )
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        f"sdp_backends:cudnn=False flash=True "
        f"mem_efficient={using_sdp_fallback} math={using_sdp_fallback}"
    )
    ...
    num_bank_layers = num_bank_layers_for(args.num_layers, args.bank_group_size)
