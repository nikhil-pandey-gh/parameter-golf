...
class Muon(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            updates_dtype = torch.bfloat16 if params[0].is_cuda else torch.float32
...
def eval_val_sliding(
    ...
    compiled_logits = (
        ...
        if getattr(base_model, "_enable_compile", False)
...
def main() -> None:
    ...
    if torch.cuda.is_available():
        ...
        if distributed and world_size != 1:
            raise RuntimeError("CPU fallback only supports WORLD_SIZE=1")
        distributed = False
        rank = 0
        world_size = 1
        local_rank = 0
    ...
    use_torch_compile = bool(args.enable_torch_compile and device.type == "cuda" and hasattr(torch, "compile"))
    ...
    if device.type == "cuda":
        ...
        # Keep fast kernels for the normal FA3 path, but allow PyTorch SDPA to
        # fall back to broadly available kernels if flash_attn_interface is absent.
    ...
    if device.type == "cuda":
        ...
        log0("Running without CUDA; using SDPA attention fallback and disabling torch.compile.", console=False)
    log0("=" * 100, console=False)
    ...
    base_model._enable_compile = use_torch_compile
    ...
    eval_model._enable_compile = use_torch_compile
    for m in eval_model.modules():
        if isinstance(m, CastedLinear):
            m.float()
    restore_low_dim_params_to_fp32(eval_model)
    eval_model.load_state_dict(deq_state, strict=True)
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=True) if use_torch_compile else eval_model
    sync_device()
