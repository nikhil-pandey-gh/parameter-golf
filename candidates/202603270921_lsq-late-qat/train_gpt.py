...
def flash_attn_3_func(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    y = F.scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, is_causal=causal)
    return y.permute(0, 2, 1, 3).contiguous()


def maybe_compile(fn, *, dynamic: bool = False, fullgraph: bool = True):
...
class Hyperparameters:
    ...
    qat_ramp_span = float(os.environ.get("QAT_RAMP_SPAN", 0.15))
...
class CastedLinear(nn.Linear):
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.lsq_log_scale = nn.Parameter(torch.zeros(out_features, dtype=torch.float32))
        self.register_buffer("qat_blend", torch.zeros((), dtype=torch.float32), persistent=False)

    def set_qat_blend(self, blend: float) -> None:
        self.qat_blend.fill_(float(max(0.0, min(1.0, blend))))

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self.training and w.ndim == 2:
            w32 = self.weight.float()
            base_scale = (w32.detach().abs().amax(dim=1) / 31.0).clamp_min(1.0 / 31.0)
            scale = (base_scale * self.lsq_log_scale.float().clamp(-4.0, 4.0).exp()).clamp_min(1e-6)
            w_scaled = w32 / scale[:, None]
            w_clipped = w_scaled.clamp(-31.0, 31.0)
            w_rounded = (w_clipped.round() - w_clipped).detach() + w_clipped
            w_q = (w_rounded * scale[:, None]).to(x.dtype)
            w = w + (w_q - w) * self.qat_blend.to(dtype=w.dtype)
...
def set_model_qat_blend(module: nn.Module, blend: float) -> None:
    for submodule in module.modules():
        if isinstance(submodule, CastedLinear):
            submodule.set_qat_blend(blend)


def late_qat_blend(args: Hyperparameters, lr_scale: float) -> float:
    if args.qat_enabled:
        return 1.0
    if args.late_qat_threshold <= 0.0:
        return 0.0
    if lr_scale >= args.late_qat_threshold:
        return 0.0
    ramp = args.qat_ramp_span if args.qat_ramp_span > 0.0 else args.late_qat_threshold
    return min((args.late_qat_threshold - lr_scale) / max(ramp, 1e-9), 1.0)
...
def main() -> None:
    ...
    args = Hyperparameters()
    zeropower_via_newtonschulz5 = maybe_compile(zeropower_via_newtonschulz5, fullgraph=False)
    ...
    set_model_qat_blend(base_model, 1.0 if args.qat_enabled else 0.0)
    ...
    if base_model.bigram is not None:
        ...
        if base_model.bigram.proj is not None:
            ...
            scalar_params.append(base_model.bigram.proj.lsq_log_scale)
    if base_model.ve_shared is not None:
        tok_params.append({"params": [base_model.ve_shared.embed.weight], "lr": token_lr, "base_lr": token_lr})
        if base_model.ve_shared.proj is not None:
            matrix_params.append(base_model.ve_shared.proj.weight)
            scalar_params.append(base_model.ve_shared.proj.lsq_log_scale)
        scalar_params.append(base_model.ve_shared.scale)
        for s in base_model.ve_layer_scales:
            scalar_params.append(s)
    if base_model.lm_head is not None:
        scalar_params.append(base_model.lm_head.lsq_log_scale)
    for head in base_model.mtp_heads:
        scalar_params.append(head.lsq_log_scale)
    ...
    log0(
        f"late_qat:threshold:{args.late_qat_threshold:.4f} ramp_span:{args.qat_ramp_span:.4f} "
        f"compile:{int(bool(int(os.environ.get('USE_TORCH_COMPILE', '1'))))}"
    )
    ...
    step = 0
    qat_blend_value = 1.0 if args.qat_enabled else 0.0
    late_qat_logged = args.qat_enabled
    late_qat_full_logged = args.qat_enabled and qat_blend_value >= 1.0
    while True:
        ...
        qat_blend_value = late_qat_blend(args, scale)
        set_model_qat_blend(base_model, qat_blend_value)
        if qat_blend_value > 0.0 and not late_qat_logged:
            late_qat_logged = True
            log0(f"late_qat:blend_started step:{step} scale:{scale:.4f} blend:{qat_blend_value:.3f}")
        if qat_blend_value >= 0.999 and not late_qat_full_logged:
            late_qat_full_logged = True
