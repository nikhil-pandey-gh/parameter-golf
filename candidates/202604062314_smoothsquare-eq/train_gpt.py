...
class Hyperparameters:
    ...
    mlp_equalize_eps = float(os.environ.get("MLP_EQUALIZE_EPS", 1e-6))
...
def equalize_relu2_mlp_state_dict(
    state_dict: dict[str, Tensor],
    num_layers: int,
    max_scale: float,
    eps: float,
) -> tuple[dict[str, Tensor], list[tuple[int, float, float, float]]]:
    out = dict(state_dict)
    stats: list[tuple[int, float, float, float]] = []
    if max_scale <= 1.0:
        return out, stats
    inv_max_scale = 1.0 / max_scale
    for layer_idx in range(num_layers):
        ...
        # relu(x)^2 is degree-2 homogeneous, so balancing the transformed up/down magnitudes
        # gives a cubic-root scale: up_row * s ~= down_col / s^2.
        scale = torch.pow(down_stat / up_stat, 1.0 / 3.0).clamp_(min=inv_max_scale, max=max_scale)
        out[up_name] = (up32 * scale[:, None]).to(dtype=up.dtype)
        out[down_name] = (down32 / scale.square()[None, :]).to(dtype=down.dtype)
        stats.append(
            (
                layer_idx,
...
def main() -> None:
    ...
    if args.mlp_equalize:
        quant_sd_cpu, mlp_equalize_stats = equalize_relu2_mlp_state_dict(
            quant_sd_cpu,
            num_layers=args.num_layers,
            max_scale=args.mlp_equalize_max_scale,
            eps=args.mlp_equalize_eps,
        )
        if mlp_equalize_stats:
            min_scale = min(stat[1] for stat in mlp_equalize_stats)
            med_scale = sum(stat[2] for stat in mlp_equalize_stats) / len(mlp_equalize_stats)
            max_scale = max(stat[3] for stat in mlp_equalize_stats)
            log0(
                "mlp_equalize:enabled "
                f"layers:{len(mlp_equalize_stats)} "
                f"scale_min:{min_scale:.4f} scale_median_mean:{med_scale:.4f} scale_max:{max_scale:.4f}"
            )
            for layer_idx, layer_min, layer_median, layer_max in mlp_equalize_stats:
                log0(
                    f"mlp_equalize:layer:{layer_idx} "
                    f"scale_min:{layer_min:.4f} scale_median:{layer_median:.4f} scale_max:{layer_max:.4f}",
                    console=False,
                )
    if master_process:
