...
class CastedLinear(nn.Linear):
    def __init__(self, *args, **kwargs):
        ...
        self.register_buffer("qat_weight", torch.zeros_like(self.weight, dtype=torch.bfloat16), persistent=False)
    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self.training and w.ndim == 2:
            w = w + (self.qat_weight.to(dtype=x.dtype) - w).detach() * self.qat_mix.to(dtype=x.dtype)
...
def qat_mix_for_scale(always_enabled: bool, threshold: float, scale: float) -> float:
    if always_enabled:
        return 1.0
    if threshold <= 0:
        return 0.0
    return min(max((threshold - scale) / threshold, 0.0), 1.0)
def set_casted_linear_qat_mix(module: nn.Module, mix: float) -> None:
    mix = float(min(max(mix, 0.0), 1.0))
    with torch.no_grad():
        for submodule in module.modules():
            if isinstance(submodule, CastedLinear):
                submodule.qat_mix.fill_(mix)
def refresh_casted_linear_qat_cache(module: nn.Module) -> None:
    with torch.no_grad():
        for submodule in module.modules():
            ...
            w_q = (torch.clamp(torch.round(w32 / scale[:, None]), -32, 31) * scale[:, None]).to(torch.bfloat16)
            submodule.qat_weight.copy_(w_q)
...
def main() -> None:
    ...
    for module in base_model.modules():
        if isinstance(module, CastedLinear):
            module.float()
            module.qat_weight.data = module.qat_weight.data.bfloat16()
    restore_low_dim_params_to_fp32(base_model)
    set_casted_linear_qat_mix(base_model, 1.0 if args.qat_enabled else 0.0)
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            if args.qat_enabled:
                refresh_casted_linear_qat_cache(base_model)
            zero_grad_all()
    ...
    while True:
        ...
        qat_mix = qat_mix_for_scale(args.qat_enabled, args.late_qat_threshold, scale)
        set_casted_linear_qat_mix(base_model, qat_mix)
        if qat_mix > 0:
            refresh_casted_linear_qat_cache(base_model)
        next_qat_mix_bucket = min(int(qat_mix * 4.0 + 1e-8), 4)
        if next_qat_mix_bucket != qat_mix_bucket and (next_qat_mix_bucket > 0 or (step == 0 and args.qat_enabled)):
            qat_mix_bucket = next_qat_mix_bucket
            log0(f"qat_mix:update step:{step} scale:{scale:.4f} mix:{qat_mix:.2f}")
        zero_grad_all()
