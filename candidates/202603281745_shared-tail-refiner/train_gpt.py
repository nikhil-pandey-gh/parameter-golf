...
class Hyperparameters:
    ...
    recurrent_tail_steps = int(os.environ.get("RECURRENT_TAIL_STEPS", 1))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.recurrent_tail_steps = recurrent_tail_steps
        ...
        if self.ve_layer_indices:
            ...
            self.ve_shared = None
            self.ve_layer_scales = nn.ParameterList()
        self.recurrent_tail_step_embs = nn.ParameterList(
            [nn.Parameter(torch.zeros(model_dim, dtype=torch.float32)) for _ in range(recurrent_tail_steps)]
        )
        self.recurrent_tail_ve_scales = nn.ParameterList(
            [nn.Parameter(torch.ones(1, dtype=torch.float32)) for _ in range(recurrent_tail_steps)]
        ) if self.ve_shared is not None else nn.ParameterList()
    ...
    def _init_weights(self) -> None:
        ...
        for step_emb in self.recurrent_tail_step_embs:
            nn.init.normal_(step_emb, mean=0.0, std=0.01)
    ...
    def _get_recurrent_tail_ve(self, step: int, input_ids: Tensor, ve_cache: dict | None = None) -> Tensor | None:
        if self.ve_shared is None or step >= len(self.recurrent_tail_ve_scales):
            return None
        if ve_cache is not None and 've' not in ve_cache:
            ve_cache['ve'] = self.ve_shared(input_ids)
        ve_base = ve_cache['ve'] if ve_cache is not None else self.ve_shared(input_ids)
        return ve_base * self.recurrent_tail_ve_scales[step].to(dtype=ve_base.dtype)
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.recurrent_tail_steps > 0:
            tail_block = self.blocks[-1]
            # Reuse the deepest block as a lightweight refinement cell instead of adding a new full block.
            for step, step_emb in enumerate(self.recurrent_tail_step_embs):
                x = x + step_emb.to(dtype=x.dtype)[None, None, :]
                ve = self._get_recurrent_tail_ve(step, input_ids, ve_cache)
                x = tail_block(x, x0, v_embed=ve)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        recurrent_tail_steps=args.recurrent_tail_steps,
    ).to(device).bfloat16()
    ...
    if base_model.ve_shared is not None:
        ...
        for s in base_model.recurrent_tail_ve_scales:
            scalar_params.append(s)
    for step_emb in base_model.recurrent_tail_step_embs:
        scalar_params.append(step_emb)
    ...
    log0(
        f"recurrent_tail:steps={args.recurrent_tail_steps} "
        f"effective_layer_passes={args.num_layers + args.recurrent_tail_steps}"
    )
