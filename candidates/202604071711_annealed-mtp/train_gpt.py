...
class Hyperparameters:
    ...
    mtp_decay_end_scale = float(os.environ.get("MTP_DECAY_END_SCALE", 0.2))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.register_buffer("mtp_aux_scale", torch.tensor(1.0, dtype=torch.float32), persistent=False)
    ...
    def set_mtp_aux_scale(self, scale: float) -> None:
        self.mtp_aux_scale.fill_(float(min(max(scale, 0.0), 1.0)))
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = self.mtp_aux_scale.to(dtype=main_loss.dtype) * self.mtp_loss_weight
...
def main() -> None:
    ...
    def mtp_aux_scale(lr_scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_decay_end_scale <= 0.0:
            return 1.0
        return min(max(lr_scale / args.mtp_decay_end_scale, 0.0), 1.0)
    if args.warmup_steps > 0:
        initial_model_state = {name: tensor.detach().cpu().clone() for name, tensor in base_model.state_dict().items()}
        initial_optimizer_states = [copy.deepcopy(opt.state_dict()) for opt in optimizers]
        model.train()
        base_model.set_mtp_aux_scale(1.0)
    ...
    while True:
        ...
        current_mtp_aux_scale = mtp_aux_scale(scale)
        base_model.set_mtp_aux_scale(current_mtp_aux_scale)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_aux_scale:{current_mtp_aux_scale:.3f}"
            )
    ...
    base_model.set_mtp_aux_scale(0.0)
