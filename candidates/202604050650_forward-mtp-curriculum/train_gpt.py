...
class Hyperparameters:
    ...
    mtp_disable_lr_scale = float(os.environ.get("MTP_DISABLE_LR_SCALE", 0.25))
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if self.training and active_mtp_heads > 0 and mtp_weights is not None:
            _, seqlen, dim = x.shape
            mtp_loss_sum = main_loss.new_zeros(())
            mtp_weights = mtp_weights.to(device=main_loss.device, dtype=main_loss.dtype)
            for k, mtp_head in enumerate(self.mtp_heads[:active_mtp_heads]):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
...
def build_mtp_weights(
    ...
    if (
        args.mtp_loss_weight <= 0.0
        or progress < args.mtp_curriculum_start_frac
        or lr_scale <= args.mtp_disable_lr_scale
    ):
        return weights, 0
    span = max(args.mtp_curriculum_end_frac - args.mtp_curriculum_start_frac, 1e-6)
    curriculum = min(max((progress - args.mtp_curriculum_start_frac) / span, 0.0), 1.0)
    per_head = args.mtp_loss_weight / args.mtp_num_heads
    head_progress = curriculum * args.mtp_num_heads
    active_heads = 0
    for i in range(args.mtp_num_heads):
        head_weight = per_head * min(max(head_progress - i, 0.0), 1.0)
        weights[i] = head_weight
        if head_weight > 0.0:
            active_heads = i + 1
    return weights, active_heads
...
def main() -> None:
    ...
    log0(
        f"mtp_curriculum:start={args.mtp_curriculum_start_frac:.2f} "
        f"end={args.mtp_curriculum_end_frac:.2f} disable_lr_scale:{args.mtp_disable_lr_scale:.2f}"
    )
    ...
    if args.warmup_steps > 0:
        warmup_mtp_weights, warmup_active_mtp_heads = build_mtp_weights(
            args,
            progress=0.0,
            lr_scale=1.0,
            device=device,
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, warmup_mtp_weights, warmup_active_mtp_heads)
    ...
    while True:
        ...
        mtp_weights, active_mtp_heads = build_mtp_weights(args, progress, scale, device)
        ...
        for micro_step in range(grad_accum_steps):
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, mtp_weights, active_mtp_heads)
        ...
        if should_log_train:
            mtp_total_weight = float(mtp_weights.sum().item()) if mtp_weights.numel() else 0.0
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms "
                f"mtp_weight:{mtp_total_weight:.3f} mtp_heads:{active_mtp_heads}"
            )
