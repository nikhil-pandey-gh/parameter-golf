...
class Hyperparameters:
    ...
    ttt_keep_ratio = float(os.environ.get("TTT_KEEP_RATIO", 0.5))
    ttt_anchor_mix = float(os.environ.get("TTT_ANCHOR_MIX", 0.002))
...
def select_low_entropy_sequence_indices(seq_entropy: Tensor, keep_ratio: float) -> Tensor:
    if not (0.0 < keep_ratio <= 1.0):
        raise ValueError(f"TTT_KEEP_RATIO must be in (0, 1], got {keep_ratio}")
    num_seqs = int(seq_entropy.numel())
    if num_seqs <= 0:
        raise ValueError("TTT sequence selection requires at least one sequence")
    if keep_ratio >= 1.0 or num_seqs == 1:
        return torch.arange(num_seqs, device=seq_entropy.device)
    keep = max(1, math.ceil(num_seqs * keep_ratio))
    return torch.topk(seq_entropy, k=keep, largest=False, sorted=False).indices


def select_low_entropy_local_indices(
    seq_entropy: Tensor,
    keep_ratio: float,
    rank: int,
    world_size: int,
    ...
    if world_size <= 1 or not dist.is_available() or not dist.is_initialized():
        return select_low_entropy_sequence_indices(seq_entropy, keep_ratio)
    local_count = torch.tensor([seq_entropy.numel()], device=seq_entropy.device, dtype=torch.int64)
    ...
    if max_count <= 0:
        raise ValueError("Distributed TTT selection requires at least one local sequence")
    padded = torch.full((max_count,), float("inf"), device=seq_entropy.device, dtype=seq_entropy.dtype)
    if seq_entropy.numel() > 0:
        padded[: seq_entropy.numel()] = seq_entropy
    gathered_entropy = [torch.empty_like(padded) for _ in range(world_size)]
    dist.all_gather(gathered_entropy, padded)
    global_entropy = torch.cat(
        [entropy[:count] for entropy, count in zip(gathered_entropy, counts) if count > 0],
        dim=0,
    )
    global_keep = select_low_entropy_sequence_indices(global_entropy, keep_ratio)
    local_offset = sum(counts[:rank])
    local_mask = (global_keep >= local_offset) & (global_keep < local_offset + seq_entropy.numel())
    return global_keep[local_mask] - local_offset


@torch.no_grad()
def apply_anchor_pullback_(params: list[nn.Parameter], anchor_params: list[Tensor], anchor_mix: float) -> None:
    if not (0.0 <= anchor_mix < 1.0):
        raise ValueError(f"TTT_ANCHOR_MIX must be in [0, 1), got {anchor_mix}")
    if anchor_mix == 0.0:
        return
    for param, anchor in zip(params, anchor_params):
        param.lerp_(anchor, anchor_mix)


def zero_ttt_loss(params: list[nn.Parameter]) -> Tensor:
    if not params:
        raise ValueError("TTT zero-loss helper requires at least one parameter")
    loss = params[0].float().sum() * 0.0
    for param in params[1:]:
        loss = loss + param.float().sum() * 0.0
    return loss
...
def eval_val_sliding_ttt(
    ...
    """Legal score-first TTT with filtered updates and anchor pullback.

    Score each chunk with sliding windows under inference mode, then adapt on the
    already-scored chunk only. Updates keep the lowest-entropy sequences across
    the distributed TTT mini-batch and optionally pull weights back toward the
    pre-TTT checkpoint to limit drift across many adaptation chunks.
    """
    ...
    log0(f"ttt_sliding:start chunks={num_chunks} chunk_tokens={ttt_chunk} "
         ...
         f"keep_ratio={args.ttt_keep_ratio} anchor_mix={args.ttt_anchor_mix}")
    ...
    if not (0.0 < args.ttt_keep_ratio <= 1.0):
        raise ValueError(f"TTT_KEEP_RATIO must be in (0, 1], got {args.ttt_keep_ratio}")
    if not (0.0 <= args.ttt_anchor_mix < 1.0):
        raise ValueError(f"TTT_ANCHOR_MIX must be in [0, 1), got {args.ttt_anchor_mix}")

    optimizer = torch.optim.SGD(ttt_params, lr=args.ttt_lr, momentum=args.ttt_momentum)
    ttt_anchor_params = [p.detach().clone() for p in ttt_params] if args.ttt_anchor_mix > 0.0 else []
    ...
    for ci in range(num_chunks):
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                max_chunk_seqs = torch.tensor([my_chunk_seqs], device=device, dtype=torch.int64)
                if world_size > 1 and dist.is_available() and dist.is_initialized():
                    dist.all_reduce(max_chunk_seqs, op=dist.ReduceOp.MAX)
                max_chunk_seqs_int = int(max_chunk_seqs.item())
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, max_chunk_seqs_int, args.ttt_batch_seqs):
                        local_batch_start = my_seq_s + bs
                        local_batch_end = min(my_seq_s + bs + args.ttt_batch_seqs, my_seq_e)
                        local_batch_count = max(local_batch_end - local_batch_start, 0)
                        optimizer.zero_grad(set_to_none=True)
                        if local_batch_count > 0:
                            start_tok = chunk_start + local_batch_start * seq_len
                            end_tok = chunk_start + local_batch_end * seq_len + 1
                            if end_tok > val_tokens.numel():
                                raise ValueError("TTT local batch extends past validation tokens")
                            ...
                            seq_nll = F.nll_loss(
                                ...
                                y.reshape(-1),
                                reduction="none",
                            ).reshape(x.size(0), seq_len).mean(dim=1)
                            seq_entropy = -(log_probs.exp() * log_probs).sum(dim=-1).mean(dim=1)
                        else:
                            seq_nll = torch.empty((0,), device=device, dtype=torch.float32)
                            seq_entropy = torch.empty((0,), device=device, dtype=torch.float32)
                        kept_count = int(seq_nll.numel())
                        if args.ttt_keep_ratio < 1.0:
                            keep_idx = select_low_entropy_local_indices(seq_entropy, args.ttt_keep_ratio, rank, world_size)
                            kept_count = int(keep_idx.numel())
                            seq_nll = seq_nll.index_select(0, keep_idx) if kept_count > 0 else seq_nll[:0]
                        local_loss_sum = seq_nll.sum() if kept_count > 0 else zero_ttt_loss(ttt_params)
                        total_kept = torch.tensor([kept_count], device=device, dtype=torch.int64)
                        if world_size > 1 and dist.is_available() and dist.is_initialized():
                            dist.all_reduce(total_kept, op=dist.ReduceOp.SUM)
                        total_kept_int = int(total_kept.item())
                        if total_kept_int <= 0:
                            raise ValueError("TTT filtering kept zero sequences globally")
                        loss = local_loss_sum / total_kept_int
                        loss.backward()
                        if world_size > 1:
                            for p in ttt_params:
                        ...
                        optimizer.step()
                        if args.ttt_anchor_mix > 0.0:
                            apply_anchor_pullback_(ttt_params, ttt_anchor_params, args.ttt_anchor_mix)
