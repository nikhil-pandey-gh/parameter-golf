...
class Hyperparameters:
    # Candidate: selective deep head gates + normalized value highway on the 2026-03-23 stack.
    ...
    gated_attention_last_n = int(os.environ.get("GATED_ATTENTION_LAST_N", 4))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_logits",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if value_residual:
            self.vr_logits = nn.Parameter(torch.zeros(2, dtype=torch.float32))
    ...
    def forward(
        ...
        raw_v = v if (self.value_residual or capture_v) else None
        if self.value_residual and v0 is not None:
            lam = torch.softmax(self.vr_logits, dim=0).to(dtype=v.dtype)
            v = lam[0] * v0 + lam[1] * v
...
class GPT(nn.Module):
    def __init__(
        ...
        self.value_residual = value_residual or value_residual_last_n > 0
        self.value_residual_anchor_layer = (
            max(0, num_layers - value_residual_last_n - 1)
            if (not value_residual and value_residual_last_n > 0)
            else None
        )
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    gated_attention=(gated_attention or (gated_attention_last_n > 0 and i >= num_layers - gated_attention_last_n)),
                    value_residual=(value_residual or (value_residual_last_n > 0 and i >= num_layers - value_residual_last_n)),
                )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            capture_v = self.value_residual_anchor_layer == i and v0 is None
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0, capture_v=capture_v)
        ...
        for i in range(self.num_decoder_layers):
            ...
            capture_v = self.value_residual_anchor_layer == bi and v0 is None
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        gated_attention_last_n=args.gated_attention_last_n,
    ...
    eval_model = GPT(
        ...
        gated_attention_last_n=args.gated_attention_last_n, value_residual_last_n=args.value_residual_last_n,
    ).to(device).bfloat16()
