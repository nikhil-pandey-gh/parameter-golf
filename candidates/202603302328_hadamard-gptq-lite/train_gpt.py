...
class Hyperparameters:
    ...
    hadamard_block_size = int(os.environ.get("HADAMARD_BLOCK_SIZE", 128))
...
def _block_hadamard_last_dim(t: Tensor, block_size: int) -> Tensor:
    if t.ndim != 2:
        raise ValueError(f"Hadamard rotation expects a 2D tensor, got shape {tuple(t.shape)}")
    ...
    if t.shape[-1] % block_size != 0:
        raise ValueError(
            f"Tensor with shape {tuple(t.shape)} is incompatible with HADAMARD_BLOCK_SIZE={block_size}"
        )
    blocks = t.float().reshape(t.shape[0], t.shape[1] // block_size, block_size).contiguous()
    h = 1
    while h < block_size:
        blocks = blocks.view(t.shape[0], t.shape[1] // block_size, block_size // (2 * h), 2, h)
        left = blocks[..., 0, :].clone()
        right = blocks[..., 1, :].clone()
        blocks[..., 0, :] = left + right
        blocks[..., 1, :] = left - right
        blocks = blocks.view(t.shape[0], t.shape[1] // block_size, block_size)
        h *= 2
    return (blocks / math.sqrt(block_size)).view_as(t).contiguous()


def _maybe_rotate_for_int6(t: Tensor, block_size: int, enabled: bool) -> tuple[Tensor, dict[str, object] | None]:
    t32 = t.float()
    if not enabled or t32.ndim != 2:
        return t32, None
    if block_size <= 1:
        return t32, None
    if not _is_power_of_two(block_size):
        raise ValueError(f"HADAMARD_BLOCK_SIZE must be a power of two, got {block_size}")
    if t32.shape[1] < block_size or t32.shape[1] % block_size != 0:
        return t32, None
    return _block_hadamard_last_dim(t32, block_size), {
        "kind": "hadamard",
        "axis": 1,
...
def _invert_int6_transform(t: Tensor, info: dict[str, object]) -> Tensor:
    transform = info.get("transform")
    if not isinstance(transform, dict):
        return t
    kind = transform.get("kind")
    if kind != "hadamard":
        raise ValueError(f"Unsupported int6 transform kind: {kind}")
    block_size = int(transform["block_size"])
    return _block_hadamard_last_dim(t.float(), block_size)
...
def quantize_int6_per_row(
    ...
    *,
    ...
    t32, transform = _maybe_rotate_for_int6(t, hadamard_block_size, hadamard_gptq)
    if t32.ndim == 2:
        ...
        return best_q, best_s, transform
    amax = t32.abs().max().item()
    scale = torch.tensor(amax / clip_range if amax > 0 else 1.0, dtype=torch.float16)
    q = torch.clamp(torch.round(t32 / scale.float()), -clip_range, clip_range).to(torch.int8)
    return q, scale, transform
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, transform = quantize_int6_per_row(
                t,
                hadamard_gptq=Hyperparameters.hadamard_gptq,
                hadamard_block_size=Hyperparameters.hadamard_block_size,
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
            if transform is not None:
                meta[name]["transform"] = transform
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict):
            deq = _invert_int6_transform(deq, info)
