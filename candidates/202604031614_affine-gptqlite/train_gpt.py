...
try:
    ...
    _COMPRESSOR = "zlib"
_SMOKE_TEST_MODE = os.environ.get("SMOKE_TEST", "0") == "1"
if _SMOKE_TEST_MODE:
    np = None
    spm = None
else:
...
AFFINE_INT6_CLIP_PCTS = (0.9990, 0.9995, 0.9999, 0.99999, 1.0)
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        zero = torch.zeros((t32.shape[0],), dtype=t32.dtype, device=t32.device)
        mean = t32.mean(dim=1)
        midrange = 0.5 * (t32.amax(dim=1) + t32.amin(dim=1))
        best_q, best_s, best_offset, best_err = None, None, None, float("inf")
        for row_offset in (zero, mean, midrange):
            centered = t32 - row_offset[:, None]
            centered_abs = centered.abs()
            for pct in AFFINE_INT6_CLIP_PCTS:
                if pct < 1.0:
                    row_clip = torch.quantile(centered_abs, pct, dim=1)
                else:
                    row_clip = centered_abs.amax(dim=1)
                s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
                q = torch.clamp(torch.round(centered / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
                recon = q.float() * s.float()[:, None] + row_offset[:, None]
                err = (t32 - recon).pow(2).mean().item()
                if err < best_err:
                    best_q, best_s, best_offset, best_err = q, s, row_offset.to(torch.float16), err
        return best_q, best_s, best_offset
    offset = torch.tensor(float(t32.mean().item()) if t32.numel() else 0.0, dtype=torch.float16)
    centered = t32 - float(offset.item())
    amax = centered.abs().max().item()
    scale = torch.tensor(amax / clip_range if amax > 0 else 1.0, dtype=torch.float16)
    q = torch.clamp(torch.round(centered / scale.float()), -clip_range, clip_range).to(torch.int8)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, offset = quantize_int6_per_row(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            result[name + ".offset"] = offset
            meta[name] = {"type": "int6_affine"}
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        offset = result.get(name + ".offset")
        if s.ndim > 0:
            restored = q.float() * s.float().view(q.shape[0], *([1] * (q.ndim - 1)))
            if offset is not None:
                restored = restored + offset.float().view(q.shape[0], *([1] * (q.ndim - 1)))
            out[name] = restored.to(orig_dtype)
        else:
            restored = q.float() * float(s.item())
            if offset is not None:
                restored = restored + float(offset.item())
            out[name] = restored.to(orig_dtype)
    return out
...
def run_smoke_test() -> None:
    ...
    unbanked_sd = _unbank_state_dict(export_sd, 2)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, 2, export_sd)
    eval_model = GPT(
        vocab_size=128,
        num_layers=2,
        model_dim=64,
    ...
    eval_model.eval()
    with torch.no_grad():
        q_loss = eval_model(x, y)
    if not torch.isfinite(loss):
        raise RuntimeError(f"smoke_test:non_finite loss:{loss.item()}")
    if not torch.isfinite(q_loss):
        raise RuntimeError(f"smoke_test:non_finite quant_loss:{q_loss.item()}")
    loss_gap = abs(q_loss.item() - loss.item())
    if loss_gap > 0.1:
        raise RuntimeError(f"smoke_test:quant_gap_too_large gap:{loss_gap:.6f}")
    print(
        f"smoke_test:ok loss:{loss.item():.6f} quant_loss:{q_loss.item():.6f} "
        f"flash_attn:{int(flash_attn_3_func is not None)}"
    )
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for full training/eval runs; use SMOKE_TEST=1 for a CPU sanity check")
    ...
    log0(f"sdp_backends:{sdp_backends}")
    log0(f"flash_attn_available:{int(flash_attn_3_func is not None)}")
