...
class Hyperparameters:
    ...
    shared_mlp_layers = int(os.environ.get("SHARED_MLP_LAYERS", 4))
...
def tensor_alias_key(t: Tensor) -> tuple[int, int, tuple[int, ...], str]:
    return (
...
def build_tensor_aliases(state_dict: dict[str, Tensor]) -> dict[str, str]:
    canonical_by_key: dict[tuple[int, int, tuple[int, ...], str], str] = {}
...
class GPT(nn.Module):
    def __init__(
        ...
        self.shared_mlp_layers = min(max(shared_mlp_layers, 0), self.num_encoder_layers)
        ...
        if self.shared_mlp_layers > 1:
            ...
            for i in range(1, self.shared_mlp_layers):
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("type") == "alias":
            alias_targets.append((name, info["target"]))
            continue
    ...
    for name, target in alias_targets:
        if target not in out:
            raise KeyError(f"Missing alias target {target} for {name}")
        out[name] = out[target]
    return out
def main() -> None:
    ...
    if flash_attn_3_func is None:
        ...
        sdp_backend_summary = "cudnn=False flash=True mem_efficient=True math=True attention=sdpa_fallback"
    else:
        enable_cudnn_sdp(False)
        enable_flash_sdp(True)
        enable_mem_efficient_sdp(False)
        enable_math_sdp(False)
        sdp_backend_summary = "cudnn=False flash=True mem_efficient=False math=False attention=flash_attn_3"
    logfile = None
    if master_process:
    ...
    log0(
        f"shared_mlp_layers:{base_model.shared_mlp_layers} bigram_vocab_size:{args.bigram_vocab_size} "
        f"leaky_relu_slope:{args.leaky_relu_slope}"
    )
    ...
    export_aliases = build_tensor_aliases(export_sd)
    if export_aliases:
        log0(f"export_tensor_aliases:{len(export_aliases)}")
    if master_process:
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, aliases=export_aliases)
    ...
    eval_model = GPT(
        ...
        shared_mlp_layers=args.shared_mlp_layers, leaky_relu_slope=args.leaky_relu_slope,
    ).to(device).bfloat16()
