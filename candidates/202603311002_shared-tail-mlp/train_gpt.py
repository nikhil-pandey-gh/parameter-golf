...
class Hyperparameters:
    ...
    shared_mlp_tail_layers = int(os.environ.get("SHARED_MLP_TAIL_LAYERS", 4))
    shared_mlp_tail_banks = int(os.environ.get("SHARED_MLP_TAIL_BANKS", 2))
    shared_mlp_repeat_scale = float(os.environ.get("SHARED_MLP_REPEAT_SCALE", 0.5))
...
def _build_shared_mlp_bank_indices(num_layers: int, shared_tail_layers: int, shared_tail_banks: int) -> list[int]:
    if shared_tail_layers < 0 or shared_tail_layers > num_layers:
        raise ValueError(
            f"SHARED_MLP_TAIL_LAYERS must be in [0, {num_layers}], got {shared_tail_layers}"
        )
    if shared_tail_layers == 0:
        return list(range(num_layers))
    if shared_tail_banks <= 0 or shared_tail_banks > shared_tail_layers:
        raise ValueError(
            "SHARED_MLP_TAIL_BANKS must be positive and no larger than SHARED_MLP_TAIL_LAYERS"
        )
    tail_start = num_layers - shared_tail_layers
    return list(range(tail_start)) + [
        tail_start + (i % shared_tail_banks) for i in range(shared_tail_layers)
    ]
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_dim: int = 128,
        shared_mlp_tail_layers: int = 0,
        shared_mlp_tail_banks: int = 0,
        shared_mlp_repeat_scale: float = 1.0,
        xsa_last_n: int = 0,
        rope_dims: int = 0,
        ln_scale: bool = False,
        dtg: bool = False,
        ...
        if shared_mlp_tail_layers > self.num_decoder_layers:
            raise ValueError(
                "SHARED_MLP_TAIL_LAYERS must stay within the decoder tail so encoder/skip capacity remains unique"
            )
        self.mlp_bank_indices = tuple(
            _build_shared_mlp_bank_indices(
                num_layers,
                shared_mlp_tail_layers,
                shared_mlp_tail_banks,
            )
        )
        self.num_mlp_banks = max(self.mlp_bank_indices, default=-1) + 1
        self.shared_mlp_repeat_scale = shared_mlp_repeat_scale
    ...
    def _init_weights(self) -> None:
        ...
        if self.shared_mlp_repeat_scale != 1.0:
            seen_mlp_banks: set[int] = set()
            for layer_idx, bank_idx in enumerate(self.mlp_bank_indices):
                if bank_idx in seen_mlp_banks:
                    self.blocks[layer_idx].mlp_scale.data.mul_(self.shared_mlp_repeat_scale)
                else:
                    seen_mlp_banks.add(bank_idx)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            mlp_bank_idx = self.mlp_bank_indices[i]
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[mlp_bank_idx], self.mlp_down_bank[mlp_bank_idx],
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            mlp_bank_idx = self.mlp_bank_indices[bi]
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[mlp_bank_idx], self.mlp_down_bank[mlp_bank_idx],
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
    ...
    for name, tensor in sd.items():
        if name == "qo_bank":
            ...
            for i in range(tensor.shape[0]):
                out[f"shared_mlp.{i}.fc.weight"] = tensor[i]
        elif name == "mlp_down_bank":
            for i in range(tensor.shape[0]):
                out[f"shared_mlp.{i}.proj.weight"] = tensor[i]
        else:
            out[name] = tensor
    return out
...
def _rebank_state_dict(sd: dict[str, Tensor], num_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    ...
    qo_slices = [None] * (2 * n)
    kv_slices = [None] * (2 * n)
    mlp_bank_count = template_sd["mlp_up_bank"].shape[0]
    up_slices = [None] * mlp_bank_count
    down_slices = [None] * mlp_bank_count
    consumed = set()
    for i in range(n):
        ...
        if vk in sd:
            kv_slices[n + i] = sd[vk]
            consumed.add(vk)
    for i in range(mlp_bank_count):
        fk = f"shared_mlp.{i}.fc.weight"
        if fk in sd:
            up_slices[i] = sd[fk]
            consumed.add(fk)
        dk = f"shared_mlp.{i}.proj.weight"
        if dk in sd:
            down_slices[i] = sd[dk]
            consumed.add(dk)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_mlp_tail_layers=args.shared_mlp_tail_layers,
        shared_mlp_tail_banks=args.shared_mlp_tail_banks,
        shared_mlp_repeat_scale=args.shared_mlp_repeat_scale,
    ...
    log0(
        f"shared_mlp_tail_layers:{args.shared_mlp_tail_layers} "
        f"shared_mlp_tail_banks:{args.shared_mlp_tail_banks} "
        f"shared_mlp_repeat_scale:{args.shared_mlp_repeat_scale} "
        f"unique_mlp_banks:{base_model.num_mlp_banks}"
    )
