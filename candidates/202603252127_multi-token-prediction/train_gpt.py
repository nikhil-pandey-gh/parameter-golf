...
def attention_backend(q: Tensor, k: Tensor, v: Tensor, *, is_causal: bool, enable_gqa: bool) -> Tensor:
    if _HAS_FLASH_ATTN and q.is_cuda and k.is_cuda and v.is_cuda and flash_attn_3_func is not None:
        return flash_attn_3_func(q, k, v, causal=is_causal)
    q_t = q.transpose(1, 2)
    k_t = k.transpose(1, 2)
    v_t = v.transpose(1, 2)
    y = F.scaled_dot_product_attention(q_t, k_t, v_t, attn_mask=None, is_causal=is_causal, enable_gqa=enable_gqa)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        y = attention_backend(q, k, v, is_causal=True, enable_gqa=(self.num_kv_heads != self.num_heads))
        if self.use_xsa:
...
def main() -> None:
    ...
    attn_backend_name = "flash_attn_3" if _HAS_FLASH_ATTN else "torch_sdpa_fallback"
    log0(
    ...
    log0(f"attention_backend:{attn_backend_name}")
