...
class Hyperparameters:
    ...
    hadamard_block_sizes = os.environ.get("HADAMARD_BLOCK_SIZES", "512,256,128")
...
# --- Rotation-aware GPTQ-lite int6 quantization ---
...
def _parse_hadamard_block_sizes(spec: str) -> tuple[int, ...]:
    ...
    for raw in spec.split(","):
        ...
        if block_size <= 0 or block_size & (block_size - 1):
            raise ValueError(f"HADAMARD_BLOCK_SIZES entries must be positive powers of two, got {block_size}")
        block_sizes.append(block_size)
    return tuple(dict.fromkeys(block_sizes))


def _normalized_hadamard_last_dim(x: Tensor) -> Tensor:
    ...
    h = 1
    while h < n:
        y = y.reshape(*orig_shape[:-1], -1, 2 * h)
        a = y[..., :h].clone()
        b = y[..., h : 2 * h].clone()
        y[..., :h] = a + b
        y[..., h : 2 * h] = a - b
...
def _apply_block_hadamard_columns(t: Tensor, block_size: int) -> Tensor:
    if t.ndim != 2:
        raise ValueError(f"Block Hadamard rotation expects a 2D tensor, got shape {tuple(t.shape)}")
    if t.shape[1] % block_size != 0:
        raise ValueError(f"Tensor with shape {tuple(t.shape)} is not divisible by block_size={block_size}")
    blocks = t.float().contiguous().reshape(t.shape[0], t.shape[1] // block_size, block_size)
    return _normalized_hadamard_last_dim(blocks).reshape_as(t).contiguous()
...
def _quantize_int6_rowwise_candidate(t32: Tensor, clip_range: int) -> tuple[Tensor, Tensor, float]:
...
def quantize_int6_per_row(
    t: Tensor, clip_range: int = 31, hadamard_block_sizes: tuple[int, ...] = ()
) -> tuple[Tensor, Tensor, dict[str, object] | None]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = _quantize_int6_rowwise_candidate(t32, clip_range)
        best_rotation = None
        for block_size in hadamard_block_sizes:
            if block_size > t32.shape[1] or t32.shape[1] % block_size != 0:
                continue
            rotated = _apply_block_hadamard_columns(t32, block_size)
            q, s, _ = _quantize_int6_rowwise_candidate(rotated, clip_range)
            recon = _apply_block_hadamard_columns(_dequantize_rowwise(q, s), block_size)
            err = (t32 - recon).pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_err = q, s, err
                best_rotation = {"kind": "hadamard_col", "block_size": block_size}
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    *,
    hadamard_gptq: bool = False,
    hadamard_block_sizes: tuple[int, ...] = (),
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, rotation = quantize_int6_per_row(
                t,
                hadamard_block_sizes=hadamard_block_sizes if hadamard_gptq else (),
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            deq = _dequantize_rowwise(q, s)
        else:
            deq = q.float() * float(s.item())
        if isinstance(info, dict):
            rotation = info.get("rotation")
            if rotation is not None:
                if rotation.get("kind") != "hadamard_col":
                    raise ValueError(f"Unsupported rotation metadata for {name}: {rotation}")
                deq = _apply_block_hadamard_columns(deq, int(rotation["block_size"]))
...
def main() -> None:
    ...
    args = Hyperparameters()
    hadamard_block_sizes = _parse_hadamard_block_sizes(args.hadamard_block_sizes) if args.hadamard_gptq else ()
    ...
    log0(f"hadamard_gptq:{args.hadamard_gptq} block_sizes:{list(hadamard_block_sizes)}")
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        hadamard_gptq=args.hadamard_gptq,
        hadamard_block_sizes=hadamard_block_sizes,
    )
    hadamard_usage: dict[int, int] = {}
    for info in quant_meta.values():
        if isinstance(info, dict):
            rotation = info.get("rotation")
            if rotation is not None:
                block_size = int(rotation["block_size"])
                hadamard_usage[block_size] = hadamard_usage.get(block_size, 0) + 1
    if hadamard_usage:
        usage_summary = ", ".join(f"{block}:{count}" for block, count in sorted(hadamard_usage.items()))
        log0(f"hadamard_gptq:selected_blocks {usage_summary}")
    else:
        log0("hadamard_gptq:selected_blocks none")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        log0(f"final_int6_lzma_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        log0(f"final_int6_lzma_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
