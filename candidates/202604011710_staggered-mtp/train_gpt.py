...
class Hyperparameters:
    ...
    mtp_stage_frac = float(os.environ.get("MTP_STAGE_FRAC", 0.15))
...
def compute_mtp_head_weights(args: Hyperparameters, step: int) -> list[float]:
    if args.mtp_num_heads <= 0:
        return []
    if args.mtp_loss_weight <= 0.0:
        return [0.0] * args.mtp_num_heads
    total_steps = max(args.iterations, 1)
    start_step = max(int(args.mtp_start_frac * total_steps), 0)
    stage_steps = max(int(args.mtp_stage_frac * total_steps), 1)
    head_weights: list[float] = []
    for head_idx in range(args.mtp_num_heads):
        final_weight = args.mtp_loss_weight * (args.mtp_head_decay ** head_idx)
        head_progress = (step - start_step - head_idx * stage_steps) / stage_steps
        clamped_progress = min(max(head_progress, 0.0), 1.0)
        head_weights.append(final_weight * clamped_progress)
    return head_weights
...
class GPT(nn.Module):
    def __init__(
        ...
        # Keep the live curriculum weights out of the exported checkpoint/state_dict.
    ...
    def set_mtp_head_weights(self, head_weights: Tensor) -> None:
        if head_weights.numel() != self.mtp_num_heads:
            raise ValueError(
                f"Expected {self.mtp_num_heads} MTP head weights, got shape {tuple(head_weights.shape)}"
            )
        self.mtp_head_weights.copy_(
            head_weights.to(device=self.mtp_head_weights.device, dtype=self.mtp_head_weights.dtype)
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            main_loss = main_loss + mtp_loss_sum
        return main_loss
...
def main() -> None:
    ...
    if args.mtp_head_decay <= 0.0:
        raise ValueError(f"MTP_HEAD_DECAY must be positive, got {args.mtp_head_decay}")
    if not 0.0 <= args.mtp_start_frac <= 1.0:
        raise ValueError(f"MTP_START_FRAC must be in [0, 1], got {args.mtp_start_frac}")
    if not 0.0 < args.mtp_stage_frac <= 1.0:
        raise ValueError(f"MTP_STAGE_FRAC must be in (0, 1], got {args.mtp_stage_frac}")
    ...
    if args.mtp_num_heads > 0:
        base_model.set_mtp_head_weights(torch.tensor(compute_mtp_head_weights(args, 0), device=device))
    ...
    mtp_final_weights = [args.mtp_loss_weight * (args.mtp_head_decay ** i) for i in range(args.mtp_num_heads)]
    log0(f"model_params:{n_params}")
    log0(f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} mtp_params:{mtp_params}")
    log0(
        f"mtp_schedule:start_frac:{args.mtp_start_frac:.3f} "
        f"stage_frac:{args.mtp_stage_frac:.3f} head_decay:{args.mtp_head_decay:.3f} "
        f"final_weights:{mtp_final_weights}"
    )
    ...
    t0 = time.perf_counter()
    step = 0
    current_mtp_weights = compute_mtp_head_weights(args, step)
    while True:
        ...
        current_mtp_weights = compute_mtp_head_weights(args, step)
        if args.mtp_num_heads > 0:
            base_model.set_mtp_head_weights(torch.tensor(current_mtp_weights, device=device))
        zero_grad_all()
        ...
        if should_log_train:
            mtp_log_suffix = ""
            if current_mtp_weights:
                mtp_log_suffix = f" mtp_weights:{','.join(f'{weight:.4f}' for weight in current_mtp_weights)}"
            log0(
