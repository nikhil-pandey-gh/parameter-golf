...
class Hyperparameters:
    ...
    shared_kv_start = int(os.environ.get("SHARED_KV_START", 7))
    shared_kv_banks = int(os.environ.get("SHARED_KV_BANKS", 2))
...
def collect_parameter_aliases(module: nn.Module) -> dict[str, str]:
    seen: dict[int, str] = {}
    aliases: dict[str, str] = {}
    for name, param in module.named_parameters(remove_duplicate=False):
        param_id = id(param)
        canonical = seen.get(param_id)
...
class GPT(nn.Module):
    def __init__(
        ...
        ve_dim: int = 128,
        ve_layers: str = "9,10",
        shared_kv_start: int = -1,
        shared_kv_banks: int = 0,
        mlp_leaky_slope: float = 0.5,
    ):
        super().__init__()
        ...
        self.shared_kv_groups = self._share_late_kv(shared_kv_start, shared_kv_banks)
        if rope_dims > 0:
        ...
        self._init_weights()
    def _share_late_kv(self, start: int, banks: int) -> tuple[tuple[int, ...], ...]:
        if banks <= 0 or start >= len(self.blocks):
            return ()
        layer_indices = list(range(max(start, 0), len(self.blocks)))
        if len(layer_indices) <= banks:
            return ()
        groups = [tuple(layer_indices[offset::banks]) for offset in range(min(banks, len(layer_indices)))]
        shared_groups = tuple(group for group in groups if len(group) > 1)
        for group in shared_groups:
            base_attn = self.blocks[group[0]].attn
            for layer_idx in group[1:]:
                attn = self.blocks[layer_idx].attn
                attn.c_k.weight = base_attn.c_k.weight
                attn.c_v.weight = base_attn.c_v.weight
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    return q, scale
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], aliases: dict[str, str] | None = None):
    ...
    for name, tensor in state_dict.items():
        if aliases is not None and name in aliases:
            meta[name] = {"type": "alias", "target": aliases[name]}
            continue
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("type") == "alias":
            aliases.append((name, info["target"]))
            continue
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_kv_start=args.shared_kv_start,
        shared_kv_banks=args.shared_kv_banks,
    ...
    log0(f"shared_kv_groups:{list(base_model.shared_kv_groups)}")
    ...
    shared_param_aliases = {
        name: target for name, target in collect_parameter_aliases(base_model).items() if name in export_sd and target in export_sd
    }
    if master_process:
        log0(f"shared_param_aliases:{len(shared_param_aliases)}")
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, aliases=shared_param_aliases)
    ...
    eval_model = GPT(
        ...
        shared_kv_start=args.shared_kv_start, shared_kv_banks=args.shared_kv_banks,
        mlp_leaky_slope=args.mlp_leaky_slope,
    ).to(device).bfloat16()
