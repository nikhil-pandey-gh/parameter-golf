...
class Hyperparameters:
    ...
    qcle_enabled = bool(int(os.environ.get("QCLE_ENABLED", "1")))
    qcle_max_scale = float(os.environ.get("QCLE_MAX_SCALE", 4.0))
    qcle_blend = float(os.environ.get("QCLE_BLEND", 1.0))
    qcle_eps = float(os.environ.get("QCLE_EPS", 1e-8))
    smoke_test_only = bool(int(os.environ.get("SMOKE_TEST_ONLY", "0")))
...
def quadratic_equalize_mlp_pairs(
    ...
    """Losslessly rebalance MLP channels before int6 export.

    relu^2 / leaky_relu^2 are 2-homogeneous, so for positive channel scales s:
        down @ phi(up @ x) == (down / s^2) @ phi((s * up) @ x)
    This uses a CLE-style cubic balancing rule to reduce extreme channel ranges
    before the existing GPTQ-lite per-row quantizer runs.
    """
    if not enabled:
        return state_dict, []
    if max_scale < 1.0:
        raise ValueError(f"QCLE_MAX_SCALE must be >= 1, got {max_scale}")
    if blend < 0.0:
        raise ValueError(f"QCLE_BLEND must be non-negative, got {blend}")
    if not math.isfinite(eps) or eps <= 0.0:
        raise ValueError(f"QCLE_EPS must be finite and > 0, got {eps}")
    out = dict(state_dict)
    ...
    for layer_idx in range(num_layers):
        ...
        if up_key not in state_dict or down_key not in state_dict:
            continue
        up = state_dict[up_key].detach().float()
        down = state_dict[down_key].detach().float()
        up_stat = up.abs().amax(dim=1).clamp_min(eps)
        down_stat = down.abs().amax(dim=0).clamp_min(eps)
        scale = (down_stat / up_stat).pow(blend / 3.0)
        scale = scale / torch.exp(scale.log().mean())
        scale = scale.clamp(1.0 / max_scale, max_scale)
        out[up_key] = (up * scale[:, None]).to(dtype=state_dict[up_key].dtype)
        out[down_key] = (down / scale[None, :].square()).to(dtype=state_dict[down_key].dtype)
        stats.append({
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    base_model = make_model(args, device, use_bfloat16=use_bfloat16)
    base_model.eval()
    batch_size = max(1, args.smoke_batch_size)
    seq_len = max(8, min(args.train_seq_len, args.smoke_seq_len))
    tokens = torch.randint(args.vocab_size, (batch_size, seq_len + 1), device=device)
    x, y = tokens[:, :-1], tokens[:, 1:]
    with torch.inference_mode():
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=use_bfloat16):
            base_loss = base_model(x, y).float()
    export_sd = {k: v.detach().cpu() for k, v in base_model.state_dict().items() if "mtp_heads" not in k}
    unbanked_sd = _unbank_state_dict(export_sd, args.num_layers)
    equalized_sd, qcle_stats = quadratic_equalize_mlp_pairs(
        unbanked_sd,
        args.num_layers,
        enabled=args.qcle_enabled,
        max_scale=args.qcle_max_scale,
        blend=args.qcle_blend,
        eps=args.qcle_eps,
    )
    equalized_state = _rebank_state_dict(equalized_sd, args.num_layers, export_sd)
    qcle_model = make_model(args, device, mtp_num_heads=0, mtp_loss_weight=0.0, use_bfloat16=use_bfloat16)
    qcle_model.load_state_dict(equalized_state, strict=True)
    qcle_model.eval()
    with torch.inference_mode():
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=use_bfloat16):
            qcle_loss = qcle_model(x, y).float()
    exact_diff = abs(base_loss.item() - qcle_loss.item())
    if exact_diff > 5e-4:
        raise RuntimeError(f"Q-CLE changed the float model unexpectedly: diff={exact_diff:.6e}")
    quant_result, quant_meta = mixed_quantize_int6(equalized_sd, {"mlp", "attn"})
    deq_unbanked = dequantize_mixed_int6(quant_result, quant_meta, equalized_sd)
    deq_state = _rebank_state_dict(deq_unbanked, args.num_layers, export_sd)
    int6_model = make_model(args, device, mtp_num_heads=0, mtp_loss_weight=0.0, use_bfloat16=use_bfloat16)
    int6_model.load_state_dict(deq_state, strict=True)
    int6_model.eval()
    with torch.inference_mode():
        with torch.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=use_bfloat16):
            int6_loss = int6_model(x, y).float()
    qcle_layers = len(qcle_stats)
    mean_scale = sum(stat["mean"] for stat in qcle_stats) / qcle_layers if qcle_layers else 1.0
    min_scale = min((stat["min"] for stat in qcle_stats), default=1.0)
    max_scale = max((stat["max"] for stat in qcle_stats), default=1.0)
    print(f"smoke:device:{device.type}")
    print(f"smoke:base_loss:{base_loss.item():.6f}")
    print(f"smoke:qcle_loss:{qcle_loss.item():.6f}")
    print(f"smoke:qcle_exact_diff:{exact_diff:.6e}")
    print(f"smoke:int6_loss:{int6_loss.item():.6f}")
    print(
        f"smoke:qcle_layers:{qcle_layers} "
        f"scale_mean:{mean_scale:.4f} scale_min:{min_scale:.4f} scale_max:{max_scale:.4f}"
    )
...
def main() -> None:
    ...
    use_external_flash = flash_attn_3_func is not None
    enable_cudnn_sdp(not use_external_flash)
    enable_flash_sdp(True)
    enable_mem_efficient_sdp(not use_external_flash)
    enable_math_sdp(not use_external_flash)
    logfile = None
    if master_process:
    ...
    base_model = make_model(args, device, use_bfloat16=True)
    ...
    unbanked_sd, qcle_stats = quadratic_equalize_mlp_pairs(
        unbanked_sd,
        args.num_layers,
        enabled=args.qcle_enabled,
        max_scale=args.qcle_max_scale,
    ...
    if qcle_stats:
        mean_scale = sum(stat["mean"] for stat in qcle_stats) / len(qcle_stats)
        min_scale = min(stat["min"] for stat in qcle_stats)
        max_scale = max(stat["max"] for stat in qcle_stats)
        log0(
            f"qcle:enabled layers:{len(qcle_stats)} "
            f"scale_mean:{mean_scale:.4f} scale_min:{min_scale:.4f} scale_max:{max_scale:.4f}"
        )
    ...
    eval_model = make_model(args, device, mtp_num_heads=0, mtp_loss_weight=0.0, use_bfloat16=True)
