...
class Hyperparameters:
    ...
    kv_latent_layers = os.environ.get("KV_LATENT_LAYERS", "0,1,2,3,4,5,6")
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.use_latent_kv = kv_latent_dim > 0
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if self.use_latent_kv:
            ...
            k = self.k_up(kv_latent).reshape(bsz, seqlen, self.num_kv_heads, self.head_dim)
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(
            dim, num_heads, num_kv_heads, rope_base, qk_gain_init, kv_latent_dim=kv_latent_dim
        )
...
class GPT(nn.Module):
    def __init__(
        ...
        kv_latent_layer_indices = [int(x) for x in kv_latent_layers.split(",") if x.strip()]
        kv_latent_limit = max(0, num_layers - xsa_last_n) if xsa_last_n > 0 else num_layers
        self.kv_latent_layer_indices = [
            i for i in kv_latent_layer_indices
            if 0 <= i < kv_latent_limit
        ]
        self.kv_latent_dim = kv_latent_dim if self.kv_latent_layer_indices else 0
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    kv_latent_dim=kv_latent_dim if i in self.kv_latent_layer_indices else 0,
                )
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        kv_latent_layers=args.kv_latent_layers,
    ).to(device).bfloat16()
    ...
    log0(f"KV_LATENT:dim:{base_model.kv_latent_dim} active_layers:{base_model.kv_latent_layer_indices}")
    ...
    eval_model = GPT(
        ...
        kv_latent_dim=args.kv_latent_dim, kv_latent_layers=args.kv_latent_layers,
    ).to(device).bfloat16()
