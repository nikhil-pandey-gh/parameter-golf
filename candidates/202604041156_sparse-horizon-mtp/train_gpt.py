...
class Hyperparameters:
    ...
    mtp_token_stride = int(os.environ.get("MTP_TOKEN_STRIDE", 4))
...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_num_heads > 0:
            if mtp_token_stride <= 0:
                raise ValueError(f"mtp_token_stride must be positive, got {mtp_token_stride}")
            if not (0.0 < mtp_horizon_decay <= 1.0):
                raise ValueError(
                    f"mtp_horizon_decay must lie in (0, 1], got {mtp_horizon_decay}"
                )
        ...
        self.mtp_token_stride = mtp_token_stride
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            mtp_weight_sum = 0.0
            stride = self.mtp_token_stride
            for k, mtp_head in enumerate(self.mtp_heads):
                valid_t = seqlen - (k + 1)
                if valid_t <= 0:
                    continue
                # Subsample MTP positions so the auxiliary heads improve sample efficiency
                # without spending too much of the fixed 10-minute wall clock on extra logits.
                mtp_hidden = x[:, :valid_t:stride, :].contiguous().view(-1, dim)
                if mtp_hidden.numel() == 0:
                    continue
                mtp_targets = target_ids[:, k + 1 :: stride].contiguous().view(-1)
                mtp_logits_proj = mtp_head(mtp_hidden)
                mtp_logits = self.logit_softcap * torch.tanh(mtp_logits_proj / self.logit_softcap)
                horizon_weight = self.mtp_horizon_decay**k
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_token_stride=args.mtp_token_stride,
    ...
    # Optimizer split:
    # - 4 parameter banks + auxiliary MTP heads -> Muon (batched Newton-Schulz)
    ...
    log0(
        ...
        f"mtp_token_stride:{args.mtp_token_stride} mtp_horizon_decay:{args.mtp_horizon_decay} "
