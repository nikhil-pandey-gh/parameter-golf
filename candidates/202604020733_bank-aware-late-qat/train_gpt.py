...
class Hyperparameters:
    ...
    late_qat_threshold = float(os.environ.get("LATE_QAT_THRESHOLD", 0.18))
    qat_clip_init = float(os.environ.get("QAT_CLIP_INIT", 4.0))
    qat_clip_floor = float(os.environ.get("QAT_CLIP_FLOOR", 0.5))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,qat_clip",
    ).split(",")
    if pattern
)
...
def clip_multiplier_from_logit(logit: Tensor | float) -> Tensor:
    if isinstance(logit, Tensor):
        logit_t = logit.float()
    else:
        logit_t = torch.tensor(float(logit), dtype=torch.float32)
    floor = Hyperparameters.qat_clip_floor
    return floor + (1.0 - floor) * torch.sigmoid(logit_t)

def fake_quantize_int6_ste(t: Tensor, clip_multiplier: Tensor | float, clip_range: int = INT6_CLIP_RANGE) -> Tensor:
    if t.ndim != 2:
        return t
    t32 = t.float()
    if isinstance(clip_multiplier, Tensor):
        clip_multiplier_t = clip_multiplier.to(device=t32.device, dtype=torch.float32)
    else:
        clip_multiplier_t = torch.tensor(float(clip_multiplier), device=t32.device, dtype=torch.float32)
    row_absmax = t32.detach().abs().amax(dim=1, keepdim=True).clamp_min(1.0 / clip_range)
    scale = (row_absmax * clip_multiplier_t.reshape(1, 1) / clip_range).clamp_min(1.0 / clip_range)
    q = torch.clamp(torch.round(t32.detach() / scale), -clip_range, clip_range)
    return (q * scale + (t32 - t32.detach())).to(dtype=t.dtype)
...
class CastedLinear(nn.Linear):
    def __init__(self, *args, **kwargs):
        ...
        self.qat_clip_logit = nn.Parameter(torch.tensor(Hyperparameters.qat_clip_init, dtype=torch.float32))

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight
        if self.qat_enabled and self.training and w.ndim == 2:
            w = fake_quantize_int6_ste(w, clip_multiplier_from_logit(self.qat_clip_logit))
        w = w.to(x.dtype)
...
class GPT(nn.Module):
    def __init__(
        ...
        clip_logit_init = Hyperparameters.qat_clip_init
        self.qo_qat_clip_logits = nn.Parameter(torch.full((2 * num_layers,), clip_logit_init, dtype=torch.float32))
        self.kv_qat_clip_logits = nn.Parameter(torch.full((2 * num_layers,), clip_logit_init, dtype=torch.float32))
        self.mlp_up_qat_clip_logits = nn.Parameter(torch.full((num_layers,), clip_logit_init, dtype=torch.float32))
        self.mlp_down_qat_clip_logits = nn.Parameter(torch.full((num_layers,), clip_logit_init, dtype=torch.float32))
    ...
    def set_qat_enabled_(self, enabled: bool) -> None:
        self.bank_qat_enabled = enabled
        for module in self.modules():
            if isinstance(module, CastedLinear):
                module.qat_enabled = enabled

    def _maybe_qat_bank(self, weight: Tensor, clip_logit: Tensor) -> Tensor:
        if self.bank_qat_enabled and self.training:
            return fake_quantize_int6_ste(weight, clip_multiplier_from_logit(clip_logit))
        return weight
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            q_w = self._maybe_qat_bank(self.qo_bank[i], self.qo_qat_clip_logits[i])
            k_w = self._maybe_qat_bank(self.kv_bank[i], self.kv_qat_clip_logits[i])
            v_w = self._maybe_qat_bank(self.kv_bank[n + i], self.kv_qat_clip_logits[n + i])
            out_w = self._maybe_qat_bank(self.qo_bank[n + i], self.qo_qat_clip_logits[n + i])
            up_w = self._maybe_qat_bank(self.mlp_up_bank[i], self.mlp_up_qat_clip_logits[i])
            down_w = self._maybe_qat_bank(self.mlp_down_bank[i], self.mlp_down_qat_clip_logits[i])
        ...
        for i in range(self.num_decoder_layers):
            ...
            q_w = self._maybe_qat_bank(self.qo_bank[bi], self.qo_qat_clip_logits[bi])
            k_w = self._maybe_qat_bank(self.kv_bank[bi], self.kv_qat_clip_logits[bi])
            v_w = self._maybe_qat_bank(self.kv_bank[n + bi], self.kv_qat_clip_logits[n + bi])
            out_w = self._maybe_qat_bank(self.qo_bank[n + bi], self.qo_qat_clip_logits[n + bi])
            up_w = self._maybe_qat_bank(self.mlp_up_bank[bi], self.mlp_up_qat_clip_logits[bi])
            down_w = self._maybe_qat_bank(self.mlp_down_bank[bi], self.mlp_down_qat_clip_logits[bi])
...
def export_bank_qat_clip_multipliers(model: GPT) -> dict[str, float]:
    qo = clip_multiplier_from_logit(model.qo_qat_clip_logits.detach().cpu())
    kv = clip_multiplier_from_logit(model.kv_qat_clip_logits.detach().cpu())
    up = clip_multiplier_from_logit(model.mlp_up_qat_clip_logits.detach().cpu())
    down = clip_multiplier_from_logit(model.mlp_down_qat_clip_logits.detach().cpu())
    n = model.num_layers
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, clip_range: int = INT6_CLIP_RANGE, clip_multiplier: float | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        if clip_multiplier is not None:
            row_clip = (t32.abs().amax(dim=1) * float(clip_multiplier)).clamp_min(1.0 / clip_range)
...
def main() -> None:
    ...
    base_model.set_qat_enabled_(args.qat_enabled)
    ...
    scalar_params.extend([
        base_model.qo_qat_clip_logits,
        base_model.kv_qat_clip_logits,
        base_model.mlp_up_qat_clip_logits,
        base_model.mlp_down_qat_clip_logits,
    ])
    scalar_params.extend(module.qat_clip_logit for module in base_model.modules() if isinstance(module, CastedLinear))
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not base_model.bank_qat_enabled:
            base_model.set_qat_enabled_(True)
            compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
            model = compiled_model
    ...
    bank_clip_multipliers = export_bank_qat_clip_multipliers(base_model) if base_model.bank_qat_enabled else None
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, clip_multipliers=bank_clip_multipliers)
    ...
    eval_model.set_qat_enabled_(False)
