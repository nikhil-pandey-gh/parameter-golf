...
def attention_with_fallback(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    y = F.scaled_dot_product_attention(
        qh, kh, vh, is_causal=True, enable_gqa=qh.size(1) != kh.size(1)
    )
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        y = attention_with_fallback(q, k, v)
        if self.use_xsa:
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
GPTQ_LITE_CLIP_PCTS = (0.9990, 0.9995, 0.9999, 0.99999, 1.0)
def collect_awq_input_sq_means(
    args: Hyperparameters,
    base_model: GPT,
    rank: int,
    world_size: int,
    ...
    for name, module in base_model.named_modules():
        ...
        if _classify_param(weight_name) not in {"mlp", "attn"}:
            continue
        def _hook(_module, inputs, weight_name=weight_name):
            if not inputs:
                return
            x = inputs[0]
            ...
            stat = target_stats.get(weight_name)
            if stat is None:
                target_stats[weight_name] = {"sum_sq": sum_sq, "count": count}
            else:
                stat["sum_sq"].add_(sum_sq)
                stat["count"].add_(count)
    ...
    try:
        with torch.inference_mode():
            for _ in range(args.awq_calibration_batches):
                x, _ = loader.next_batch(calib_batch_tokens, args.train_seq_len, 1)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=device.type == "cuda"):
                    base_model.forward_logits(x)
        if dist.is_available() and dist.is_initialized():
            for stat in target_stats.values():
                dist.all_reduce(stat["sum_sq"], op=dist.ReduceOp.SUM)
        ...
        if was_training:
            base_model.train()
    input_sq_means = {
        name: (stat["sum_sq"] / stat["count"].clamp_min(1.0)).cpu()
        for name, stat in target_stats.items()
    }
    log0(
        f"awq:enabled layers:{len(input_sq_means)} batches:{args.awq_calibration_batches} "
        f"batch_tokens:{calib_batch_tokens}"
    )
    return input_sq_means
def quantize_int6_per_row(
    t: Tensor, clip_range: int = 31, input_sq_mean: Tensor | None = None
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        act_weight = None
        if input_sq_mean is not None and input_sq_mean.ndim == 1 and input_sq_mean.numel() == t32.size(1):
            act_weight = input_sq_mean.float().clamp_min(1e-6)
            act_weight = act_weight / act_weight.mean().clamp_min(1e-6)
        best_q, best_s, best_err = None, None, float('inf')
        for pct in GPTQ_LITE_CLIP_PCTS:
            if pct < 1.0:
            ...
            err = (err_sq * act_weight[None, :]).mean().item() if act_weight is not None else err_sq.mean().item()
            if err < best_err:
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    awq_input_sq_means: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, input_sq_mean=None if awq_input_sq_means is None else awq_input_sq_means.get(name))
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    awq_input_sq_means = collect_awq_input_sq_means(
        args, base_model, rank, world_size, grad_accum_steps, device, log0
    )
    torch.cuda.synchronize()
    if awq_input_sq_means:
        log0(f"awq:collection_time:{1000.0 * (time.perf_counter() - t_awq):.0f}ms")
    sd_cpu = {k: v.detach().cpu() for k, v in export_sd.items()}
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, awq_input_sq_means)
