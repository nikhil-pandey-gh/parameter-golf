...
def eval_val_sliding_ttt(
    ...
    orig_mtp_num_heads = getattr(base_model, "mtp_num_heads", 0)
    orig_mtp_loss_weight = getattr(base_model, "mtp_loss_weight", 0.0)
    if orig_mtp_num_heads > 0 and orig_mtp_loss_weight > 0.0:
        log0(
            f"ttt_sliding:disabling_mtp_aux heads={orig_mtp_num_heads} "
            f"weight={orig_mtp_loss_weight:.3f}"
        )
        base_model.mtp_num_heads = 0
        base_model.mtp_loss_weight = 0.0
    ...
    for p in base_model.parameters():
        p.requires_grad_(True)
    base_model.mtp_num_heads = orig_mtp_num_heads
    base_model.mtp_loss_weight = orig_mtp_loss_weight
    base_model.eval()
...
def main() -> None:
    ...
    # - bigram proj, VE proj -> AdamW (small matrix params not worth banking)
    # - lm_head / mtp_heads -> Adam at head_lr
    matrix_params = [
