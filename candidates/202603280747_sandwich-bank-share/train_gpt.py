...
def default_bank_layer_map(num_layers: int) -> list[int]:
    ...
    prefix = list(range(num_layers - 4))
    a = len(prefix)
    b = a + 1
    return prefix + [a, b, a, b]


def parse_bank_layer_map(num_layers: int, spec: str) -> list[int]:
    spec = spec.strip()
    if not spec:
        return default_bank_layer_map(num_layers)
    bank_layer_map = [int(x.strip()) for x in spec.split(",") if x.strip()]
    if len(bank_layer_map) != num_layers:
        raise ValueError(
            f"BANK_LAYER_MAP must contain NUM_LAYERS={num_layers} comma-separated indices, got {bank_layer_map}"
        )
    if min(bank_layer_map) < 0:
        raise ValueError(f"BANK_LAYER_MAP indices must be non-negative, got {bank_layer_map}")
    unique = sorted(set(bank_layer_map))
    if unique != list(range(max(unique) + 1)):
        raise ValueError(
            "BANK_LAYER_MAP indices must form a contiguous 0..N-1 range so bank tensors stay compact; "
            f"got {bank_layer_map}"
        )
    return bank_layer_map
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bank_layer_map = parse_bank_layer_map(num_layers, bank_layer_map)
    ...
    def _bank_idx(self, layer_idx: int) -> int:
        return self.bank_layer_map[layer_idx]
...
def _classify_param(name: str) -> str:
    if "tok_emb" in name or "lm_head" in name:
        return "embed"
    if name in {"mlp_up_bank", "mlp_down_bank"} or ".mlp." in name:
        return "mlp"
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            out[name] = (q.float() * s.float().view(*q.shape[:-1], 1)).to(orig_dtype)
        else:
...
def main() -> None:
    ...
    log0(f"bank_layers:logical={args.num_layers} physical={base_model.num_bank_layers} map={base_model.bank_layer_map}")
    ...
    if master_process:
        ...
        torch.save({"bank_layer_map": list(base_model.bank_layer_map)}, "final_model.bank_map.pt")
        model_bytes = os.path.getsize("final_model.pt")
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model: {model_bytes} bytes")
        log0(f"Code size: {code_bytes} bytes")
    # Quantize bank tensors directly so shared late layers are not duplicated on disk.
    ...
    quant_bank_layer_map = quant_state.get("config", {}).get("bank_layer_map", base_model.bank_layer_map)
    eval_model = GPT(
        ...
        bank_layer_map=",".join(str(i) for i in quant_bank_layer_map),
    ).to(device).bfloat16()
