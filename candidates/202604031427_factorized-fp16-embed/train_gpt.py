...
class Hyperparameters:
    ...
    embed_export_fp16 = bool(int(os.environ.get("EMBED_EXPORT_FP16", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.embed_dim = model_dim if embed_dim <= 0 else embed_dim
        self.tok_emb = nn.Embedding(vocab_size, self.embed_dim)
        self.emb_proj = CastedLinear(self.embed_dim, model_dim, bias=False) if self.embed_dim != model_dim else None
    ...
    def _embed_tokens(self, input_ids: Tensor) -> Tensor:
        ...
        if self.emb_proj is not None:
            x = self.emb_proj(x)
        return x
    def _tied_logits(self, x: Tensor) -> Tensor:
        if self.emb_proj is not None:
            x = F.linear(x, self.emb_proj.weight.t().to(dtype=x.dtype))
        return F.linear(x, self.tok_emb.weight.to(dtype=x.dtype))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.tie_embeddings:
            logits_proj = self._tied_logits(x_flat)
        else:
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        x = self.final_norm(x)
        if self.tie_embeddings:
            logits_proj = self._tied_logits(x)
        else:
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], *, embed_export_fp16: bool = False):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        if embed_export_fp16 and name == "tok_emb.weight":
...
def main() -> None:
    ...
    if base_model.emb_proj is not None:
        tok_params.append({"params": [base_model.emb_proj.weight], "lr": token_lr, "base_lr": token_lr})
    if base_model.bigram is not None:
    ...
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_dim:{base_model.embed_dim} "
        f"embed_export_fp16:{args.embed_export_fp16} embed_lr:{token_lr} "
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd,
        {"mlp", "attn"},
        embed_export_fp16=args.embed_export_fp16,
    )
