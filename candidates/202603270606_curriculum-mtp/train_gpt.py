...
class Hyperparameters:
    ...
    mtp_curriculum_end = float(os.environ.get("MTP_CURRICULUM_END", 0.40))
    ...
    def forward(
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            if mtp_head_weights is None:
                head_weights = x.new_ones((self.mtp_num_heads,))
            else:
                if mtp_head_weights.numel() != self.mtp_num_heads:
                    raise ValueError(
                        f"Expected {self.mtp_num_heads} MTP head weights, got {mtp_head_weights.numel()}"
                    )
                head_weights = mtp_head_weights.to(device=x.device, dtype=x.dtype)
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_sum = mtp_loss_sum + head_weights[k] * mtp_loss
                mtp_loss_count += 1
            if mtp_loss_count > 0:
...
def main() -> None:
    ...
    if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0:
        log0(
            f"mtp_curriculum:start={args.mtp_curriculum_start:.2f} "
            f"end={args.mtp_curriculum_end:.2f}"
        )
    ...
    if args.mtp_num_heads > 0 and args.mtp_curriculum_end <= args.mtp_curriculum_start:
        raise ValueError(
            "MTP_CURRICULUM_END must be greater than MTP_CURRICULUM_START; "
            f"got {args.mtp_curriculum_start} and {args.mtp_curriculum_end}"
        )
    ...
    def build_mtp_head_weights(step: int, elapsed_ms: float, device: torch.device) -> Tensor:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return torch.empty((0,), device=device, dtype=torch.float32)
        progress = mtp_progress(step, elapsed_ms)
        ramp = min(
            max(
                (progress - args.mtp_curriculum_start)
                / (args.mtp_curriculum_end - args.mtp_curriculum_start),
                0.0,
            ),
            1.0,
        )
        phase = torch.tensor(ramp * args.mtp_num_heads, device=device, dtype=torch.float32)
        head_idx = torch.arange(args.mtp_num_heads, device=device, dtype=torch.float32)
        return (phase - head_idx).clamp_(0.0, 1.0)
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            mtp_head_weights = build_mtp_head_weights(0, 0.0, device)
            for micro_step in range(grad_accum_steps):
                if distributed:
                    model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
                x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, mtp_head_weights)
    ...
    while True:
        ...
        mtp_head_weights = build_mtp_head_weights(step, elapsed_ms, device)
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
        ...
        if should_log_train:
            mtp_log = ""
            if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0:
                weights_str = ",".join(f"{w:.2f}" for w in mtp_head_weights.detach().cpu().tolist())
                mtp_log = f" mtp_head_weights:[{weights_str}]"
            log0(
