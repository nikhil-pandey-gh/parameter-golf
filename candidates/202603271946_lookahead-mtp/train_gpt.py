...
class Hyperparameters:
    ...
    mtp_init_from_main = bool(int(os.environ.get("MTP_INIT_FROM_MAIN", "1")))
...
class CausalSelfAttention(nn.Module):
    ...
    def _flash_or_sdpa(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        ...
        k_t = k.transpose(1, 2)
        v_t = v.transpose(1, 2)
        if self.num_heads != self.num_kv_heads and _SDPA_SUPPORTS_GQA:
            y = F.scaled_dot_product_attention(q_t, k_t, v_t, is_causal=True, enable_gqa=True)
        else:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_init_from_main = mtp_init_from_main
        ...
        self._init_weights()
        self._init_mtp_heads_from_main()
    ...
    def _init_mtp_heads_from_main(self) -> None:
        if not self.mtp_heads or not self.mtp_init_from_main:
            return
        source = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
        with torch.no_grad():
            for head in self.mtp_heads:
                head.weight.copy_(source)
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = maybe_compile(base_model.forward_logits, args.compile_enabled and hasattr(torch, "compile"))
    with torch.inference_mode():
...
def main() -> None:
    ...
    if distributed and not torch.cuda.is_available():
        raise RuntimeError("Distributed CPU execution is not supported; use a single-process smoke check")
    ...
    compile_enabled = args.compile_enabled and hasattr(torch, "compile")
    ...
    if device.type == "cuda":
        ...
        log0("nvidia-smi: unavailable (CPU fallback path)", console=False)
    log0("=" * 100, console=False)
    ...
    base_model = GPT(
        ...
        mtp_init_from_main=args.mtp_init_from_main,
    ...
    log0(
        " ".join(
            (
                ...
                f"mtp_loss_decay:{args.mtp_loss_decay}",
                f"mtp_init_from_main:{args.mtp_init_from_main}",
    ...
    attn_backend = "flash_attn_3" if flash_attn_3_func is not None and device.type == "cuda" else "sdpa_fallback"
    if device.type == "cuda":
        log0("sdp_backends:cudnn=False flash=True mem_efficient=True math=True")
    else:
        log0("sdp_backends:cpu_sdpa")
    log0(f"device:{device.type} compile_enabled:{compile_enabled} attention_backend:{attn_backend}")
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_loss_decay=args.mtp_loss_decay, mtp_init_from_main=False,
