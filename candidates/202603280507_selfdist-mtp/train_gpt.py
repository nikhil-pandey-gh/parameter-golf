...
class Hyperparameters:
    ...
    mtp_distill_temp = float(os.environ.get("MTP_DISTILL_TEMP", 1.5))
...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_distill_temp <= 0.0:
            raise ValueError(f"mtp_distill_temp must be positive, got {mtp_distill_temp}")
        self.mtp_distill_weight = mtp_distill_weight
        self.mtp_distill_temp = mtp_distill_temp
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if self.mtp_distill_weight > 0.0:
                    teacher = teacher_logits[:, k + 1 :, :].reshape(-1, teacher_logits.size(-1))
                    mtp_loss = mtp_loss + self.mtp_distill_weight * F.kl_div(
                        F.log_softmax(mtp_logits.float() / temp, dim=-1),
                        F.softmax(teacher.float() / temp, dim=-1),
                        reduction="batchmean",
                    ) * (temp * temp)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_distill_temp=args.mtp_distill_temp,
    ...
    log0(
        " ".join(
            [
                ...
                f"mtp_distill_temp:{args.mtp_distill_temp}",
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_distill_weight=0.0, mtp_distill_temp=1.0,
