...
class Hyperparameters:
    ...
    mtp_stride = int(os.environ.get("MTP_STRIDE", 4))
    mtp_stop_scale = float(os.environ.get("MTP_STOP_SCALE", 0.35))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_stride = max(1, mtp_stride)
    ...
    def _main_loss(self, hidden_states: Tensor, target_ids: Tensor) -> Tensor:
    ...
    def _mtp_loss(self, hidden_states: Tensor, target_ids: Tensor) -> Tensor | None:
        if self.mtp_num_heads <= 0 or self.mtp_loss_weight <= 0.0:
            return None
        _, seqlen, dim = hidden_states.shape
        mtp_loss_sum = hidden_states.new_zeros(())
        mtp_loss_count = 0
        for k, mtp_head in enumerate(self.mtp_heads):
            valid_t = seqlen - (k + 1)
            if valid_t <= 0:
                continue
            mtp_hidden = hidden_states[:, :valid_t, :]
            mtp_targets = target_ids[:, k + 1 : k + 1 + valid_t]
            if self.mtp_stride > 1:
                offset = k % self.mtp_stride
                mtp_hidden = mtp_hidden[:, offset::self.mtp_stride, :]
                mtp_targets = mtp_targets[:, offset::self.mtp_stride]
            if mtp_targets.numel() == 0:
                continue
            mtp_logits_proj = mtp_head(mtp_hidden.reshape(-1, dim))
            mtp_logits = self.logit_softcap * torch.tanh(mtp_logits_proj / self.logit_softcap)
            mtp_loss_sum = mtp_loss_sum + F.cross_entropy(mtp_logits.float(), mtp_targets.reshape(-1), reduction="mean")
            mtp_loss_count += 1
        if mtp_loss_count == 0:
            return None
        return mtp_loss_sum / mtp_loss_count
    def forward_main(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        hidden_states = self._forward_backbone(input_ids)
        return self._main_loss(hidden_states, target_ids)
    def forward_with_mtp(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        hidden_states = self._forward_backbone(input_ids)
        main_loss = self._main_loss(hidden_states, target_ids)
        mtp_loss = self._mtp_loss(hidden_states, target_ids)
        if mtp_loss is None:
            return main_loss
        return main_loss + self.mtp_loss_weight * mtp_loss
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            return self.forward_with_mtp(input_ids, target_ids)
        return self.forward_main(input_ids, target_ids)
...
def main() -> None:
    ...
    def compile_train_forwards() -> tuple:
        compiled_main = torch.compile(base_model.forward_main, dynamic=False, fullgraph=True)
        compiled_mtp = None
        if args.mtp_num_heads > 0 and args.mtp_loss_weight > 0.0:
            compiled_mtp = torch.compile(base_model.forward_with_mtp, dynamic=False, fullgraph=True)
        return compiled_main, compiled_mtp

    train_forward_main, train_forward_mtp = compile_train_forwards()
    ...
    log0(
        ...
        f"mtp_stride:{args.mtp_stride} mtp_stop_scale:{args.mtp_stop_scale} mtp_params:{mtp_params}"
    )
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                warmup_forward = train_forward_mtp if train_forward_mtp is not None else train_forward_main
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = warmup_forward(x, y)
    ...
    while True:
        ...
        use_mtp = train_forward_mtp is not None and (args.mtp_stop_scale <= 0.0 or scale > args.mtp_stop_scale)
        if train_forward_mtp is not None and not use_mtp and not mtp_disabled_logged:
            log0(f"mtp:disabled_for_warmdown step:{step} scale:{scale:.4f}")
            mtp_disabled_logged = True
        train_forward = train_forward_mtp if use_mtp else train_forward_main
        zero_grad_all()
