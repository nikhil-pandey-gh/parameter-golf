...
class Hyperparameters:
    ...
    quant_rotation_block_size = int(os.environ.get("QUANT_ROTATION_BLOCK_SIZE", 128))
...
def normalized_hadamard(x: Tensor) -> Tensor:
    size = x.shape[-1]
    if size <= 0 or size & (size - 1):
        raise ValueError(f"Hadamard size must be a positive power of two, got {size}")
    y = x.reshape(-1, size).clone()
    stride = 1
    while stride < size:
        y = y.view(-1, size // (2 * stride), 2, stride)
        a = y[:, :, 0, :].clone()
        b = y[:, :, 1, :].clone()
        y[:, :, 0, :] = a + b
        y[:, :, 1, :] = a - b
...
def quant_rotation_signs(name: str, block_size: int) -> Tensor:
    seed = zlib.crc32(name.encode("utf-8")) & 0xFFFFFFFF
    rng = np.random.default_rng(np.uint32(seed))
    signs = rng.integers(0, 2, size=block_size, dtype=np.int8).astype(np.float32)
    signs = 2.0 * signs - 1.0
    return torch.from_numpy(signs)


def apply_block_sign_hadamard(x: Tensor, signs: Tensor, block_size: int) -> Tensor:
    blocks = x.reshape(*x.shape[:-1], x.shape[-1] // block_size, block_size)
    sign_shape = (1,) * (blocks.ndim - 1) + (block_size,)
    return normalized_hadamard(blocks * signs.view(sign_shape)).reshape_as(x)


def undo_block_sign_hadamard(x: Tensor, signs: Tensor, block_size: int) -> Tensor:
    blocks = x.reshape(*x.shape[:-1], x.shape[-1] // block_size, block_size)
    sign_shape = (1,) * (blocks.ndim - 1) + (block_size,)
    return (normalized_hadamard(blocks) * signs.view(sign_shape)).reshape_as(x)


def maybe_apply_quant_rotation(
    name: str,
    t: Tensor,
    enabled: bool,
    block_size: int,
) -> tuple[Tensor, dict[str, object] | None]:
    if not enabled or t.ndim != 2 or block_size <= 1 or t.shape[-1] < block_size or t.shape[-1] % block_size != 0:
        return t.float(), None
    if block_size & (block_size - 1):
        raise ValueError(f"QUANT_ROTATION_BLOCK_SIZE must be a power of two, got {block_size}")
    signs = quant_rotation_signs(name, block_size).to(dtype=torch.float32)
    rotated = apply_block_sign_hadamard(t.float(), signs, block_size).contiguous()
    return rotated, {"rotation": "sign_hadamard", "block_size": block_size}


def maybe_inverse_quant_rotation(name: str, t: Tensor, info: dict[str, object]) -> Tensor:
    if info.get("rotation") != "sign_hadamard":
        return t
    block_size = int(info["block_size"])
    work = t.float().contiguous()
    signs = quant_rotation_signs(name, block_size).to(dtype=work.dtype)
    restored = undo_block_sign_hadamard(work, signs, block_size)
    return restored.to(dtype=t.dtype).contiguous()
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        t_for_quant, rotation_info = maybe_apply_quant_rotation(name, t, rotate, rotation_block_size)
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t_for_quant)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
        ...
        if rotation_info is not None:
            meta[name].update(rotation_info)
    return result, meta
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and "rotation" in info:
            t = maybe_inverse_quant_rotation(name, t, info)
...
def run_cpu_smoke_test(args: Hyperparameters) -> None:
    ...
    input_ids = torch.randint(0, 256, (2, 64), dtype=torch.int64)
    target_ids = torch.randint(0, 256, (2, 64), dtype=torch.int64)
    loss = model(input_ids, target_ids)
    if not torch.isfinite(loss):
        raise RuntimeError(f"cpu_smoke_test produced non-finite loss: {loss.item()}")
    state_dict = {k: v.detach().cpu() for k, v in model.state_dict().items()}
    quant_result, quant_meta = mixed_quantize_int6(
        state_dict,
        {"mlp", "attn"},
        rotate=args.quant_rotation_enabled,
        rotation_block_size=args.quant_rotation_block_size,
    )
    roundtrip = dequantize_mixed_int6(quant_result, quant_meta, state_dict)
    model.load_state_dict(roundtrip, strict=True)
    roundtrip_loss = model(input_ids, target_ids)
    if not torch.isfinite(roundtrip_loss):
        raise RuntimeError(f"cpu_smoke_test roundtrip produced non-finite loss: {roundtrip_loss.item()}")
    print(f"cpu_smoke_ok loss:{loss.item():.4f} roundtrip_loss:{roundtrip_loss.item():.4f}")
def main() -> None:
    ...
    log0(
        f"quant_rotation:enabled:{int(args.quant_rotation_enabled)} "
        f"block_size:{args.quant_rotation_block_size}"
    )
