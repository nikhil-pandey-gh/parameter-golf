...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    edge_int8_attn_layers: tuple[int, ...] = (),
):
    ...
    for name, tensor in state_dict.items():
        ...
        keep_edge_attn_int8 = cat == "attn" and layer_idx in edge_int8_attn_layers
        if not t.is_floating_point():
            result[name] = t
...
def main() -> None:
    ...
    edge_int8_attn_layers = tuple(sorted({0, max(args.num_layers - 1, 0)}))
    ...
    log0(f"mlp_activation:leaky_relu_sq negative_slope:0.5")
    log0(f"quant_edge_int8_attn_layers:{list(edge_int8_attn_layers)}")
    log0(
    ...
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, edge_int8_attn_layers)
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model mixed-int6-int8+{_COMPRESSOR}: {quant_file_bytes} bytes")
        log0(f"Total submission size mixed-int6-int8+{_COMPRESSOR}: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
