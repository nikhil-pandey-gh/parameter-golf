...
class Hyperparameters:
    ...
    shared_bank_mode = os.environ.get("SHARED_BANK_MODE", "mirror")
    int6_cats = tuple(cat for cat in os.environ.get("INT6_CATS", "mlp").split(",") if cat)
...
def build_bank_layer_map(num_layers: int, share_mode: str) -> tuple[int, ...]:
    mode = share_mode.strip().lower()
    if mode in {"", "none", "off"}:
        return tuple(range(num_layers))
    if mode == "mirror":
        return tuple(min(i, num_layers - 1 - i) for i in range(num_layers))
    raise ValueError(f"Unsupported SHARED_BANK_MODE={share_mode!r}")
...
class GPT(nn.Module):
    def __init__(
        ...
        self.shared_bank_mode = shared_bank_mode
        self.bank_layer_map = build_bank_layer_map(num_layers, shared_bank_mode)
        self.num_bank_layers = max(self.bank_layer_map, default=-1) + 1
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.num_bank_layers):
            ...
            nn.init.zeros_(self.qo_bank.data[self.num_bank_layers + i])
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)
            nn.init.orthogonal_(self.kv_bank.data[self.num_bank_layers + i], gain=1.0)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            bank_i = self.bank_layer_map[i]
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[bank_i], self.kv_bank[bank_i], self.kv_bank[bank_n + bank_i],
                self.qo_bank[bank_n + bank_i], self.mlp_up_bank[bank_i], self.mlp_down_bank[bank_i],
                v_embed=ve, v0=v0)
...
def _classify_param(name: str) -> str:
    if "tok_emb" in name or "lm_head" in name or ".embed.weight" in name:
        return "embed"
    if ".mlp." in name:
        return "mlp"
...
def _unbank_state_dict(sd: dict[str, Tensor], num_bank_layers: int) -> dict[str, Tensor]:
    """Convert 3D bank tensors into unique 2D tensors so shared banks stay shared on disk."""
    out: dict[str, Tensor] = {}
    n = num_bank_layers
    for name, tensor in sd.items():
        if name == "qo_bank":
            for i in range(n):
                out[f"bank_blocks.{i}.attn.c_q.weight"] = tensor[i]
                out[f"bank_blocks.{i}.attn.proj.weight"] = tensor[n + i]
        elif name == "kv_bank":
            for i in range(n):
                out[f"bank_blocks.{i}.attn.c_k.weight"] = tensor[i]
                out[f"bank_blocks.{i}.attn.c_v.weight"] = tensor[n + i]
        elif name == "mlp_up_bank":
            for i in range(n):
                out[f"bank_blocks.{i}.mlp.fc.weight"] = tensor[i]
        elif name == "mlp_down_bank":
            for i in range(n):
                out[f"bank_blocks.{i}.mlp.proj.weight"] = tensor[i]
        else:
            out[name] = tensor
    return out

def _rebank_state_dict(sd: dict[str, Tensor], num_bank_layers: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert individual unique-bank tensors back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    n = num_bank_layers
    qo_slices = [None] * (2 * n)
    kv_slices = [None] * (2 * n)
    ...
    consumed = set()
    for i in range(n):
        qk = f"bank_blocks.{i}.attn.c_q.weight"
        if qk in sd:
            qo_slices[i] = sd[qk]
            consumed.add(qk)
        ok = f"bank_blocks.{i}.attn.proj.weight"
        if ok in sd:
            qo_slices[n + i] = sd[ok]
            consumed.add(ok)
        kk = f"bank_blocks.{i}.attn.c_k.weight"
        if kk in sd:
            kv_slices[i] = sd[kk]
            consumed.add(kk)
        vk = f"bank_blocks.{i}.attn.c_v.weight"
        if vk in sd:
            kv_slices[n + i] = sd[vk]
            consumed.add(vk)
        fk = f"bank_blocks.{i}.mlp.fc.weight"
        if fk in sd:
            up_slices[i] = sd[fk]
            consumed.add(fk)
        dk = f"bank_blocks.{i}.mlp.proj.weight"
        if dk in sd:
            down_slices[i] = sd[dk]
            consumed.add(dk)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str], fp16_embed_export: bool):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        if fp16_embed_export and cat == "embed":
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_bank_mode=args.shared_bank_mode,
    ...
    mtp_head_params = [head.weight for head in base_model.mtp_heads]
    optimizer_muon = Muon(
        matrix_params,
        lr=args.matrix_lr,
    ...
    if base_model.lm_head is not None:
        head_param_groups.append({"params": [base_model.lm_head.weight], "lr": args.head_lr, "base_lr": args.head_lr})
        replicated_params.append(base_model.lm_head.weight)
    if mtp_head_params:
        head_param_groups.append({"params": mtp_head_params, "lr": args.head_lr, "base_lr": args.head_lr})
    optimizer_head = None
    if head_param_groups:
    ...
    log0(f"bank_sharing:mode={args.shared_bank_mode} unique_bank_layers:{base_model.num_bank_layers} logical_layers:{args.num_layers}")
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, base_model.num_bank_layers)
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, set(args.int6_cats), args.fp16_embed_export)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, base_model.num_bank_layers, sd_cpu)
    eval_model = GPT(
