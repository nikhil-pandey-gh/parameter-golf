...
class Hyperparameters:
    ...
    num_shared_cores = int(os.environ.get("NUM_SHARED_CORES", 6))
    ...
    ve_layers = os.environ.get("VE_LAYERS", "8,9,10,11")
...
class GPT(nn.Module):
    def __init__(
        ...
        if num_shared_cores <= 0 or num_shared_cores > num_layers:
            raise ValueError(
                f"num_shared_cores must be in [1, num_layers], got {num_shared_cores} for num_layers={num_layers}"
            )
        ...
        self.num_weight_cores = num_shared_cores
        # Sandwich-style sharing: the first shared-core slots are unique, then later layers mirror inward.
        self.layer_to_core = tuple(
            layer_idx if layer_idx < self.num_weight_cores else min(self.num_layers - 1 - layer_idx, self.num_weight_cores - 1)
        ...
        # Parameter banks store only the shared cores; per-depth specialization lives in Block-local params.
        head_dim = model_dim // num_heads
        kv_dim = num_kv_heads * head_dim
        mlp_dim = int(mlp_mult * model_dim)
        self.qo_bank = nn.Parameter(torch.empty(2 * self.num_weight_cores, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.num_weight_cores, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(self.num_weight_cores, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(self.num_weight_cores, model_dim, mlp_dim))
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.num_weight_cores):
            nn.init.orthogonal_(self.qo_bank.data[i], gain=1.0)        # Q
            nn.init.zeros_(self.qo_bank.data[self.num_weight_cores + i])                    # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[self.num_weight_cores + i], gain=1.0)    # V
            ...
            self.qo_bank.data[self.num_weight_cores + i].mul_(proj_scale)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[core_idx], self.kv_bank[core_idx], self.kv_bank[n + core_idx],
                self.qo_bank[n + core_idx], self.mlp_up_bank[core_idx], self.mlp_down_bank[core_idx],
                v_embed=ve, v0=v0)
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim >= 2:
        ...
        rows = t32.reshape(-1, orig_shape[-1])
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            q_rows = torch.clamp(torch.round(rows / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
            recon = q_rows.float() * s.float()[:, None]
            err = (rows - recon).pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_err = q_rows, s, err
        if best_q is None or best_s is None:
            raise RuntimeError("int6 row quantization failed to find a valid scale")
        return best_q.reshape(orig_shape).contiguous(), best_s.reshape(orig_shape[:-1]).contiguous()
...
def main() -> None:
    ...
    log0(f"shared_cores:{args.num_shared_cores} layer_to_core:{base_model.layer_to_core}")
    ...
    # Preserve shared banks through export so repeated cores remain shared in the compressed artifact.
