...
class Hyperparameters:
    ...
    attn_logit_softcap = float(os.environ.get("ATTN_LOGIT_SOFTCAP", 50.0))
...
def causal_attention(q: Tensor, k: Tensor, v: Tensor, softcap: float) -> Tensor:
    if flash_attn_3_func is not None and q.device.type == "cuda":
        if softcap > 0.0:
            try:
                return flash_attn_3_func(q, k, v, causal=True, softcap=softcap)
            except TypeError as exc:
                raise RuntimeError(
                    "flash_attn_func on CUDA must support softcap for this candidate; "
                    "use a compatible FlashAttention build or set ATTN_LOGIT_SOFTCAP=0."
                ) from exc
        else:
    ...
    logits = torch.matmul(q_t.float(), k_t.float().transpose(-2, -1)) * scale
    if softcap > 0.0:
        logits = softcap * torch.tanh(logits / softcap)
    causal_mask = torch.triu(torch.ones((q.size(1), k.size(1)), device=q.device, dtype=torch.bool), diagonal=1)
    logits = logits.masked_fill(causal_mask, float("-inf"))
    probs = torch.softmax(logits, dim=-1)
    y = torch.matmul(probs, v_t.float()).to(dtype=q.dtype)
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if attn_logit_softcap < 0.0:
            raise ValueError(f"attn_logit_softcap must be non-negative, got {attn_logit_softcap}")
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = causal_attention(q, k, v, self.attn_logit_softcap)
        if self.use_xsa:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        attn_logit_softcap=args.attn_logit_softcap,
    ...
    log0(f"attn_logit_softcap:{args.attn_logit_softcap} gated_attention:{args.gated_attention}")
    ...
    eval_model = GPT(
        ...
        logit_softcap=args.logit_softcap, attn_logit_softcap=args.attn_logit_softcap,
