...
class Hyperparameters:
    ...
    embed_dim = int(os.environ.get("EMBED_DIM", 160))
    num_shared_blocks = int(os.environ.get("NUM_SHARED_BLOCKS", 6))
    shared_block_mode = os.environ.get("SHARED_BLOCK_MODE", "mirror")
...
class GPT(nn.Module):
    def __init__(
        ...
        if embed_dim <= 0 or embed_dim > model_dim:
            raise ValueError(f"embed_dim must be in [1, model_dim], got embed_dim={embed_dim} model_dim={model_dim}")
        if num_shared_blocks <= 0 or num_shared_blocks > num_layers:
            raise ValueError(
                f"num_shared_blocks must be in [1, num_layers], got num_shared_blocks={num_shared_blocks} num_layers={num_layers}"
            )
        if shared_block_mode not in {"mirror", "group"}:
            raise ValueError(f"shared_block_mode must be 'mirror' or 'group', got {shared_block_mode}")
        if shared_block_mode == "mirror" and num_shared_blocks > (num_layers + 1) // 2:
            raise ValueError(
                "mirror sharing can use at most ceil(num_layers / 2) unique shared blocks; "
                f"got num_shared_blocks={num_shared_blocks} num_layers={num_layers}"
            )
        self.embed_dim = embed_dim
        self.num_shared_blocks = num_shared_blocks
        self.shared_block_mode = shared_block_mode
        ...
        self.embed_up = CastedLinear(embed_dim, model_dim, bias=False)
        ...
        self.shared_block_map = [self._shared_block_index(i) for i in range(num_layers)]
        if rope_dims > 0:
            head_dim = model_dim // num_heads
        ...
        self._init_weights()
    def _shared_block_index(self, layer_idx: int) -> int:
        if self.shared_block_mode == "group":
            return min((layer_idx * self.num_shared_blocks) // self.num_layers, self.num_shared_blocks - 1)
        mirror_bins = (self.num_layers + 1) // 2
        depth = min(layer_idx, self.num_layers - 1 - layer_idx)
        return min((depth * self.num_shared_blocks) // mirror_bins, self.num_shared_blocks - 1)
    def _init_weights(self) -> None:
        if self.tie_embeddings:
            nn.init.normal_(self.tok_emb.weight, mean=0.0, std=self.tied_embed_init_std)
        n = self.num_shared_blocks
    ...
    def _project_logits(self, x: Tensor) -> Tensor:
        if self.tie_embeddings:
            embed_hidden = F.linear(x, self.embed_up.weight.t().to(dtype=x.dtype))
            logits_proj = F.linear(embed_hidden, self.tok_emb.weight.to(dtype=x.dtype))
        else:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        bank_n = self.num_shared_blocks
        x = self.embed_up(self.tok_emb(input_ids))
        ...
        for i in range(self.num_encoder_layers):
            si = self.shared_block_map[i]
            ve = self._get_ve(i, input_ids, ve_cache)
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[si], self.kv_bank[si], self.kv_bank[bank_n + si],
                self.qo_bank[bank_n + si], self.mlp_up_bank[si], self.mlp_down_bank[si],
                v_embed=ve, v0=v0)
...
def eval_val_sliding_ttt(
    ...
    frozen_shared_ids = (
        {base_model.shared_block_map[i] for i in frozen_block_ids}
        if frozen_block_ids and hasattr(base_model, "shared_block_map")
        else set()
    )
    ttt_params = []
    ...
    def mask_frozen_bank_grads() -> None:
        if not frozen_shared_ids or not hasattr(base_model, "num_shared_blocks"):
            return
        bank_n = int(base_model.num_shared_blocks)
        frozen = sorted(frozen_shared_ids)
        attn_rows = frozen + [bank_n + idx for idx in frozen]
        if base_model.qo_bank.grad is not None:
            base_model.qo_bank.grad[attn_rows].zero_()
        if base_model.kv_bank.grad is not None:
            base_model.kv_bank.grad[attn_rows].zero_()
        if base_model.mlp_up_bank.grad is not None:
            base_model.mlp_up_bank.grad[frozen].zero_()
        if base_model.mlp_down_bank.grad is not None:
            base_model.mlp_down_bank.grad[frozen].zero_()

    log0(f"ttt_sliding:params unfrozen={sum(p.numel() for p in ttt_params)} "
          f"frozen={sum(p.numel() for p in base_model.parameters() if not p.requires_grad)}")
    if frozen_shared_ids:
        log0(f"ttt_sliding:frozen_shared_blocks={sorted(frozen_shared_ids)}")
...
def _classify_param(name: str) -> str:
    if "tok_emb" in name or "lm_head" in name or "embed_up" in name:
        return "embed"
    if ".mlp." in name:
        return "mlp"
...
def _unbank_state_dict(sd: dict[str, Tensor], num_shared_blocks: int) -> dict[str, Tensor]:
    """Convert 3D shared-bank tensors into individual 2D tensors with stable names."""
    out: dict[str, Tensor] = {}
    n = num_shared_blocks
...
def _rebank_state_dict(sd: dict[str, Tensor], num_shared_blocks: int, template_sd: dict[str, Tensor]) -> dict[str, Tensor]:
    """Convert individual 2D shared-block tensors back into 3D bank tensors."""
    out: dict[str, Tensor] = {}
    n = num_shared_blocks
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_block_mode=args.shared_block_mode,
    ...
    # - token embedding + factorized embed_up -> Adam
    ...
    tok_params = [{"params": [base_model.tok_emb.weight, base_model.embed_up.weight], "lr": token_lr, "base_lr": token_lr}]
    if base_model.bigram is not None:
    ...
    log0(
        f"embed_dim:{args.embed_dim} num_shared_blocks:{args.num_shared_blocks} "
        f"shared_block_mode:{args.shared_block_mode}"
    )
    ...
    unbanked_sd = _unbank_state_dict(sd_cpu, args.num_shared_blocks)
    ...
    deq_state = _rebank_state_dict(deq_unbanked, args.num_shared_blocks, sd_cpu)
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim,
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult,
        tie_embeddings=args.tie_embeddings, tied_embed_init_std=args.tied_embed_init_std,
        logit_softcap=args.logit_softcap, rope_base=args.rope_base, qk_gain_init=args.qk_gain_init,
        embed_dim=args.embed_dim, num_shared_blocks=args.num_shared_blocks, shared_block_mode=args.shared_block_mode,
