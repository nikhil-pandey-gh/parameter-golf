...
def maybe_compile(module_or_fn, enabled: bool):
    return torch.compile(module_or_fn, dynamic=False, fullgraph=True) if enabled else module_or_fn
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = maybe_compile(base_model.forward_logits, device.type == "cuda" and bool(int(os.environ.get("USE_TORCH_COMPILE", "1"))))
    with torch.inference_mode():
...
def main() -> None:
    ...
    compile_enabled = device.type == "cuda" and bool(int(os.environ.get("USE_TORCH_COMPILE", "1")))
    ...
    if device.type == "cuda":
        ...
        log0("Running on CPU fallback path", console=False)
    log0("=" * 100, console=False)
    ...
    log0("sdp_backends:cudnn=False flash=True mem_efficient=False math=False" if device.type == "cuda" else "sdp_backends:cpu-fallback")
