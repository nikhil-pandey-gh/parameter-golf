...
def flash_attn_or_sdpa(q: Tensor, k: Tensor, v: Tensor, *, causal: bool) -> Tensor:
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        y = flash_attn_or_sdpa(q, k, v, causal=True)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        layer_scale = self.ln_scale_factor if ln_scale_factor is None else ln_scale_factor
        attn_out = self.attn(
            self.attn_norm(x_in) * layer_scale,
            v_embed=v_embed,
            lora=lora,
            use_xsa=use_xsa,
        )
        x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
        x_out = x_out + self.mlp_scale.to(dtype=x_out.dtype)[None, None, :] * self.mlp(self.mlp_norm(x_out) * layer_scale)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.num_loops = max(num_loops, 1)
        self.lora_rank = max(lora_rank, 0)
        self.effective_depth = self.num_unique_layers * self.num_loops
        ...
        if self.lora_rank > 0 and self.num_loops > 1:
            self.lora_adapters = nn.ModuleList(
                [
                    nn.ModuleList([AttentionLoRA(model_dim, kv_dim, self.lora_rank) for _ in range(self.num_unique_layers)])
        ...
        parsed_ve_layers = [int(x) for x in ve_layers.split(",") if x.strip()] if ve_enabled else []
        self.ve_layer_indices = sorted({idx for idx in parsed_ve_layers if 0 <= idx < self.effective_depth})
    ...
    def _effective_use_xsa(self, layer_idx: int) -> bool:
        return self.xsa_last_n > 0 and layer_idx >= self.effective_depth - min(self.xsa_last_n, self.effective_depth)
    def _get_lora(self, loop_idx: int, block_idx: int) -> AttentionLoRA | None:
        if self.lora_adapters is None or loop_idx == 0:
            return None
        return self.lora_adapters[loop_idx - 1][block_idx]
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for loop_idx in range(self.num_loops):
            for block_idx in range(self.num_unique_layers):
                ...
                lora = self._get_lora(loop_idx, block_idx)
                ln_scale_factor = 1.0 / math.sqrt(eff_idx + 1) if block.ln_scale_enabled else None
                if eff_idx < self.num_encoder_layers:
                    x = block(
                        ...
                        v_embed=ve,
                        lora=lora,
                        use_xsa=self._effective_use_xsa(eff_idx),
...
def main() -> None:
    ...
    log0(
        f"model_params:{n_params} unique_layers:{args.num_layers} loops:{args.num_loops} "
        f"effective_depth:{base_model.effective_depth} lora_rank:{args.lora_rank} lora_params:{lora_param_count}"
    )
    log0(f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} mtp_params:{mtp_params}")
    xsa_layers = [i for i in range(base_model.effective_depth) if base_model._effective_use_xsa(i)]
    log0(f"XSA:last_{args.xsa_last_n} active_effective_layers:{xsa_layers}")
    log0(f"VE:effective_layers:{base_model.ve_layer_indices}")
    ...
    eval_model = GPT(
        ...
        num_loops=args.num_loops, lora_rank=args.lora_rank,
    ).to(device).bfloat16()
