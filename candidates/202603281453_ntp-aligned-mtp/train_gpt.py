...
try:
    ...
    _ATTENTION_IMPL = "flash_attn_3"
except ImportError:
    flash_attn_3_func = None
    _ATTENTION_IMPL = "sdpa_fallback"
...
def run_causal_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
...
class Hyperparameters:
    ...
    mtp_warmup_steps = int(os.environ.get("MTP_WARMUP_STEPS", 400))
    mtp_disable_lr_scale = float(os.environ.get("MTP_DISABLE_LR_SCALE", 0.35))
    mtp_align_init = bool(int(os.environ.get("MTP_ALIGN_INIT", "1")))
...
def eval_val(
    ...
    mtp_disabled = torch.zeros((), device=device, dtype=torch.float32)
    model.eval()
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                batch_loss = model(x, y, mtp_disabled).detach()
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_align_init = mtp_align_init
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_align_init and self.tie_embeddings and len(self.mtp_heads) > 0:
            source = self.tok_emb.weight
            with torch.no_grad():
                for head in self.mtp_heads:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_weight: Tensor | float | None = None) -> Tensor:
        n = self.num_layers
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            if mtp_loss_count > 0:
                if mtp_weight is None:
                    mtp_weight = x.new_tensor(float(self.mtp_loss_weight))
                elif not isinstance(mtp_weight, Tensor):
                    mtp_weight = x.new_tensor(float(mtp_weight))
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_align_init=args.mtp_align_init,
    ...
    log0(
        ...
        f"disable_lr_scale:{args.mtp_disable_lr_scale:.3f} align_init:{args.mtp_align_init}"
    )
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"attention_impl:{_ATTENTION_IMPL}")
    ...
    def mtp_aux_weight(step: int, scale: float) -> float:
        ...
        cooldown = 1.0
        if 0.0 < args.mtp_disable_lr_scale < 1.0 and scale < args.mtp_disable_lr_scale:
            cooldown = max(scale / args.mtp_disable_lr_scale, 0.0)
        return args.mtp_loss_weight * warm * cooldown
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            zero_grad_all()
            warmup_mtp_weight = torch.tensor(mtp_aux_weight(warmup_step, 1.0), device=device)
    ...
    while True:
        ...
        mtp_weight_value = mtp_aux_weight(step, scale)
        mtp_weight = torch.tensor(mtp_weight_value, device=device)
        ...
        if should_log_train:
            log0(
                ...
                f"mtp_weight:{mtp_weight_value:.4f} "
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_align_init=False,
