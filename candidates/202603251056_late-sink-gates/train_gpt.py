...
class Hyperparameters:
    ...
    dtg_enabled = bool(int(os.environ.get("DTG_ENABLED", "1")))
    ...
    sink_gate_last_n = int(os.environ.get("SINK_GATE_LAST_N", 4))
    value_residual_last_n = int(os.environ.get("VALUE_RESIDUAL_LAST_N", 4))
    attn_gate_detach = bool(int(os.environ.get("ATTN_GATE_DETACH", "1")))
    dtg_gate_detach = bool(int(os.environ.get("DTG_GATE_DETACH", "1")))
    attn_gate_init_bias = float(os.environ.get("ATTN_GATE_INIT_BIAS", 3.0))
    dtg_gate_init_bias = float(os.environ.get("DTG_GATE_INIT_BIAS", 2.0))
    ...
    cpu_smoke_seq_len = int(os.environ.get("CPU_SMOKE_SEQ_LEN", 64))
    cpu_smoke_batch_size = int(os.environ.get("CPU_SMOKE_BATCH_SIZE", 2))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        attn_gate_detach: bool = True,
        attn_gate_init_bias: float = 3.0,
    ):
        super().__init__()
        if dim % num_heads != 0:
        ...
        self.attn_gate_detach = attn_gate_detach
        if gated_attention:
            self.attn_gate = nn.Linear(dim, num_heads, bias=True)
            nn.init.zeros_(self.attn_gate.weight)
            nn.init.constant_(self.attn_gate.bias, attn_gate_init_bias)
    ...
    def _attention(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        if flash_attn_3_func is not None and q.device.type == "cuda":
    ...
    def forward(
        ...
        if self.gated_attention:
            ...
            if self.attn_gate_detach:
                gate_source = gate_source.detach()
            gate = torch.sigmoid(self.attn_gate(gate_source)).unsqueeze(-1)
            y = y * gate
...
class Block(nn.Module):
    def __init__(
        ...
        value_residual: bool = False,
        attn_gate_detach: bool = True,
        dtg_gate_detach: bool = True,
        attn_gate_init_bias: float = 3.0,
        dtg_gate_init_bias: float = 2.0,
    ):
        super().__init__()
        ...
        self.attn = CausalSelfAttention(
            ...
            attn_gate_detach=attn_gate_detach,
        ...
        self.dtg_gate_detach = dtg_gate_detach
        if dtg:
            self.dtg_gate = nn.Linear(dim, 1, bias=True)
            nn.init.zeros_(self.dtg_gate.weight)
            nn.init.constant_(self.dtg_gate.bias, dtg_gate_init_bias)
        else:
            self.dtg_gate = None
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if self.dtg_gate is not None:
            gate_input = x_in.detach() if self.dtg_gate_detach else x_in
            gate = torch.sigmoid(self.dtg_gate(gate_input))
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        sink_gate_last_n: int = 0,
        value_residual_last_n: int = 0,
        attn_gate_detach: bool = True,
        dtg_gate_detach: bool = True,
        attn_gate_init_bias: float = 3.0,
        dtg_gate_init_bias: float = 2.0,
        ...
        sink_gate_start = max(0, num_layers - sink_gate_last_n) if sink_gate_last_n > 0 else num_layers
        value_residual_start = (
            max(0, num_layers - value_residual_last_n) if value_residual_last_n > 0 else num_layers
        )
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    layer_idx=i,
                    ln_scale=ln_scale,
                    dtg=dtg and i >= sink_gate_start,
                    gated_attention=gated_attention and i >= sink_gate_start,
                    value_residual=value_residual and i >= value_residual_start,
                    attn_gate_detach=attn_gate_detach,
                    dtg_gate_detach=dtg_gate_detach,
                    attn_gate_init_bias=attn_gate_init_bias,
                    dtg_gate_init_bias=dtg_gate_init_bias,
                )
...
def main() -> None:
    ...
    if args.cpu_smoke_test:
        ...
        smoke_seq_len = max(2, min(args.cpu_smoke_seq_len, args.train_seq_len))
        smoke_batch = max(1, args.cpu_smoke_batch_size)
        smoke_model = GPT(
            ...
            sink_gate_last_n=args.sink_gate_last_n,
            value_residual_last_n=args.value_residual_last_n,
            attn_gate_detach=args.attn_gate_detach,
            dtg_gate_detach=args.dtg_gate_detach,
            attn_gate_init_bias=args.attn_gate_init_bias,
            dtg_gate_init_bias=args.dtg_gate_init_bias,
        ).float()
        restore_low_dim_params_to_fp32(smoke_model)
        smoke_model.eval()
        x = torch.arange(smoke_batch * smoke_seq_len, dtype=torch.int64).reshape(smoke_batch, smoke_seq_len)
        x %= args.vocab_size
        y = (x + 1) % args.vocab_size
        with torch.inference_mode():
            loss = smoke_model(x, y)
            logits = smoke_model.forward_logits(x[:1, : min(smoke_seq_len, 8)])
        print(f"cpu_smoke:ok loss:{loss.item():.4f} logits_shape:{tuple(logits.shape)}")
        return
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required unless CPU_SMOKE_TEST=1")
    ...
    gated_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.gated_attention]
    dtg_layers = [i for i, b in enumerate(base_model.blocks) if b.dtg_gate is not None]
    vr_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.value_residual]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(
        f"sink_control:last_{args.sink_gate_last_n} gated_attention_layers:{gated_layers} "
        f"dtg_layers:{dtg_layers} value_residual_last_{args.value_residual_last_n}:{vr_layers}"
    )
