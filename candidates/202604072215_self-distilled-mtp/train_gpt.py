...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            bsz, seqlen, dim = x.shape
            teacher_logits = logits.detach().reshape(bsz, seqlen, -1).float()
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_logits = self.logit_softcap * torch.tanh(mtp_logits_proj / self.logit_softcap).float()
                mtp_loss = F.cross_entropy(mtp_logits, mtp_targets, reduction="mean")
                if self.mtp_distill_weight > 0.0:
                    teacher_shifted = teacher_logits[:, k + 1 :, :].reshape(-1, teacher_logits.size(-1))
                    temp = self.mtp_distill_temp
                    student_log_probs = F.log_softmax(mtp_logits / temp, dim=-1)
                    teacher_probs = F.softmax(teacher_shifted / temp, dim=-1)
                    distill_loss = F.kl_div(student_log_probs, teacher_probs, reduction="batchmean") * (temp * temp)
                    mtp_loss = mtp_loss + self.mtp_distill_weight * distill_loss
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_distill_weight:{args.mtp_distill_weight} mtp_distill_temp:{args.mtp_distill_temp} "
