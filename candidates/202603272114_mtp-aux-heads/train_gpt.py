...
def attention_kernel(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    return F.scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, is_causal=causal).transpose(1, 2)
...
class Hyperparameters:
    ...
    # MTP heads are a training-time auxiliary; export still strips them from the artifact.
...
def main() -> None:
    ...
    if base_model.mtp_num_heads > 0:
        scalar_params.extend(p for p in base_model.mtp_heads.parameters() if p.requires_grad)
    ...
    log0(
        f"sdp_backends:cudnn=False flash=True mem_efficient={use_sdpa_fallback} "
        f"math={use_sdpa_fallback} external_flash_attn={flash_attn_3_func is not None}"
    )
