...
class Hyperparameters:
    ...
    calibration_batches = int(os.environ.get("CALIBRATION_BATCHES", 16))
...
CLIP_SEARCH_CANDIDATES = (0.9990, 0.9995, 0.9999, 0.99999, 1.0)
...
def keep_float_tensor(name: str, t: Tensor, passthrough_orig_dtypes: dict[str, str]) -> Tensor:
    ...
    return t
def _activation_weighted_row_error(delta: Tensor, act_sq: Tensor | None) -> Tensor:
    if act_sq is None or delta.ndim != 2 or act_sq.ndim != 1 or act_sq.numel() != delta.shape[1]:
        return delta.pow(2).mean(dim=1)
    weights = act_sq.to(dtype=delta.dtype, device=delta.device)
    weights = weights / weights.mean().clamp_min(1e-12)
    return (delta.pow(2) * weights[None, :]).mean(dim=1)
def quantize_float_tensor(t: Tensor, act_sq: Tensor | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q = None
        best_s = None
        best_err = None
        for pct in (*CLIP_SEARCH_CANDIDATES[:-1], INT8_CLIP_Q, CLIP_SEARCH_CANDIDATES[-1]):
            clip_abs = (
            ...
            row_err = _activation_weighted_row_error(t32 - recon, act_sq)
            if best_err is None:
                best_q, best_s, best_err = q, scale, row_err
                continue
...
def _record_activation(calibration_state: dict[str, list[Tensor]] | None, name: str, x: Tensor) -> None:
    if calibration_state is None:
        return
    flat = x.detach().float().reshape(-1, x.shape[-1])
    sum_sq = flat.square().sum(dim=0)
    count = torch.tensor(float(flat.shape[0]), device=sum_sq.device, dtype=torch.float32)
    if name in calibration_state:
        calibration_state[name][0].add_(sum_sq)
        calibration_state[name][1].add_(count)
    else:
        calibration_state[name] = [sum_sq, count]
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        v0: Tensor | None = None,
        calibration_state: dict[str, list[Tensor]] | None = None,
        ...
        if layer_idx is not None:
            _record_activation(calibration_state, f"blocks.{layer_idx}.attn.proj.weight", y)
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        x = x.square()
        if layer_idx is not None:
            _record_activation(calibration_state, f"blocks.{layer_idx}.mlp.proj.weight", x)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        _record_activation(calibration_state, f"blocks.{self.layer_idx}.attn.c_q.weight", attn_in)
        _record_activation(calibration_state, f"blocks.{self.layer_idx}.attn.c_k.weight", attn_in)
        _record_activation(calibration_state, f"blocks.{self.layer_idx}.attn.c_v.weight", attn_in)
        ...
        _record_activation(calibration_state, f"blocks.{self.layer_idx}.mlp.fc.weight", mlp_in)
    ...
    def forward_logits(
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[i], self.mlp_down_bank[i],
                v_embed=ve, v0=v0, calibration_state=calibration_state)
            if v0 is None and raw_v is not None:
                v0 = raw_v
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0, calibration_state=calibration_state)
        x = self.final_norm(x)
        head_name = "tok_emb.weight" if self.tie_embeddings else "lm_head.weight"
        _record_activation(calibration_state, head_name, x)
        if self.tie_embeddings:
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, clip_range: int = 31, act_sq: Tensor | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q = None
        best_s = None
        best_err = None
        for pct in CLIP_SEARCH_CANDIDATES:
            if pct < 1.0:
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        act_sq = activation_stats.get(name) if activation_stats is not None else None
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, act_sq=act_sq)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
            q, s = quantize_float_tensor(t, act_sq=act_sq)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int8"}
    return result, meta
...
def collect_activation_stats(
    ...
    if not args.activation_aware_quant or args.calibration_batches <= 0:
        return {}
    calibration_loader = DistributedTokenLoader(args.train_files, rank, world_size, device)
    calibration_state: dict[str, list[Tensor]] = {}
    torch.cuda.synchronize()
    t0 = time.perf_counter()
    base_model.eval()
    with torch.inference_mode():
        for _ in range(args.calibration_batches):
            x, _ = calibration_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                base_model.forward_logits(x, calibration_state=calibration_state)
    if dist.is_available() and dist.is_initialized():
        for name in sorted(calibration_state):
            sum_sq, count = calibration_state[name]
            dist.all_reduce(sum_sq, op=dist.ReduceOp.SUM)
            dist.all_reduce(count, op=dist.ReduceOp.SUM)
    stats = {
        name: (sum_sq / count.clamp_min(1.0)).detach().cpu()
        for name, (sum_sq, count) in calibration_state.items()
    }
    torch.cuda.synchronize()
    log0(
        f"activation_calibration:batches:{args.calibration_batches} tensors:{len(stats)} "
        f"time:{1000.0 * (time.perf_counter() - t0):.0f}ms"
    )
    return stats
...
def main() -> None:
    ...
    log0(
        f"activation_aware_quant:{args.activation_aware_quant} "
    ...
    activation_stats = collect_activation_stats(
        args, base_model, rank, world_size, device, grad_accum_steps, log0
    )
    ...
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, activation_stats=activation_stats)
