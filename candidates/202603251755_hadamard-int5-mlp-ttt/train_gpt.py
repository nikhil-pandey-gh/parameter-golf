...
class Hyperparameters:
    ...
    quant_rotate_mlp = bool(int(os.environ.get("QUANT_ROTATE_MLP", "1")))
    quant_rotation_block = int(os.environ.get("QUANT_ROTATION_BLOCK", 128))
...
# --- Rotated mixed-bit export quantization ---
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def _clip_range_from_bits(bits: int) -> int:
    if not 2 <= bits <= 8:
        raise ValueError(f"Expected quantization bits in [2, 8], got {bits}")
    return (1 << (bits - 1)) - 1

def _fwht_last_dim(x: Tensor) -> Tensor:
    n = x.size(-1)
    if n <= 0 or (n & (n - 1)) != 0:
        raise ValueError(f"FWHT requires a power-of-two trailing dim, got {n}")
...
def apply_block_hadamard_last_dim(x: Tensor, block_size: int) -> Tensor:
    if block_size <= 1:
        return x
    if (block_size & (block_size - 1)) != 0:
        raise ValueError(f"Block Hadamard size must be a power of two, got {block_size}")
    if x.ndim != 2:
        return x
    cols = x.size(-1)
    if cols % block_size != 0:
        return x
    blocks = x.reshape(x.shape[0], cols // block_size, block_size)
    return _fwht_last_dim(blocks).reshape_as(x)

def quantize_int_per_row(t: Tensor, clip_range: int = 31, rotate_block_size: int = 0) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if rotate_block_size > 1 and t32.ndim == 2:
        t32 = apply_block_hadamard_last_dim(t32, rotate_block_size)
    if t32.ndim == 2:
...
def mixed_quantize_rotated(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in {"mlp", "attn"} and t.ndim >= 1:
            bits = mlp_bits if cat == "mlp" else attn_bits
            rotate_block_size = rotation_block if cat == "mlp" and rotate_mlp and t.ndim == 2 else 0
            q, s = quantize_int_per_row(t, clip_range=_clip_range_from_bits(bits), rotate_block_size=rotate_block_size)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {
                "type": f"int{bits}",
...
def dequantize_mixed_rotated(
    ...
    for name, orig in template_sd.items():
        ...
        rotate_block_size = int(info.get("rotate_block_size", 0)) if isinstance(info, dict) else 0
        if rotate_block_size > 1 and restored.ndim == 2:
            restored = apply_block_hadamard_last_dim(restored, rotate_block_size)
...
def main() -> None:
    ...
    log0(
        f"quant_export mlp_bits:{args.mlp_quant_bits} attn_bits:{args.attn_quant_bits} "
        f"rotate_mlp:{int(args.quant_rotate_mlp)} rotation_block:{args.quant_rotation_block}"
    )
    quant_result, quant_meta = mixed_quantize_rotated(
        unbanked_sd,
        mlp_bits=args.mlp_quant_bits,
        attn_bits=args.attn_quant_bits,
        rotate_mlp=args.quant_rotate_mlp,
        rotation_block=args.quant_rotation_block,
    )
    ...
    if master_process:
        with open("final_model.mixedint.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model mixedint+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size mixedint+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    with open("final_model.mixedint.ptz", "rb") as f:
        quant_blob_disk = f.read()
    quant_state = torch.load(
        io.BytesIO(lzma.decompress(quant_blob_disk)),
        map_location="cpu",
    )
    deq_unbanked = dequantize_mixed_rotated(quant_state["w"], quant_state["m"], unbanked_sd)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_mixedint_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_mixedint_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixedint_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_mixedint_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
    ...
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_mixedint_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(f"final_mixedint_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
