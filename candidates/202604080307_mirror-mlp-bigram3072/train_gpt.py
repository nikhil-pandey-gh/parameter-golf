...
class Hyperparameters:
    ...
    shared_mlp_mode = os.environ.get("SHARED_MLP_MODE", "mirror")
...
def build_mlp_share_map(num_layers: int, mode: str) -> list[int]:
    mode = mode.lower()
    if mode == "none":
        return list(range(num_layers))
    if mode != "mirror":
        raise ValueError(f"Unsupported SHARED_MLP_MODE={mode!r}; expected 'none' or 'mirror'")
    num_encoder_layers = num_layers // 2
    num_decoder_layers = num_layers - num_encoder_layers
    mapping = list(range(num_encoder_layers))
    for decoder_idx in range(num_decoder_layers):
        if decoder_idx < num_encoder_layers:
            mapping.append(num_encoder_layers - 1 - decoder_idx)
        else:
            mapping.append(num_encoder_layers + decoder_idx - num_encoder_layers)
    return mapping
...
class GPT(nn.Module):
    def __init__(
        ...
        # Store shared MLP weights once so the compressed export actually benefits from tying.
        self.shared_mlp_mode = shared_mlp_mode.lower()
        self.block_to_shared_mlp = build_mlp_share_map(num_layers, self.shared_mlp_mode)
        self.num_shared_mlps = max(self.block_to_shared_mlp, default=-1) + 1
        self.shared_mlps = nn.ModuleList([MLP(model_dim, mlp_mult) for _ in range(self.num_shared_mlps)])
    ...
    def _mlp_for_layer(self, layer_idx: int) -> MLP:
        return self.shared_mlps[self.block_to_shared_mlp[layer_idx]]
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self.blocks[i](x, x0, self._mlp_for_layer(i), v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self.blocks[bi](x, x0, self._mlp_for_layer(bi), v_embed=ve)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_mlp_mode=args.shared_mlp_mode,
    ).to(device).bfloat16()
    ...
    log0(
        f"shared_mlp_mode:{args.shared_mlp_mode} shared_mlps:{base_model.num_shared_mlps} "
        f"mlp_map:{base_model.block_to_shared_mlp}"
    )
