...
class Hyperparameters:
    ...
    segment_len = int(os.environ.get("SEGMENT_LEN", 1024))
    mem_len = int(os.environ.get("MEM_LEN", 512))
    mem_last_n = int(os.environ.get("MEM_LAST_N", 4))
...
class CausalSelfAttention(nn.Module):
    ...
    def _sdpa(self, q: Tensor, k: Tensor, v: Tensor, mem_tokens: int) -> Tensor:
        ...
        if mem_tokens > 0:
            q_tokens = q.size(1)
            causal = torch.ones((q_tokens, q_tokens), dtype=torch.bool, device=q.device).tril()
            prefix = torch.ones((q_tokens, mem_tokens), dtype=torch.bool, device=q.device)
            attn_mask = torch.cat((prefix, causal), dim=1)[None, None, :, :]
    ...
    def forward(
        ...
        mem_v_embed: Tensor | None = None,
    ) -> tuple[Tensor, Tensor | None]:
        bsz, seqlen, dim = x.shape
        mem_tokens = 0 if mem is None else mem.size(1)
        source = x if mem_tokens == 0 else torch.cat((mem, x), dim=1)
        q = F.linear(x, q_w.to(x.dtype)).reshape(bsz, seqlen, self.num_heads, self.head_dim)
        k = F.linear(source, k_w.to(x.dtype)).reshape(bsz, source.size(1), self.num_kv_heads, self.head_dim)
        v = F.linear(source, v_w.to(x.dtype))
        if mem_tokens > 0 and mem_v_embed is not None:
            v[:, :mem_tokens] = v[:, :mem_tokens] + mem_v_embed.to(dtype=v.dtype)
        if v_embed is not None:
            v[:, mem_tokens:] = v[:, mem_tokens:] + v_embed.to(dtype=v.dtype)
        v = v.reshape(bsz, source.size(1), self.num_kv_heads, self.head_dim)
        current_v = v[:, mem_tokens:]
        raw_v = current_v if self.value_residual else None
        if self.value_residual and v0 is not None:
            lam = self.vr_lambda.to(dtype=v.dtype)
            current_v = lam[0] * v0 + lam[1] * current_v
            v = torch.cat((v[:, :mem_tokens], current_v), dim=1) if mem_tokens > 0 else current_v
        q = F.rms_norm(q, (q.size(-1),))
        k = F.rms_norm(k, (k.size(-1),))
        total_len = source.size(1)
        cos, sin = self.rotary(total_len, x.device, q.dtype)
        q = apply_rotary_emb(q, cos[:, mem_tokens:], sin[:, mem_tokens:], self.rope_dims)
        k = apply_rotary_emb(k, cos, sin, self.rope_dims)
        q = q * self.q_gain.to(dtype=q.dtype)[None, None, :, None]
        if flash_attn_3_func is not None and mem_tokens == 0 and x.is_cuda:
            y = flash_attn_3_func(q, k, v, causal=True)
        else:
            y = self._sdpa(q, k, v, mem_tokens)
        if self.use_xsa:
            y = self._xsa_efficient(y, current_v)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.rotary_train_seq_len = max(int(rotary_train_seq_len), 1)
        self.value_residual = value_residual
        self.segment_len = max(int(segment_len), 0)
        self.mem_len = max(int(mem_len), 0)
        self.mem_last_n = max(int(mem_last_n), 0)
        self.memory_layer_indices = tuple(
            range(max(0, num_layers - self.mem_last_n), num_layers)
        ) if self.segment_len > 0 and self.mem_len > 0 and self.mem_last_n > 0 else ()
        self.segment_recurrence_enabled = bool(self.memory_layer_indices)
        ...
        for block in self.blocks:
            block.attn.rotary.train_seq_len = self.rotary_train_seq_len
        if rope_dims > 0:
            head_dim = model_dim // num_heads
            for block in self.blocks:
                ...
                block.attn.rotary = Rotary(
                    head_dim,
                    base=rope_base,
                    train_seq_len=self.rotary_train_seq_len,
                    rope_dims=rope_dims,
                )
    ...
    @staticmethod
    def _append_memory(prev: Tensor | None, current: Tensor, mem_len: int) -> Tensor | None:
        if mem_len <= 0:
            return None
        current = current.detach()
        if prev is None or prev.numel() == 0:
            return current[:, -mem_len:, :].contiguous()
        merged = torch.cat((prev, current), dim=1)
        return merged[:, -mem_len:, :].contiguous()
    @staticmethod
    def _append_token_memory(prev: Tensor | None, current: Tensor, mem_len: int) -> Tensor | None:
        if mem_len <= 0:
            return None
        current = current.detach()
        if prev is None or prev.numel() == 0:
        ...
        merged = torch.cat((prev, current), dim=1)
        return merged[:, -mem_len:].contiguous()
    def _build_inputs(self, input_ids: Tensor) -> Tensor:
        x = self.tok_emb(input_ids)
        if self.bigram is not None:
            x = x + self.bigram(input_ids)
        x = F.rms_norm(x, (x.size(-1),))
        return self.smear(x)
    def _run_block_stack(self, x: Tensor, input_ids: Tensor) -> Tensor:
        n = self.num_layers
        x0 = x
        v0 = None
        skips: list[Tensor] = []
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                ...
                v_embed=ve, v0=v0)
        return x
    def _run_block_stack_segmented(self, x_all: Tensor, input_ids: Tensor) -> Tensor:
        n = self.num_layers
        memory_layers = self.memory_layer_indices
        memory_states: list[Tensor | None] = [None] * n
        memory_token_ids: list[Tensor | None] = [None] * n
        outputs: list[Tensor] = []
        for start in range(0, x_all.size(1), self.segment_len):
            end = min(start + self.segment_len, x_all.size(1))
            x = x_all[:, start:end, :]
            ...
            for i in range(self.num_encoder_layers):
                block = self.blocks[i]
                ve = self._get_ve(i, segment_ids, ve_cache)
                mem = memory_states[i] if i in memory_layers else None
                mem_ve = self._get_ve(i, memory_token_ids[i], None) if memory_token_ids[i] is not None else None
                mix = block.resid_mix.to(dtype=x.dtype)
                attn_mem_src = block.attn_norm(
                    mix[0][None, None, :] * x + mix[1][None, None, :] * x0
                ) * block.ln_scale_factor
                x, raw_v = block(
                    x,
                    x0,
                    self.qo_bank[i],
                ...
                if i in memory_layers:
                    memory_states[i] = self._append_memory(memory_states[i], attn_mem_src, self.mem_len)
                    if i in self.ve_layer_indices:
                        memory_token_ids[i] = self._append_token_memory(memory_token_ids[i], segment_ids, self.mem_len)
                if v0 is None and raw_v is not None:
                    v0 = raw_v
                skips.append(x)
            for i in range(self.num_decoder_layers):
                ...
                ve = self._get_ve(bi, segment_ids, ve_cache)
                mem = memory_states[bi] if bi in memory_layers else None
                mem_ve = self._get_ve(bi, memory_token_ids[bi], None) if memory_token_ids[bi] is not None else None
                mix = block.resid_mix.to(dtype=x.dtype)
                attn_mem_src = block.attn_norm(
                    mix[0][None, None, :] * x + mix[1][None, None, :] * x0
                ) * block.ln_scale_factor
                ...
                if bi in memory_layers:
                    memory_states[bi] = self._append_memory(memory_states[bi], attn_mem_src, self.mem_len)
                    if bi in self.ve_layer_indices:
                        memory_token_ids[bi] = self._append_token_memory(memory_token_ids[bi], segment_ids, self.mem_len)
            outputs.append(x)
        return torch.cat(outputs, dim=1)
    def _hidden_states(self, input_ids: Tensor) -> Tensor:
        x = self._build_inputs(input_ids)
        if self.segment_recurrence_enabled and input_ids.size(1) > self.segment_len:
            x = self._run_block_stack_segmented(x, input_ids)
        else:
            x = self._run_block_stack(x, input_ids)
        x = self.final_norm(x)
        return x
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        x = self._hidden_states(input_ids)
...
def build_gpt(args: Hyperparameters, **overrides: object) -> GPT:
    kwargs: dict[str, object] = dict(
        ...
        rotary_train_seq_len=max(args.train_seq_len, args.segment_len + args.mem_len),
        ...
        mem_last_n=args.mem_last_n,
    )
    kwargs.update(overrides)
    return GPT(**kwargs)

def prepare_model_for_runtime(model: GPT) -> GPT:
...
def run_cpu_smoke(args: Hyperparameters) -> None:
    smoke_segment_len = max(64, min(args.segment_len if args.segment_len > 0 else 128, 128))
    smoke_mem_len = min(args.mem_len if args.mem_len > 0 else smoke_segment_len // 2, smoke_segment_len)
    smoke_seq_len = max(2 * smoke_segment_len, 128)
    smoke_seq_len = min(smoke_seq_len, 256)
    smoke_model = prepare_model_for_runtime(
        build_gpt(
            args,
            segment_len=smoke_segment_len,
            mem_len=smoke_mem_len,
            mem_last_n=max(args.mem_last_n, 1),
        )
    )
    smoke_model.train()
    x = torch.randint(args.vocab_size, (2, smoke_seq_len), dtype=torch.int64)
    y = torch.randint(args.vocab_size, (2, smoke_seq_len), dtype=torch.int64)
    loss = smoke_model(x, y)
    if not torch.isfinite(loss):
        raise RuntimeError(f"CPU smoke produced non-finite loss: {loss.item()}")
    logits = smoke_model.forward_logits(x[:1])
    expected_shape = (1, smoke_seq_len, args.vocab_size)
    if tuple(logits.shape) != expected_shape:
        raise RuntimeError(f"CPU smoke logits shape mismatch: got {tuple(logits.shape)}, expected {expected_shape}")
    print(f"cpu_smoke_ok loss:{float(loss.detach()):.4f} logits_shape:{tuple(logits.shape)}")
...
def main() -> None:
    ...
    args = Hyperparameters()
    if bool(int(os.environ.get("CPU_SMOKE_TEST", "0"))):
        run_cpu_smoke(args)
        return
    ...
    base_model = prepare_model_for_runtime(build_gpt(args).to(device).bfloat16())
    ...
    if base_model.mtp_heads:
        scalar_params.extend(head.weight for head in base_model.mtp_heads)
    ...
    log0(
        f"segment_recurrence:enabled={base_model.segment_recurrence_enabled} "
        f"segment_len:{args.segment_len} mem_len:{args.mem_len} mem_layers:{list(base_model.memory_layer_indices)}"
    )
    ...
    eval_model = prepare_model_for_runtime(
        build_gpt(args, mtp_num_heads=0, mtp_loss_weight=0.0).to(device).bfloat16()
    )
