...
class CastedLinear(nn.Linear):
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and w.ndim == 2 and bool(getattr(self, "_qat_enabled", False)):
            with torch.no_grad():
...
def set_qat_enabled(module: nn.Module, enabled: bool) -> None:
    for submodule in module.modules():
        if isinstance(submodule, CastedLinear):
            submodule._qat_enabled = enabled
...
def main() -> None:
    ...
    # - MTP heads -> Muon (same pattern as earlier pre-banking MTP experiments)
    matrix_params = [
