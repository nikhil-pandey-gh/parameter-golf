...
class CausalDepthwiseConv1d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.kernel_size == 1:
            return x
        y = self.conv(x.transpose(1, 2).contiguous())
        return y[..., : x.size(1)].transpose(1, 2).contiguous()
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.q_conv = CausalDepthwiseConv1d(dim, primer_kernel_size) if primer_kernel_size > 1 else None
        self.k_conv = CausalDepthwiseConv1d(kv_dim, primer_kernel_size) if primer_kernel_size > 1 else None
        self.v_conv = CausalDepthwiseConv1d(kv_dim, primer_kernel_size) if primer_kernel_size > 1 else None
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, primer_kernel_size)
...
def main() -> None:
    ...
    log0(f"primer_depthwise_qkv kernel_size:{args.primer_kernel_size} flash_attn:{flash_attn_3_func is not None}")
    log0(
