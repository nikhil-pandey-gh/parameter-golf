    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        if flash_attn_3_func is not None and x.is_cuda:
            ...
            if self.num_heads != self.num_kv_heads:
                ...
                k_attn = k_attn.repeat_interleave(group_size, dim=1)
                v_attn = v_attn.repeat_interleave(group_size, dim=1)
            y = F.scaled_dot_product_attention(q_attn, k_attn, v_attn, is_causal=True).permute(0, 2, 1, 3)
        if self.use_xsa:
...
class GPT(nn.Module):
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads:
            ...
            with torch.no_grad():
                for head in self.mtp_heads:
                    head.weight.copy_(base_head_weight)
