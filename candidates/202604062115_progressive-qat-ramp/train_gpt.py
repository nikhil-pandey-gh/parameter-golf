...
class Hyperparameters:
    ...
    qat_enabled = bool(int(os.environ.get("QAT_ENABLED", "0")))  # immediate full-strength override
...
@torch.no_grad()
def fake_quantize_int6_weight(t: Tensor, clip_range: int = 31) -> Tensor:
    if t.ndim != 2:
        return t
    t32 = t.float()
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training and w.ndim == 2:
            w_q = fake_quantize_int6_weight(self.weight).to(dtype=x.dtype)
            # Keep the fake-quant path in the compiled graph and control it with a buffer,
            # avoiding dead-code elimination from a late on/off QAT switch.
            w = w + (w_q - w).detach() * self.qat_mix.to(dtype=x.dtype)
...
def set_qat_mix(module: nn.Module, mix: float) -> None:
    clamped = min(max(float(mix), 0.0), 1.0)
    with torch.no_grad():
        for submodule in module.modules():
            if isinstance(submodule, CastedLinear):
                submodule.qat_mix.fill_(clamped)


def compute_qat_mix(args: Hyperparameters, scale: float) -> float:
    if args.qat_enabled:
        return 1.0
    if args.qat_ramp_start <= 0.0:
        return 0.0
    if scale >= args.qat_ramp_start:
        return 0.0
    if args.qat_ramp_end >= args.qat_ramp_start or scale <= args.qat_ramp_end:
        return 1.0
    return (args.qat_ramp_start - scale) / max(args.qat_ramp_start - args.qat_ramp_end, 1e-8)
...
def main() -> None:
    ...
    set_qat_mix(base_model, 1.0 if args.qat_enabled else 0.0)
    ...
    log0(
        ...
        f"qat_ramp_end:{args.qat_ramp_end:.3f}"
    )
    log0(
    ...
    while True:
        ...
        qat_mix = compute_qat_mix(args, scale)
        set_qat_mix(base_model, qat_mix)
        qat_bucket = min(int(qat_mix * 4.0 + 1e-8), 4)
        if qat_bucket != last_qat_bucket:
            log0(f"qat_mix:{qat_mix:.2f} step:{step} scale:{scale:.4f}")
            last_qat_bucket = qat_bucket
        zero_grad_all()
