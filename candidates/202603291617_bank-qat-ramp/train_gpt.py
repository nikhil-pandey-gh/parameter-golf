...
def flash_attn_3_func(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    y = F.scaled_dot_product_attention(q_t, k_t, v_t, dropout_p=0.0, is_causal=causal)
...
class Hyperparameters:
    ...
    bank_qat_clip_range = int(os.environ.get("BANK_QAT_CLIP_RANGE", 31))
...
def fake_quantize_int6_ste(w: Tensor, clip_range: int = 31, mix: Tensor | float = 1.0) -> Tensor:
    if w.ndim < 2:
        return w
    w32 = w.float()
    ...
    mix_t = torch.as_tensor(mix, device=w.device, dtype=w.dtype)
    return w + (w_q.to(dtype=w.dtype) - w).detach() * mix_t


class CastedLinear(nn.Linear):
    _qat_enabled: bool = False
    _qat_mix: Tensor | float = 1.0
    _qat_clip_range: int = 31
    def forward(self, x: Tensor) -> Tensor:
        w = self.weight
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            w = fake_quantize_int6_ste(
                w,
                clip_range=CastedLinear._qat_clip_range,
                mix=CastedLinear._qat_mix,
            )
        w = w.to(x.dtype)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        v0: Tensor | None = None,
        qat_mix: Tensor | None = None,
        qat_clip_range: int = 31,
    ) -> tuple[Tensor, Tensor | None]:
        bsz, seqlen, dim = x.shape
        if self.training and qat_mix is not None:
            q_w = fake_quantize_int6_ste(q_w, clip_range=qat_clip_range, mix=qat_mix)
            k_w = fake_quantize_int6_ste(k_w, clip_range=qat_clip_range, mix=qat_mix)
            v_w = fake_quantize_int6_ste(v_w, clip_range=qat_clip_range, mix=qat_mix)
            out_w = fake_quantize_int6_ste(out_w, clip_range=qat_clip_range, mix=qat_mix)
...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        # No CastedLinear -- weights come from banks
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor, qat_mix: Tensor | None = None, qat_clip_range: int = 31) -> Tensor:
        if self.training and qat_mix is not None:
            up_w = fake_quantize_int6_ste(up_w, clip_range=qat_clip_range, mix=qat_mix)
            down_w = fake_quantize_int6_ste(down_w, clip_range=qat_clip_range, mix=qat_mix)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bank_qat_clip_range = bank_qat_clip_range
    ...
    def enable_bank_qat(self) -> None:
        self.bank_qat_active = True

    def set_bank_qat_mix(self, mix: float) -> None:
        self.bank_qat_mix.fill_(min(max(float(mix), 0.0), 1.0))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        qat_mix = self.bank_qat_mix if self.training and self.bank_qat_active else None
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                v_embed=ve, v0=v0, qat_mix=qat_mix, qat_clip_range=self.bank_qat_clip_range)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_qat_clip_range=args.bank_qat_clip_range,
    ).to(device).bfloat16()
    CastedLinear._qat_mix = base_model.bank_qat_mix
    CastedLinear._qat_clip_range = args.bank_qat_clip_range
    if args.late_qat_threshold <= 0 and (args.bank_qat_enabled or args.qat_enabled):
        if args.bank_qat_enabled:
            base_model.enable_bank_qat()
        base_model.set_bank_qat_mix(1.0)
    ...
    log0(
        f"late_qat threshold:{args.late_qat_threshold:.3f} "
        f"bank_qat:{int(args.bank_qat_enabled)} clip_range:{args.bank_qat_clip_range} "
        f"casted_qat:{int(args.qat_enabled)}"
    )
    ...
    while True:
        ...
        if (
            ...
            and not (base_model.bank_qat_active or CastedLinear._qat_enabled)
            and (args.bank_qat_enabled or args.qat_enabled)
        ):
            if args.bank_qat_enabled:
                base_model.enable_bank_qat()
            ...
            training_time_ms += compile_ms
            t0 = time.perf_counter()
            log0(
                f"late_qat:enabled step:{step} scale:{scale:.4f} "
                f"bank_qat:{int(base_model.bank_qat_active)} "
                f"casted_qat:{int(CastedLinear._qat_enabled)} "
                f"bank_qat_mix:{base_model.bank_qat_mix.item():.3f} compile_ms:{compile_ms:.0f}"
            )
        if (base_model.bank_qat_active or CastedLinear._qat_enabled) and args.late_qat_threshold > 0:
            base_model.set_bank_qat_mix(1.0 - min(scale / args.late_qat_threshold, 1.0))
        zero_grad_all()
        ...
        if should_log_train:
            log0(
                ...
                f"bank_qat_mix:{base_model.bank_qat_mix.item():.3f}"
            )
