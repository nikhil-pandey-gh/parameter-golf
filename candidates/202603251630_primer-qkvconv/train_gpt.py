...
class Hyperparameters:
    ...
    primer_conv_last_n = int(os.environ.get("PRIMER_CONV_LAST_N", 4))
...
PRIMER_DEPTHWISE_NAME_PATTERNS = ("q_conv.weight", "k_conv.weight", "v_conv.weight")
...
def flash_or_sdp_attention(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    out = F.scaled_dot_product_attention(q_heads, k_heads, v_heads, **kwargs)
...
class DepthwiseCausalConv1d(nn.Module):
    def __init__(self, channels: int, kernel_size: int):
        ...
        with torch.no_grad():
            self.weight[:, -1].fill_(1.0)

    def forward(self, x: Tensor) -> Tensor:
        x_t = x.transpose(1, 2)
        weight = self.weight.to(dtype=x.dtype).unsqueeze(1)
        y = F.conv1d(F.pad(x_t, (self.kernel_size - 1, 0)), weight, groups=x_t.size(1))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if primer_kernel_size > 1:
            self.q_conv = DepthwiseCausalConv1d(dim, primer_kernel_size)
            self.k_conv = DepthwiseCausalConv1d(kv_dim, primer_kernel_size)
            self.v_conv = DepthwiseCausalConv1d(kv_dim, primer_kernel_size)
        else:
            self.q_conv = None
            self.k_conv = None
            self.v_conv = None
...
class GPT(nn.Module):
    def __init__(
        ...
        primer_start = max(0, num_layers - primer_conv_last_n)
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    ln_scale=ln_scale,
                    dtg=dtg,
                    primer_kernel_size=primer_kernel_size if i >= primer_start else 0,
                )
...
def main() -> None:
    ...
    use_strict_flash_only = flash_attn_3_func is not None
    enable_cudnn_sdp(False)
    enable_flash_sdp(True)
    enable_mem_efficient_sdp(not use_strict_flash_only)
    enable_math_sdp(not use_strict_flash_only)
    logfile = None
    if master_process:
    ...
    base_model = GPT(
        ...
        primer_conv_last_n=args.primer_conv_last_n,
        primer_kernel_size=args.primer_kernel_size,
    ).to(device).bfloat16()
    for module in base_model.modules():
        if isinstance(module, (CastedLinear, DepthwiseCausalConv1d)):
            module.float()
    ...
    matrix_params = [
        ...
        if (
            ...
            and not any(pattern in name for pattern in PRIMER_DEPTHWISE_NAME_PATTERNS)
        )
    ]
    ...
    scalar_params = [
        ...
        if (
            ...
            or any(pattern in name for pattern in PRIMER_DEPTHWISE_NAME_PATTERNS)
        )
    ]
    ...
    primer_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.q_conv is not None]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(
        f"primer_qkv_conv:last_{args.primer_conv_last_n} kernel:{args.primer_kernel_size} "
        f"active_layers:{primer_layers}"
    )
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        "sdp_backends:"
        f"cudnn=False flash=True mem_efficient={not use_strict_flash_only} "
        f"math={not use_strict_flash_only}"
    )
    log0(f"attention_kernel:{'flash_attn_3' if flash_attn_3_func is not None else 'torch_sdp_fallback'}")
    ...
    for m in eval_model.modules():
        if isinstance(m, (CastedLinear, DepthwiseCausalConv1d)):
            m.float()
