...
class GPT(nn.Module):
    def __init__(
        ...
        if mtp_loss_decay < 0.0:
            raise ValueError(f"mtp_loss_decay must be non-negative, got {mtp_loss_decay}")
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / mtp_weight_sum.clamp_min(1e-12))
        return main_loss
...
def run_smoke_test(args: Hyperparameters) -> None:
    ...
    model.train()
    seq_len = max(2, min(args.train_seq_len, 16))
    ...
    loss.backward()
    print(
        f"smoke_test_ok loss:{loss.item():.4f} seq_len:{seq_len} "
        f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_decay:{args.mtp_loss_decay}"
    )
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_loss_decay:{args.mtp_loss_decay} mtp_params:{mtp_params}"
    )
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_loss_decay=args.mtp_loss_decay,
