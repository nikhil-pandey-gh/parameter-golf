...
class Hyperparameters:
    ...
    share_attention_banks = bool(int(os.environ.get("SHARE_ATTENTION_BANKS", "1")))
    share_mlp_banks = bool(int(os.environ.get("SHARE_MLP_BANKS", "1")))
    bank_share_mode = os.environ.get("BANK_SHARE_MODE", "adjacent").strip().lower()
    bank_share_group_size = int(os.environ.get("BANK_SHARE_GROUP_SIZE", 2))
...
def build_bank_share_index(num_layers: int, mode: str, group_size: int) -> tuple[list[int], int]:
    if num_layers <= 0:
        raise ValueError(f"num_layers must be positive, got {num_layers}")
    mode = mode.strip().lower()
    if mode == "none":
        index = list(range(num_layers))
    elif mode == "adjacent":
        if group_size <= 0:
            raise ValueError(f"BANK_SHARE_GROUP_SIZE must be positive, got {group_size}")
        index = [i // group_size for i in range(num_layers)]
    elif mode == "mirror":
        index = [min(i, num_layers - 1 - i) for i in range(num_layers)]
    else:
        raise ValueError(f"Unsupported BANK_SHARE_MODE={mode!r}; expected one of none, adjacent, mirror")
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        share_attention_banks: bool = False,
        share_mlp_banks: bool = False,
        bank_share_mode: str = "none",
        bank_share_group_size: int = 2,
    ):
        super().__init__()
        ...
        self.bank_share_mode = bank_share_mode
        self.bank_share_group_size = bank_share_group_size
        self.share_attention_banks = share_attention_banks
        self.share_mlp_banks = share_mlp_banks
        ...
        attn_mode = bank_share_mode if share_attention_banks else "none"
        mlp_mode = bank_share_mode if share_mlp_banks else "none"
        self.attn_bank_index, self.num_attn_bank_layers = build_bank_share_index(num_layers, attn_mode, bank_share_group_size)
        self.mlp_bank_index, self.num_mlp_bank_layers = build_bank_share_index(num_layers, mlp_mode, bank_share_group_size)
        self.qo_bank = nn.Parameter(torch.empty(2 * self.num_attn_bank_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * self.num_attn_bank_layers, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(self.num_mlp_bank_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(self.num_mlp_bank_layers, model_dim, mlp_dim))
    ...
    def _attn_weights(self, layer_idx: int) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        bank_idx = self.attn_bank_index[layer_idx]
        return (
            self.qo_bank[bank_idx],
            self.kv_bank[bank_idx],
            self.kv_bank[self.num_attn_bank_layers + bank_idx],
            self.qo_bank[self.num_attn_bank_layers + bank_idx],
        )

    def _mlp_weights(self, layer_idx: int) -> tuple[Tensor, Tensor]:
        bank_idx = self.mlp_bank_index[layer_idx]
        return self.mlp_up_bank[bank_idx], self.mlp_down_bank[bank_idx]
    ...
    def _init_weights(self) -> None:
        ...
        for i in range(self.num_attn_bank_layers):
            nn.init.orthogonal_(self.qo_bank.data[i], gain=1.0)        # Q
            nn.init.zeros_(self.qo_bank.data[self.num_attn_bank_layers + i])  # Out (zero init)
            nn.init.orthogonal_(self.kv_bank.data[i], gain=1.0)        # K
            nn.init.orthogonal_(self.kv_bank.data[self.num_attn_bank_layers + i], gain=1.0)  # V
            self.qo_bank.data[self.num_attn_bank_layers + i].mul_(proj_scale)
        for i in range(self.num_mlp_bank_layers):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            up_w, down_w = self._mlp_weights(i)
        ...
        for i in range(self.num_decoder_layers):
            ...
            up_w, down_w = self._mlp_weights(bi)
...
def eval_val_sliding_ttt(
    ...
    if frozen_block_ids:
        raise ValueError(
            "TTT_FREEZE_BLOCKS>0 is unsupported in the banked model path because bank weights are shared "
            "outside blocks.* names; use TTT_FREEZE_BLOCKS=0 when enabling TTT."
        )
    ttt_params = []
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            scale_shape = list(s.shape) + [1] * (q.ndim - s.ndim)
            out[name] = (q.float() * s.float().view(*scale_shape)).to(orig_dtype)
        else:
            out[name] = (q.float() * float(s.item())).to(orig_dtype)
    return out


def gpt_kwargs_from_args(args: Hyperparameters) -> dict[str, object]:
    return {
        ...
        "share_attention_banks": args.share_attention_banks,
        "share_mlp_banks": args.share_mlp_banks,
        "bank_share_mode": args.bank_share_mode,
        "bank_share_group_size": args.bank_share_group_size,
    }
...
def main() -> None:
    ...
    gpt_kwargs = gpt_kwargs_from_args(args)
    base_model = GPT(
        **gpt_kwargs,
    ...
    log0(
        "bank_sharing:"
        f" mode={args.bank_share_mode}"
        f" group_size={args.bank_share_group_size}"
        f" share_attention={args.share_attention_banks}"
        f" share_mlp={args.share_mlp_banks}"
        f" attn_groups={base_model.num_attn_bank_layers}"
        f" mlp_groups={base_model.num_mlp_bank_layers}"
        f" attn_index={base_model.attn_bank_index}"
        f" mlp_index={base_model.mlp_bank_index}"
    )
    log0(
    ...
    torch.save({"cfg": gpt_kwargs, "w": quant_result, "m": quant_meta}, quant_buf)
    ...
    artifact_gpt_kwargs = quant_state.get("cfg", gpt_kwargs)
    eval_model = GPT(**artifact_gpt_kwargs, mtp_num_heads=0, mtp_loss_weight=0.0).to(device).bfloat16()
