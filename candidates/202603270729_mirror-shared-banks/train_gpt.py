...
class Hyperparameters:
    ...
    shared_mirror_mlp = bool(int(os.environ.get("SHARED_MIRROR_MLP", "1")))
...
def build_mirror_bank_index(num_layers: int, enabled: bool) -> list[int]:
    if not enabled:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mlp_bank_index_lookup = build_mirror_bank_index(num_layers, shared_mirror_mlp)
        self.num_mlp_bank_layers = max(self.mlp_bank_index_lookup) + 1
        self.register_buffer(
            "mlp_bank_index_map",
            torch.tensor(self.mlp_bank_index_lookup, dtype=torch.int16),
            persistent=True,
        )
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            mlp_bank_i = self.mlp_bank_index_lookup[i]
            x, raw_v = self.blocks[i](x, x0,
                self.qo_bank[i], self.kv_bank[i], self.kv_bank[n + i],
                self.qo_bank[n + i], self.mlp_up_bank[mlp_bank_i], self.mlp_down_bank[mlp_bank_i],
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            mlp_bank_i = self.mlp_bank_index_lookup[bi]
            x, _ = self.blocks[bi](x, x0,
                self.qo_bank[bi], self.kv_bank[bi], self.kv_bank[n + bi],
                self.qo_bank[n + bi], self.mlp_up_bank[mlp_bank_i], self.mlp_down_bank[mlp_bank_i],
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim >= 2:
        ...
        return best_q.reshape_as(t32), best_s.view(t32.shape[:-1])
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if s.ndim > 0:
            scale = s.float().reshape(*q.shape[:-1], 1)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_mirror_mlp=args.shared_mirror_mlp,
    ).to(device).bfloat16()
    ...
    log0(
        f"shared_mirror_mlp:{args.shared_mirror_mlp} "
        f"unique_mlp_banks:{base_model.num_mlp_bank_layers} "
        f"mlp_bank_map:{base_model.mlp_bank_index_lookup}"
    )
    ...
    # Quantize the export state directly so shared MLP banks stay shared on disk.
