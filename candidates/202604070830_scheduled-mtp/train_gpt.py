...
class Hyperparameters:
    ...
    mtp_fade_scale = float(os.environ.get("MTP_FADE_SCALE", 0.3))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_base_loss_weight = float(mtp_loss_weight)
        # Keep the live MTP weight in a tensor buffer so torch.compile sees runtime updates.
    ...
    def set_mtp_loss_weight(self, weight: float) -> None:
        self.mtp_loss_weight_buf.fill_(float(max(weight, 0.0)))
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += 1
            if mtp_loss_count > 0:
                mtp_weight = self.mtp_loss_weight_buf.to(device=main_loss.device, dtype=main_loss.dtype)
...
def main() -> None:
    ...
    base_model.set_mtp_loss_weight(args.mtp_loss_weight if args.mtp_num_heads > 0 else 0.0)
    ...
    log0(
        ...
        f"mtp_fade_scale:{args.mtp_fade_scale} mtp_params:{mtp_params}"
    )
    ...
    head_like_lr = args.head_lr if (base_model.lm_head is not None or mtp_head_params) else 0.0
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} "
        f"head_lr:{head_like_lr} matrix_lr:{args.matrix_lr} scalar_lr:{args.scalar_lr}"
    )
    log0(
    ...
    def mtp_weight_for_scale(scale: float) -> float:
        if args.mtp_num_heads <= 0 or args.mtp_loss_weight <= 0.0:
            return 0.0
        if args.mtp_fade_scale <= 0.0:
            return args.mtp_loss_weight
        return args.mtp_loss_weight * min(scale / args.mtp_fade_scale, 1.0)
