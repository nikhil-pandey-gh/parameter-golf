...
class Hyperparameters:
    ...
    qat_clip_percentile = float(os.environ.get("QAT_CLIP_PERCENTILE", 1.0))
    ...
    quant_calibration_steps = int(os.environ.get("QUANT_CALIBRATION_STEPS", 4))
    quant_calibration_batch_tokens = int(os.environ.get("QUANT_CALIBRATION_BATCH_TOKENS", 131_072))
    quant_calibration_seq_len = int(os.environ.get("QUANT_CALIBRATION_SEQ_LEN", 512))
...
def flash_or_sdpa_attention(q: Tensor, k: Tensor, v: Tensor, enable_gqa: bool) -> Tensor:
...
def fake_quantize_int6_weight(weight: Tensor, clip_percentile: float) -> Tensor:
    w32 = weight.float()
    if w32.ndim != 2:
        return weight
    if clip_percentile < 1.0:
        row_clip = torch.quantile(w32.abs(), clip_percentile, dim=1)
    else:
        row_clip = w32.abs().amax(dim=1)
    scale = (row_clip / INT6_CLIP_RANGE).clamp_min(1.0 / INT6_CLIP_RANGE)
    q = torch.clamp(torch.round(w32 / scale[:, None]), -INT6_CLIP_RANGE, INT6_CLIP_RANGE)
    return (q * scale[:, None]).to(dtype=weight.dtype)

def maybe_fake_quantize_int6_weight(weight: Tensor, enabled: bool, clip_percentile: float) -> Tensor:
    if not enabled or weight.ndim != 2:
        return weight
    with torch.no_grad():
        q = fake_quantize_int6_weight(weight, clip_percentile)
    return weight + (q - weight).detach()

class ActivationCalibration:
    def __init__(self):
        self.sum_sq: dict[str, Tensor] = {}
        self.count: dict[str, int] = {}

    @torch.no_grad()
    def record(self, name: str, x: Tensor) -> None:
        if x.ndim == 0 or x.shape[-1] == 0:
            return
        flat = x.detach().float().reshape(-1, x.shape[-1])
        if flat.numel() == 0:
            return
        sum_sq = flat.square().sum(dim=0).cpu()
        if name in self.sum_sq:
            self.sum_sq[name] += sum_sq
            self.count[name] += int(flat.shape[0])
        else:
            self.sum_sq[name] = sum_sq
            self.count[name] = int(flat.shape[0])

    def column_weights(self, name: str, width: int, device: torch.device) -> Tensor | None:
        sum_sq = self.sum_sq.get(name)
        count = self.count.get(name, 0)
        if sum_sq is None or count <= 0:
            return None
        if int(sum_sq.numel()) != int(width):
            raise ValueError(
                f"Activation calibration width mismatch for {name}: expected {width}, got {sum_sq.numel()}"
            )
        weights = (sum_sq / count).clamp_min(1e-8).to(device=device, dtype=torch.float32)
        return (weights / weights.mean().clamp_min(1e-8)).contiguous()
class CastedLinear(nn.Linear):
    _qat_enabled: bool = False
    _qat_clip_percentile: float = 1.0
    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            with torch.no_grad():
                w_q = fake_quantize_int6_weight(self.weight, CastedLinear._qat_clip_percentile).to(x.dtype)
            w = w + (w_q - w).detach()
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.layer_idx = -1
        self.calibration_state: ActivationCalibration | None = None
    ...
    def forward(
        ...
        qat_clip_percentile: float = 1.0,
    ) -> tuple[Tensor, Tensor | None]:
        bsz, seqlen, dim = x.shape
        if self.calibration_state is not None:
            self.calibration_state.record(f"blocks.{self.layer_idx}.attn.c_q.weight", x)
            self.calibration_state.record(f"blocks.{self.layer_idx}.attn.c_k.weight", x)
            self.calibration_state.record(f"blocks.{self.layer_idx}.attn.c_v.weight", x)
        q_w = maybe_fake_quantize_int6_weight(q_w, qat_enabled and self.training, qat_clip_percentile)
        k_w = maybe_fake_quantize_int6_weight(k_w, qat_enabled and self.training, qat_clip_percentile)
        v_w = maybe_fake_quantize_int6_weight(v_w, qat_enabled and self.training, qat_clip_percentile)
        ...
        y = flash_or_sdpa_attention(q, k, v, enable_gqa=(self.num_kv_heads != self.num_heads))
        if self.use_xsa:
        ...
        if self.calibration_state is not None:
            self.calibration_state.record(f"blocks.{self.layer_idx}.attn.proj.weight", y)
        out_w = maybe_fake_quantize_int6_weight(out_w, qat_enabled and self.training, qat_clip_percentile)
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        if self.calibration_state is not None:
            self.calibration_state.record(f"blocks.{self.layer_idx}.mlp.fc.weight", x)
        up_w = maybe_fake_quantize_int6_weight(up_w, qat_enabled and self.training, qat_clip_percentile)
        hidden = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=0.5)
        hidden_sq = hidden.square()
        if self.calibration_state is not None:
            self.calibration_state.record(f"blocks.{self.layer_idx}.mlp.proj.weight", hidden_sq)
        down_w = maybe_fake_quantize_int6_weight(down_w, qat_enabled and self.training, qat_clip_percentile)
        return F.linear(hidden_sq, down_w.to(x.dtype))
...
class Block(nn.Module):
    ...
    def forward(
        ...
        attn_out, raw_v = self.attn(
            ...
            qat_clip_percentile=qat_clip_percentile,
        )
...
class GPT(nn.Module):
    ...
    def set_calibration_state(self, calibration_state: ActivationCalibration | None) -> None:
        self.calibration_state = calibration_state
        for block in self.blocks:
            block.attn.calibration_state = calibration_state
            block.mlp.calibration_state = calibration_state
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                ...
                v_embed=ve, v0=v0,
                qat_enabled=self.bank_qat_enabled, qat_clip_percentile=self.qat_clip_percentile)
...
def quantize_int6_per_row(
    ...
    col_weights: Tensor | None = None,
) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        weights = None
        if col_weights is not None:
            if int(col_weights.numel()) != int(t32.shape[1]):
                raise ValueError(
                    f"Activation weights mismatch: expected {t32.shape[1]}, got {col_weights.numel()}"
                )
            weights = col_weights.to(device=t32.device, dtype=torch.float32).view(1, -1)
        ...
        for pct in INT6_CLIP_PERCENTILES:
            ...
            err_term = (t32 - recon).pow(2)
            err = (err_term * weights).mean().item() if weights is not None else err_term.mean().item()
            if err < best_err:
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            col_weights = (
                activation_calibration.column_weights(name, t.shape[1], t.device)
                if activation_calibration is not None and t.ndim == 2
                else None
            )
            q, s = quantize_int6_per_row(t, col_weights=col_weights)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def collect_export_calibration(
    args: Hyperparameters,
    model: GPT,
    rank: int,
    world_size: int,
    device: torch.device,
    log0,
) -> ActivationCalibration | None:
    if args.quant_calibration_steps <= 0 or rank != 0:
        return None
    seq_len = min(max(args.quant_calibration_seq_len, 1), args.train_seq_len)
    batch_tokens = max(args.quant_calibration_batch_tokens, seq_len)
    ...
    model.eval()
    model.set_calibration_state(calibration)
    with torch.inference_mode():
        for _ in range(args.quant_calibration_steps):
            local = stream.take(batch_tokens + 1).to(device=device, dtype=torch.int64)
            x = local[:-1].reshape(-1, seq_len)
            y = local[1:].reshape(-1, seq_len)
            model(x, y)
    model.set_calibration_state(None)
    if was_training:
        model.train()
    log0(
        f"quant_calibration:steps:{args.quant_calibration_steps} "
        f"seq_len:{seq_len} tokens_per_step:{batch_tokens} tensors:{len(calibration.sum_sq)}"
    )
    return calibration
...
def main() -> None:
    ...
    CastedLinear._qat_clip_percentile = args.qat_clip_percentile
    base_model = GPT(
    ...
    base_model.bank_qat_enabled = args.qat_enabled
    base_model.qat_clip_percentile = args.qat_clip_percentile
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not base_model.bank_qat_enabled:
            ...
            log0(
                ...
                f"clip_pct:{args.qat_clip_percentile:.5f} bank_qat:1"
            )
        zero_grad_all()
    ...
    activation_calibration = collect_export_calibration(args, base_model, rank, world_size, device, log0)
