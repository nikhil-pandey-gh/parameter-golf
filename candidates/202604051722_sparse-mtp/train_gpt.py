...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_token_stride = max(1, mtp_token_stride)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            mtp_stride = self.mtp_token_stride
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if valid_t <= 0:
                    continue
                mtp_hidden = x[:, :valid_t:mtp_stride, :].reshape(-1, dim)
                mtp_targets = target_ids[:, k + 1 : k + 1 + valid_t : mtp_stride].reshape(-1)
...
def main() -> None:
    ...
    log0(
        ...
        f"mtp_token_stride:{max(args.mtp_token_stride, 1)} mtp_params:{mtp_params}"
    )
