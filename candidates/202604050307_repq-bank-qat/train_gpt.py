...
def find_repo_root() -> Path:
    ...
    for parent in here.parents:
        if (parent / "data").exists() and (parent / "records").exists():
            return parent
...
class Hyperparameters:
    ...
    bank_qat = bool(int(os.environ.get("BANK_QAT", "1")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,attn_gate,vr_lambda,bank_qat",
    ).split(",")
    if pattern
)
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and self.training and w.ndim == 2:
            w = fake_quantize_weight_matrix(self.weight).to(x.dtype)
...
def fake_quantize_weight_matrix(weight: Tensor, clip_range: int = 31) -> Tensor:
    w32 = weight.float()
    with torch.no_grad():
        row_max = w32.abs().amax(dim=1).clamp_min(1.0 / clip_range)
        scale = (row_max / clip_range).clamp_min(1.0 / clip_range)
        q = torch.clamp(torch.round(w32 / scale[:, None]), -clip_range, clip_range)
        w_q = (q * scale[:, None]).to(weight.dtype)
    return weight + (w_q - weight).detach()
def fake_quantize_bank(bank: Tensor, log_scale: Tensor, clip_range: int = 31) -> Tensor:
    bank32 = bank.float()
    with torch.no_grad():
        row_max = bank32.abs().amax(dim=-1).clamp_min(1.0 / clip_range)
    clip_mult = torch.exp(log_scale.float().clamp(-0.7, 0.7))
    scale = (row_max * clip_mult / clip_range).clamp_min(1.0 / clip_range)
    scaled = bank32 / scale[..., None]
    q = torch.clamp(torch.round(scaled), -clip_range, clip_range)
    q = (q - scaled).detach() + scaled
    return (q * scale[..., None]).to(bank.dtype)
...
class GPT(nn.Module):
    ...
    def bank_qat_parameters(self) -> list[nn.Parameter]:
        return [
            self.qo_bank_qat_log_scale,
            self.kv_bank_qat_log_scale,
            self.mlp_up_bank_qat_log_scale,
            self.mlp_down_bank_qat_log_scale,
        ]
    def set_bank_qat_enabled(self, enabled: bool) -> None:
        if enabled and not self.bank_qat_enabled:
            with torch.no_grad():
                for param in self.bank_qat_parameters():
                    param.zero_()
        self.bank_qat_enabled = enabled
    def _get_banked_weights(self) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        if not (self.bank_qat_enabled and self.training):
            return self.qo_bank, self.kv_bank, self.mlp_up_bank, self.mlp_down_bank
        return (
            fake_quantize_bank(self.qo_bank, self.qo_bank_qat_log_scale),
            fake_quantize_bank(self.kv_bank, self.kv_bank_qat_log_scale),
            fake_quantize_bank(self.mlp_up_bank, self.mlp_up_bank_qat_log_scale),
            fake_quantize_bank(self.mlp_down_bank, self.mlp_down_bank_qat_log_scale),
        )
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        n = self.num_layers
        qo_bank, kv_bank, mlp_up_bank, mlp_down_bank = self._get_banked_weights()
...
def _classify_param(name: str) -> str:
    ...
    if ".attn." in name or (".proj." in name and ".mlp." not in name):
        return "attn"
    return "other"
def quantize_int6_per_row(t: Tensor, clip_range: int = 31, log_scale: Tensor | None = None) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    clip_mult = None
    ...
    if t32.ndim == 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            if clip_mult is not None:
                row_clip = row_clip * clip_mult.to(row_clip.device)
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    export_log_scales: dict[str, Tensor] | None = None,
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(t, log_scale=export_log_scales.get(name) if export_log_scales else None)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = {"type": "int6"}
        else:
...
def main() -> None:
    ...
    base_model.set_bank_qat_enabled(args.bank_qat and args.qat_enabled)
    ...
    scalar_params.extend(base_model.bank_qat_parameters())
    ...
    while True:
        ...
        late_qat_pending = (
            ...
            and (not CastedLinear._qat_enabled or (args.bank_qat and not base_model.bank_qat_enabled))
        )
        if late_qat_pending:
            ...
            log0(
                ...
                f"bank_qat:{int(args.bank_qat)} recompiling:1"
            )
            compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
            model = compiled_model
        zero_grad_all()
    ...
    export_sd = {
        ...
        if "mtp_heads" not in k and "_qat_log_scale" not in k
    }
    excluded_mtp = sum(int(t.numel()) for k, t in full_state_dict.items() if "mtp_heads" in k)
    if excluded_mtp > 0:
        log0(f"export_excluding_mtp_params:{excluded_mtp}")
    excluded_bank_qat = sum(int(t.numel()) for k, t in full_state_dict.items() if "_qat_log_scale" in k)
    if excluded_bank_qat > 0:
        log0(f"export_excluding_bank_qat_params:{excluded_bank_qat}")
    if master_process:
    ...
    export_log_scales = _unbank_state_dict(
        {
            "qo_bank": full_state_dict["qo_bank_qat_log_scale"].detach().cpu(),
            "kv_bank": full_state_dict["kv_bank_qat_log_scale"].detach().cpu(),
            "mlp_up_bank": full_state_dict["mlp_up_bank_qat_log_scale"].detach().cpu(),
            "mlp_down_bank": full_state_dict["mlp_down_bank_qat_log_scale"].detach().cpu(),
        },
        args.num_layers,
    ) if args.bank_qat else {}
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, export_log_scales)
    ...
    incompatible = eval_model.load_state_dict(deq_state, strict=False)
    allowed_missing = {name for name in incompatible.missing_keys if name.endswith("_qat_log_scale")}
    unexpected = [name for name in incompatible.unexpected_keys if not name.endswith("_qat_log_scale")]
    missing = [name for name in incompatible.missing_keys if name not in allowed_missing]
    if unexpected or missing:
        raise RuntimeError(
            f"Unexpected export/load mismatch. missing={missing} unexpected={unexpected}"
        )
