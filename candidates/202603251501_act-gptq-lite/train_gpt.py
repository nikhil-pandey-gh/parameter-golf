...
class Hyperparameters:
    ...
    act_gptq_enabled = bool(int(os.environ.get("ACT_GPTQ_ENABLED", "1")))
    act_gptq_calibration_steps = int(os.environ.get("ACT_GPTQ_CALIBRATION_STEPS", 8))
    act_gptq_clip_percentiles = tuple(
        float(x.strip())
        for x in os.environ.get("ACT_GPTQ_CLIP_PERCENTILES", "0.9990,0.9995,0.9999,0.99999,1.0").split(",")
        if x.strip()
    )
...
def collect_activation_sq_means(
    ...
    def make_hook(weight_name: str):
        def _hook(module: nn.Module, inputs: tuple[Tensor, ...]) -> None:
            ...
            if not isinstance(x, Tensor) or x.ndim < 2:
                return
            expected = getattr(module, "weight", None)
            if expected is None or x.size(-1) != expected.shape[1]:
                return
            x_sq = x.detach().float().pow(2).sum(dim=tuple(range(x.ndim - 1)))
            sample_count = torch.tensor(float(x.numel() // x.size(-1)), device=x.device, dtype=torch.float64)
            if weight_name not in sum_sq:
                sum_sq[weight_name] = x_sq
                counts[weight_name] = sample_count
            else:
                sum_sq[weight_name] += x_sq
                counts[weight_name] += sample_count
        return _hook
    ...
    if dist.is_available() and dist.is_initialized():
        for name in sum_sq:
            dist.all_reduce(sum_sq[name], op=dist.ReduceOp.SUM)
            dist.all_reduce(counts[name], op=dist.ReduceOp.SUM)
    return {
        name: (sum_sq[name] / counts[name].clamp_min(1.0).to(dtype=sum_sq[name].dtype)).detach().cpu().contiguous()
        for name in sum_sq
    }
...
def quantize_int6_per_row(
    t: Tensor,
    activation_sq_mean: Tensor | None = None,
    ...
    if t32.ndim == 2:
        ...
        col_weight = None
        if activation_sq_mean is not None and activation_sq_mean.ndim == 1 and activation_sq_mean.numel() == t32.shape[1]:
            col_weight = activation_sq_mean.to(dtype=torch.float32, device=t32.device).clamp_min(1e-8)
        ...
        for pct in clip_percentiles:
            ...
            row_err = (t32 - recon).pow(2)
            if col_weight is not None:
                row_err = row_err * col_weight[None, :]
            row_err = row_err.mean(dim=1)
            improved = row_err < best_err
            best_q[improved] = q[improved]
            best_s[improved] = s[improved]
            best_err = torch.where(improved, row_err, best_err)
...
def mixed_quantize_int6(
    ...
    int6_cats: set[str],
    activation_sq_means: dict[str, Tensor] | None = None,
    clip_percentiles: tuple[float, ...] = (0.9990, 0.9995, 0.9999, 0.99999, 1.0),
):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s = quantize_int6_per_row(
                t,
                activation_sq_mean=None if activation_sq_means is None else activation_sq_means.get(name),
...
def main() -> None:
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            if args.act_gptq_enabled and len(act_gptq_batches) < args.act_gptq_calibration_steps:
                act_gptq_batches.append(x.detach().cpu())
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
    ...
    if args.act_gptq_enabled:
        activation_sq_means = collect_activation_sq_means(act_gptq_batches, base_model, device)
        clip_desc = ",".join(f"{pct:.5f}" for pct in args.act_gptq_clip_percentiles)
        log0(
            f"act_gptq:enabled calib_batches:{len(act_gptq_batches)} "
            f"calib_batch_tokens:{args.train_batch_tokens // (world_size * grad_accum_steps)} "
            f"clip_percentiles:{clip_desc} layers:{len(activation_sq_means)}"
        )
    else:
        log0("act_gptq:disabled")
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu,
        {"mlp", "attn"},
        activation_sq_means=activation_sq_means if args.act_gptq_enabled else None,
        clip_percentiles=args.act_gptq_clip_percentiles,
    )
