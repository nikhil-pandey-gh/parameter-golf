...
def main() -> None:
    ...
    sdp_fallback_enabled = flash_attn_3_func is None
    enable_mem_efficient_sdp(sdp_fallback_enabled)
    enable_math_sdp(sdp_fallback_enabled)
    logfile = None
    if master_process:
    ...
    log0(f"mlp_activation:leaky_relu_squared negative_slope:{args.mlp_negative_slope}")
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"flash_attn_available:{flash_attn_3_func is not None}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        f"sdp_backends:cudnn=False flash=True mem_efficient={sdp_fallback_enabled} "
        f"math={sdp_fallback_enabled}"
    )
