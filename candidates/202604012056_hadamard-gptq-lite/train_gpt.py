...
HADAMARD_INT6_ENABLED = bool(int(os.environ.get("HADAMARD_INT6", "1")))
HADAMARD_INT6_CATEGORIES = frozenset(
    part.strip() for part in os.environ.get("HADAMARD_INT6_CATEGORIES", "mlp,attn").split(",") if part.strip()
)
HADAMARD_BLOCK_SIZE = int(os.environ.get("HADAMARD_BLOCK_SIZE", "512"))
HADAMARD_MIN_BLOCK_SIZE = 64
...
def _choose_hadamard_block(last_dim: int) -> int:
    block = min(HADAMARD_BLOCK_SIZE, last_dim)
    while block >= HADAMARD_MIN_BLOCK_SIZE:
        if block & (block - 1) == 0 and last_dim % block == 0:
            return block
        block //= 2
    return 0
...
def _fwht_last_dim(x: Tensor) -> Tensor:
    n = x.shape[-1]
    if n & (n - 1):
        raise ValueError(f"Hadamard transform requires a power-of-two last dim, got {n}")
    ...
    while h < n:
        ...
        a = y[:, :, 0, :].clone()
        b = y[:, :, 1, :].clone()
        y = torch.cat((a + b, a - b), dim=-1).contiguous()
        h *= 2
    return y.view(orig_shape) / math.sqrt(n)


def _maybe_rotate_for_quant(cat: str, t: Tensor) -> tuple[Tensor, dict[str, object] | None]:
    if not HADAMARD_INT6_ENABLED or cat not in HADAMARD_INT6_CATEGORIES or t.ndim != 2:
        return t.float(), None
    block_size = _choose_hadamard_block(t.shape[-1])
    if block_size == 0:
        return t.float(), None
    rotated = _fwht_last_dim(t.view(t.shape[0], t.shape[1] // block_size, block_size)).view_as(t)
    return rotated, {"rotation": "hadamard", "block_size": block_size}


def _maybe_inverse_rotate_after_quant(t: Tensor, info: object) -> Tensor:
    if not isinstance(info, dict) or info.get("rotation") != "hadamard":
        return t
    block_size = int(info["block_size"])
    return _fwht_last_dim(t.view(t.shape[0], t.shape[1] // block_size, block_size)).view_as(t)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str]):
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            ...
            if t.ndim == 2:
                quant_src, rotation_info = _maybe_rotate_for_quant(cat, t)
                if rotation_info is not None:
                    info.update(rotation_info)
            q, s = quantize_int6_per_row(quant_src)
            result[name + ".q"] = q
            result[name + ".scale"] = s
            meta[name] = info
        else:
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        out[name] = _maybe_inverse_rotate_after_quant(deq, info).to(orig_dtype)
    return out
def main() -> None:
    ...
    log0(
        "int6_rotation:"
        f"{'hadamard' if HADAMARD_INT6_ENABLED else 'disabled'} "
        f"cats:{','.join(sorted(HADAMARD_INT6_CATEGORIES)) or 'none'} "
        f"block:{HADAMARD_BLOCK_SIZE}"
    )
