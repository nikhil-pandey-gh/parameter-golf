...
def flash_attention(q: Tensor, k: Tensor, v: Tensor, causal: bool = True) -> Tensor:
    ...
    return F.scaled_dot_product_attention(q_t, k_t, v_t, dropout_p=0.0, is_causal=causal).transpose(1, 2)
...
class Hyperparameters:
    ...
    mtp_head_decay = float(os.environ.get("MTP_HEAD_DECAY", 0.7))
    mtp_init_from_main_head = bool(int(os.environ.get("MTP_INIT_FROM_MAIN_HEAD", "1")))
...
class GPT(nn.Module):
    def __init__(
        ...
        mtp_head_decay: float = 0.7,
        mtp_init_from_main_head: bool = True,
        ...
        if mtp_head_decay <= 0.0:
            raise ValueError(f"mtp_head_decay must be positive, got {mtp_head_decay}")
        if mtp_init_from_main_head and not tie_embeddings:
            raise ValueError("MTP_INIT_FROM_MAIN_HEAD requires TIE_EMBEDDINGS=1")
        self.mtp_head_decay = mtp_head_decay
        self.mtp_init_from_main_head = mtp_init_from_main_head
    ...
    def _init_weights(self) -> None:
        ...
        if self.mtp_heads:
            base_head_weight = self.tok_emb.weight if self.tie_embeddings else self.lm_head.weight
            for head in self.mtp_heads:
                if self.mtp_init_from_main_head:
                    head.weight.data.copy_(base_head_weight.data)
                else:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                mtp_loss_count += head_weight
            main_loss = main_loss + self.mtp_loss_weight * (mtp_loss_sum / max(mtp_loss_count, 1e-8))
        return main_loss
...
def run_smoke_test() -> None:
    ...
    print(f"smoke_test:ok loss:{loss.item():.4f} logits_shape:{tuple(logits.shape)} exported_tensors:{len(export_sd)}")
...
def main() -> None:
    ...
    use_sdpa_fallback = flash_attn_3_func is None
    enable_cudnn_sdp(False)
    enable_flash_sdp(True)
    enable_mem_efficient_sdp(use_sdpa_fallback)
    enable_math_sdp(use_sdpa_fallback)
    logfile = None
    if master_process:
    ...
    base_model = GPT(
        ...
        mtp_init_from_main_head=args.mtp_init_from_main_head,
    ...
    log0(
        ...
        f"mtp_head_decay:{args.mtp_head_decay} mtp_init_from_main_head:{args.mtp_init_from_main_head} "
    ...
    log0(
        f"sdp_backends:cudnn=False flash=True mem_efficient={use_sdpa_fallback} math={use_sdpa_fallback} "
    ...
    eval_model = GPT(
        ...
        mtp_head_decay=args.mtp_head_decay, mtp_init_from_main_head=args.mtp_init_from_main_head,
