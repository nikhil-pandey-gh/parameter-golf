...
class Hyperparameters:
    ...
    shared_mlp_layers = os.environ.get("SHARED_MLP_LAYERS", "0,1,2")
    shared_mlp_adapter_rank = int(os.environ.get("SHARED_MLP_ADAPTER_RANK", 16))
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.fc(x)
        x = F.leaky_relu(x, negative_slope=self.negative_slope) if self.negative_slope > 0 else torch.relu(x)
...
class Block(nn.Module):
    def __init__(
        ...
        shared_mlp_adapter_rank: int = 0,
    ):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        self.mlp = MLP(dim, mlp_mult, negative_slope=mlp_negative_slope) if use_local_mlp else None
        self.shared_mlp_adapter = LowRankAdapter(dim, shared_mlp_adapter_rank) if shared_mlp_adapter_rank > 0 else None
    ...
    def forward(
        ...
        mlp_module = shared_mlp if shared_mlp is not None else self.mlp
        if mlp_module is None:
            raise RuntimeError("Block requires either a local MLP or a shared MLP override")
        mlp_out = mlp_module(mlp_in)
        if shared_mlp is not None and self.shared_mlp_adapter is not None:
            mlp_out = mlp_out + self.shared_mlp_adapter(mlp_in)
...
class GPT(nn.Module):
    def __init__(
        ...
        shared_layer_indices = sorted({int(x) for x in shared_mlp_layers.split(",") if x.strip()})
        self.shared_mlp_layer_indices = [i for i in shared_layer_indices if 0 <= i < num_layers]
        if len(self.shared_mlp_layer_indices) < 2:
            self.shared_mlp_layer_indices = []
        self.shared_mlp_layer_set = set(self.shared_mlp_layer_indices)
        self.shared_mlp = (
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    use_local_mlp=i not in self.shared_mlp_layer_set,
                    shared_mlp_adapter_rank=shared_mlp_adapter_rank if i in self.shared_mlp_layer_set else 0,
                )
    ...
    def _run_block(self, layer_idx: int, x: Tensor, x0: Tensor, input_ids: Tensor, ve_cache: dict) -> Tensor:
        ...
        shared_mlp = self.shared_mlp if layer_idx in self.shared_mlp_layer_set else None
        return self.blocks[layer_idx](x, x0, v_embed=ve, shared_mlp=shared_mlp)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        shared_mlp_layers=args.shared_mlp_layers,
        shared_mlp_adapter_rank=args.shared_mlp_adapter_rank,
    ...
    if base_model.shared_mlp is not None:
        matrix_params.extend([p for p in base_model.shared_mlp.parameters() if p.ndim == 2])
    ...
    log0(
        f"shared_mlp_layers:{base_model.shared_mlp_layer_indices} "
        f"shared_mlp_adapter_rank:{args.shared_mlp_adapter_rank} "
