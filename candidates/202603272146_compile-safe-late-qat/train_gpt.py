...
def find_repo_root() -> Path:
    for parent in (Path(__file__).resolve().parent, *Path(__file__).resolve().parents):
        if (parent / "data").is_dir() and (parent / "README.md").is_file():
            return parent
    raise FileNotFoundError("Could not locate repository root containing data/ and README.md")
...
class Hyperparameters:
    ...
    late_qat_eager = bool(int(os.environ.get("LATE_QAT_EAGER", "1")))
...
def build_training_model(base_model: nn.Module, distributed: bool, local_rank: int, enable_compile: bool) -> nn.Module:
    model: nn.Module = torch.compile(base_model, dynamic=False, fullgraph=True) if enable_compile else base_model
...
def main() -> None:
    ...
    training_compiled = True
    model: nn.Module = build_training_model(base_model, distributed, local_rank, enable_compile=training_compiled)
    ...
    log0(
        ...
        f"late_qat_eager:{args.late_qat_eager} "
        f"qat_enabled_start:{args.qat_enabled}"
    )
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            ...
            if args.late_qat_eager and training_compiled:
                if distributed:
                    dist.barrier()
                old_model = model
                model = build_training_model(base_model, distributed, local_rank, enable_compile=False)
                training_compiled = False
                del old_model
                torch.cuda.empty_cache()
                log0(f"late_qat:enabled step:{step} scale:{scale:.4f} mode:eager_tail")
            else:
                log0(f"late_qat:enabled step:{step} scale:{scale:.4f} mode:compiled")
        zero_grad_all()
