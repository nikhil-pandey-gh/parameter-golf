...
class Hyperparameters:
    ...
    mtp_distill_weight = float(os.environ.get("MTP_DISTILL_WEIGHT", 0.5))
    mtp_distill_temperature = float(os.environ.get("MTP_DISTILL_TEMPERATURE", 1.5))
...
class GPT(nn.Module):
    def __init__(
        ...
        mtp_distill_weight: float = 0.0,
        mtp_distill_temperature: float = 1.0,
        ...
        if not 0.0 <= mtp_distill_weight <= 1.0:
            raise ValueError(f"mtp_distill_weight must be in [0, 1], got {mtp_distill_weight}")
        if mtp_distill_temperature <= 0.0:
            raise ValueError(f"mtp_distill_temperature must be positive, got {mtp_distill_temperature}")
        ...
        self.mtp_distill_weight = mtp_distill_weight
        self.mtp_distill_temperature = mtp_distill_temperature
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        logits_seq = logits.reshape(input_ids.size(0), input_ids.size(1), -1)
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if self.mtp_distill_weight > 0.0:
                    teacher_logits = logits_seq[:, k + 1 :, :].detach().reshape(-1, logits_seq.size(-1))
                    temp = self.mtp_distill_temperature
                    distill_loss = F.kl_div(
                        F.log_softmax(mtp_logits.float() / temp, dim=-1),
                        F.softmax(teacher_logits.float() / temp, dim=-1),
                        reduction="batchmean",
                    ) * (temp * temp)
                    mtp_loss = (1.0 - self.mtp_distill_weight) * mtp_loss + self.mtp_distill_weight * distill_loss
...
def run_smoke_test() -> None:
    ...
    scalar_params = [model.smear.gate]
    if model.skip_weights.numel() > 0:
        scalar_params.append(model.skip_weights)
    tok_params: list[dict[str, object]] = []
    add_optional_adam_params(model, scalar_params, tok_params, token_lr=0.01)
    if not any(param is model.mtp_heads[0].weight for param in scalar_params):
        raise RuntimeError("MTP head weight was not added to the Adam parameter group")
    optimizer = torch.optim.AdamW([{"params": scalar_params, "lr": 0.01}], betas=(0.9, 0.95), eps=1e-8)
    ...
    loss.backward()
    optimizer.step()
    delta = (model.mtp_heads[0].weight.detach() - before).abs().max().item()
    if delta <= 0.0:
        raise RuntimeError("MTP head weight did not update during smoke test")
    print(f"smoke_test_loss:{loss.item():.6f} mtp_weight_delta:{delta:.6e}")
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mtp_distill_weight=args.mtp_distill_weight,
        mtp_distill_temperature=args.mtp_distill_temperature,
    ...
    add_optional_adam_params(base_model, scalar_params, tok_params, token_lr)
    ...
    log0(
        ...
        f"mtp_distill_weight:{args.mtp_distill_weight} mtp_distill_temperature:{args.mtp_distill_temperature} "
