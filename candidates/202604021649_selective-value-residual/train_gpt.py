...
def causal_attention(q: Tensor, k: Tensor, v: Tensor, enable_gqa: bool) -> Tensor:
    if _HAS_FLASH_ATTN_3 and q.is_cuda and k.is_cuda and v.is_cuda:
    ...
    if enable_gqa:
        ...
        # Keep the fallback broadly compatible across torch versions instead of relying
        # on the newer SDPA enable_gqa keyword.
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    value_residual=value_residual and (
                        i == 0 or (value_residual_last_n > 0 and i >= num_layers - value_residual_last_n)
                    ),
                )
...
def main() -> None:
    ...
    vr_source_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.value_residual and i == 0]
    vr_injection_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.value_residual and i != 0]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(
        f"value_residual:enabled={args.value_residual} last_n={args.value_residual_last_n} "
        f"source_layers:{vr_source_layers} injection_layers:{vr_injection_layers}"
    )
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(
        f"sdp_backends:cudnn=False flash={_HAS_FLASH_ATTN_3} "
