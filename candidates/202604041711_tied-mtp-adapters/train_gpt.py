...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if CastedLinear._qat_enabled and not getattr(self, "_skip_qat", False) and self.training and w.ndim == 2:
            with torch.no_grad():
...
class TiedMTPAdapter(nn.Module):
    """Horizon-specific low-rank adapter that reuses the main LM head."""
    def __init__(self, dim: int, rank: int):
        super().__init__()
        if rank <= 0:
            raise ValueError(f"mtp rank must be positive, got {rank}")
        self.offset = nn.Parameter(torch.zeros(dim, dtype=torch.float32))
        self.scale = nn.Parameter(torch.tensor(1.0, dtype=torch.float32))
        self.up = CastedLinear(dim, rank, bias=False)
        self.down = CastedLinear(rank, dim, bias=False)
        self.up._skip_qat = True
        self.down._skip_qat = True
        self.down._zero_init = True
    def forward(self, x: Tensor) -> Tensor:
        delta = self.down(F.silu(self.up(x)))
        y = x + self.offset.to(dtype=x.dtype)[None, None, :] + self.scale.to(dtype=x.dtype) * delta
...
class GPT(nn.Module):
    def __init__(
        ...
        self.enable_mtp = mtp_num_heads > 0 and mtp_loss_weight > 0.0
        ...
        self.mtp_adapters = nn.ModuleList(
            [TiedMTPAdapter(model_dim, mtp_rank) for _ in range(mtp_num_heads)]
        )
        if xsa_last_n > 0:
    ...
    def _init_weights(self) -> None:
        ...
        # Init remaining nn.Linear modules (bigram proj, mtp adapters, lm_head)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.enable_mtp and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
...
def main() -> None:
    ...
    # - bigram proj, low-rank tied MTP adapters, VE proj -> Adam (small matrix params not worth banking)
    matrix_params = [
    ...
    scalar_params.extend(base_model.mtp_adapters.parameters())
    ...
    log0(
        f"mtp_num_heads:{args.mtp_num_heads} mtp_rank:{args.mtp_rank} "
        f"mtp_loss_weight:{args.mtp_loss_weight} mtp_params:{mtp_params}"
    )
