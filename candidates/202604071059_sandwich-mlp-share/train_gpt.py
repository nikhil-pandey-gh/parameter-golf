...
class GPT(nn.Module):
    def __init__(
        ...
        if shared_mlp_layers < 0 or shared_mlp_layers > num_layers or shared_mlp_layers % 2 != 0:
            raise ValueError(
                f"SHARED_MLP_LAYERS must be an even integer in [0, {num_layers}], got {shared_mlp_layers}"
            )
        ...
        for i in range(1, shared_mlp_layers, 2):
            self.blocks[i].mlp = self.blocks[i - 1].mlp
        if rope_dims > 0:
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    return q, scale
def build_param_alias_map(state_dict: dict[str, Tensor]) -> dict[str, str]:
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        if name in alias_map:
            meta[name] = {"type": "alias", "target": alias_map[name]}
            continue
...
def main() -> None:
    ...
    log0(f"shared_mlp_layers:{args.shared_mlp_layers} shared_mlp_pairs:{args.shared_mlp_layers // 2}")
    ...
    alias_map = build_param_alias_map(export_sd)
    if alias_map:
        log0(f"shared_param_aliases:{len(alias_map)}")
    sd_cpu = {k: v.detach().cpu() for k, v in export_sd.items()}
    quant_result, quant_meta = mixed_quantize_int6(sd_cpu, {"mlp", "attn"}, alias_map)
