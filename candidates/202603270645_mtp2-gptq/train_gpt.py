...
class Hyperparameters:
    ...
    mtp_head_weights = os.environ.get("MTP_HEAD_WEIGHTS", "1.0,0.5")
...
class GPT(nn.Module):
    def __init__(
        ...
        parsed_mtp_head_weights = [float(x) for x in mtp_head_weights.split(",") if x.strip()]
        if not parsed_mtp_head_weights:
            parsed_mtp_head_weights = [1.0]
        while len(parsed_mtp_head_weights) < mtp_num_heads:
            parsed_mtp_head_weights.append(parsed_mtp_head_weights[-1] * 0.5)
        self.mtp_head_weights = parsed_mtp_head_weights[:mtp_num_heads]
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_head in enumerate(self.mtp_heads):
                ...
                if valid_t <= 0:
                    continue
                horizon_weight = self.mtp_head_weights[k] if k < len(self.mtp_head_weights) else self.mtp_head_weights[-1]
...
def main() -> None:
    ...
    log0(f"mtp_num_heads:{args.mtp_num_heads} mtp_loss_weight:{args.mtp_loss_weight} mtp_head_weights:{args.mtp_head_weights} mtp_params:{mtp_params}")
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0("sdp_backends:cudnn=False flash=True mem_efficient=False math=False")
    log0(f"attention_backend:{FLASH_ATTN_BACKEND}")
    ...
    eval_model = GPT(
        ...
        mtp_num_heads=0, mtp_loss_weight=0.0, mtp_head_weights=args.mtp_head_weights,
