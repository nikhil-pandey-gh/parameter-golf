...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        if flash_attn_3_func is not None and q.is_cuda:
            ...
            if self.num_kv_heads != self.num_heads:
                ...
                k_attn = k_attn.repeat_interleave(repeats, dim=1)
                v_attn = v_attn.repeat_interleave(repeats, dim=1)
            y = F.scaled_dot_product_attention(q_attn, k_attn, v_attn, attn_mask=None, is_causal=True).transpose(1, 2)
        if self.use_xsa:
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, mtp_loss_weight: Tensor | None = None) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0:
            ...
            mtp_weight = (
                torch.as_tensor(mtp_loss_weight, device=main_loss.device, dtype=main_loss.dtype)
                if mtp_loss_weight is not None
                else main_loss.new_tensor(self.mtp_loss_weight)
            )
...
def main() -> None:
    ...
    use_math_sdp = flash_attn_3_func is None
    ...
    log0(
        ...
        f"gated_attention:{args.gated_attention} value_residual:{args.value_residual} "
        f"mtp_weight_schedule:lr_scaled"
    )
    xsa_layers = [i for i, b in enumerate(base_model.blocks) if b.attn.use_xsa]
    log0(f"XSA:last_{args.xsa_last_n} active_layers:{xsa_layers}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    log0(f"sdp_backends:cudnn=False flash=True mem_efficient=False math={use_math_sdp}")
    ...
    while True:
        ...
        mtp_weight = torch.tensor(args.mtp_loss_weight * scale, device=device, dtype=torch.float32)
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
        ...
        if should_log_train:
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} mtp_w:{mtp_weight.item():.4f} "
