...
class Hyperparameters:
    ...
    outlier_row_frac = float(os.environ.get("OUTLIER_ROW_FRAC", 0.005))
...
def quantize_int6_per_row(
    t: Tensor,
    clip_range: int = 31,
    outlier_row_frac: float = 0.0,
) -> tuple[Tensor, Tensor, Tensor | None, Tensor | None]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_recon, best_err = None, None, None, float("inf")
        ...
        protect_idx = None
        protect_rows = None
        if outlier_row_frac > 0.0 and best_recon is not None:
            row_mse = (t32 - best_recon).pow(2).mean(dim=1)
            row_energy = t32.pow(2).mean(dim=1).clamp_min(1e-8)
            keep_rows = int(round(row_mse.numel() * outlier_row_frac))
            if keep_rows > 0:
                protect_idx = torch.topk(row_mse / row_energy, k=keep_rows, largest=True, sorted=False).indices
                protect_idx = protect_idx.sort().values.to(torch.int16)
                protect_rows = t32[protect_idx.long()].to(torch.float16).contiguous()
                best_q = best_q.clone()
                best_s = best_s.clone()
                best_q[protect_idx.long()] = 0
                best_s[protect_idx.long()] = 0
        return best_q, best_s, protect_idx, protect_rows
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            q, s, protect_idx, protect_rows = quantize_int6_per_row(
                t,
                outlier_row_frac=outlier_row_frac,
            )
            result[name + ".q"] = q
            result[name + ".scale"] = s
            info: dict[str, object] = {"type": "int6"}
            if protect_idx is not None and protect_rows is not None and protect_idx.numel() > 0:
                result[name + ".outlier_row_idx"] = protect_idx
                result[name + ".outlier_rows"] = protect_rows
                info["outlier_rows"] = int(protect_idx.numel())
            meta[name] = info
        else:
            q, s = quantize_float_tensor(t)
            result[name + ".q"] = q
...
def dequantize_mixed_int6(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if isinstance(info, dict) and info.get("type") == "int6" and name + ".outlier_row_idx" in result:
            outlier_idx = result[name + ".outlier_row_idx"].long()
            restored[outlier_idx] = result[name + ".outlier_rows"].to(restored.dtype)
...
def main() -> None:
    ...
    log0(f"quantization:int6_outlier_row_frac:{args.outlier_row_frac:.4f} late_qat_threshold:{args.late_qat_threshold:.4f}")
    ...
    quant_result, quant_meta = mixed_quantize_int6(
        sd_cpu,
        {"mlp", "attn"},
        outlier_row_frac=args.outlier_row_frac,
    )
    protected_rows = sum(
        int(info.get("outlier_rows", 0))
        for info in quant_meta.values()
        if isinstance(info, dict)
    )
    if master_process:
        log0(f"outlier_rows:protected:{protected_rows}")
