...
class TeacherForcedMTPHead(nn.Module):
    """Training-only multi-token head conditioned on the teacher-forced previous future token."""

    def __init__(self, model_dim: int, vocab_size: int):
        super().__init__()
        self.norm = RMSNorm()
        self.adapter = CastedLinear(model_dim, model_dim, bias=False)
        self.adapter._zero_init = True
        self.teacher_scale = nn.Parameter(torch.tensor(0.5, dtype=torch.float32))
        self.resid_scale = nn.Parameter(torch.tensor(0.25, dtype=torch.float32))
        self.head = CastedLinear(model_dim, vocab_size, bias=False)

    def forward(self, hidden: Tensor, teacher_emb: Tensor, logit_softcap: float) -> tuple[Tensor, Tensor]:
        conditioned = hidden + self.teacher_scale.to(dtype=hidden.dtype) * teacher_emb
        adapted = conditioned + self.resid_scale.to(dtype=hidden.dtype) * self.adapter(self.norm(conditioned))
        logits_proj = self.head(adapted.reshape(-1, adapted.size(-1)))
...
class GPT(nn.Module):
    def __init__(
        ...
        self.mtp_modules = nn.ModuleList(
            [TeacherForcedMTPHead(model_dim, vocab_size) for _ in range(mtp_num_heads)]
        )
        if xsa_last_n > 0:
    ...
    def _init_weights(self) -> None:
        ...
        # Init remaining nn.Linear modules (bigram proj, training-only MTP heads, lm_head)
        ...
        if self.tie_embeddings:
            for mtp_module in self.mtp_modules:
                mtp_module.head.weight.data.copy_(self.tok_emb.weight.data)
        elif self.lm_head is not None:
            for mtp_module in self.mtp_modules:
                mtp_module.head.weight.data.copy_(self.lm_head.weight.data)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            ...
            for k, mtp_module in enumerate(self.mtp_modules):
                ...
                if valid_t <= 0:
                    continue
                mtp_state = mtp_state[:, :valid_t, :]
                teacher_tokens = target_ids[:, k : k + valid_t]
                teacher_emb = F.rms_norm(self.tok_emb(teacher_tokens), (dim,))
                mtp_state, mtp_logits = mtp_module(mtp_state, teacher_emb, self.logit_softcap)
                mtp_targets = target_ids[:, k + 1 : k + 1 + valid_t].reshape(-1)
...
def main() -> None:
    ...
    # - scalars/control tensors and small training-only auxiliaries -> Adam
    # - bigram proj, MTP modules, VE proj -> Adam (small matrix params not worth banking)
    matrix_params = [
    ...
    if base_model.mtp_num_heads > 0:
        scalar_params.extend(list(base_model.mtp_modules.parameters()))
    ...
    mtp_params = sum(p.numel() for p in base_model.mtp_modules.parameters())
