...
class Hyperparameters:
    ...
    qk_head_dim = int(os.environ.get("QK_HEAD_DIM", 16))
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        self.qk_head_dim = qk_head_dim if qk_head_dim > 0 else self.head_dim
        if self.head_dim % 2 != 0:
            raise ValueError("head_dim must be even for RoPE")
        if self.qk_head_dim > self.head_dim:
            raise ValueError("qk_head_dim must be <= head_dim")
        if self.qk_head_dim % 2 != 0:
            raise ValueError("qk_head_dim must be even for RoPE")
        q_dim = self.num_heads * self.qk_head_dim
        kv_qk_dim = self.num_kv_heads * self.qk_head_dim
        kv_dim = self.num_kv_heads * self.head_dim
        self.c_q = CastedLinear(dim, q_dim, bias=False)
        self.c_k = CastedLinear(dim, kv_qk_dim, bias=False)
        self.c_v = CastedLinear(dim, kv_dim, bias=False)
        self.proj = CastedLinear(dim, dim, bias=False)
        self.proj._zero_init = True
        self.q_gain = nn.Parameter(torch.full((num_heads,), qk_gain_init, dtype=torch.float32))
        self.rope_dims = 0  # set by GPT.__init__ for partial RoPE
        self.rotary = Rotary(self.qk_head_dim, base=rope_base, train_seq_len=1024)
    ...
    def forward(self, x: Tensor, v_embed: Tensor | None = None) -> Tensor:
        ...
        q = self.c_q(x).reshape(bsz, seqlen, self.num_heads, self.qk_head_dim)
        k = self.c_k(x).reshape(bsz, seqlen, self.num_kv_heads, self.qk_head_dim)
        v = self.c_v(x)
        ...
        if flash_attn_3_func is not None and self.qk_head_dim == self.head_dim:
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, qk_head_dim, rope_base, qk_gain_init)
    ...
    def __init__(
        ...
        if rope_dims > 0:
            for block in self.blocks:
                block.attn.rope_dims = min(rope_dims, block.attn.qk_head_dim)
                block.attn.rotary = Rotary(
                    block.attn.qk_head_dim,
                    base=rope_base,
                    train_seq_len=1024,
                    rope_dims=block.attn.rope_dims,
                )
...
def main() -> None:
    ...
    thin_qk_active = args.qk_head_dim > 0 and args.qk_head_dim != args.model_dim // args.num_heads
    enable_cudnn_sdp(False)
    enable_flash_sdp(True)
    enable_mem_efficient_sdp(thin_qk_active)
    enable_math_sdp(thin_qk_active)
    logfile = None
    if master_process:
    ...
    log0(f"sdp_backends:cudnn=False flash=True mem_efficient={thin_qk_active} math={thin_qk_active}")
    log0(
        f"attention_mode:gqa num_heads:{args.num_heads} "
        f"num_kv_heads:{args.num_kv_heads} qk_head_dim:{args.qk_head_dim}"
    )
    log0(
        "attention_backend:"
        + (
            "flash_attn_3"
            if flash_attn_3_func is not None and args.qk_head_dim == args.model_dim // args.num_heads
            else "torch_sdpa"
        )
    )
    log0(
    ...
    eval_model = GPT(
        ...
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, qk_head_dim=args.qk_head_dim, mlp_mult=args.mlp_mult,
