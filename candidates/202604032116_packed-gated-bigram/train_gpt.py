...
# --- Packed mixed low-bit quantization ---
...
LOWBIT_ROW_CLIP_PCTS = (0.9990, 0.9995, 0.9999, 0.99999, 1.0)
LOWBIT_PACK_CHUNK = 1_000_000  # Keeps both 5-bit and 6-bit chunks byte-aligned.
LOWBIT_SPECS = {
    "mlp": {"bits": 5, "clip_range": 15},
    "attn": {"bits": 6, "clip_range": 31},
}


def quantize_lowbit_per_row(t: Tensor, clip_range: int) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        best_q, best_s, best_err = None, None, float('inf')
        for pct in LOWBIT_ROW_CLIP_PCTS:
            if pct < 1.0:
...
def _pack_uint_values(values: np.ndarray, bits: int) -> np.ndarray:
    if values.ndim != 1:
    ...
    for start in range(0, values.size, LOWBIT_PACK_CHUNK):
        chunk = values[start : start + LOWBIT_PACK_CHUNK].astype(np.uint8, copy=False)
        bitplanes = ((chunk[:, None] >> shifts[None, :]) & 1).astype(np.uint8, copy=False)
        packed_chunks.append(np.packbits(bitplanes.reshape(-1), bitorder="little"))
    return np.concatenate(packed_chunks) if len(packed_chunks) > 1 else packed_chunks[0]


def _unpack_uint_values(packed: np.ndarray, count: int, bits: int) -> np.ndarray:
    if count == 0:
        return np.empty((0,), dtype=np.uint8)
    raw_bits = np.unpackbits(packed, bitorder="little")[: count * bits].reshape(count, bits)
    weights = (1 << np.arange(bits, dtype=np.uint32)).reshape(1, bits)
    return (raw_bits.astype(np.uint32) * weights).sum(axis=1).astype(np.uint8, copy=False)


def pack_lowbit_tensor(q: Tensor, bits: int, clip_range: int) -> Tensor:
    shifted = (q.to(torch.int16).reshape(-1) + clip_range).cpu().numpy().astype(np.uint8, copy=False)
    return torch.from_numpy(_pack_uint_values(shifted, bits).copy())


def unpack_lowbit_tensor(packed: Tensor, shape: tuple[int, ...], bits: int, clip_range: int) -> Tensor:
    flat = _unpack_uint_values(packed.cpu().numpy(), math.prod(shape), bits)
    signed = flat.astype(np.int16, copy=False) - clip_range
    return torch.from_numpy(signed.reshape(shape))


def encode_scale_tensor(scale: Tensor) -> tuple[dict[str, Tensor], str]:
    s = scale.detach().cpu().contiguous()
    if s.ndim == 0:
        return {"scale": s.to(torch.float32).reshape(())}, "scalar_fp32"
    if s.numel() < 256:
        return {"scale": s.to(torch.float16)}, "fp16"
    log_s = s.float().clamp_min(1e-8).log()
    lo = log_s.min()
    hi = log_s.max()
    if (not torch.isfinite(lo)) or (not torch.isfinite(hi)) or float((hi - lo).abs().item()) < 1e-6:
        return {"scale": s.to(torch.float16)}, "fp16"
    step = (hi - lo) / 255.0
    q = torch.clamp(torch.round((log_s - lo) / step), 0, 255).to(torch.uint8).contiguous()
    return {
        "scale_q": q,
        "scale_lo": lo.to(torch.float32).reshape(()),
        "scale_step": step.to(torch.float32).reshape(()),
    }, "log_uint8"


def decode_scale_tensor(result: dict[str, Tensor], name: str, scale_kind: str) -> Tensor:
    if scale_kind in {"scalar_fp32", "fp16"}:
        return result[name + ".scale"].float()
    if scale_kind != "log_uint8":
        raise ValueError(f"Unknown scale encoding: {scale_kind}")
    q = result[name + ".scale_q"].float()
    lo = float(result[name + ".scale_lo"].item())
    step = float(result[name + ".scale_step"].item())
    return torch.exp(q * step + lo)
...
def mixed_quantize_packed(state_dict: dict[str, Tensor]):
    ...
    for name, tensor in state_dict.items():
        ...
        if any(p in name for p in CONTROL_TENSOR_NAME_PATTERNS):
            ...
            continue
        spec = LOWBIT_SPECS.get(cat)
        if spec is not None and t.ndim >= 1:
            q, s = quantize_lowbit_per_row(t, clip_range=spec["clip_range"])
            result[name + ".packed"] = pack_lowbit_tensor(q, bits=spec["bits"], clip_range=spec["clip_range"])
            scale_payload, scale_kind = encode_scale_tensor(s)
            for suffix, payload in scale_payload.items():
                result[f"{name}.{suffix}"] = payload
            meta[name] = {
                "type": "packed",
                "bits": spec["bits"],
            ...
            for suffix, payload in scale_payload.items():
                result[f"{name}.{suffix}"] = payload
            meta[name] = {"type": "int8", "scale_kind": scale_kind}
    return result, meta


def dequantize_mixed_packed(result: dict[str, Tensor], meta: dict[str, object],
    ...
    for name, orig in template_sd.items():
        ...
        if info in ("passthrough", "passthrough_ctrl", "passthrough_fp16"):
            ...
            out[name] = t
            continue
        s = decode_scale_tensor(result, name, info["scale_kind"])
        if info["type"] == "packed":
            q = unpack_lowbit_tensor(
                result[name + ".packed"],
                tuple(info["shape"]),
                bits=int(info["bits"]),
                clip_range=int(info["clip_range"]),
            )
        else:
            q = result[name + ".q"]
        if s.ndim > 0:
            out[name] = (q.float() * s.view(q.shape[0], *([1] * (q.ndim - 1)))).to(orig_dtype)
        else:
...
def main() -> None:
    ...
    quant_result, quant_meta = mixed_quantize_packed(unbanked_sd)
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
    quant_raw = quant_buf.getvalue()
    quant_blob = lzma.compress(quant_raw, preset=6)
    if master_process:
        with open("final_model.packed.ptz", "wb") as f:
            f.write(quant_blob)
        quant_file_bytes = len(quant_blob)
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model packed+lzma: {quant_file_bytes} bytes")
        log0(f"Total submission size packed+lzma: {quant_file_bytes + code_bytes} bytes")
    if distributed:
        dist.barrier()
    with open("final_model.packed.ptz", "rb") as f:
        quant_blob_disk = f.read()
    quant_state = torch.load(
        io.BytesIO(lzma.decompress(quant_blob_disk)),
        map_location="cpu",
    )
    deq_unbanked = dequantize_mixed_packed(quant_state["w"], quant_state["m"], unbanked_sd)
    ...
    torch.cuda.synchronize()
    log0(
        f"final_packed_roundtrip val_loss:{q_val_loss:.4f} val_bpb:{q_val_bpb:.4f} "
        f"eval_time:{1000.0 * (time.perf_counter() - t_qeval):.0f}ms"
    )
    log0(f"final_packed_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_packed_sliding_window val_loss:{sw_val_loss:.4f} val_bpb:{sw_val_bpb:.4f} "
            f"stride:{args.eval_stride} eval_time:{1000.0 * (time.perf_counter() - t_slide):.0f}ms"
        )
        log0(f"final_packed_sliding_window_exact val_loss:{sw_val_loss:.8f} val_bpb:{sw_val_bpb:.8f}")
    ...
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_packed_sliding_window_s64 val_loss:{sw64_val_loss:.4f} val_bpb:{sw64_val_bpb:.4f} "
            f"stride:64 eval_time:{1000.0 * (time.perf_counter() - t_slide64):.0f}ms"
        )
        log0(f"final_packed_sliding_window_s64_exact val_loss:{sw64_val_loss:.8f} val_bpb:{sw64_val_bpb:.8f}")
