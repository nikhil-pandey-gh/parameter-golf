...
class Hyperparameters:
    ...
    bank_qat_eager = bool(int(os.environ.get("BANK_QAT_EAGER", "1")))
    mixed_int8_mlp_down_last_n = int(os.environ.get("MIXED_INT8_MLP_DOWN_LAST_N", 4))
    mixed_int8_attn_out_last_n = int(os.environ.get("MIXED_INT8_ATTN_OUT_LAST_N", 0))
...
def attention_or_sdpa(q: Tensor, k: Tensor, v: Tensor) -> Tensor:
    ...
    if k_sdpa.size(1) != q_sdpa.size(1):
        repeat = q_sdpa.size(1) // k_sdpa.size(1)
        k_sdpa = k_sdpa.repeat_interleave(repeat, dim=1)
        v_sdpa = v_sdpa.repeat_interleave(repeat, dim=1)
    y = F.scaled_dot_product_attention(q_sdpa, k_sdpa, v_sdpa, dropout_p=0.0, is_causal=True)
...
def fake_quantize_per_row(weight: Tensor, clip_range: int) -> Tensor:
    w32 = weight.float()
    if w32.ndim != 2:
        amax = w32.abs().amax().clamp_min(1.0 / clip_range)
        scale = amax / clip_range
        clipped = torch.clamp(w32, -amax, amax)
        dequant = torch.clamp(torch.round(clipped / scale), -clip_range, clip_range) * scale
        return weight + (dequant.to(dtype=weight.dtype) - weight).detach()
    row_clip = w32.abs().amax(dim=1).clamp_min(1.0 / clip_range)
    scale = row_clip / clip_range
    clipped = torch.maximum(torch.minimum(w32, row_clip[:, None]), -row_clip[:, None])
    dequant = torch.clamp(torch.round(clipped / scale[:, None]), -clip_range, clip_range) * scale[:, None]
    return weight + (dequant.to(dtype=weight.dtype) - weight).detach()
...
class GPT(nn.Module):
    def __init__(
        ...
        gated_attention: bool = False,
        value_residual: bool = False,
        mixed_int8_mlp_down_last_n: int = 0,
        mixed_int8_attn_out_last_n: int = 0,
    ):
        super().__init__()
        ...
        self.bank_qat_mlp_down_last_n = max(int(mixed_int8_mlp_down_last_n), 0)
        self.bank_qat_attn_out_last_n = max(int(mixed_int8_attn_out_last_n), 0)
        self.bank_qat_mlp_down_layers = set(
            range(max(num_layers - self.bank_qat_mlp_down_last_n, 0), num_layers)
        )
        self.bank_qat_attn_out_layers = set(
            range(max(num_layers - self.bank_qat_attn_out_last_n, 0), num_layers)
        )
    ...
    def _bank_clip_range(self, bank_kind: str, layer_idx: int) -> int:
        if bank_kind == "mlp_down" and layer_idx in self.bank_qat_mlp_down_layers:
            return 127
        if bank_kind == "attn_out" and layer_idx in self.bank_qat_attn_out_layers:
            return 127
        return 31

    def _bank_weight(self, bank_kind: str, layer_idx: int, weight: Tensor) -> Tensor:
        if not (self.bank_qat_active and self.training):
            return weight
        return fake_quantize_per_row(weight, self._bank_clip_range(bank_kind, layer_idx))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x, raw_v = self.blocks[i](x, x0,
                self._bank_weight("attn_q", i, self.qo_bank[i]),
                self._bank_weight("attn_k", i, self.kv_bank[i]),
                self._bank_weight("attn_v", i, self.kv_bank[n + i]),
                self._bank_weight("attn_out", i, self.qo_bank[n + i]),
                self._bank_weight("mlp_up", i, self.mlp_up_bank[i]),
                self._bank_weight("mlp_down", i, self.mlp_down_bank[i]),
                v_embed=ve, v0=v0)
        ...
        for i in range(self.num_decoder_layers):
            ...
            x, _ = self.blocks[bi](x, x0,
                self._bank_weight("attn_q", bi, self.qo_bank[bi]),
                self._bank_weight("attn_k", bi, self.kv_bank[bi]),
                self._bank_weight("attn_v", bi, self.kv_bank[n + bi]),
                self._bank_weight("attn_out", bi, self.qo_bank[n + bi]),
                self._bank_weight("mlp_up", bi, self.mlp_up_bank[bi]),
                self._bank_weight("mlp_down", bi, self.mlp_down_bank[bi]),
                v_embed=ve, v0=v0)
        x = self.final_norm(x)
...
def mixed_quantize_int6(
    ...
    for name, tensor in state_dict.items():
        ...
        layer_idx = _parse_block_index(name)
        ...
        if layer_idx is not None:
            if name.endswith(".mlp.proj.weight") and layer_idx >= max(num_layers_total - mixed_int8_mlp_down_last_n, 0):
                use_int8 = True
            if name.endswith(".attn.proj.weight") and layer_idx >= max(num_layers_total - mixed_int8_attn_out_last_n, 0):
                use_int8 = True
        if cat in int6_cats and t.ndim >= 1 and not use_int8:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        mixed_int8_mlp_down_last_n=args.mixed_int8_mlp_down_last_n,
        mixed_int8_attn_out_last_n=args.mixed_int8_attn_out_last_n,
    ).to(device).bfloat16()
    ...
    log0(
        f"bank_qat_enabled:{args.bank_qat_enabled} bank_qat_eager:{args.bank_qat_eager} "
        f"mixed_int8_mlp_down_last_n:{args.mixed_int8_mlp_down_last_n} "
        f"mixed_int8_attn_out_last_n:{args.mixed_int8_attn_out_last_n}"
    )
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold:
            ...
            if args.bank_qat_enabled and not base_model.bank_qat_active:
                ...
                log0(
                    ...
                    f"castedlinear_qat:{int(castedlinear_just_enabled)} bank_qat:1 mode:{mode} "
                    f"mlp_down_last_n:{args.mixed_int8_mlp_down_last_n} "
                    f"attn_out_last_n:{args.mixed_int8_attn_out_last_n}"
                )
            elif castedlinear_just_enabled:
                log0(f"late_qat:enabled step:{step} scale:{scale:.4f} bank_qat:0")
        zero_grad_all()
