...
class Hyperparameters:
    ...
    physical_layers = int(os.environ.get("PHYSICAL_LAYERS", 6))
    ...
    bank_share_scheme = os.environ.get("BANK_SHARE_SCHEME", "mirror")
...
def build_layer_bank_indices(num_layers: int, physical_layers: int, scheme: str) -> list[int]:
    if physical_layers <= 0:
        raise ValueError(f"PHYSICAL_LAYERS must be positive, got {physical_layers}")
    if physical_layers > num_layers:
        raise ValueError(
            f"PHYSICAL_LAYERS={physical_layers} cannot exceed NUM_LAYERS={num_layers}"
        )
    if physical_layers == num_layers or scheme == "none":
        if physical_layers != num_layers:
            raise ValueError(
                "BANK_SHARE_SCHEME=none requires PHYSICAL_LAYERS to match NUM_LAYERS"
            )
        return list(range(num_layers))
    if scheme == "mirror":
        required = (num_layers + 1) // 2
        if physical_layers != required:
            raise ValueError(
                "BANK_SHARE_SCHEME=mirror requires "
                f"PHYSICAL_LAYERS=(NUM_LAYERS+1)//2={required}, got {physical_layers}"
            )
        return [min(i, num_layers - 1 - i) for i in range(num_layers)]
    if scheme == "cycle":
        return [i % physical_layers for i in range(num_layers)]
    raise ValueError(f"Unknown BANK_SHARE_SCHEME={scheme!r}")
...
class GPT(nn.Module):
    def __init__(
        ...
        self.bank_share_scheme = bank_share_scheme
        self.layer_bank_indices = build_layer_bank_indices(num_layers, physical_layers, bank_share_scheme)
        # Parameter banks: contiguous 3D tensors for batched optimizer
        head_dim = model_dim // num_heads
        kv_dim = num_kv_heads * head_dim
        mlp_dim = int(mlp_mult * model_dim)
        self.qo_bank = nn.Parameter(torch.empty(2 * physical_layers, model_dim, model_dim))
        self.kv_bank = nn.Parameter(torch.empty(2 * physical_layers, kv_dim, model_dim))
        self.mlp_up_bank = nn.Parameter(torch.empty(physical_layers, mlp_dim, model_dim))
        self.mlp_down_bank = nn.Parameter(torch.empty(physical_layers, model_dim, mlp_dim))
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            bank_i = self.layer_bank_indices[i]
        ...
        for i in range(self.num_decoder_layers):
            ...
            bank_i = self.layer_bank_indices[bi]
            if skips:
...
def quantize_int6_per_row(t: Tensor, clip_range: int = 31) -> tuple[Tensor, Tensor]:
    ...
    if t32.ndim >= 2:
        ...
        for pct in [0.9990, 0.9995, 0.9999, 0.99999, 1.0]:
            ...
            q_flat = torch.clamp(torch.round(flat / s.float()[:, None]), -clip_range, clip_range).to(torch.int8)
            recon = q_flat.float() * s.float().unsqueeze(-1)
            err = (flat - recon).pow(2).mean().item()
            if err < best_err:
                best_q, best_s, best_err = q_flat, s, err
        return best_q.reshape(t32.shape), best_s.reshape(t32.shape[:-1])
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        bank_share_scheme=args.bank_share_scheme,
    ...
    log0(
        "bank_sharing:"
        f" scheme={args.bank_share_scheme}"
        f" physical_layers={args.physical_layers}"
        f" logical_layers={args.num_layers}"
        f" layer_to_bank={base_model.layer_bank_indices}"
    )
