...
class Hyperparameters:
    ...
    late_qat_recompile = bool(int(os.environ.get("LATE_QAT_RECOMPILE", "1")))
...
def fake_quantize_int6_ste(t: Tensor, percentile: float = 1.0, clip_range: int = INT6_CLIP_RANGE) -> Tensor:
    t32 = t.float()
    if t32.ndim == 2:
        if percentile < 1.0:
...
class CausalSelfAttention(nn.Module):
    ...
    def _maybe_fake_quantize_bank(self, w: Tensor) -> Tensor:
        if self.training and self.bank_qat_enabled:
            return fake_quantize_int6_ste(w, percentile=self.bank_qat_percentile)
        return w
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        q_w = self._maybe_fake_quantize_bank(q_w)
        k_w = self._maybe_fake_quantize_bank(k_w)
        v_w = self._maybe_fake_quantize_bank(v_w)
        out_w = self._maybe_fake_quantize_bank(out_w)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        up_w = self._maybe_fake_quantize_bank(up_w)
        down_w = self._maybe_fake_quantize_bank(down_w)
...
class GPT(nn.Module):
    ...
    def set_bank_qat(self, enabled: bool, percentile: float = 1.0) -> None:
        self.bank_qat_enabled = enabled
        self.bank_qat_percentile = percentile
        for block in self.blocks:
            block.attn.bank_qat_enabled = enabled
            block.attn.bank_qat_percentile = percentile
            block.mlp.bank_qat_enabled = enabled
            block.mlp.bank_qat_percentile = percentile
...
def main() -> None:
    ...
    args = Hyperparameters()
    if args.bank_qat_percentile != 1.0:
        raise ValueError("BANK_QAT_PERCENTILE must be 1.0 for the compiled fullgraph bank-QAT path")
    ...
    base_model.set_bank_qat(False, args.bank_qat_percentile)
    ...
    while True:
        ...
        late_qat_reached = args.late_qat_threshold > 0 and scale < args.late_qat_threshold
        legacy_qat_changed = False
        bank_qat_changed = False
        if late_qat_reached and not CastedLinear._qat_enabled:
            CastedLinear._qat_enabled = True
            legacy_qat_changed = True
        if late_qat_reached and args.bank_qat_enabled and not base_model.bank_qat_enabled:
            base_model.set_bank_qat(True, args.bank_qat_percentile)
            bank_qat_changed = True
        if legacy_qat_changed or bank_qat_changed:
            should_recompile = args.late_qat_recompile or bank_qat_changed
            if should_recompile:
                compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
                model = compiled_model
            log0(
                f"late_qat:enabled step:{step} scale:{scale:.4f} "
                f"legacy_qat:{int(CastedLinear._qat_enabled)} "
                f"bank_qat:{int(base_model.bank_qat_enabled)} pct:{args.bank_qat_percentile:.4f} "
                f"recompile:{int(should_recompile)}"
            )
        zero_grad_all()
